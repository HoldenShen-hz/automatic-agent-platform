...
describe("useSettingsVm", () => {
    ...
    it("keeps the saving state visible before flipping to saved and refreshes drafts from upstream preferences", async () => {
        ...
        mocks.mockUpdatePreferences.mockImplementationOnce(() => new Promise((resolve) => setTimeout(() => resolve({ ok: true }), 20)));
        act(() => {
        ...
        expect(mocks.mockUpdatePreferences).toHaveBeenCalledWith(mocks.mockClient, { theme: "light", locale: "zh-CN" }, "etag-1");
        preferencesData = {
            theme: "high-contrast",
            locale: "en-US",
            etag: "etag-2",
