...
describe("useHitlVm", () => {
    ...
    it("submits patch actions through the approval decision API and removes the item", async () => {
        ...
        expect(mocks.mockSubmitApprovalTextInput).toHaveBeenCalledWith(mocks.mockClient, "approval-1", JSON.stringify({ action: "patch", patch: { field: "value" } }));
        await waitFor(() => {
    ...
    it("submits override actions through the approval decision API and removes the item", async () => {
        ...
        expect(mocks.mockSubmitApprovalTextInput).toHaveBeenCalledWith(mocks.mockClient, "approval-2", JSON.stringify({ action: "override", override: { mode: "full" } }));
        await waitFor(() => {
