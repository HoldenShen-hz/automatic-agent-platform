...
describe("PolicyWebView empty state", () => {
  it("shows backend-empty messaging in both list and detail panes", () => {
    render(<PolicyWebView />);

    expect(screen.getAllByText("No governance policies published by the backend.")).toHaveLength(2);
    expect(screen.queryByText("No policy selected")).toBeNull();
  });
});
