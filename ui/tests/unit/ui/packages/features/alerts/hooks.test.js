...
const mockClient = {};
const mutationCalls = [];
const createMutation = (path) => {
    const mutateAsync = vi.fn(async (payload) => {
...
vi.mock("@aa/shared-state", () => ({
    ...
    useMutation: ({ path }) => createMutation(path),
    useWsClient: () => ({
        subscribe: () => () => undefined,
...
describe("useAlertsVm", () => {
    ...
    it("sorts incidents by severity and applies severity filters in the mapped vm", () => {
        ...
        const allVm = mapAlertsToVm(incidents, { severity: "all", domain: "all", timeRange: "all" }, [], "idle", 0, {
            onAcknowledge: vi.fn(),
            onDismiss: vi.fn(),
            onEscalate: vi.fn(),
            onSnooze: vi.fn(),
        ...
        const filteredVm = mapAlertsToVm(incidents, { severity: "critical", domain: "all", timeRange: "all" }, [], "idle", 0, {
            onAcknowledge: vi.fn(),
            onDismiss: vi.fn(),
            onEscalate: vi.fn(),
            onSnooze: vi.fn(),
