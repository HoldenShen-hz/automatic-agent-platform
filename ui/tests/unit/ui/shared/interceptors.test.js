...
import { DEFAULT_ACCEPT_VERSIONS, createTraceInterceptor, createContractVersionInterceptor, createAuthInterceptor, createTenantInterceptor, createIdempotencyKeyInterceptor, createCsrfInterceptor, createRetryInterceptor, createDedupeInterceptor, } from "@aa/shared-api-client";
function createMockRequest(method = "GET", extraHeaders = {}) {
...
async function withCsrfMetaToken(token, run) {
...
describe("createAuthInterceptor - Token Refresh (Issue #2071)", () => {
    it("rejects static bearer token strings and requires a dynamic resolver", async () => {
        ...
        const request = createMockRequest();
        await expect(interceptor.onRequest(request)).rejects.toThrow("auth.dynamic_token_required:Static bearer tokens are not supported");
    });
...
describe("createDedupeInterceptor", () => {
    ...
    it("differentiates by method", async () => {
        ...
        expect(result1.dedupeKey).toBeUndefined();
        expect(result2.dedupeKey).toBeDefined();
        expect(result2.dedupeKey).toContain("POST:/api/v1/test");
    });
});
