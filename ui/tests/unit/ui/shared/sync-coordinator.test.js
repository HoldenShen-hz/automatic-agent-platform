...
describe("SyncCoordinator", () => {
    it("flush dispatches queued mutations before draining the queue", async () => {
        ...
        const coordinator = new SyncCoordinator(queue, new ConflictResolver(), { dispatch });
    ...
    it("replays stored auth and idempotency headers during flush", async () => {
        ...
        await queue.whenReady();
        const coordinator = new SyncCoordinator(queue, new ConflictResolver(), new FetchSyncMutationDispatcher(fetchImplementation));
