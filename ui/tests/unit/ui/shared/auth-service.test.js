...
describe("AuthService", () => {
    ...
    it("handleAuthorizationCallback fails closed when no token exchange handler is configured", async () => {
        ...
        const authUrl = await authService.initiateCodeFlow("https://app.example.com/auth/callback");
        const state = new URLSearchParams(authUrl.split("?")[1]).get("state");
        await expect(authService.handleAuthorizationCallback(new URLSearchParams(`state=${state}&code=valid-auth-code`))).rejects.toThrow(/auth.token_exchange_not_configured/);
        expect(authService.getSession()).toBeNull();
    });
    ...
    it("refreshes only once for concurrent callers when token is inside the 60s window", async () => {
        const refreshFn = vi.fn(async (refreshToken) => ({
