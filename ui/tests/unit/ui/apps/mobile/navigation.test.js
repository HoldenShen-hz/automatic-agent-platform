...
import { mobileNavigation, modalNavigator, resolveMobileScreen, rootTabNavigator, settingsStack, settingsSubRoutes, } from "../../../../../apps/mobile/src/navigation";
describe("mobileNavigation", () => {
    ...
    describe("modalFlows configuration", () => {
        ...
        it("has approval-detail modal flow", () => {
            const approvalDetail = mobileNavigation.modalFlows.find((m) => m.id === "approval-detail");
        ...
        it("modal flows have dynamic path parameters", () => {
            const hasDynamicPath = mobileNavigation.modalFlows.some((m) => m.path.includes(":"));
...
describe("navigation path structure", () => {
    it("tabs use mission-control paths", () => {
        const missionControlTabs = mobileNavigation.tabs.filter((t) => t.path.startsWith("/mission-control"));
        expect(missionControlTabs.length).toBeGreaterThan(0);
    });
    it("tabs use shared paths for common features", () => {
        const sharedTabs = mobileNavigation.tabs.filter((t) => t.path.startsWith("/shared"));
        expect(sharedTabs.length).toBeGreaterThan(0);
    });
    it("extended features use /extended prefix", () => {
        const extendedFlows = mobileNavigation.modalFlows.filter((m) => m.path.startsWith("/extended"));
