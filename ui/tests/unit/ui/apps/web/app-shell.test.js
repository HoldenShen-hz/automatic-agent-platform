...
function createMockFeature(overrides = {}) {
    return {
        ...
        Component: () => _jsx("div", { "data-testid": "mock-component", children: "Mock Feature Content" }),
        ...overrides,
    };
}
function createMockSubPage(id, label) {
    return {
        id,
        path: id,
        label,
        Component: () => _jsx("div", { "data-testid": `subpage-${id}`, children: label }),
    };
}
function renderShell(features, options) {
    return render(_jsx(WebAppShell, { features: features, router: options?.router ?? "memory", ...(options?.initialEntries == null ? {} : { initialEntries: options.initialEntries }), ...(options?.authContext == null ? {} : { authContext: options.authContext }) }));
}
describe("WebAppShell", () => {
    ...
    it("passes auth context into feature guard evaluation", () => {
        ...
        expect(sharedDomainMocks.createFeatureGuardContext).toHaveBeenCalledWith(expect.objectContaining({
            authenticated: true,
            tenantId: "tenant-abc",
            permissions: ["read", "write"],
            roles: ["operator"],
    ...
    it("renders the first feature for unknown routes", () => {
        const firstFeature = createMockFeature({
            ...
            Component: () => _jsx("div", { "data-testid": "first-feature-body", children: "First Feature" }),
        });
        const secondFeature = createMockFeature({
            ...
            Component: () => _jsx("div", { children: "Second Feature" }),
        });
