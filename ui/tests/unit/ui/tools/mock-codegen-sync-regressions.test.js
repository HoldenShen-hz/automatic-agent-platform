...
describe("UI tooling regressions", () => {
    it("mock server resolves extended API surfaces beyond dashboard/tasks/workflows", () => {
        expect(resolveMockRequest("/api/v1/approvals")).toEqual(expect.arrayContaining([expect.objectContaining({ approvalId: "approval-1", riskLevel: "critical" })]));
        expect(resolveMockRequest("/api/v1/meta/contract-version")).toEqual(expect.objectContaining({ contractVersion: "2026-04-01" }));
    });
    ...
    it("conflict resolver supports CRDT-style merge semantics", () => {
        ...
        const resolved = resolver.resolve({ id: "doc", version: 1, status: "server" }, { id: "doc", version: 2, status: "local" }, "merge", {
            lamportTimestamp: 1,
