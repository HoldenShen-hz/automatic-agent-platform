...
describe("offline recovery sync flow", () => {
    it("rehydrates queued mutations, flushes them after reconnect, and preserves conflict resolution inputs", async () => {
        ...
        const coordinator = new SyncCoordinator(recoveredQueue, undefined, {
            async dispatch() {
                return undefined;
            },
        });
        ...
        expect(coordinator.resolveConflict({ version: 1, title: "server" }, { version: 2, title: "local" }, "merge")).toEqual({ version: 2, title: "local" });
