...
import { UiRuntimeProvider, createQueryClientFactory, createRealtimeStore, createSyncStore, useUiState, } from "@aa/shared-state";
...
describe("shared auth/sync/state split modules", () => {
    it("fails closed on URL token callbacks and still validates explicit sessions and guard state", async () => {
        ...
        await expect(authService.handleSsoCallback(new URLSearchParams("access_token=a1&refresh_token=r1&locale=en-US"))).rejects.toThrow(/auth.redirecting/);
    ...
    it("tracks pending offline mutations and flushes them deterministically", async () => {
        const dispatched = [];
        const coordinator = new SyncCoordinator(undefined, undefined, {
