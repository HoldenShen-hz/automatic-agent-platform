...
import { FeatureScaffold, FeatureWorkbenchPanel, MetricGrid, MiniTrendBars, SystemStatusBar, createSystemHealthSummary, designTokens, darkTheme, highContrastTheme, lightTheme, resolveTheme, } from "@aa/ui-core";
import { buildMobileRouteMap, createMobileFeatureCard, createMobileScreenDescriptor, describeNativeModules, mobileNavigationBaseline, } from "@aa/ui-mobile";
describe("ui-core split modules", () => {
    it("renders core components from split directories", () => {
        render(_jsxs(FeatureScaffold, { title: "Split UI", summary: "split modules wired", status: "Implemented/Internal", children: [_jsx(MetricGrid, { metrics: [{ label: "Tasks", value: 12 }] }), _jsx(MiniTrendBars, { values: [1, 3, 2, 5] }), _jsx(SystemStatusBar, { status: { wsStatus: "connected", offlineQueueSize: 2, syncStatus: "queued", panicActivated: false } })] }));
    ...
    it("renders the interactive workbench baseline for L1 features", () => {
        render(_jsx(FeatureWorkbenchPanel, { metrics: [{ label: "Open", value: 3 }], items: [
