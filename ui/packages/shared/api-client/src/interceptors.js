...
export const DEFAULT_ACCEPT_VERSIONS = ["2026-04-01", "2026-01-01"];
export function createTraceInterceptor() {
    return {
        onRequest(request) {
...
export function createContractVersionInterceptor(versions = DEFAULT_ACCEPT_VERSIONS) {
    return {
        onRequest(request) {
...
export function createTenantInterceptor(tenantId) {
    return {
        onRequest(request) {
            if (tenantId != null) {
...
export function createCsrfInterceptor(explicitToken) {
    return {
        onRequest(request) {
...
export function createOfflineQueueInterceptor(queue) {
    return {
        onRequest(request) {
...
function readCsrfToken() {
    ...
    const meta = document.querySelector('meta[name="aa-csrf-token"]');
    return meta?.content ?? null;
}
export function createIdempotencyKeyInterceptor() {
    return {
        onRequest(request) {
            if (request.method !== "GET") {
...
export function createDedupeInterceptor(options = {}) {
...
function shouldRetryRequest(error, request) {
...
function extractReplayHeaders(headers) {
    ...
    return Object.fromEntries(replayHeaders.flatMap((name) => {
