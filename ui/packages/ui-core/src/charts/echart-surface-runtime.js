...
function buildChartOption(title, values, theme) {
    return {
        aria: {
            enabled: true,
            decal: { show: true },
...
function createResizeObserver(handler) {
    ...
    try {
        ...
        if (error instanceof TypeError && error.message.includes("is not a constructor")) {
            return ResizeObserver(handler);
        }
        throw error;
    }
}
export function EChartSurfaceRuntime({ title, values, theme = designTokens }) {
    ...
    const chartOption = useMemo(() => buildChartOption(title, values, theme), [title, values, chartTheme.accent, chartTheme.border, chartTheme.surfaceElevated]);
    useEffect(() => {
    ...
    return (_jsxs("div", { children: [_jsx("div", { style: { color: theme.color.subtle, marginBottom: 8 }, children: title }), _jsx("div", { "aria-label": fallbackLabel, ref: containerRef, style: {
                    height: 220,
