...
import { Inline as Inline2, LayoutFrame as LayoutFrame2, Stack as Stack2, ThreePaneLayout as ThreePaneLayout2 } from "../layouts";
...
function ListCard({ items }) {
  return createElement(
    ...
    ...items.map((item, index) => createElement(
      ...
      item.actionLabel == null || item.onAction == null ? null : createElement(
        "button",
        {
          type: "button",
...
function FeatureScaffold({ title, summary, status, children }) {
...
function resolveActionDisabled(action, selectedItem) {
  const disabled = typeof action.disabled === "function" ? action.disabled(selectedItem) : action.disabled ?? false;
  return disabled || action.onTrigger == null;
}
function buildWorkbenchActionHandler(scope, actionId, options = {}) {
  return async (item) => {
...
${item.description}`;
    ...
    if (typeof window !== "undefined") {
      ...
      const target = typeof options.deepLinkPath === "function" ? options.deepLinkPath(item) : options.deepLinkPath ?? null;
...
function navigateToWorkbenchTarget(target) {
  if (typeof window === "undefined") {
    return;
  }
...
function FeatureWorkbench({
  ...
  function moveSelection(delta) {
    ...
    const nextIndex = currentIndex < 0 ? 0 : (currentIndex + delta + filteredItems.length) % filteredItems.length;
    setSelectedId(filteredItems[nextIndex]?.id ?? null);
  }
  const metricsBlock = metrics != null && metrics.length > 0 ? createElement(
    "div",
    ...
    createElement(ThreePaneLayout, {
      left: filteredItems.length === 0 ? createElement("p", { style: { color: designTokens.color.subtle } }, resolvedLabels.emptyState) : createElement(
        "div",
        {
          ...
          "aria-activedescendant": selectedId == null ? void 0 : `feature-workbench-option-${selectedId}`,
          onKeyDown: (event) => {
        ...
        ...filteredItems.map((item) => createElement(
          "div",
          {
            key: item.id,
          ...
          createElement("strong", void 0, item.title),
          createElement("div", { style: { color: designTokens.color.subtle, marginTop: 8 } }, item.description)
        ))
      ),
      center: selectedItem == null ? createElement("p", { style: { color: designTokens.color.subtle } }, resolvedLabels.emptyState) : createElement(
        "div",
        ...
        selectedItem.detailRows != null && selectedItem.detailRows.length > 0 ? createElement(KeyValueTable, { rows: selectedItem.detailRows }) : null
      ),
      right: showActivityLog ? createElement(
        "div",
...
function FeatureWorkbenchPanel({
  ...
  const normalizedActions = useMemo(() => actions.map((action) => ({
    ...
    buildActivity: (item) => ({
      title: item == null ? action.label : `${action.label} \xB7 ${item.title}`,
      description: action.activityDescription ?? (item == null ? "A system-level action has been recorded." : `${item.title} has entered the ${action.label} flow.`)
    })
  })), [actions]);
...
export {
  ...
  Inline2 as Inline,
  KeyValueTable,
  LayoutFrame2 as LayoutFrame,
  ListCard,
  Stack2 as Stack,
  StatusPill,
  ThreePaneLayout2 as ThreePaneLayout,
  Timeline,
