...
export function getAnimationDuration(fast: string = "60ms", normal: string = "120ms", slow: string = "200ms"): { fast: string; normal: string; slow: string } {
...
export function getAnimationEasing(
  standard: string = "cubic-bezier(0.2, 0.8, 0.2, 1)",
  enter: string = "cubic-bezier(0, 0, 0.2, 1)",
  exit: string = "cubic-bezier(0.2, 0, 1, 1)",
): { standard: string; enter: string; exit: string } {
  if (prefersReducedMotion()) {
...
export function createKeyframes(name: string, frames: Record<string, string>): string {
  if (prefersReducedMotion()) {
    return "";
  }
