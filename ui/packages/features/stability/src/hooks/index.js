...
export function mapStabilityToVm(snapshot, incidents, workers, queues, agents) {
    ...
    return {
        ...
        })).concat(workers.slice(0, 2).map((worker) => ({
...
export function useStabilityVm() {
    return mapStabilityToVm(useDashboardSnapshotQuery().data ?? null, useIncidentsQuery().data ?? [], useWorkersQuery().data ?? [], useQueuesQuery().data ?? [], useAgentsQuery().data ?? []);
}
