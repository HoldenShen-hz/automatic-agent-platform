...
export function HitlWebView() {
    ...
    return (_jsx(FeatureScaffold, { title: "HITL", summary: "\u4EBA\u5DE5\u4ECB\u5165\u3001Inspect\u3001Takeover\u3001Resume \u7684\u7EDF\u4E00\u5165\u53E3", status: "Implemented/Partial", children: _jsxs(Stack, { children: [_jsxs(Inline, { children: [_jsx("button", { disabled: !hasApprovalItems || actionsDisabled, onClick: () => {
                                void vm.bulkApprove(approvalItems.map((item) => item.id));
                            }, type: "button", children: "Bulk Approve" }), _jsx("button", { disabled: !hasApprovalItems || actionsDisabled, onClick: () => {
                    ...
                    })) }), vm.items.map((item) => (_jsxs("div", { style: { border: `1px solid ${designTokens.color.border}`, borderRadius: 12, padding: 12, display: "grid", gap: 8 }, children: [_jsxs("div", { children: [_jsx("strong", { children: item.title }), _jsx("div", { children: item.description }), item.secondsRemaining != null && _jsx("div", { children: `SLA: ${item.secondsRemaining}s remaining` }), item.escalationTarget != null && _jsx("div", { children: `Escalation: ${item.escalationTarget}` })] }), item.type === "approval" ? (_jsxs(Inline, { children: [_jsx("button", { disabled: actionsDisabled, onClick: () => { void vm.approve(item.id); }, type: "button", children: "Approve" }), _jsx("button", { disabled: actionsDisabled, onClick: () => { void vm.reject(item.id); }, type: "button", children: "Reject" }), _jsx("button", { disabled: actionsDisabled, onClick: () => {
                                        setEditorMode("patch");
                                        setEditorTargetId(item.id);
                                        setEditorError(null);
                                    }, type: "button", children: "Patch" }), _jsx("button", { disabled: actionsDisabled, onClick: () => {
                                        setEditorMode("override");
                                        setEditorTargetId(item.id);
                                        setEditorError(null);
                                    }, type: "button", children: "Override" })] })) : (_jsx("button", { disabled: actionsDisabled, onClick: () => { void vm.resume(item.id, "normal"); }, type: "button", children: "Resume" }))] }, item.id))), editorMode != null && (_jsx("form", { onSubmit: (event) => {
                        event.preventDefault();
                        void applyEditor();
                    }, children: _jsxs(Stack, { gap: 8, children: [_jsx("textarea", { disabled: actionsDisabled, onChange: (event) => setEditorValue(event.target.value), value: editorValue }), editorError != null ? _jsx("p", { role: "alert", children: editorError }) : null, _jsx("button", { disabled: actionsDisabled, type: "submit", children: "Apply" })] }) }))] }) }));
}
