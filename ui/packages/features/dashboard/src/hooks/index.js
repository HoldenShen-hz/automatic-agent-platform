import { useAgentsQuery, useAnalyticsQuery, useDashboardSnapshotQuery, useIncidentsQuery, useQueuesQuery, useWorkersQuery, } from "@aa/shared-state";
...
export function mapDashboardSnapshotToVm(snapshot, analytics = [], incidents = [], workers = [], queues = [], agents = []) {
    return {
        ...
        panelGroups: snapshot == null
            ? []
            : buildPanelGroups(snapshot, analytics, incidents, workers, queues, agents),
    };
}
export function useDashboardVm() {
    ...
    return useMemo(() => mapDashboardSnapshotToVm(snapshot, analytics, incidents, workers, queues, agents), [
        agents,
        analytics,
        incidents,
        queues,
