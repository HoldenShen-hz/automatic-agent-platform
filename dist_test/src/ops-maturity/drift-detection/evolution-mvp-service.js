...
import { assertEvolutionScope, buildRecommendedBudgetPolicy, parsePolicyValue, parseProposalPayload, roundCurrency, roundRatio, summarizeBudgetProposal, } from "./evolution-mvp-support.js";
export class EvolutionMvpService {
    ...
    constructor(db, store, approvalService, memoryService) {
        this.db = db;
        this.store = store;
     ...
     */
    proposeExperiencePromotion(input) {
        assertEvolutionScope(input.scopeType, input.scopeRef);
        const createdAt = nowIso();
     ...
     */
    syncProposalApprovalStatus(proposalId, syncedAt = nowIso()) {
    ...
    applyProposal(input) {
        ...
        const previousPolicy = this.store.evolution.listEvolutionPolicies({
    ...
    rollbackProposal(input) {
        ...
        if (payload.kind === "experience_promotion") {
            const policyValue = parsePolicyValue(activePolicy);
     ...
     */
    resolveBudgetPolicy(basePolicy, scopeType, scopeRef) {
        ...
        const value = parsePolicyValue(activePolicy);
        return {
    ...
    toProposalView(proposal) {
        ...
        const approval = approvalRecord?.requestJson
            ? JSON.parse(approvalRecord.requestJson)
            : null;
        return {
            proposal,
            approval,
