...
class BasePluginRuntimeHost {
    ...
    rejectReady = null;
    stopping = false;
    constructor(options, sandboxRoot) {
    ...
    async invoke(action, context, input) {
        await this.start();
    ...
    attachChild(child, command = process.execPath, args = [this.childModulePath]) {
        this.child = child;
...
export class ForkedPluginRuntimeHost extends BasePluginRuntimeHost {
    constructor(options) {
        super(options, options.isolation === "sandboxed_process"
    ...
    sendRequest(child, request, requestId, reject) {
    ...
    sendShutdown(child, shutdownMessage, timer) {
...
export class ContainerizedPluginRuntimeHost extends BasePluginRuntimeHost {
    ...
    sendShutdown(child, shutdownMessage, timer) {
        child.stdin.write(`${JSON.stringify(shutdownMessage)}\n`, (error) => {
            if (!error) {
                return;
            }
            clearTimeout(timer);
...
export function buildPluginRuntimeSandboxRoot(pluginId, baseDir = join(process.cwd(), "data", "plugin-runtime-sandboxes")) {
    return resolve(baseDir, sanitizePluginIdForPath(pluginId));
}
export function buildPluginRuntimeExecArgv(options) {
...
function buildSandboxReadRoots(workspaceRoot, execArgs) {
    const roots = [
...
function deriveReadablePathFromExecArg(arg) {
    for (const prefix of ["--import=", "--loader=", "--require="]) {
...
function buildPluginRuntimeEnvironment(options) {
    const forwardedKeys = [
        "PATH",
        "HOME",
        "TMPDIR",
...
export function buildContainerizedPluginRuntimeLaunchSpec(options) {
    const rawTemplate = options.env.AA_PLUGIN_RUNTIME_CONTAINER_COMMAND_JSON?.trim();
    if (!rawTemplate) {
        throw new ValidationError("plugin_spi.container_launcher_missing", "plugin_spi.container_launcher_missing", {
            retryable: false,
            details: {
    ...
    try {
        ...
        throw new ValidationError("plugin_spi.container_launcher_invalid_json", "plugin_spi.container_launcher_invalid_json", {
            retryable: false,
            details: {
                pluginId: options.pluginId,
            },
    ...
    if (!Array.isArray(template) || template.some((value) => typeof value !== "string")) {
        throw new ValidationError("plugin_spi.container_launcher_invalid_shape", "plugin_spi.container_launcher_invalid_shape", {
            retryable: false,
            details: {
                pluginId: options.pluginId,
            },
    ...
    if (rendered.length === 0 || rendered[0].trim().length === 0) {
        throw new ValidationError("plugin_spi.container_launcher_empty_command", "plugin_spi.container_launcher_empty_command", {
            retryable: false,
            details: {
                pluginId: options.pluginId,
            },
...
function renderContainerizedToken(token, options) {
    const substitutions = {
        "{pluginId}": options.pluginId,
...
//# sourceMappingURL=plugin-runtime-host.js.map