...
function loadRegistrationPolicy() {
    ...
    const config = new ConfigGovernanceService(configRoot ? { configRoot, sandboxPolicy: createWorkspaceWritePolicy(configRoot) } : {}).loadBundle("dev");
    const security = config.layers.security ?? {};
    const registration = security.remoteWorkerRegistration != null
        && typeof security.remoteWorkerRegistration === "object"
        && !Array.isArray(security.remoteWorkerRegistration)
        ? security.remoteWorkerRegistration
        : {};
    return {
        challengeTtlMs: typeof registration.challengeTtlMs === "number" && Number.isFinite(registration.challengeTtlMs)
            ? registration.challengeTtlMs
            : 300_000,
        allowedCapabilities: Array.isArray(registration.allowedCapabilities)
            ? registration.allowedCapabilities.filter((item) => typeof item === "string")
            : ["bash", "edit", "mcp"],
    };
}
/**
...
main();
//# sourceMappingURL=worker-register.js.map