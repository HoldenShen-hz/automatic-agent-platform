...
import { openAsyncAuthoritativeStorageBackend, openAsyncAuthoritativeStorageContext, openAuthoritativeStorageBackend, openAuthoritativeStorageContext, planAuthoritativeStorageBackend, requireSqliteAuthoritativeStorageBackend, } from "../../platform/state-evidence/truth/storage-backend-factory.js";
 ...
 */
export function deriveCliWorkspaceRoot(dbPath) {
 ...
 */
function registerCliShutdownHandler(storage, label) {
 ...
 */
function resolveCliSyncStorageTarget(dbPath = resolveCliDbPath(), env = process.env) {
    ...
    if (shadowSqlitePath == null || shadowSqlitePath.length === 0) {
        throw new StorageError("storage.cli_sync_shadow_sqlite_required", "storage.cli_sync_shadow_sqlite_required", {
            retryable: false,
            details: {
 ...
 */
export function openCliAuthoritativeStorageBackend(dbPath = resolveCliDbPath()) {
    const target = resolveCliSyncStorageTarget(dbPath);
    return registerCliShutdownHandler(openAuthoritativeStorageBackend({
 ...
 */
export function openCliAuthoritativeStorageContext(dbPath = resolveCliDbPath()) {
    const target = resolveCliSyncStorageTarget(dbPath);
    return registerCliShutdownHandler(openAuthoritativeStorageContext({
        dbPath: target.dbPath,
        env: target.env,
    }), target.dbPath);
}
 ...
 */
export function withCliStorage(runner, options = {}) {
 ...
 */
export function withPersistentCliStorage(runner, options = {}) {
    const storage = openCliAuthoritativeStorageContext(options.dbPath);
    if (options.migrate !== false) {
        storage.migrate();
    }
 ...
 */
export async function openCliAuthoritativeStorageBackendAsync(dbPath = resolveCliDbPath()) {
    return registerCliShutdownHandler(await openAsyncAuthoritativeStorageBackend({
        dbPath,
    }), dbPath);
}
/**
 ...
 */
export async function withCliStorageBackendAsync(runner, options = {}) {
 ...
 */
export async function openCliAuthoritativeStorageContextAsync(dbPath = resolveCliDbPath()) {
    return registerCliShutdownHandler(await openAsyncAuthoritativeStorageContext({
        dbPath,
    }), dbPath);
}
/**
 ...
 */
export async function withCliStorageAsync(runner, options = {}) {
 ...
 */
export async function withPersistentCliStorageAsync(runner, options = {}) {
    const storage = await openCliAuthoritativeStorageContextAsync(options.dbPath);
    if (options.migrate !== false) {
        await storage.migrate();
    }
 ...
 */
export function describeCliAuthoritativeStoragePlan(dbPath = resolveCliDbPath()) {
 ...
 * @param dbPath - Optional explicit database path
 */
export function assertCliAuthoritativeStorageExecutable(dbPath = resolveCliDbPath()) {
 ...
 */
export function requireCliSqliteDatabase(storage) {
    return requireSqliteAuthoritativeStorageBackend(storage);
}
//# sourceMappingURL=authoritative-storage.js.map