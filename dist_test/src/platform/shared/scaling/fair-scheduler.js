...
export class FairScheduler {
     ...
     */
    registerTenant(tenantId, weight, guaranteed) {
     ...
     */
    admitTask(tenantId, taskId, requested) {
     ...
     */
    releaseResources(tenantId, released) {
        const tenant = this.tenants.get(tenantId);
        if (!tenant)
            return;
        ...
        tenant.currentlyUsed.maxConcurrentWorkflows = Math.max(0, tenant.currentlyUsed.maxConcurrentWorkflows - (released.maxConcurrentWorkflows ?? 0));
        tenant.currentlyUsed.maxConcurrentWorkers = Math.max(0, tenant.currentlyUsed.maxConcurrentWorkers - (released.maxConcurrentWorkers ?? 0));
        tenant.currentlyUsed.llmTokensPerMinute = Math.max(0, tenant.currentlyUsed.llmTokensPerMinute - (released.llmTokensPerMinute ?? 0));
        tenant.currentlyUsed.llmRequestsPerMinute = Math.max(0, tenant.currentlyUsed.llmRequestsPerMinute - (released.llmRequestsPerMinute ?? 0));
    }
    /**
     * Trigger reclaim of borrowed resources back to lenders.
     */
    reclaimBorrowed(tenantId, amount, resourceType) {
        const tenant = this.tenants.get(tenantId);
        if (!tenant)
            return;
        ...
        for (const lender of lenders) {
            ...
            if (lenderTenant) {
                ...
                const key = keyMapping[borrowedKey];
                lenderTenant.currentlyUsed[key] += amountToReturn;
            }
        }
     ...
     */
    tryBorrow(requesterId, requested) {
     ...
     */
    checkResourceFit(available, used, requested) {
        const remaining = {
    ...
    getTenantStats(tenantId) {
        ...
        if (!tenant)
            return null;
        const guaranteed = tenant.guaranteedAllocation;
        const totalUsage = tenant.currentlyUsed.maxConcurrentWorkflows
            + tenant.currentlyUsed.maxConcurrentWorkers
            + tenant.currentlyUsed.llmTokensPerMinute;
        const totalGuaranteed = guaranteed.maxConcurrentWorkflows
    ...
    getAllUtilization() {
        ...
        for (const [tenantId, tenant] of this.tenants) {
            const totalUsage = tenant.currentlyUsed.maxConcurrentWorkflows
                + tenant.currentlyUsed.maxConcurrentWorkers
                + tenant.currentlyUsed.llmTokensPerMinute;
            const totalGuaranteed = tenant.guaranteedAllocation.maxConcurrentWorkflows
...
//# sourceMappingURL=fair-scheduler.js.map