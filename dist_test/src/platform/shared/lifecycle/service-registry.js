...
export class ServiceRegistry {
     ...
     */
    get(name) {
        return this.getRecursive(name, new Set());
    }
    /**
    ...
    getRecursive(name, visiting) {
        ...
        const registration = this.services.get(name);
        if (!registration) {
            throw new InternalAppError("service_registry.not_registered", `ServiceRegistry: no service registered with name "${name}"`, { source: "internal", details: { serviceName: name } });
        }
        ...
        const instance = registration.init();
