...
function seedClaimedExecution(dbPath) {
...
async function runCompletionScenario(outputDir) {
...
async function runDuplicateScenario(outputDir) {
    return measureScenario("duplicate_writeback_rejected", async () => {
        ...
        db.close();
        return {
            passed: duplicate.accepted === false &&
...
async function runStaleFencingScenario(outputDir) {
    return measureScenario("stale_fencing_writeback_rejected", async () => {
        ...
        db.close();
        return {
            passed: reacquired.lease?.fencingToken === 2 &&
...
export async function runStableWorkerWritebackRehearsal(options) {
...
export function writeStableWorkerWritebackRehearsalReport(path, report) {
    writeJson(path, report);
}
//# sourceMappingURL=stable-worker-writeback-rehearsal.js.map