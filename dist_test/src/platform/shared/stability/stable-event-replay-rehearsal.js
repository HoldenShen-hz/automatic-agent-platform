...
function seedTaskAndExecution(db, store) {
    const now = nowIso();
    db.transaction(() => {
...
async function runFailedConsumerAckReplay(outputDir) {
    return measureScenario(async () => {
        ...
        ops.subscribe("task_projection", async () => {
            throw new WorkflowStateError("projection replay rehearsal failure", "projection replay rehearsal failure", {
                retryable: false,
            });
        });
        ...
        db.close();
        return {
            passed: firstAttempt.outcome === "failed" &&
...
export async function runStableEventReplayRehearsal(options) {
...
export function writeStableEventReplayRehearsalReport(outputFile, report) {
    writeJson(outputFile, report);
}
//# sourceMappingURL=stable-event-replay-rehearsal.js.map