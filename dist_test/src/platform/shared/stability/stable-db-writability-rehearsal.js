 ...
 */
function buildReadOnlyBackpressureSnapshot() {
    return {
        status: "unhealthy",
 ...
 */
async function runHealthAndDoctorScenario(outputDir) {
    return measureScenario("health_and_doctor_fail_close_when_db_is_not_writable", async () => {
        ...
        const doctor = new DoctorService(healthService, new StartupConsistencyChecker(db, store), null, null, new SqliteReliabilityService(db), `${dbPath}.backup`, null, null, new WorkerRegistryService(store), null, null, null, { store });
        ...
        db.close();
        return {
            passed: healthReport.dbWritable === false &&
 ...
 * - Admission rejection event is emitted
 */
async function runMultiStepAdmissionScenario(outputDir) {
    return measureScenario("multi_step_admission_rejects_new_work_in_read_only_mode", async () => {
        ...
        return {
            passed: result.snapshot.task.status === "cancelled" &&
 ...
 */
async function runDispatchReadOnlyScenario(outputDir) {
    return measureScenario("dispatch_blocks_claims_without_dropping_pending_ticket_in_read_only_mode", async () => {
        ...
        db.close();
        return {
            passed: decision.outcome === "blocked" &&
 ...
 */
export async function runStableDbWritabilityRehearsal(options) {
 ...
 */
export function writeStableDbWritabilityRehearsalReport(path, report) {
    writeJson(path, report);
}
//# sourceMappingURL=stable-db-writability-rehearsal.js.map