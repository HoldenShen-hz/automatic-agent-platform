 ...
 */
async function runExpiredLockReleased(outputDir) {
    return measureScenario("expired_lock_released", async () => {
        ...
        db.close();
        return {
            passed: before.findings.some((finding) => finding.code === "expired_file_lock") &&
 ...
 */
async function runActiveExecutionConflictFailClosed(outputDir) {
    return measureScenario("active_execution_conflict_fail_closed", async () => {
        ...
        db.close();
        return {
            passed: report.status === "fail_closed" &&
...
async function runCompetingWriteTransactionsFailClosed(outputDir) {
    return measureScenario("competing_write_transactions_fail_closed", async () => {
        ...
        contenderDb.close();
        return {
            passed: isSqliteWriteContentionError(contentionError) &&
 ...
 */
export async function runStableConcurrencyRehearsal(options) {
 ...
 */
export function writeStableConcurrencyRehearsalReport(outputFile, report) {
    writeJson(outputFile, report);
}
//# sourceMappingURL=stable-concurrency-rehearsal.js.map