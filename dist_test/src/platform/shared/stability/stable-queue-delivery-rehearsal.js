 ...
 */
function seedWorkflowAndSession(db, store, taskId) {
    const now = nowIso();
    db.transaction(() => {
 ...
 */
async function runQueueReplayScenario(outputDir) {
    return measureScenario("queue_replay_rebuilds_dispatchable_ticket", async () => {
        ...
        const replacementTicketId = repaired.applied.find((item) => item.applied && item.replacementTicketId != null)?.replacementTicketId ?? null;
 ...
 */
async function runDuplicateDeliveryScenario(outputDir) {
    return measureScenario("duplicate_delivery_blocked_and_reconciled", async () => {
        ...
        return {
            passed: claim.accepted &&
                ...
                duplicateDispatch.trace?.evaluations.some((evaluation) => evaluation.workerId === "worker-duplicate-delivery" && evaluation.rejectionReason === "worker_capacity_full") === true &&
                duplicateDispatch.ticket?.id === duplicateTicketId &&
                writebackDecision.accepted &&
                repaired.issues.some((issue) => issue.issueType === "terminal_execution_ticket" && issue.ticketId === duplicateTicketId) &&
 ...
 * 2. Duplicate delivery is blocked and reconciled
 */
export async function runStableQueueDeliveryRehearsal(options) {
 ...
 */
export function writeStableQueueDeliveryRehearsalReport(outputFile, report) {
    writeJson(outputFile, report);
}
//# sourceMappingURL=stable-queue-delivery-rehearsal.js.map