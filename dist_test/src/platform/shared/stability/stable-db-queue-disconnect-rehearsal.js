 ...
 */
function seedTaskExecutionWorkflowAndSession(db, store, input) {
    const now = nowIso();
    db.transaction(() => {
 ...
 */
async function runQueueDisconnectDegradeScenario(outputDir) {
    return measureScenario("queue_disconnect_degrades_without_silent_drop", async () => {
        ...
        db.close();
        return {
            passed: created.outcome === "created" &&
 ...
 */
async function runMissingTicketRepairScenario(outputDir) {
    return measureScenario("missing_dispatch_ticket_rebuilt_after_queue_reconnect", async () => {
        ...
        const rebuiltCapabilities = rebuiltTicket ? JSON.parse(rebuiltTicket.requiredCapabilitiesJson) : [];
        const events = store.event.listEventsForTask("task-queue-disconnect-repair");
        db.close();
        return {
            passed: issues.some((issue) => issue.issueType === "missing_dispatch_ticket" && issue.recoveredFromPlan) &&
 ...
 */
async function runAuthoritativeWritebackFailureScenario(outputDir) {
    return measureScenario("authoritative_writeback_failure_fails_closed_until_store_recovers", async () => {
        ...
        const transitionService = writeback.transitions;
        ...
        transitionService.applyTaskTerminalState = (() => {
            throw new StorageError("db_queue_disconnect_drill:authoritative_store_unavailable", "db_queue_disconnect_drill:authoritative_store_unavailable", {
                retryable: true,
            });
        });
 ...
 */
export async function runStableDbQueueDisconnectRehearsal(options) {
 ...
 */
export function writeStableDbQueueDisconnectRehearsalReport(path, report) {
    writeJson(path, report);
}
//# sourceMappingURL=stable-db-queue-disconnect-rehearsal.js.map