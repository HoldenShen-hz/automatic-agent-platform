...
import { buildStableReleaseGateReport, writeStableReleaseGateReport, } from "./stable-release-gate.js";
/** Supported evidence profile names */
const SUPPORTED_PROFILES = ["smoke", "24h", "72h"];
...
function collectProfiles(evidenceRootDir) {
...
function buildNextActions(gate, profiles) {
    const actions = [];
    ...
    const migrationCompatibilityCriterion = gate.criteria.find((criterion) => criterion.criterionId === "migration_compatibility_tested");
    ...
    if (gate.targetStatus === "production_ready" && acceptanceCriterion?.status !== "pass") {
        actions.push("Continue the long-run campaign until the QA-64 stable acceptance line truthfully reaches a full 14-day soak window.");
    }
    ...
    if (gate.overallVerdict === "promote_approved") {
        ...
        actions.push(gate.targetStatus === "production_ready"
...
function buildRecommendedCommands(targetStatus) {
    const commands = [
...
function buildReleaseChecklist(gate, profiles) {
    ...
    const recoveryCriteriaSummary = summarizeCriteria(gate.optionalCriteria.filter((criterion) => criterion.criterionId === "chaos_drill_results"
    ...
    const items = [
        {
            ...
            status: gate.overallVerdict === "promote_approved"
                ? "pass"
        ...
        },
        {
            ...
            detail: smokeProfile?.present && smokeProfile.passed === true
        ...
        },
        {
            ...
            status: gate.targetStatus === "production_ready"
                ? (acceptanceCriterion?.status ?? "fail")
                : "pass",
            detail: gate.targetStatus === "production_ready"
        ...
        },
        {
            ...
            status: gate.targetStatus === "canary"
                ? "pass"
                : grayCriterion?.status ?? "fail",
            detail: gate.targetStatus === "canary"
 ...
 */
export function createStableReleasePackage(options) {
    ...
    const missingRequiredProfiles = gate.requiredProfiles.filter((profile) => !profiles.some((item) => item.profile === profile && item.present));
...
//# sourceMappingURL=stable-release-package.js.map