...
export class ObservabilityRetentionService {
     ...
     */
    buildEventTierSummary(eventTier, retentionDays, mode, evaluatedAt) {
        ...
        const eligibleParams = [eventTier, cutoff, ...TERMINAL_TASK_STATUSES];
        // Count events eligible for deletion: old events on terminal tasks
        const eligibleCount = this.selectCount(`SELECT COUNT(*) AS count
       FROM events e
       LEFT JOIN tasks t ON t.id = e.task_id
       WHERE e.event_tier = ?
         AND e.created_at < ?
         AND (t.id IS NULL OR t.status IN (${toPlaceholders(TERMINAL_TASK_STATUSES.length)}))`, eligibleParams);
        // Find oldest eligible event for reporting
        const oldestEligibleCreatedAt = this.selectString(`SELECT MIN(e.created_at) AS value
       FROM events e
       LEFT JOIN tasks t ON t.id = e.task_id
       WHERE e.event_tier = ?
         AND e.created_at < ?
        ...
        const deletedCount = mode === "enforced" && eligibleCount > 0
            ? this.db.connection
     ...
     */
    buildMessageSummary(mode, evaluatedAt) {
        ...
        const eligibilityParams = [cutoff, ...preservedTypes, ...TERMINAL_SESSION_STATUSES, ...TERMINAL_TASK_STATUSES];
       ...
       WHERE m.created_at < ?
         AND ${deletableClause}
         AND ${terminalCondition}`, eligibilityParams);
        // Count preserved summary messages
        const preservedSummaryCount = this.selectCount(`SELECT COUNT(*) AS count
       FROM messages m
       WHERE m.created_at < ?
         AND ${preservedClause}`, [cutoff, ...preservedTypes]);
        // Count preserved messages from active sessions
        const preservedActiveSessionCount = this.selectCount(`SELECT COUNT(*) AS count
       FROM messages m
       JOIN sessions s ON s.id = m.session_id
       JOIN tasks t ON t.id = s.task_id
       WHERE m.created_at < ?
         AND ${deletableClause}
         AND NOT ${terminalCondition}`, eligibilityParams);
        // Perform actual deletion in enforced mode
        const deletedCount = mode === "enforced" && eligibleCount > 0
            ? this.db.connection
    ...
    selectCount(sql, params = []) {
        const row = this.db.connection.prepare(sql).get(...params);
        return Number(row?.count ?? 0);
    }
    /**
     * Executes a query and returns a string value from a 'value' column.
     */
    selectString(sql, params = []) {
        const row = this.db.connection.prepare(sql).get(...params);
...
//# sourceMappingURL=observability-retention-service.js.map