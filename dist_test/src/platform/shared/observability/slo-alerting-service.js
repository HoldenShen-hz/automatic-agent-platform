...
export class SloAlertingService {
    ...
    defineAlertRule(input) {
        const now = nowIso();
        const rule = { id: newId("arule"), createdAt: now, ...input };
        this.db.connection
    ...
    fireAlert(ruleId, title, detail) {
        ...
        const severity = rule ? String(rule.severity) : "warning";
        const channelKind = rule ? String(rule.channel_kind) : "log";
        const event = {
            id: newId("alert"),
            ruleId,
            severity,
    ...
    fireAlertToPagerDuty(title, detail) {
        ...
        if (!pagerDutyChannel) {
            ...
            return this.fireAlertWithChannel("slo_error_budget_exhausted", title, detail + " (PagerDuty channel not configured - requires manual notification)", "critical", "log");
        }
        return this.fireAlertWithChannel("slo_error_budget_exhausted", title, detail, "page", "pagerduty");
    }
    /**
