...
function remoteLogEntry(record) {
...
function stepOutputEntry(stepOutput) {
    return {
        id: stepOutput.id,
        kind: "step_output",
...
function approvalEntry(approval) {
    ...
    return {
        ...
        summary: approval.status === "requested"
            ? "Approval requested"
...
function dispatchEntry(decision, occurredAt) {
    return {
...
function buildDispatchSummary(decision) {
    const selectedWorker = decision.selectedWorkerId == null
        ? null
...
function extractDispatchTicketId(event) {
    try {
...
function consumeDispatchOccurredAt(decision, dispatchEventTimes) {
...
function buildEventSummary(event, data) {
    if (event.eventType === "worker:claim_rejected"
        ...
        const action = event.eventType === "worker:claim_rejected"
            ? "claim"
            : event.eventType === "worker:heartbeat_rejected"
                ? "heartbeat"
                : "writeback";
...
//# sourceMappingURL=task-timeline-service.js.map