 ...
 */
export function buildIncidentTimelineMarkdown(report) {
    const lines = [
        ...
        ...report.warnings.entries.map((entry) => `- ${entry.code} [${entry.category}] severity=${entry.severity} escalation=${entry.escalation} count=${entry.count}`),
        "",
        "## Timeline",
        "",
        ...report.entries.map((entry) => `- ${entry.occurredAt} [${entry.source}] [${entry.severity}] ${entry.title}: ${entry.summary}`),
    ];
    return lines.join("\n");
}
export function extractRemoteAuthorityViolations(events) {
    const violations = [];
...
export function toIncidentTimelineEntry(entry) {
    return {
        id: entry.id,
        source: entry.kind,
...
export function messageToIncidentTimelineEntry(message, taskId) {
    return {
        id: message.id,
        source: "message",
...
export function compactionToIncidentTimelineEntry(record, taskId) {
    return {
        id: record.id,
        source: "compaction",
...
export function logToIncidentTimelineEntry(entry, taskId) {
    return {
...
export function buildTraceSummary(entries, fallbackTraceId, fallbackCorrelationId) {
    ...
    return {
        ...
        spanIds: [...new Set(entries.map((entry) => entry.spanId).filter((value) => typeof value === "string"))],
    };
}
export function buildPrimaryTraceContext(events, fallbackTraceId, fallbackCorrelationId) {
    for (const event of events) {
...
export function countIncidentEntries(entries, source) {
    return entries.filter((entry) => entry.source === source).length;
}
export function resolveDurationMs(startedAt, endedAt) {
...
export function inferTimelineSeverity(entry) {
...
export function resolveHighestIncidentSeverity(entries, warningSeverity) {
    const entrySeverity = resolveHighestWarningSeverity(entries.map((entry, index) => ({
...
export function buildIncidentRootCauses(snapshot, debugDump) {
    const hints = [];
    ...
    const remoteErrorLogCount = snapshot.timeline.filter((entry) => entry.kind === "remote_log"
        && (entry.data.level === "error" || entry.data.level === "warn")).length;
...
export function selectRemoteTimelineEntries(entries) {
    return entries.filter((entry) => {
        if (entry.source === "remote_log") {
            return true;
        }
        if (entry.source === "dispatch") {
            return (entry.data.dispatchTarget === "prefer_remote"
...
export function extractRemoteWorkerIds(entries) {
 ...
 */
export function previewText(content, maxLength = 120) {
 ...
 */
export function safeParseRecord(raw) {
    if (!raw) {
        return null;
    }
    try {
...
export function buildDiagnosticWarningSummary(warnings) {
...
export function classifyDiagnosticWarning(code) {
...
export function resolveHighestWarningSeverity(entries) {
    let highestRank = 0;
...
export function warningSeverityRank(severity) {
    switch (severity) {
        case "critical":
            return 3;
        case "warning":
...
//# sourceMappingURL=diagnostics-support.js.map