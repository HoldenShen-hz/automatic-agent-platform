...
import { buildLeaseHandoverSummary, buildRemoteRoutingSummary, buildStepResultEnvelopes, enrichDispatchDecisionTrace, findActiveExecutionId, normalizeLimit, parseApprovalDecisionSummary, parseApprovalRequestSummary, parseDispatchDecisionTraceFromEvent, parseJsonArray, } from "./inspect-service-support.js";
export class InspectService {
     ...
     */
    getTaskInspectView(taskId, tenantId) {
        ...
        return {
            ...
            recoverySummary: {
                activeExecutionId: snapshot.execution && !["succeeded", "failed", "cancelled", "superseded"].includes(snapshot.execution.status)
     ...
     */
    getExecutionInspectView(executionId) {
     ...
     */
    getApprovalInspectView(approvalId) {
        ...
        const stepResults = buildStepResultEnvelopes(snapshot.stepOutputs, snapshot.artifacts);
        const dispatchDecisions = approval.executionId == null
            ? []
            : this.store.event.listDispatchDecisionTracesByExecution(approval.executionId).map(enrichDispatchDecisionTrace);
        const recentEvents = snapshot.events.filter((event) => event.eventType.startsWith("decision:") || event.executionId === approval.executionId);
        return {
            task: snapshot.task,
            workflowState: snapshot.workflow,
            execution,
    ...
    queryTaskInspectSummaries(query = {}) {
    ...
    queryWorkflowInspectSummaries(query = {}) {
        const limit = normalizeLimit(query.limit, 25);
        const tenantTaskIds = query.tenantId === undefined
            ? null
            : new Set(this.store
                .listTasks()
    ...
    queryDecisionInspectSummaries(query = {}) {
    ...
    queryWorkerInspectSummaries(query = {}) {
        const limit = normalizeLimit(query.limit, 50);
        return this.store
            .listWorkerSnapshots()
            .map((worker) => {
     ...
     */
    listSessionCompactionRecords(taskId) {
        const snapshot = this.store.operations.loadTaskSnapshot(taskId);
    ...
    buildWorkflowInspectSummary(workflow) {
    ...
    buildDecisionSummariesForTask(taskId) {
