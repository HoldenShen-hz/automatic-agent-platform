...
export class PrometheusMetricsExporter {
    ...
    metricsService;
    processStartTime;
    constructor(db, metricsService, options = {}) {
     ...
     */
    recordHttpRequest(method, path, status, durationMs = null) {
    ...
    renderCounterSeries(name) {
        return runtimeMetricsRegistry.getCounters(name).map((series) => (`${name}${formatPrometheusLabels(series.labels)} ${series.value}`));
    }
    renderHistogramSeries(name) {
        const lines = [];
...
//# sourceMappingURL=prometheus-metrics-exporter.js.map