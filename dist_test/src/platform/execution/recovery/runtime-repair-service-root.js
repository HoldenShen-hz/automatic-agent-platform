...
export class RuntimeRepairService {
     ...
     */
    requeueExecution(action, occurredAt) {
    ...
    ensurePendingDispatchTicket(executionId, occurredAt) {
     ...
     */
    reconcileDispatchTicket(action, occurredAt) {
     ...
     */
    closeOrphanSession(action, occurredAt) {
    ...
    replaceTerminalSession(action, occurredAt) {
        const session = this.store.dispatch.getSession(action.targetId);
        if (!session) {
            return {
                action: action.action,
     ...
     */
    releaseStaleLock(action, occurredAt) {
        this.db.transaction(() => {
     ...
     */
    async rebuildAck(action, occurredAt) {
...
//# sourceMappingURL=runtime-repair-service-root.js.map