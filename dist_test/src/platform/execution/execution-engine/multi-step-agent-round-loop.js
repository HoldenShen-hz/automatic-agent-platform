...
export async function executeAgentRoundLoop(input) {
...
${priorContext}`,
    ...
    const llmTools = input.tools?.map((tool) => {
        const item = {
            type: "function",
            name: tool.name,
...
export function parseStepOutput(content, stepId) {
...
export function fallbackStepOutput(input) {
    let summary;
    let result;
    switch (input.stepId) {
        case "intake_triage":
...
export async function buildStepOutput(input) {
...
//# sourceMappingURL=multi-step-agent-round-loop.js.map