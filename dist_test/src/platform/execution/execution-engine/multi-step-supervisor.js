...
import { decideWorkflowStepRetry } from "../../orchestration/oapeflir/workflow/workflow-step-retry-policy.js";
...
export function normalizeStepFailurePlan(value) {
    return typeof value === "string" ? { errorCode: value } : value;
}
export function resolveStepFailurePlan(input, stepId, attempt) {
...
export function normalizeStepErrorCode(error) {
    const message = error instanceof Error ? error.message : String(error);
    if (message.startsWith("workflow.output_schema_invalid"))
        return "validation.schema_mismatch";
    if (message.startsWith("workflow.output_schema_missing"))
        return "validation.invalid_input";
    return "internal.unexpected_error";
}
export function buildStepFailureSummary(stepId, decision) {
...
export async function executeStepLoop(ctx, deps) {
    const { taskId, sessionId, traceId, traceContext, streamId, input, routing, plannedWorkflow, toolExposureService, } = ctx;
    ...
    for (const [index, step] of plannedWorkflow.executionSteps.entries()) {
        ...
        for (let attempt = 1; attempt <= step.maxAttempts; attempt += 1) {
            ...
            let validation;
            try {
                validation = validateWorkflowStepOutput(step, stepData);
            }
            catch (error) {
            ...
            const upstreamArtifactRefs = stepOutputs.flatMap((item) => {
                if (!item.artifactsJson)
                    return [];
                try {
                    return JSON.parse(item.artifactsJson);
                }
                catch {
                    return [];
                }
...
//# sourceMappingURL=multi-step-supervisor.js.map