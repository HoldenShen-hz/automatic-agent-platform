...
function isOapeflirPlanRequest(request) {
    return request.startsWith(OAPEFLIR_PLAN_PREFIX);
}
function deserializeOapeflirPlan(request) {
    const json = request.slice(OAPEFLIR_PLAN_PREFIX.length);
    return JSON.parse(json);
}
function resolveOapeflirRoleId(_step) {
    return "general_executor";
}
function oapeflirStepToMinimalStep(step) {
    return {
        stepId: step.stepId,
...
function buildOapeflirPlannedWorkflow(steps, planId) {
    const workflowDef = {
        workflowId: `oapeflir_${planId}`,
        divisionId: "general_ops",
        steps: steps.map(oapeflirStepToMinimalStep),
    };
    const executionSteps = workflowDef.steps.map((step) => {
...
export async function runMultiStepOrchestration(input) {
    ...
    if (isOapeflirPlanRequest(input.request)) {
        ...
        routing = {
            ...
            classification: { intent: "create", confidence: 1.0, continuation: "new_task", matchedRules: [] },
        };
    }
    else {
    ...
    try {
        ...
        const backpressureSnapshot = input.admissionBackpressureSnapshot
            ?? (() => healthService.getReport());
        const admission = new AdmissionController(store, input.admissionPolicy, backpressureSnapshot);
        ...
        return await provideContext({
            ...
            let workflowLastErrorCode = null;
            let blockedForDecision = false;
            let skippedStepIds = new Set();
            let failedStepIds = new Set();
            const stepResult = await executeStepLoop({
                ...
                createContext: (reasonCode) => createContext(traceContext, reasonCode),
            });
            ...
            const finalOutput = outputs.final ?? outputs;
            const allStepsFailedOrSkipped = plannedWorkflow.executionSteps.every((step) => failedStepIds.has(step.stepId) || skippedStepIds.has(step.stepId));
...
//# sourceMappingURL=multi-step-orchestration.js.map