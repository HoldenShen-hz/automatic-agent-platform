...
export class WalCheckpointService {
    ...
    writeWalEntry(input) {
        ...
        this.db.connection
            .prepare(`INSERT INTO wal_entries (id, entry_type, execution_id, task_id, session_id, payload, created_at, checkpoint_id, sequence_number)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(entry.id, entry.entryType, entry.executionId, entry.taskId, entry.sessionId, JSON.stringify(entry.payload), entry.createdAt, entry.checkpointId, entry.sequenceNumber);
    ...
    createCheckpoint(options) {
        ...
        this.db.connection
            .prepare(`INSERT INTO checkpoints (id, execution_id, state, created_at, last_wal_sequence, metadata)
         VALUES (?, ?, ?, ?, ?, ?)`)
            .run(checkpoint.id, checkpoint.executionId, JSON.stringify(checkpoint.state), checkpoint.createdAt, checkpoint.lastWalSequence, checkpoint.metadata ? JSON.stringify(checkpoint.metadata) : null);
     ...
     */
    getWalEntriesSince(checkpointId, limit) {
     ...
     */
    saveReplayPosition(consumerName, position) {
        this.db.connection
            .prepare(`INSERT OR REPLACE INTO event_replay_positions (id, consumer_name, last_processed_event_id, last_processed_sequence, last_checkpoint_id, updated_at)
         VALUES (?, ?, ?, ?, ?, ?)`)
            .run(newId("replaypos"), consumerName, position.lastProcessedEventId, position.lastProcessedSequence, position.lastCheckpointId, nowIso());
    }
    /**
     * Gets the replay position for a consumer.
     */
    getReplayPosition(consumerName) {
        const row = this.db.connection
     ...
     */
    async replayEvents(consumerName, handler, options) {
 ...
 */
export function createWalCheckpointService(options) {
    return new WalCheckpointService(options);
}
//# sourceMappingURL=wal-checkpoint-service.js.map