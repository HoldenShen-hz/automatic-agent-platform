...
import { isBlockedOutboundHostname, isInternalNetworkUrl, } from "../../control-plane/iam/outbound-url-policy.js";
 ...
 */
export function isBlockedIpOrHostname(hostname) {
 ...
 */
export function isDomainAllowed(hostname, allowedDomains, blockedDomains) {
...
export function createWebFetchTool() {
    return {
        ...
        async execute(request) {
            ...
            if (!isDomainAllowed(url.hostname, request.allowedDomains, request.blockedDomains)) {
                return {
                    success: false,
                    status: "blocked",
            ...
            if (method === "GET") {
                try {
                    ...
                    body = decoder.decode(Buffer.concat(chunks), { stream: bodyTruncated });
                }
                catch (err) {
