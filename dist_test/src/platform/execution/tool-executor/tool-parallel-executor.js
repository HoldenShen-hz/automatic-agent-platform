 ...
 */
export function isConcurrentSafe(metadata) {
 ...
 */
export function requiresExclusiveExecution(metadata) {
 ...
 */
export function partitionParallelToolGroups(toolMetadatas) {
 ...
 */
export function canExecuteInParallel(metadatas) {
 ...
 */
export function partitionToolsByExecutionMode(toolMetadatas) {
    if (toolMetadatas.length === 0) {
        return {
            parallelIndices: [],
            exclusiveIndices: [],
 ...
 */
export async function executeToolsInParallel(toolFunctions, toolMetadatas, options = {}) {
    const { maxParallelism = 4, failFast = false, logger } = options;
    if (toolFunctions.length !== toolMetadatas.length) {
        throw new ValidationError("tool_parallel.length_mismatch", "tool_parallel.length_mismatch: toolFunctions and toolMetadatas must have same length", {
            source: "tool",
            details: {
    ...
    const results = new Array(toolFunctions.length);
    const errors = [];
 ...
 * @returns Promise resolving to parallel execution results
 */
export async function executeToolItemsInParallel(items, options = {}) {
...
//# sourceMappingURL=tool-parallel-executor.js.map