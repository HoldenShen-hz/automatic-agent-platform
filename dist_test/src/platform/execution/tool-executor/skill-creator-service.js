 ...
 */
const OPTIONAL_RESOURCE_DIRECTORIES = ["scripts", "references", "assets"];
/**
 ...
 */
function uniqueResourceDirectories(items) {
    return Array.from(new Set((items ?? DEFAULT_RESOURCE_DIRECTORIES).filter((item) => OPTIONAL_RESOURCE_DIRECTORIES.includes(item))));
}
/**
...
export class SkillCreatorService {
    ...
    createSkill(request) {
        ...
        if (!validation.valid) {
            throw new ValidationError(`skill_creator.validation_failed:${validation.errors.join(";")}`, `skill_creator.validation_failed:${validation.errors.join(";")}`, {
                source: "tool",
        ...
        writeTextFile(skillMarkdownPath, renderSkillMarkdown({
            skillSlug,
            name: request.name,
            description: request.description,
            requiredTools,
        ...
        if (includeOpenAiAgent) {
            ...
            writeTextFile(openAiAgentPath, renderOpenAiAgentYaml({
                skillSlug,
                description: request.description,
                requiredTools,
            }));
     ...
     */
    assertTargetPathAllowed(skillRoot, skillPath) {
     ...
     */
    validateSkillDefinition(request) {
        if (this.options.governance) {
...
//# sourceMappingURL=skill-creator-service.js.map