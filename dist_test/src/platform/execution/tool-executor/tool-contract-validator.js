 ...
 */
export function validateToolExecutionMetadata(metadata) {
    const violations = [];
    ...
    if (metadata.retryableErrorCodes.some((code) => typeof code !== "string" || code.trim().length === 0)) {
        violations.push({
            toolName: metadata.toolName,
 ...
 */
export function validateToolMetadataRegistry(metadataItems) {
