 ...
 */
function normalizeExcludedPath(path) {
 ...
 */
function sanitizePathForReason(path) {
 ...
 */
function isIgnoredRelativePath(relativePath, excludedPaths) {
 ...
 */
function computeEntrySizeBytes(path, budget) {
...
export class ShadowSnapshotService {
     ...
     */
    validateShadowRoot(workspaceRoot) {
        ...
        if (Array.from(workspaceCandidates).some((candidate) => isWithin(resolvedShadowRoot, candidate))) {
            throw new PolicyDeniedError("shadow_snapshot.shadow_root_inside_workspace", "shadow_snapshot.shadow_root_inside_workspace", {
     ...
     */
    guardWorkspaceEntrySizes(workspaceRoot) {
        ...
        for (const entry of entries) {
            ...
            if (sizeBytes > this.maxEntryBytes) {
                throw new ValidationError(`shadow_snapshot.entry_too_large:${sanitizePathForReason(relativePath)}`, `shadow_snapshot.entry_too_large:${sanitizePathForReason(relativePath)}`, {
                    source: "tool",
                    details: {
                        relativePath,
                        sizeBytes,
     ...
     */
    ensureRepository(workspaceRoot, shadowRoot) {
     ...
     */
    loadRecord(snapshotId, shadowRoot) {
     ...
     */
    git(args, workspaceRoot, shadowRoot) {
...
//# sourceMappingURL=shadow-snapshot-service.js.map