...
import { listBuiltinToolExecutionMetadata, resolveToolExecutionMetadata, } from "./tool-metadata.js";
...
function normalizeCompactToolName(toolName) {
 ...
 */
export function expandToolNames(toolNames) {
...
function buildNormalizedCandidateMap() {
...
function buildFuzzyCandidateList() {
...
function findUniqueFuzzyToolMatch(normalizedCompactName, candidates) {
 ...
 */
export function inferPromotedToolNames(taskContext, toolNames) {
 ...
 */
function computeKeywordRelevance(keywords, tool) {
 ...
 */
export function defaultRelevanceScore(taskContext, tool) {
...
export class ToolRecommendService {
    fullExposureThreshold;
    recallLimit;
    constructor(fullExposureThreshold = DEFAULT_FULL_EXPOSURE_THRESHOLD, recallLimit = DEFAULT_RECALL_LIMIT) {
        this.fullExposureThreshold = fullExposureThreshold;
    ...
    recommendExposure(request) {
        const { taskContext, toolNames, promoteToolNames = [], fullExposureThreshold = this.fullExposureThreshold, recallLimit = this.recallLimit, computeRelevanceScore = defaultRelevanceScore, } = request;
        ...
        const unresolvedToolNames = [
            ...
            ...expandedPromoteTools.unresolvedToolNames.filter((toolName) => !expandedTools.unresolvedToolNames.includes(toolName)),
        ];
        const corrections = [
            ...expandedTools.corrections,
            ...expandedPromoteTools.corrections.filter((candidate) => !expandedTools.corrections.some((existing) => existing.inputToolName === candidate.inputToolName &&
     ...
     */
    getRecallScore(taskContext, toolName) {
...
export class ToolRecommendManager {
    services = new BoundedCache(20);
    /**
     * Gets or creates a recommend service for a session
     */
    getService(sessionId, threshold, recallLimit) {
...
//# sourceMappingURL=tool-recommend-service.js.map