...
export class ExecutionLeaseServiceAsync {
    ...
    acquireLeaseSync(input, occurredAt) {
    ...
    renewLeaseSync(input, occurredAt) {
    ...
    releaseLeaseSync(input, occurredAt) {
        const lease = this.store.worker.getExecutionLease(input.leaseId);
        if (!lease) {
            return {
                outcome: "blocked",
    ...
    validateWriteAccessSync(input, occurredAt) {
        const activeLease = this.store.worker.getActiveExecutionLease(input.executionId);
        if (!activeLease) {
            return {
                allowed: false,
     ...
     */
    reclaimExpiredLeases(occurredAt) {
        return this.db.transaction(() => {
     ...
     */
    async handoverLease(input) {
        const occurredAt = input.occurredAt ?? nowIso();
        return this.db.transaction(() => {
            return this.handoverLeaseSync(input, occurredAt);
        });
    }
    handoverLeaseSync(input, occurredAt) {
...
//# sourceMappingURL=execution-lease-service-async.js.map