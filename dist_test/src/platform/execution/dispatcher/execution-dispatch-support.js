...
export function isRemoteSessionReadyForDispatch(worker) {
    return (worker.placement !== "remote"
...
export function parseJsonArray(value, onError) {
    try {
...
export function resolveDispatchTarget(target) {
    switch (target) {
        case "local_only":
        case "prefer_remote":
        case "require_remote":
...
export function resolveRequiredIsolationLevel(level) {
    switch (level) {
        case "hardened":
        case "strict":
            return level;
...
export function resolveRequiredRepoVersion(value) {
    if (typeof value !== "string")
        return null;
    const normalized = value.trim();
    return normalized.length > 0 ? normalized : null;
}
export function meetsIsolationRequirement(workerIsolationLevel, requiredIsolationLevel) {
    return ISOLATION_LEVEL_ORDER[workerIsolationLevel] >= ISOLATION_LEVEL_ORDER[requiredIsolationLevel];
}
export function resolveRemoteAvailability(dispatchTarget, evaluations) {
    if (dispatchTarget !== "prefer_remote" && dispatchTarget !== "require_remote")
        return null;
    const remoteEvaluations = evaluations.filter((evaluation) => evaluation.placement === "remote" && evaluation.rejectionReason !== "worker_placement_mismatch");
    ...
    if (remoteEvaluations.some((evaluation) => evaluation.rejectionReason === "worker_degraded_filtered"
        || evaluation.rejectionReason === "worker_untrusted"
        || evaluation.rejectionReason === "worker_remote_session_unready")) {
        return "degraded";
    }
    if (remoteEvaluations.every((evaluation) => evaluation.rejectionReason === "worker_unavailable"
...
export function resolveRemoteRepoVersionReason(dispatchTarget, evaluations, requiredRepoVersion) {
    if (requiredRepoVersion == null || (dispatchTarget !== "prefer_remote" && dispatchTarget !== "require_remote"))
        return null;
    const remoteEvaluations = evaluations.filter((evaluation) => evaluation.placement === "remote" && evaluation.rejectionReason !== "worker_placement_mismatch");
    if (remoteEvaluations.length === 0)
        return null;
...
export function resolveRemoteSessionReason(dispatchTarget, evaluations) {
    if (dispatchTarget !== "prefer_remote" && dispatchTarget !== "require_remote")
        return null;
    const remoteEvaluations = evaluations.filter((evaluation) => evaluation.placement === "remote" && evaluation.rejectionReason !== "worker_placement_mismatch");
    if (remoteEvaluations.length === 0)
...
export function resolveRemoteTrustReason(dispatchTarget, evaluations) {
    if (dispatchTarget !== "prefer_remote" && dispatchTarget !== "require_remote")
        return null;
    const remoteEvaluations = evaluations.filter((evaluation) => evaluation.placement === "remote" && evaluation.rejectionReason !== "worker_placement_mismatch");
    if (remoteEvaluations.length === 0)
...
export function selectWorkersForDispatch(dispatchTarget, eligibleWorkers, remoteAvailability, remoteTrustReason, remoteSessionReason, remoteRepoVersionReason) {
    ...
    return {
        ...
        reasonCode: localWorkers.length > 0
            ? (remoteTrustReason
                ?? remoteSessionReason
...
export function toWorkerEvaluation(worker, accepted, rejectionReason, missingCapabilities) {
    return {
...
export function buildDispatchAgentExecutionRecord(store, execution, occurredAt, updates) {
...
export function resolveDispatchBackpressureReason(ticket, snapshot) {
    if (!snapshot)
        return null;
    if (snapshot.degradationMode === "read_only_operations_only")
        return "backpressure.read_only_mode";
    if (snapshot.degradationMode === "pause_non_critical" && !isElevatedPriority(ticket.priority))
        return "backpressure.pause_non_critical";
    ...
    if (snapshot.degradationMode === "queue_only" && !isElevatedPriority(ticket.priority))
        return "backpressure.queue_only";
    return null;
}
export function isElevatedPriority(priority) {
    return priority === "high" || priority === "urgent";
}
//# sourceMappingURL=execution-dispatch-support.js.map