...
export function createConsoleRoutes(deps) {
    return [
        {
            method: "GET",
            pathname: "/console",
        ...
        },
        {
            ...
            handler: (ctx) => {
                const principal = requirePrincipal(ctx.request, deps.authService, "viewer");
                return buildHtmlResponse(buildWorkflowListHtml(deps.missionControlService.listWorkflowCockpits(25, principal.tenantId != null ? principal.tenantId : undefined)));
            },
        },
        {
            method: "GET",
        ...
        },
        {
            ...
            handler: (ctx) => {
                const principal = requirePrincipal(ctx.request, deps.authService, "viewer");
                return buildHtmlResponse(buildApprovalCenterHtml(deps.missionControlService.listApprovalQueue(25, principal.tenantId != null ? principal.tenantId : undefined)));
            },
        },
        {
            method: "GET",
...
function buildMissionControlHtml(snapshot) {
    const firstTaskId = snapshot.taskBoard[0]?.taskId ?? null;
    const taskItems = snapshot.taskBoard
        .map((item) => `<li><a href="/console/tasks/${encodeURIComponent(item.taskId)}">${escapeHtml(item.title)}</a> <strong>${escapeHtml(item.taskStatus)}</strong> <a href="/console/workflows/${encodeURIComponent(item.taskId)}">workflow</a> <a href="/console/admin/tasks/${encodeURIComponent(item.taskId)}">admin</a></li>`)
        .join("");
...
function buildTaskCockpitHtml(cockpit) {
    return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
...
function buildWorkflowListHtml(workflows) {
    return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
  ...
  <body>
    ...
    <ul>
      ${workflows
        .map((workflow) => `<li><a href="/console/workflows/${encodeURIComponent(workflow.taskId)}">${escapeHtml(workflow.workflowId)}</a> :: ${escapeHtml(workflow.workflowStatus)} :: task ${escapeHtml(workflow.taskId)} :: step ${workflow.currentStepIndex}</li>`)
        .join("") || "<li>No workflows available</li>"}
    </ul>
  </body>
</html>`;
}
function buildWorkflowCockpitHtml(cockpit) {
    return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
...
function buildApprovalCenterHtml(approvals) {
    return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
...
function buildStabilityPanelHtml(panel) {
    return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
...
function buildAdminTakeoverHtml(view) {
    return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
...
function buildGatewayTargetsHtml(targets) {
    return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
...
function buildConsoleNav(active) {
    const items = [
...
//# sourceMappingURL=console-routes.js.map