 ...
 */
function mapGatewayTargetRecord(record) {
    return {
 ...
 * @returns Directory entry format
 */
function mapSessionCandidate(candidate) {
    const canonicalTargetId = buildCanonicalTargetId(candidate.channel, "session", normalizeHumanKey(candidate.externalSessionId ?? candidate.sessionId));
 ...
 */
function resolveGatewayTargetFromEntries(entries, query) {
 ...
 */
function collectMatches(entries, normalizedQuery, mode) {
    const matches = [];
 ...
 */
function matchExact(entry, normalizedQuery) {
 ...
 * @returns Resolution if matched, null otherwise
 */
function matchPrefix(entry, normalizedQuery) {
 ...
 */
function compareGatewayTargets(left, right) {
...
function filterEntries(entries, query) {
    ...
    const normalizedQuery = normalizeHumanKey(query);
    return entries.filter((entry) => normalizeHumanKey(entry.targetId).includes(normalizedQuery)
 ...
 */
function buildLatestMessagePreview(latestMessage) {
 ...
 */
function buildCanonicalTargetId(channel, targetKind, rawTargetId) {
...
//# sourceMappingURL=gateway-target-directory-service.js.map