 ...
 */
export const REQUIRED_LAYER_NAMES = ["bootstrap", "gateways", "providers", "runtime", "security", "workflows"];
/**
 * Schema for the bootstrap configuration layer.
 * Contains fundamental application settings.
 */
export const BOOTSTRAP_LAYER_SCHEMA = {
    kind: "object",
 ...
 */
export const GATEWAYS_LAYER_SCHEMA = {
    kind: "object",
 ...
 */
export const PROVIDERS_LAYER_SCHEMA = {
    kind: "object",
 ...
 */
export const RUNTIME_LAYER_SCHEMA = {
    kind: "object",
 ...
 */
export const SECURITY_LAYER_SCHEMA = {
    kind: "object",
 ...
 */
export const WORKFLOWS_LAYER_SCHEMA = {
    kind: "object",
 ...
 */
export function parseJsonObject(raw, filePath) {
    let parsed;
    try {
...
export function normalizeJsonc(raw) {
    const withoutComments = stripJsonComments(raw);
    return stripTrailingCommas(withoutComments);
}
export function validateLayerSchema(layer, schema, issues) {
    if (layer === undefined) {
        return;
    }
    validateConfigField(layer, schema, issues);
}
export function validateConfigField(value, schema, issues) {
    if (value === undefined) {
        if (!schema.optional) {
...
export function addConfigIssue(issues, issue) {
...
export function stripTrailingCommas(raw) {
    let output = "";
    let index = 0;
    let inString = false;
    let stringQuote = "\"";
...
export function mergeConfigObjects(base, overlay) {
    const merged = { ...base };
...
export function diffObjects(before, after, prefix = "") {
    ...
    if (isPlainObject(before) && isPlainObject(after)) {
        ...
        return [...keys]
            .sort()
            .flatMap((key) => diffObjects(before[key], after[key], prefix.length > 0 ? `${prefix}.${key}` : key));
    }
...
//# sourceMappingURL=config-governance-support.js.map