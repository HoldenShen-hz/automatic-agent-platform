...
import { DEFAULT_CONFIG_ROOT, DEFAULT_REPO_ROOT, LocalReleasePipelineCommandRunner, ROTATION_GUARDED_ENVIRONMENTS, buildExecutionMarkdown, buildMarkdown, extractWorkflowDispatchReceipt, sanitizeConfigBundleRef, sanitizeCommitSha, sanitizeImageRepository, sanitizeRegistry, sanitizeSecretRef, sanitizeVersion, } from "./release-pipeline-support.js";
export class ReleasePipelineService {
    ...
    async buildBundle(input) {
        ...
        const registryCredentialRef = sanitizeSecretRef(config.registryCredentialRef, "release.invalid_registry_credential_ref");
        const deploymentCredentialRef = sanitizeSecretRef(config.deploymentCredentialRef, "release.invalid_deployment_credential_ref");
    ...
    async executeAndExport(input) {
        ...
        const { jsonArtifact: bundleJsonArtifact, markdownArtifact: bundleMarkdownArtifact } = this.persistBundle(bundle, persistedTaskId);
     ...
     */
    persistExecutionReport(report, persistedTaskId, jsonArtifactUri, markdownArtifactUri) {
        if (this.store == null) {
            return;
        }
        const record = {
     ...
     */
    requireEnvironmentConfig(environment) {
     ...
     */
    async assertManagedSecretReady(environment, secretRef, usage) {
        if (this.secretManagementService == null) {
            return;
        }
        ...
        if (ROTATION_GUARDED_ENVIRONMENTS.has(environment) &&
     ...
     */
    persistBundle(bundle, persistedTaskId) {
        const taskId = persistedTaskId ?? "release_pipeline";
     ...
     */
    async executeBundle(bundle, persistedTaskId) {
        if (this.secretManagementService == null) {
            throw new ValidationError("release.secret_management_required_for_execute", "release.secret_management_required_for_execute", {
                retryable: false,
            });
        ...
        try {
            ...
            if (registryLease != null) {
                const revoked = await this.secretManagementService.revokeSecretLease({
                    ...
                    reasonCode: buildExitCode == null || buildExitCode !== 0
                        ? "build_failed"
     ...
     */
    async issueRegistryLease(input) {
        if (this.secretManagementService == null) {
            throw new ValidationError("release.secret_management_required_for_execute", "release.secret_management_required_for_execute", {
                retryable: false,
            });
...
//# sourceMappingURL=release-pipeline-service.js.map