...
import { executionTerminalForTask, normalizeInputJson, normalizeJson, parseOutputs, resolveManualStepOutputSummary, resolveWorkflowStepTarget, serializeSnapshot, sessionTerminalForTask, throwTakeoverStorageError, throwTakeoverWorkflowError, workflowTerminalForTask, } from "./human-takeover-support.js";
/**
...
export class HumanTakeoverService {
    ...
    modifyInput(input) {
        return this.recordAction(input.takeoverSessionId, "modify_input", input.reasonCode, (snapshot, session, now) => {
            this.store.task.updateTaskInput(snapshot.task.id, input.inputJson, input.normalizedInputJson ?? normalizeInputJson(input.inputJson), now);
            return {
                payload: {
    ...
    retryExecution(input) {
        return this.recordAction(input.takeoverSessionId, "retry_execution", input.reasonCode, (snapshot, _session, now) => {
            ...
            this.store.execution.updateExecutionStatus(previousExecution.id, "superseded", now, null, now, "takeover.retry_execution");
    ...
    writeStepOutput(input) {
        return this.recordAction(input.takeoverSessionId, "write_step_output", input.reasonCode, (snapshot, _session, now) => {
            ...
            const parsedOutput = JSON.parse(normalizedOutputJson);
    ...
    completeTask(input) {
        return this.recordAction(input.takeoverSessionId, "complete_task", input.reasonCode, (snapshot, session, now) => {
            ...
            if (snapshot.execution) {
                this.store.execution.updateExecutionStatus(snapshot.execution.id, executionTerminal, now, null, now, input.terminalStatus === "failed" ? input.reasonCode : null);
            }
            this.store.approval.closeTakeoverSession(session.id, now);
     ...
     */
    recordAction(takeoverSessionId, actionType, reasonCode, mutate, tenantId) {
        ...
        const operatorActionId = newId("opact");
        const now = nowIso();
        let executionId = before.execution?.id ?? null;
        this.db.transaction(() => {
     ...
     */
    requireOpenSession(takeoverSessionId, tenantId) {
...
//# sourceMappingURL=human-takeover-service.js.map