...
export class TenantExecutionIsolationService {
    ...
    defineQuota(input) {
        ...
        this.db.connection
            .prepare(`INSERT OR REPLACE INTO tenant_quotas (id, tenant_id, quota_kind, limit_value, window_seconds, enforcement_action, enabled, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    ...
    recordResourceUsage(usage) {
        this.db.connection
            .prepare(`INSERT OR REPLACE INTO execution_resource_usage (id, execution_id, tenant_id, cpu_ms, memory_bytes, network_bytes, duration_ms, recorded_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
        ...
        if (quota) {
            this.recordQuotaSample(usage.tenantId, "total_compute_minutes", usage.durationMs / 1000 / 60, quota.windowSeconds, usage.recordedAt);
        }
    }
    /**
     * Records a single quota usage sample for rate-based quota tracking.
     */
    recordQuotaSample(tenantId, quotaKind, value, windowSeconds = 60, recordedAt = nowIso()) {
        ...
        this.db.connection
            .prepare(`INSERT INTO quota_usage_samples (id, tenant_id, quota_kind, sample_value, window_start, recorded_at)
         VALUES (?, ?, ?, ?, ?, ?)`)
     ...
     */
    getQuotaUsage(tenantId, quotaKind) {
        ...
        const samples = this.db.connection
            ...
            .get(tenantId, quotaKind, windowStart);
        ...
        if (percentUsed >= 100)
            status = "exceeded";
        else if (percentUsed >= this.quotaCriticalPercent)
            status = "critical";
        else if (percentUsed >= this.quotaWarningPercent)
            status = "warning";
        else
            status = "ok";
        return {
    ...
    getIsolationStatus(tenantId) {
        ...
        for (const quota of quotas) {
            ...
            if (usage) {
                ...
                else if (overallStatus !== "quota_exceeded" && usage.status === "critical")
        ...
        const signals = this.db.connection
            ...
            .all(tenantId, new Date(Date.now() - 300_000).toISOString());
        for (const signal of signals) {
            const severity = String(signal.severity);
            const cnt = Number(signal.cnt);
            if (severity === "critical")
                noisyNeighborScore += cnt * 30;
            else if (severity === "high")
                noisyNeighborScore += cnt * 20;
            else if (severity === "medium")
                noisyNeighborScore += cnt * 10;
            else
                noisyNeighborScore += cnt * 5;
        }
     ...
     */
    checkQuota(tenantId, quotaKind, incrementBy = 1) {
        const quota = this.getQuota(tenantId, quotaKind);
     ...
     */
    recordNoisyNeighborSignal(tenantId, signalType, severity, metadata) {
        this.db.connection
            .prepare(`INSERT INTO noisy_neighbor_signals (id, tenant_id, signal_type, severity, detected_at, metadata)
         VALUES (?, ?, ?, ?, ?, ?)`)
     ...
     */
    resolveNoisyNeighborSignal(signalId) {
        this.db.connection
     ...
     */
    listActiveNoisyNeighborSignals(tenantId) {
     ...
     */
    purgeOldSamples(olderThanSeconds = 86400) {
     ...
     */
    purgeResolvedSignals(olderThanSeconds = 86400) {
        const cutoff = new Date(Date.now() - olderThanSeconds * 1000).toISOString();
        this.db.connection
    ...
    mapQuota(row) {
        return {
            ...
            quotaKind: String(row.quota_kind ?? "executions_per_minute"),
            limitValue: Number(row.limit_value ?? 0),
            windowSeconds: Number(row.window_seconds ?? 60),
            enforcementAction: String(row.enforcement_action ?? "log_only"),
...
//# sourceMappingURL=tenant-execution-isolation-service.js.map