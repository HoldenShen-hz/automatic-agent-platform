 ...
 */
export const DEFAULT_SIX_LAYER_TRANSITION_RULES = [
 ...
 */
export function mapScopeToSixLayer(scope) {
    switch (scope) {
        case "task_runtime":
        case "working":
            return "working";
 ...
 */
export function mapLayerToScope(layer) {
    switch (layer) {
        case "working":
            return "working";
        case "session":
 ...
 */
export function getNextLayer(current) {
    switch (current) {
        case "working":
            return "session";
        case "session":
 ...
 */
export function getPreviousLayer(current) {
    switch (current) {
        case "working":
            return null;
        case "session":
...
export class LayerTransitionService {
    rules;
    constructor(rules = DEFAULT_SIX_LAYER_TRANSITION_RULES) {
        this.rules = rules;
    }
    /**
     * Evaluates whether a memory can transition to the next layer
     */
    evaluateTransition(memory, evaluatedAt = new Date().toISOString()) {
     ...
     */
    createTransitionRecord(memory, evaluation) {
        if (!evaluation.canTransition || evaluation.targetLayer === null) {
            return null;
        }
        const currentLayer = mapScopeToSixLayer(memory.scope);
...
//# sourceMappingURL=layer-transition-service.js.map