...
function toKnowledgeHit(result, archive) {
    ...
    if (chunkId != null) {
        ...
        if (record) {
            return [{
                    ...
                    documentId: record.record.document.documentId,
                    score: obj.score ?? 1,
                    matchType: obj.matchType ?? "semantic",
...
function mergeHits(localHits, pluginHits, limit) {
...
export class KnowledgePlaneService {
    ...
    semanticVectorStore;
    eventPublisher;
    pendingSemanticSync = Promise.resolve();
    semanticSyncScheduled = false;
    constructor(options = {}) {
        ...
        this.retrieval = new KnowledgeRetrievalService(index, this.archive, namespaces, this.semanticGraph, this.semanticVectorStore, options.knowledgeAuditLogger ?? new KnowledgeAuditLogger());
    ...
    async queryForDomain(keyword, options) {
        const startedAt = Date.now();
        try {
    ...
    scheduleSemanticUpsertRecords(records) {
        if (!this.semanticVectorStore) {
            return;
        }
        ...
        this.pendingSemanticSync = this.pendingSemanticSync
            .then(async () => {
            await this.semanticVectorStore?.upsertChunks(records.flatMap((record) => (record.chunks
    ...
    async awaitSemanticSync() {
        await this.pendingSemanticSync;
    }
}
//# sourceMappingURL=knowledge-plane-service.js.map