 ...
 */
export const EVENT_SCHEMA_REGISTRY = Object.fromEntries(Object.entries(RAW_EVENT_SCHEMA_REGISTRY).map(([type, schema]) => [
    type,
    {
        ...schema,
 ...
 */
export function hasEventSchema(type) {
 ...
 */
export function getRegisteredConsumers(type) {
    if (!hasEventSchema(type)) {
        return [];
    }
    return EVENT_SCHEMA_REGISTRY[type].consumers;
}
/**
...
export function getEventSchema(type) {
    ...
    return EVENT_SCHEMA_REGISTRY[type];
}
/**
...
export function validateEventPayload(type, payload) {
    getEventSchema(type);
    const validator = EVENT_PAYLOAD_VALIDATORS[type] ?? genericEventPayloadSchema;
 ...
 */
function buildPayloadSchemaRef(type) {
