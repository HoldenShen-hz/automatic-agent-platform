...
import { WORKER_TELEMETRY_HEARTBEAT_SQL, WORKER_RESTART_SEMANTICS_SQL, AGENT_EXECUTION_RECORD_SQL, REMOTE_FALLBACK_ROUTING_SQL, WORKER_ISOLATION_ROUTING_SQL, MESSAGE_PARTS_SQL, REMOTE_REPO_VERSION_ROUTING_SQL, REMOTE_SESSION_TELEMETRY_SQL, REMOTE_LOG_AGGREGATION_SQL, TRUSTED_REMOTE_WORKER_REGISTRATION_SQL, EVENT_SESSION_ID_SQL, } from "./sqlite-migration-runtime-part1.js";
import { REMOTE_WORKSPACE_SYNC_TELEMETRY_SQL, SECRET_MANAGEMENT_FOUNDATION_SQL, TIER1_AUDIT_EVENT_INTEGRITY_SQL, MEMORY_SCOPE_AND_QUALITY_SQL, EVOLUTION_MVP_SQL, EXPERIENCE_CACHE_SQL, PMF_VALIDATION_REPORTS_SQL, BILLING_FOUNDATION_SQL, PERCEPTION_MVP_SQL, GATEWAY_TARGET_DIRECTORY_SQL, ENTERPRISE_FOUNDATION_SQL, MARKETPLACE_GOVERNANCE_SQL, } from "./sqlite-migration-runtime-part2.js";
import { TENANT_DATA_NAMESPACE_FOUNDATION_SQL, DATA_PLANE_FLOW_FOUNDATION_SQL, RELEASE_DEPLOYMENT_LEDGER_SQL, SECRET_LEASES_SQL, RELEASE_EXECUTION_REPORTS_SQL, WORKFLOW_DISPATCH_RECEIPT_AUDIT_SQL, LLM_EVAL_AND_GOVERNANCE_FOUNDATION_SQL, ENTERPRISE_GOVERNANCE_FOUNDATION_SQL, CONTROL_PLANE_LOAD_BALANCING_FOUNDATION_SQL, SKILL_GOVERNANCE_FOUNDATION_SQL, TASK_TENANT_SCOPE_SQL, BILLING_COLLECTION_FOUNDATION_SQL, PRODUCT_GOVERNANCE_TENANT_SCOPE_SQL, MEMORY_ENHANCEMENT_SQL, EVENT_DEAD_LETTERS_SQL, SESSION_EVENTS_SQL, } from "./sqlite-migration-runtime-part3.js";
 ...
 */
function defineMigration(version, name, sql, options = {}) {
    const normalizedSql = normalizeSql(sql);
    const checksum = checksumSql(normalizedSql);
    const downSql = normalizeSql(options.downSql
        ?? `-- down migration placeholder for ${name}\nSELECT 'manual rollback required for ${name}' AS rollback_notice;`);
    const compatibleChecksums = Array.from(new Set((options.compatibleSql ?? [])
 ...
 */
export function getLatestSqliteMigrationVersion() {
    return SQLITE_MIGRATIONS.at(-1)?.version ?? 0;
}
//# sourceMappingURL=sqlite-migration-plan.js.map