...
function buildSummaryText(rows, terminalStatus) {
...
export function maybeCreateTerminalSessionSummary(connection, sessionId, terminalStatus, createdAt) {
    ...
    const messageRows = connection
        .prepare(`SELECT direction, message_type AS messageType, content
       FROM messages
       WHERE session_id = ?
    ...
    connection
        ...
        .run(newId("summ"), sessionId, session.taskId, null, summaryText, null, JSON.stringify([`session_${terminalStatus}`]), null, estimateTextTokens(summaryText), createdAt);
}
//# sourceMappingURL=session-summary-autogen.js.map