...
export class DispatchRepository {
    ...
    listExecutionsByStatuses(statuses) {
        if (statuses.length === 0)
            return [];
        const placeholders = statuses.map(() => "?").join(", ");
        return queryAll(this.conn, `SELECT ${EXECUTION_COLS_NO_PREFIX}
       FROM executions
       WHERE status IN (${placeholders})
       ORDER BY updated_at ASC, created_at ASC, id ASC`, ...statuses);
    }
    /** Get an execution by ID with optional tenant scoping. */
    getExecution(executionId, tenantId) {
        const scopedTenantId = resolveTenantScope(tenantId);
        if (scopedTenantId !== undefined) {
            return queryOne(this.conn, `SELECT ${EXECUTION_COLS}
         FROM executions e
         INNER JOIN tasks t ON t.id = e.task_id
         WHERE e.id = ? AND t.tenant_id = ?`, executionId, scopedTenantId) ?? null;
        }
        return queryOne(this.conn, `SELECT ${EXECUTION_COLS_NO_PREFIX}
       FROM executions WHERE id = ?`, executionId) ?? null;
    }
    /** Get an execution precheck by execution ID. */
    getExecutionPrecheck(executionId, tenantId) {
        const scopedTenantId = resolveTenantScope(tenantId);
        if (scopedTenantId !== undefined) {
            return queryOne(this.conn, `SELECT ep.id, ep.execution_id AS executionId, ep.allowed, ep.reason_code AS reasonCode,
         ...
         INNER JOIN tasks t ON t.id = e.task_id
         WHERE ep.execution_id = ? AND t.tenant_id = ?`, executionId, scopedTenantId) ?? null;
        }
        return queryOne(this.conn, `SELECT id, execution_id AS executionId, allowed, reason_code AS reasonCode,
       ...
       FROM execution_prechecks WHERE execution_id = ?`, executionId) ?? null;
    }
    /** Get a dead letter record by execution ID. */
    getDeadLetterByExecutionId(executionId, tenantId) {
        const scopedTenantId = resolveTenantScope(tenantId);
        if (scopedTenantId !== undefined) {
            return queryOne(this.conn, `SELECT d.id, d.execution_id AS executionId, d.task_id AS taskId,
         ...
         WHERE d.execution_id = ? AND t.tenant_id = ?`, executionId, scopedTenantId) ?? null;
        }
        return queryOne(this.conn, `SELECT id, execution_id AS executionId, task_id AS taskId, final_reason_code AS finalReasonCode,
              retry_count AS retryCount, last_error_message AS lastErrorMessage, moved_at AS movedAt
       FROM dead_letters WHERE execution_id = ?`, executionId) ?? null;
    }
    /** List dead letters for a task. */
    listDeadLettersByTask(taskId, tenantId) {
        const scopedTenantId = resolveTenantScope(tenantId);
        if (scopedTenantId !== undefined) {
            return queryAll(this.conn, `SELECT d.id, d.execution_id AS executionId, d.task_id AS taskId,
                d.final_reason_code AS finalReasonCode, d.retry_count AS retryCount,
                d.last_error_message AS lastErrorMessage, d.moved_at AS movedAt
         FROM dead_letters d INNER JOIN tasks t ON t.id = d.task_id
         WHERE d.task_id = ? AND t.tenant_id = ? ORDER BY d.moved_at ASC`, taskId, scopedTenantId);
        }
        return queryAll(this.conn, `SELECT id, execution_id AS executionId, task_id AS taskId, final_reason_code AS finalReasonCode,
              retry_count AS retryCount, last_error_message AS lastErrorMessage, moved_at AS movedAt
       FROM dead_letters WHERE task_id = ? ORDER BY moved_at ASC`, taskId);
    }
    /** Get a session by ID. */
    getSession(sessionId, tenantId) {
        const scopedTenantId = resolveTenantScope(tenantId);
        if (scopedTenantId !== undefined) {
            return queryOne(this.conn, `SELECT s.id, s.task_id AS taskId, s.channel, s.status,
         ...
         WHERE s.id = ? AND t.tenant_id = ?`, sessionId, scopedTenantId) ?? null;
        }
        return queryOne(this.conn, `SELECT id, task_id AS taskId, channel, status, external_session_id AS externalSessionId,
              created_at AS createdAt, updated_at AS updatedAt
       FROM sessions WHERE id = ?`, sessionId) ?? null;
    }
    /** Get the latest session for a task. */
    selectLatestSessionByTask(taskId) {
        return queryOne(this.conn, `SELECT id, task_id AS taskId, channel, status, external_session_id AS externalSessionId,
              created_at AS createdAt, updated_at AS updatedAt
       FROM sessions WHERE task_id = ?
       ORDER BY created_at DESC, updated_at DESC, rowid DESC LIMIT 1`, taskId) ?? null;
    }
    /** Get a gateway target by ID. */
    getGatewayTarget(targetId) {
        return queryOne(this.conn, `SELECT target_id AS targetId, channel, target_kind AS targetKind,
       ...
       FROM gateway_targets WHERE target_id = ?`, targetId) ?? null;
    }
    ...
    listGatewayTargets(limit = 100, channel) {
        ...
        return queryAll(this.conn, sql, ...(channel ? [channel, limit] : [limit]));
    }
    /** List messages for a session. */
    listMessagesBySession(sessionId, tenantId) {
        const scopedTenantId = resolveTenantScope(tenantId);
        if (scopedTenantId !== undefined) {
            return queryAll(this.conn, `SELECT m.id, m.session_id AS sessionId, m.direction, m.message_type AS messageType,
         ...
         WHERE m.session_id = ? AND t.tenant_id = ? ORDER BY m.created_at ASC, m.id ASC`, sessionId, scopedTenantId);
        }
        return queryAll(this.conn, `SELECT id, session_id AS sessionId, direction, message_type AS messageType,
    ...
    getWorkerSnapshot(workerId) {
        return queryOne(this.conn, `SELECT ${WORKER_SNAPSHOT_COLS} FROM worker_snapshots WHERE worker_id = ?`, workerId) ?? null;
    }
}
