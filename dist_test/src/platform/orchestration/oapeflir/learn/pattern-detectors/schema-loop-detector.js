 ...
 */
export function detectSchemaValidationLoop(signals, minOccurrences = 3) {
    // Group failure_pattern signals by stepId
    const stepIdMap = new Map();
    for (const signal of signals) {
        if (signal.learningType !== "failure_pattern")
            continue;
        const stepId = String(signal.evidence.stepId ?? "");
        if (!stepId)
            continue;
    ...
    for (const [_key, group] of stepIdMap) {
        if (group.length >= minOccurrences) {
            ...
            return {
                ...
                taskId: first.taskId,
                stepId: String(first.evidence?.stepId ?? ""),
                ...
                recommendation: "Simplify the output schema for this step, or switch to a more capable model for structured output tasks.",
                detectedAt: Date.now(),
            };
        }
    }
...
//# sourceMappingURL=schema-loop-detector.js.map