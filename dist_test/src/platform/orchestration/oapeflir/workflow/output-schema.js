 ...
 */
function expectPlainObject(value, sourcePath, field) {
 ...
 */
function expectStringArray(value, sourcePath, field) {
 ...
 */
function normalizePropertySchema(value, sourcePath, propertyName) {
    ...
    if (minLengthCandidate != null) {
        if (typeof minLengthCandidate !== "number" || !Number.isInteger(minLengthCandidate) || minLengthCandidate < 0) {
            throw new WorkflowStateError("workflow.output_schema_invalid_document", `Invalid minLength at ${sourcePath}:properties.${propertyName}.minLength`, { details: { sourcePath, propertyName, minLength: minLengthCandidate } });
        }
    }
    return {
        type: "string",
 ...
 */
export function parseWorkflowOutputSchema(raw, sourcePath) {
    let parsed;
    try {
    ...
    const properties = Object.fromEntries(Object.entries(propertiesObject).map(([propertyName, propertySchema]) => [
        propertyName,
 ...
 */
export function loadWorkflowOutputSchema(schemaPath) {
 ...
 */
export function validateWorkflowStepOutput(step, data) {
