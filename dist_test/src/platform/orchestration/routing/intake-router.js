...
export class IntakeRouter {
    ...
    route(input) {
        ...
        routeTrace.push(matchedHints.length > 0
        ...
        routeTrace.push(classification.matchedRules.length > 0
     ...
     */
    selectDivision(normalizedInput, routeTrace) {
        ...
        routeTrace.push(`matched_divisions:${candidates.map((candidate) => `${candidate.division.id}:${candidate.matchedTrigger}`).join(",")}`);
 ...
 */
function findMatchedTrigger(division, normalizedInput) {
    // Expand trigger patterns into individual alternatives
    const alternatives = division.triggers.flatMap((trigger) => trigger
        .split("|")
 ...
 */
function classifyIntent(normalizedInput, matchedHints) {
 ...
 */
function selectIntent(normalizedInput, matchedRulesByIntent) {
 ...
 */
function scoreIntentMatches(matches, normalizedInput, intent) {
 ...
 */
function selectContinuation(normalizedInput, correctionMatches) {
 ...
 */
function shouldRequireOrchestration(normalizedInput, matchedHints, classification) {
 ...
 */
function computeConfidence(normalizedInput, matchedRules, matchedHints, continuation, intent) {
    let confidence = 0.52;
 ...
 */
function findRuleMatches(rules, normalizedInput) {
 ...
 */
function matchesRule(normalizedInput, rule) {
