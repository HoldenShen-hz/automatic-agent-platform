...
import { SINGLE_TASK_GOLDEN_TASKS, buildGoldenTaskInventoryBaseline, runGoldenTaskCase, } from "../../src/platform/shared/stability/golden-task-runner.js";
import { cleanupPath, createTempWorkspace } from "../helpers/fs.js";
test("single-task golden task suite stays stable", async () => {
    const workspace = createTempWorkspace("aa-golden-");
    try {
        const results = await Promise.all(SINGLE_TASK_GOLDEN_TASKS.map((testCase) => runGoldenTaskCase(workspace, testCase)));
...
//# sourceMappingURL=phase1a-golden-tasks.test.js.map