...
import { listEnabledConnectors, summarizeConnectorHealth, selectPreferredRegion, shouldReplicateToRegion, resolveRegionFailover, isQuotaExceeded, orderFairQueue, choosePreemptionVictim, detectSlaBreach, allocateReservedCapacity, resolveHighestPriorityTier, } from "../../../src/scale-ecosystem/index.js";
test("scale-ecosystem support modules coordinate connector, region, quota, and SLA decisions", () => {
    ...
    assert.equal(summarizeConnectorHealth([
        {
            connectorId: "slack_primary",
            status: "healthy",
            latencyMs: 120,
    ...
    assert.equal(shouldReplicateToRegion({
    ...
    assert.equal(isQuotaExceeded({ scopeId: "tenant_b", hardLimit: 5, currentUsage: 4 }, 2), true);
    ...
    assert.deepEqual(detectSlaBreach({ latencyMs: 600, successRate: 0.97, queueWaitMs: 3_500 }, { maxLatencyMs: 300, minSuccessRate: 0.99, maxQueueWaitMs: 1_000 }), ["sla.latency_breach", "sla.success_rate_breach", "sla.queue_wait_breach"]);
    assert.deepEqual(resolveRegionFailover({
