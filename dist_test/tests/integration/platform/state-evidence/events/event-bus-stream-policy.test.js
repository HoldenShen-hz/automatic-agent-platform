...
test("event bus, stream bridge, policy, and approval form a minimal week2 baseline", async () => {
    ...
    try {
        ...
        assert.deepEqual(bridge.replayAfterSequence(streamId, 1).map((frame) => frame.sequence), frames.filter((sequence) => sequence > 1));
        db.close();
    }
    finally {
...
//# sourceMappingURL=event-bus-stream-policy.test.js.map