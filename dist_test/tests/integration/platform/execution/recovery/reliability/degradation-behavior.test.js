    ...
    try {
        ...
        db.transaction(() => {
            for (let i = 0; i < 5; i++) {
                ...
                db.connection
                    .prepare("INSERT INTO events (id, task_id, execution_id, event_type, event_tier, payload_json, trace_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
                    .run(eventId, taskId, null, "task:status_changed", "tier1_reliable", JSON.stringify({ fromStatus: "pending", toStatus: "in_progress", sequence: i }), newId("trace"), now);
            }
        });
...
test("reliability: Multiple concurrent reads don't block each other", () => {
    ...
    try {
        ...
        const readPromises = Array.from({ length: 20 }, () => Promise.resolve(store.listTasks(100)));
...
//# sourceMappingURL=degradation-behavior.test.js.map