...
test("smoke: CLI commands produce output when executed", async () => {
    ...
    try {
        ...
        assert.ok(output.length > 0, "CLI should produce output when executed");
        ...
        assert.ok(isValidJson || output.length > 0, "CLI output should be valid");
    }
    finally {
...
test("smoke: CLI produces error for invalid environment", async () => {
    ...
    try {
        ...
        assert.ok(output.length > 0, "CLI should produce output even when database doesn't exist");
    }
    finally {
        cleanupPath(workspace);
    }
