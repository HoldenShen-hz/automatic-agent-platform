...
test("orphan cleanup service closes orphan sessions, requeues orphan claims, and prunes worker execution refs", () => {
    ...
    try {
        ...
        db.connection.prepare(`UPDATE tasks SET status = ?, completed_at = ? WHERE id = ?`).run("done", "2026-04-07T13:00:01.000Z", "task-orphan-cleanup");
        store.insertSession({
        ...
        store.updateExecutionStatus("exec-worker-terminal-int", "failed", "2026-04-07T13:00:03.000Z", null, "2026-04-07T13:00:03.000Z", "tool_failed");
