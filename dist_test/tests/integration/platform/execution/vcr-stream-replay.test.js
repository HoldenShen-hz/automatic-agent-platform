...
test("multi-step orchestration stream frames can be recorded and replayed through VCR fixtures", async () => {
    ...
    try {
        ...
        const request = {
            ...
            messages: [
                { role: "system", content: "Replay the multi-step orchestration fixture." },
                { role: "user", content: "Summarize the task in detail and create a comprehensive summary document." },
            ],
            tools: ["analysis"],
        ...
        assert.deepEqual(replayed.streamChunks?.map((frame) => frame.sequence), result.streamFrames.map((frame) => frame.sequence));
...
//# sourceMappingURL=vcr-stream-replay.test.js.map