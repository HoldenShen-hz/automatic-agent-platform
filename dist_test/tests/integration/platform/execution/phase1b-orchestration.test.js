...
test("multi-step orchestration runs intake routing, planned multi-step workflow, and streaming frames", async () => {
    ...
    try {
        ...
        assert.deepEqual(result.snapshot.stepOutputs.map((item) => item.stepId), ["intake_triage", "conduct_research", "analyze_findings", "final_review"]);
        assert.ok(result.snapshot.events.some((event) => event.eventType === "division:completed"));
        assert.ok(result.streamFrames.some((frame) => frame.eventType === "completed" || frame.eventType === "progress"));
        ...
        assert.deepEqual(JSON.parse(toolResult.partsJson).map((part) => part.partType), ["summary", "artifact_ref", "tool_result"]);
...
test("multi-step orchestration records full role permissions and deferred visible tools in execution prechecks", async () => {
    ...
    try {
        ...
        for (const execution of engineerExecutions) {
            const allowedTools = JSON.parse(execution.allowedToolsJson ?? "[]");
            const precheck = store.getExecutionPrecheck(execution.id);
            const visibleTools = JSON.parse(precheck?.resolvedToolsJson ?? "[]");
            assert.deepEqual(allowedTools, ["read", "edit_replace", "edit_batch", "apply_patch", "bash"]);
...
test("multi-step orchestration cancels new work when read-only admission backpressure is active", async () => {
    ...
    try {
        ...
        assert.deepEqual(result.snapshot.events.map((event) => event.eventType), ["routing:decided", "workflow:planned", "task:status_changed", "admission:rejected"]);
...
test("multi-step orchestration cascade-skips downstream steps when a hard dependency fails", async () => {
    ...
    try {
        ...
        assert.ok(result.snapshot.events.some((event) => event.eventType === "workflow:step_skipped"));
    }
    finally {
        cleanupPath(workspace);
    }
...
test("multi-step orchestration retries transient failures and still completes the workflow", async () => {
    ...
    try {
        ...
        const store = new AuthoritativeTaskStore(db);
        const executions = store.listExecutionsByTask(result.snapshot.task.id);
        const retryMessages = store.listMessagesBySession(result.snapshot.session.id).filter((message) => message.messageType === "workflow_retry");
...
test("multi-step orchestration escalates destructive failures instead of retrying", async () => {
    ...
    try {
        ...
        assert.equal(result.snapshot.events.some((event) => event.eventType === "workflow:step_retry_scheduled"), false);
        assert.deepEqual(result.snapshot.stepOutputs.map((output) => output.stepId), ["intake_triage"]);
    }
    finally {
        cleanupPath(workspace);
    }
...
test("multi-step orchestration derives queue-only admission backpressure from the local health snapshot by default", async () => {
    ...
    try {
        ...
        assert.deepEqual(result.snapshot.events.map((event) => event.eventType), ["routing:decided", "workflow:planned", "admission:queued"]);
...
//# sourceMappingURL=phase1b-orchestration.test.js.map