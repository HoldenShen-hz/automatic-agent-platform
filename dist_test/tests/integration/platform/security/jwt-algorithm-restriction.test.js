...
test("security: JWT with alg=none is rejected", () => {
    ...
    assert.throws(() => service.authenticate({ authorization: `Bearer ${maliciousToken}` }), (err) => {
...
test("security: JWT with non-whitelisted algorithm is rejected", () => {
    ...
    assert.throws(() => service.authenticate({ authorization: `Bearer ${tokenWithRsa}` }), (err) => {
...
test("security: malformed JWT is rejected", () => {
    ...
    assert.throws(() => service.authenticate({ authorization: "Bearer not-a-valid-jwt" }), (err) => {
...
test("security: JWT without Bearer prefix throws error", () => {
    ...
    assert.throws(() => service.authenticate({ authorization: "some-token-without-bearer" }), (err) => {
...
test("security: neither JWT nor API key throws auth_required", () => {
    ...
    assert.throws(() => service.authenticate({}), (err) => {
...
test("security: role requirement is enforced", () => {
    ...
    assert.throws(() => service.requireRole({}, "admin"), (err) => {
        return err?.code === "api.auth_required";
    }, "No credentials should throw auth_required for requireRole");
});
//# sourceMappingURL=jwt-algorithm-restriction.test.js.map