...
test("edit batch service blocks writes outside the workspace sandbox", () => {
    ...
    harness.db.connection.prepare(`UPDATE executions SET allowed_tools_json = ? WHERE id = ?`).run(JSON.stringify(["edit_replace", "edit_batch"]), "exec-edit-security");
    try {
        const blockedPath = join(outside, "blocked.ts");
        createFile(blockedPath, "const blocked = true;\n");
...
test("edit replacement service blocks writes outside execution path scope even inside the sandbox", () => {
    ...
    harness.db.connection.prepare(`UPDATE executions SET allowed_paths_json = ? WHERE id = ?`).run(JSON.stringify([allowedDir]), "exec-edit-security");
    try {
...
test("edit replacement service blocks execution-level tool permissions before sandbox-safe writes", () => {
    ...
    harness.db.connection.prepare(`UPDATE executions SET allowed_tools_json = ? WHERE id = ?`).run(JSON.stringify(["read"]), "exec-edit-security");
    try {
...
test("edit batch service blocks execution-level tool permissions before sandbox-safe writes", () => {
    ...
    harness.db.connection.prepare(`UPDATE executions SET allowed_tools_json = ? WHERE id = ?`).run(JSON.stringify(["edit_replace"]), "exec-edit-security");
    try {
        createFile(filePath, "const blocked = true;\n");
        const result = harness.service.executeBatch({
...
test("edit replacement service fail-closes malformed execution allowlists before sandbox-safe writes", () => {
    ...
    harness.db.connection.prepare(`UPDATE executions SET allowed_tools_json = ?, allowed_paths_json = ? WHERE id = ?`).run(JSON.stringify(["edit_replace", 1]), JSON.stringify(["", 2]), "exec-edit-security");
    try {
        createFile(filePath, "const blocked = true;\n");
        const result = harness.service.execute({
...
//# sourceMappingURL=sandbox-edit-replacement-service.test.js.map