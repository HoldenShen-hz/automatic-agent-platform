...
test("http api server supports api-key token exchange, gateway delivery, and control-plane selection", async () => {
    ...
    try {
        ...
        const operatorTokenPayload = readJson(operatorToken);
        ...
        const deliveryPayload = readJson(delivery);
        ...
        const globalAdminTokenPayload = readJson(globalAdminToken);
        ...
        const summaryPayload = readJson(summary);
        ...
        const tenantAdminTokenPayload = readJson(tenantAdminToken);
...
//# sourceMappingURL=http-api-server-auth-gateway-load.test.js.map