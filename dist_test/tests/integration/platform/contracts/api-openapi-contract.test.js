...
test("contract: all listed routes exist in OpenAPI document", () => {
    ...
    for (const route of routes) {
        assert.ok(document.paths[route.path], `Route ${route.path} should exist in OpenAPI document`);
        assert.ok(document.paths[route.path][route.method.toLowerCase()], `Method ${route.method} for ${route.path} should exist`);
    }
});
test("contract: each path has at least one valid HTTP method", () => {
    ...
    for (const [path, methods] of Object.entries(document.paths)) {
        ...
        for (const method of methodKeys) {
            assert.ok(["get", "post", "put", "delete", "patch"].includes(method.toLowerCase()), `Method ${method} for ${path} should be a valid HTTP method`);
        }
    }
});
    ...
    for (const [path, methods] of Object.entries(document.paths)) {
        for (const [method, spec] of Object.entries(methods)) {
            if (["get", "post", "put", "delete", "patch"].includes(method.toLowerCase())) {
                assert.ok(spec.summary && spec.summary.length > 0, `Method ${method} for ${path} should have a summary`);
            }
        }
    }
});
test("contract: each method has response definitions", () => {
    ...
    for (const [path, methods] of Object.entries(document.paths)) {
        for (const [method, spec] of Object.entries(methods)) {
            if (["get", "post", "put", "delete", "patch"].includes(method.toLowerCase())) {
                assert.ok(spec.responses, `Method ${method} for ${path} should have responses`);
                assert.ok(spec.responses["200"], `Method ${method} for ${path} should have 200 response defined`);
            }
        }
    }
});
test("contract: routes list matches paths in document", () => {
    ...
    for (const routePath of routePaths) {
        assert.ok(documentPaths.has(routePath), `Route path ${routePath} should exist in document`);
    }
    // Every document path should have at least one route
    for (const docPath of documentPaths) {
        assert.ok(routePaths.has(docPath), `Document path ${docPath} should have a corresponding route`);
    }
});
test("contract: listApiRoutes returns unique routes", () => {
    ...
    for (const route of routes) {
        ...
        assert.ok(!seen.has(key), `Duplicate route found: ${key}`);
        seen.add(key);
    }
});
test("contract: all routes have required fields", () => {
    const routes = listApiRoutes();
    for (const route of routes) {
        assert.ok(route.method === "GET" || route.method === "POST", `Route ${route.path} should have GET or POST method`);
        assert.ok(route.path.startsWith("/"), `Route path ${route.path} should start with /`);
        assert.ok(route.summary.length > 0, `Route ${route.path} should have a summary`);
        assert.ok(Array.isArray(route.tags) && route.tags.length > 0, `Route ${route.path} should have at least one tag`);
    }
});
//# sourceMappingURL=api-openapi-contract.test.js.map