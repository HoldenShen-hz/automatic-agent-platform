...
test("contract: event payload JSON is valid and parseable", () => {
    ...
    try {
        ...
        for (const payload of payloads) {
            ...
            assert.ok(typeof parsed === "object" && parsed !== null, `Payload for ${payload.type} should be a valid JSON object`);
        }
        db.close();
    }
    finally {
...
test("contract: event tier values are valid", () => {
    ...
    try {
        ...
        for (const tier of tiers) {
            ...
            assert.equal(retrieved.eventTier, tier, `Event tier should be ${tier}`);
        }
        db.close();
    }
    finally {
...
//# sourceMappingURL=event-payload-consumer-contract.test.js.map