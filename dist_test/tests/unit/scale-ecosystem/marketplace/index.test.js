...
import { assertIdentifier, assertPositiveNumber, roundCurrency, monthWindow, PackSecurityService, } from "../../../../src/scale-ecosystem/marketplace/index.js";
...
test("assertIdentifier throws on empty string", () => {
    assert.throws(() => assertIdentifier("", "test.code"), /test.code/);
});
...
test("assertPositiveNumber throws on zero", () => {
    assert.throws(() => assertPositiveNumber(0, "test.code"), /test.code/);
});
test("assertPositiveNumber throws on negative", () => {
    assert.throws(() => assertPositiveNumber(-1, "test.code"), /test.code/);
});
