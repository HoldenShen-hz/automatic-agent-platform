...
test("scale-ecosystem support modules provide contract-aligned helpers", () => {
    assert.deepEqual(analyzeFeedbackSignals([
        {
            signalId: "sig_1",
            taskId: "task_1",
            source: "user",
    ...
    assert.deepEqual(summarizeImprovementTracking([
    ...
    assert.equal(listEnabledConnectors([
    ...
    assert.equal(summarizeConnectorHealth([{ connectorId: "slack", status: "degraded", latencyMs: 100, checkedAt: "2026-04-20T00:00:00.000Z" }]), "degraded");
    assert.equal(sortMarketplaceCatalog([
        { listingId: "l1", title: "Sandbox", trustLevel: "sandboxed", lifecycleState: "published" },
        { listingId: "l2", title: "Enterprise", trustLevel: "enterprise", lifecycleState: "published" },
    ])[0]?.listingId, "l2");
    assert.equal(isMarketplaceListingCertified({ listingId: "l1", certificationId: "c1", status: "approved", approvedAt: "2026-04-20T00:00:00.000Z" }), true);
    assert.equal(canPublisherReleaseArtifact({ publisherId: "pub_1", displayName: "Publisher", trustLevel: "verified", allowedArtifactTypes: ["plugin"] }, "plugin"), true);
    assert.equal(shouldReplicateToRegion({ sourceRegionId: "cn-sh", targetRegionIds: ["cn-bj"], residencyMode: "same_jurisdiction" }, "cn-bj"), true);
    assert.deepEqual(resolveRegionFailover({ primaryHealthy: false, candidateRegionIds: ["us-west-2"] }), { shouldFailover: true, targetRegionId: "us-west-2" });
    assert.equal(selectPreferredRegion([
    ...
    assert.deepEqual(detectSlaBreach({ latencyMs: 500, successRate: 0.98, queueWaitMs: 3000 }, { maxLatencyMs: 300, minSuccessRate: 0.99, maxQueueWaitMs: 1000 }), ["sla.latency_breach", "sla.success_rate_breach", "sla.queue_wait_breach"]);
    assert.deepEqual(allocateReservedCapacity(100, [
        { tierId: "enterprise", reservedPercent: 40 },
        { tierId: "standard", reservedPercent: 20 },
    ]), { enterprise: 40, standard: 20 });
    assert.equal(resolveHighestPriorityTier([
