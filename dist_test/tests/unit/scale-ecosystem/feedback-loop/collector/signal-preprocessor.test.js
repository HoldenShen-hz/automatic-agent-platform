...
function createFeedbackBatch(signals = []) {
...
test("mergeCorrelated adds correlatedCategories to payload", () => {
    ...
    const result = preprocessor.mergeCorrelated(signals);
    assert.equal(result.length, 1);
    const categories = (result[0]?.payload).correlatedCategories;
...
test("toLearningSignals infers correct confidence by source", () => {
    ...
    const result = preprocessor.toLearningSignals(feedback);
    const userCorrection = result.find(s => s.sourceSignalIds.some((id) => id.includes("sig_1")));
    const failure = result.find(s => s.sourceSignalIds.some((id) => id.includes("sig_2")));
...
test("toLearningSignals handles partial category with lower confidence", () => {
    ...
    const result = preprocessor.toLearningSignals(feedback);
    const partial = result.find(s => s.sourceSignalIds.some((id) => id.includes("sig_1")));
...
test("toLearningSignals infers confidence 1 for hitl source correction", () => {
    ...
    const result = preprocessor.toLearningSignals(feedback);
    const hitlCorrection = result.find(s => s.sourceSignalIds.some((id) => id.includes("sig_1")));
...
//# sourceMappingURL=signal-preprocessor.test.js.map