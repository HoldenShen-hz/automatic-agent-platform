...
import { isValidLifecycleTransition, canAutoPromote, isTerminalState, listActiveAgents, VALID_LIFECYCLE_TRANSITIONS, } from "../../../../../src/ops-maturity/agent-lifecycle/agent-registry/index.js";
// ---------------------------------------------------------------------------
// Helper - create a minimal agent definition for testing
// ---------------------------------------------------------------------------
function makeAgent(id, state) {
    return {
        agentId: id,
        name: `Agent ${id}`,
        domainId: "domain-1",
...
test("canAutoPromote - only canary returns true", () => {
    const canaryStates = ["canary"];
    const nonCanaryStates = [
        "draft",
        "testing",
        "staging",
        "active",
...
test("VALID_LIFECYCLE_TRANSITIONS contains all states", () => {
    ...
    for (const state of allStates) {
        assert.ok(VALID_LIFECYCLE_TRANSITIONS.has(state), `State ${state} should be in VALID_LIFECYCLE_TRANSITIONS`);
    }
...
test("isValidLifecycleTransition handles unknown states gracefully", () => {
    ...
    assert.equal(isValidLifecycleTransition("unknown-state", "active"), false);
    // @ts-expect-error - testing runtime behavior with invalid input
    assert.equal(isValidLifecycleTransition("canary", "unknown-state"), false);
});
// ---------------------------------------------------------------------------
...
test("Full lifecycle path: draft -> testing -> staging -> canary -> active -> deprecated -> archived", () => {
    ...
    for (let i = 0; i < path.length - 1; i++) {
        const from = path[i];
        const to = path[i + 1];
        assert.equal(isValidLifecycleTransition(from, to), true, `Transition from ${from} to ${to} should be valid`);
    }
});
...
//# sourceMappingURL=agent-registry-comprehensive.test.js.map