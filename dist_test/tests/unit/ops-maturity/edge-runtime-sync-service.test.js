...
test("EdgeRuntimeSyncService executes offline with an allowed local model and explicit sync envelope", () => {
    ...
    const execution = service.executeOffline(profile, [{ modelId: "local-vision", modalities: ["image", "text"] }], {
        edgeNodeId: "edge_factory_1",
    ...
    const envelope = service.buildSyncEnvelope(profile, execution.record, "digest:vision:1", 3, "internal", "2026-04-20T00:05:00.000Z");
...
test("EdgeRuntimeSyncService rejects restricted uploads when sync policy forbids them and resolves conflicts explicitly", () => {
    ...
    const restrictedEnvelope = service.buildSyncEnvelope(profile, record, "digest:restricted", 5, "restricted", "2026-04-20T00:10:00.000Z");
    const conflictEnvelope = service.buildSyncEnvelope(profile, record, "digest:newer", 1, "internal", "2026-04-20T00:11:00.000Z");
...
//# sourceMappingURL=edge-runtime-sync-service.test.js.map