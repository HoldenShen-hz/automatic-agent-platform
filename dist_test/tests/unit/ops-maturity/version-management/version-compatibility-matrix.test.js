...
test("VersionCompatibilityMatrix checks wildcard source compatibility", () => {
    ...
    const result = matrix.checkCompatibility({ packId: "pack-a", version: "2.5.0" }, { packId: "pack-b", version: "1.5.0" });
    assert.equal(result.compatible, true);
...
test("VersionCompatibilityMatrix respects version range patterns", () => {
    ...
    const result1 = matrix.checkCompatibility({ packId: "pack-a", version: "1.5.0" }, { packId: "pack-b", version: "1.5.0" });
    assert.equal(result1.compatible, true);
    // Should not match because 2.0.0 does not satisfy ^1.0.0
    const result2 = matrix.checkCompatibility({ packId: "pack-a", version: "2.0.0" }, { packId: "pack-b", version: "2.0.0" });
...
test("VersionCompatibilityMatrix strict mode can be disabled", () => {
    const matrix = new VersionCompatibilityMatrix({ strictMode: false });
    const result = matrix.checkCompatibility({ packId: "pack-unknown", version: "1.0.0" }, { packId: "pack-other", version: "1.0.0" });
    // In non-strict mode without any rules, should return warning-compatible
    assert.equal(result.compatible, true);
    assert.equal(result.level, "warning");
});
//# sourceMappingURL=version-compatibility-matrix.test.js.map