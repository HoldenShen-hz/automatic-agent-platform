...
test("DR config exists and is valid JSON", () => {
    assert.ok(existsSync(DR_CONFIG_PATH), `DR config should exist at ${DR_CONFIG_PATH}`);
...
test("DR config has valid RTO/RPO thresholds", () => {
    ...
    assert.ok(config.rtoSeconds >= config.rpoSeconds, "RTO should be >= RPO (RTO is typically larger)");
});
...
test("DR config has all required components", () => {
    ...
    for (const comp of requiredComponents) {
        assert.ok(config.components.includes(comp), `Component '${comp}' should be in config.components`);
    }
});
...
test("DR script supports --help option", () => {
    ...
    assert.ok(content.includes("full") && content.includes("incremental") && content.includes("verify"), "Script should document all modes");
});
...
//# sourceMappingURL=dr-drill.test.js.map