...
test("HrRoleGovernanceService analyzeGap returns matched roles and missing capabilities", () => {
    ...
    assert.ok(result.matchedRoleIds.includes("role_coder"), "role_coder should match bash+write capabilities");
...
test("HrRoleGovernanceService analyzeGap throws when division not found", () => {
    const registry = makeMockDivision([]);
    const service = new HrRoleGovernanceService(registry, null);
    assert.throws(() => service.analyzeGap({
        taskId: "task_003",
...
test("HrRoleGovernanceService validateProposal detects duplicate role ID", () => {
    ...
    const service = new HrRoleGovernanceService(registry, null);
    const result = service.validateProposal(makeMinimalProposal({ roleId: "existing_role" }));
...
test("HrRoleGovernanceService validateProposal detects forbidden tools", () => {
    const registry = makeMockDivision([]);
    const service = new HrRoleGovernanceService(registry, null);
    const result = service.validateProposal(makeMinimalProposal({ tools: ["read", "spawn_agent", "send_message"] }));
    assert.equal(result.valid, false, "Forbidden tools should fail validation");
    assert.ok(result.errors.some((e) => e.includes("hr.tool_forbidden")), `Should report hr.tool_forbidden error, got: ${result.errors.join(", ")}`);
});
test("HrRoleGovernanceService validateProposal detects command restriction without boundary", () => {
    ...
    assert.ok(result.errors.some((e) => e.includes("command_boundary_missing")), `Should report command_boundary_missing, got: ${result.errors.join(", ")}`);
});
...
test("HrRoleGovernanceService analyzeGap deduplicates requested capabilities", () => {
    ...
    assert.equal(result.matchedRoleIds.length, 1, "Should handle duplicate and whitespace-varied capabilities");
});
