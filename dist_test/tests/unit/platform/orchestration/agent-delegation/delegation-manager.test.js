...
test("DelegationManagerService rejects depth exceeding limit", async () => {
    ...
    const spec = createDelegationSpec();
    await assert.rejects(async () => service.delegate(parent, spec), DelegationDepthExceededError);
});
test("DelegationManagerService rejects fanout exceeding limit", async () => {
    ...
    const spec = createDelegationSpec();
    await assert.rejects(async () => service.delegate(parent, spec), DelegationFanoutExceededError);
});
test("DelegationManagerService rejects cycle detection", async () => {
    ...
    await assert.rejects(async () => service.delegate(parent, createDelegationSpec({
...
test("DelegationManagerService cannot cancel completed delegation", async () => {
    ...
    const handle = await service.delegate(parent, spec);
    await service.complete(handle.delegationId);
    await assert.rejects(async () => service.cancel(handle.delegationId), Error);
});
...
test("DelegationManagerService with allowedPackIds restricts delegation", async () => {
    ...
    await assert.rejects(async () => service.delegate(parent, spec), Error);
});
...
test("DelegationManagerService cannot fail non-existent delegation", async () => {
    const service = createDelegationManager();
    await assert.rejects(async () => service.fail("non-existent", "error"), Error);
});
test("DelegationManagerService cannot complete non-existent delegation", async () => {
    const service = createDelegationManager();
    await assert.rejects(async () => service.complete("non-existent"), Error);
});
test("DelegationManagerService cannot cancel non-existent delegation", async () => {
    const service = createDelegationManager();
    await assert.rejects(async () => service.cancel("non-existent"), Error);
});
...
test("DelegationManagerService depth validation enforces 3 by default", async () => {
    ...
    await assert.rejects(async () => service.delegate(parentDepth2, spec), DelegationDepthExceededError);
});
//# sourceMappingURL=delegation-manager.test.js.map