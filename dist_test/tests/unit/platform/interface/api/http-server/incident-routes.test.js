...
function createMockContext(pathname = "/v1/incidents", segments = [], body = null) {
    return {
...
test("POST /v1/incidents creates a new incident", async () => {
    ...
    const response = await callRoute(routes, createMockContext("/v1/incidents", ["v1", "incidents"], JSON.stringify({ severity: "critical", title: "Provider outage" })));
    if (!response)
        throw new Error("handler returned null");
    assert.equal(response.statusCode, 201);
    assert.ok(response.body.includes("Provider outage"));
});
//# sourceMappingURL=incident-routes.test.js.map