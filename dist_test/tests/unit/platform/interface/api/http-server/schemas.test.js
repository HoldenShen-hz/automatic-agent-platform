...
import { parseAuthTokenPayload, parseGatewaySendPayload, parseApprovalDecisionPayload, parseBillingReconcilePayload, } from "../../../../../../src/platform/interface/api/http-server/schemas.js";
...
test("parseApprovalDecisionPayload parses option_selected decision", () => {
    const decision = parseApprovalDecisionPayload("approval_123", "user_456", {
...
test("parseApprovalDecisionPayload parses confirmed decision", () => {
    const decision = parseApprovalDecisionPayload("approval_confirmed", "user_confirm", { decisionType: "confirmed" });
...
test("parseApprovalDecisionPayload parses text_input decision", () => {
    const decision = parseApprovalDecisionPayload("approval_text", "user_text", {
...
test("parseApprovalDecisionPayload parses rejected decision", () => {
    const decision = parseApprovalDecisionPayload("approval_rejected", "user_reject", { decisionType: "rejected" });
    assert.equal(decision.decisionType, "rejected");
});
test("parseApprovalDecisionPayload parses expired decision", () => {
    const decision = parseApprovalDecisionPayload("approval_expired", "system", { decisionType: "expired" });
...
test("parseApprovalDecisionPayload uses custom respondedAt when provided", () => {
    ...
    const decision = parseApprovalDecisionPayload("approval_123", "user", { decisionType: "confirmed", respondedAt: customTime });
...
test("parseApprovalDecisionPayload generates respondedAt when not provided", () => {
    ...
    const decision = parseApprovalDecisionPayload("approval_123", "user", { decisionType: "confirmed" });
...
test("parseBillingReconcilePayload parses all gateway kinds", () => {
    for (const kind of ["manual", "stripe", "paddle"]) {
        const payload = parseBillingReconcilePayload({
            gatewayKind: kind,
