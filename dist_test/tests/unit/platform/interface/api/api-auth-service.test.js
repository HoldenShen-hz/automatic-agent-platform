...
test("api auth service rejects expired, tampered, and under-privileged credentials", () => {
    ...
    try {
        ...
        assert.throws(() => service.authenticate({
            authorization: `Bearer ${expiredToken}`,
        }), (error) => error?.code === "api.token_expired"
        ...
        }), (error) => error?.code === "api.invalid_token_signature"
            && error?.statusCode === 401);
    }
    finally {
        Date.now = originalNow;
    }
    assert.throws(() => service.requireRole({
        "x-api-key": "viewer-key",
    }, "admin"), (error) => error?.code === "api.forbidden"
...
test("api auth service errors converge to AppError metadata", () => {
    ...
    assert.throws(() => service.authenticate({}), (error) => error?.code === "api.auth_required"
        && error?.category === "auth"
        && error?.source === "gateway"
        && error?.retryable === false);
});
function createJwtWithValidSignature(headerAlg, payload, secret) {
...
test("api auth service rejects JWT with alg: 'none'", () => {
    ...
    assert.throws(() => service.authenticate({ authorization: `Bearer ${noneAlgToken}` }), (error) => error?.code === "api.unsupported_algorithm"
        && error?.statusCode === 401, "JWT with alg 'none' should be rejected as unsupported");
});
test("api auth service rejects JWT with unsupported algorithm", () => {
    ...
    assert.throws(() => service.authenticate({ authorization: `Bearer ${unsupportedAlgToken}` }), (error) => error?.code === "api.unsupported_algorithm"
        && error?.statusCode === 401, "JWT with unsupported algorithm should be rejected");
});
test("api auth service rejects empty algorithm", () => {
    ...
    assert.throws(() => service.authenticate({ authorization: `Bearer ${emptyAlgToken}` }), (error) => error?.code === "api.unsupported_algorithm"
        && error?.statusCode === 401, "JWT with empty algorithm should be rejected as unsupported");
});
test("api auth service rejects token that is too old via maxTokenAgeMs option", () => {
    ...
    }), (error) => error?.code === "api.token_too_old"
        && error?.statusCode === 401, "JWT issued too long ago should be rejected");
});
//# sourceMappingURL=api-auth-service.test.js.map