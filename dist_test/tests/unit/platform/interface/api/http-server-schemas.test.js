...
import { parseApprovalDecisionPayload, parseAuthTokenPayload, parseBillingReconcilePayload, parseGatewaySendPayload, } from "../../../../../src/platform/interface/api/http-server/schemas.js";
...
test("gateway send payload rejects unknown keys and malformed metadata", () => {
    try {
        ...
        assert.equal(error.code, "api.invalid_gateway_payload:payload");
    }
    try {
        parseGatewaySendPayload({
            text: "hello",
        ...
        assert.equal(error.code, "api.invalid_gateway_payload:metadata");
    }
});
test("approval decision payload enforces conditional fields", () => {
    try {
        ...
        assert.equal(error.code, "api.invalid_decision_payload:selectedOptionId");
    }
...
test("billing reconcile payload validates enums and timestamp shape", () => {
    ...
    try {
        ...
        assert.equal(error.code, "api.invalid_billing_reconcile_payload:occurredAt");
    }
});
//# sourceMappingURL=http-server-schemas.test.js.map