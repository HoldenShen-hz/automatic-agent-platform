...
import { GatewayTargetAmbiguousError, GatewayTargetDirectoryService, GatewayTargetNotFoundError, } from "../../../../../src/platform/interface/channel-gateway/gateway-target-directory-service.js";
...
test("GatewayTargetDirectoryService", async () => {
    ...
    try {
        ...
        assert.throws(() => service.resolveTarget({ query: "fin", channel: "telegram" }), (error) => error instanceof GatewayTargetAmbiguousError);
        assert.throws(() => service.resolveTarget({ query: "unknown-target", channel: "telegram" }), (error) => error instanceof GatewayTargetNotFoundError);
        db.close();
    }
    finally {
        cleanupPath(workspace);
