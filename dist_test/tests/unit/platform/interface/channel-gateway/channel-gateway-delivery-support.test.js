...
import { CHANNEL_DELIVERY_DDL, DEFAULT_DELIVERY_CONFIG, DEFAULT_RATE_LIMIT_CONFIG, calculateBackoffForAttempt, toDeliveryMessageRecord, buildDeadLetterQuery, buildDeadLetterCountQuery, } from "../../../../../src/platform/interface/channel-gateway/channel-gateway-delivery-support.js";
test("DeliveryAttempt status accepts valid values", () => {
    const statuses = ["pending", "success", "failed", "retrying"];
    assert.equal(statuses.length, 4);
});
test("DeliveryReceipt status accepts valid values", () => {
    const statuses = ["delivered", "failed", "pending_retry"];
    assert.equal(statuses.length, 3);
});
test("DeliveryReceipt finalStatus accepts valid values", () => {
    const finalStatuses = ["success", "permanent_failure", "exhausted_retries"];
    assert.equal(finalStatuses.length, 3);
});
test("DeliveryFailureResolution outcome accepts valid values", () => {
    const outcomes = ["retry_scheduled", "dead_lettered"];
...
//# sourceMappingURL=channel-gateway-delivery-support.test.js.map