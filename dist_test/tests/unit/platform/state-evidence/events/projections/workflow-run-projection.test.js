...
import { workflowRunProjectionHandler, createEmptyWorkflowRunState, createWorkflowRunProjectionHandler, } from "../../../../../../src/platform/state-evidence/events/projections/workflow-run-projection.js";
/**
...
test("workflowRunProjectionHandler initializes state correctly", () => {
    ...
    const state = workflowRunProjectionHandler(null, event);
...
test("workflowRunProjectionHandler handles workflow:step_completed", () => {
    const event = makeEvent("evt_step_1", "workflow:step_completed", "task_1", '{"stepId":"step_1","status":"completed"}');
    const state = workflowRunProjectionHandler(null, event);
    assert.deepEqual(state.completedSteps, ["step_1"]);
    assert.equal(state.eventCount, 1);
});
test("workflowRunProjectionHandler handles division:completed", () => {
    const event = makeEvent("evt_div_1", "division:completed", "task_1", '{"divisionId":"div_1","reasonCode":"success"}');
    const state = workflowRunProjectionHandler(null, event);
    assert.equal(state.status, "running");
...
test("workflowRunProjectionHandler handles division:failed", () => {
    ...
    const completedEvent = makeEvent("evt_div_1", "division:completed", "task_1", '{"divisionId":"div_1"}');
    const stateAfterCompleted = workflowRunProjectionHandler(null, completedEvent);
    assert.equal(stateAfterCompleted.status, "running");
    // Now apply division:failed - status should remain running
    const failedEvent = makeEvent("evt_div_fail", "division:failed", "task_1", '{"divisionId":"div_2","reasonCode":"error"}');
    const state = workflowRunProjectionHandler(stateAfterCompleted, failedEvent);
...
test("workflowRunProjectionHandler handles subtask:completed", () => {
    const event = makeEvent("evt_subtask_1", "subtask:completed", "task_1", '{"stepId":"step_sub_1","subtaskId":"subtask_1","status":"completed"}');
    const state = workflowRunProjectionHandler(null, event);
...
test("workflowRunProjectionHandler handles subtask:failed", () => {
    const event = makeEvent("evt_subtask_fail", "subtask:failed", "task_1", '{"stepId":"step_sub_2","subtaskId":"subtask_2","reasonCode":"failed"}');
    const state = workflowRunProjectionHandler(null, event);
...
test("workflowRunProjectionHandler handles task:status_changed to completed", () => {
    const event = makeEvent("evt_status", "task:status_changed", "task_1", '{"fromStatus":"running","toStatus":"completed","reasonCode":"done"}');
    const state = workflowRunProjectionHandler(null, event);
    assert.equal(state.status, "completed");
    assert.ok(state.completedAt !== null);
});
test("workflowRunProjectionHandler handles task:status_changed to failed", () => {
    const event = makeEvent("evt_fail", "task:status_changed", "task_1", '{"fromStatus":"running","toStatus":"failed","reasonCode":"error","reasonDetail":"Something went wrong"}');
    const state = workflowRunProjectionHandler(null, event);
    assert.equal(state.status, "failed");
    assert.ok(state.failedAt !== null);
    assert.ok(state.error !== null);
...
test("workflowRunProjectionHandler is idempotent - same event applied twice", () => {
    ...
    const state1 = workflowRunProjectionHandler(null, event);
    const state2 = workflowRunProjectionHandler(state1, event);
...
test("workflowRunProjectionHandler deduplicates event_ids", () => {
    ...
    const state1 = workflowRunProjectionHandler(null, event);
    const state2 = workflowRunProjectionHandler(state1, event);
    const state3 = workflowRunProjectionHandler(state2, event);
    // Should only count once
...
test("workflowRunProjectionHandler tracks failed steps", () => {
    const event = makeEvent("evt_fail_step", "subtask:failed", "task_1", '{"stepId":"step_bad","subtaskId":"subtask_bad","reasonCode":"error"}');
    const state = workflowRunProjectionHandler(null, event);
...
test("createWorkflowRunProjectionHandler returns handler function", () => {
    ...
    assert.equal(state.workflowId, "task_test");
});
...
test("workflowRunProjectionHandler extracts workflowId from payload when available", () => {
    const event = makeEvent("evt_wf", "workflow:step_completed", "task_1", '{"workflowId":"wf_123","stepId":"step_1"}');
    const state = workflowRunProjectionHandler(null, event);
    assert.equal(state.workflowId, "wf_123");
    assert.equal(state.taskId, "task_1");
});
test("workflowRunProjectionHandler handles task:status_changed to cancelled", () => {
    const event = makeEvent("evt_cancel", "task:status_changed", "task_1", '{"fromStatus":"running","toStatus":"cancelled"}');
    const state = workflowRunProjectionHandler(null, event);
    assert.equal(state.status, "cancelled");
    assert.ok(state.failedAt !== null);
});
test("workflowRunProjectionHandler handles task:status_changed to paused", () => {
    const event = makeEvent("evt_pause", "task:status_changed", "task_1", '{"fromStatus":"running","toStatus":"paused"}');
    const state = workflowRunProjectionHandler(null, event);
...
test("workflowRunProjectionHandler preserves existing state when handling events", () => {
    ...
    const state1 = workflowRunProjectionHandler(null, event1);
    const state2 = workflowRunProjectionHandler(state1, event2);
...
//# sourceMappingURL=workflow-run-projection.test.js.map