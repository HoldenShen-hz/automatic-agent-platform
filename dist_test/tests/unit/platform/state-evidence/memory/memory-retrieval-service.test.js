...
function createMockStore(memories = []) {
    const mockConnection = {
        exec: () => { },
        prepare: (sql) => ({
            run: (..._args) => { },
            get: () => ({ count: memories.length }),
            all: (..._args) => memories.map(m => ({ ...m, rank: -1 })),
        }),
    };
    const mockMemoryRepo = {
        listMemories: (_query) => memories,
    };
    return {
...
test("MemoryRetrievalService.retrieveMemories applies limit and offset to keyword search", () => {
    const memories = Array.from({ length: 20 }, (_, i) => createTestMemory({ id: `mem_${i}`, contentJson: "test content" }));
    const store = createMockStore(memories);
    const service = new MemoryRetrievalService(store);
...
//# sourceMappingURL=memory-retrieval-service.test.js.map