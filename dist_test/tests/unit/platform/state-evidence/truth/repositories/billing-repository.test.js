...
function createTestExecution(db, execId, taskId, now, status) {
...
function insertBillingFixture(repo, now, tenantId = "tenant-billing") {
...
test("BillingRepository updateTaskStatus changes task status", () => {
    ...
    try {
        ...
        const task = repo.getTask("task-status-change");
        assert.ok(task);
...
test("BillingRepository listQueuedTasksByTenant returns tasks ordered by created_at ASC", () => {
    ...
    try {
        ...
        const tasks = repo.listQueuedTasksByTenant("tenant-order");
...
test("BillingRepository updateTaskStatus updates updatedAt timestamp", () => {
    ...
    try {
        ...
        const task = repo.getTask("task-updated-at");
        assert.ok(task);
...
test("BillingRepository usage quota ledger and entitlement queries return latest account records", () => {
    ...
    try {
        ...
        const counter = repo.getQuotaCounter("acct-001", "task_execution", "2026-04-01T00:00:00.000Z", "2026-05-01T00:00:00.000Z");
        ...
        assert.deepEqual(usageEvents.map((event) => event.usageId), ["usage-002", "usage-001"]);
...
test("BillingRepository payment session list respects tenant and status filters", () => {
    ...
    try {
        ...
        assert.deepEqual(tenantAPayments.map((payment) => payment.sessionId), ["pay-001"]);
        assert.deepEqual(failedTenantBPayments.map((payment) => payment.sessionId), ["pay-002"]);
...
//# sourceMappingURL=billing-repository.test.js.map