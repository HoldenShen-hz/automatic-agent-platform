...
function createNamespace(orgRepo, namespaceId = "ns-1", plane = "analytics") {
...
test("OperationsRepository loadTaskSnapshot returns complete snapshot", () => {
    ...
    try {
        ...
        db.transaction(() => {
            ...
            db.connection.prepare(`INSERT INTO workflow_state (
          ...
          last_error_code, retry_count, resumable_from_step, started_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run("task-1", "general_ops", "wf-1", 1, "running", "{}", null, 0, null, now, now);
            ...
            db.connection.prepare(`INSERT INTO workflow_step_outputs (
          id, task_id, step_id, role_id, status, data_json, summary, artifacts_json,
          token_cost, duration_ms, validation_json, produced_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run("stepout-1", "task-1", "step-1", "general_executor", "succeeded", "{}", "ok", "[]", 10, 1000, null, now);
            db.connection.prepare(`INSERT INTO artifacts (
        ...
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run("artifact-1", "task-1", "exec-1", "step-1", "report", "/tmp/report.json", "report.json", "application/json", 12, null, null, now);
        });
...
//# sourceMappingURL=operations-repository.test.js.map