...
import { getLatestPostgresMigrationVersion, POSTGRES_MIGRATIONS, PHASE_1A_SCHEMA_DDL, translateSqliteToPostgresDdl, } from "../../../../../../src/platform/state-evidence/truth/postgres/pg-schema.js";
...
test("migrations are in ascending version order", () => {
    ...
    if (migrations.length < 2)
        return; // Need at least 2 to test ordering
    for (let i = 1; i < migrations.length; i++) {
        ...
        assert.ok(current.version > previous.version, `Migration ${i} version should be greater than previous`);
    }
});
...
test("all PostgreSQL migrations cover all SQLite schema tables and marketplace governance parity", () => {
    ...
    for (const table of expectedTables) {
        assert.ok(allDdl.includes(`CREATE TABLE IF NOT EXISTS ${table}`), `Table ${table} should exist in PostgreSQL schema`);
    }
...
test("each migration DDL uses PostgreSQL types correctly", () => {
    for (const migration of POSTGRES_MIGRATIONS) {
        ...
        assert.ok(ddl.includes("DOUBLE PRECISION") || ddl.includes("INTEGER") || (!ddl.includes("DOUBLE PRECISION") && !ddl.includes("INTEGER")), `Migration ${migration.version} should either use numeric types or be text-only`);
    }
});
//# sourceMappingURL=pg-schema.test.js.map