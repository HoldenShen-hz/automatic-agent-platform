...
test("authoritative task store applies tenant context to task lifecycle queries", () => {
    ...
    try {
        ...
        provideContext({
            ...
            assert.deepEqual(store.listTasks().map((task) => task.id), ["task-tenant-a"]);
            ...
            assert.deepEqual(store.listTaskBoardItems(10).map((item) => item.taskId), ["task-tenant-a"]);
            assert.deepEqual(store.listActiveTasksWithoutWorkflow().map((item) => item.taskId), []);
            assert.deepEqual(store.listWorkflowStates().map((workflow) => workflow.taskId), ["task-tenant-a"]);
            assert.deepEqual(store.listStaleExecutions("9999-12-31T23:59:59.999Z").map((execution) => execution.executionId), ["exec-tenant-a"]);
            assert.deepEqual(store.listRecoverableExecutingRuns("9999-12-31T23:59:59.999Z").map((record) => record.executionId), ["exec-tenant-a"]);
            assert.deepEqual(store.listBlockedRunsAwaitingApproval().map((record) => record.executionId), []);
            assert.deepEqual(store.listStaleRuns("9999-12-31T23:59:59.999Z").map((record) => record.executionId), ["exec-tenant-a"]);
            assert.deepEqual(store.listOrphanSessions().map((record) => record.sessionId), []);
            assert.deepEqual(store.listWorkflowTerminalMismatches().map((record) => record.taskId), []);
            assert.deepEqual(store.listActiveTasksWithTerminalSessions().map((record) => record.taskId), []);
...
test("runtime context functions throw when called outside provideContext scope", () => {
    ...
    assert.throws(() => getContext(), /runtime_context.missing/);
...
//# sourceMappingURL=tenant-context-scope.test.js.map