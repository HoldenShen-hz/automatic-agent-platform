...
import { ScopedExternalAccessSandbox, createScopedExternalAccessSandbox, } from "../../../../../src/platform/execution/plugin-executor/scoped-external-access-sandbox.js";
// ─────────────────────────────────────────────────────────────────────────────
...
test("ScopedExternalAccessSandbox handles special characters in URLs", async () => {
    ...
    const result = await sandbox.validateOutboundRequest("https://api.example.com/path?param=value&another=test");
    assert.equal(result, true);
});
test("ScopedExternalAccessSandbox handles port numbers in URLs", async () => {
    ...
    const result = await sandbox.validateOutboundRequest("https://api.example.com:8080/endpoint");
    assert.equal(result, true);
});
test("ScopedExternalAccessSandbox handles IP addresses as domains", async () => {
    ...
    assert.equal(sandbox.validateOutboundRequest("https://192.168.1.1/api"), true);
    assert.equal(sandbox.validateOutboundRequest("https://192.168.1.2/api"), false);
});
//# sourceMappingURL=scoped-external-access-sandbox.test.js.map