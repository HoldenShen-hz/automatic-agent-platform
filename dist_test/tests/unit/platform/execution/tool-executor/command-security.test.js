...
test("command safety classifier blocks variable expansion and newline command injection syntax", () => {
    const classifier = new CommandSafetyClassifier();
    assert.equal(classifier.assess("echo", ["${HOME}"]).reasonCode, "tool.command_meta_syntax_denied");
    assert.equal(classifier.assess("echo", ["hello\nwhoami"]).reasonCode, "tool.command_meta_syntax_denied");
});
...
//# sourceMappingURL=command-security.test.js.map