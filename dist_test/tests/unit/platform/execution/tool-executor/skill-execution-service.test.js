...
function createService(runner, options = {}) {
...
test("skill execution service records retries, step outputs, and completion evidence", async () => {
    ...
    try {
        ...
        assert.deepEqual(result.steps.map((step) => [step.stepId, step.status, step.retryCount]), [
...
test("skill execution service applies tool metadata timeout defaults and retries timed out steps", async () => {
    ...
    const { workspace, db, inspect, service, store } = createService(async (request) => {
...
test("skill execution service fail-closes when execution allowed tools contain malformed entries", async () => {
    ...
    db.connection.prepare(`UPDATE executions SET allowed_tools_json = ? WHERE id = ?`).run(JSON.stringify(["read", 1]), "exec-skill");
    try {
        await assert.rejects(() => service.execute({
            executionId: "exec-skill",
            skill,
...
test("skill execution service fail-closes when the execution exceeds the tool call ceiling", async () => {
    ...
    try {
        ...
        assert.equal(executionInspect.agentExecution?.lastErrorCode, "agent.resource_limit.tool_calls_exceeded");
        db.close();
    }
    finally {
        cleanupPath(workspace);
...
//# sourceMappingURL=skill-execution-service.test.js.map