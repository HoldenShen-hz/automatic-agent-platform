...
import { isConcurrentSafe, requiresExclusiveExecution, partitionParallelToolGroups, canExecuteInParallel, partitionToolsByExecutionMode, executeToolsInParallel, executeToolItemsInParallel, } from "../../../../../src/platform/execution/tool-executor/tool-parallel-executor.js";
...
test("requiresExclusiveExecution returns true for non-idempotent local_file", () => {
    assert.equal(requiresExclusiveExecution(makeMetadata({ readOnly: false, idempotent: false, sideEffectScope: "local_file" })), true);
});
...
test("executeToolsInParallel length mismatch throws", async () => {
    ...
    const metas = [makeMetadata({ toolName: "t1" })];
    await assert.rejects(async () => executeToolsInParallel(fns, metas), /length_mismatch/);
});
...
//# sourceMappingURL=tool-parallel-executor.test.js.map