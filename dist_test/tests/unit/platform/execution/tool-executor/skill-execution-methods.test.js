...
import { SkillExecutionService, } from "../../../../../src/platform/execution/tool-executor/skill-execution-service.js";
...
function createService() {
    ...
    const service = new SkillExecutionService(db, store, async () => ({ success: true, summary: "ok", output: "ok" }));
...
test("SkillExecutionService resolveModelProfile handles blank, valid, and unknown profiles", () => {
    ...
    try {
        ...
        assert.throws(() => service.resolveModelProfile("missing-profile"), (error) => error instanceof ValidationError && error.code === "skill.model_profile_unknown:missing-profile");
    }
    finally {
        db.close();
...
test("SkillExecutionService resolves cache lookup lifecycle from disabled to miss to hit to expired", () => {
    ...
    try {
        ...
        const stored = service.storeCacheEntry(skill, miss.metadata, [
...
test("SkillExecutionService buildStepOutput embeds cache hit metadata in the serialized payload", () => {
    ...
    try {
        ...
        const stepOutput = service.buildStepOutput("task-1", skill, {
            stepId: "read",
            requestedToolName: "read",
            resolvedToolName: "read",
            status: "succeeded",
        ...
        const validation = JSON.parse(stepOutput.validationJson ?? "{}");
...
//# sourceMappingURL=skill-execution-methods.test.js.map