...
function createDispatchDecision(overrides = {}) {
    return {
        ticketId: "ticket-1",
        executionId: "exec-1",
        taskId: "task-1",
...
test("TaskTimelineService builds a sorted timeline across events, dispatches, artifacts, approvals, and remote logs", () => {
    ...
    assert.deepEqual(timeline.entries.map((entry) => entry.kind), ["event", "dispatch", "step_output", "approval", "artifact", "remote_log", "remote_log"]);
    ...
    assert.deepEqual(dispatchEntry.data.remoteAcceptedWorkerIds ?? [], ["worker-remote-1"]);
...
//# sourceMappingURL=task-timeline-service.test.js.map