...
test("observability retention preview reports eligible tiered events and preserves summaries", () => {
    ...
    try {
        ...
        db.connection.prepare(`UPDATE tasks SET status = ?, completed_at = ? WHERE id = ?`).run("done", "2026-01-01T00:00:00.000Z", "task-retention-terminal");
        db.connection.prepare(`UPDATE executions SET status = ?, finished_at = ? WHERE id = ?`).run("succeeded", "2026-01-01T00:00:00.000Z", "exec-retention-terminal");
        store.insertSession({
