...
import { buildGoldenTaskInventoryBaseline, SINGLE_TASK_GOLDEN_TASKS, REQUIRED_GOLDEN_TASK_CLASSES, } from "../../../../../src/platform/shared/stability/golden-task-runner.js";
...
test("buildGoldenTaskInventoryBaseline case summaries preserve expectedClass", () => {
    ...
    for (const testCase of baseline.cases) {
        assert.ok(REQUIRED_GOLDEN_TASK_CLASSES.includes(testCase.expectedClass), `Case ${testCase.caseId} has valid expectedClass: ${testCase.expectedClass}`);
    }
});
...
test("buildGoldenTaskInventoryBaseline case summaries preserve latency band", () => {
    const baseline = buildGoldenTaskInventoryBaseline();
    for (const testCase of baseline.cases) {
        assert.ok(testCase.latencyBand === "interactive" || testCase.latencyBand === "extended", `Case ${testCase.caseId} has valid latencyBand: ${testCase.latencyBand}`);
    }
});
...
test("SINGLE_TASK_GOLDEN_TASKS has one case per required class", () => {
    ...
    for (const requiredClass of REQUIRED_GOLDEN_TASK_CLASSES) {
        assert.ok(classCounts.has(requiredClass) && classCounts.get(requiredClass) === 1, `Required class ${requiredClass} should have exactly one case`);
    }
});
...
test("SINGLE_TASK_GOLDEN_TASKS coding class uses interactive latency", () => {
    const codingCases = SINGLE_TASK_GOLDEN_TASKS.filter((t) => t.metadata.expectedClass === "coding");
...
test("SINGLE_TASK_GOLDEN_TASKS crash_recovery class uses extended latency", () => {
    const crashRecoveryCases = SINGLE_TASK_GOLDEN_TASKS.filter((t) => t.metadata.expectedClass === "crash_recovery");
...
test("SINGLE_TASK_GOLDEN_TASKS high_risk_approval class has supervised_review_expected", () => {
    const highRiskCases = SINGLE_TASK_GOLDEN_TASKS.filter((t) => t.metadata.expectedClass === "high_risk_approval");
    assert.equal(highRiskCases[0].metadata.approvalExpectation, "supervised_review_expected");
});
//# sourceMappingURL=golden-task-runner.test.js.map