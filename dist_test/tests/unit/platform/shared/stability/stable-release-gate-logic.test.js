...
function createMockEvidenceReport(overrides = {}) {
    return {
...
function createMockAcceptanceLineReport(status = "pass") {
    return {
...
test("buildStableReleaseGateReport with multiple profiles marks availableProfiles correctly", () => {
    ...
    try {
        ...
        for (const profile of ["smoke", "24h"]) {
...
test("buildStableReleaseGateReport includes all criteria (required + optional)", () => {
    ...
    try {
        ...
        assert.equal(gateReport.criteria.length, gateReport.requiredCriteria.length + gateReport.optionalCriteria.length);
    }
    finally {
        rmSync(dir, { recursive: true, force: true });
    }
});
//# sourceMappingURL=stable-release-gate-logic.test.js.map