...
import { isBlockedOutboundHostname, isSupportedOutboundProtocol, isInternalNetworkUrl, parseSafeOutboundUrl, sanitizeUrlForTelemetry, } from "../../../../../src/platform/control-plane/iam/outbound-url-policy.js";
...
test("parseSafeOutboundUrl throws for invalid URL", () => {
    assert.throws(() => parseSafeOutboundUrl("not-a-url", { invalid: "url.invalid", blocked: "url.blocked" }), (error) => error.code === "url.invalid");
});
test("parseSafeOutboundUrl throws for internal URL", () => {
    assert.throws(() => parseSafeOutboundUrl("http://localhost/path", { invalid: "url.invalid", blocked: "url.blocked" }), (error) => error.code === "url.blocked");
});
...
//# sourceMappingURL=outbound-url-policy.test.js.map