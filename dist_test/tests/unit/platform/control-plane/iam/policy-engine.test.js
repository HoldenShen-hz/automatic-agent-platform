...
test("PolicyEngine.evaluate throws for empty decisionId", () => {
    ...
    assert.throws(() => engine.evaluate(request), (e) => e.code === "policy.invalid_decision_id" && e instanceof ValidationError);
});
test("PolicyEngine.evaluate throws for empty taskId", () => {
    const engine = new PolicyEngine({ budgetPolicy: createMockBudgetPolicy() });
    const request = createValidRequest({ taskId: "   " });
    assert.throws(() => engine.evaluate(request), (e) => e.code === "policy.invalid_task_id" && e instanceof ValidationError);
});
test("PolicyEngine.evaluate throws for empty subjectId", () => {
    const engine = new PolicyEngine({ budgetPolicy: createMockBudgetPolicy() });
    const request = createValidRequest({ subjectId: "" });
    assert.throws(() => engine.evaluate(request), (e) => e.code === "policy.invalid_subject_id" && e instanceof ValidationError);
});
test("PolicyEngine.evaluate throws for invalid action", () => {
    const engine = new PolicyEngine({ budgetPolicy: createMockBudgetPolicy() });
    const request = createValidRequest({ action: null });
    assert.throws(() => engine.evaluate(request), (e) => e.code === "policy.invalid_action");
});
test("PolicyEngine.evaluate throws for invalid riskCategory", () => {
    const engine = new PolicyEngine({ budgetPolicy: createMockBudgetPolicy() });
    const request = createValidRequest({ riskCategory: null });
    assert.throws(() => engine.evaluate(request), (e) => e.code === "policy.invalid_risk_category");
});
test("PolicyEngine.evaluate throws for invalid mode", () => {
    const engine = new PolicyEngine({ budgetPolicy: createMockBudgetPolicy() });
    const request = createValidRequest({ mode: null });
    assert.throws(() => engine.evaluate(request), (e) => e.code === "policy.invalid_mode");
});
