...
test("DataHandlingDimension type accepts all valid values", () => {
    const dimensions = ["prompt", "logs", "memory", "artifact", "cross_worker", "debug"];
    assert.equal(dimensions.length, 6);
});
test("PiiType type accepts all valid values", () => {
    const types = ["email", "phone", "ssn", "credit_card", "ip_address", "name", "address", "dob", "none"];
    assert.equal(types.length, 9);
});
//# sourceMappingURL=data-classification-service.test.js.map