...
test("release pipeline service throws when executeAndExport is called without secret management service", async () => {
    ...
    try {
        ...
        assert.rejects(async () => await service.executeAndExport({
            environment: "staging",
            version: "2.3.4",
            commitSha: "abcdef1234567890",
            rolloutStrategy: "canary",
        }), (error) => error?.code === "release.secret_management_required_for_execute");
    }
    finally {
        cleanupPath(workspace);
    }
...
test("release pipeline fail-closes staged releases when managed credentials are due for rotation", async () => {
    ...
    try {
        ...
        assert.rejects(async () => await service.buildBundle({
            environment: "staging",
            version: "2.3.4",
            commitSha: "abcdef1234567890",
            rolloutStrategy: "canary",
...
//# sourceMappingURL=release-pipeline-service.test.js.map