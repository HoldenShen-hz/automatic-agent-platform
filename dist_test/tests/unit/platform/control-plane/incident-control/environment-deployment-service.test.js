...
import { EnvironmentDeploymentService } from "../../../../../src/platform/control-plane/incident-control/environment-deployment-service.js";
...
function createService(harness, useSecretManagement = false) {
...
function markEnvironmentReady(store, environment, verifiedAt) {
...
test("environment deployment service exports a target-ready release plan once prerequisites are satisfied", async () => {
    ...
    try {
        ...
        assert.equal(exported.report.entries.find((entry) => entry.environment === "prod")?.secretInjection.registryCredentialRegistered, true);
...
//# sourceMappingURL=environment-deployment-service.test.js.map