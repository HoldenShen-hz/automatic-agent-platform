...
import { TrafficRoutingService, TRAFFIC_ROUTING_DDL, DEFAULT_CANARY_CONFIG, } from "../../../../../src/platform/control-plane/rollout-controller/traffic-routing-service.js";
test("TrafficRoutingService type exports are correct", () => {
    const slot = "canary";
    assert.ok(slot === "canary");
    const status = "active";
    assert.ok(status === "active");
    const shiftStatus = "in_progress";
    assert.ok(shiftStatus === "in_progress");
    const trigger = "health_check_failed";
...
//# sourceMappingURL=traffic-routing-service.test.js.map