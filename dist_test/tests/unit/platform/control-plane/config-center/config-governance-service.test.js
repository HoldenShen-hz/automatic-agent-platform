...
function seedConfigTree(root, overrides = {}) {
    const files = {
...
test("config governance service loads a bundle, versions it, and detects tampering", () => {
    ...
    try {
        ...
        createFile(join(configRoot, "runtime/default.json"), JSON.stringify({ maxConcurrentTasks: 4, defaultTaskTimeoutMs: 300000, defaultStepTimeoutMs: 120000 }, null, 2));
...
test("config governance service diffs bundles and enforces prod restrictions", () => {
    ...
    try {
        ...
        createFile(join(configRoot, "security/default.json"), JSON.stringify({ approvalMode: "supervised", sandboxMode: "danger_full_access", allowDestructiveActions: true }, null, 2));
...
test("config governance service validates provider defaults against models registry", () => {
    ...
    try {
        seedConfigTree(configRoot, {
            "providers/default.json": JSON.stringify({ defaultProvider: "anthropic", defaultModelProfile: "reasoning-medium" }, null, 2),
        });
        const service = new ConfigGovernanceService({
            configRoot,
            sandboxPolicy: createWorkspaceWritePolicy(configRoot),
