...
import { OpenAIAPIError, OpenAIChatService, createOpenAIChatService, createOpenAIChatServiceFromEnvironment, } from "../../../../../src/platform/model-gateway/provider-registry/openai/openai-chat-service.js";
// ---------------------------------------------------------------------------
// Helper
// ---------------------------------------------------------------------------
    ...
    const service = new OpenAIChatService({
        ...
        fetchImpl: async (_input, init) => {
            const header = init?.headers instanceof Headers
                ...
                : init?.headers?.["OpenAI-Organization"];
...
test("OpenAI chat service fails over to next credential after rate limit with retry-after-ms", async () => {
    ...
    const service = new OpenAIChatService({
        ...
        fetchImpl: async (input, init) => {
            requests.push(init?.headers instanceof Headers
                ? init.headers.get("Authorization") ?? ""
                : init?.headers?.Authorization ?? "");
...
test("OpenAI chat service streaming assembles content from chunks", async () => {
    ...
    await service.createStreamingChatCompletion({ model: "gpt-4o", messages: [{ role: "user", content: "Hi" }] }, (chunk, isFinal) => {
    ...
    assert.ok(finalChunk?.content?.includes("Hello") && finalChunk?.content?.includes("world"), "Final content should include accumulated content");
});
test("OpenAI chat service streaming accumulates tool calls across chunks", async () => {
    ...
    await service.createStreamingChatCompletion({ model: "gpt-4o", messages: [{ role: "user", content: "Weather in Boston?" }] }, (_chunk, isFinal) => {
        if (isFinal) {
...
test("OpenAI chat service streaming handles empty response body", async () => {
    ...
    await assert.rejects(() => service.createStreamingChatCompletion({ model: "gpt-4o", messages: [{ role: "user", content: "Hi" }] }, () => { }), (error) => {
        assert.ok(error instanceof OpenAIAPIError);
...
test("createOpenAIChatService creates service with single credential", async () => {
    ...
    const testFetchImpl = async (_input, _init) => {
        const header = _init?.headers instanceof Headers
...
//# sourceMappingURL=openai-chat-service.test.js.map