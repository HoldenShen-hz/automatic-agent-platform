...
test("CircuitBreaker records failure and increments counter", async () => {
    const breaker = createBreaker();
    await assert.rejects(() => breaker.execute(async () => { throw new Error("fail"); }), { message: "fail" });
    const metrics = breaker.getMetrics();
...
test("CircuitBreaker open state rejects execute immediately", async () => {
    ...
    await assert.rejects(() => breaker.execute(async () => { throw new Error("fail"); }), { message: "fail" });
    await assert.rejects(() => breaker.execute(async () => "should fail"), (error) => {
...
test("CircuitBreaker rejects when already open from consecutive failures", async () => {
    ...
    await assert.rejects(() => breaker.execute(async () => { throw new Error("fail"); }), { message: "fail" });
    assert.equal(breaker.getState(), "open");
    await assert.rejects(() => breaker.execute(async () => "ok"), (error) => error instanceof CircuitBreakerOpenError);
});
