...
import { MiniMaxAPIError, MiniMaxChatService, createMiniMaxChatServiceFromEnvironment, } from "../../../../../src/platform/model-gateway/provider-registry/minimax/minimax-chat-service.js";
test("MiniMax chat service fails over to the next credential after retry-after cooldown", async () => {
    ...
    const service = new MiniMaxChatService({
        ...
        fetchImpl: async (input, init) => {
            void input;
            requests.push(String(init?.headers instanceof Headers ? init.headers.get("Authorization") : init?.headers?.Authorization));
...
test("MiniMax chat service disables exhausted credential and surfaces structured error", async () => {
    ...
    const service = new MiniMaxChatService({
        credentialPool: pool,
        fetchImpl: async () => new Response("quota exhausted", {
            status: 402,
...
test("MiniMax chat service can bootstrap its credential pool from managed secret refs", async () => {
    ...
    const service = createMiniMaxChatServiceFromEnvironment({
        ...
        fetchImpl: async (_input, init) => {
            requests.push(String(init?.headers instanceof Headers
...
//# sourceMappingURL=minimax-chat-service.test.js.map