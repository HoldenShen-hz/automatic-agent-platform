...
test("CrmAdapter.authenticate throws on missing token", async () => {
    const adapter = createCrmAdapterPlugin();
    await assert.rejects(async () => adapter.authenticate({}), { message: /crm_adapter\.missing_token/ });
});
...
test("CrmAdapter.execute throws PolicyDeniedError when egress blocked", async () => {
    ...
    await assert.rejects(async () => adapter.execute("get_contacts", {}), (err) => {
...
//# sourceMappingURL=crm-adapter.test.js.map