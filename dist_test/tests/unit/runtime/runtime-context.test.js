...
import { provideContext, getContext, getContextOrNull, withContextPatch, assertContext, } from "../../../src/platform/execution/execution-engine/runtime-context.js";
...
test("getContext throws when called outside provideContext", () => {
    assert.throws(() => getContext(), (err) => err.message.includes("runtime_context.missing"));
});
...
async function nestedAsyncRead() {
...
test("withContextPatch throws when called outside provideContext", () => {
    assert.throws(() => withContextPatch({ executionId: "exec_x" }, () => { }), (err) => err.message.includes("runtime_context.missing"));
});
...
test("assertContext throws listing missing keys", () => {
    provideContext(baseContext(), () => {
        assert.throws(() => assertContext("traceId", "executionId", "agentId"), (err) => {
