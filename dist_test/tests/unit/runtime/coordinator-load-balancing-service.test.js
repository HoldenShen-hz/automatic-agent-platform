...
test("coordinator load balancing summarizes fleet status and selects an eligible coordinator", () => {
    ...
    try {
        ...
        assert.ok(decision.evaluations.some((evaluation) => evaluation.coordinatorId === "coord-draining" && evaluation.reasonCode === "coordinator_inactive"));
        assert.ok(decision.evaluations.some((evaluation) => evaluation.coordinatorId === "coord-cn-hot" && evaluation.reasonCode === "tenant_shard_mismatch"));
    }
    finally {
        harness.close();
    }
...
test("selectCoordinator rejects inactive coordinator", () => {
    ...
    try {
        ...
        assert.equal(decision.outcome, "no_candidate");
        assert.equal(decision.selectedCoordinatorId, null);
        assert.ok(decision.evaluations.some((e) => e.coordinatorId === "coord-inactive" && e.reasonCode === "coordinator_inactive"));
    }
    finally {
        harness.close();
    }
...
//# sourceMappingURL=coordinator-load-balancing-service.test.js.map