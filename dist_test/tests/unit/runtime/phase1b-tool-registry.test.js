...
import { executeMultiStepToolCallForTests, resetMultiStepToolRegistryForTests, } from "../../../src/platform/execution/execution-engine/multi-step-orchestration.js";
test("multi-step tool registry executes repo-map searches", async () => {
    ...
    assert.ok(result.files.some((file) => file.relativePath.includes("multi-step") || file.relativePath.includes("orchestration/") ||
...
test("multi-step tool registry can wait on a spawned child agent result", async () => {
    ...
    const spawned = JSON.parse(spawnRaw);
    ...
    const waited = JSON.parse(waitRaw);
...
//# sourceMappingURL=phase1b-tool-registry.test.js.map