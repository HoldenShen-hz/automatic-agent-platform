...
function setupDatabase(workspace) {
...
function createWorkerAndLease(store, workerId, taskId, executionId, ticketId, options = {}) {
    const now = nowIso();
    const { status = "idle", placement = "local", registrationVerifiedAt = null, fenceToken = 1, leaseStatus = "active", ticketStatus = "claimed", } = options;
    ...
    store.upsertWorkerSnapshot({
        ...
        registrationVerifiedAt: registrationVerifiedAt,
...
function createTaskAndExecution(store, taskId, executionId, options = {}) {
    const now = nowIso();
    const { taskStatus = "in_progress", executionStatus = "created", startedAt = null, } = options;
    store.insertTask({
        id: taskId,
        parentId: null,
        rootId: taskId,
...
//# sourceMappingURL=execution-handshake.test.js.map