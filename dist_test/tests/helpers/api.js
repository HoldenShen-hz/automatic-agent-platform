...
export function createSeededApiContext(workspace, options = {}) {
    ...
    const seededTask = store
        .listTasks(200)
        .find((task) => task.title === "API seeded task"
    ...
    pluginKnowledgeRef = { knowledgeRef: `knowledge:${pluginRecord.chunks[0]?.chunkId ?? "missing"}`, snippet: "Plugin supplied snippet", score: 0.9, namespace: "coding/repo", chunkId: pluginRecord.chunks[0]?.chunkId ?? "chunk:missing", documentId: "doc:plugin", matchType: "semantic" };
    return {
        db,
        store,
