...
function createMinimalTaskSituation() {
    return {
        taskId: newId("task"),
        timestamp: Date.now(),
...
function createMultiStepWorkflow(steps) {
...
test("performance: PlanBuilder.build() with 3-step workflow P99 < 50ms", () => {
    ...
    assert.ok(p99 < 50, `PlanBuilder.build() P99 latency ${p99.toFixed(3)}ms exceeds 50ms target`);
    assert.ok(p50 < 25, `PlanBuilder.build() P50 latency ${p50.toFixed(3)}ms seems unexpectedly high`);
});
test("performance: PlanBuilder.build() with 10-step workflow P99 < 100ms", () => {
    ...
    latencies.sort((a, b) => a - b);
    const p99 = latencies[Math.floor(iterations * 0.99)];
    assert.ok(p99 < 100, `PlanBuilder.build() (10-step) P99 latency ${p99.toFixed(3)}ms exceeds 100ms relaxed target`);
});
//# sourceMappingURL=planning-perf.test.js.map