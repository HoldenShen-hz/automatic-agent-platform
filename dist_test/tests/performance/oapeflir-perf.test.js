...
class MinimalExecuteBridge {
    ...
    toDualChannelStepOutputs(_result) {
        return [];
    }
}
...
test("performance: OAPEFLIR full loop (happy path) < 30s P99", async () => {
    ...
    assert.ok(p99 < 30_000, `OAPEFLIR full loop P99 latency ${(p99 / 1000).toFixed(3)}s exceeds 30s target`);
    assert.ok(p50 < 15_000, `OAPEFLIR full loop P50 latency ${(p50 / 1000).toFixed(3)}s seems unexpectedly high`);
...
test("performance: OAPEFLIR loop single iteration baseline", async () => {
    ...
    assert.ok(latency < 30_000, `OAPEFLIR loop single iteration ${(latency / 1000).toFixed(3)}s exceeds 30s target`);
    console.log(`OAPEFLIR single iteration: ${(latency / 1000).toFixed(3)}s`);
});
//# sourceMappingURL=oapeflir-perf.test.js.map