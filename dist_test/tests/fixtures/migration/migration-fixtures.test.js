...
test("migrations can be applied incrementally in order", () => {
    ...
    try {
        ...
        assert.ok(failedMigrations.length <= 4, `Expected at most 4 migrations to fail with duplicate column (got ${failedMigrations.length}): ${JSON.stringify(failedMigrations)}`);
