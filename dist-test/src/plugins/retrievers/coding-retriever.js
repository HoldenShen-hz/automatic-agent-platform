...
export function createCodingRetrieverPlugin(options = {}) {
    ...
    return {
        ...
        async retrieve(query) {
            ...
            const symbolResults = search.symbols.slice(0, 8).map((symbol) => ({
                knowledgeRef: `knowledge:repo:${relative(rootPath, symbol.filePath)}#L${symbol.line}`,
            ...
            const fileResults = search.files.slice(0, 8).map((file) => ({
                knowledgeRef: `knowledge:repo:${file.relativePath}`,
            ...
            return [...symbolResults, ...fileResults].slice(0, 12);
        },
    };
}
//# sourceMappingURL=coding-retriever.js.map