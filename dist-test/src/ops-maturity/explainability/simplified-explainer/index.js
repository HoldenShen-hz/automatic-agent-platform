 ...
 */
export function simplifyExplanation(stageName, summary, decisionFactors, causalLinks, riskLevel = "medium") {
 ...
 */
function generateHeadline(stageName, summary, icon) {
 ...
 */
function simplifyWhatHappened(summary, factors) {
 ...
 */
function simplifyWhyItMatters(factors, _causalLinks) {
    if (factors.length === 0) {
    ...
    const relevantFactors = factors.filter((f) => impactKeywords.some((kw) => f.toLowerCase().includes(kw)));
    ...
    const impacts = relevantFactors.slice(0, 2).map((f) => {
        ...
        if (lower.includes("cost"))
            return "may affect costs";
        if (lower.includes("risk"))
            return "involves some level of risk";
        if (lower.includes("time") || lower.includes("delay"))
            return "may cause delays";
        if (lower.includes("security") || lower.includes("compliance"))
            return "has compliance implications";
 ...
 */
function generateRecommendedAction(stageName, riskLevel) {
 ...
 */
function calculateConfidence(factors, causalLinks) {
 ...
 */
export function formatAsMarkdown(explanation) {
    const lines = [];
 ...
 */
export function formatAsNotification(explanation) {
    return [
