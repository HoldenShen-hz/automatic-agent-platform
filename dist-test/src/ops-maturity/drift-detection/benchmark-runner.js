...
export class SimpleBenchmarkRunner {
    ...
    addBenchmarkCase(benchmarkCase) {
    ...
    isRelevantCase(testCase, proposal) {
        const target = proposal.target.toLowerCase();
        const kind = proposal.kind.toLowerCase();
        if (kind.includes('tool') && testCase.taskType.includes('tool'))
            return true;
        if (kind.includes('skill') && testCase.taskType.includes('skill'))
            return true;
        if (kind.includes('workflow') && target.includes('workflow'))
            return true;
