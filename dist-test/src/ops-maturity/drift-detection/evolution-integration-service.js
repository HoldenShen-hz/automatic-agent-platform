...
export class EvolutionIntegrationService {
     ...
     */
    async triggerReflection(taskType) {
     ...
     */
    async createProposalFromReflection(reflection) {
    ...
    inferTaskType(reasonCode) {
        if (reasonCode.includes('timeout'))
            return 'timeout_task';
        if (reasonCode.includes('type') || reasonCode.includes('schema'))
            return 'type_error_task';
        if (reasonCode.includes('test'))
            return 'test_failure_task';
        if (reasonCode.includes('security') || reasonCode.includes('forbidden'))
            return 'security_task';
        return 'general_task';
    }
    classifyFailureMode(reasonCode) {
        if (reasonCode.includes('timeout'))
            return 'timeout';
        if (reasonCode.includes('type'))
            return 'type_error';
        if (reasonCode.includes('schema'))
            return 'schema_error';
        if (reasonCode.includes('test'))
            return 'unit_test_failure';
        if (reasonCode.includes('lint'))
            return 'lint_error';
        if (reasonCode.includes('security'))
            return 'security_policy_violation';
        if (reasonCode.includes('forbidden'))
            return 'forbidden_path';
        return 'complex_repair_failure';
    }
    inferFailureCategory(reasonCode) {
        if (reasonCode.includes('type') || reasonCode.includes('schema'))
            return 'type_error';
        if (reasonCode.includes('test'))
            return 'unit_test_failure';
        ...
        if (reasonCode.includes('security') || reasonCode.includes('forbidden'))
            return 'forbidden_path';
        if (reasonCode.includes('security_policy'))
            return 'security_policy_violation';
        return 'complex_repair_failure';
    }
    inferProposalKind(taskType) {
        if (taskType.includes('type') || taskType.includes('schema'))
            return 'tool_routing_rule';
        if (taskType.includes('timeout'))
