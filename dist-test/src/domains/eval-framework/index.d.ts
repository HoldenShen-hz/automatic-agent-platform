import { z } from "zod";
export declare const DomainEvaluatorSchema: z.ZodObject<{
    evaluatorId: z.ZodString;
    metric: z.ZodString;
    threshold: z.ZodNumber;
    blocking: z.ZodDefault<z.ZodBoolean>;
}, "strip", z.ZodTypeAny, {
    evaluatorId: string;
    metric: string;
    threshold: number;
    blocking: boolean;
}, {
    evaluatorId: string;
    metric: string;
    threshold: number;
    blocking?: boolean | undefined;
}>;
export declare const DomainEvalFrameworkSchema: z.ZodObject<{
    frameworkId: z.ZodString;
    domainId: z.ZodString;
    fewShotExamples: z.ZodDefault<z.ZodArray<z.ZodString, "many">>;
    evaluators: z.ZodDefault<z.ZodArray<z.ZodObject<{
        evaluatorId: z.ZodString;
        metric: z.ZodString;
        threshold: z.ZodNumber;
        blocking: z.ZodDefault<z.ZodBoolean>;
    }, "strip", z.ZodTypeAny, {
        evaluatorId: string;
        metric: string;
        threshold: number;
        blocking: boolean;
    }, {
        evaluatorId: string;
        metric: string;
        threshold: number;
        blocking?: boolean | undefined;
    }>, "many">>;
    onlineMetrics: z.ZodDefault<z.ZodArray<z.ZodString, "many">>;
    releaseGates: z.ZodDefault<z.ZodObject<{
        minFewShotCount: z.ZodDefault<z.ZodNumber>;
        minRegressionCaseCount: z.ZodDefault<z.ZodNumber>;
        requirePromptInjectionCoverage: z.ZodDefault<z.ZodBoolean>;
    }, "strip", z.ZodTypeAny, {
        minFewShotCount: number;
        minRegressionCaseCount: number;
        requirePromptInjectionCoverage: boolean;
    }, {
        minFewShotCount?: number | undefined;
        minRegressionCaseCount?: number | undefined;
        requirePromptInjectionCoverage?: boolean | undefined;
    }>>;
}, "strip", z.ZodTypeAny, {
    domainId: string;
    frameworkId: string;
    fewShotExamples: string[];
    evaluators: {
        evaluatorId: string;
        metric: string;
        threshold: number;
        blocking: boolean;
    }[];
    onlineMetrics: string[];
    releaseGates: {
        minFewShotCount: number;
        minRegressionCaseCount: number;
        requirePromptInjectionCoverage: boolean;
    };
}, {
    domainId: string;
    frameworkId: string;
    fewShotExamples?: string[] | undefined;
    evaluators?: {
        evaluatorId: string;
        metric: string;
        threshold: number;
        blocking?: boolean | undefined;
    }[] | undefined;
    onlineMetrics?: string[] | undefined;
    releaseGates?: {
        minFewShotCount?: number | undefined;
        minRegressionCaseCount?: number | undefined;
        requirePromptInjectionCoverage?: boolean | undefined;
    } | undefined;
}>;
export type DomainEvaluator = z.infer<typeof DomainEvaluatorSchema>;
export type DomainEvalFramework = z.infer<typeof DomainEvalFrameworkSchema>;
export declare function listBlockingEvaluators(framework: DomainEvalFramework): DomainEvaluator[];
export * from "./domain-evaluation-gate-service.js";
