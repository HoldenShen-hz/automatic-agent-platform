...
import { seedDomainMetaModel, MetaModelValidator, } from "./canonical-meta-model/index.js";
import { DomainDescriptorOrchestrationService } from "./domain-descriptor-orchestration-service.js";
...
function configPathFor(domainId) {
...
function divisionIdFor(domainId) {
...
function securityLevelForRisk(riskLevel) {
...
function thresholdForMetric(metric, riskLevel) {
...
function buildRiskProfile(seed) {
    return {
...
function buildKnowledgeSchema(seed) {
    return {
...
function buildEvalFramework(seed) {
...
function buildPromptLibrary(seed) {
    return {
...
function buildRecipes(seed, workflowId, bundleId) {
    return [
        {
...
function buildInteractionRules(seed) {
    return [
        {
...
function buildGovernancePolicy(seed) {
    return {
...
function buildDefinition(seed, workflowId, bundleId, contractId, promptId) {
    return {
        domainId: seed.domainId,
...
function buildDomainBaseline(seed) {
...
export const VERTICAL_DOMAIN_BASELINES = Object.freeze(DOMAIN_SEEDS.map((seed) => buildDomainBaseline(seed)));
export function listVerticalDomainBaselines() {
    return VERTICAL_DOMAIN_BASELINES;
}
export function listVerticalDomainIds() {
    return VERTICAL_DOMAIN_BASELINES.map((item) => item.domainId);
}
export function listLegacyVerticalDomainIds() {
    return Object.keys(LEGACY_DOMAIN_ID_ALIASES);
}
export function resolveCanonicalVerticalDomainId(domainId) {
    if (VERTICAL_DOMAIN_BASELINES.some((item) => item.domainId === domainId)) {
        return domainId;
    }
    return LEGACY_DOMAIN_ID_ALIASES[domainId] ?? null;
}
export function getVerticalDomainBaseline(domainId) {
...
//# sourceMappingURL=domain-baseline-catalog.js.map