...
import { SkillCreatorService } from "../../platform/execution/tool-executor/skill-creator-service.js";
import { SkillGovernanceService } from "../../platform/execution/tool-executor/skill-governance-service.js";
/**
        ...
        switch (envConfig.action) {
            case "create": {
                ...
                if (envConfig.version)
                    createRequest.version = envConfig.version;
                if (envConfig.author)
                    createRequest.author = envConfig.author;
                if (envConfig.requiredTools)
                    createRequest.requiredTools = envConfig.requiredTools;
                if (envConfig.requiredPermissions)
                    createRequest.requiredPermissions = envConfig.requiredPermissions;
                if (envConfig.tags)
                    createRequest.tags = envConfig.tags;
                if (envConfig.applicableRoles)
                    createRequest.applicableRoles = envConfig.applicableRoles;
                if (envConfig.resourceDirectories) {
                    createRequest.resourceDirectories = envConfig.resourceDirectories;
                }
                if (envConfig.includeOpenAiAgent != null)
                    createRequest.includeOpenAiAgent = envConfig.includeOpenAiAgent;
                if (envConfig.overwriteAllowed != null)
                    createRequest.overwriteAllowed = envConfig.overwriteAllowed;
                if (envConfig.cacheable != null)
                    createRequest.cacheable = envConfig.cacheable;
                if (envConfig.cacheTtlSeconds != null)
                    createRequest.cacheTtlSeconds = envConfig.cacheTtlSeconds;
                if (envConfig.riskLevel)
                    createRequest.riskLevel = envConfig.riskLevel;
                if (envConfig.lifecycle)
                    createRequest.lifecycle = envConfig.lifecycle;
