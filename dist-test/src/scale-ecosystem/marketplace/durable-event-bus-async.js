...
export class DurableEventBusAsync extends EventEmitter {
     ...
     */
    subscribe(consumerId, handler, priority = "normal") {
     ...
     * @param handler - The handler function to call for each event
     */
    subscribeHighPriority(consumerId, handler) {
     ...
     * @param handler - The handler function to call for each event
     */
    subscribeLowPriority(consumerId, handler) {
     ...
     */
    async deliverPending(consumerId, options) {
        this.assertNotDisposed();
     ...
     */
    async executePublishWithRetry(input, timeoutMs) {
        let lastError = null;
     ...
     */
    async executeDeliveryWithRetry(consumerId, timeoutMs) {
        const startedAt = Date.now();
    ...
    async doFlushBatch() {
        ...
        const results = await Promise.allSettled(batch.map((item) => this.publish(item.input, { signal: item.abortController.signal })
            .then(item.resolve)
     ...
     */
    updatePublishLatency(newLatencyMs) {
     ...
     */
    updateDeliveryLatency(newLatencyMs) {
