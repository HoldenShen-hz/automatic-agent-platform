 ...
 */
function assertSimpleIdentifier(value, code) {
 ...
 */
function assertSemverish(value, code) {
 ...
 */
function assertChecksum(value, code) {
 ...
 */
function assertList(values, code) {
 ...
 */
function hashPermissionSurface(permissions) {
...
function buildMarkdownReport(report) {
    ...
    for (const entry of report.entries) {
        lines.push(`| \`${entry.extensionId}@${entry.version}\` | \`${entry.packageType}\` | \`${entry.reviewStatus}\` | \`${entry.publicationStatus}\` | \`${entry.trustLevel}\` | \`${entry.signatureVerified ? "verified" : "missing"}\` | \`${entry.channel ?? "none"}\` | ${entry.reasonCodes.join(", ") || "none"} |`);
    }
    return `${lines.join("\n")}\n`;
}
/**
...
export class MarketplaceGovernanceService {
     ...
     */
    registerExtensionPackage(input) {
        const now = nowIso();
     ...
     */
    submitReview(input) {
        const packageRecord = this.store.marketplace.getExtensionPackage(assertSimpleIdentifier(input.packageId, "marketplace.invalid_package_id"), input.tenantId ?? undefined);
        ...
        const permissions = JSON.parse(packageRecord.permissionsJson);
     ...
     */
    decideReview(input) {
        const existing = this.store.marketplace.getMarketplaceReview(assertSimpleIdentifier(input.reviewId, "marketplace.invalid_review_id"), input.tenantId ?? undefined);
        if (existing == null) {
     ...
     */
    revokePublication(input) {
        const existing = this.store.marketplace.getMarketplacePublication(assertSimpleIdentifier(input.publicationId, "marketplace.invalid_publication_id"), input.tenantId ?? undefined);
        if (existing == null) {
     ...
     */
    buildCatalog(generatedAt = nowIso(), tenantId) {
        ...
        const entries = packages.map((record) => {
            ...
            const compatibility = JSON.parse(record.compatibilityJson);
            const capabilities = JSON.parse(record.capabilitiesJson);
            const permissions = JSON.parse(record.permissionsJson);
            const reasonCodes = [];
            // Check review status
            if (review == null) {
        ...
        const summary = entries.reduce((aggregate, entry) => {
            aggregate.total += 1;
     ...
     */
    exportCatalog(generatedAt = nowIso(), tenantId) {
    ...
    listPublications(limit = 100, tenantId) {
    ...
    listReports(limit = 20, tenantId) {
        return this.store.marketplace.listMarketplaceGovernanceReports(limit, tenantId);
    }
    requirePackage(packageId, tenantId) {
        const packageRecord = this.store.marketplace.getExtensionPackage(assertSimpleIdentifier(packageId, "marketplace.invalid_package_id"), tenantId ?? undefined);
        if (packageRecord == null) {
            throw new StorageError("marketplace.package_not_found", "marketplace.package_not_found", {
                statusCode: 404,
                retryable: false,
...
//# sourceMappingURL=marketplace-governance-service.js.map