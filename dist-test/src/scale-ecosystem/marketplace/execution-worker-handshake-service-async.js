...
export class ExecutionWorkerHandshakeServiceAsync extends EventEmitter {
     ...
     */
    constructor(db, store, options = {}, asyncOptions = {}) {
        super();
     ...
     */
    async claimExecution(input, options) {
        return this.executeWithRetryAndTimeout("claimExecution", () => this.sync.claimExecution(input), options?.timeoutMs ?? this.options.defaultTimeoutMs, options?.signal);
    }
    /**
     ...
     * @returns Promise resolving to the handshake decision
     */
    async recordHeartbeat(input, options) {
        return this.executeWithRetryAndTimeout("recordHeartbeat", () => this.sync.recordHeartbeat(input), options?.timeoutMs ?? this.options.defaultTimeoutMs, options?.signal);
    }
    /**
     ...
     * @returns Promise resolving to the handshake decision
     */
    enqueueClaimExecution(input, options) {
        return this.enqueueOperation("claimExecution", () => this.sync.claimExecution(input), options?.timeoutMs ?? this.options.defaultTimeoutMs, options?.signal, options?.priority ?? 0);
    }
    /**
     ...
     * @returns Promise resolving to the handshake decision
     */
    enqueueHeartbeat(input, options) {
        return this.enqueueOperation("recordHeartbeat", () => this.sync.recordHeartbeat(input), options?.timeoutMs ?? this.options.defaultTimeoutMs, options?.signal, options?.priority ?? 0);
    }
    /**
     ...
     */
    enqueueOperation(operationName, operation, timeoutMs, signal, priority = 0) {
        if (this.disposed) {
