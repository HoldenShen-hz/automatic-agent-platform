...
import { ManualBillingPaymentGateway, } from "./billing-payment-gateway.js";
import { loadBillingPlanCatalog, } from "../../platform/control-plane/config-center/billing-plan-catalog.js";
import { newId, nowIso } from "../../platform/contracts/types/ids.js";
import { assertIdentifier, assertPositiveNumber, buildBillingMarkdown, monthWindow, roundCurrency, } from "./billing/utils.js";
/**
 * Billing Service
 *
...
export class BillingService {
    ...
    recordUsage(input) {
        ...
        const metricType = assertIdentifier(input.metricType, "billing.invalid_metric_type");
        ...
        const quotaCounter = quota == null
            ? null
            : {
                counterId: existingCounter?.counterId ??
                    newId("quota"),
                accountId: account.accountId,
                metricType,
    ...
    buildAccountSummary(accountId) {
        ...
        const quotas = this.store.billing.listQuotaCounters(account.accountId).map((counter) => ({
            ...
            remainingQuantity: counter.limitQuantity == null ? null : roundCurrency(Math.max(0, counter.limitQuantity - counter.usedQuantity)),
     ...
     */
    exportAccountSummary(accountId) {
    ...
    reconcilePaymentSession(input) {
        const gatewaySessionRef = assertIdentifier(input.gatewaySessionRef, "billing.invalid_gateway_session_ref");
        ...
        if (!existing) {
            throw new MonetizationError(`billing.payment_session_not_found:${input.gatewayKind}:${gatewaySessionRef}`, `Billing payment session not found for gateway reference: ${gatewaySessionRef}`, {
     ...
     */
    async reconcilePendingPaymentSessions(input = {}) {
    ...
    listInvoices(accountId, limit = 50, tenantId) {
        const account = this.requireActiveAccount(accountId);
    ...
    listPaymentSessions(invoiceId, limit = 50, tenantId) {
    ...
    requireInvoice(invoiceId, tenantId) {
    ...
    requirePaymentSession(sessionId, tenantId) {
     ...
     */
    async fetchGatewayStatusSnapshot(session, invoice, account) {
     ...
     */
    ensureBillingArtifactTask(createdAt) {
