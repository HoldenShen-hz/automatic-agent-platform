...
import { buildSummary, calculatePercentile, mergeThresholds, roundMetric, safeDividePercent, subtractDaysIso, validateDivisionId, validateProfileName, validateWindowDays, } from "./pmf-validation/utils.js";
...
export class PmfValidationService {
    ...
    metricsService;
    artifactStore;
    constructor(db, store, artifactStoreOptions = {}) {
        this.db = db;
        this.store = store;
    ...
    buildReport(options = {}) {
       ...
       FROM tasks
       ${taskFilters.whereClause}`, taskFilters.parameters);
        // Query session activation counts
        const sessionCounts = this.selectRow(`SELECT
         COUNT(*) AS sessionCount,
         SUM(CASE WHEN t.status = 'done' THEN 1 ELSE 0 END) AS activationSessionCount
       FROM sessions s
       INNER JOIN tasks t ON t.id = s.task_id
       ${sessionFilters.whereClause}`, sessionFilters.parameters);
        // Query repeat usage (root workloads with multiple tasks)
        const repeatCounts = this.selectRow(`SELECT
         COUNT(*) AS rootCount,
       ...
       ${approvalFilters.whereClause}`, approvalFilters.parameters);
        ...
        const approvalResolutionRatePct = safeDividePercent(approvalCounts.resolvedApprovalCount, approvalCounts.approvalCount);
        const crossDivisionUsageRatePct = safeDividePercent(taskCounts.crossDivisionTaskCount, taskCounts.taskCount);
        // Build individual check results
        const checks = [
            {
                checkId: "sample_size",
                status: taskCounts.taskCount >= thresholds.minTaskCount && sessionCounts.sessionCount >= thresholds.minSessionCount
                    ? "pass"
                    : "fail",
                detail: taskCounts.taskCount >= thresholds.minTaskCount && sessionCounts.sessionCount >= thresholds.minSessionCount
            ...
            this.evaluateMinimumCheck("task_success_rate", taskSuccessRatePct, thresholds.minTaskSuccessRatePct, "pct", "successful terminal tasks"),
            this.evaluateMinimumCheck("activation_rate", activationRatePct, thresholds.minActivationRatePct, "pct", "sessions that reached a successful task outcome"),
            this.evaluateMinimumCheck("repeat_usage_rate", repeatUsageRatePct, thresholds.minRepeatUsageRatePct, "pct", "root workloads with at least two tasks in the validation window"),
            this.evaluateMinimumCheck("approval_resolution_rate", approvalResolutionRatePct, thresholds.minApprovalResolutionRatePct, "pct", "approvals resolved inside the validation window"),
            this.evaluateMaximumCheck("average_success_cost", roundMetric(taskCounts.averageSuccessfulTaskCostUsd), thresholds.maxAverageSuccessfulTaskCostUsd, "usd", "average actual cost for successful tasks"),
            this.evaluateMaximumCheck("p95_step_duration", p95StepDurationMs, thresholds.maxP95StepDurationMs, "ms", "p95 workflow step duration"),
        ];
     ...
     */
    evaluateMinimumCheck(checkId, observed, threshold, unit, label) {
        if (observed == null) {
            return {
                checkId,
                status: "warn",
     ...
     */
    evaluateMaximumCheck(checkId, observed, threshold, unit, label) {
        if (observed == null) {
            return {
                checkId,
                status: "warn",
     ...
     */
    ensurePmfArtifactTask(createdAt) {
    ...
    buildTaskFilterClause(windowStart, windowEnd, divisionId) {
        if (divisionId == null) {
            return {
    ...
    buildSessionFilterClause(windowStart, windowEnd, divisionId) {
        if (divisionId == null) {
            return {
    ...
    buildApprovalFilterClause(windowStart, windowEnd, divisionId) {
        if (divisionId == null) {
            return {
        ...
        return {
            whereClause: "INNER JOIN tasks t ON t.id = a.task_id WHERE a.created_at >= ? AND a.created_at <= ? AND t.division_id = ?",
            parameters: [windowStart, windowEnd, divisionId],
        };
    }
    /** Builds SQL filter for step duration queries with optional division scope */
    buildStepFilterClause(windowStart, windowEnd, divisionId) {
        if (divisionId == null) {
            return {
    ...
    selectRow(sql, parameters) {
        return this.db.connection.prepare(sql).get(...parameters);
    }
}
//# sourceMappingURL=pmf-validation-service.js.map