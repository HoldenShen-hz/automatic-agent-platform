...
import { parseLearningSignal } from "./feedback-model.js";
function sortUnique(values) {
    return Array.from(new Set(values.filter((value) => value.trim().length > 0))).sort();
}
function normalizeStepOutputRefs(refs) {
...
export class SignalPreprocessor {
    deduplicate(signals) {
        ...
        for (const signal of signals.map((item) => parseFeedbackSignal(item))) {
            ...
            deduplicated.set(key, {
                ...
                payload: {
                    ...existing.payload,
                    occurrenceCount: (typeof existing.payload.occurrenceCount === "number" && Number.isFinite(existing.payload.occurrenceCount)
                        ? existing.payload.occurrenceCount
                        : 1) + 1,
                    mergedSignalIds: sortUnique([
                        ...(Array.isArray(existing.payload.mergedSignalIds)
                            ? existing.payload.mergedSignalIds.filter((value) => typeof value === "string")
                            : [existing.signalId]),
                        signal.signalId,
                    ]),
                },
    ...
    normalize(signals) {
        ...
        for (const signal of signals.map((item) => parseFeedbackSignal(item))) {
            ...
            const occurrenceCount = typeof existing.payload.occurrenceCount === "number" && Number.isFinite(existing.payload.occurrenceCount)
                ? existing.payload.occurrenceCount
                : 1;
            normalized.set(key, {
                ...existing,
    ...
    toLearningSignals(feedback, options = {}) {
        ...
        for (const signal of signals) {
            ...
            if (!options.includeInformationalSignals
                && signal.category === "success"
                && signal.severity === "info") {
                continue;
            }
            const learningType = signal.category === "correction"
...
//# sourceMappingURL=signal-preprocessor.js.map