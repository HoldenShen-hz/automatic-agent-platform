import { buildFactProvenanceFromRecord, extractStructuredMemoryText, parseStructuredMemoryContent, } from "./memory-schema.js";
export function extractMemorySnippet(record) {
...
export function hasExplicitMemoryBoundary(query) {
    return Boolean(query.taskId != null ||
        query.sessionId != null ||
        query.agentId != null ||
        query.executionId != null ||
        (query.scopes != null && query.scopes.length > 0));
}
export function buildMemoryConsolidationSummary(records, targetLayer) {
    ...
    return {
        summaryText,
        averageQualityScore: qualityScores.length > 0 ? qualityScores.reduce((sum, score) => sum + score, 0) / qualityScores.length : null,
