...
import { getLayerMetadata, } from "./layer-transition-service.js";
/**
 * Default decay configs per layer
 */
export const DEFAULT_DECAY_CONFIGS = {
    working: {
 ...
 */
export function calculateFreshness(memory, config, evaluatedAt = nowIso()) {
 ...
 */
export function calculateDecayAmount(memory, previousFreshness, config, evaluatedAt = nowIso()) {
...
export class MemoryDecayService {
    decayConfigs;
    constructor(decayConfigs = DEFAULT_DECAY_CONFIGS) {
        this.decayConfigs = decayConfigs;
    }
    /**
     * Gets the decay configuration for a memory's layer
     */
    getDecayConfig(memory) {
        const layerMeta = getLayerMetadata(memory.scope);
        if (layerMeta) {
     ...
     */
    calculateFreshness(memory, evaluatedAt = nowIso()) {
     ...
     */
    calculateDecay(memory, previousFreshness, evaluatedAt = nowIso()) {
        const config = this.getDecayConfig(memory);
        const currentFreshness = calculateFreshness(memory, config, evaluatedAt);
     ...
     */
    generateDecaySummary(memories, evaluatedAt = nowIso()) {
        const byLayer = {
     ...
     */
    evaluateCompressionCandidates(memories, evaluatedAt = nowIso(), maxCandidates) {
        const candidates = [];
        for (const memory of memories) {
            const freshness = this.calculateFreshness(memory, evaluatedAt);
            const config = this.getDecayConfig(memory);
            ...
            const compressionScore = (qualityScore * 0.35 +
