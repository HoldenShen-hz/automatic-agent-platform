...
import { KnowledgeAccessControl, } from "../governance/access-control.js";
...
function normalizeQueryTerms(keyword) {
...
function freshnessMultiplierFor(action) {
    switch (action) {
        case "warn":
            return 0.95;
        case "demote":
...
export class KnowledgeRetrievalService {
    ...
    semanticGraph;
    semanticVectorStore;
    accessControl = new KnowledgeAccessControl();
    auditLogger;
    freshness = new FreshnessTracker();
    trustPolicies = new SourceTrustPolicyRegistry();
    citations = new CitationBuilder();
    constructor(index, archive, namespacePolicies, semanticGraph = null, semanticVectorStore = null, auditLogger = new KnowledgeAuditLogger()) {
        this.index = index;
    ...
    buildQueryResults(keyword, options, semanticCandidates) {
        ...
        const candidateKnowledgeRefs = new Set(directKnowledgeRefs);
        ...
        return [...candidateKnowledgeRefs]
            .map((knowledgeRef) => this.buildRankedHit(knowledgeRef, queryTerms, directHits, directKnowledgeRefs, semanticScores, options, accessDecisions))
            .filter((hit) => hit != null)
            .sort((left, right) => {
    ...
    buildRankedHit(knowledgeRef, queryTerms, directHits, directKnowledgeRefs, semanticScores, options, accessDecisions) {
        ...
        const matchedTerms = new Set(directHits.get(knowledgeRef)?.matchedTerms ?? []);
        for (const term of queryTerms) {
        ...
        const graphBoost = (sharedKeywordNeighborCount * 0.75)
        ...
        const exactMatchScore = directMatch?.exactMatchScore
        ...
        const finalScore = Number(((baseScore * accessContext.trustPolicy.maxRetrievalWeight * freshnessMultiplier * namespaceBoost)).toFixed(4));
        ...
        const hit = {
            ...
            knowledgeRef,
            reasoningSummary: reasoningPaths.length > 0
                ? reasoningPaths.join(" | ")
                : "keyword_index",
            rankingSignals: {
    ...
    collectSemanticCandidates(keyword, options) {
    ...
    async collectSemanticCandidatesAsync(keyword, options) {
        if (!this.semanticVectorStore) {
    ...
    getAccessContext(chunkRecord, options, accessDecisions) {
