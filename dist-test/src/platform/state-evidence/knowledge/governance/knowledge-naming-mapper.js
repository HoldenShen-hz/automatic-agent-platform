...
export function isTrustLevelAtOrAbove(level, minimum) {
    ...
    const normalizedLevel = typeof level === "string" && level in TRUST_LEVEL_TO_CODE && TRUST_LEVEL_TO_CODE[level]
        ? TRUST_LEVEL_TO_CODE[level]
        : level;
    const normalizedMinimum = typeof minimum === "string" && minimum in TRUST_LEVEL_TO_CODE && TRUST_LEVEL_TO_CODE[minimum]
...
export function isMemoryLayerAtOrAbove(layer, minimum) {
    ...
    // Check if the value exists as a key in the mapping before using it
    const normalizedLayer = typeof layer === "string" && layer in MEMORY_LAYER_TO_CODE && MEMORY_LAYER_TO_CODE[layer]
        ? MEMORY_LAYER_TO_CODE[layer]
        : layer;
    const normalizedMinimum = typeof minimum === "string" && minimum in MEMORY_LAYER_TO_CODE && MEMORY_LAYER_TO_CODE[minimum]
