...
export class MarketplaceRepository {
    ...
    upsertMarketplaceReview(record) {
        execute(this.db.connection, `INSERT INTO marketplace_reviews (
    ...
    upsertMarketplacePublication(record) {
        execute(this.db.connection, `INSERT INTO marketplace_publications (
    ...
    insertMarketplaceGovernanceReport(record) {
        execute(this.db.connection, `INSERT INTO marketplace_governance_reports (
        report_id, tenant_id, summary_json, report_json, generated_at
      ) VALUES (?, ?, ?, ?, ?)`, record.reportId, record.tenantId, record.summaryJson, record.reportJson, record.generatedAt);
    }
    upsertExtensionPackage(record) {
        execute(this.db.connection, `INSERT INTO extension_packages (
    ...
    getExtensionPackage(packageId, tenantId) {
       ...
       WHERE package_id = ?`;
        if (scopedTenantId === undefined) {
            return queryOne(this.db.connection, `${sql} LIMIT 1`, packageId) ?? null;
        }
        return queryOne(this.db.connection, `${sql} AND tenant_id IS ? LIMIT 1`, packageId, scopedTenantId) ?? null;
    }
    listExtensionPackages(limit = 100, tenantId) {
       ...
       FROM extension_packages`;
        if (scopedTenantId === undefined) {
            return queryAll(this.db.connection, `${sql} ORDER BY updated_at DESC, package_id DESC LIMIT ?`, safeLimit);
        }
        return queryAll(this.db.connection, `${sql} WHERE tenant_id IS ? ORDER BY updated_at DESC, package_id DESC LIMIT ?`, scopedTenantId, safeLimit);
    }
    getMarketplaceReview(reviewId, tenantId) {
        const scopedTenantId = resolveTenantScope(tenantId);
        const sql = `SELECT
       ...
       WHERE review_id = ?`;
        if (scopedTenantId === undefined) {
            return queryOne(this.db.connection, `${sql} LIMIT 1`, reviewId) ?? null;
        }
        return queryOne(this.db.connection, `${sql} AND tenant_id IS ? LIMIT 1`, reviewId, scopedTenantId) ?? null;
    }
    listMarketplaceReviews(limit = 100, tenantId) {
        const safeLimit = Number.isFinite(limit) ? Math.max(1, Math.trunc(limit)) : 100;
        const scopedTenantId = resolveTenantScope(tenantId);
        const sql = `SELECT
         review_id AS reviewId,
       ...
       FROM marketplace_reviews`;
        if (scopedTenantId === undefined) {
            return queryAll(this.db.connection, `${sql} ORDER BY submitted_at DESC, review_id DESC LIMIT ?`, safeLimit);
        }
        return queryAll(this.db.connection, `${sql} WHERE tenant_id IS ? ORDER BY submitted_at DESC, review_id DESC LIMIT ?`, scopedTenantId, safeLimit);
    }
    getLatestMarketplaceReviewForPackage(packageId, tenantId) {
        const scopedTenantId = resolveTenantScope(tenantId);
        const sql = `SELECT
         review_id AS reviewId,
         tenant_id AS tenantId,
       ...
       WHERE package_id = ?`;
        if (scopedTenantId === undefined) {
            return queryOne(this.db.connection, `${sql} ORDER BY submitted_at DESC, review_id DESC LIMIT 1`, packageId) ?? null;
        }
        return queryOne(this.db.connection, `${sql} AND tenant_id IS ? ORDER BY submitted_at DESC, review_id DESC LIMIT 1`, packageId, scopedTenantId) ?? null;
    }
    getMarketplacePublication(publicationId, tenantId) {
        const scopedTenantId = resolveTenantScope(tenantId);
        const sql = `SELECT
       ...
       WHERE publication_id = ?`;
        if (scopedTenantId === undefined) {
            return queryOne(this.db.connection, `${sql} LIMIT 1`, publicationId) ?? null;
        }
        return queryOne(this.db.connection, `${sql} AND tenant_id IS ? LIMIT 1`, publicationId, scopedTenantId) ?? null;
    }
    getActiveMarketplacePublicationForPackage(packageId, tenantId) {
        const scopedTenantId = resolveTenantScope(tenantId);
        const sql = `SELECT
         publication_id AS publicationId,
         tenant_id AS tenantId,
       ...
       WHERE package_id = ? AND status = 'published'`;
        if (scopedTenantId === undefined) {
            return queryOne(this.db.connection, `${sql} ORDER BY updated_at DESC, publication_id DESC LIMIT 1`, packageId) ?? null;
        }
        return queryOne(this.db.connection, `${sql} AND tenant_id IS ? ORDER BY updated_at DESC, publication_id DESC LIMIT 1`, packageId, scopedTenantId) ?? null;
    }
    listMarketplacePublications(limit = 100, tenantId) {
        const safeLimit = Number.isFinite(limit) ? Math.max(1, Math.trunc(limit)) : 100;
        const scopedTenantId = resolveTenantScope(tenantId);
        const sql = `SELECT
         publication_id AS publicationId,
       ...
       FROM marketplace_publications`;
        if (scopedTenantId === undefined) {
            return queryAll(this.db.connection, `${sql} ORDER BY updated_at DESC, publication_id DESC LIMIT ?`, safeLimit);
        }
        return queryAll(this.db.connection, `${sql} WHERE tenant_id IS ? ORDER BY updated_at DESC, publication_id DESC LIMIT ?`, scopedTenantId, safeLimit);
    }
    listMarketplaceGovernanceReports(limit = 20, tenantId) {
       ...
       FROM marketplace_governance_reports`;
        if (scopedTenantId === undefined) {
            return queryAll(this.db.connection, `${sql} ORDER BY generated_at DESC LIMIT ?`, safeLimit);
        }
        return queryAll(this.db.connection, `${sql} WHERE tenant_id IS ? ORDER BY generated_at DESC LIMIT ?`, scopedTenantId, safeLimit);
    }
}
