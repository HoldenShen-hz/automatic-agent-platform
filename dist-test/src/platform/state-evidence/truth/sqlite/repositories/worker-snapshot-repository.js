...
export class WorkerSnapshotRepository {
    ...
    insertHeartbeatSnapshot(snapshot) {
        execute(this.conn, `INSERT INTO heartbeat_snapshots (
        id, execution_id, agent_id, runtime_instance_id, restart_generation,
        status, progress_message, cpu_pct, memory_mb, sampled_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, snapshot.id, snapshot.executionId, snapshot.agentId, snapshot.runtimeInstanceId, snapshot.restartGeneration, snapshot.status, snapshot.progressMessage, snapshot.cpuPct, snapshot.memoryMb, snapshot.sampledAt);
    }
    upsertWorkerSnapshot(snapshot) {
        execute(this.conn, `INSERT INTO worker_snapshots (
    ...
    upsertCoordinatorInstanceSnapshot(snapshot) {
        execute(this.conn, `INSERT INTO coordinator_instance_snapshots (
    ...
    getWorkerSnapshot(workerId) {
        return queryOne(this.conn, `${WORKER_SNAPSHOT_SELECT}
    ...
    listStaleWorkerSnapshots(heartbeatBefore) {
        return queryAll(this.conn, `${WORKER_SNAPSHOT_SELECT}
       WHERE last_heartbeat_at < ?
       ORDER BY last_heartbeat_at ASC`, heartbeatBefore);
    }
    getCoordinatorInstanceSnapshot(coordinatorId) {
        return queryOne(this.conn, `${COORDINATOR_SNAPSHOT_SELECT}
       WHERE coordinator_id = ?`, coordinatorId);
    }
    listCoordinatorInstanceSnapshots(limit = 100) {
        return queryAll(this.conn, `${COORDINATOR_SNAPSHOT_SELECT}
    ...
    listHeartbeatSnapshotsByExecution(executionId, tenantId) {
        ...
        if (scopedTenantId !== undefined) {
            return queryAll(this.conn, `${HEARTBEAT_SNAPSHOT_SELECT}
        ...
        return queryAll(this.conn, `${HEARTBEAT_SNAPSHOT_SELECT}
       WHERE h.execution_id = ?
       ORDER BY h.sampled_at ASC, h.id ASC`, executionId);
    }
}
//# sourceMappingURL=worker-snapshot-repository.js.map