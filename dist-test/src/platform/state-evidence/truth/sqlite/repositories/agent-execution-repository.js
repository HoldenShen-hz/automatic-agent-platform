...
export class AgentExecutionRepository {
    ...
    insertRemoteLog(record) {
        execute(this.conn, `INSERT INTO remote_log_entries (
        id, task_id, execution_id, worker_id, runtime_instance_id, level, message, context_json, created_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, record.id, record.taskId, record.executionId, record.workerId, record.runtimeInstanceId, record.level, record.message, record.contextJson, record.createdAt);
    }
    upsertAgentExecutionRecord(record) {
        execute(this.conn, `INSERT INTO agent_execution_records (
    ...
    getAgentExecutionRecord(executionId, tenantId) {
        ...
        if (scopedTenantId !== undefined) {
            return queryOne(this.conn, `${AGENT_EXECUTION_SELECT}
    ...
    listAgentExecutionRecordsByTask(taskId, tenantId) {
        const scopedTenantId = resolveTenantScope(tenantId);
        if (scopedTenantId !== undefined) {
            return queryAll(this.conn, `${AGENT_EXECUTION_SELECT}
         INNER JOIN tasks t ON t.id = a.task_id
         WHERE a.task_id = ?
           AND t.tenant_id = ?
        ...
        return queryAll(this.conn, `${AGENT_EXECUTION_SELECT}
       WHERE a.task_id = ?
       ORDER BY a.updated_at ASC, a.execution_id ASC`, taskId);
    }
    listRemoteLogsByTask(taskId, tenantId) {
        const scopedTenantId = resolveTenantScope(tenantId);
        if (scopedTenantId !== undefined) {
            return queryAll(this.conn, `${REMOTE_LOG_SELECT}
        ...
        return queryAll(this.conn, `${REMOTE_LOG_SELECT}
       WHERE r.task_id = ?
       ORDER BY r.created_at ASC, r.id ASC`, taskId);
    }
    listRemoteLogsByExecution(executionId, tenantId) {
        ...
        return queryAll(this.conn, `${REMOTE_LOG_SELECT}
       WHERE r.execution_id = ?
       ORDER BY r.created_at ASC, r.id ASC`, executionId);
    }
}
