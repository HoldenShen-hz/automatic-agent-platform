...
class RedisQueueClient {
    ...
    async hmset(key, data) {
        ...
        for (const [k, v] of Object.entries(data))
            args.push(k, v);
        await this.redis.hmset(args[0], data);
    }
    async del(key) {
        await this.ensureConnected();
    ...
    async zrangebyscore(key, min, max, offset, count) {
        await this.ensureConnected();
...
export class RedisQueueAdapter {
    ...
    backendKind = "redis";
    client;
    jobTtlSeconds = 86400 * 7;
    constructor(config) {
        this.config = config;
        this.client = new RedisQueueClient(config);
    }
    jobKey(jobId) { return `job:${jobId}`; }
    waitingKey(queueName) { return `queue:${queueName}:waiting`; }
    activeKey(queueName) { return `queue:${queueName}:active`; }
    completedKey(queueName) { return `queue:${queueName}:completed`; }
    deadLetterKey(queueName) { return `queue:${queueName}:dead_letter`; }
    queueSetKey() { return "queues"; }
    enqueue(input) {
    ...
    dequeue(_queueName) { throw this.unsupported("dequeue"); }
    getJob(_jobId) { throw this.unsupported("getJob"); }
    listJobs(_queueName, _status, _limit) { throw this.unsupported("listJobs"); }
    moveToDeadLetter(_jobId, _reason) { throw this.unsupported("moveToDeadLetter"); }
    retryJob(_jobId) { throw this.unsupported("retryJob"); }
    purge(_queueName, _olderThan) { throw this.unsupported("purge"); }
    stats(_queueName) { throw this.unsupported("stats"); }
    listQueues() { throw this.unsupported("listQueues"); }
    unsupported(method) {
    ...
    async enqueueAsync(input) {
        try {
            ...
            throw new StorageError(`queue.enqueue_failed: ${err.message}`, "queue.enqueue_failed", { cause: err, retryable: true });
        }
    }
    async dequeueAsync(queueName) {
        const now = Date.now();
        ...
        if (!jobData.id || jobData.status === "delayed")
            return null;
    ...
    async listJobsAsync(queueName, status, limit = 100) {
        let jobIds = [];
        if (!status || status === "waiting") {
            jobIds = [...jobIds, ...(await this.client.zrangebyscore(this.waitingKey(queueName), "+inf", "-inf", 0, limit))];
        }
        if (!status || status === "active")
            jobIds = [...jobIds, ...(await this.client.smembers(this.activeKey(queueName)))];
        if (!status || status === "completed")
            jobIds = [...jobIds, ...(await this.client.smembers(this.completedKey(queueName)))];
        if (!status || status === "dead_letter")
            jobIds = [...jobIds, ...(await this.client.smembers(this.deadLetterKey(queueName)))];
    ...
    async moveToDeadLetterAsync(jobId, reason) {
    ...
    async purgeAsync(queueName, olderThan) {
    ...
    async listQueuesAsync() { return this.client.smembers(this.queueSetKey()); }
