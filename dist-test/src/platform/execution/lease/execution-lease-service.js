...
import { buildWorkerSnapshotRefreshInput, mergeExecutionIds, parseJsonArray, plusMs, removeExecutionId, } from "./utils.js";
...
export class ExecutionLeaseService {
    ...
    handoverLease(input) {
        ...
        return this.db.transaction(() => {
            ...
            if (!nextWorkerRunningExecutionIds.includes(previousLease.executionId)
     ...
     */
    reclaimExpiredLeases(now = nowIso()) {
        return this.db.transaction(() => {
    ...
    reclaimActiveLease(executionId, occurredAt = nowIso(), reasonCode = "lease_reclaimed") {
     ...
     */
    expireActiveLeaseIfNeeded(executionId, occurredAt, reasonCode) {
     ...
     */
    insertLeaseAudit(input) {
        this.store.worker.insertLeaseAudit({
            id: newId("laudit"),
            ...input,
        });
    }
    updateWorkerExecutionOwnership(previousWorkerId, nextWorkerId, executionId, occurredAt) {
    ...
    clearWorkerExecutionOwnership(workerId, executionId, occurredAt) {
    ...
    reclaimActiveLeaseInternal(executionId, occurredAt, reasonCode) {
        const activeLease = this.store.worker.getActiveExecutionLease(executionId);
        if (!activeLease) {
            return null;
        }
    ...
    updateAgentExecutionOwnership(executionId, agentId, occurredAt, handoverReason) {
...
//# sourceMappingURL=execution-lease-service.js.map