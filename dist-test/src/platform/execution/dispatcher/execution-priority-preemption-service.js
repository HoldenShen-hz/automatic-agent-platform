...
function resolveCandidatePriority(taskPriority, latestTicket) {
    return latestTicket?.priority ?? taskPriority;
}
function isRemoteSessionReadyForDispatch(worker) {
...
export class ExecutionPriorityPreemptionService {
    ...
    preemptForUrgentTicket(input) {
        ...
        const trace = this.db.transaction(() => {
            const reclaimedLease = this.leases.reclaimActiveLease(candidate.execution.id, input.occurredAt, "priority_preempted");
            if (!reclaimedLease) {
     ...
     */
    isCompatibleFullWorker(worker, input) {
     ...
     */
    toCandidate(worker, sourceExecutionId) {
     ...
     */
    createReplacementTicket(candidate, occurredAt) {
        const requiredCapabilitiesJson = candidate.latestTicket?.requiredCapabilitiesJson
            ?? JSON.stringify([]);
        const ticket = {
            id: newId("ticket"),
            executionId: candidate.execution.id,
     ...
     */
    upsertAgentExecutionRecord(candidate, occurredAt) {
        ...
        const record = {
            ...
            restartedFromRuntimeInstanceId: existing?.restartedFromRuntimeInstanceId ?? candidate.worker.restartedFromRuntimeInstanceId,
    ...
    buildTrace(triggerPriority, candidate, replacementTicketId, reasonCode) {
        return {
...
//# sourceMappingURL=execution-priority-preemption-service.js.map