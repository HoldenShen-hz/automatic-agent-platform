...
export class RuntimeRecoveryReplayService {
     ...
     */
    buildTaskReplayReport(taskId, generatedAt = nowIso()) {
        ...
        const reports = executions.map((execution) => this.buildExecutionReport(execution, 
     ...
     */
    buildExecutionReplayReport(executionId, generatedAt = nowIso()) {
     ...
     * @returns Execution recovery replay report
     */
    buildExecutionReport(execution, candidate, recoveryEvents, deadLetter, precheck) {
        ...
        const decisions = timeline
            .filter((event) => event.decisionAction !== null)
            .map((event) => ({
        ...
        const repairs = timeline
            .filter((event) => event.repairAction !== null)
            .map((event) => ({
            eventId: event.eventId,
            createdAt: event.createdAt,
 ...
 */
function matchesExecution(event, executionId) {
 ...
 */
function summarizeEvent(eventType, input) {
    switch (eventType) {
 ...
 */
function compareRecoveryEventOrder(left, right) {
 ...
 */
function recoveryEventPriority(eventType) {
    switch (eventType) {
        case "recovery:repair_applied":
            return 1;
        case "recovery:decision_recorded":
 ...
 */
function inferExecutionOutcome(execution, candidate, deadLetter, decisions, repairs, timeline) {
 ...
 */
function inferTaskOutcome(executions) {
 ...
 */
function toPrecheckView(precheck) {
    if (!precheck) {
        return null;
    }
    return {
...
//# sourceMappingURL=runtime-recovery-replay-service-root.js.map