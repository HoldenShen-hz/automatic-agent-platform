...
export class TaskTransitionService {
    ...
    apply(command) {
        ...
        const affected = this.repository.updateTaskStatusCas(command.entityId, command.fromStatus, command.toStatus, command.occurredAt, null, command.toStatus === "done" || command.toStatus === "failed" || command.toStatus === "cancelled"
            ? command.occurredAt
            : null);
        if (affected === 0) {
            throw new Error(`task.transition_cas_failed:${command.entityId}:${command.fromStatus}->${command.toStatus}`);
        }
...
export class WorkflowTransitionService {
    ...
    apply(command) {
        ...
        if (current.status !== command.fromStatus) {
            throw new Error(`workflow.transition_fromStatus_mismatch:${command.entityId}:${command.fromStatus}->${current.status}`);
        }
        const affected = this.repository.updateWorkflowStateCas(command.entityId, current.currentStepIndex, current.status, command.toStatus, command.currentStepIndex, command.outputsJson, command.occurredAt);
        if (affected === 0) {
            throw new Error(`workflow.transition_cas_failed:${command.entityId}:${command.fromStatus}->${command.toStatus}`);
        }
    }
}
/**
...
export class SessionTransitionService {
    ...
    apply(command) {
        ...
        const affected = this.repository.updateSessionStatusCas(command.entityId, command.fromStatus, command.toStatus, command.occurredAt);
        if (affected === 0) {
            throw new Error(`session.transition_cas_failed:${command.entityId}:${command.fromStatus}->${command.toStatus}`);
        }
    }
}
/**
...
export class ExecutionTransitionService {
    ...
    apply(command) {
        ...
        const startedAt = command.toStatus === "prechecking" || command.toStatus === "executing"
            ? command.occurredAt
            : null;
        const finishedAt = command.toStatus === "succeeded" || command.toStatus === "failed" || command.toStatus === "cancelled"
            ? command.occurredAt
            : null;
        ...
        const affected = this.repository.updateExecutionStatusCas(command.entityId, command.fromStatus, command.toStatus, command.occurredAt, startedAt, finishedAt, lastErrorCode);
        if (affected === 0) {
            throw new Error(`execution.transition_cas_failed:${command.entityId}:${command.fromStatus}->${command.toStatus}`);
        }
    }
}
/**
...
export class ApprovalTransitionService {
    ...
    apply(command) {
        ...
        if (affected === 0) {
            throw new Error(`approval.transition_cas_failed:${command.entityId}:${command.fromStatus}->${command.toStatus}`);
        }
    }
}
/**
...
class TaskTerminalTransitionService {
    ...
    apply(input) {
        ...
        const workflowTerminal = input.terminalStatus === "done" ? "completed" : input.terminalStatus;
        const sessionTerminal = input.terminalStatus === "done" ? "completed" : input.terminalStatus;
        const executionTerminal = input.terminalStatus === "done" ? "succeeded" : input.terminalStatus;
        ...
        this.repository.updateTaskStatus(input.taskId, input.terminalStatus, input.context.occurredAt, input.terminalStatus === "failed" ? input.context.reasonCode : null, input.context.occurredAt);
        this.repository.updateWorkflowState(input.taskId, workflowTerminal, workflowTerminal === "completed" ? 1 : 0, input.outputsJson, input.context.occurredAt);
        this.repository.updateSessionStatus(input.sessionId, sessionTerminal, input.context.occurredAt);
        this.repository.updateExecutionStatus(input.executionId, executionTerminal, input.context.occurredAt, null, input.context.occurredAt, input.terminalStatus === "failed" ? input.context.reasonCode : null);
        this.repository.createTier1StatusEvent({
            taskId: input.taskId,
...
class ApprovalBlockingTransitionService {
    ...
    sessions;
    executions;
    constructor(db, repository, tasks, workflows, sessions, executions) {
        this.db = db;
        this.repository = repository;
        this.tasks = tasks;
...
export class TransitionService {
    ...
    terminalTasks;
    approvalBlocks;
    constructor(db, store, repository = createRuntimeLifecycleRepository(store)) {
        ...
        this.approvalBlocks = new ApprovalBlockingTransitionService(db, repository, this.tasks, this.workflows, this.sessions, this.executions);
    }
 ...
 */
function buildEventTraceContext(context, taskId) {
 ...
 */
function buildStatusTransitionEventPayload(command) {
    return {
