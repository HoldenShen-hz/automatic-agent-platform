...
import { deriveProviderApiKeyEnvName as deriveProviderApiKeyEnvNameFromPool, deriveProviderApiKeySecretRefEnvName, deriveProviderApiKeySecretRefsJsonEnvName, deriveProviderApiKeysJsonEnvName, loadProviderCredentialRecordsFromEnv, } from "../../model-gateway/provider-registry/provider-credential-pool.js";
 ...
 */
function normalizeConfigRoot(configRoot) {
...
export function deriveProviderApiKeysJsonEnvNameForStartup(providerId) {
    return deriveProviderApiKeysJsonEnvName(providerId);
}
/** Derives the environment variable name for a provider's secret ref. */
export function deriveProviderApiKeySecretRefEnvNameForStartup(providerId) {
...
export function deriveProviderApiKeySecretRefsJsonEnvNameForStartup(providerId) {
 ...
 */
export function buildDefaultStartupConfigValidator(options = {}) {
    ...
    return () => {
        try {
            ...
            const trustedContextIssues = trustedContextReport.findings.map((finding) => `${finding.code}:${finding.severity}:${finding.message}`);
 ...
 */
export function buildEnvironmentProviderReadinessProbe(options = {}) {
    ...
    return (configValidation) => {
        ...
        const modelRegistry = loadModelMetadataRegistry(configValidation.bundle.configRoot, createWorkspaceWritePolicy(configValidation.bundle.configRoot));
 ...
 */
export function createDefaultStartupConsistencyCheckerOptions(options = {}) {
    return {
