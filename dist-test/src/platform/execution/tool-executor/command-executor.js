...
import { coerceCommandToolRequest, formatToolArgumentCoercionWarnings, } from "./tool-argument-coercion.js";
...
import { getProcessTracker, spawnTracked } from "../resource/process-tracker.js";
 ...
 */
function killProcessTree(child, forceKillAfterDelayMs = 5000) {
    const pid = child.pid;
    if (pid == null) {
        return;
    }
...
export class CommandExecutor {
    ...
    async execute(request, signal) {
        ...
        try {
            ...
            if ((normalizedRequest.allowedPathRoots !== undefined && !isStringArray(normalizedRequest.allowedPathRoots))
     ...
     */
    blocked(request, reasonCode, coercions = []) {
        const warnings = formatToolArgumentCoercionWarnings(coercions);
        return this.buildResult(request, "blocked", sanitizeToolOutput("Command blocked by sandbox or command policy.", {
            persistedMessageLimitChars: this.persistedMessageLimitChars,
        }), [], 0, {
            code: reasonCode,
            message: reasonCode,
            retryable: false,
            source: "security",
        }, null, coercions, warnings);
    }
    buildResult(request, status, output, artifacts, durationMs, error, executionReceipt, coercions = [], extraWarnings = []) {
