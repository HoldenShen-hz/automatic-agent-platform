...
import { isToolFailureRetryable, resolveToolTimeoutMs, } from "./tool-metadata.js";
import { StructuredLogger } from "../../shared/observability/structured-logger.js";
import { normalizeAttempts, } from "./skill-execution-support.js";
...
export const skillExecutionCoreMethods = {
    async execute(input) {
        ...
        if (runtimeAllowedToolsResolution.errorCode != null) {
            throw new ValidationError(`skill.execution_allowed_tools_invalid:${input.executionId}`, `skill.execution_allowed_tools_invalid:${input.executionId}`, {
                source: "tool",
                details: {
        ...
        if (cacheLookup.entry) {
            ...
            for (const cachedStep of cacheLookup.entry.steps) {
                this.store.workflow.insertStepOutput(this.buildCachedStepOutput({
        ...
        const storedCacheMetadata = finalStatus === "succeeded"
    ...
    async executeToolCallWithPolicy(request, metadata) {
        ...
        try {
            const result = await new Promise((resolveResult, rejectResult) => {
                timer = setTimeout(() => {
                    resolveResult(this.normalizeToolCallResult({
                        success: false,
                        status: "timed_out",
                ...
                Promise.resolve(this.toolRunner({
                    ...request,
                    timeoutMs,
    ...
    normalizeToolCallResult(result, metadata, startedAtMs) {
        ...
        const errorCode = sanitizedResult.errorCode
        ...
        return {
            ...
            errorCode,
            errorSource,
            retryable: sanitizedResult.success
                ? false
    ...
    validateSkillDefinition(skill) {
        ...
        for (const step of skill.steps) {
            if (!declaredTools.has(step.toolName)) {
                throw new ValidationError(`skill.definition_tool_not_declared:${step.stepId}:${step.toolName}`, `skill.definition_tool_not_declared:${step.stepId}:${step.toolName}`, {
                    source: "tool",
            ...
            for (const override of step.modelOverrides ?? []) {
                if (!declaredTools.has(override.toolName)) {
                    throw new ValidationError(`skill.definition_override_tool_not_declared:${step.stepId}:${override.toolName}`, `skill.definition_override_tool_not_declared:${step.stepId}:${override.toolName}`, {
                        source: "tool",
                ...
                if (overrideMcpIssue) {
                    throw new ValidationError(`skill.mcp_tool_${overrideMcpIssue.code}:${override.toolName}`, `skill.mcp_tool_${overrideMcpIssue.code}:${override.toolName}`, {
                        source: "tool",
    ...
    validateResolvedSteps(skill, resolvedSteps) {
        const declaredTools = new Set(skill.requiredTools);
        for (const step of resolvedSteps) {
            if (!declaredTools.has(step.resolvedToolName)) {
                throw new ValidationError(`skill.resolved_tool_not_declared:${step.stepId}:${step.resolvedToolName}`, `skill.resolved_tool_not_declared:${step.stepId}:${step.resolvedToolName}`, {
                    source: "tool",
            ...
            if (mcpIssue) {
                throw new ValidationError(`skill.mcp_tool_${mcpIssue.code}:${step.resolvedToolName}`, `skill.mcp_tool_${mcpIssue.code}:${step.resolvedToolName}`, {
                    source: "tool",
    ...
    validateAllowedTools(skill, resolvedSteps, allowedTools) {
...
//# sourceMappingURL=skill-execution-core-methods.js.map