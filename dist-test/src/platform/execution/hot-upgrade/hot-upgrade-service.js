...
export class HotUpgradeService {
    ...
    registerVersionCompatibility(compat) {
        this.db.connection
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?)`)
            .run(newId("vcompat"), compat.fromVersion, compat.toVersion, compat.compatibilityLevel, compat.migrationRequired ? 1 : 0, compat.rollbackSupported ? 1 : 0, nowIso());
    }
    getVersionCompatibility(fromVersion, toVersion) {
        const row = this.db.connection
            ...
            .get(fromVersion, toVersion);
        return row ? this.mapCompatibility(row) : null;
    }
    isUpgradeSafe(fromVersion, toVersion) {
    ...
    // ── Upgrade Planning ───────────────────────────────────────────────
    createUpgradePlan(upgradeId, targets, policy) {
        ...
        this.db.transaction(() => {
            this.db.connection
                .prepare(`INSERT INTO upgrade_plans (plan_id, upgrade_id, created_at, targets_json, batches_json, policy_json, current_phase, status)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
                .run(plan.planId, plan.upgradeId, plan.createdAt, JSON.stringify(plan.targets), JSON.stringify(plan.batches), JSON.stringify(plan.policy), plan.currentPhase, plan.status);
    ...
    startBatch(batchId) {
        return this.db.transaction(() => {
            ...
            batch.startedAt = now;
            this.db.connection
                .prepare(`UPDATE upgrade_batches SET status = 'in_progress', started_at = ?, health_checks_json = ? WHERE batch_id = ?`)
    ...
    completeBatch(batchId, healthChecks) {
        return this.db.transaction(() => {
            const row = this.db.connection
                .prepare(`SELECT * FROM upgrade_batches WHERE batch_id = ?`)
                .get(batchId);
    ...
    triggerRollback(upgradeId, reason, message) {
        return this.db.transaction(() => {
            const trigger = {
                triggerId: newId("rbt"),
                upgradeId,
    ...
    // ── Audit Trail ────────────────────────────────────────────────────
    recordAudit(upgradeId, eventType, actor, message, details) {
...
//# sourceMappingURL=hot-upgrade-service.js.map