...
import { mapEpoch, mapFailoverDecision, mapLease, mapNode, } from "./mappers.js";
...
export class HaCoordinatorService {
    ...
    // ── Node Management ────────────────────────────────────────────────
    registerNode(nodeId, region, metadata) {
        return this.db.transaction(() => {
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
                .run(node.nodeId, node.region, node.status, node.isLeader ? 1 : 0, node.leadershipEpoch, node.lastHeartbeatAt, node.metadata ? JSON.stringify(node.metadata) : null, existing?.lastHeartbeatAt ?? now, now);
    ...
    acquireLeadership(input) {
        return this.db.transaction(() => {
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
            ...
            this.db.connection
                .prepare(`INSERT INTO leadership_epochs (epoch, leader_node_id, started_at, cause, fencing_token)
           VALUES (?, ?, ?, ?, ?)`)
     ...
     */
    authorizeAction(requestingNodeId, actionType, requiredAuthority) {
        const now = nowIso();
    ...
    getFailoverHistory(limit = 100) {
        return this.db.connection
            ...
            .all(limit).map((r) => mapFailoverDecision(r));
    }
    ...
    purgeOldFailoverDecisions(olderThanDays = 7) {
    ...
    recordActionAudit(actionType, requestingNodeId, leaderNodeId, epoch, fencingToken, authorized, reasonCode) {
...
//# sourceMappingURL=ha-coordinator-service-inner.js.map