...
export class PromptModelPolicyGovernanceService {
    ...
    registerModelRelease(input) {
        ...
        const rollbackProfileName = normalizeNullableString(input.rollbackProfileName ?? fallbackProfiles[0] ?? null);
        return this.insertRelease({
            releaseType: "model",
    ...
    evaluateReleaseGate(input) {
        ...
        if (suiteId == null) {
            throw new ValidationError(`governance.release_missing_eval_suite:${release.id}`, `governance.release_missing_eval_suite:${release.id}`, {
                retryable: false,
        ...
        const gate = this.evalService.runCiGate(suiteId, input.modelId, input.promptVersion, gateOptions);
        ...
        this.db.transaction(() => {
            this.db.connection
                ...
                .run(event.id, event.releaseId, event.suiteId, event.modelId, event.promptVersion, event.baselinePromptVersion, event.decision, event.verdict, event.passed ? 1 : 0, event.shouldDegrade ? 1 : 0, event.recommendedFallbackKey, event.summary, event.createdAt, event.metadata);
    ...
    insertRelease(input) {
        ...
        this.db.connection
            ...
            .run(record.id, record.releaseType, record.objectKey, record.version, record.owner, record.reviewRequired ? 1 : 0, record.rolloutScope, record.rollbackVersion, record.evaluationSuiteId, record.status, record.metadata, record.createdAt, record.updatedAt);
        return record;
    }
    /**
     * Rolls back sibling releases when a new one becomes active.
     */
    promoteSiblingRollbackState(release, nextStatus) {
    ...
    mapGateEvent(row) {
        return {
            ...
            decision: String(row.decision ?? "hold"),
            verdict: String(row.verdict ?? "inconclusive"),
 ...
 */
function determineGateDecision(release, gate) {
    if (gate.passed) {
 ...
 */
function determineNextReleaseStatus(release, gate, promoteTo) {
    if (gate.passed) {
 ...
 */
function resolveRecommendedFallbackKey(release) {
    if (release.releaseType !== "model") {
        return release.rollbackVersion;
    }
    const metadata = JSON.parse(release.metadata);
 ...
 */
function mapReleaseStatusToProfileStatus(status) {
    switch (status) {
        case "active":
        case "approved":
        case "canary":
 ...
 */
function normalizeRequiredString(value, reasonCode) {
...
//# sourceMappingURL=prompt-model-policy-governance-service.js.map