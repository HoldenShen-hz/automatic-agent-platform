...
import { classifyAnomalyEvent, newId, nowIso, } from "../../contracts/types/index.js";
import { DEFAULT_CONFIG, } from "./anomaly-detection/constants.js";
/**
...
export class AnomalyDetectionService {
    config;
    history = new BoundedCache(100);
    thresholds = new BoundedCache(100);
    anomalies = new BoundedCache(100);
    signatures = new BoundedCache(50);
    constructor(options) {
     ...
     */
    ingest(metricName, value, timestamp) {
     ...
     */
    detect(metricName, value, timestamp) {
        const ts = timestamp ?? nowIso();
     ...
     */
    checkSignatures(metricName, value, timestamp) {
        const combined = `${metricName} ${value} ${timestamp}`;
        for (const sig of this.signatures.values()) {
            if (sig.pattern.test(combined)) {
                const record = this.createAnomalyRecord(metricName, timestamp, sig.category, sig.severity, 1.0, value, value, { signatureId: sig.id, signatureName: sig.name });
     ...
     */
    detectZScore(metricName, value, timestamp) {
        const series = this.history.get(metricName) ?? [];
        ...
        if (isAnomaly) {
            const record = this.createAnomalyRecord(metricName, timestamp, category, severity, score, mean, value, { algorithm: "zscore", zScore, stdDev, threshold });
            this.addAnomalyRecord(metricName, record);
        }
        return {
            isAnomaly,
     ...
     */
    detectIQR(metricName, value, timestamp) {
        const series = this.history.get(metricName) ?? [];
        ...
        if (isAnomaly) {
            const record = this.createAnomalyRecord(metricName, timestamp, category, severity, score, mid, value, { algorithm: "iqr", q1, q3, iqr, upper, lower });
            this.addAnomalyRecord(metricName, record);
        }
        return {
            isAnomaly,
     ...
     */
    detectEWMA(metricName, value, timestamp) {
        const series = this.history.get(metricName) ?? [];
        const baseline = series.slice(-this.config.windowSize);
        ...
        if (isAnomaly) {
            const record = this.createAnomalyRecord(metricName, timestamp, category, severity, score, ewma, value, { algorithm: "ewma", alpha, ewmaStd, threshold });
            this.addAnomalyRecord(metricName, record);
        }
        return {
            isAnomaly,
     ...
     */
    detectGradient(metricName, value, timestamp) {
        const series = this.history.get(metricName) ?? [];
        const baseline = series.slice(-this.config.windowSize);
        ...
        const recentGradient = (last.value - first.value) /
        ...
        const score = isAnomaly
            ...
            : 0;
        const severity = this.scoreToSeverity(score);
        const category = recentGradient > slope * 1.5 ? "spike" : recentGradient < slope * 0.5 ? "dip" : "trend_change";
        const classified = this.classifyDetection(metricName, severity, {
            algorithm: "gradient",
            score,
            category,
        ...
        if (isAnomaly) {
            const record = this.createAnomalyRecord(metricName, timestamp, category, severity, score, expectedValue, value, { algorithm: "gradient", slope, recentGradient, deviation });
            this.addAnomalyRecord(metricName, record);
        }
        return {
            isAnomaly,
     ...
     */
    createAnomalyRecord(metricName, timestamp, category, severity, score, expectedValue, observedValue, context) {
        ...
        return {
            ...
            deviationPercent: expectedValue !== 0 ? (Math.abs(observedValue - expectedValue) / Math.abs(expectedValue)) * 100 : 0,
            context,
            resolved: false,
            resolvedAt: null,
        };
    }
    classifyDetection(metricName, severity, context) {
     ...
     */
    addAnomalyRecord(metricName, record) {
     ...
     */
    getAnomalies(metricName, options) {
        const allRecords = metricName ? this.anomalies.get(metricName) ?? [] : [...this.anomalies.values()].flat();
        return allRecords.filter((r) => {
            if (options?.unresolvedOnly && r.resolved)
                return false;
            if (options?.since && r.timestamp < options.since)
                return false;
            if (options?.minSeverity) {
                const severityOrder = { info: 0, warning: 1, critical: 2, emergency: 3 };
                if (severityOrder[r.severity] < severityOrder[options.minSeverity])
                    return false;
            }
            return true;
        });
     ...
     */
    unregisterSignature(signatureId) {
     ...
     */
    classifyAnomalyCategory(value, baseline, history) {
        ...
        if (history.length >= 10) {
            const recentMean = history.slice(-5).reduce((s, p) => s + p.value, 0) / 5;
            const olderMean = history.slice(-10, -5).reduce((s, p) => s + p.value, 0) / Math.max(1, 5);
    ...
    analyzeTrend(metricName) {
        ...
        const direction = Math.abs(slope) < avgValue * 0.01 ? "stable" : slope > 0 ? "increasing" : "decreasing";
