...
export class HealthService {
    ...
    buildQueueGovernanceSummary(nowMs) {
        const tickets = this.store.worker.listExecutionTicketsByStatuses(["pending", "claimed"]);
        const queueNames = Array.from(new Set(tickets
            .map((ticket) => ticket.queueName ?? "default")
            .filter((queueName) => queueName.length > 0))).sort();
        ...
        const dispatchableBacklogSize = pendingTickets.filter((ticket) => ticket.dispatchAfter == null || ticket.dispatchAfter <= nowIso)
            .length;
        const oldestWaitSeconds = oldestAgeSeconds(pendingTickets.map((ticket) => ticket.createdAt), nowMs);
        const oldestClaimAgeSeconds = oldestAgeSeconds(claimedTickets
            .map((ticket) => ticket.claimedAt)
            .filter((claimedAt) => claimedAt != null), nowMs);
        return {
            ...
            queueNames,
            starvationDetected: (oldestWaitSeconds ?? 0) >= this.options.queueStarvationThresholdSeconds ||
                (oldestClaimAgeSeconds ?? 0) >= this.options.queueStarvationThresholdSeconds,
        };
    }
    buildWorkerHealthSummary(nowMs) {
        ...
        const loadSkew = summarizeWorkerLoadSkew(workers.map((worker) => {
        ...
        return {
            ...
            remoteConnectedWorkers: workers.filter((worker) => worker.placement === "remote" && worker.remoteSessionStatus === "connected").length,
            ...
            remoteConsistencyMismatchWorkers: workers.filter((worker) => worker.placement === "remote" && worker.sessionConsistencyCheckStatus === "mismatch").length,
            remoteWorkspaceSyncConflictWorkers: workers.filter((worker) => worker.placement === "remote" && worker.workspaceSyncStatus === "conflict").length,
            ...
            staleBusyWorkers: staleWorkers.filter((worker) => worker.status === "busy" || worker.status === "draining" || parseRunningExecutionCount(worker.runningExecutionsJson) > 0).length,
    ...
    buildReport(nowMs, dbWritable) {
        const activeExecutions = this.selectCount("SELECT COUNT(*) AS count FROM executions WHERE status IN ('created','prechecking','executing','blocked')");
        const queuedTasks = this.selectCount("SELECT COUNT(*) AS count FROM tasks WHERE status IN ('queued','pending')");
        const tier1AckBacklog = this.selectCount("SELECT COUNT(*) AS count FROM event_consumer_acks WHERE status = 'pending'");
        const memoryRssMb = Math.round((process.memoryUsage().rss / 1024 / 1024) * 100) / 100;
        const providerSummary = this.options.providerTracker?.getSummary(this.options.providerWindowMs) ?? {
            status: "healthy",
            successRate: 1,
            totalCalls: 0,
        };
        ...
        else if (tier1AckBacklog > this.options.tier1AckOverloadedThreshold ||
        ...
        const queuePressureDetected = queueGovernance.starvationDetected ||
            queueGovernance.backlogSize > this.options.queuedTaskDegradedThreshold ||
            activeExecutions > this.options.activeExecutionOverloadedThreshold ||
            queuedTasks > this.options.queuedTaskDegradedThreshold ||
            workerHealth.staleBusyWorkers > 0;
        const performancePressureDetected = memoryRssMb > this.options.memoryHighWatermarkMb || eventLoopLagMs > this.options.eventLoopLagThresholdMs;
        const severePerformancePressureDetected = memoryRssMb > this.options.memoryHighWatermarkMb * 1.1 ||
            eventLoopLagMs > this.options.eventLoopLagThresholdMs * 1.5;
        const degradationMode = status === "ok"
            ? "none"
            : !dbWritable
...
function oldestAgeSeconds(timestamps, nowMs) {
    let oldest = null;
...
function parseRunningExecutionCount(runningExecutionsJson) {
    try {
        const parsed = JSON.parse(runningExecutionsJson);
