...
export class FairScheduler {
    ...
    releaseResources(tenantId, released) {
        ...
        tenant.currentlyUsed.maxConcurrentWorkflows = Math.max(0, tenant.currentlyUsed.maxConcurrentWorkflows - releasedWorkflows);
        tenant.currentlyUsed.maxConcurrentWorkers = Math.max(0, tenant.currentlyUsed.maxConcurrentWorkers - releasedWorkers);
        tenant.currentlyUsed.llmTokensPerMinute = Math.max(0, tenant.currentlyUsed.llmTokensPerMinute - releasedTokens);
        tenant.currentlyUsed.llmRequestsPerMinute = Math.max(0, tenant.currentlyUsed.llmRequestsPerMinute - releasedRequests);
    }
    /**
    ...
    getAllUtilization() {
        ...
        for (const [tenantId, tenant] of this.tenants) {
            ...
            result.push({
                tenantId,
                utilizationPercent: totalGuaranteed > 0 ? (totalUsage / totalGuaranteed) * 100 : 0,
                isBorrowing: tenant.borrowedResources.workflows > 0
