 ...
 */
import { SINGLE_TASK_GOLDEN_TASKS, } from "./golden-task-runner.js";
/**
 ...
 */
export const STABLE_ACCEPTANCE_P95_BUDGET_MS = {
    interactive: 30_000,
    extended: 120_000,
};
...
function buildLatencyBudgetStatuses(runs, cases) {
    const caseLatencyBands = new Map(cases.map((testCase) => [testCase.id, testCase.metadata.latencyBand]));
    return Object.entries(STABLE_ACCEPTANCE_P95_BUDGET_MS).map(([latencyBand, budgetMs]) => {
 ...
 */
export function buildStableAcceptanceLineReport(options) {
    ...
    const orphanQueueClaimCount = options.repairReport.before.findings.filter((finding) => finding.code === "orphan_queue_claim").length;
    // Count zombie/expired locks from repair findings and doctor lock summary
    const zombieLockCount = Math.max(options.repairReport.before.findings.filter((finding) => finding.code === "expired_file_lock").length, options.doctorReport.lockSummary.expiredLockCount);
    // Count database manual intervention signals
    const manualDbRepairSignalCount = options.repairReport.before.repairActions.filter((action) => action.action === "manual_intervention_required" && action.targetType === "database").length +
    ...
    const automaticRecoveryActions = options.repairReport.before.repairActions.filter((action) => action.action !== "manual_intervention_required");
    const recoverySucceededCount = automaticRecoveryActions.filter((action) => findAppliedRepairResult(options.repairReport.applied, action)?.applied === true).length;
    const recoveryAttemptCount = automaticRecoveryActions.length;
    const recoverySuccessRatePct = recoveryAttemptCount === 0 ? 100 : roundMetric((recoverySucceededCount / recoveryAttemptCount) * 100);
    // Build all criteria
    const criteria = [
        {
            criterionId: "long_run_evidence",
            status: options.soakReport.failedRuns > 0 || options.soakReport.integrityFailures > 0 || options.soakReport.backupFailures > 0
                ? "fail"
                : soakDurationMs >= STABLE_ACCEPTANCE_REQUIRED_DURATION_MS
                    ? "pass"
                    : "partial",
            detail: options.soakReport.failedRuns > 0 || options.soakReport.integrityFailures > 0 || options.soakReport.backupFailures > 0
        ...
        },
        {
            ...
            detail: manualDbRepairSignalCount === 0
        ...
        },
        {
            ...
            detail: orphanQueueClaimCount === 0
        ...
        },
        {
            ...
            status: recoverySuccessRatePct === 100 && options.repairReport.after.status === "pass"
                ? "pass"
                : "fail",
            detail: recoverySuccessRatePct === 100 && options.repairReport.after.status === "pass"
    ...
    if (soakDurationMs < STABLE_ACCEPTANCE_REQUIRED_DURATION_MS) {
        truthNotes.push(`Current evidence covers ${formatDurationHours(soakDurationMs)} of soak duration; this does not truthfully prove a full 14-day continuous run.`);
    }
...
//# sourceMappingURL=stable-acceptance-line.js.map