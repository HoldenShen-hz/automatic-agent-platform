...
import { DiagnosticsService } from "../observability/diagnostics-service.js";
import { HealthService } from "../observability/health-service.js";
import { InspectService } from "../observability/inspect-service.js";
import { ObservabilityRetentionService } from "../observability/observability-retention-service.js";
...
import { buildTakeoverEvidenceSample, resolveStableEvidenceProfile, writeJson, } from "./stable-evidence-bundle-support.js";
import { runStableBackupRestoreRehearsal, writeStableBackupRestoreRehearsalReport, } from "./stable-backup-restore-rehearsal.js";
import { runStableEventReplayRehearsal, writeStableEventReplayRehearsalReport, } from "./stable-event-replay-rehearsal.js";
import { runStableDbWritabilityRehearsal, writeStableDbWritabilityRehearsalReport, } from "./stable-db-writability-rehearsal.js";
import { runStableDbQueueDisconnectRehearsal, writeStableDbQueueDisconnectRehearsalReport, } from "./stable-db-queue-disconnect-rehearsal.js";
import { runStableQueueDeliveryRehearsal, writeStableQueueDeliveryRehearsalReport, } from "./stable-queue-delivery-rehearsal.js";
import { runStableMigrationCompatibilityRehearsal, writeStableMigrationCompatibilityRehearsalReport, } from "./stable-migration-compatibility-rehearsal.js";
import { runStableRollingUpgradeRehearsal, writeStableRollingUpgradeRehearsalReport, } from "./stable-rolling-upgrade-rehearsal.js";
import { runStableMaintenanceRehearsal, writeStableMaintenanceRehearsalReport, } from "./stable-maintenance-rehearsal.js";
import { runStableGrayReleaseRehearsal, writeStableGrayReleaseRehearsalReport, } from "./stable-gray-release-rehearsal.js";
import { runStableChaosSmoke, writeStableChaosSmokeReport, } from "./stable-chaos-smoke.js";
import { runStablePromptInjectionRedTeam, writeStablePromptInjectionRedTeamReport, } from "./stable-prompt-injection-red-team.js";
import { runStableConcurrencyRehearsal, writeStableConcurrencyRehearsalReport, } from "./stable-concurrency-rehearsal.js";
import { runStableLeaseRehearsal, writeStableLeaseRehearsalReport, } from "./stable-lease-rehearsal.js";
import { runStableRollbackRehearsal, writeStableRollbackRehearsalReport, } from "./stable-rollback-rehearsal.js";
import { runStableSoak, writeStableSoakReport } from "./stable-runtime-soak-runner.js";
import { runStableValidation } from "./stable-runtime-validator.js";
import { buildStableAcceptanceLineReport } from "./stable-acceptance-line.js";
export async function createStableEvidenceBundle(options) {
    ...
    const chaosReport = await runStableChaosSmoke({
        outputDir: join(options.outputDir, "chaos"),
    });
    writeStableChaosSmokeReport(artifacts.chaosReportPath, chaosReport);
    const promptInjectionReport = await runStablePromptInjectionRedTeam({
    ...
    const concurrencyReport = await runStableConcurrencyRehearsal({
    ...
    const rollingUpgradeReport = await runStableRollingUpgradeRehearsal({
    ...
    const maintenanceReport = await runStableMaintenanceRehearsal({
    ...
    const grayReleaseReport = await runStableGrayReleaseRehearsal({
    ...
    const eventReplayReport = await runStableEventReplayRehearsal({
    ...
    const dbQueueDisconnectReport = await runStableDbQueueDisconnectRehearsal({
    ...
    const dbWritabilityReport = await runStableDbWritabilityRehearsal({
    ...
    const queueDeliveryReport = await runStableQueueDeliveryRehearsal({
    ...
    const migrationCompatibilityReport = await runStableMigrationCompatibilityRehearsal({
        outputDir: join(options.outputDir, "migration-compatibility"),
    });
    writeStableMigrationCompatibilityRehearsalReport(artifacts.migrationCompatibilityReportPath, migrationCompatibilityReport);
    ...
    const soakReport = options.soakReport ??
        (await runStableSoak({
    ...
    const doctor = new DoctorService(healthService, checker, new RuntimeRecoveryService(store), null, new SqliteReliabilityService(db), `${artifacts.runtimeDbPath}.backup`, null, null, new WorkerRegistryService(store), observabilityRetention, new StalledExecutionEscalationService(new StalledExecutionDetector(store), diagnostics), null, { store });
    ...
    const validationPassed = validationReport.failedRuns === 0 &&
    ...
    const summary = {
        passed: chaosReport.failedScenarios === 0 &&
        ...
        takeoverSampleClosedLoop: takeoverSample.finalTaskStatus === "done" &&
...
//# sourceMappingURL=stable-evidence-bundle.js.map