...
async function runStaleExecutionRepairScenario(outputDir) {
    return measureScenario("stale_execution_repair", async () => {
        ...
        db.close();
        return {
            passed: before.findings.some((finding) => finding.code === "stale_execution") &&
...
async function runOrphanSessionRepairScenario(outputDir) {
    return measureScenario("orphan_session_cleanup", async () => {
        ...
        db.close();
        return {
            passed: before.findings.some((finding) => finding.code === "orphan_session") &&
...
async function runOrphanQueueClaimRepairScenario(outputDir) {
    return measureScenario("orphan_queue_claim_reconciled_via_runtime_repair", async () => {
        ...
        db.close();
        return {
            passed: before.findings.some((finding) => finding.code === "orphan_queue_claim") &&
...
async function runDuplicateApprovalIdempotencyScenario(outputDir) {
...
async function runMissingAckReplayScenario(outputDir) {
    return measureScenario("missing_ack_rebuild_and_replay", async () => {
        ...
        db.close();
        return {
            passed: before.findings.some((finding) => finding.code === "event_consumer_mismatch") &&
...
export async function runStableChaosSmoke(options) {
...
export function writeStableChaosSmokeReport(outputFile, report) {
    writeJson(outputFile, report);
}
//# sourceMappingURL=stable-chaos-smoke.js.map