...
function seedExecution(db, store, input) {
    db.transaction(() => {
...
function seedCrossDivisionRecoveryDataset(dbPath) {
...
async function runCrossDivisionOverviewScenario(outputDir) {
    return measureScenario("cross_division_overview", async () => {
        ...
        db.close();
        return {
            passed: staleRuns.some((item) => item.executionId === "exec-general-stale-drill") &&
                blockedRuns.some((item) => item.executionId === "exec-engineering-blocked-drill") &&
                overview.length === 2 &&
                overview.some((item) => item.divisionId === "engineering_ops" &&
                    item.activeCandidateCount === 1 &&
                    item.blockedApprovalCount === 1 &&
                    item.staleExecutionCount === 0) &&
                overview.some((item) => item.divisionId === "general_ops" &&
                    item.activeCandidateCount === 1 &&
...
async function runCrossDivisionReplayScenario(outputDir) {
    return measureScenario("cross_division_replay_matrix", async () => {
        ...
        db.close();
        return {
            passed: general.outcome === "repair_pending" &&
...
export async function runStableCrossDivisionRecoveryDrill(options) {
...
export function writeStableCrossDivisionRecoveryDrillReport(outputFile, report) {
    writeJson(outputFile, report);
}
//# sourceMappingURL=stable-cross-division-recovery-drill.js.map