 ...
 */
async function runCapableWorkerDispatch(outputDir) {
    return measureScenario("dispatch_claims_capable_worker", async () => {
        ...
        return {
            passed: ticket.outcome === "created" &&
                ...
                decisionPayload?.evaluations.some((item) => item.workerId === "worker-readonly" && item.rejectionReason === "missing_capabilities") === true,
...
async function runAffinityLoadBalancingScenario(outputDir) {
    return measureScenario("dispatch_balances_affinity_against_hotspot_load", async () => {
        ...
        db.close();
        return {
            passed: decision.outcome === "dispatched" &&
                decision.worker?.workerId === "worker-general-spare" &&
                decision.trace?.evaluations.some((item) => item.workerId === "worker-affinity-hotspot" &&
                    item.affinityMatched === true &&
                    item.loadSkewPenaltyApplied === true) === true,
 ...
 */
async function runDispatchAfterScenario(outputDir) {
    return measureScenario("dispatch_respects_dispatch_after", async () => {
        ...
        db.close();
        return {
            passed: early.outcome === "no_ticket" &&
 ...
 */
async function runCapabilityGapScenario(outputDir) {
    return measureScenario("dispatch_reports_no_worker_for_capability_gap", async () => {
        ...
        db.close();
        return {
            passed: decision.outcome === "no_worker" &&
                pendingTicket?.status === "pending" &&
                decision.trace?.outcome === "no_worker" &&
                decisionPayload?.outcome === "no_worker" &&
                decisionPayload?.evaluations.some((item) => item.workerId === "worker-basic" && item.rejectionReason === "missing_capabilities") === true,
 ...
 */
export async function runStableDispatchRehearsal(options) {
 ...
 */
export function writeStableDispatchRehearsalReport(outputFile, report) {
    writeJson(outputFile, report);
}
//# sourceMappingURL=stable-dispatch-rehearsal.js.map