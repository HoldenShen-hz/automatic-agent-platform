...
function buildGrayReleaseRuntimeVersionSnapshot(outputDir) {
...
async function runGrayCohortRoutingScenario(outputDir) {
    return measureScenario("gray_cohort_routes_only_to_canary_worker_group", async () => {
        ...
        db.close();
        const generalRejected = decision.trace?.evaluations.some((evaluation) => evaluation.workerId === "worker-general-stable" && evaluation.rejectionReason === "queue_affinity_mismatch") ?? false;
        const oldGrayRejected = decision.trace?.evaluations.some((evaluation) => evaluation.workerId === "worker-gray-old-version" && evaluation.rejectionReason === "worker_repo_version_mismatch") ?? false;
        ...
        return {
            ...
            details: {
                ...
                evaluations: decision.trace?.evaluations.map((evaluation) => ({
...
async function runGrayRollbackScenario(outputDir) {
    return measureScenario("gray_rollback_switch_restores_stable_routing", async () => {
        ...
        db.close();
        const drainingRejected = afterRollback.trace?.evaluations.some((evaluation) => evaluation.workerId === "worker-gray-pool" && evaluation.rejectionReason === "worker_draining") ?? false;
        const passed = beforeRollback.worker?.workerId === "worker-gray-pool"
        ...
        return {
            ...
            details: {
                ...
                afterRollbackEvaluations: afterRollback.trace?.evaluations.map((evaluation) => ({
                    workerId: evaluation.workerId,
                    accepted: evaluation.accepted,
                    rejectionReason: evaluation.rejectionReason,
                })) ?? [],
...
export async function runStableGrayReleaseRehearsal(options) {
...
export function writeStableGrayReleaseRehearsalReport(outputFile, report) {
    writeJson(outputFile, report);
}
//# sourceMappingURL=stable-gray-release-rehearsal.js.map