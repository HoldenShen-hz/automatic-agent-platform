...
function formatFeatureFlagVersion(flags) {
    return flags.length > 0 ? flags.join(",") : "none_enabled";
}
function buildRollbackRuntimeVersionSnapshot(outputDir) {
...
async function runRuntimeRepairRehearsal(outputDir) {
...
async function runManualTakeoverRehearsal(outputDir) {
    return measureScenario("manual_takeover_rehearsal", async () => {
        ...
        db.close();
        return {
            passed: snapshot.task.status === "done" &&
...
export async function runStableRollbackRehearsal(options) {
...
export function writeStableRollbackRehearsalReport(outputFile, report) {
    writeJson(outputFile, report);
}
//# sourceMappingURL=stable-rollback-rehearsal.js.map