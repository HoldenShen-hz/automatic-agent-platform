...
function buildMaintenanceRuntimeVersionSnapshot(outputDir) {
...
export function buildStableMaintenancePlaybook(input) {
    ...
    const currentVersion = runtimeVersionSnapshot.buildCommit
...
async function runDrainRejectsDispatchScenario(outputDir) {
    return measureScenario("draining_worker_rejects_new_dispatches", async () => {
        ...
        db.close();
        const drainingEvaluation = dispatched.trace?.evaluations.find((evaluation) => evaluation.workerId === "worker-maintenance-draining");
        const passed = activeLease.outcome === "granted"
        ...
        return {
            ...
            details: {
                ...
                drainingEvaluation: drainingEvaluation == null
                    ? null
                    : {
...
async function runStepBoundaryHandoverScenario(outputDir) {
    return measureScenario("step_boundary_handover_preserves_execution_lineage", async () => {
        ...
        db.close();
        const passed = granted.outcome === "granted"
            ...
            && audits.some((audit) => audit.eventType === "lease_handover" && audit.reasonCode === "maintenance_drain_handover")
...
export async function runStableMaintenanceRehearsal(options) {
...
export function writeStableMaintenanceRehearsalReport(outputFile, report) {
    writeJson(outputFile, report);
}
//# sourceMappingURL=stable-maintenance-rehearsal.js.map