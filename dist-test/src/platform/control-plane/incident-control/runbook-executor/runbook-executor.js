...
export class RunbookExecutor {
     ...
     */
    confirmStep(executionId, stepIndex) {
     ...
     */
    async executeStep(executionId, sectionName, stepIndex, simulatedResult) {
        if (!this.currentExecution || this.currentExecution.executionId !== executionId) {
            return null;
        }
        const section = this.currentExecution.sectionResults.find((s) => s.sectionName === sectionName);
     ...
     */
    async simulateStepExecution(command, simulatedResult) {
     ...
     */
    skipStep(executionId, sectionName, stepIndex) {
        if (!this.currentExecution || this.currentExecution.executionId !== executionId) {
            return null;
        }
        const section = this.currentExecution.sectionResults.find((s) => s.sectionName === sectionName);
     ...
     */
    createStepResult(step, baseTime) {
        return {
            step,
            status: "pending",
