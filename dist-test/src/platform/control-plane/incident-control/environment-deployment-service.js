...
function toCanonicalEnvironmentName(environment) {
    switch (environment) {
        case "development":
            return "dev";
        case "production":
...
const DEPLOYMENT_BOUNDARY_ENVS = new Set(["staging", "pre-prod", "prod"]);
 ...
 */
function parseSecondaryGates(value) {
    try {
...
function buildMarkdown(report) {
    ...
    for (const entry of report.entries) {
        ...
        lines.push(`- Missing Readiness: \`${entry.readiness.missingComponentTypes.join(",") || "none"}\``);
...
export class EnvironmentDeploymentService {
    ...
    async buildReport(input = {}) {
        ...
        const releaseConfigs = new Map(this.releasePipelineService.listEnvironmentConfigs().map((item) => [item.environment, item]));
        ...
        const entries = await Promise.all(ENVIRONMENT_ORDER.map((environment, index) => this.buildEntry({
            environment,
            order: index + 1,
            generatedAt,
    ...
    async buildEntry(input) {
        ...
        const staleComponentTypes = [];
        const blockedGateRefs = [];
        let readyCount = 0;
        ...
        const secretInjection = {
            ...
            ready: typeof input.releaseConfig?.configBundleRef === "string" &&
        ...
        return {
            ...
            deployment: {
                bindingCount,
                deploymentModes: uniqueSorted(scopedBindings.map((item) => item.deploymentMode)),
     ...
     */
    resolveHighestReadyEnvironment(entries) {
        let highest = null;
     ...
     */
    resolvePromotionPrerequisiteBlockers(entries, targetEnvironment) {
        const blockers = [];
        for (const environment of ENVIRONMENT_ORDER) {
     ...
     */
    buildRecommendedCommands(targetEnvironment, targetReleaseBundle) {
        const commands = [
            "npm run doctor",
     ...
     */
    async tryDescribeSecret(secretRef) {
...
//# sourceMappingURL=environment-deployment-service.js.map