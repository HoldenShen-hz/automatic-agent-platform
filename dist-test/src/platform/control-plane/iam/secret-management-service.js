...
import { assertEnum, assertNonEmpty, computeLeaseExpiry, computeNextRotationDueAt, createDefaultProviders, normalizeLeaseStatus, normalizeRotationPolicy, toJson, } from "./secret-management-support.js";
export class SecretManagementService {
    ...
    registerSecret(input) {
        return this.db.transaction(() => {
            ...
            const category = assertEnum(input.category, [
                "provider_api_key",
                "tenant_credential",
                "oauth_client_secret",
                "signing_key",
     ...
     */
    listRotationDueSecrets(asOf = nowIso()) {
        return this.store
     ...
     */
    buildAuditSummary(secretRef, generatedAt = nowIso()) {
        const registry = this.requireRegistryRecord(secretRef);
        return {
            registry,
     ...
     */
    requestDueRotations(asOf = nowIso(), requestedBy = "system.rotation") {
     ...
     */
    startDailyRotationScheduler(intervalMs = 24 * 60 * 60 * 1000) {
    ...
    async issueSecretLease(input) {
        ...
        return this.db.transaction(() => {
            ...
            let expiresAt = providerIssuedLease?.expiresAt ?? explicitExpiresAt;
     ...
     */
    listSecretLeases(secretRef, asOf = nowIso()) {
     ...
     */
    requireRegistryRecord(secretRef) {
     ...
     */
    mergeMetadataJson(current, next) {
        if (next == null) {
            return current;
        }
        const parsed = current == null ? {} : JSON.parse(current);
        const currentValue = parsed != null && typeof parsed === "object" && !Array.isArray(parsed)
            ? parsed
            : {};
...
//# sourceMappingURL=secret-management-service.js.map