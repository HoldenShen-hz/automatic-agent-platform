...
import { deriveSecretEnvName, deriveSecretScope, maskSecretValue, validateSecretRef, } from "./env-secret-provider.js";
/**
 * Returns the environment variable prefix for a provider kind.
 */
function providerEnvPrefix(providerKind) {
    switch (providerKind) {
        case "vault":
            return "AA_VAULT";
        case "kms":
 ...
 */
function normalizeRecord(value, code) {
 ...
 */
function normalizeEntry(rawEntry, fallbackLocator) {
    ...
    const locator = typeof rawLocator === "string" && rawLocator.trim().length > 0
        ? rawLocator.trim()
        : fallbackLocator;
    return {
        value,
 ...
 */
function normalizeIssuedLease(rawLease, fallbackLocator) {
 ...
 */
function normalizeSecretRefAlias(key) {
...
export class ExternalSecretProvider {
    ...
    async describeSecret(secretRef) {
        ...
        for (const [key, rawEntry] of Object.entries(configuredSource.entries)) {
            normalizedEntries.set(normalizeSecretRefAlias(key), normalizeEntry(rawEntry, locatorFallback));
        }
        ...
        if (filePath.length > 0) {
            ...
            try {
                return {
                    sourceName: fileEnv,
                    filePath,
                    entries: normalizeRecord(JSON.parse(readFileSync(filePath, "utf8")), `secret.provider_config_invalid:${this.providerKind}:${filePath}`),
                };
            }
            catch (error) {
        ...
        if (inlineJson.length > 0) {
            try {
                return {
                    ...
                    filePath: null,
                    entries: normalizeRecord(JSON.parse(inlineJson), `secret.provider_config_invalid:${this.providerKind}:${inlineEnv}`),
                };
            }
            catch (error) {
                if (error instanceof Error && error.message.startsWith("secret.provider_config_invalid:")) {
 ...
 */
export class ExternalSecretProviderAdapter {
    provider;
    providerKind;
...
//# sourceMappingURL=external-secret-provider.js.map