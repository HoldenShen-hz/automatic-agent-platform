 ...
 */
export function isBlockedOutboundHostname(hostname) {
 ...
 */
export function isSupportedOutboundProtocol(protocol) {
 ...
 */
export function parseSafeOutboundUrl(urlString, errorCodes) {
    let parsed;
    try {
...
export function sanitizeUrlForTelemetry(url) {
    ...
    try {
        ...
        return raw
            .replace(/\/bot[^/]+\//, "/bot***/")
            .replace(/:\/\/[^/@:]+:[^/@]+@/, "://***:***@")
            .replace(/([?&](?:access_token|api_key|auth|authorization|credential|credentials|key|passwd|password|secret|sig|signature|token))=[^&]+/gi, "$1=***");
    }
