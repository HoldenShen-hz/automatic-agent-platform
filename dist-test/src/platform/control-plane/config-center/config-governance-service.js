...
import { BOOTSTRAP_LAYER_SCHEMA, GATEWAYS_LAYER_SCHEMA, PROVIDERS_LAYER_SCHEMA, REQUIRED_LAYER_NAMES, RUNTIME_LAYER_SCHEMA, SECURITY_LAYER_SCHEMA, WORKFLOWS_LAYER_SCHEMA, diffObjects, isPlainObject, mergeConfigObjects, parseJsonObject, sha256, stableStringify, validateLayerSchema, } from "./config-governance-support.js";
export class ConfigGovernanceService {
     ...
     */
    detectTampering(expectedVersionId, environment = "dev") {
        ...
        return {
            ...
            issues: bundle.version.versionId !== expectedVersionId
    ...
    validateBundle(bundle) {
        ...
        if (typeof providers.defaultProvider === "string" &&
        ...
        if (typeof providers.defaultModelProfile === "string" &&
    ...
    loadLayerConfig(configRoot, layerName, environment, effectivePolicy) {
        ...
        if (!defaultCheck.allowed) {
            throw new PolicyDeniedError(defaultCheck.reasonCode ?? "config.layer_denied", defaultCheck.reasonCode ?? "config.layer_denied");
        }
        ...
        if (!environmentCheck.allowed) {
            throw new PolicyDeniedError(environmentCheck.reasonCode ?? "config.environment_layer_denied", environmentCheck.reasonCode ?? "config.environment_layer_denied");
        }
     ...
     */
    buildVersion(layerHashes) {
        const bundleHash = sha256(stableStringify(Object.entries(layerHashes)
            .sort(([left], [right]) => left.localeCompare(right))
            .reduce((accumulator, [layerName, layerHash]) => {
 ...
 */
//# sourceMappingURL=config-governance-service.js.map