...
const PositiveInteger = z.string().refine((v) => {
    const n = Number.parseInt(v, 10);
    return Number.isFinite(n) && n > 0;
}, { message: "must be a positive integer" });
const NonNegativeInteger = z.string().refine((v) => {
    const n = Number.parseInt(v, 10);
    return Number.isFinite(n) && n >= 0;
}, { message: "must be a non-negative integer" });
const PositivePort = z.string().refine((v) => {
    const n = Number.parseInt(v, 10);
 ...
 */
export function requireValidStartupEnv(env = process.env) {
...
//# sourceMappingURL=startup-env-schema.js.map