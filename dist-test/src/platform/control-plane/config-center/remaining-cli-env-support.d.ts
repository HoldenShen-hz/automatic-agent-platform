...
export declare const ENVIRONMENT_NAMES: readonly ["dev", "test", "staging", "pre-prod", "prod"];
export declare const TENANT_ACTIONS: readonly ["create_workspace", "add_workspace_member", "create_organization", "add_organization_member", "create_tenant", "bind_deployment", "create_namespace", "topology"];
export declare const ENTERPRISE_ACTIONS: readonly ["register_readiness", "summary", "export", "list_readiness", "list_reports"];
export declare const MARKETPLACE_ACTIONS: readonly ["register_package", "submit_review", "decide_review", "publish", "revoke", "summary", "export", "list_packages", "list_reviews", "list_publications", "list_reports"];
export declare const DEPLOYMENT_EXECUTION_ACTIONS: readonly ["summary", "export", "build_report"];
export declare const CONTROL_PLANE_ACTIONS: readonly ["summary", "heartbeat", "select"];
export declare const OPS_GOVERNANCE_ACTIONS: readonly ["summary", "export", "check", "report", "audit"];
export declare const SECRET_ACTIONS: readonly ["register", "resolve", "rotate", "issue", "revoke", "leases", "due", "request_due", "refresh", "summary"];
export declare const WORKER_REGISTER_ACTIONS: readonly ["issue", "complete"];
export declare const GATEWAY_TARGET_ACTIONS: readonly ["upsert", "list", "resolve"];
export declare const INSPECT_KINDS: readonly ["task", "execution", "approval", "tasks", "workflows", "decisions", "workers"];
export declare const SKILL_CREATOR_ACTIONS: readonly ["create", "validate"];
export declare const SHADOW_SNAPSHOT_ACTIONS: readonly ["create", "list", "restore"];
export declare const MEMORY_ACTIONS: readonly ["initialize", "remember", "prefetch", "queue_prefetch", "system_prompt_block", "sync_turn", "shutdown", "list", "quality", "consolidate", "revoke"];
export declare const MODEL_ROUTE_CLASSES: readonly ["default", "classification", "writing", "coding", "reasoning"];
export declare const MODEL_ROUTE_RISK_LEVELS: readonly ["low", "medium", "high", "critical"];
/**
...
export interface EnterpriseCapabilityCliEnvConfig {
    ...
    componentType: "provider" | "gateway" | "sandbox" | "worker_fleet" | "artifact_store" | "notification_channel" | "external_service" | null;
 ...
 */
export declare function missingEnv(name: string): never;
/**
 * Throws an invalid environment variable error.
 */
export declare function invalidEnv(name: string): never;
/**
 * Throws an invalid gate value error for secondary gates.
 */
export declare function invalidGateValue(name: string): never;
/**
 * Reads a required environment variable, throwing if missing or empty.
 */
export declare function requiredEnv(env: NodeJS.ProcessEnv, name: string): string;
/**
 * Reads an optional environment variable, returning null if missing or empty.
 */
export declare function optionalEnv(env: NodeJS.ProcessEnv, name: string): string | null;
/**
 ...
 */
export declare function optionalNumber(env: NodeJS.ProcessEnv, name: string): number | null;
/**
 * Parses a required number from environment, throwing if missing or invalid.
 */
export declare function requiredNumber(env: NodeJS.ProcessEnv, name: string): number;
/**
 ...
 */
export declare function optionalEnumValue<const T extends readonly string[]>(env: NodeJS.ProcessEnv, name: string, allowed: T): T[number] | null;
/**
 * Reads a required enum value from environment, throwing if missing or invalid.
 */
export declare function requiredEnumValue<const T extends readonly string[]>(env: NodeJS.ProcessEnv, name: string, allowed: T): T[number];
/**
 * Parses a JSON array of strings from environment variable.
 * @param required - If true, throws on missing; if false, returns null
 */
export declare function parseStringArrayJson(env: NodeJS.ProcessEnv, name: string, required: boolean): string[] | null;
/**
 * Parses a JSON object from environment variable, returning null if missing.
 */
export declare function parseObjectJson(env: NodeJS.ProcessEnv, name: string): Record<string, unknown> | null;
export declare function parseJsonValue(env: NodeJS.ProcessEnv, name: string): unknown | null;
/**
 ...
 */
export declare function parseBooleanMapJson(env: NodeJS.ProcessEnv, name: string): Record<string, boolean> | undefined;
export declare function parseBoolean(env: NodeJS.ProcessEnv, name: string): boolean | undefined;
export declare function parseInteger(env: NodeJS.ProcessEnv, name: string): number | undefined;
export declare function parseStringArrayFromCsv(env: NodeJS.ProcessEnv, name: string): string[] | null;
/**
 * Parses marketplace compatibility JSON with apiContract, permissionSurface, and runtimeCapability fields.
 */
export declare function parseCompatibilityJson(env: NodeJS.ProcessEnv, name: string): MarketplaceCliEnvConfig["compatibility"];
export declare function parseTypedJson<T>(env: NodeJS.ProcessEnv, name: string): T | undefined;
export declare function parseProviderHealthJson(env: NodeJS.ProcessEnv, name: string): Record<string, ProviderHealthSummary>;
export declare function buildStructuredMemoryContent(env: NodeJS.ProcessEnv): StructuredMemoryContent | undefined;
/**
