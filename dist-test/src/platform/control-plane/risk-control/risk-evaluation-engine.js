...
export class RiskEvaluationEngine {
     ...
     */
    computeFactorBreakdown(factors) {
     ...
     */
    computePriorFailureValue(percent) {
        const { priorFailureRateThresholds } = this.config;
        if (percent <= priorFailureRateThresholds.low.maxPercent)
            return priorFailureRateThresholds.low.value;
        if (percent <= priorFailureRateThresholds.medium.maxPercent)
            return priorFailureRateThresholds.medium.value;
        if (percent <= priorFailureRateThresholds.high.maxPercent)
            return priorFailureRateThresholds.high.value;
    ...
    mapScoreToLevel(score) {
        ...
        if (score >= riskLevelThresholds.critical)
            return "critical";
        if (score >= riskLevelThresholds.high)
            return "high";
        if (score >= riskLevelThresholds.medium)
            return "medium";
        return "low";
    }
    /**
     ...
     */
    applyDomainOverride(baseLevel, domainId) {
        if (!domainId)
            return baseLevel;
...
//# sourceMappingURL=risk-evaluation-engine.js.map