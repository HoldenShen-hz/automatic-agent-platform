 ...
 */
export function loadRiskConfig(configPath = DEFAULT_CONFIG_PATH, sandboxPolicy) {
    ...
    if (sandboxPolicy != null) {
        ...
        if (!check.allowed) {
            throw new PolicyDeniedError(check.reasonCode ?? "config.risk_denied", check.reasonCode ?? "config.risk_denied");
        }
