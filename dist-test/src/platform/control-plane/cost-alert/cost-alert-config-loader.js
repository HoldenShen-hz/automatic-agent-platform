 ...
 */
export function loadCostAlertConfig(configPath = DEFAULT_CONFIG_PATH, sandboxPolicy) {
    if (cachedConfig) {
        return cachedConfig;
    }
    ...
    if (sandboxPolicy != null) {
        ...
        if (!check.allowed) {
            throw new PolicyDeniedError(check.reasonCode ?? "config.cost_alert_denied", check.reasonCode ?? "config.cost_alert_denied");
        }
