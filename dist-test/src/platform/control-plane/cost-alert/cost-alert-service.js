...
export class CostAlertService extends EventEmitter {
    ...
    MAX_ACCUMULATORS = 500;
    ACCUMULATOR_TTL_MS = 60 * 60 * 1000; // 1 hour
    lastEvictionTime = 0;
    ...
    recordCost(input) {
        ...
        const thresholdRatio = policy.limitCostUsd
            ...
            : 0;
        const warningThresholdCost = (policy.limitCostUsd ?? Infinity) * (policy.warningThreshold ?? this.config.defaultWarningThreshold);
        const criticalThresholdCost = (policy.limitCostUsd ?? Infinity) * DEFAULT_CRITICAL_THRESHOLD;
     ...
     */
    resetAccumulator(scope, scopeId) {
     ...
     */
    resolvePolicy(scope, scopeId, tenantId) {
        switch (scope) {
            case "platform":
     ...
     */
    getAccumulatorKey(scope, scopeId) {
        return `${scope}:${scopeId}`;
    }
    /**
     * Gets the reason code for exceeded budget.
     */
    getExceededReasonCode(scope, policy) {
        switch (scope) {
            case "step":
...
//# sourceMappingURL=cost-alert-service.js.map