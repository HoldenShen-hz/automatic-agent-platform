...
import { SLACK_API_URL, TELEGRAM_API_URL, } from "../../control-plane/config-center/provider-defaults.js";
import { parseSafeOutboundUrl, sanitizeUrlForTelemetry, } from "../../control-plane/iam/outbound-url-policy.js";
import { GatewayDeliveryError, GatewayRateLimitError, normalizeGatewayDeliveryFailure, } from "./errors.js";
import { parseMetadata, readTrackedDeliveryPayload, requireNonEmpty, } from "./helpers.js";
...
export class ChannelGatewayService {
    ...
    async sendMessage(input) {
        ...
        const trackedMessageId = deliveryService != null
            ? deliveryService.createDeliveryMessage(channel, resolution.entry.targetId, {
                targetId: resolution.entry.targetId,
                text,
     ...
     */
    async processRetryQueue(limit = 100) {
        const deliveryService = this.options.deliveryService;
        if (deliveryService == null) {
            return {
                scanned: 0,
        ...
        for (const queuedMessage of queuedMessages) {
            ...
            try {
                ...
                const outcome = this.recordFailedDelivery(queuedMessage.messageId, queuedMessage.channel, trackedPayload.targetId, deliveryService, error);
     ...
     */
    enforceRateLimit(channel, deliveryService) {
        if (deliveryService == null) {
            return;
        }
        const result = deliveryService.checkRateLimit(channel);
        if (!result.allowed) {
            throw new GatewayRateLimitError(channel, result.retryAfterMs ?? result.windowMs, result.limit, result.currentCount);
        }
    }
    /**
     ...
     */
    recordSuccessfulDelivery(channel, messageId, receipt, deliveryService) {
     ...
     */
    recordFailedDelivery(messageId, channel, targetId, deliveryService, error) {
     ...
     */
    async deliverResolvedTarget(input) {
     ...
     */
    async sendTelegramMessage(targetId, externalTargetId, text) {
        ...
        const requestUrl = parseSafeOutboundUrl(`${config.baseUrl ?? TELEGRAM_API_URL}/bot${config.botToken}/sendMessage`, {
     ...
     */
    async sendSlackMessage(targetId, externalTargetId, text) {
        ...
        const requestUrl = parseSafeOutboundUrl(`${config.baseUrl ?? SLACK_API_URL}/chat.postMessage`, {
     ...
     */
    async sendWebhookMessage(targetId, externalTargetId, text, metadata) {
...
//# sourceMappingURL=channel-gateway-service.js.map