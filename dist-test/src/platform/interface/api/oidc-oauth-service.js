 ...
 */
import { createHmac, createHash, timingSafeEqual, randomBytes, createVerify, createPublicKey } from "node:crypto";
...
import { ecAlgToNode, hmacAlgToNode, rsaAlgToNode, } from "./oidc-oauth/crypto-utils.js";
import { decodeJwtJsonSegment, parseFederatedTokenClaims, parseJwtHeader, } from "./oidc-oauth/jwt-utils.js";
 ...
 */
function throwOidcProviderError(code, details = {}, statusCode = 502) {
 ...
 */
function throwOidcValidationError(code, details = {}) {
...
export class OidcOAuthService {
    providers = new BoundedCache(50);
    jwksCache = new BoundedCache(200);
    apiKeys = new BoundedCache(500);
    rotationKeys = new BoundedCache(100);
    trustedIssuers;
    audience;
    fetchImpl;
    /** Set to true to skip signature verification (for unit testing only) */
    _skipSignatureVerification;
    /**
     ...
     */
    constructor(providers = [], trustedIssuers = [], audience = "automatic-agent-platform", fetchImpl, skipSignatureVerification = false) {
     ...
     */
    async fetchOidcDiscovery(issuer) {
     ...
     */
    async validateFederatedToken(token) {
        try {
     ...
     */
    registerApiKey(apiKey, actorId, roles, expiresAt) {
     ...
     */
    async verifySignature(token, header, signature, issuer) {
        try {
     ...
     */
    verifyRsaSignature(token, signature, key, alg) {
        try {
            if (!key.n || !key.e)
                return false;
     ...
     */
    verifyEcSignature(token, signature, key, alg) {
        try {
            if (!key.x || !key.y || !key.crv)
                return false;
            const jwk = { kty: "EC", x: key.x, y: key.y, crv: key.crv };
            const publicKey = createPublicKey({ format: "jwk", key: jwk });
     ...
     */
    verifyHmacSignature(token, signature, key, alg) {
        try {
     ...
     */
    async exchangeCodeForTokens(code, redirectUri, codeVerifier, provider, clientId, clientSecret) {
     ...
     */
    buildAuthorizationUrl(provider, clientId, redirectUri, state, codeChallenge, scopes) {
...
//# sourceMappingURL=oidc-oauth-service.js.map