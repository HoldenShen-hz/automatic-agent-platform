...
export class DekStore {
        ...
        if (existingActiveDek) {
            ...
            if (existing && existing.status === "active") {
                if (input.replacesDekId !== existing.dekId) {
                    throw new AppError("dek.active_exists", `An active DEK already exists for subject ${input.subjectId}. Rotate the existing DEK first.`, { statusCode: 409, category: "storage", source: "internal" });
                }
            }
            if (existing) {
     ...
     */
    async markRotated(dekId, replacedByDekId) {
        const metadata = this.metadata.get(dekId);
        if (!metadata) {
...
export class DekManager {
     ...
     */
    async createForSubject(subjectId) {
     ...
     */
    async destroyForSubject(subjectId) {
        const activeDek = await this.store.getActiveForSubject(subjectId);
        if (!activeDek) {
            logger.log({
                level: "warn",
     ...
     */
    async encryptForSubject(subjectId, plaintext) {
     ...
     */
    async decrypt(dekId, ciphertext) {
