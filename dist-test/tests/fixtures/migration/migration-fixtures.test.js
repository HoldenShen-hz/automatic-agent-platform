...
import { SQLITE_MIGRATIONS, SQLITE_MIGRATION_LEDGER_SQL, getLatestSqliteMigrationVersion, } from "../../../src/platform/state-evidence/truth/sqlite/sqlite-migration-plan.js";
...
test("applying all migrations populates the ledger with correct entries", () => {
    ...
    try {
        ...
        assert.ok(rows.length >= SQLITE_MIGRATIONS.length - 5, `Ledger should have entries for most migrations (${rows.length}/${SQLITE_MIGRATIONS.length})`);
...
test("each migration SQL is non-empty and contains CREATE TABLE or ALTER", () => {
    for (const migration of SQLITE_MIGRATIONS) {
        assert.ok(migration.sql.length > 0, `Migration ${migration.version} (${migration.name}) should have non-empty SQL`);
        const hasDdl = migration.sql.includes("CREATE TABLE") ||
            migration.sql.includes("ALTER TABLE") ||
            migration.sql.includes("CREATE INDEX") ||
            migration.sql.includes("CREATE UNIQUE INDEX");
        assert.ok(hasDdl, `Migration ${migration.version} (${migration.name}) should contain DDL statements`);
    }
});
test("each migration has a valid checksum", () => {
    for (const migration of SQLITE_MIGRATIONS) {
        assert.ok(migration.checksum.length === 64, `Migration ${migration.version} should have a SHA256 checksum (64 hex chars)`);
        assert.ok(/^[a-f0-9]{64}$/.test(migration.checksum), `Migration ${migration.version} checksum should be lowercase hex`);
    }
});
test("migration names are prefixed with zero-padded version number", () => {
    for (const migration of SQLITE_MIGRATIONS) {
        assert.ok(/^\d{4}_/.test(migration.name), `Migration ${migration.version} name should start with 4-digit version prefix (e.g., 0001_)`);
    }
});
test("migrations can be applied incrementally in order", () => {
    ...
    try {
        ...
        assert.ok(failedMigrations.length <= 5, `Expected at most 5 compatibility skips (got ${failedMigrations.length}): ${JSON.stringify(failedMigrations)}`);
        assert.ok(appliedCount >= SQLITE_MIGRATIONS.length - failedMigrations.length, `Should have applied most migrations (applied ${appliedCount}/${SQLITE_MIGRATIONS.length})`);
        // Verify most expected tables exist
        const tableCount = db.prepare("SELECT COUNT(*) as c FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'").all()[0].c;
...
test("snapshot generation script can generate manifest structure", () => {
    ...
    for (let i = 1; i < SNAPSHOT_VERSIONS.length; i++) {
        assert.ok(SNAPSHOT_VERSIONS[i] > SNAPSHOT_VERSIONS[i - 1], "Snapshot versions should be in ascending order");
    }
});
...
//# sourceMappingURL=migration-fixtures.test.js.map