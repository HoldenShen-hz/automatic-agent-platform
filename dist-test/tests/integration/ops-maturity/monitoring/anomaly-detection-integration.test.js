...
test("AnomalyDetectionService provides root cause hints for error_rate", () => {
    ...
    assert.ok(errorAlert?.rootCauseHints.some(hint => hint.includes("deployments") || hint.includes("error logs")));
});
test("AnomalyDetectionService provides root cause hints for latency", () => {
    ...
    assert.ok(latencyAlert?.rootCauseHints.some(hint => hint.includes("database") || hint.includes("queue")));
});
...
//# sourceMappingURL=anomaly-detection-integration.test.js.map