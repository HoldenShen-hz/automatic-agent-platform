...
import { DegradationController, DegradationLevel, } from "../../../../src/platform/model-gateway/degradation/index.js";
...
test("ModelGateway: CircuitBreaker state transitions", () => {
    ...
    assert.rejects(() => breaker.execute(async () => "should not execute"), (err) => err instanceof CircuitBreakerOpenError && err.circuitName === "test-breaker");
...
test("ModelGateway: Cache service stores and retrieves entries", () => {
    const cache = new ModelGatewayCacheService();
...
test("ModelGateway: DegradationController D4 throws service unavailable", async () => {
    ...
    await assert.rejects(() => controller.route(request), /LLM service is currently unavailable/);
});
...
test("ModelGateway: UnifiedChatProvider complete uses configured model", async () => {
    ...
    await assert.rejects(() => provider.complete("test prompt"), /OpenAI provider is not configured/);
});
...
//# sourceMappingURL=model-gateway-integration.test.js.map