...
test("CircuitBreaker integration: execute respects circuit state", async () => {
    ...
    assert.equal(cb.getState(), "open");
    // Execute should fail
    await assert.rejects(async () => cb.execute(async () => "should not run"), (err) => err instanceof Error && err.message.includes("Circuit breaker"));
});
...
//# sourceMappingURL=circuit-breaker-integration.test.js.map