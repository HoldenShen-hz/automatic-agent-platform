...
test("cost tracking: cost events can be stored and retrieved", () => {
    ...
    try {
        ...
        db.connection
            .prepare(`INSERT INTO tasks (id, parent_id, root_id, division_id, tenant_id, title, status, source, priority, input_json, normalized_input_json, output_json, estimated_cost_usd, actual_cost_usd, error_code, created_at, updated_at, completed_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(taskId, null, taskId, "general_ops", null, "Cost tracking test", "in_progress", "user", "normal", "{}", null, null, 0, 0, null, now, now, null);
        // Insert cost event directly using correct schema columns
        db.connection
            .prepare(`INSERT INTO cost_events (id, task_id, session_id, execution_id, agent_id, provider, model, input_tokens, output_tokens, cost_usd, budget_scope, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(costEventId, taskId, null, null, null, "anthropic", "claude-3-5-sonnet", 1000, 500, 0.025, "task_execution", now);
        // Retrieve cost event
...
test("cost tracking: multiple cost events for same task can be aggregated", () => {
    ...
    try {
        ...
        db.connection
            .prepare(`INSERT INTO tasks (id, parent_id, root_id, division_id, tenant_id, title, status, source, priority, input_json, normalized_input_json, output_json, estimated_cost_usd, actual_cost_usd, error_code, created_at, updated_at, completed_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(taskId, null, taskId, "general_ops", null, "Cost aggregation test", "in_progress", "user", "normal", "{}", null, null, 0, 0, null, now, now, null);
...
test("cost tracking: cost events with different providers", () => {
    ...
    try {
        ...
        db.connection
            .prepare(`INSERT INTO tasks (id, parent_id, root_id, division_id, tenant_id, title, status, source, priority, input_json, normalized_input_json, output_json, estimated_cost_usd, actual_cost_usd, error_code, created_at, updated_at, completed_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(taskId, null, taskId, "general_ops", null, "Cost providers test", "in_progress", "user", "normal", "{}", null, null, 0, 0, null, now, now, null);
        const providers = [
...
//# sourceMappingURL=cost-tracking-integration.test.js.map