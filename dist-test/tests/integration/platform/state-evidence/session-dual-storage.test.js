...
test("[SYS-REL-2.8] dual storage detects and repairs partial write", async () => {
    ...
    try {
        ...
        assert.equal(sessionLines.filter((l) => l.length > 0).length, indexLines.filter((l) => l.length > 0).length, "Session file and task index must have same line count");
    }
    finally {
...
test("[SYS-REL-2.8] verifyDualStorageConsistency detects line count mismatch", async () => {
    ...
    try {
        ...
        const sessionLines = readFileSync(sessionFile, "utf8").trim().split("\n").filter((l) => l.length > 0);
        const indexLines = readFileSync(taskIndexFile, "utf8").trim().split("\n").filter((l) => l.length > 0);
        assert.strictEqual(sessionLines.length, indexLines.length, `Session file and task index must have same line count: session=${sessionLines.length}, index=${indexLines.length}`);
...
test("[SYS-REL-2.8] appendSessionEvent writes to both session and task index", async () => {
    ...
    try {
        ...
        const sessionLines = readFileSync(sessionFile, "utf8").trim().split("\n").filter((l) => l.length > 0);
        const indexLines = readFileSync(taskIndexFile, "utf8").trim().split("\n").filter((l) => l.length > 0);
        assert.strictEqual(sessionLines.length, indexLines.length, "After appendSessionEvent, both files must have same line count");
    }
    finally {
        cleanupPath(workspace);
    }
...
test("[SYS-REL-2.8] recordMessageAdded maintains dual storage consistency", async () => {
    ...
    try {
        ...
        const sessionLines = readFileSync(sessionFile, "utf8").trim().split("\n").filter((l) => l.length > 0);
        const indexLines = readFileSync(taskIndexFile, "utf8").trim().split("\n").filter((l) => l.length > 0);
        assert.strictEqual(sessionLines.length, indexLines.length, "After recordMessageAdded, session and task index must have same line count");
    }
    finally {
        cleanupPath(workspace);
    }
...
test("[SYS-REL-2.8] recordSessionCompleted writes to both storages", async () => {
    ...
    try {
        ...
        const sessionLines = readFileSync(sessionFile, "utf8").trim().split("\n").filter((l) => l.length > 0);
        const indexLines = readFileSync(taskIndexFile, "utf8").trim().split("\n").filter((l) => l.length > 0);
        assert.strictEqual(sessionLines.length, indexLines.length, "After recordSessionCompleted, session and task index must have same line count");
...
//# sourceMappingURL=session-dual-storage.test.js.map