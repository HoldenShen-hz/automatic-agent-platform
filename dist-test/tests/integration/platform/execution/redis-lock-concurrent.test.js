...
class InMemoryLockAdapter {
    ...
    async forceStealAsync(lockKey, newOwner, _reason) {
        const key = `lock:${lockKey}`;
...
test("[SYS-REL-2.2] concurrent extendAsync on same lock grants only one", async () => {
    ...
    assert.ok(succeeded.length === 1 || failed.length === 2, `Expected exactly one successful extend or both failing due to owner mismatch, got ${succeeded.length} succeeded`);
});
test("[SYS-REL-2.2] concurrent forceStealAsync does not create double lock", async () => {
    ...
    const results = await Promise.allSettled(workers.map((owner) => lock.forceStealAsync("shared", owner, "concurrent steal")));
    ...
    const finalLock = lock.locks.get("lock:shared");
    assert.ok(finalLock, "A final lock record should remain after concurrent steals");
    assert.ok(workers.includes(finalLock.owner), `Final owner should be one of the steal contenders, got ${finalLock.owner}`);
});
...
//# sourceMappingURL=redis-lock-concurrent.test.js.map