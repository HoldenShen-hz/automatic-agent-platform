...
import { createPatchBundle } from "../../../../../src/platform/execution/recovery/patch-bundle.js";
import { createValidationReport } from "../../../../../src/platform/execution/recovery/validation-report.js";
...
function createTestTaskCard(riskLevel = "medium") {
...
function createTestPatchBundle(taskId) {
...
function createTestValidationReport(taskId, bundleId, decision, passedChecks = 1, failedChecks = 0) {
    const checks = [];
    if (passedChecks > 0) {
        checks.push({
            checkId: newId("check"),
...
function createTestReviewReport(taskId, bundleId, verdict, blockingIssues = 0) {
    const issues = [];
...
test("recovery: blocked execution awaiting approval can be detected", () => {
    ...
    try {
        ...
        db.transaction(() => {
            ...
            db.connection
                .prepare(`INSERT INTO approvals (id, task_id, execution_id, status, request_json, response_json, timeout_policy, created_at, responded_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
                .run(approval.id, approval.taskId, approval.executionId, approval.status, approval.requestJson, approval.responseJson, approval.timeoutPolicy, approval.createdAt, approval.respondedAt);
        });
...
//# sourceMappingURL=execution-recovery-flow.test.js.map