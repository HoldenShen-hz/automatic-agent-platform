...
test("worker registry service filters workers by capability, queue affinity, and free capacity", () => {
    ...
    try {
        ...
        db.close();
        assert.deepEqual(eligibleDefault.map((worker) => worker.workerId), ["worker-a"]);
        ...
        assert.deepEqual(eligibleResearch.map((worker) => worker.workerId), ["worker-c"]);
...
test("worker registry service reports stale workers by heartbeat age", () => {
    ...
    try {
        ...
        db.close();
        assert.deepEqual(staleWorkers.map((worker) => worker.workerId), ["worker-old"]);
    }
    finally {
        cleanupPath(workspace);
    }
...
test("worker registry service enforces required isolation levels with stronger workers satisfying weaker requirements", () => {
    ...
    try {
        ...
        db.close();
        assert.deepEqual(hardenedEligible.map((worker) => worker.workerId), ["worker-strict"]);
        assert.equal(hardenedEligible[0]?.isolationLevel, "strict");
        assert.deepEqual(standardEligible.map((worker) => worker.workerId), ["worker-standard", "worker-strict"]);
    }
    finally {
        cleanupPath(workspace);
    }
});
//# sourceMappingURL=worker-registry-service.test.js.map