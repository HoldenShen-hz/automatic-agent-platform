...
test("runtime recovery replay service builds a deterministic dead-letter replay timeline", () => {
    ...
    try {
        ...
        db.transaction(() => {
           ...
           WHERE id = ?`)
                .run(2, "unexpected_runtime_error", "tool crashed twice", "2026-04-04T10:05:00.000Z", "exec-replay-dead-letter");
        ...
        assert.deepEqual([...new Set(executionReport?.timeline.map((event) => event.eventType))].sort(), ["recovery:dead_lettered", "recovery:decision_recorded", "recovery:repair_applied"]);
...
test("runtime recovery replay service marks precheck-denied execution as cancelled", () => {
    ...
    try {
        ...
        assert.deepEqual(report.timeline.map((event) => event.eventType), ["recovery:decision_recorded", "recovery:cancelled"]);
...
//# sourceMappingURL=runtime-recovery-replay-service.test.js.map