...
test("single-task execution persists task, workflow, execution, session, and tier1 events", async () => {
    ...
    try {
        ...
        assert.deepEqual(snapshot.events.map((event) => event.eventType), ["task:status_changed", "workflow:step_completed", "task:status_changed"]);
        ...
        try {
            ...
            assert.deepEqual(JSON.parse(persistedExecution.allowedToolsJson ?? "[]").sort(), ["bash", "read"]);
            assert.deepEqual(JSON.parse(persistedPrecheck.resolvedToolsJson ?? "[]").sort(), ["bash", "read"]);
...
test("single-task execution respects queue-only admission backpressure and defers execution", async () => {
    ...
    try {
        ...
        assert.deepEqual(snapshot.events.map((event) => event.eventType), ["admission:queued"]);
...
test("single-task execution rejects invalid step output before commit", async () => {
    const workspace = createTempWorkspace("aa-happy-");
    try {
        await assert.rejects(async () => await runSingleTaskExecution({
