...
test("[SYS-REL-2.6] task state transition writes outbox entry in same transaction", async () => {
    ...
    try {
        ...
        assert.ok(outboxEntries.length > 0, "Outbox entry must exist after task transition (defect: transition-service bypasses outbox)");
...
test("[SYS-REL-2.6] multiple transitions write multiple outbox entries", async () => {
    ...
    try {
        ...
        assert.ok(outboxEntries.length >= 2, `Expected at least 2 outbox entries for 2 transitions, got ${outboxEntries.length}`);
        db.close();
    }
    finally {
        cleanupPath(workspace);
...
//# sourceMappingURL=outbox-transition-integrations.test.js.map