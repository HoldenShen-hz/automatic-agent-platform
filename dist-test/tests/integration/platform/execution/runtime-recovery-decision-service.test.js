...
test("runtime recovery decision service moves repeated execution failures into dead letter and records audit events", () => {
    ...
    try {
        ...
        db.transaction(() => {
           ...
           WHERE id = ?`)
                .run("executing", 2, "unexpected_runtime_error", "tool crashed twice", "2026-04-04T10:05:00.000Z", "exec-dead-letter");
        ...
        assert.equal(structuredFailureMemory.facts.some((fact) => fact.category === "reason_code" && fact.content === "unexpected_runtime_error"), true);
        assert.ok(view.recentRecoveryEvents.some((event) => event.eventType === "recovery:decision_recorded"));
        assert.ok(view.recentRecoveryEvents.some((event) => event.eventType === "recovery:dead_lettered" && event.deadLetterId === deadLetter?.id));
...
//# sourceMappingURL=runtime-recovery-decision-service.test.js.map