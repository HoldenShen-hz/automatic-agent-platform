...
test("smoke: fresh database runs all migrations successfully", () => {
    ...
    try {
        ...
        for (const essential of essentialTables) {
            assert.ok(tableNames.includes(essential), `Essential table "${essential}" should exist after migration`);
        }
        db.close();
    }
    finally {
...
test("smoke: database WAL mode is enabled", () => {
    ...
    try {
        ...
        const walMode = db.connection
            ...
            .get();
        assert.strictEqual(walMode.journal_mode.toLowerCase(), "wal", "Database should be in WAL mode");
        db.close();
    }
    finally {
        cleanupPath(workspace);
...
//# sourceMappingURL=database-smoke.test.js.map