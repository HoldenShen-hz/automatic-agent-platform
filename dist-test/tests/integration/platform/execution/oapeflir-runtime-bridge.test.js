...
test("RuntimeExecuteBridge executes a 2-step OAPEFLIR plan via runMultiStepOrchestration oapeflir://plan bypass", async () => {
    ...
    try {
        ...
        const result = await bridge.executePlan(plan, { taskId: "task_test" });
        ...
        if (result.results.length > 0) {
            assert.ok(result.results.every((r) => ["succeeded", "failed", "skipped"].includes(r.status)), "all emitted step results should have valid status");
        }
...
test("OapeflirLoopService with RuntimeExecuteBridge executes plan and returns step outputs", async () => {
    ...
    try {
        ...
        assert.deepEqual(result.timeline.map((e) => e.stage), ["observe", "assess", "plan", "execute", "feedback", "learn", "improve", "release"]);
        ...
        assert.ok(result.stepOutputs.every((o) => o.stepId && o.planRef), "step outputs should have stepId and planRef");
    }
    finally {
        cleanupPath(workspace);
    }
...
test("OapeflirLoopService with dbPath uses RuntimeExecuteBridge by default", async () => {
    ...
    try {
        ...
        assert.ok(result.timeline.some((e) => e.stage === "execute"), "execute stage should be recorded");
        assert.equal(result.outcome.taskId, "task_default_test");
        assert.ok(["completed", "failed", "repairable", "partial", "escalated"].includes(result.feedback.outcome), "feedback outcome should remain within the declared contract");
    }
    finally {
        cleanupPath(workspace);
    }
});
//# sourceMappingURL=oapeflir-runtime-bridge.test.js.map