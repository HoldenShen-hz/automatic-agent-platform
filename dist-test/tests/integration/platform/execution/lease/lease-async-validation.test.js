...
test("lease async: acquire grants lease and returns decision", async () => {
    ...
    try {
        ...
        const repo = createLeaseRepository({ driver: "sqlite", sql: db, sqlite: db });
...
test("lease async: reclaimExpiredLeases returns reclaimed lease IDs", async () => {
    ...
    try {
        ...
        assert.ok(reclaimed.every((id) => typeof id === "string"), "All reclaimed IDs should be strings");
        db.close();
    }
    finally {
        cleanupPath(workspace);
...
//# sourceMappingURL=lease-async-validation.test.js.map