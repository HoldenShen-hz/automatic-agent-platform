...
test("[SYS-REL-2.6] task state transition writes outbox entry", () => {
    ...
    try {
        ...
        const payload = JSON.parse(String(outboxEntries[0]?.payload_json ?? "{}"));
...
test("[SYS-REL-2.6] task terminal transition writes outbox entry", () => {
    ...
    try {
        ...
        const latestPayload = JSON.parse(String(outboxEntries.at(-1)?.payload_json ?? "{}"));
