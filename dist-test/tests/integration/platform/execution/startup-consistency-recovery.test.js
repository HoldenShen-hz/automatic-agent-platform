...
function seedStartupConfigTree(root, overrides = {}) {
    const files = {
...
test("startup consistency checker produces repair actions for recoverable Week 3 scenarios", () => {
    ...
    try {
        ...
        store.updateExecutionStatus("exec-terminal-ticket", "succeeded", "2026-04-03T10:09:11.000Z", null, "2026-04-03T10:09:11.000Z", null);
        const checker = new StartupConsistencyChecker(db, store);
        const report = checker.run({
            now,
            staleExecutionAfterMs: 60_000,
        ...
        assert.ok(report.findings.some((finding) => finding.code === "terminal_execution_ticket" && finding.entityId === terminalTicket.ticket.id));
...
test("runtime repair service reclaims stale execution ownership and recreates a pending ticket for redispatch", async () => {
    ...
    try {
        ...
        db.transaction(() => {
            ...
            store.updateExecutionStatus("exec-ownership-recovery", "created", staleAt, staleAt, null, null);
        });
        workers.recordHeartbeat({
...
//# sourceMappingURL=startup-consistency-recovery.test.js.map