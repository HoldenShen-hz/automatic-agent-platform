...
function makeRouteInput(title, request) {
...
test("IntakeRouter detects orchestration hints and routes to multi-step workflow", () => {
    ...
    try {
        ...
        assert.ok(result.workflowId === "single_division_multi_step_orchestration" ||
            result.workflowId.includes("orchestration"), "Should route to orchestration workflow");
    }
    finally {
        ctx.cleanup();
    }
...
test("IntakeRouter assigns correct division ID", () => {
    ...
    try {
        ...
        assert.ok(result.divisionId === "general_ops" ||
...
//# sourceMappingURL=intake-router-integration.test.js.map