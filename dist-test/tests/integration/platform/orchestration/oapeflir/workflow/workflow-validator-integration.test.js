...
function createValidWorkflow(overrides) {
    return {
...
test("assertWorkflowValid throws for invalid workflow", () => {
    const workflow = createValidWorkflow({ steps: [] });
    assert.throws(() => assertWorkflowValid(workflow), (err) => err instanceof ValidationError && err.code === "workflow.invalid:workflow.empty");
});
