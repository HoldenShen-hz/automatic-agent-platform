/**
 * Integration Test: HITL Decision Flow
 *
 * Tests HITL (Human-in-the-Loop) decision flow integration with
 * approval orchestration, explainability, and operator console services.
 */
...
function createHitlContext(prefix) {
...
function makeConstraintPack(override = {}) {
    return {
...
test("integration: hitl approval orchestration creates packet and explanation for high-risk task", async () => {
...
test("integration: hitl approval orchestration applies confirmed decision with continue effect", async () => {
...
test("integration: hitl approval orchestration applies text_input decision with revise_plan effect", async () => {
...
test("integration: hitl approval orchestration applies rejected decision with block_candidate effect", async () => {
...
test("integration: hitl approval orchestration builds timeout decision for approve policy", async () => {
...
test("integration: hitl approval orchestration builds timeout decision for reject policy", async () => {
...
test("integration: hitl runtime opens and resolves human review requests", () => {
...
test("integration: hitl runtime rejects request and run aborts", () => {
    const ctx = createHitlContext("aa-hitl-reject-");
    try {
        const runtime = new HitlRuntime();
        const request = runtime.open({
...
test("integration: hitl explainability service generates decision explanation", () => {
    const ctx = createHitlContext("aa-hitl-explain-");
    try {
        const explainabilityService = new HITLExplainabilityService(ctx.store);
        const explanation = explainabilityService.explainApprovalRequired("task_hitl_exp_001", {
            riskLevel: "high",
...
test("integration: hitl explainability service records and retrieves satisfaction feedback", () => {
    const ctx = createHitlContext("aa-hitl-feedback-");
    try {
        const explainabilityService = new HITLExplainabilityService(ctx.store);
        const feedback = explainabilityService.recordFeedback(4, "satisfaction", "operator_feedback_001", {
...
test("integration: hitl explainability service calculates overall satisfaction metrics", () => {
...
test("integration: harness openHitlReview transitions run to waiting_hitl status", () => {
...
test("integration: harness resolveHitlReview approves and resumes run", () => {
    ...
    try {
        ...
        const resolvedPayload = resolvedEvents[0].payload;
...
test("integration: harness resolveHitlReview rejects and aborts run", () => {
...
test("integration: hitl approval blocks critical auto-approve without break-glass", async () => {
    ...
    try {
        ...
        }, /hitl_approval\.critical_timeout_auto_approve_forbidden/, "Should reject critical risk level with approve timeout without break-glass");
    }
    finally {
        ctx.db.close();
        cleanupPath(ctx.workspace);
...
test("integration: hitl approval allows critical with break-glass approved", async () => {
...
test("integration: hitl approval orchestration list and get packets", async () => {
...
//# sourceMappingURL=hitl-integration.test.js.map