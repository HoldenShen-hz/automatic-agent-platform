...
import { HarnessRuntimeService, HitlRuntime, } from "../../../../../../src/platform/orchestration/harness/index.js";
...
test("resolveHitlReview with rejected aborts the run", () => {
    ...
    try {
        ...
        // Verify HITL resolution timeline event
        const hitlResolvedEvent = rejected.timeline.find((e) => e.type === "hitl_resolved");
        assert.ok(hitlResolvedEvent);
        assert.deepEqual(hitlResolvedEvent.payload, {
            resolution: "rejected",
...
test("resolveHitlReview throws when run has no hitlRequest", () => {
    ...
    try {
        ...
        assert.throws(() => service.resolveHitlReview(run, "approved", "someone"), /harness\.hitl\.request_not_found/);
    }
    finally {
        ctx.cleanup();
    }
...
//# sourceMappingURL=hitl-flow.integration.test.js.map