...
function createOapeflirContext(prefix) {
...
test("integration: harness maps planner step to OAPEFLIR plan phase", () => {
    ...
    try {
        ...
        const semanticPhase = mapHarnessStepToOapeflirPhase(lastStep.role, lastStep.stage);
...
test.skip("integration: OAPEFLIR routing respects iteration budget", () => {
    ...
    try {
        ...
        assert.ok(violations.every((v) => v !== "harness.invariant.iteration_exceeds_budget"), "Should not exceed budget");
    }
    finally {
        ctx.db.close();
        cleanupPath(ctx.workspace);
...
//# sourceMappingURL=oapeflir-routing-integration.test.js.map