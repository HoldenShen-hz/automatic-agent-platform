...
import { buildDefaultStartupConfigValidator, buildEnvironmentProviderReadinessProbe, } from "../../../../../src/platform/execution/startup/startup-preflight.js";
...
test("doctor service summarizes a clean runtime as ok", async () => {
    ...
    try {
        ...
        const doctor = new DoctorService(healthService, new StartupConsistencyChecker(db, store), new RuntimeRecoveryService(store), stalledDetector, new SqliteReliabilityService(db), `${dbPath}.backup`, null, null, workers, null, new StalledExecutionEscalationService(stalledDetector, diagnostics), null, { store });
...
test("doctor service fail-closes when the authoritative store is not writable", () => {
    ...
    try {
        ...
        const workers = new WorkerRegistryService(store);
        const healthService = new HealthService(db, store);
        const healthInternals = healthService;
        ...
        const doctor = new DoctorService(healthService, new StartupConsistencyChecker(db, store), new RuntimeRecoveryService(store), null, new SqliteReliabilityService(db), `${dbPath}.backup`, null, null, workers, null, null, null, { store });
        const report = doctor.run();
...
test("doctor service fail-closes when tier1 audit integrity is compromised", () => {
    ...
    try {
        ...
        assert.ok(report.checks.find((check) => check.checkId === "audit_integrity")?.findings.includes("audit_event_checksum_mismatch:evt-audit-doctor"));
        db.close();
    }
    finally {
        cleanupPath(workspace);
...
test("doctor service groups expired locks and tier1 backlog into unified self-check findings", () => {
    ...
    try {
        ...
        const doctor = new DoctorService(new HealthService(db, store), new StartupConsistencyChecker(db, store), new RuntimeRecoveryService(store), new StalledExecutionDetector(store), new SqliteReliabilityService(db), `${dbPath}.backup`, null, null, workers, null, null, null, { store });
        const report = doctor.run();
...
test("doctor service exposes build and feature flag metadata in the version snapshot", async () => {
    ...
    try {
        await withEnv({
            ...
            const store = new AuthoritativeTaskStore(db);
            const workers = new WorkerRegistryService(store);
            const doctor = new DoctorService(new HealthService(db, store), new StartupConsistencyChecker(db, store), new RuntimeRecoveryService(store), new StalledExecutionDetector(store), new SqliteReliabilityService(db), `${dbPath}.backup`, null, null, workers);
            const report = doctor.run();
...
test("doctor service degrades when a running execution becomes stalled", () => {
    ...
    try {
        ...
        const doctor = new DoctorService(healthService, new StartupConsistencyChecker(db, store), new RuntimeRecoveryService(store), stalledDetector, new SqliteReliabilityService(db), join(workspace, "doctor-stalled.backup.db"), null, null, workers, null, new StalledExecutionEscalationService(stalledDetector, diagnostics));
        const report = doctor.run();
        assert.equal(report.status, "degraded");
...
test("doctor service degrades when workflow terminal state is not reconciled into task and session status", () => {
    ...
    try {
        ...
        db.transaction(() => {
            ...
            store.updateSessionStatus("sess_task-doctor-terminal-mismatch", "streaming", "2026-04-04T18:05:00.000Z");
        });
        const doctor = new DoctorService(healthService, new StartupConsistencyChecker(db, store), new RuntimeRecoveryService(store), new StalledExecutionDetector(store), new SqliteReliabilityService(db), `${join(workspace, "doctor-terminal-mismatch.db")}.backup`);
        const report = doctor.run();
        assert.equal(report.status, "degraded");
...
test("doctor service degrades when an active execution exceeds a resource ceiling", async () => {
    ...
    try {
        ...
        db.connection.prepare(`UPDATE executions SET status = ?, started_at = ?, updated_at = ? WHERE id = ?`).run("executing", "2026-04-04T10:00:00.000Z", "2026-04-04T10:00:05.000Z", "exec-resource-limit");
        store.insertWorkflowState({
            taskId: "task-resource-limit",
            divisionId: "general_ops",
            workflowId: "single_agent_minimal",
        ...
        const doctor = new DoctorService(new HealthService(db, store), new StartupConsistencyChecker(db, store), new RuntimeRecoveryService(store), new StalledExecutionDetector(store), new SqliteReliabilityService(db), `${dbPath}.backup`, null, null, workers, null, null, new ExecutionResourceMonitor(store, new ExecutionResourceCeilingGuard({
            maxToolCalls: 2,
            maxMemoryMb: null,
            maxElapsedMs: null,
        })));
...
test("doctor service fail-closes when startup consistency reports invalid tool contracts", async () => {
    ...
    try {
        ...
        const store = new AuthoritativeTaskStore(db);
        const workers = new WorkerRegistryService(store);
        const doctor = new DoctorService(new HealthService(db, store), new StartupConsistencyChecker(db, store, {
        ...
        }), new RuntimeRecoveryService(store), new StalledExecutionDetector(store), new SqliteReliabilityService(db), `${dbPath}.backup`, null, null, workers);
        const report = doctor.run();
        assert.equal(report.status, "fail_closed");
...
test("doctor service degrades when protected governance drift is detected", async () => {
    ...
    try {
        ...
        const store = new AuthoritativeTaskStore(db);
        const workers = new WorkerRegistryService(store);
        const doctor = new DoctorService(new HealthService(db, store), new StartupConsistencyChecker(db, store), new RuntimeRecoveryService(store), new StalledExecutionDetector(store), new SqliteReliabilityService(db), `${dbPath}.backup`, integrityService, null, workers);
        const report = doctor.run();
        assert.equal(report.status, "degraded");
...
test("doctor service degrades when managed storage remains over quota", async () => {
    ...
    try {
        ...
        const doctor = new DoctorService(new HealthService(db, store), new StartupConsistencyChecker(db, store), new RuntimeRecoveryService(store), new StalledExecutionDetector(store), new SqliteReliabilityService(db), `${dbPath}.backup`, null, storageQuota, workers);
        const report = doctor.run();
        assert.equal(report.status, "degraded");
...
test("doctor service reports observability retention backlog without deleting protected summaries", () => {
    ...
    try {
        ...
        db.connection.prepare(`UPDATE tasks SET status = ?, completed_at = ? WHERE id = ?`).run("done", "2026-01-01T00:00:00.000Z", "task-doctor-retention");
        store.insertSession({
        ...
        const doctor = new DoctorService(new HealthService(db, store), new StartupConsistencyChecker(db, store), new RuntimeRecoveryService(store), new StalledExecutionDetector(store), new SqliteReliabilityService(db), `${dbPath}.backup`, null, null, workers, new ObservabilityRetentionService(db, {
