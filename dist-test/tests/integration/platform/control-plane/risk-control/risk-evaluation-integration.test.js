...
test("risk-engine: break-glass approval for critical", () => {
    ...
    assert.strictEqual(result.approvalType, "break_glass");
});
test("risk-engine: factor breakdown is correct", () => {
    ...
    for (const factor of result.factorBreakdown) {
        assert.strictEqual(factor.weightedValue, factor.value * factor.weight, `Weighted value for ${factor.factor} should be value * weight`);
    }
});
...
test("risk-engine: production targeting increases risk", () => {
    ...
    assert.ok(productionResult.riskScore > internalResult.riskScore, "Production targeting should have higher risk than internal");
});
test("risk-engine: platform blast radius increases risk", () => {
    ...
    assert.ok(platformResult.riskScore > singleTaskResult.riskScore, "Platform blast radius should have higher risk than single task");
});
//# sourceMappingURL=risk-evaluation-integration.test.js.map