...
test("contract: KnowledgeBoundaryService logs and redacts denied access", () => {
    ...
    const result = service.evaluateAccess({
        ...
        allowedOrgNodeIds: [],
    }, "user_1", "dept_hr", "investigate", [], undefined, "2026-04-20T00:00:00.000Z");
    assert.equal(result.allowed, false);
...
test("contract: EdgeRuntimeSyncService blocks restricted uploads when sync policy forbids them", () => {
    ...
    const execution = service.executeOffline(profile, [{ modelId: "local-text", modalities: ["text"] }], {
        edgeNodeId: "edge_store_1",
        taskId: "task_inventory_1",
        modality: "text",
        createdAt: "2026-04-20T00:00:00.000Z",
    });
    const restrictedEnvelope = service.buildSyncEnvelope(profile, execution.record, "digest:restricted", 3, "restricted", "2026-04-20T00:06:00.000Z");
...
test("contract: residency, quota, and SLA violations must surface as explicit structured decisions", () => {
    ...
    assert.deepEqual(slaDecision.breachRecords[0]?.breachCodes, ["sla.latency_breach", "sla.success_rate_breach", "sla.queue_wait_breach"]);
});
//# sourceMappingURL=extension-contracts.test.js.map