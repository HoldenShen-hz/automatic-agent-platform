...
test("incident timeline export stays within the configured artifact root and records both artifacts", async () => {
    ...
    try {
        ...
        const diagnostics = new DiagnosticsService(new InspectService(store), new HealthService(db, store), new StructuredLogger());
        ...
        assert.deepEqual(store.listArtifactsByTask(snapshot.task.id).slice(-2).map((artifact) => artifact.kind), ["incident_timeline_report", "incident_timeline_markdown"]);
        db.close();
    }
    finally {
...
test("stalled escalation export stays within the configured artifact root and records escalation artifacts", () => {
    ...
    try {
        ...
        db.connection.prepare(`UPDATE workflow_state SET status = ?, current_step_index = ?, updated_at = ? WHERE task_id = ?`).run("running", 0, "2000-01-01T00:00:00.000Z", "task-stalled-export-security");
        db.connection.prepare(`UPDATE tasks SET status = ?, completed_at = ?, updated_at = ? WHERE id = ?`).run("in_progress", null, "2000-01-01T00:00:00.000Z", "task-stalled-export-security");
        db.connection.prepare(`UPDATE executions SET status = ?, finished_at = ?, updated_at = ? WHERE id = ?`).run("executing", null, "2000-01-01T00:00:00.000Z", "exec-stalled-export-security");
        ...
        const diagnostics = new DiagnosticsService(new InspectService(store), new HealthService(db, store), new StructuredLogger());
        const escalationService = new StalledExecutionEscalationService(new StalledExecutionDetector(store), diagnostics);
        const exportService = new DiagnosticsExportService(diagnostics, store, {
            rootDir: artifactRoot,
        });
        ...
        assert.deepEqual(store.listArtifactsByTask("task-stalled-export-security").slice(-1).map((artifact) => artifact.kind), ["stalled_execution_escalation"]);
        db.close();
    }
    finally {
        cleanupPath(workspace);
...
//# sourceMappingURL=incident-timeline-export.test.js.map