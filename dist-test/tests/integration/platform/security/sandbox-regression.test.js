...
test("sandbox rejects symlink traversal via relative path", async () => {
    ...
    try {
        ...
        assert.equal(result.error?.code, "sandbox.command_arg_path_denied", `Expected sandbox.command_arg_path_denied but got ${result.error?.code}`);
    }
    finally {
...
test("sandbox rejects double-encoded path traversal (%2e%2e%2f)", async () => {
    ...
    try {
        ...
        assert.ok(result.error?.code === "sandbox.command_arg_path_denied" ||
            result.error?.code === "tool.path_scope_command_arg_denied", `Expected sandbox.command_arg_path_denied or tool.path_scope_command_arg_denied but got ${result.error?.code}`);
    }
    finally {
        cleanupPath(workspace);
        cleanupPath(outside);
...
//# sourceMappingURL=sandbox-regression.test.js.map