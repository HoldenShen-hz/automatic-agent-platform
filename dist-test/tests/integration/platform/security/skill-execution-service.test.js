...
test("skill execution service does not allow model overrides to bypass execution-level tool permissions", async () => {
    ...
    db.connection.prepare(`UPDATE executions SET allowed_tools_json = ? WHERE id = ?`).run(JSON.stringify(["edit_batch"]), "exec-skill-model-perm-security");
    const skill = {
...
test("skill execution service fail-closes malformed execution allowlists before any tool runner call", async () => {
    ...
    db.connection.prepare(`UPDATE executions SET allowed_tools_json = ? WHERE id = ?`).run(JSON.stringify(["read", 1]), "exec-skill-allowlist-security");
    const skill = {
