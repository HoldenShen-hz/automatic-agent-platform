...
test("stable maintenance rehearsal fails closed on new dispatch and stale writes during worker drain", async () => {
    ...
    try {
        ...
        const handoverScenario = report.scenarios.find((scenario) => scenario.scenarioId === "step_boundary_handover_preserves_execution_lineage");
        const drainingEvaluation = drainScenario?.details.drainingEvaluation;
        const staleWrite = handoverScenario?.details.staleWrite;
