...
test("console html escapes task titles and inspect endpoints require actor headers", async () => {
    ...
    const server = new HttpApiServer({
        ...
        missionControlService: new MissionControlService(store, new HealthService(db, store), new MetricsService(db, new HealthService(db, store)), inspect, {
    ...
    try {
        ...
        const inspectPayload = inspectResponse.json();
        ...
        const targetsPayload = targetsResponse.json();
        ...
        const ambiguousPayload = ambiguousResolve.json();
