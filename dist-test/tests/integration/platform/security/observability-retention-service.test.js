...
test("observability retention enforcement preserves tier1 audit events and active-session messages", () => {
    ...
    try {
        ...
        db.connection.prepare(`UPDATE tasks SET status = ?, completed_at = ? WHERE id = ?`).run("done", "2026-01-01T00:00:00.000Z", "task-retention-sec-terminal");
        db.connection.prepare(`UPDATE executions SET status = ?, finished_at = ? WHERE id = ?`).run("succeeded", "2026-01-01T00:00:00.000Z", "exec-retention-sec-terminal");
        store.insertSession({
        ...
        assert.deepEqual(store.listMessagesBySession("sess-retention-sec-terminal").map((message) => message.id), ["msg-retention-sec-terminal-summary"]);
        assert.deepEqual(store.listMessagesBySession("sess-retention-sec-active").map((message) => message.id), ["msg-retention-sec-active-tool"]);
        db.close();
    }
    finally {
...
//# sourceMappingURL=observability-retention-service.test.js.map