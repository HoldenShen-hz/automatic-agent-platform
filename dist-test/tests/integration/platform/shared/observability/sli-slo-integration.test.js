...
import { SliCollectionService, } from "../../../../../src/platform/shared/observability/sli-collection-service.js";
...
test("SliCollectionService initializes default SLO definitions", () => {
    const { workspace, dbPath, db, store, healthService, metricsService, sloService, collectionService } = buildTestSystem();
    try {
...
test("SliCollectionService.collectAllSlis returns valid summary", () => {
    ...
    try {
        ...
        const summary = collectionService.collectAllSlis();
...
test("SloAlertingService.defineSlo creates a persisted SLO and getSlo retrieves it", () => {
    ...
    try {
        const slo = sloService.defineSlo({
...
test("SloAlertingService.collectSli stores a sample and listSliSamples retrieves it", () => {
    ...
    try {
        ...
        const sli = sloService.collectSli(slo.id, 342.5, "ms", { source: "test" });
        ...
        const samples = sloService.listSliSamples(slo.id, 10);
...
test("SloAlertingService.evaluateSlo returns met when target is met", () => {
    ...
    try {
        ...
        sloService.collectSli(slo.id, 0.95, "ratio");
        const status = sloService.evaluateSlo(slo.id);
        assert.ok(status === "met" || status === "at_risk", `SLO with avg 0.95 vs target 0.90 should be met or at_risk, got: ${status}`);
        db.close();
    }
    finally {
        cleanupPath(workspace);
...
test("SloAlertingService.fireAlert creates an alert event delivered to log channel", () => {
    ...
    try {
        ...
        const alertEvent = sloService.fireAlert(rule.id, "SLO breached", "Task success rate dropped to 85%");
...
test("SloCollectionService stop() prevents automatic collection", async () => {
    ...
    try {
        ...
        const summary1 = collectionService.collectAllSlis();
...
test("SloCollectionService collectAllSlis produces samples queryable via evaluateSlo", () => {
    ...
    try {
        ...
        const status = sloService.evaluateSlo(taskSlo.id);
        assert.ok(status === "met" || status === "at_risk" || status === "breached" || status === "unknown", `evaluateSlo should return a valid SloStatus, got: ${status}`);
        db.close();
    }
    finally {
        cleanupPath(workspace);
...
//# sourceMappingURL=sli-slo-integration.test.js.map