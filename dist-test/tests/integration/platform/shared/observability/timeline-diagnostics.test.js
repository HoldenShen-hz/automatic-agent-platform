...
test("timeline and diagnostics expose approvals, events, logs, and health in one view", async () => {
    ...
    try {
        ...
        const diagnostics = new DiagnosticsService(inspectService, healthService, logger, new ObservabilityRetentionService(db));
        ...
        assert.equal(debugDump.warningSummary.entries.some((entry) => entry.code === "approval_pending" && entry.category === "approval" && entry.count === 1), true);
        assert.equal(debugDump.warningSummary.entries.some((entry) => entry.code === "provider:failed" && entry.category === "provider" && entry.severity === "critical"), true);
...
test("timeline and diagnostics aggregate persisted remote worker logs into remote timeline views", () => {
    ...
    try {
        ...
        assert.equal(remoteTimeline.entries.some((entry) => entry.source === "remote_log" && entry.summary.toLowerCase().includes("checkout drift detected")), true);
        db.close();
    }
    finally {
        cleanupPath(workspace);
...
test("timeline and diagnostics surface remote fallback dispatches as first-class routing events", () => {
    ...
    try {
        ...
        assert.ok(timeline.entries.some((entry) => entry.kind === "event" && entry.traceId === "trace-timeline-remote-fallback" && entry.correlationId != null));
...
test("timeline and diagnostics surface partial_available remote routing as an explicit blocked state", () => {
    ...
    try {
        ...
        assert.equal(debugDump.warningSummary.entries.some((entry) => entry.code === "dispatch:blocked" && entry.category === "dispatch" && entry.severity === "warning"), true);
        assert.ok(timeline.entries.some((entry) => entry.kind === "dispatch"
...
test("timeline and diagnostics surface remote authority rejection reasons from worker runtime events", () => {
    ...
    try {
        ...
        const rejectionEntry = timeline.entries.find((entry) => entry.kind === "event" && entry.title === "worker:heartbeat_rejected");
        ...
        assert.equal(debugDump.warnings.filter((warning) => warning === "remote_authority:remote_session_resume_offset_missing").length, 1);
        assert.equal(debugDump.warningSummary.suppressedDuplicateCount, 1);
        assert.equal(debugDump.warningSummary.entries.some((entry) => entry.code === "remote_authority:remote_session_resume_offset_missing"
            && entry.category === "remote_authority"
            && entry.count === 2
            && entry.suppressedCount === 1
            && entry.escalation === "operator"), true);
        db.close();
    }
    finally {
        cleanupPath(workspace);
...
//# sourceMappingURL=timeline-diagnostics.test.js.map