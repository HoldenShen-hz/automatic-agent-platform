...
test("inspect service exposes cascade-denied sibling approvals after a session rejection", () => {
    ...
    try {
        ...
        const cascadeEvent = approvalInspect.recentEvents.find((event) => event.eventType === "decision:responded" && event.payloadJson.includes(`"approvalId":"${sibling.approvalId}"`));
        ...
        assert.equal(approvalInspect.approvals.filter((item) => item.status === "requested").length, 1);
        db.close();
    }
    finally {
...
//# sourceMappingURL=approval-cascade.test.js.map