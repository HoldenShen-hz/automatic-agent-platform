...
test.describe("Outbox transition integration tests", () => {
    ...
    test("task state transition writes outbox entry in same transaction", () => {
        ...
        const statusChangeEntry = pendingAfter.find((entry) => entry.eventType === "task:status_changed" && entry.aggregateId === taskId);
    ...
    test("task state transition does not write duplicate outbox entries on same transition", () => {
        ...
        const pendingAfter = outboxService.getPendingEntries();
        const statusChangeEntries = pendingAfter.filter((entry) => entry.eventType === "task:status_changed" && entry.aggregateId === taskId);
    ...
    test("outbox entry is atomic with task status change", () => {
        ...
        const taskStatusChangeOutboxEntries = pendingAfter.filter((entry) => entry.eventType === "task:status_changed" && entry.aggregateId === taskId);
        assert.ok(taskStatusChangeOutboxEntries.length > 0, "Should have outbox entries for task:status_changed events");
    });
});
//# sourceMappingURL=outbox-transition.test.js.map