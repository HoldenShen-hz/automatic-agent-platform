...
test("SSO-SCIM SAML: SamlService rejects assertion with wrong issuer", () => {
    ...
    try {
        ...
        assert.throws(() => samlService.consumeAssertion("saml-bad-issuer", assertion, new Date()), /saml.invalid_issuer/);
        db.close();
    }
    finally {
        cleanupPath(workspace);
...
test("SSO-SCIM SAML: SamlService rejects assertion with wrong fingerprint", () => {
    ...
    try {
        ...
        assert.throws(() => samlService.consumeAssertion("saml-bad-fp", assertion, new Date()), /saml.invalid_fingerprint/);
        db.close();
    }
    finally {
        cleanupPath(workspace);
...
test("SSO-SCIM SAML: SamlService rejects expired assertion", () => {
    ...
    try {
        ...
        assert.throws(() => samlService.consumeAssertion("saml-expired", assertion, new Date("2026-04-20T12:00:00.000Z")), /saml.assertion_expired/);
        db.close();
    }
    finally {
        cleanupPath(workspace);
...
test("SSO-SCIM SAML: SamlService rejects assertion not yet valid (clock skew)", () => {
    ...
    try {
        ...
        assert.throws(() => samlService.consumeAssertion("saml-clock-skew", assertion, new Date("2026-04-20T12:00:00.000Z")), /saml.assertion_expired/);
        db.close();
    }
    finally {
        cleanupPath(workspace);
...
test("SSO-SCIM SAML: SamlService rejects unknown provider", () => {
    ...
    try {
        ...
        assert.throws(() => samlService.buildLoginRequest("unknown-provider"), /saml.provider_not_found/);
        db.close();
    }
    finally {
        cleanupPath(workspace);
...
test("SSO-SCIM SAML: SamlService rejects empty nameId", () => {
    ...
    try {
        ...
        assert.throws(() => samlService.consumeAssertion("saml-empty-nameid", assertion, new Date()), /saml.invalid_subject/);
        db.close();
    }
    finally {
        cleanupPath(workspace);
...
//# sourceMappingURL=saml-service-integration.test.js.map