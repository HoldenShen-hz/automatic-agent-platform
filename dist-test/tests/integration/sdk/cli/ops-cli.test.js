...
function runCli(scriptName, env) {
...
function seedCliDispatchPreemptionScenario(db, store, input) {
...
function seedCliConfigTree(root) {
...
serialTest("inspect CLI returns task, execution, and approval views", async () => {
    ...
    try {
        ...
        db.close();
        const taskInspect = runCli("inspect.js", {
            AA_DB_PATH: dbPath,
            AA_INSPECT_KIND: "task",
            AA_TASK_ID: snapshot.task.id,
        });
        const executionInspect = runCli("inspect.js", {
            AA_DB_PATH: dbPath,
            AA_INSPECT_KIND: "execution",
            AA_EXECUTION_ID: snapshot.execution?.id ?? "",
        });
        const approvalInspect = runCli("inspect.js", {
            AA_DB_PATH: dbPath,
...
serialTest("inspect CLI exposes structured dispatch decisions for active execution routing", () => {
    ...
    try {
        ...
        assert.ok(taskInspect.dispatchDecisions[0]?.evaluations.some((evaluation) => evaluation.workerId === "worker-cli-inspect-basic" && evaluation.rejectionReason === "missing_capabilities"));
...
serialTest("inspect CLI summarizes partial and degraded remote routing outcomes", () => {
    ...
    try {
        ...
        db.close();
        const partialTaskInspect = runCli("inspect.js", {
            AA_DB_PATH: dbPath,
            AA_INSPECT_KIND: "task",
            AA_TASK_ID: "task-cli-inspect-remote-partial",
        });
        const degradedTaskInspect = runCli("inspect.js", {
            AA_DB_PATH: dbPath,
            AA_INSPECT_KIND: "task",
...
serialTest("inspect CLI query mode lists task workflow and decision summaries", () => {
    ...
    try {
        ...
        db.connection.prepare(`UPDATE tasks SET status = ?, updated_at = ? WHERE id = ?`).run("awaiting_decision", "2026-04-05T13:00:00.000Z", "task-cli-query");
        db.connection.prepare(`UPDATE executions SET status = ?, updated_at = ? WHERE id = ?`).run("created", "2026-04-05T13:00:00.000Z", "exec-cli-query");
        ...
        assert.ok(decisionSummaries.some((summary) => summary.decisionType === "approval" && summary.decisionId === approval.approvalId));
        assert.ok(decisionSummaries.some((summary) => summary.decisionType === "dispatch" && summary.selectedWorkerId === "worker-cli-query"));
    }
    finally {
        cleanupPath(workspace);
    }
...
serialTest("inspect CLI query mode lists remote worker session telemetry summaries", async () => {
    ...
    try {
        ...
        db.close();
        const workerSummaries = runCli("inspect.js", {
            AA_DB_PATH: dbPath,
...
serialTest("diagnostics CLI returns snapshot debug repro and export outputs", async () => {
    ...
    try {
        ...
        const taskSnapshot = runCli("diagnostics.js", {
            AA_DB_PATH: dbPath,
            AA_TASK_ID: snapshot.task.id,
            AA_DIAGNOSTICS_KIND: "snapshot",
        });
        const debugDump = runCli("diagnostics.js", {
            AA_DB_PATH: dbPath,
            AA_TASK_ID: snapshot.task.id,
            AA_DIAGNOSTICS_KIND: "debug",
        });
        const reproBundle = runCli("diagnostics.js", {
            AA_DB_PATH: dbPath,
            AA_TASK_ID: snapshot.task.id,
            AA_DIAGNOSTICS_KIND: "repro",
        });
        const exported = runCli("diagnostics.js", {
            AA_DB_PATH: dbPath,
            AA_TASK_ID: snapshot.task.id,
...
serialTest("diagnostics CLI returns runtime metrics summaries", async () => {
    ...
    try {
        ...
        db.connection
         ...
         WHERE id = ?`)
            .run("failed", 0.75, "2026-04-06T12:00:00.000Z", "2026-04-06T12:00:00.000Z", "task.failed", "task-cli-diagnostics-metrics-failed");
        db.connection
            .prepare(`UPDATE executions SET status = ?, attempt = ?, updated_at = ?, finished_at = ? WHERE id = ?`)
            .run("failed", 2, "2026-04-06T12:00:00.000Z", "2026-04-06T12:00:00.000Z", "exec-cli-diagnostics-metrics-failed");
        store.insertEvent({
        ...
        db.close();
        const metrics = runCli("diagnostics.js", {
            AA_DB_PATH: dbPath,
...
serialTest("diagnostics CLI returns incident timeline reports and exports markdown/json artifacts", async () => {
    ...
    try {
        ...
        db.close();
        const incident = runCli("diagnostics.js", {
            AA_DB_PATH: dbPath,
            AA_TASK_ID: snapshot.task.id,
        ...
        assert.ok(incident.candidateRootCauses.includes("Execution is waiting on an operator approval before it can continue."));
...
serialTest("diagnostics CLI returns stalled escalation packages and exports escalation artifacts", () => {
    ...
    try {
        ...
        db.connection.prepare(`UPDATE workflow_state SET status = ?, current_step_index = ?, updated_at = ? WHERE task_id = ?`).run("running", 0, "2000-01-01T00:00:00.000Z", "task-cli-stalled-escalation");
        db.connection.prepare(`UPDATE tasks SET status = ?, completed_at = ?, updated_at = ? WHERE id = ?`).run("in_progress", null, "2000-01-01T00:00:00.000Z", "task-cli-stalled-escalation");
        db.connection.prepare(`UPDATE executions SET status = ?, finished_at = ?, updated_at = ? WHERE id = ?`).run("executing", null, "2000-01-01T00:00:00.000Z", "exec-cli-stalled-escalation");
        store.upsertAgentExecutionRecord({
            executionId: "exec-cli-stalled-escalation",
            taskId: "task-cli-stalled-escalation",
            agentId: "agent-1",
        ...
        db.close();
        const escalations = runCli("diagnostics.js", {
            AA_DB_PATH: dbPath,
...
serialTest("diagnostics CLI returns remote timeline views for persisted remote worker logs", () => {
    ...
    try {
        ...
        db.close();
        const remoteTimeline = runCli("diagnostics.js", {
            AA_DB_PATH: dbPath,
        ...
        assert.equal(remoteTimeline.entries.some((entry) => entry.source === "remote_log" && entry.title === "remote_log:info"), true);
        assert.equal(remoteTimeline.entries.some((entry) => entry.source === "dispatch" && entry.summary.includes("Dispatch routed")), true);
    }
    finally {
        cleanupPath(workspace);
    }
...
serialTest("drain-events CLI drains default consumers and clears pending tier1 acks", () => {
    ...
    try {
        ...
        db.close();
        const drained = runCli("drain-events.js", {
            AA_DB_PATH: dbPath,
        });
        ...
        assert.ok(drained.some((result) => result.consumerId === "task_projection"
            && result.pendingBefore >= 1
            && result.pendingAfter === 0
            && result.outcome === "delivered"));
        assert.ok(drained.some((result) => result.consumerId === "inspect_projection"
            && result.pendingBefore >= 1
            && result.pendingAfter === 0
            && result.outcome === "delivered"));
        reloaded.close();
...
serialTest("replay-events CLI replays failed consumer acknowledgements", () => {
    ...
    try {
        ...
        db.close();
        const replayed = runCli("replay-events.js", {
            AA_DB_PATH: dbPath,
...
serialTest("replay-recovery CLI returns deterministic dead-letter recovery history", () => {
    ...
    try {
        ...
        db.transaction(() => {
           ...
           WHERE id = ?`)
                .run(2, "unexpected_runtime_error", "tool crashed twice", "2026-04-04T10:25:00.000Z", "exec-cli-replay-recovery");
        ...
        db.close();
        const replayed = runCli("replay-recovery.js", {
            AA_DB_PATH: dbPath,
        ...
        assert.deepEqual(replayed.executions[0]?.timeline.map((event) => event.eventType), ["recovery:decision_recorded", "recovery:dead_lettered"]);
    }
    finally {
        cleanupPath(workspace);
    }
...
serialTest("dispatch-execution CLI creates and dispatches a ticket to an eligible worker", () => {
    ...
    try {
        ...
        db.close();
        const dispatched = runCli("dispatch-execution.js", {
            AA_DB_PATH: dbPath,
        ...
        assert.ok(dispatched.dispatched?.trace?.evaluations.some((item) => item.workerId === "worker-cli-dispatch" && item.rejectionReason === null));
...
serialTest("dispatch-execution CLI forwards required isolation level and rejects weaker workers", () => {
    ...
    try {
        ...
        assert.ok(dispatched.dispatched?.trace?.evaluations.some((item) => item.workerId === "worker-cli-standard" && item.rejectionReason === "worker_isolation_mismatch"));
    }
    finally {
        cleanupPath(workspace);
    }
...
serialTest("dispatch-execution CLI forwards required repo version and fail-closes repo-mismatched remote workers", () => {
    ...
    try {
        ...
        assert.ok(dispatched.dispatched?.trace?.evaluations.some((item) => item.workerId === "worker-cli-remote-repo-old" && item.rejectionReason === "worker_repo_version_mismatch"));
    }
    finally {
        cleanupPath(workspace);
    }
...
serialTest("dispatch-reconcile CLI requeues orphan claimed tickets back into a pending ticket", () => {
    ...
    try {
        ...
        db.close();
        const reconciled = runCli("dispatch-reconcile.js", {
            AA_DB_PATH: dbPath,
...
serialTest("orphan-cleanup CLI repairs orphan sessions, orphan claimed tickets, and worker execution refs", () => {
    ...
    try {
        ...
        db.connection.prepare(`UPDATE tasks SET status = ?, completed_at = ? WHERE id = ?`).run("done", "2026-04-07T14:00:01.000Z", "task-cli-orphan");
        store.insertSession({
            id: "sess-cli-orphan",
            taskId: "task-cli-orphan",
            channel: "cli",
        ...
        db.close();
        const repaired = runCli("orphan-cleanup.js", {
            AA_DB_PATH: dbPath,
...
serialTest("worker-register CLI issues and completes a trusted remote worker registration", () => {
    ...
    try {
        ...
        const issued = runCli("worker-register.js", {
            AA_DB_PATH: dbPath,
        ...
        const completed = runCli("worker-register.js", {
            AA_DB_PATH: dbPath,
            AA_CONFIG_ROOT: configRoot,
...
serialTest("doctor CLI returns grouped self-check sections for operator review", () => {
    ...
    try {
        ...
        assert.deepEqual(report.checks.map((check) => check.checkId), ["db", "config", "backup", "locks", "workers", "event_backlog", "audit_integrity", "provider_health"]);
...
serialTest("worker-handshake CLI claims a dispatched ticket and renews worker heartbeat", () => {
    ...
    try {
        ...
        db.close();
        const claimed = runCli("worker-handshake.js", {
            AA_DB_PATH: dbPath,
        ...
        const heartbeat = runCli("worker-handshake.js", {
            AA_DB_PATH: dbPath,
...
serialTest("worker-writeback CLI records a terminal worker completion and releases runtime ownership", () => {
    ...
    try {
        ...
        db.close();
        const writeback = runCli("worker-writeback.js", {
            AA_DB_PATH: dbPath,
...
serialTest("lease-handover CLI transfers an active lease to a replacement worker with lineage", () => {
    ...
    try {
        ...
        db.close();
        const handover = runCli("lease-handover.js", {
            AA_DB_PATH: dbPath,
...
serialTest("takeover CLI opens a session and applies operator overrides", () => {
    ...
    try {
        ...
        db.close();
        const opened = runCli("takeover.js", {
            AA_DB_PATH: dbPath,
...
serialTest("takeover CLI supports workflow repair actions for manual output injection and step reset", () => {
    ...
    try {
        ...
        assert.deepEqual(operatorActions.map((action) => action.actionType), ["take_over_task", "write_step_output", "set_current_step"]);
        reloaded.close();
    }
    finally {
        cleanupPath(workspace);
...
serialTest("stable evidence CLI writes a short local evidence bundle", () => {
    ...
    try {
        const report = runCli("stable-evidence.js", {
...
serialTest("stable chaos CLI writes a local chaos smoke report", () => {
    ...
    try {
        const report = runCli("stable-chaos.js", {
...
serialTest("stable prompt injection CLI writes a local red-team report", () => {
    ...
    try {
        const report = runCli("stable-prompt-injection.js", {
...
serialTest("stable concurrency CLI writes a local concurrency rehearsal report", () => {
    ...
    try {
        const report = runCli("stable-concurrency.js", {
...
serialTest("stable rollback CLI writes a local rollback rehearsal report", () => {
    ...
    try {
        const report = runCli("stable-rollback.js", {
...
serialTest("stable recovery drill CLI writes a local cross-division recovery drill report", () => {
    ...
    try {
        const report = runCli("stable-recovery-drill.js", {
...
serialTest("stable dispatch CLI writes a local dispatch rehearsal report", () => {
    ...
    try {
        const report = runCli("stable-dispatch.js", {
...
serialTest("stable dispatch reconcile CLI writes a local dispatch reconciliation rehearsal report", () => {
    ...
    try {
        const report = runCli("stable-dispatch-reconcile.js", {
...
serialTest("stable worker handshake CLI writes a local worker handshake rehearsal report", () => {
    ...
    try {
        const report = runCli("stable-worker-handshake.js", {
...
serialTest("stable worker writeback CLI writes a local worker writeback rehearsal report", () => {
    ...
    try {
        const report = runCli("stable-worker-writeback.js", {
...
serialTest("stable lease CLI writes a local lease rehearsal report", () => {
    ...
    try {
        const report = runCli("stable-lease.js", {
...
serialTest("stable restore CLI writes a local backup restore rehearsal report", () => {
    ...
    try {
        const report = runCli("stable-restore.js", {
...
serialTest("stable upgrade CLI writes a local rolling upgrade rehearsal report", () => {
    ...
    try {
        const report = runCli("stable-upgrade.js", {
...
serialTest("stable maintenance CLI writes a local maintenance rehearsal report", () => {
    ...
    try {
        const report = runCli("stable-maintenance.js", {
...
serialTest("stable DB queue disconnect CLI writes a local disconnect drill report", () => {
    ...
    try {
        const report = runCli("stable-db-queue-disconnect.js", {
        ...
        assert.ok(report.scenarios.some((scenario) => scenario.scenarioId === "authoritative_writeback_failure_fails_closed_until_store_recovers"));
...
serialTest("stable DB writability CLI writes a local read-only fail-close drill report", () => {
    ...
    try {
        const report = runCli("stable-db-writability.js", {
...
serialTest("stable queue delivery CLI writes a local queue replay / duplicate delivery rehearsal report", () => {
    ...
    try {
        const report = runCli("stable-queue-delivery.js", {
...
serialTest("stable migration compatibility CLI writes a local PG portability rehearsal report", () => {
    ...
    try {
        const report = runCli("stable-migration-compatibility.js", {
...
serialTest("stable replay CLI writes a local event replay rehearsal report", () => {
    ...
    try {
        const report = runCli("stable-replay.js", {
...
serialTest("stable gray CLI writes a local tenant-gray rehearsal report", () => {
    ...
    try {
        const report = runCli("stable-gray.js", {
...
serialTest("stable package CLI writes a local release package with gate and summary artifacts", () => {
    ...
    try {
        ...
        const report = runCli("stable-package.js", {
...
serialTest("stable validate CLI writes reusable baseline and report artifacts", () => {
    ...
    try {
        const firstReport = runCli("stable-validate.js", {
            AA_VALIDATION_OUTPUT_DIR: outputDir,
            AA_VALIDATION_ITERATIONS: "1",
        });
        const secondReport = runCli("stable-validate.js", {
            AA_VALIDATION_OUTPUT_DIR: outputDir,
            AA_VALIDATION_ITERATIONS: "1",
        });
...
serialTest("stable campaign CLI accumulates local segments and finalizes an evidence bundle", () => {
    ...
    try {
        const first = runCli("stable-campaign.js", {
        ...
        const second = runCli("stable-campaign.js", {
            AA_STABLE_CAMPAIGN_OUTPUT_DIR: outputDir,
            AA_STABLE_CAMPAIGN_PROFILE: "24h",
            AA_STABLE_CAMPAIGN_TARGET_DURATION_MS: "40",
            AA_STABLE_CAMPAIGN_SEGMENT_DURATION_MS: "25",
...
//# sourceMappingURL=ops-cli.test.js.map