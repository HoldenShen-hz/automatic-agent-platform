...
function runBuiltCli(scriptName, options = {}) {
    return execFileSync(process.execPath, [join(process.cwd(), "dist", "src", "sdk", "cli", scriptName), ...(options.args ?? [])], {
...
function prepareDlqDatabase(dbPath) {
...
test("stable-sequence CLI can run-until-complete with a smoke profile and persist artifacts", () => {
    ...
    try {
        ...
        assert.deepEqual(parsed.state.profiles.map((profile) => ({
...
test("stable-soak CLI persists the same JSON payload that it prints", () => {
    ...
    try {
        ...
        const persisted = JSON.parse(readFileSync(join(outputDir, "stable-soak-report.json"), "utf8"));
...
//# sourceMappingURL=additional-cli-entrypoints.test.js.map