...
// Test Fixtures
// ─────────────────────────────────────────────────────────────────────────────
function createMockTaskSource(tasks) {
    return { list: () => tasks };
}
function createMockSystemSource(status = "ok", queueSize = 0) {
    return {
        build: () => ({
            healthStatus: status,
...
test("integration: DashboardAggregationService builds operator dashboard snapshot", () => {
    const tasks = [
        { taskId: "task-1", title: "Report generation", taskStatus: "done", priority: "normal", divisionId: "marketing", workflowStatus: null, currentStepIndex: null, sessionStatus: null, latestEventAt: null, updatedAt: nowIso() },
        { taskId: "task-2", title: "Deploy to staging", taskStatus: "in_progress", priority: "normal", divisionId: "engineering", workflowStatus: null, currentStepIndex: null, sessionStatus: null, latestEventAt: null, updatedAt: nowIso() },
        { taskId: "task-3", title: "Fix critical bug", taskStatus: "failed", priority: "normal", divisionId: "engineering", workflowStatus: null, currentStepIndex: null, sessionStatus: null, latestEventAt: null, updatedAt: nowIso() },
    ];
...
test("integration: DashboardAggregationService builds domain admin dashboard", () => {
    const tasks = [
        { taskId: "task-1", title: "Marketing campaign", taskStatus: "in_progress", priority: "normal", divisionId: "marketing", workflowStatus: null, currentStepIndex: null, sessionStatus: null, latestEventAt: null, updatedAt: nowIso() },
        { taskId: "task-2", title: "Another marketing task", taskStatus: "pending", priority: "normal", divisionId: "marketing", workflowStatus: null, currentStepIndex: null, sessionStatus: null, latestEventAt: null, updatedAt: nowIso() },
    ];
    const service = new DashboardAggregationService({
        taskSource: createMockTaskSource(tasks),
...
test("integration: DashboardAggregationService builds platform ops dashboard", () => {
    const tasks = [
        { taskId: "task-1", title: "System task", taskStatus: "in_progress", priority: "normal", divisionId: "platform", workflowStatus: null, currentStepIndex: null, sessionStatus: null, latestEventAt: null, updatedAt: nowIso() },
    ];
    const service = new DashboardAggregationService({
        taskSource: createMockTaskSource(tasks),
...
test("integration: DashboardAggregationService builds fleet dashboard", () => {
    const tasks = [
        { taskId: "task-1", title: "Marketing task", taskStatus: "done", priority: "normal", divisionId: "marketing", workflowStatus: null, currentStepIndex: null, sessionStatus: null, latestEventAt: null, updatedAt: nowIso() },
        { taskId: "task-2", title: "Engineering task", taskStatus: "failed", priority: "normal", divisionId: "engineering", workflowStatus: null, currentStepIndex: null, sessionStatus: null, latestEventAt: null, updatedAt: nowIso() },
        { taskId: "task-3", title: "HR task", taskStatus: "pending", priority: "normal", divisionId: "hr", workflowStatus: null, currentStepIndex: null, sessionStatus: null, latestEventAt: null, updatedAt: nowIso() },
    ];
    const service = new DashboardAggregationService({
        taskSource: createMockTaskSource(tasks),
...
test("integration: DashboardAggregationService getSnapshot returns correct metrics", async () => {
    const tasks = [
        { taskId: "task-1", title: "Task 1", taskStatus: "done", priority: "normal", divisionId: "ops", workflowStatus: null, currentStepIndex: null, sessionStatus: null, latestEventAt: null, updatedAt: nowIso() },
        { taskId: "task-2", title: "Task 2", taskStatus: "failed", priority: "normal", divisionId: "ops", workflowStatus: null, currentStepIndex: null, sessionStatus: null, latestEventAt: null, updatedAt: nowIso() },
    ];
    const service = new DashboardAggregationService({
        taskSource: createMockTaskSource(tasks),
...
test("integration: DashboardProjectionService processes events and generates deltas", () => {
    ...
    const delta = service.processEvent("task:status_changed", { taskId: "task-456", previousStatus: "pending", newStatus: "completed", changedAt: nowIso() });
    assert.ok(delta);
    assert.equal(delta.changes[0]?.changeType, "task_updated");
});
test("integration: DashboardProjectionService consumes pending deltas", () => {
    const service = new DashboardProjectionService({ emitDebounceMs: 10 });
    service.processEvent("task:status_changed", { taskId: "task-789", previousStatus: "pending", newStatus: "in_progress", changedAt: nowIso() });
...
test("integration: DashboardProjectionService flushes pending deltas immediately", () => {
    ...
    service.processEvent("task:status_changed", { taskId: "task-flush", previousStatus: "pending", newStatus: "done", changedAt: nowIso() });
    assert.ok(service.hasPendingDeltas());
...
test("integration: DashboardProjectionService builds state from projection records", () => {
    ...
    const projections = [
        { projectionId: "proj-1", sourceEventId: "evt-1", projectionName: "task_summary", entityRef: "task-1", state: { taskStatus: "done" }, updatedAt: nowIso() },
        { projectionId: "proj-2", sourceEventId: "evt-2", projectionName: "task_summary", entityRef: "task-2", state: { taskStatus: "failed" }, updatedAt: nowIso() },
        { projectionId: "proj-3", sourceEventId: "evt-3", projectionName: "task_summary", entityRef: "task-3", state: { taskStatus: "pending" }, updatedAt: nowIso() },
        { projectionId: "proj-4", sourceEventId: "evt-4", projectionName: "incident_summary", entityRef: "inc-1", state: { priority: "high", resolved: false }, updatedAt: nowIso() },
        { projectionId: "proj-5", sourceEventId: "evt-5", projectionName: "workflow_summary", entityRef: "wf-1", state: {}, updatedAt: nowIso() },
    ];
...
test("integration: DashboardProjectionService clears pending deltas", () => {
    ...
    service.processEvent("task:status_changed", { taskId: "task-clear" });
    assert.ok(service.hasPendingDeltas());
...
test("integration: Full dashboard pipeline - task events flow through projection to WebSocket", () => {
    ...
    projectionService.processEvent("task:status_changed", { taskId: "task-pipeline-1", previousStatus: "pending", newStatus: "in_progress", changedAt: nowIso() });
    projectionService.processEvent("task:status_changed", { taskId: "task-pipeline-1", previousStatus: "in_progress", newStatus: "completed", changedAt: nowIso() });
    projectionService.processEvent("task:status_changed", { taskId: "task-pipeline-2", previousStatus: "pending", newStatus: "failed", changedAt: nowIso() });
...
test("integration: DashboardAggregationService with attention items includes incidents and warnings", () => {
    const tasks = [
        { taskId: "task-1", title: "Failed deployment", taskStatus: "failed", priority: "high", divisionId: "engineering", workflowStatus: "completed", currentStepIndex: 0, sessionStatus: "failed", latestEventAt: nowIso(), updatedAt: nowIso() },
        { taskId: "task-2", title: "Pending approval", taskStatus: "pending", priority: "normal", divisionId: "ops", workflowStatus: "running", currentStepIndex: 1, sessionStatus: "open", latestEventAt: nowIso(), updatedAt: nowIso() },
    ];
    const suggestions = [
        {
            itemType: "suggestion",
...
//# sourceMappingURL=dashboard-integration.test.js.map