...
test("integration: interaction-governance mainline composes intake, UX, autonomy, approvals, identity, knowledge, and dashboard flow", async () => {
    ...
    try {
        ...
        const conversation = new ConversationContextManager().addTurn(intakeRequest.tenantId, intakeRequest.userId, intakeRequest.message, detailedIntent.detectedIntents[0]);
        ...
        assert.equal(autonomySnapshot.decisions.find((item) => item.capabilityId === "approval_review")?.recommendedLevel, "semi_auto");
        assert.equal(autonomySnapshot.decisions.find((item) => item.capabilityId === "budget_change")?.recommendedLevel, "suggestion");
        const orgNodes = [
            {
                orgNodeId: "root",
                nodeType: "company",
        ...
        const delegationResult = delegatedGovernance.resolve("manager_finance", {
            orgNodeId: "dept_finance",
            domainId: "finance",
        ...
        const approvalResult = approval.route({
            ...
            riskLevel: "high",
            amountUsd: 12000,
        }, "2026-04-22T00:00:00.000Z", "2026-04-22T01:00:00.000Z");
        ...
        const knowledgeDecision = knowledge.evaluateAccess({
            ...
            allowedOrgNodeIds: [],
        }, "manager_finance", "dept_finance", "review approval packet", [], {
...
//# sourceMappingURL=interaction-governance-mainline-integration.test.js.map