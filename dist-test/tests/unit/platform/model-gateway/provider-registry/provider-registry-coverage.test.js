...
import { parseRetryAfterMs, parseResetAt, shouldRetryWithinPool, BaseAPIError, } from "../../../../../src/platform/model-gateway/provider-registry/base-chat-provider.js";
import { ProviderCredentialPool, } from "../../../../../src/platform/model-gateway/provider-registry/provider-credential-pool.js";
import { deriveProviderApiKeyEnvName, deriveProviderApiKeysJsonEnvName, deriveProviderApiKeySecretRefEnvName, deriveProviderApiKeySecretRefsJsonEnvName, normalizeCredentialRecord, addMilliseconds, computeEffectiveStatus, isRetryableCredentialFailure, loadProviderCredentialRecordsFromEnv, } from "../../../../../src/platform/model-gateway/provider-registry/provider-credential-pool-support.js";
/**
...
test("UnifiedChatProvider handles unknown model routing", async () => {
    ...
    await assert.rejects(() => provider.createChatCompletion(request), /OpenAI provider is not configured/);
});
...
test("UnifiedChatProvider complete method behavior", async () => {
    ...
    await assert.rejects(() => provider.complete("hello"), /OpenAI provider is not configured/);
});
...
test("UnifiedChatProvider complete rejects for disposed provider", async () => {
    ...
    provider.dispose();
    await assert.rejects(() => provider.complete("hello"), (error) => typeof error === "object" &&
        error !== null &&
        "code" in error &&
...
test("UnifiedChatProvider embed rejects for disposed provider", async () => {
    ...
    provider.dispose();
    await assert.rejects(() => provider.embed("hello"), (error) => typeof error === "object" &&
        error !== null &&
        "code" in error &&
        error.code === "provider.disposed");
});
...
test("UnifiedChatProvider createChatCompletion routes to correct provider based on model", async () => {
    ...
    await assert.rejects(() => anthropicProvider.createChatCompletion({
        model: "claude-opus-4-5",
        messages: [{ role: "user", content: "hello" }],
        maxTokens: 100,
    }), /Anthropic provider is not configured/);
...
test("UnifiedChatProvider createChatCompletion routes minimax model to minimax", async () => {
    // Use a provider WITHOUT API key so it throws "not configured"
    const provider = new UnifiedChatProvider({});
    await assert.rejects(() => provider.createChatCompletion({
        model: "MiniMax-M2.7",
        messages: [{ role: "user", content: "hello" }],
        maxTokens: 100,
    }), /MiniMax provider is not configured/);
...
test("loadProviderCredentialRecordsFromEnv JSON with empty string apiKey throws", () => {
    assert.throws(() => loadProviderCredentialRecordsFromEnv("test", {
...
//# sourceMappingURL=provider-registry-coverage.test.js.map