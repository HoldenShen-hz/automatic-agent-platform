...
import { deriveProviderApiKeyEnvName, deriveProviderApiKeysJsonEnvName, deriveProviderApiKeySecretRefEnvName, deriveProviderApiKeySecretRefsJsonEnvName, normalizeCredentialRecord, addMilliseconds, computeEffectiveStatus, isRetryableCredentialFailure, throwProviderCredentialValidation, throwProviderCredentialRuntimeError, requireResolvedSecretValue, loadProviderCredentialRecordsFromEnv, } from "../../../../../src/platform/model-gateway/provider-registry/provider-credential-pool-support.js";
...
test("throwProviderCredentialValidation throws ValidationError with correct code", () => {
    assert.throws(() => throwProviderCredentialValidation("test-provider", "expected_array"), (error) => {
...
test("throwProviderCredentialRuntimeError throws ProviderError with correct code", () => {
    assert.throws(() => throwProviderCredentialRuntimeError("test-provider", "resolve_failed"), (error) => {
...
test("requireResolvedSecretValue throws when resolver is null", () => {
    assert.throws(() => requireResolvedSecretValue("test-provider", "secret_ref", null), (error) => {
...
test("requireResolvedSecretValue throws when resolved value is empty", () => {
    const resolver = () => "   ";
    assert.throws(() => requireResolvedSecretValue("test-provider", "secret_ref", resolver), (error) => {
...
test("loadProviderCredentialRecordsFromEnv JSON secret refs with secretLeaseIssuer works", () => {
    const leaseIssuer = () => ({ apiKey: "leased-key", leaseId: "lease_1", expiresAt: "2024-01-01T00:00:00.000Z", leaseSource: "provider_issued" });
    const records = loadProviderCredentialRecordsFromEnv("test", {
        TEST_API_KEY_SECRET_REFS_JSON: JSON.stringify([
            { credentialId: "cred_1", secretRef: "ref-1" },
        ]),
...
//# sourceMappingURL=provider-credential-pool-support.test.js.map