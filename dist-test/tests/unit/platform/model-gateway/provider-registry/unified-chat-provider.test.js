...
test("UnifiedChatProvider.createChatCompletion throws for unconfigured provider", async () => {
    ...
    await assert.rejects(() => provider.createChatCompletion(request), /Anthropic provider is not configured/);
});
...
test("UnifiedChatProvider.fromProfile is a factory method", () => {
    const provider = UnifiedChatProvider.fromProfile({ profile: "test" }, { anthropic: { apiKey: "key" } });
    assert.equal(provider.hasProvider("anthropic"), true);
});
...
//# sourceMappingURL=unified-chat-provider.test.js.map