...
test("[SYS-QUAL-7.3] API route handlers call schema.parse on request body", () => {
    ...
    assert.equal(violations.length, 0, `${violations.length} route files have POST/PUT/PATCH handlers without .parse() validation: ` +
        violations.map((v) => `${v.file} (${v.handler})`).join(", "));
});
//# sourceMappingURL=zod-boundary-validation.test.js.map