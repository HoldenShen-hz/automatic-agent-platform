...
test("[SYS-QUAL-7.4] no duplicate route registration in any route file", () => {
    ...
    for (const routeFile of ROUTE_FILES) {
        try {
            const content = readFileSync(routeFile, "utf8")
                .replace(/\/\*[\s\S]*?\*\//g, "")
                .replace(/\/\/.*$/gm, "");
            const routeKeys = Array.from(content.matchAll(/method:\s*["']([A-Z]+)["'],\s*\n\s*pathname:\s*["']([^"']+)["']/g), (match) => `${match[1]} ${match[2]}`);
            // Find duplicates
            const seen = new Set();
            const duplicates = [];
    ...
    assert.equal(allViolations.length, 0, `Duplicate route registrations found:\n${allViolations.map((v) => `${v.file}: duplicates [${v.duplicates.join(", ")}]`).join("\n")}`);
});
//# sourceMappingURL=no-duplicate-routes.test.js.map