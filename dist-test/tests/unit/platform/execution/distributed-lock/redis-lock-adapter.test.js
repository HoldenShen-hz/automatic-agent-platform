...
test("RedisLockAdapter release() throws sync_release_not_supported", () => {
    ...
    assert.throws(() => adapter.release("test-key", "test-owner"), (error) => error?.code === "E7lock.sync_release_not_supported"
        && error?.message.includes("releaseAsync"));
});
test("RedisLockAdapter extend() throws sync_extend_not_supported", () => {
    const adapter = new RedisLockAdapter({ host: "localhost", port: 6379 });
    assert.throws(() => adapter.extend("test-key", "test-owner", 5000), (error) => error?.code === "E7lock.sync_extend_not_supported"
        && error?.message.includes("extendAsync"));
});
test("RedisLockAdapter forceSteal() throws sync_forceSteal_not_supported", () => {
    const adapter = new RedisLockAdapter({ host: "localhost", port: 6379 });
    assert.throws(() => adapter.forceSteal("test-key", "new-owner", "test reason"), (error) => error?.code === "E7lock.sync_forceSteal_not_supported"
        && error?.message.includes("forceStealAsync"));
});
test("RedisLockAdapter inspect() throws sync_inspect_not_supported", () => {
    const adapter = new RedisLockAdapter({ host: "localhost", port: 6379 });
    assert.throws(() => adapter.inspect("test-key"), (error) => error?.code === "E7lock.sync_inspect_not_supported"
        && error?.message.includes("inspectAsync"));
});
...
test("RedisLockAdapter acquire() throws sync_acquire_deprecated", () => {
    const adapter = new RedisLockAdapter({ host: "localhost", port: 6379 });
    assert.throws(() => adapter.acquire({ lockKey: "test-key", owner: "test-owner" }), (error) => error?.code === "E7lock.sync_acquire_deprecated"
        && error?.message.includes("acquireAsync"));
});
...
test("RedisLockAdapter acquire() throws with provided TTL", () => {
    const adapter = new RedisLockAdapter({ host: "localhost", port: 6379 });
    assert.throws(() => adapter.acquire({ lockKey: "test-key", owner: "test-owner", ttlMs: 60000 }), (error) => error?.code === "E7lock.sync_acquire_deprecated");
});
// =============================================================================
...
test("RedisLockAdapter ensureConnected throws when reconnect fails after status end", async () => {
    ...
    const adapter = createAdapterWithMockRedis(mockRedis);
    await assert.rejects(adapter.ensureConnected(), (error) => error?.code === "E7lock.redis_connection_closed");
});
// =============================================================================
...
test("RedisLockAdapter acquireAsync throws LockingError when Redis connection is closed during acquireAsync", async () => {
    ...
    await assert.rejects(adapter.acquireAsync({
        lockKey: "test-lock",
        owner: "test-owner",
        ttlMs: 30_000,
    }), (err) => err?.code === "E7lock.redis_connection_closed");
    await adapter.close();
});
test("RedisLockAdapter ensureConnected throws on reconnection failure", async () => {
    ...
    await assert.rejects(adapter.ensureConnected.call(adapter), (err) => err?.code === "E7lock.redis_connection_closed");
    await adapter.close();
});
test("RedisLockAdapter.releaseAsync throws when Lua script evaluation fails", async () => {
    ...
    const adapter = createAdapterWithMockRedis(mockRedis);
    await assert.rejects(adapter.releaseAsync("test-lock", "test-owner"), (err) => err instanceof Error);
});
test("RedisLockAdapter.extendAsync throws when Lua script evaluation fails", async () => {
    ...
    const adapter = createAdapterWithMockRedis(mockRedis);
    await assert.rejects(adapter.extendAsync("test-lock", "test-owner", 60_000), (err) => err instanceof Error);
});
...
test("RedisLockAdapter inspectAsync handles malformed lock data", async () => {
    ...
    const adapter = createAdapterWithMockRedis(mockRedis);
    await assert.rejects(adapter.inspectAsync("test-lock"), (err) => err instanceof Error);
});
test("RedisLockAdapter listHeldAsync handles scan errors gracefully", async () => {
    ...
    const adapter = createAdapterWithMockRedis(mockRedis);
    await assert.rejects(adapter.listHeldAsync(100), (err) => err instanceof Error);
});
...
test("RedisLockAdapter forceStealAsync throws when lock does not exist", async () => {
    ...
    const adapter = createAdapterWithMockRedis(mockRedis);
    await assert.rejects(adapter.forceStealAsync("nonexistent-key", "new-owner", "test reason"), (error) => error?.code === "E7lock.forceSteal_lock_not_found"
        && error?.message.includes("nonexistent-key"));
});
test("RedisLockAdapter forceStealAsync throws on connection error", async () => {
    ...
    const adapter = createAdapterWithMockRedis(mockRedis);
    await assert.rejects(adapter.forceStealAsync("test-key", "new-owner", "test reason"), (error) => error instanceof Error && error.message === "ECONNREFUSED");
});
test("RedisLockAdapter forceStealAsync throws on redis set error", async () => {
    ...
    const adapter = createAdapterWithMockRedis(mockRedis);
    await assert.rejects(adapter.forceStealAsync("test-key", "new-owner", "test reason"), (error) => error instanceof Error);
});
test("RedisLockAdapter forceStealAsync includes forceStealReason in metadata", async () => {
    const mockRedis = createMockRedis({
        status: "ready",
        set: async (_key, value, ..._args) => {
...
test("RedisLockAdapter extendAsync throws on connection error during eval", async () => {
    ...
    const adapter = createAdapterWithMockRedis(mockRedis);
    await assert.rejects(adapter.extendAsync("test-key", "test-owner", 5000), (error) => error instanceof Error && error.message === "Connection lost to Redis");
});
test("RedisLockAdapter extendAsync throws on connection error during get", async () => {
    ...
    const mockRedis = createMockRedis({
        ...
        get: async () => {
            if (!evalCalled)
                throw new Error("eval should be called first");
            throw new Error("Connection lost during GET");
        },
    });
    const adapter = createAdapterWithMockRedis(mockRedis);
    await assert.rejects(adapter.extendAsync("test-key", "test-owner", 5000), (error) => error instanceof Error && error.message === "Connection lost during GET");
});
...
test("RedisLockAdapter extendAsync handles partial JSON in lock data", async () => {
    ...
    const adapter = createAdapterWithMockRedis(mockRedis);
    await assert.rejects(adapter.extendAsync("test-key", "test-owner", 5000), (error) => error instanceof Error);
});
...
test("RedisLockAdapter releaseAsync throws on connection error", async () => {
    ...
    const adapter = createAdapterWithMockRedis(mockRedis);
    await assert.rejects(adapter.releaseAsync("test-key", "test-owner"), (error) => error instanceof Error && error.message === "Connection timeout");
});
test("RedisLockAdapter inspectAsync throws on connection error", async () => {
    ...
    const adapter = createAdapterWithMockRedis(mockRedis);
    await assert.rejects(adapter.inspectAsync("test-key"), (error) => error instanceof Error && error.message === "READONLY error");
});
test("RedisLockAdapter listHeldAsync throws on mget error", async () => {
    ...
    const adapter = createAdapterWithMockRedis(mockRedis);
    await assert.rejects(adapter.listHeldAsync(), (error) => error instanceof Error);
});
test("RedisLockAdapter acquireAsync throws on connection error", async () => {
    ...
    const adapter = createAdapterWithMockRedis(mockRedis);
    await assert.rejects(adapter.acquireAsync({ lockKey: "test-key", owner: "test-owner" }), (error) => error instanceof Error);
});
...
test("RedisLockAdapter releaseAsync handles redis error during DEL", async () => {
    ...
    const adapter = createAdapterWithMockRedis(mockRedis);
    await assert.rejects(adapter.releaseAsync("test-key", "test-owner"), (error) => error instanceof Error);
});
test("RedisLockAdapter close rethrows quit error", async () => {
    ...
    await assert.rejects(adapter.close(), (error) => error instanceof Error && error.message === "Quit failed - connection already closed");
});
...
test("RedisLockAdapter listHeldAsync respects limit and stops early", async () => {
    ...
    const mockRedis = createMockRedis({
        ...
        mget: async (..._keys) => {
            mgetCalls++;
            return _keys.map(() => JSON.stringify({
                owner: "test-owner",
                fencingToken: 1,
                ttlMs: 30000,
                acquiredAt: new Date().toISOString(),
...
//# sourceMappingURL=redis-lock-adapter.test.js.map