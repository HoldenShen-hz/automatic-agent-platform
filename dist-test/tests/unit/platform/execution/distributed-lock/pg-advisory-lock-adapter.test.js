...
test("PgAdvisoryLockAdapter constructor uses config.dsn when provided", () => {
    ...
    assert.equal(adapter.dsn, "postgresql://custom:custom@custom-host:5433/customdb");
});
test("PgAdvisoryLockAdapter constructor uses config.ssl when provided as object", () => {
    ...
    assert.deepEqual(adapter.ssl, { rejectUnauthorized: true });
});
test("PgAdvisoryLockAdapter constructor uses config.ssl when provided as false", () => {
    ...
    assert.equal(adapter.ssl, false);
});
test("PgAdvisoryLockAdapter constructor uses config.env when provided", () => {
    ...
    assert.equal(adapter.dsn, "postgresql://env:env@env-host:5432/envdb");
    assert.equal(adapter.poolMin, 2);
    assert.equal(adapter.poolMax, 20);
    assert.equal(adapter.idleTimeoutSeconds, 30);
    assert.equal(adapter.connectTimeoutSeconds, 15);
    assert.deepEqual(adapter.ssl, { rejectUnauthorized: true });
});
test("PgAdvisoryLockAdapter constructor uses config.postgresFactory when provided", () => {
    const customFactory = (_dsn, _options) => createMockDriver();
    const adapter = new PgAdvisoryLockAdapter({
        dsn: "postgresql://test:test@localhost/test",
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        postgresFactory: customFactory,
    });
    assert.equal(adapter.postgresFactory, customFactory);
});
test("PgAdvisoryLockAdapter ensureConnected throws connection error", () => {
    ...
    adapter.connectionError = null;
    assert.throws(() => adapter.ensureConnected(), (error) => error.message === "Connection refused");
});
