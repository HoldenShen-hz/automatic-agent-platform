...
test("[SYS-REL-2.2] concurrent extend by multiple owners - only one succeeds", async () => {
    ...
    const results = await Promise.allSettled(owners.map((owner) => Promise.resolve(adapter.extend("multi-extend-lock", owner, 60000))));
...
test("[SYS-REL-2.2] concurrent forceSteal on same lock - all steal attempts complete", async () => {
    ...
    const results = await Promise.allSettled(thieves.map((thief) => Promise.resolve(adapter.forceSteal("steal-lock", thief, "concurrent steal test"))));
...
test("[SYS-REL-2.2] concurrent forceSteal - final state has exactly one owner", async () => {
    ...
    await Promise.allSettled(attackers.map((attacker) => Promise.resolve(adapter.forceSteal("contested-steal", attacker, "race test"))));
...
test("[SYS-REL-2.2] concurrent forceSteal using runConcurrentInvariant", async () => {
    ...
    assert.ok(Array.from({ length: 5 }, (_, i) => `worker-${i}`).includes(finalLock.owner), `Final owner should be one of the workers`);
    db.close();
});
...
//# sourceMappingURL=sqlite-lock-adapter-concurrent.test.js.map