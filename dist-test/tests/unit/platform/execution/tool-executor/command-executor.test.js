...
function createCommandHarness(prefix) {
...
test("command executor blocks execution-scoped tool calls outside the allowed tool set", async () => {
    ...
    harness.db.connection.prepare(`UPDATE executions SET allowed_tools_json = ? WHERE id = ?`).run(JSON.stringify(["read"]), "exec-command");
    try {
...
test("command executor honors execution path roots when the request omits an explicit allowlist", async () => {
    ...
    harness.db.connection.prepare(`UPDATE executions SET allowed_paths_json = ? WHERE id = ?`).run(JSON.stringify([scopedDir]), "exec-command");
    try {
        createFile(join(scopedDir, "keep.txt"), "ok\n");
        createFile(outsideScopedPath, "outside\n");
        const result = await harness.executor.execute({
...
test("command executor fail-closes when execution allowed tools contain malformed entries", async () => {
    const harness = createCommandHarness("aa-command-auth-");
    harness.db.connection.prepare(`UPDATE executions SET allowed_tools_json = ? WHERE id = ?`).run(JSON.stringify(["command_exec", 1]), "exec-command");
    try {
        const result = await harness.executor.execute({
...
test("command executor fail-closes when execution allowed paths contain malformed entries", async () => {
    const harness = createCommandHarness("aa-command-auth-");
    harness.db.connection.prepare(`UPDATE executions SET allowed_paths_json = ? WHERE id = ?`).run(JSON.stringify(["", 7]), "exec-command");
    try {
        const result = await harness.executor.execute({
