...
function insertCompetingExecution(harness, taskId, executionId, traceId) {
    const now = nowIso();
...
test("edit replacement service fails with lock conflict when another execution holds the lock", async () => {
    ...
    try {
        createFile(filePath, "const value = 1;\n");
        const normalizedFilePath = checkSandboxPath(createWorkspaceWritePolicy(harness.workspace), filePath).normalizedPath;
...
test("edit replacement service rolls back all edits when a middle edit fails in batch", async () => {
    ...
    harness.db.connection.prepare(`UPDATE executions SET allowed_tools_json = ? WHERE id = ?`).run(JSON.stringify(["edit_replace", "edit_batch"]), "exec-edit");
    try {
        createFile(filePath, ["const alpha = 1;", "const beta = 2;", "const beta = 2;", "const gamma = 3;", ""].join("\n"));
        const result = await harness.service.executeBatch({
        ...
        assert.equal(readFileSync(filePath, "utf8"), ["const alpha = 1;", "const beta = 2;", "const beta = 2;", "const gamma = 3;", ""].join("\n"));
    }
    finally {
        harness.db.close();
        cleanupPath(harness.workspace);
