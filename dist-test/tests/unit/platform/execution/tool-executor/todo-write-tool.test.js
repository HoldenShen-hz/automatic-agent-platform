...
import { TodoWriteToolService, validateTodoTitle, validateTodoId, validateProgressPercent, executeTodoOperation, } from "../../../../../src/platform/execution/tool-executor/todo-write-tool.js";
...
test("TodoWriteToolService.deleteTodo removes todo", () => {
    ...
    service.deleteTodo({ todoId: created.todoId });
    assert.equal(service.getTodo({ todoId: created.todoId }), null);
});
test("TodoWriteToolService.deleteTodo throws for non-existent todo", () => {
    const service = new TodoWriteToolService();
    assert.throws(() => service.deleteTodo({ todoId: "todo_nonexistent" }), /not found/);
});
test("TodoWriteToolService.getTodo returns todo", () => {
    const service = new TodoWriteToolService();
    const created = service.createTodo(createMockRequest({ operation: "create" }));
    const retrieved = service.getTodo({ todoId: created.todoId });
...
test("TodoWriteToolService.getTodo returns null for non-existent", () => {
    const service = new TodoWriteToolService();
    const result = service.getTodo({ todoId: "todo_nonexistent" });
...
test("TodoWriteToolService.listTodos returns all todos", () => {
    ...
    const todos = service.listTodos({ operation: "list" });
...
//# sourceMappingURL=todo-write-tool.test.js.map