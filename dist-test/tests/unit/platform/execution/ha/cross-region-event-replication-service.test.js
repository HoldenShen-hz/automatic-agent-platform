...
// Test Fixtures
// ─────────────────────────────────────────────────────────────────────────────
function createTargetRegion(overrides = {}) {
    return {
        regionId: "region-b",
        status: "active",
...
test("replicate throws error when no target regions configured", () => {
    const publisher = createMockPublisher();
    const service = new CrossRegionEventReplicationService(publisher, "region-a");
    assert.throws(() => service.replicate("task.created", { taskId: "test" }), /No target regions configured/);
});
test("replicate replicates event to all registered targets", () => {
    ...
    service.registerTargetRegion(createTargetRegion({ regionId: "region-b" }));
    service.registerTargetRegion(createTargetRegion({ regionId: "region-c" }));
    const planId = service.replicate("task.created", { taskId: "test-task" });
    assert.ok(planId);
    const events = publisher.getPublishedEvents();
    assert.equal(events.length, 2);
    assert.ok(events.every((e) => e.eventType === "task.created"));
});
test("replicate replicates to specific target region when targetRegionIds provided", () => {
    ...
    service.replicate("task.created", { taskId: "test-task" }, ["region-b", "region-c"]);
    const events = publisher.getPublishedEvents();
    assert.equal(events.length, 2);
});
...
test("getReplicationStatus returns status for replicated event", () => {
    ...
    const service = new CrossRegionEventReplicationService(publisher, "region-a");
    service.registerTargetRegion(createTargetRegion({ regionId: "region-b" }));
    const eventId = service.replicate("task.created", { taskId: "test-task" });
...
test("getMetrics returns correct counts after replication", () => {
    ...
    const service = new CrossRegionEventReplicationService(publisher, "region-a");
    service.registerTargetRegion(createTargetRegion({ regionId: "region-b" }));
    service.replicate("task.created", { taskId: "test-task" });
    const metrics = service.getMetrics();
...
test("createCrossRegionEventReplicationService factory works", () => {
    ...
    service.registerTargetRegion(createTargetRegion({ regionId: "region-b" }));
    service.replicate("task.created", { taskId: "test" });
...
test("multiple replicates to same target region accumulate", () => {
    ...
    const service = new CrossRegionEventReplicationService(publisher, "region-a");
    service.registerTargetRegion(createTargetRegion({ regionId: "region-b" }));
    service.replicate("task.created", { taskId: "task-1" });
    service.replicate("task.updated", { taskId: "task-2" });
    const events = publisher.getPublishedEvents();
...
test("replicate event with different payload types", () => {
    ...
    const service = new CrossRegionEventReplicationService(publisher, "region-a");
    service.registerTargetRegion(createTargetRegion({ regionId: "region-b" }));
    service.replicate("task.created", { taskId: "test", priority: "high" });
    service.replicate("agent.delegated", { agentId: "agent-1", targetId: "agent-2" });
    const events = publisher.getPublishedEvents();
    assert.equal(events.length, 2);
});
...
//# sourceMappingURL=cross-region-event-replication-service.test.js.map