...
function createService(options = {}) {
    ...
    const serviceOptions = {
        onStuckRunDetected: options.onStuckRunDetected ?? ((run) => {
            events.push({ type: "detected", run, timestamp: clock.now() });
        }),
        onWarningIssued: options.onWarningIssued ?? ((run) => {
            events.push({ type: "warning", run, timestamp: clock.now() });
        }),
        onRunKilled: options.onRunKilled ?? ((run) => {
            events.push({ type: "killed", run, timestamp: clock.now() });
        }),
        onRunCleanedUp: options.onRunCleanedUp ?? ((run) => {
