...
import { HaCoordinatorService, DEFAULT_LEASE_TTL_MS, MAX_LEASE_TTL_MS, MIN_LEASE_TTL_MS, HA_COORDINATOR_DDL, } from "../../../../../src/platform/execution/ha/ha-coordinator-service-inner.js";
class MockDatabase {
    tables = new Map();
    fencingTokenCounter = 1;
    transaction(fn) {
        return fn();
    }
    get connection() {
...
function createService() {
    ...
    return new HaCoordinatorService(db);
}
// ─────────────────────────────────────────────────────────────────────────────
...
test("HaCoordinatorService - acquireLeadership fails for unregistered node", () => {
    const service = createService();
    assert.throws(() => service.acquireLeadership({ nodeId: "unknown-node" }), /Must register node before acquiring leadership/i);
});
...
//# sourceMappingURL=ha-coordinator-service-inner.test.js.map