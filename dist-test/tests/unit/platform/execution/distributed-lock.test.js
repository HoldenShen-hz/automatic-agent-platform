...
test("SqliteLockAdapter forceSteal throws LockingError on database error", () => {
    ...
    db.close();
    assert.throws(() => adapter.forceSteal("test-lock", "new-owner", "test"), (error) => error.code?.includes("lock.force_steal_failed"));
});
...
test("PgAdvisoryLockAdapter lockKeyToAdvisoryKey handles empty string", () => {
    ...
    const key = adapter.lockKeyToAdvisoryKey("");
    assert.equal(typeof key, "bigint");
});
test("PgAdvisoryLockAdapter lockKeyToAdvisoryKey handles unicode characters", () => {
    const adapter = new PgAdvisoryLockAdapter({ dsn: "postgres://localhost/test" });
    const key1 = adapter.lockKeyToAdvisoryKey("lock-测试-🔒");
    const key2 = adapter.lockKeyToAdvisoryKey("lock-测试-🔒");
...
test("PgAdvisoryLockAdapter lockKeyToAdvisoryKey handles long strings", () => {
    ...
    const key = adapter.lockKeyToAdvisoryKey(longKey);
    assert.equal(typeof key, "bigint");
...
test("PgAdvisoryLockAdapter acquireAsync throws when postgres module not found", async () => {
    ...
    await assert.rejects(adapter.acquireAsync({ lockKey: "test-key", owner: "test-owner" }), (error) => error.code?.includes("lock.pg_advisory_not_implemented"));
});
...
test("PgAdvisoryLockAdapter releaseAsync throws when postgres module not found", async () => {
    ...
    await assert.rejects(adapter.releaseAsync("test-key", "test-owner"), (error) => error.code?.includes("lock.pg_advisory_not_implemented"));
});
...
test("RedisLockAdapter uses default host and port", () => {
    ...
    assert.equal(adapter.port, 6379);
});
...
test("RedisLockAdapter acquireAsync throws on connection error", async () => {
    ...
    await assert.rejects(adapter.acquireAsync({ lockKey: "test-key", owner: "test-owner" }), (error) => error instanceof Error && error.message === "Connection reset by peer");
});
test("RedisLockAdapter releaseAsync throws on Lua script error", async () => {
    ...
    await assert.rejects(adapter.releaseAsync("test-key", "test-owner"), (error) => error instanceof Error && error.message === "ERR user script too slow");
});
...
test("DISTRIBUTED_LOCKS_DDL creates valid SQLite table", () => {
  ...
  `).run("ddl-test", "owner-1", 1, "held", new Date().toISOString(), 30000);
    const row = db.prepare("SELECT * FROM distributed_locks WHERE lock_key = ?").get("ddl-test");
