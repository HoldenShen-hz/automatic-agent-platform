...
function createMockDiagnosticsService(snapshot = {}, warnings = { critical: [], warning: [] }, incident = {}) {
    return {
        buildTaskSnapshot: () => snapshot,
        buildDebugDump: () => ({
...
function createMockStalledDetector(findings = []) {
    return {
        detect: (options) => findings,
    };
}
...
test("StalledExecutionEscalationService passes through incident data", () => {
    ...
    const detector = createMockStalledDetector(findings);
    const diagnostics = createMockDiagnosticsService(mockSnapshot, { critical: ["memory"], warning: ["slow"] }, {
        summary: { totalEntries: 5, highestSeverity: "warning" },
