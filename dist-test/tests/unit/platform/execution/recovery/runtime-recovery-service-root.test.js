...
function makeRecoveryRecord(overrides = {}) {
    return {
        executionId: "exec-1",
        taskId: "task-1",
        divisionId: null,
...
test("RuntimeRecoveryService.buildRuntimeRecoveryView throws when task not found", () => {
    ...
    const service = new RuntimeRecoveryService(store);
    assert.throws(() => service.buildRuntimeRecoveryView("nonexistent-task"), (err) => err.message.includes("Task not found"));
});
...
//# sourceMappingURL=runtime-recovery-service-root.test.js.map