...
function createMockStore(overrides = {}) {
    return {
        task: {
            getTask: (id) => overrides.tasks?.find((t) => t.id === id) ?? null,
        },
        execution: {
...
test("RuntimeRecoveryReplayService.buildTaskReplayReport throws when task not found", () => {
    ...
    const service = new RuntimeRecoveryReplayService(store);
    assert.throws(() => service.buildTaskReplayReport("nonexistent-task"), (err) => err.message.includes("Task not found"));
});
test("RuntimeRecoveryReplayService.buildExecutionReplayReport throws when execution not found", () => {
    ...
    const service = new RuntimeRecoveryReplayService(store);
    assert.throws(() => service.buildExecutionReplayReport("nonexistent-exec"), (err) => err.message.includes("Execution not found"));
});
...
//# sourceMappingURL=runtime-recovery-replay-service-root.test.js.map