...
import { PluginExecutionService, PluginExecutorService, ScopedExternalAccessSandbox, createScopedExternalAccessSandbox, BrowserExecutor, } from "../../../../src/platform/execution/plugin-executor/index.js";
...
const createTestHooks = (overrides = {}) => ({
    initialize: async () => { },
    onLoad: async () => { },
    onActivate: async () => { },
    onDeactivate: async () => { },
...
const createBrowserTestContext = (overrides = {}) => ({
    executionId: "exec-123",
    taskId: "task-456",
    tenantId: "tenant-789",
    correlationId: "corr-abc",
...
const createStepDefinition = (stepId, name, action, overrides = {}) => ({
    stepId,
    name,
    action,
    maxRetries: 3,
...
const createWorkflowDefinition = (workflowId, steps, overrides = {}) => ({
    workflowId,
...
test("PluginExecutionService.execute() throws for unregistered plugin", async () => {
    ...
    await assert.rejects(async () => service.execute(request), (err) => {
...
test("PluginExecutionService.listPlugins() returns copy of plugins array", () => {
    ...
    const plugin = {
        pluginId: "test-plugin",
        actions: ["action1"],
        execute: () => ({ pluginId: "test-plugin", action: "action1", status: "ok", output: {} }),
    };
    service.register(plugin);
    const plugins = service.listPlugins();
...
test("PluginExecutorService.load() throws for unregistered plugin", async () => {
    const service = new PluginExecutorService();
    await assert.rejects(async () => service.load("nonexistent-plugin"), (err) => err.message.includes("not found"));
});
...
test("PluginExecutorService.register() twice with same pluginId throws", () => {
    ...
    const hooks = createTestHooks();
    service.register(manifest, hooks);
    assert.throws(() => service.register(manifest, hooks), (err) => err.message.includes("already registered"));
});
// ─────────────────────────────────────────────────────────────────────────────
...
test("PluginExecutorService.execute() with retriever action returns output", async () => {
    ...
    const hooks = createTestHooks({
        retriever: async (input) => ({ retrieved: true, data: input }),
    });
    service.register(manifest, hooks);
    await service.load("test-plugin");
    await service.activate("test-plugin");
...
test("PluginExecutorService.execute() with validator action returns output", async () => {
    ...
    const hooks = createTestHooks({
        validator: async (input) => ({ valid: true, input }),
    });
    service.register(manifest, hooks);
    await service.load("test-plugin");
    await service.activate("test-plugin");
...
test("PluginExecutorService.unregister() throws for unknown pluginId", async () => {
    const service = new PluginExecutorService();
    await assert.rejects(async () => service.unregister("nonexistent-plugin"), (err) => err.message.includes("not registered"));
});
// ─────────────────────────────────────────────────────────────────────────────
...
test("BrowserExecutor.closeSession() can be called multiple times if session already deleted", () => {
    ...
    const sessionId = executor.createSession(context);
    executor.closeSession(sessionId);
    assert.throws(() => executor.closeSession(sessionId), (err) => err.message.includes("not found"));
});
// ─────────────────────────────────────────────────────────────────────────────
...
test("SubWorkflowExecutor.executeWorkflow() throws for cancelled workflow", async () => {
    ...
    await assert.rejects(async () => executor.executeWorkflow(executionId), (err) => err.message.includes("cannot be executed"));
});
...
test("SubWorkflowExecutor.performRollbackFromId() throws for rollback policy none", async () => {
    ...
    await assert.rejects(async () => executor.performRollbackFromId(executionId), (err) => err.message.includes("Rollback is not allowed"));
});
...
test("SubWorkflowExecutor.cancelWorkflow() throws for completed workflow", async () => {
    ...
    const executionId = executor.createWorkflow(definition, context);
    await executor.executeWorkflow(executionId);
    await assert.rejects(async () => executor.cancelWorkflow(executionId), (err) => err.message.includes("cannot be cancelled"));
});
test("SubWorkflowExecutor.pauseWorkflow() throws for non-running workflow", () => {
    ...
    const executionId = executor.createWorkflow(definition, context);
    assert.throws(() => executor.pauseWorkflow(executionId), (err) => err.message.includes("cannot be paused"));
});
test("SubWorkflowExecutor.pauseWorkflow() throws for unknown workflow", () => {
    const executor = new SubWorkflowExecutor();
    assert.throws(() => executor.pauseWorkflow("nonexistent"), (err) => err.message.includes("not found"));
});
test("SubWorkflowExecutor.cancelWorkflow() throws for unknown workflow", async () => {
    const executor = new SubWorkflowExecutor();
    await assert.rejects(async () => executor.cancelWorkflow("nonexistent"), (err) => err.message.includes("not found"));
});
test("SubWorkflowExecutor.skipStep() throws for unknown workflow", () => {
    const executor = new SubWorkflowExecutor();
    assert.throws(() => executor.skipStep("nonexistent", "step-1", "reason"), (err) => err.message.includes("not found"));
});
test("SubWorkflowExecutor.skipStep() throws for unknown step", () => {
    ...
    const executionId = executor.createWorkflow(definition, context);
    assert.throws(() => executor.skipStep(executionId, "nonexistent", "reason"), (err) => err.message.includes("not found"));
});
test("SubWorkflowExecutor.skipStep() throws for non-pending step", async () => {
    ...
    const executionId = executor.createWorkflow(definition, context);
    await executor.executeWorkflow(executionId);
    assert.throws(() => executor.skipStep(executionId, "s1", "reason"), (err) => err.message.includes("cannot be skipped"));
});
test("SubWorkflowExecutor.retryStep() throws for unknown workflow", async () => {
    const executor = new SubWorkflowExecutor();
    await assert.rejects(async () => executor.retryStep("nonexistent", "step-1"), (err) => err.message.includes("not found"));
});
test("SubWorkflowExecutor.retryStep() throws for unknown step", async () => {
    ...
    const executionId = executor.createWorkflow(definition, context);
    await assert.rejects(async () => executor.retryStep(executionId, "nonexistent"), (err) => err.message.includes("not found"));
});
test("SubWorkflowExecutor.retryStep() throws for non-failed step", async () => {
    ...
    const executionId = executor.createWorkflow(definition, context);
    await assert.rejects(async () => executor.retryStep(executionId, "s1"), (err) => err.message.includes("cannot be retried"));
});
test("SubWorkflowExecutor.retryStep() throws when max retries exceeded", async () => {
    ...
    await assert.rejects(async () => executor.retryStep(executionId, "s1"), (err) => err.message.includes("exceeded maximum retry count"));
});
test("SubWorkflowExecutor.performRollbackFromId() throws for unknown workflow", async () => {
    const executor = new SubWorkflowExecutor();
    await assert.rejects(async () => executor.performRollbackFromId("nonexistent"), (err) => err.message.includes("not found"));
});
test("SubWorkflowExecutor.enforces max nested depth", () => {
    ...
    assert.throws(() => executor.createWorkflow(definition, context), (err) => err.message.includes("Maximum nested workflow depth"));
});
...
test("SubWorkflowExecutor.getExecutionLog() returns all results", async () => {
    ...
    const id1 = executor.createWorkflow(createWorkflowDefinition("wf-1", [createStepDefinition("s1", "S1", "a1")]), context);
    const id2 = executor.createWorkflow(createWorkflowDefinition("wf-2", [createStepDefinition("s2", "S2", "a2")]), context);
...
test("SubWorkflowExecutor.getExecutionLog() returns immutable copy", async () => {
    ...
    const id = executor.createWorkflow(createWorkflowDefinition("wf-1", [createStepDefinition("s1", "S1", "a1")]), context);
...
//# sourceMappingURL=plugin-executor.test.js.map