...
test("[SYS-REL-2.4] enqueue propagates Redis write failure", async () => {
    ...
    await assert.rejects(async () => enqueue({ queueName: "test-queue", payload: { data: "test" } }), { message: /READONLY/ }, "Enqueue should reject when Redis write fails with READONLY");
});
test("[SYS-REL-2.4] pipeline .catch(() => {}) swallows errors - defect demonstration", async () => {
    ...
    const promise = mockPipeline.exec().catch((err) => {
        errorCaught = true;
...
test("[SYS-REL-2.4] enqueueAsync properly propagates write failures", async () => {
    ...
    const enqueueAsync = async (input) => {
    ...
    await assert.rejects(async () => enqueueAsync({ queueName: "test-queue", payload: { data: "test" } }), { message: /OOM/ }, "enqueueAsync should reject when hmset fails");
...
test("[SYS-REL-2.4] queue adapter should track failed enqueue operations", async () => {
    ...
    const trackFailure = (operation, error) => {
    ...
    assert.ok(failedOperations.every((f) => f.operation === "enqueue"), "All failures should be enqueue operations");
});
test("[SYS-REL-2.4] redis queue adapter pipeline error must be caught and re-thrown", async () => {
    ...
    const mockPipeline = {
        ...
        hmset: function (_key, _data) { return mockPipeline; },
        expire: function (_key, _seconds) { return mockPipeline; },
        sadd: function (_key, _member) { return mockPipeline; },
        zadd: function (_key, _score, _member) { return mockPipeline; },
    };
    const mockRedis = {
    ...
    assert.ok(caughtError.message.includes("MISCONF") || caughtError.message.includes("queue.enqueue_failed"), "Error should indicate enqueue failure");
});
...
//# sourceMappingURL=redis-queue-adapter-error-propagation.test.js.map