...
import { PluginExecutorService } from "../../../../../src/platform/execution/plugin-executor/index.js";
// ─────────────────────────────────────────────────────────────────────────────
// Test Fixtures
// ─────────────────────────────────────────────────────────────────────────────
...
const createActionHooks = (action, handler, overrides = {}) => createTestHooks({
    [action]: handler,
    ...overrides,
});
...
test("PluginExecutorService.execute() rejects executing inactive plugin", async () => {
    ...
    await assert.rejects(() => service.execute("test-plugin", "retriever", context, {}), (err) => {
...
test("PluginExecutorService.execute() throws for unknown plugin", async () => {
    const service = new PluginExecutorService();
    const context = createTestContext();
    await assert.rejects(() => service.execute("nonexistent-plugin", "retriever", context, {}), (err) => {
...
test("PluginExecutorService.execute() rejects action not in manifest spiTypes", async () => {
    ...
    await service.activate("test-plugin");
    const context = createTestContext();
    await assert.rejects(() => service.execute("test-plugin", "validator", context, {}), (err) => {
...
test("ScopedExternalAccessSandbox validates domain whitelist", async () => {
    const { ScopedExternalAccessSandbox } = await import("../../../../../src/platform/execution/plugin-executor/scoped-external-access-sandbox.js");
...
test("PluginExecutorService.activate() throws for disabled plugin", async () => {
    ...
    service.register(manifest, hooks);
    await service.unregister("test-plugin");
    await assert.rejects(() => service.activate("test-plugin"), (err) => {
        return err.message.includes("disabled");
    });
});
//# sourceMappingURL=plugin-executor.service.test.js.map