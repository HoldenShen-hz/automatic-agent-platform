...
function closeAndCleanup(path, db) {
    db.close();
...
test("[SYS-REL-2.2] SQLite lock adapter honours TTL on stale locks - evicts expired lock", () => {
    ...
    try {
        ...
        db.prepare(`INSERT INTO distributed_locks (lock_key, owner, fencing_token, status, acquired_at, ttl_ms)
       VALUES (?, ?, ?, 'held', ?, ?)`).run("stale-key", "dead-owner", 100, "2020-01-01T00:00:00.000Z", 1000);
...
test("[SYS-REL-2.2] SQLite lock adapter ttlMs=0 means infinite TTL (never expires)", () => {
    ...
    try {
        ...
        db.prepare(`INSERT INTO distributed_locks (lock_key, owner, fencing_token, status, acquired_at, ttl_ms)
       VALUES (?, ?, ?, 'held', ?, 0)`).run("infinite-key", "some-owner", 1, "2020-01-01T00:00:00.000Z");
...
//# sourceMappingURL=sqlite-lock-adapter-robustness.test.js.map