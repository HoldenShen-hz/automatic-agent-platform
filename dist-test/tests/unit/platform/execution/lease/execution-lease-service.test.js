...
function createMockStore(state = {
    ...
    return {
        worker: {
            ...
            listExpiredExecutionLeases(_now) {
                const now = new Date();
                return Array.from(state.leases.values()).filter((l) => l.status === "active" && new Date(l.expiresAt) < now);
            },
            ...
            renewExecutionLease(leaseId, _expiresAt, _lastHeartbeatAt) {
            ...
            getWorkerSnapshot(_workerId) {
                return state.workers.get(_workerId);
            },
            upsertWorkerSnapshot(_snapshot) {
                // Mock implementation - just store it
            },
            getAgentExecutionRecord(executionId) {
                return state.agentExecutionRecords.get(executionId);
            },
            upsertAgentExecutionRecord(_record) { },
        },
        dispatch: {
        ...
        execution: {
            updateExecutionAgent(_executionId, _agentId, _occurredAt) { },
        },
        event: {
            insertEvent(_event) { },
        },
...
function createMockDb(transactionFn) {
    return {
        filePath: ":memory:",
        backendType: "sqlite",
...
function createExecution(id = "exec-1", attempt = 1) {
    return {
        id,
        attempt,
        taskId: "task-1",
...
test("acquireLease throws when execution not found", () => {
    ...
    const db = createMockDb((work) => work());
    const service = new ExecutionLeaseService(db, store);
    assert.throws(() => service.acquireLease({ executionId: "nonexistent", workerId: "worker-1", ttlMs: 10_000 }), (err) => err.code === "storage.execution_not_found");
});
...
//# sourceMappingURL=execution-lease-service.test.js.map