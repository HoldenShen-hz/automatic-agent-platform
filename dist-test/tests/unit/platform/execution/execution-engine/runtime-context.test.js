...
import { provideContext, getContext, getContextOrNull, withContextPatch, assertContext, getTenantId, getTenantIdOrNull, getWorkspaceId, getWorkspaceIdOrNull, hasTenantContext, hasWorkspaceContext, } from "../../../../../src/platform/execution/execution-engine/runtime-context.js";
...
test("getContext throws when called outside provideContext", () => {
    assert.throws(() => getContext(), /runtime_context.missing/, "getContext should throw when outside provideContext");
});
...
test("withContextPatch throws when called outside provideContext", () => {
    assert.throws(() => withContextPatch({ taskId: "new_task" }, () => "result"), /runtime_context.missing/, "withContextPatch should throw when outside provideContext");
});
...
test("assertContext throws when keys are missing", () => {
    ...
    provideContext(snapshot, () => {
        assert.throws(() => assertContext("executionId", "sessionId"), /Missing required context fields: executionId, sessionId/, "assertContext should throw when required keys are missing");
    });
});
test("assertContext throws with empty string", () => {
    ...
    provideContext(snapshot, () => {
        assert.throws(() => assertContext("taskId"), /Missing required context fields: taskId/, "assertContext should throw for empty string");
    });
});
