...
import { AgentExecutor, initializeAgentExecutor, getAgentExecutorContext, getGlobalAgentMiddlewareChain, createAgentExecutor, } from "../../../../../src/platform/execution/execution-engine/agent-executor.js";
// Test helper to reset executor context between tests
function resetExecutorContext() {
...
test("AgentExecutor instantiates with default options", () => {
    const executor = new AgentExecutor();
    assert.ok(executor instanceof AgentExecutor, "AgentExecutor should be instantiable");
});
test("AgentExecutor instantiates with custom options", () => {
    const options = {
        failOpen: true,
...
test("AgentExecutor getRegisteredHooks returns hook names", () => {
    const executor = new AgentExecutor();
...
test("AgentExecutor getLoopDetectionPatterns returns array", () => {
    const executor = new AgentExecutor();
...
test("AgentExecutor resetLoopDetection does not throw", () => {
    const executor = new AgentExecutor();
    assert.doesNotThrow(() => executor.resetLoopDetection(), "resetLoopDetection should not throw");
});
test("AgentExecutor executeAgentRound handles missing context gracefully", async () => {
    const executor = new AgentExecutor();
    const input = {
...
test("AgentExecutor wrapToolCall returns result and warnings", async () => {
    const executor = new AgentExecutor();
...
test("initializeAgentExecutor returns context", () => {
...
test("initializeAgentExecutor returns same context on multiple calls", () => {
...
test("getAgentExecutorContext returns context after initialization", () => {
...
test("getGlobalAgentMiddlewareChain returns middleware chain", () => {
...
test("createAgentExecutor factory creates AgentExecutor instance", () => {
    const executor = createAgentExecutor();
    assert.ok(executor instanceof AgentExecutor, "createAgentExecutor should create AgentExecutor instance");
});
test("AgentExecutor with loop detection config", () => {
    const loopConfig = {
        warnThreshold: 2,
        escalateThreshold: 4,
        askAtWarn: true,
...
test("AgentExecutor with null loop detection disables loop detection", () => {
...
test("AgentExecutor executeAgentRound with stepId", async () => {
    const executor = new AgentExecutor();
    const input = {
        request: "test request",
        history: [],
...
test("AgentExecutor executeAgentRound with model override", async () => {
    const executor = new AgentExecutor();
    const input = {
        request: "test request",
        history: [],
...
test("AgentExecutor executeAgentRound tracks agent round increment", async () => {
    const executor = new AgentExecutor();
    const context = {
        traceId: newId("trace"),
        taskId: newId("task"),
...
test("AgentExecutor executeAgentRound with empty messages", async () => {
    const executor = new AgentExecutor();
    const input = {
...
test("AgentExecutor executeAgentRound with history", async () => {
    const executor = new AgentExecutor();
    const input = {
...
test("AgentExecutor context includes sessionId when provided", () => {
    const ctx = {
        traceId: newId("trace"),
        taskId: newId("task"),
        executionId: newId("exec"),
...
test("AgentExecutorResult has correct structure", async () => {
    const executor = new AgentExecutor();
    const input = {
        request: "test",
        history: [],
