...
import { ModelCallProviderService, resetModelCallProvider, getModelCallProvider, initializeModelCallProvider, } from "../../../../../src/platform/execution/execution-engine/model-call-provider.js";
...
test("ModelCallProviderService getDefaultModel returns configured default", () => {
    const config = { defaultModel: "claude-3-5-sonnet" };
    const provider = new ModelCallProviderService(config);
...
test("ModelCallProviderService createCompletion throws when no provider configured", async () => {
    ...
    await assert.rejects(async () => provider.createCompletion(request), (error) => {
...
test("ModelCallProviderService createStreamingCompletion throws when no provider configured", async () => {
    ...
    const onChunk = () => { };
    await assert.rejects(async () => provider.createStreamingCompletion(request, onChunk), (error) => {
        if (error instanceof Error && error.message.includes("No model provider configured")) {
            return true;
        }
        return false;
...
//# sourceMappingURL=model-call-provider.test.js.map