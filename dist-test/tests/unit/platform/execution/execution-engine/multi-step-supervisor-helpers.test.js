...
import { normalizeStepFailurePlan, resolveStepFailurePlan, normalizeStepErrorCode, buildStepFailureSummary, } from "../../../../../src/platform/execution/execution-engine/multi-step-supervisor.js";
// =============================================================================
...
test("normalizeStepFailurePlan returns StepFailurePlan unchanged", () => {
    const input = { errorCode: "validation.schema_mismatch", summary: "Schema validation failed" };
...
test("normalizeStepFailurePlan preserves message when present", () => {
    const input = { errorCode: "timeout", message: "Step timed out after 30s" };
    const result = normalizeStepFailurePlan(input);
...
test("buildStepFailureSummary returns retry message when action is retry", () => {
    const decision = { action: "retry", errorCode: "tool.timeout", failureClass: "transient", retryable: true, backoff: "none", retryDelayMs: 1000 };
...
test("buildStepFailureSummary returns escalate message when action is escalate", () => {
    const decision = { action: "escalate", errorCode: "workflow.circuit_broken", failureClass: "destructive", retryable: false, backoff: "none", retryDelayMs: 0 };
...
test("buildStepFailureSummary returns step failed message for default action", () => {
    const decision = { action: "fail", errorCode: "internal.error", failureClass: "non_retryable", retryable: false, backoff: "none", retryDelayMs: 0 };
...
test("buildStepFailureSummary includes error code in message", () => {
    const decision = { action: "retry", errorCode: "validation.timeout", failureClass: "transient", retryable: true, backoff: "exponential", retryDelayMs: 2000 };
...
//# sourceMappingURL=multi-step-supervisor-helpers.test.js.map