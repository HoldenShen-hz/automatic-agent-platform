...
function createAdapterWithMockRedis(mockRedis) {
    ...
    adapter.client.redis = mockRedis;
    return adapter;
}
...
test("RedisQueueAdapter dequeueAsync nack requeues when under maxAttempts", async () => {
    ...
    const mockRedis = createMockRedisClient({
        ...
        hincrby: async () => 1,
        hget: async (_key, field) => field === "max_attempts" ? "3" : "1",
        hmset: async (_key, data) => {
            hmsetCalls.push(data);
        },
        sadd: async () => 1,
...
test("RedisQueueAdapter retryJobAsync resets failed job to waiting", async () => {
    ...
    const mockRedis = createMockRedisClient({
        ...
        srem: async (key) => {
            if (key.includes("active"))
                sremCalledForActive = true;
            if (key.includes("dead_letter"))
                sremCalledForDl = true;
            return 1;
        },
        sadd: async () => 1,
        zadd: async () => 1,
...
test("RedisQueueAdapter purgeAsync deletes completed jobs older than cutoff", async () => {
    ...
    const mockRedis = createMockRedisClient({
        ...
        del: async (key) => {
            if (key.includes("old-job"))
                deletedJobs.push("old-job");
            if (key.includes("new-job"))
                deletedJobs.push("new-job");
            return 1;
        },
        srem: async () => 1,
    });
...
test("RedisQueueAdapter dequeue throws sync_not_supported", () => {
    const adapter = new RedisQueueAdapter({ host: "localhost", port: 6379 });
    assert.throws(() => adapter.dequeue("test-queue"), (err) => err.message.includes("sync_dequeue_not_supported"));
});
test("RedisQueueAdapter getJob throws sync_not_supported", () => {
    const adapter = new RedisQueueAdapter({ host: "localhost", port: 6379 });
    assert.throws(() => adapter.getJob("job-id"), (err) => err.message.includes("sync_getJob_not_supported"));
});
test("RedisQueueAdapter listJobs throws sync_not_supported", () => {
    const adapter = new RedisQueueAdapter({ host: "localhost", port: 6379 });
    assert.throws(() => adapter.listJobs("test-queue"), (err) => err.message.includes("sync_listJobs_not_supported"));
});
test("RedisQueueAdapter moveToDeadLetter throws sync_not_supported", () => {
    const adapter = new RedisQueueAdapter({ host: "localhost", port: 6379 });
    assert.throws(() => adapter.moveToDeadLetter("job-id", "reason"), (err) => err.message.includes("sync_moveToDeadLetter_not_supported"));
});
test("RedisQueueAdapter retryJob throws sync_not_supported", () => {
    const adapter = new RedisQueueAdapter({ host: "localhost", port: 6379 });
    assert.throws(() => adapter.retryJob("job-id"), (err) => err.message.includes("sync_retryJob_not_supported"));
});
test("RedisQueueAdapter purge throws sync_not_supported", () => {
    const adapter = new RedisQueueAdapter({ host: "localhost", port: 6379 });
    assert.throws(() => adapter.purge("test-queue", "2026-04-01"), (err) => err.message.includes("sync_purge_not_supported"));
});
test("RedisQueueAdapter stats throws sync_not_supported", () => {
    const adapter = new RedisQueueAdapter({ host: "localhost", port: 6379 });
    assert.throws(() => adapter.stats("test-queue"), (err) => err.message.includes("sync_stats_not_supported"));
});
test("RedisQueueAdapter listQueues throws sync_not_supported", () => {
    const adapter = new RedisQueueAdapter({ host: "localhost", port: 6379 });
    assert.throws(() => adapter.listQueues(), (err) => err.message.includes("sync_listQueues_not_supported"));
});
//# sourceMappingURL=redis-queue-adapter-functions.test.js.map