...
test("RedisQueueAdapter mapRedisToJobRecord handles invalid status value", () => {
    ...
    assert.equal(result.status, "invalid-status");
});
// ─────────────────────────────────────────────────────────────────────────────
// nack() When currentAttempts >= maxAttempts (Dead Letter Path)
...
test("SYS-REL-2.4 sync enqueue() must propagate pipeline failure to caller", async () => {
    ...
    assert.deepEqual(runtimeMetricsRegistry.getCounters("queue_enqueue_failures_total").map((series) => ({
        labels: series.labels,
        value: series.value,
    })), [{ labels: { backend: "redis", mode: "sync" }, value: 1 }], "SYS-REL-2.4: sync enqueue() should record pipeline.exec() failure in metrics");
});
test("SYS-REL-2.4 sync enqueue() must propagate pipeline result-level failure to caller", async () => {
    ...
    assert.deepEqual(runtimeMetricsRegistry.getCounters("queue_enqueue_failures_total").map((series) => ({
        labels: series.labels,
        value: series.value,
    })), [{ labels: { backend: "redis", mode: "sync" }, value: 1 }], "SYS-REL-2.4: sync enqueue() should record result-level pipeline failures in metrics");
});
//# sourceMappingURL=redis-queue-adapter-errors.test.js.map