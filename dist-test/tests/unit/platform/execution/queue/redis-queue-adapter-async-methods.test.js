    ...
    const mockRedis = createMockRedisClient({
        smembers: async (key) => {
            if (key.includes("completed"))
                return ["purge-job-1", "purge-job-2"];
            return [];
        },
        hgetall: async (key) => {
...
test("RedisQueueAdapter ping throws when Redis is unavailable", async () => {
    ...
    const adapter = createAdapterWithMockRedis(mockRedis);
    await assert.rejects(adapter.ping(), (err) => err instanceof Error);
});
// =============================================================================
// close Tests
// =============================================================================
...
//# sourceMappingURL=redis-queue-adapter-async-methods.test.js.map