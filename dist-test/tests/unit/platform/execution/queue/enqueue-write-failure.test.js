...
function getMockClient(adapter) {
    return adapter;
}
function createMockPipeline(execResult) {
    return {
...
function isWrappedEnqueueFailure(err, expectedCauseFragment) {
...
test("[SYS-REL-2.4] enqueueAsync properly propagates errors - contrast with enqueue", async () => {
    ...
    }, (err) => isWrappedEnqueueFailure(err, "HMSET"));
});
test("[SYS-REL-2.4] enqueueAsync expire error propagates", async () => {
    ...
    }, (err) => isWrappedEnqueueFailure(err, "EXPIRE"));
});
test("[SYS-REL-2.4] enqueueAsync sadd error propagates", async () => {
    ...
    }, (err) => isWrappedEnqueueFailure(err, "SADD"));
});
test("[SYS-REL-2.4] enqueueAsync zadd error propagates", async () => {
    ...
    }, (err) => isWrappedEnqueueFailure(err, "ZADD"));
});
...
//# sourceMappingURL=enqueue-write-failure.test.js.map