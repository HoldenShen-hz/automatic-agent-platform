...
test("assertTransition allows valid transition", () => {
    const transitions = { queued: ["pending", "failed"], pending: [], failed: [] };
...
test("assertTransition rejects invalid transition", () => {
    const transitions = { a: ["c"], b: [] };
    const machine = new StateTransitionMachine("test", transitions);
    assert.throws(() => machine.assertTransition("a", "b"), (err) => err instanceof WorkflowStateError && err.message.includes("invalid_transition"));
});
test("assertTransition rejects transition from terminal state", () => {
    const transitions = { active: ["done"], done: [] };
