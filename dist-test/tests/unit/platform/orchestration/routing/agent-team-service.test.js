...
function createMockWorkflow(steps = []) {
    return {
        workflow: {
            workflowId: "wf_test",
...
test("AgentTeamService.buildPlan includes planner lane", () => {
    ...
    const plan = service.buildPlan(input);
    const plannerLane = plan.lanes.find((l) => l.stage === "plan");
...
test("AgentTeamService.buildPlan includes reviewer lane", () => {
    ...
    const plan = service.buildPlan(input);
    const reviewerLane = plan.lanes.find((l) => l.stage === "review");
...
test("AgentTeamService.buildPlan maps workflow steps to build lanes", () => {
    ...
    const plan = service.buildPlan(input);
    const buildLanes = plan.lanes.filter((l) => l.stage === "build");
    assert.equal(buildLanes.length, 2);
    assert.ok(buildLanes.some((l) => l.ownerRoleId === "coder"));
    assert.ok(buildLanes.some((l) => l.ownerRoleId === "tester"));
});
test("AgentTeamService.buildPlan sets correct model tiers for high risk review", () => {
    ...
    const plan = service.buildPlan(input);
    const reviewLane = plan.lanes.find((l) => l.stage === "review");
...
test("AgentTeamService.buildPlan includes repair lane for steps with compensation", () => {
    ...
    const plan = service.buildPlan(input);
    const repairLanes = plan.lanes.filter((l) => l.stage === "repair");
...
test("AgentTeamService.buildPlan assigns validate lane", () => {
    ...
    const plan = service.buildPlan(input);
    const validateLane = plan.lanes.find((l) => l.stage === "validate");
...
test("AgentTeamService.buildPlan assigns release lane", () => {
    ...
    const plan = service.buildPlan(input);
    const releaseLane = plan.lanes.find((l) => l.stage === "release");
...
test("AgentTeamStage type accepts all valid values", () => {
    const stages = ["plan", "build", "review", "validate", "repair", "release"];
...
//# sourceMappingURL=agent-team-service.test.js.map