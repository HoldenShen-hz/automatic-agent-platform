...
class CollaborationModeService {
    ...
    getCurrentPipelineStage(collaborationId) {
        const state = this.pipelines.get(collaborationId);
        if (!state)
            return null;
        return state.stages[state.currentStageIndex] ?? null;
    }
    advancePipelineStage(collaborationId, output) {
        const state = this.pipelines.get(collaborationId);
        if (!state)
            throw new Error("not_found: Pipeline collaboration not found");
        const stage = state.stages[state.currentStageIndex];
        if (!stage)
            throw new Error("stage_not_found: Current stage not found");
    ...
    isPipelineComplete(collaborationId) {
        const state = this.pipelines.get(collaborationId);
        if (!state)
            return false;
        return state.currentStageIndex >= state.stages.length;
    }
    getPipelineState(collaborationId) {
        return this.pipelines.get(collaborationId) ?? null;
    }
    initiateNegotiation(parent, spec) {
        ...
        const state = {
            ...
            selectionPolicy: spec.negotiationSelectionPolicy ?? "highest_confidence",
            proposals: [],
            rounds: [],
        };
    ...
    getNegotiationState(collaborationId) {
        return this.negotiations.get(collaborationId) ?? null;
    }
    submitProposal(collaborationId, agentId, agentType, payload, confidence, reasoning) {
        const state = this.negotiations.get(collaborationId);
        if (!state)
            throw new Error("not_found: Negotiation collaboration not found");
    ...
    getCurrentProposals(collaborationId) {
        const state = this.negotiations.get(collaborationId);
        return state?.proposals ?? [];
    }
    advanceNegotiationRound(collaborationId) {
        const state = this.negotiations.get(collaborationId);
        if (!state)
            throw new Error("not_found: Negotiation collaboration not found");
    ...
    isNegotiationComplete(collaborationId) {
        const state = this.negotiations.get(collaborationId);
        if (!state)
            return false;
        return state.selectedProposalId !== undefined || state.currentRound > state.maxRounds;
    }
    selectProposal(collaborationId, proposalId) {
        const state = this.negotiations.get(collaborationId);
        if (!state)
            throw new Error("not_found: Negotiation collaboration not found");
        const proposal = state.proposals.find((p) => p.proposalId === proposalId);
        if (!proposal)
            throw new Error("proposal_not_found: Proposal not found");
        state.selectedProposalId = proposalId;
        return proposal;
    }
    getWinningProposal(collaborationId) {
        const state = this.negotiations.get(collaborationId);
        if (!state || state.proposals.length === 0)
            return null;
        return state.proposals.reduce((best, current) => current.confidence > best.confidence ? current : best);
    }
    completeCollaboration(collaborationId, mode, outputs, errors) {
        const result = {
    ...
    getCollaborationResult(collaborationId) {
...
function createCollaborationModeService() {
    return new CollaborationModeService();
}
// ─────────────────────────────────────────────────────────────────────────────
// Test Fixtures
// ─────────────────────────────────────────────────────────────────────────────
function createTestAgentContext(overrides = {}) {
    return {
...
function createNegotiationSpec() {
    return {
...
test("CollaborationModeService rejects pipeline with no stages", () => {
    ...
    assert.throws(() => service.initiatePipeline(parent, spec), { message: /pipeline_no_stages/ });
});
...
test("CollaborationModeService throws for invalid pipeline collaboration", () => {
    const service = createCollaborationModeService();
    assert.throws(() => service.advancePipelineStage("non-existent", { result: null }), { message: /not_found/ });
});
...
test("CollaborationModeService submits proposals", () => {
    ...
    const spec = createNegotiationSpec();
    const collaborationId = service.initiateNegotiation(parent, spec);
    const proposalId1 = service.submitProposal(collaborationId, "agent-1", "worker", { answer: "option-a" }, 0.8, "Based on analysis");
    const proposalId2 = service.submitProposal(collaborationId, "agent-2", "worker", { answer: "option-b" }, 0.9);
...
test("CollaborationModeService rejects proposal for non-existent collaboration", () => {
    const service = createCollaborationModeService();
    assert.throws(() => service.submitProposal("non-existent", "agent-1", "worker", {}, 0.5), { message: /not_found/ });
});
test("CollaborationModeService rejects invalid proposal selection", () => {
    ...
    const spec = createNegotiationSpec();
    const collaborationId = service.initiateNegotiation(parent, spec);
    assert.throws(() => service.selectProposal(collaborationId, "non-existent"), { message: /proposal_not_found/ });
});
...
test("CollaborationModeService completes collaboration", () => {
    ...
    const result = service.completeCollaboration(collaborationId, "pipeline", [{ result: "1" }, { result: "2" }, { result: "3" }], []);
...
test("CollaborationModeService marks failed collaboration", () => {
    ...
    service.advancePipelineStage(collaborationId, { result: "1" });
    service.advancePipelineStage(collaborationId, { result: "2" });
    const result = service.completeCollaboration(collaborationId, "pipeline", [{ result: "1" }], ["Stage 3 failed: timeout"]);
...
//# sourceMappingURL=collaboration-modes.test.js.map