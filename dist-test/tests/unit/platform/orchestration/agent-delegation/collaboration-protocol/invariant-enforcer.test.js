...
import { ACPInvariantEnforcer, CollaborationProtocolService } from "../../../../../../src/platform/orchestration/agent-delegation/collaboration-protocol/index.js";
...
test("ACPInvariantEnforcer reports violations when invariants are broken", () => {
    const enforcer = new ACPInvariantEnforcer();
    const result = enforcer.enforceAll(createMessage({
        depth: 9,
        risk_level: 99,
...
test("CollaborationProtocolService validates completion evidence and takeover audit", () => {
    ...
    const completion = protocol.validateAndSend(createMessage({
        messageType: "completion_report",
        payload: { evidence: ["artifact:1"], result_summary: "done", artifacts: ["artifact:1"] },
    }), context);
    const takeover = protocol.handleIncoming(createMessage({
...
//# sourceMappingURL=invariant-enforcer.test.js.map