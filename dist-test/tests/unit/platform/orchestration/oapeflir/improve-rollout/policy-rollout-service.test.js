...
function withFrozenRollouts(fn) {
...
function createMinimalCandidate(overrides = {}) {
    return {
...
function createStrategyVersion(overrides = {}) {
    return {
...
function createRolloutRecord(overrides = {}) {
    return {
...
test("promote throws when metrics gate blocks without rollback", () => {
    ...
    assert.throws(() => service.promote(candidate, record, "partial_25"), /rollout.metrics_required/);
});
...
test("inferLevelFromStatus maps draft/rejected/rolled_back/paused to off", () => {
    ...
    const terminalStatuses = ["draft", "rejected", "rolled_back", "paused"];
...
test("inferLevelFromStatus maps progressive statuses correctly", () => {
    ...
    for (const transition of progressiveTransitions) {
        ...
        assert.equal(result.level, transition.expectedLevel, `${transition.from} -> ${transition.to} should yield level ${transition.expectedLevel}`);
    }
});
...
test("inferLevelFromStatus maps rejected/rolled_back/paused to off via self-transition", () => {
    const service = new PolicyRolloutService();
    const candidate = createMinimalCandidate();
    const terminalStatuses = ["rejected", "rolled_back", "paused"];
    for (const status of terminalStatuses) {
        const record = createRolloutRecord({ status });
...
//# sourceMappingURL=policy-rollout-service.test.js.map