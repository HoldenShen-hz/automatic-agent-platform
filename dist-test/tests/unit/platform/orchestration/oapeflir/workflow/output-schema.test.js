...
import { parseWorkflowOutputSchema, validateWorkflowStepOutput, } from "../../../../../../src/platform/orchestration/oapeflir/workflow/output-schema.js";
test("parseWorkflowOutputSchema parses valid schema", () => {
    const schema = parseWorkflowOutputSchema(JSON.stringify({
        type: "object",
        required: ["key1"],
        properties: {
    ...
    assert.equal(schema.properties["key1"]?.minLength, 1);
    assert.equal(schema.additionalProperties, false);
});
test("parseWorkflowOutputSchema throws for invalid JSON", () => {
    assert.throws(() => parseWorkflowOutputSchema("not json", "test://schema/1"), (e) => e.code === "workflow.output_schema_parse_error");
});
test("parseWorkflowOutputSchema throws for non-object root", () => {
    assert.throws(() => parseWorkflowOutputSchema('"string"', "test://schema/1"), (e) => e.code === "workflow.output_schema_invalid_document");
});
test("parseWorkflowOutputSchema throws for invalid property type", () => {
    assert.throws(() => parseWorkflowOutputSchema(JSON.stringify({
        type: "object",
        properties: {
            key1: { type: "number" }, // not supported
        },
    }), "test://schema/1"), (e) => e.code === "workflow.output_schema_unsupported");
});
test("parseWorkflowOutputSchema throws for invalid minLength", () => {
    ...
    }), "test://schema/1"), (e) => e.code === "workflow.output_schema_invalid_document");
});
...
test("parseWorkflowOutputSchema defaults minLength to 0", () => {
    ...
    assert.equal(schema.properties["key1"]?.minLength, 0);
});
...
test("validateWorkflowStepOutput throws for missing output schema path", () => {
    assert.throws(() => validateWorkflowStepOutput({ stepId: "step_1", outputSchemaPath: null }, { key: "value" }), (e) => e.code === "workflow.output_schema_missing");
});
...
//# sourceMappingURL=output-schema.test.js.map