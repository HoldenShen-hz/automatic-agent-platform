...
function createValidWorkflow(steps) {
    return {
...
test("WorkflowValidator detects empty workflow", () => {
    ...
    assert.ok(report.issues.some((i) => i.code === "workflow.empty"));
});
test("WorkflowValidator detects missing step ID", () => {
    ...
    const report = validator.validate(workflow);
    assert.equal(report.ok, false);
    assert.ok(report.issues.some((i) => i.code === "step.missing_id"));
});
...
test("WorkflowValidator detects missing output key", () => {
    ...
    const report = validator.validate(workflow);
    assert.equal(report.ok, false);
    assert.ok(report.issues.some((i) => i.code === "step.missing_output_key"));
});
test("WorkflowValidator detects missing output schema", () => {
    ...
    const report = validator.validate(workflow);
    assert.equal(report.ok, false);
    assert.ok(report.issues.some((i) => i.code === "step.missing_output_schema"));
});
...
test("WorkflowValidator detects missing input key dependency", () => {
    ...
    const report = validator.validate(workflow);
    assert.equal(report.ok, false);
    assert.ok(report.issues.some((i) => i.code === "dependency.missing_input_key"));
});
...
test("assertWorkflowValid throws for invalid workflow", () => {
    const workflow = createValidWorkflow([]);
    assert.throws(() => assertWorkflowValid(workflow), (error) => error.code === "workflow.invalid:workflow.empty");
});
