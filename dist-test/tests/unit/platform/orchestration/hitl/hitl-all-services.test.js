...
import { ApprovalContextSummaryService } from "../../../../../src/platform/orchestration/hitl/approval-context-summary-service.js";
...
function makePacketOption(overrides = {}) {
    return {
...
test("HitlOperatorConsoleService routing rules add channels by risk level", async () => {
    const service = new HitlOperatorConsoleService([{ channel: "slack", minRiskLevel: "high" }], async (input) => {
...
test("HitlOperatorConsoleService routing rules add channels by stage", async () => {
    const service = new HitlOperatorConsoleService([{ channel: "pager", minRiskLevel: "low", stages: ["execute"] }], async () => ({ delivered: true, deliveryId: "pager:id" }));
...
test("HitlOperatorConsoleService routing rules filter by tenant", async () => {
    const service = new HitlOperatorConsoleService([{ channel: "email", minRiskLevel: "low", tenantIds: ["tenant-special"] }], async () => ({ delivered: true, deliveryId: "email:id" }));
...
test("HitlInboxService buildInbox marks decided when feedbackSignalId exists", () => {
    const service = new HitlInboxService();
    const items = service.buildInbox([makePacket({ approvalId: "in-decided" })], [{ approvalId: "in-decided", taskId: "task-1", stageRef: "plan", loopIteration: null, refId: null, feedbackSignalId: "sig-1", decisionEffect: "continue" }], "2026-04-22T12:00:00.000Z");
    assert.equal(items[0]?.status, "decided");
});
test("HitlInboxService buildInbox marks decided when decisionEffect is not continue", () => {
    const service = new HitlInboxService();
    const items = service.buildInbox([makePacket({ approvalId: "in-effect" })], [{ approvalId: "in-effect", taskId: "task-1", stageRef: "plan", loopIteration: null, refId: null, feedbackSignalId: null, decisionEffect: "revise_plan" }], "2026-04-22T12:00:00.000Z");
    assert.equal(items[0]?.status, "decided");
});
test("HitlInboxService buildInbox marks due_soon when within 15 minutes", () => {
    const service = new HitlInboxService();
    const items = service.buildInbox([makePacket({ approvalId: "in-due15", deadlineAt: "2026-04-22T12:05:00.000Z" })], [], "2026-04-22T12:00:00.000Z");
...
test("HITLExplainabilityService isFeedbackDue returns false when feedback already submitted", () => {
    ...
    const session = { id: "session-existing", closedAt: new Date(Date.now() - 200).toISOString() };
...
test("HITLExplainabilityService isFeedbackDue returns false when feature disabled", () => {
    ...
    const session = { id: "session-off", closedAt: new Date(Date.now() - 10000).toISOString() };
    assert.equal(service.isFeedbackDue(session), false);
});
test("HITLExplainabilityService isFeedbackDue returns false when session not closed", () => {
    const service = new HITLExplainabilityService(mockStore, { feedbackReminderAfterMs: 100 });
    const session = { id: "session-open", closedAt: null };
    assert.equal(service.isFeedbackDue(session), false);
});
test("HITLExplainabilityService isFeedbackDue respects feedbackReminderAfterMs", () => {
    ...
    const session = { id: "session-recent", closedAt: new Date(Date.now() - 1000).toISOString() };
    assert.equal(service.isFeedbackDue(session), false);
});
test("HITLExplainabilityService getSessionFeedbackDue delegates to isFeedbackDue", () => {
    ...
    const session = { id: "session-delegate", closedAt: new Date(Date.now() - 2000).toISOString() };
...
test("HitlApprovalOrchestrationService getPacket returns stored packet", async () => {
    const service = new HitlApprovalOrchestrationService(createMockApprovalService(), createMockExplainabilityService());
...
test("HitlApprovalOrchestrationService applyDecision rejects unknown approval", async () => {
    const service = new HitlApprovalOrchestrationService({}, createMockExplainabilityService());
    assert.throws(() => {
...
test("ApprovalContextSummaryService parseSummaryFromResponse extracts JSON from mixed content", async () => {
    ...
    const service = new ApprovalContextSummaryService({ provider: provider });
    const context = {
        taskId: "task-parse",
        stageRef: "plan",
        riskLevel: "medium",
...
test("ApprovalContextSummaryService defaultKeyPoints uses stage name", async () => {
    ...
    assert.ok(result.keyPoints.some((p) => p.includes("execute")));
});
test("ApprovalContextSummaryService defaultRiskFactors for high risk", async () => {
    ...
    assert.ok(result.riskFactors.some((r) => r.includes("high") || r.includes("risk")));
});
...
//# sourceMappingURL=hitl-all-services.test.js.map