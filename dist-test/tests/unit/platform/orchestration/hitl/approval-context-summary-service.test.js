...
test("ApprovalContextSummaryService generates summary for basic context", async () => {
    ...
    const service = new ApprovalContextSummaryService({ provider: mockProvider });
    const context = {
        taskId: "task_1",
        executionId: "exec_1",
...
test("ApprovalContextSummaryService template summary for error context", async () => {
    ...
    assert.ok(result.keyPoints.some((p) => p.includes("error") || p.includes("5")));
});
test("ApprovalContextSummaryService template summary for blocked context", async () => {
    ...
    const result = await service.generateSummary(context);
    assert.ok(result.summary.includes("blocked") || result.keyPoints.some((p) => p.includes("blocker")));
});
test("ApprovalContextSummaryService handles context with completed steps", async () => {
    ...
    const result = await service.generateSummary(context);
    assert.ok(result.keyPoints.some((p) => p.includes("3 step") || p.includes("execute")));
});
...
//# sourceMappingURL=approval-context-summary-service.test.js.map