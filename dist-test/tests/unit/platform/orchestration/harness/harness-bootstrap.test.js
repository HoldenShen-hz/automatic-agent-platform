...
import { buildHarnessBootstrap, HARNESS_BOOTSTRAP_SERVICE_ID, HARNESS_CATALOG_SERVICE_ID, registerHarnessBootstrap, } from "../../../../../src/platform/orchestration/harness/harness-bootstrap.js";
...
test("registerHarnessBootstrap registers catalog service before bootstrap service", async () => {
    ...
    try {
        const bootstrap = registerHarnessBootstrap(registry);
        const catalogService = registry.get(HARNESS_CATALOG_SERVICE_ID);
...
test("bootstrap service depends on catalog service", async () => {
    ...
    try {
        ...
        const bootstrap = registry.get(HARNESS_BOOTSTRAP_SERVICE_ID);
        assert.ok(bootstrap);
        const catalog = registry.get(HARNESS_CATALOG_SERVICE_ID);
...
//# sourceMappingURL=harness-bootstrap.test.js.map