...
function createLoopInput(overrides = {}) {
    return {
        taskId: "task-1",
        domainId: "coding",
...
test("AsyncHarnessService.execute throws for unknown runId", async () => {
    ...
    const service = new AsyncHarnessService(runtime);
    const runId = await service.createRun(createLoopInput());
    await assert.rejects(async () => service.execute("non-existent-run-id"), /harness\.async\.run_not_found/);
});
...
test("AsyncHarnessService.execute throws for truly nonexistent runId", async () => {
    const runtime = new HarnessRuntimeService();
    const service = new AsyncHarnessService(runtime);
    await assert.rejects(async () => service.execute("truly-nonexistent-run"), /harness\.async\.run_not_found/);
});
...
//# sourceMappingURL=async-harness-service.test.js.map