...
test("HarnessLoopController constructor applies overrides", () => {
    ...
    const controller = new HarnessLoopController(pack, { maxIterations: 5, maxReplans: 2, maxCost: 50, maxDurationMs: 5000 });
    const guards = controller.getGuards();
...
test("HarnessLoopController constructor respects initialState", () => {
    const pack = createMockConstraintPack({ maxSteps: 30 });
    const controller = new HarnessLoopController(pack, {}, { iteration: 5, replanCount: 2, totalCost: 25, startedAt: 1000000 });
