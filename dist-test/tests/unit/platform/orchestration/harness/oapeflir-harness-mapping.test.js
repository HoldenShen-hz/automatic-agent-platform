...
import { mapHarnessStepToOapeflirPhase, } from "../../../../../src/platform/orchestration/harness/oapeflir-harness-mapping.js";
...
test("mapHarnessStepToOapeflirPhase handles edge cases", () => {
    ...
    const result = mapHarnessStepToOapeflirPhase("loop_controller", "unknown_stage");
...
test("mapHarnessStepToOapeflirPhase returns observe for unrecognized roles", () => {
    assert.equal(mapHarnessStepToOapeflirPhase("unrecognized_role", "any"), "observe");
    assert.equal(mapHarnessStepToOapeflirPhase("random_role", "some_stage"), "observe");
});
...
test("mapHarnessStepToOapeflirPhase stage parameter affects priority role matching", () => {
    ...
    assert.equal(mapHarnessStepToOapeflirPhase("any_role", "plan"), "plan");
    // When stage is "execute", returns "execute" (second condition matches)
    assert.equal(mapHarnessStepToOapeflirPhase("any_role", "execute"), "execute");
    // When stage is "evaluate", returns "feedback" (third condition matches)
    assert.equal(mapHarnessStepToOapeflirPhase("any_role", "evaluate"), "feedback");
});
test("mapHarnessStepToOapeflirPhase returns all valid OapeflirSemanticPhase values", () => {
    const allPhases = new Set();
    const roles = ["planner", "generator", "evaluator", "hitl_operator", "loop_controller", "unknown"];
...
test("mapHarnessStepToOapeflirPhase handles empty string stage", () => {
    ...
    assert.equal(mapHarnessStepToOapeflirPhase("unknown", ""), "observe");
});
...
test("OapeflirSemanticPhase type contains all expected phase values", () => {
    const phases = ["observe", "assess", "plan", "execute", "feedback", "learn", "improve", "release"];
    for (const phase of phases) {
        const result = mapHarnessStepToOapeflirPhase("unknown", "any");
        // Just ensure no error is thrown when using valid phase values
        assert.ok(typeof result === "string");
    }
});
//# sourceMappingURL=oapeflir-harness-mapping.test.js.map