...
import { createTopologyValidator, DEFAULT_MAX_DEPTH, DEFAULT_MAX_FANOUT, DelegationDepthExceededError, DelegationFanoutExceededError, DelegationCycleDetectedError, } from "../../../src/platform/agent-delegation/index.js";
import { ContextIsolator, createContextIsolator, IsolationLevel, } from "../../../src/platform/agent-delegation/index.js";
import { DelegationTracker, createDelegationTracker, } from "../../../src/platform/agent-delegation/index.js";
import { DelegationManagerService, createDelegationManager, } from "../../../src/platform/agent-delegation/index.js";
// ─────────────────────────────────────────────────────────────────────────────
...
test("TopologyValidator.validateDepth throws when at max depth", () => {
    ...
    assert.throws(() => validator.validateDepth(3), DelegationDepthExceededError);
});
...
test("TopologyValidator.validateFanout throws when at max fanout", () => {
    ...
    assert.throws(() => validator.validateFanout(10), DelegationFanoutExceededError);
});
...
test("TopologyValidator.detectCycle throws when packId in chain", () => {
    const validator = createTopologyValidator();
    assert.throws(() => validator.detectCycle("pack-1", ["pack-2", "pack-1", "pack-3"]), DelegationCycleDetectedError);
});
...
test("TopologyValidator.validatePackId throws when packId not allowed", () => {
    ...
    assert.throws(() => validator.validatePackId("forbidden-pack"));
});
...
test("TopologyValidator.validate runs all checks", () => {
    ...
    assert.throws(() => validator.validate({ currentDepth: 3, activeDelegations: 3, targetPackId: "allowed", delegationChain: [] }), DelegationDepthExceededError);
});
...
test("ContextIsolator.mergePermissions with empty override keeps base", () => {
    const isolator = createContextIsolator();
    const base = {
        resources: ["resource-1"],
        actions: ["action-1"],
...
test("DelegationManagerService.getDelegationChain returns chain", async () => {
    ...
    await manager.delegate(parent, spec);
    const chain = manager.getDelegationChain("parent-1");
    assert.ok(chain !== null);
    assert.equal(chain.rootAgentId, "parent-1");
    assert.equal(chain.totalDelegations, 1);
});
test("DelegationManagerService.revokeExpiredDelegations returns scan result", () => {
    const manager = createDelegationManager();
    const result = manager.revokeExpiredDelegations();
    assert.equal(typeof result.scanned, "number");
    assert.equal(typeof result.expired, "number");
    assert.ok(Array.isArray(result.errors));
});
test("DelegationManagerService.getExpiredDelegations returns list", () => {
    const manager = createDelegationManager();
    const expired = manager.getExpiredDelegations();
    assert.ok(Array.isArray(expired));
});
test("DelegationManagerService.getPendingExpirationCount returns number", () => {
    const manager = createDelegationManager();
    const count = manager.getPendingExpirationCount();
...
//# sourceMappingURL=agent-delegation.test.js.map