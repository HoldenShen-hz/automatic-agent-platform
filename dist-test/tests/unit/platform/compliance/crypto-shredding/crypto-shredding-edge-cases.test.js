...
describe("DekStore additional coverage", () => {
    ...
    describe("markRotated edge cases", () => {
        it("should throw when DEK not found", async () => {
            await assert.rejects(async () => store.markRotated("non-existent-dek", "replacement-dek"), (error) => {
                if (error instanceof Error && "code" in error) {
                    return error.code === "dek.not_found";
                }
                return false;
            });
        });
        it("should throw when DEK is not active", async () => {
            ...
            await assert.rejects(async () => store.markRotated(created.metadata.dekId, "another-replacement"), (error) => {
                if (error instanceof Error && "code" in error) {
                    return error.code === "dek.not_active";
                }
                return false;
            });
        });
    ...
    describe("destroy edge cases", () => {
        it("should throw when DEK not found", async () => {
            await assert.rejects(async () => store.destroy("non-existent-dek"), (error) => {
                if (error instanceof Error && "code" in error) {
                    return error.code === "dek.not_found";
                }
                return false;
...
describe("CryptoShreddingService encryptRecordForSubject edge cases", () => {
    ...
    it("should handle nested field paths with dot notation", async () => {
        ...
        const nestedRecord = result.encryptedRecord;
    ...
    it("should handle array index field paths", async () => {
        ...
        const result = await service.encryptRecordForSubject("user-123", record);
        const arrayRecord = result.encryptedRecord;
...
describe("CryptoShreddingService decryptField edge cases", () => {
    ...
    it("should throw for non-string encrypted value", async () => {
        await manager.createForSubject("user-123");
        await assert.rejects(async () => service.decryptField("dek-id", "name", { name: 123 }), (error) => {
            if (error instanceof Error && "code" in error) {
                return error.code === "decrypt.invalid_value";
            }
            return false;
        });
    });
...
describe("DekManager decrypt edge cases", () => {
    ...
    it("should throw for non-existent DEK", async () => {
        await assert.rejects(async () => manager.decrypt("non-existent-dek", "some:ciphertext"), (error) => {
            if (error instanceof Error && "code" in error) {
                return error.code === "dek.not_found";
            }
            return false;
    ...
    it("should throw for invalid ciphertext format", async () => {
        ...
        await assert.rejects(async () => manager.decrypt(created.metadata.dekId, "invalid-format"), (error) => {
            if (error instanceof Error && "code" in error) {
                return error.code === "dek.invalid_ciphertext";
            }
            return false;
        });
    });
...
describe("InMemoryShredAuditTrail interface compliance", () => {
    ...
    it("should handle concurrent record operations", async () => {
        const trail = new InMemoryShredAuditTrail();
        const promises = Array.from({ length: 10 }, (_, i) => trail.record({
            shredId: `shred-${i}`,
            subjectId: `subject-${i}`,
...
//# sourceMappingURL=crypto-shredding-edge-cases.test.js.map