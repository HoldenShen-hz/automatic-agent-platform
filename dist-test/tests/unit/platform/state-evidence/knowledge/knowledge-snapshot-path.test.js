...
test("[SYS-SEC-4.2] KnowledgeSnapshotStore rejects path traversal attempts", () => {
    assert.throws(() => new KnowledgeSnapshotStore({ snapshotPath: "../../etc/passwd" }), /sandbox|path|denied/i, "Should reject paths outside sandbox root");
});
test("[SYS-SEC-4.2] KnowledgeSnapshotStore rejects absolute paths outside workspace", () => {
    assert.throws(() => new KnowledgeSnapshotStore({ snapshotPath: "/etc/shadow" }), /sandbox|path|denied/i, "Should reject absolute paths outside sandbox");
});
