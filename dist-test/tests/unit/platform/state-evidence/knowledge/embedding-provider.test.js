...
test("HashEmbeddingProvider produces 32-dim normalized vectors", async () => {
    ...
    assert.ok(result.vector.some((v, i) => v !== result3.vector[i]), "different text should produce different vector");
});
test("HashEmbeddingProvider.embedBatch processes multiple texts", async () => {
    ...
    assert.ok(results[0].vector.some((v, i) => v !== results[1].vector[i]), "batch results should be distinct");
});
...
test("OpenAIEmbeddingProvider produces 1536-dim vectors (mock test - no real API call)", async () => {
    ...
    await assert.rejects(() => provider.embed("test"), /fetch|network|failed/, "should fail without valid endpoint");
});
...
//# sourceMappingURL=embedding-provider.test.js.map