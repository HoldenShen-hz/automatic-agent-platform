...
test("NamespacePolicyStore rejects invalid namespace path", () => {
    ...
    assert.throws(() => store.register(invalidNamespace), /Invalid namespace path format/);
});
test("NamespacePolicyStore rejects empty path", () => {
    const store = new NamespacePolicyStore();
    const emptyPathNamespace = createNamespace({ path: "" });
    assert.throws(() => store.register(emptyPathNamespace), /Namespace path must be a non-empty string/);
});
...
test("Validates namespace with all trust levels", () => {
    const store = new NamespacePolicyStore();
    const trustLevels = ["verified", "reviewed", "community", "unverified"];
...
test("Path validation requires lowercase", () => {
    ...
    assert.throws(() => store.register(upperCase), /Invalid namespace path format/);
});
test("Path validation requires alphanumeric with dots", () => {
    const store = new NamespacePolicyStore();
    assert.throws(() => store.register(createNamespace({ path: "has_underscore.path" })), /Invalid namespace path format/);
    assert.throws(() => store.register(createNamespace({ path: "has-special.chars" })), /Invalid namespace path format/);
});
...
//# sourceMappingURL=namespace-strategy.test.js.map