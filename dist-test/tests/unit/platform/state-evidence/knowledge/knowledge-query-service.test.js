...
test("hits are actually truncated when snippet exceeds max chars", () => {
    ...
    const service = new KnowledgeQueryService(mockRetrievalService, {
...
test("getLastConfidence returns 0 for empty results", () => {
    ...
    const service = new KnowledgeQueryService(emptyRetrievalService);
...
test("expandWithGraphTraversal returns original hits when no connections found", async () => {
    ...
    const service = new KnowledgeQueryService(createMockRetrievalService(), {}, undefined, mockGraph);
    const hits = await service.queryAsync("test", {}, QueryLevel.Deep);
    assert.ok(Array.isArray(hits));
});
// =============================================================================
...
//# sourceMappingURL=knowledge-query-service.test.js.map