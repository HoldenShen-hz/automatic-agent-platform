...
test("EventTopologyService buildGraph creates nodes for all producers and consumers", () => {
    ...
    const consumerNodes = graph.nodes.filter((n) => n.kind === "consumer");
    ...
    const emitEdges = graph.edges.filter((e) => e.relation === "emits");
    const consumeEdges = graph.edges.filter((e) => e.relation === "consumes");
...
test("EventTopologyService listNamespaceEntries filters correctly", () => {
    ...
    assert.ok(taskEntries.every((e) => e.namespace === "task"));
    const dispatchEntries = service.listNamespaceEntries("dispatch");
    assert.ok(dispatchEntries.length > 0);
    assert.ok(dispatchEntries.every((e) => e.namespace === "dispatch"));
...
test("EventReliabilityInventoryService marks contract gaps for missing consumers", () => {
    ...
    const gateway = surfaces.find((s) => s.consumerId === "gateway_projection");
    if (gateway) {
...
test("ProjectionInventoryService includes all baseline projections", () => {
    ...
    for (const expected of expectedProjections) {
        assert.ok(records.some((r) => r.projectionName === expected), `Missing projection: ${expected}`);
    }
});
...
test("TypedEventBusPublisher delegates publish to underlying bus", () => {
    ...
    assert.deepEqual(receivedInput.payload, { fromStatus: "queued", toStatus: "in_progress" });
});
test("TypedEventBusPublisher passes trace context correctly", () => {
    let receivedTraceId = undefined;
    let receivedTraceContext = undefined;
    const mockBus = {
        publish: ((input) => {
...
test("EVENT_SCHEMA_REGISTRY includes delegation namespace events when registered", () => {
    ...
    for (const eventType of knownTier1Events) {
        const schema = EVENT_SCHEMA_REGISTRY[eventType];
        if (schema) {
...
test("validateEventPayload rejects subtask event missing both stepId and subtaskId", () => {
    ...
    assert.throws(() => validateEventPayload("subtask:completed", { roleId: "agent", status: "completed" }), /Invalid payload for event type: subtask:completed/);
});
