...
import { approvalQueueProjectionHandler, createEmptyApprovalQueueState, createApprovalQueueProjectionHandler, } from "../../../../../../src/platform/state-evidence/events/projections/approval-queue-projection.js";
...
test("approvalQueueProjectionHandler handles decision:requested", () => {
    ...
    const state = approvalQueueProjectionHandler(null, event);
...
test("approvalQueueProjectionHandler is idempotent - same event applied twice", () => {
    ...
    const state1 = approvalQueueProjectionHandler(null, event);
    const state2 = approvalQueueProjectionHandler(state1, event);
...
test("createApprovalQueueProjectionHandler returns handler function", () => {
    ...
    assert.equal(state.approvalId, "test_handler");
});
//# sourceMappingURL=approval-queue-projection.test.js.map