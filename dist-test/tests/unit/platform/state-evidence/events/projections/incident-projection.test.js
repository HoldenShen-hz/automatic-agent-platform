...
test("incidentProjectionHandler handles incident:investigating without rootCause", () => {
    const event = makeEvent("evt_inv_no_cause", "incident:investigating", null, JSON.stringify({}));
    const state = incidentProjectionHandler(null, event);
    assert.equal(state.status, "investigating");
    assert.equal(state.rootCause, null);
});
...
test("incidentProjectionHandler handles compliance:violation_detected without resourceId", () => {
    const event = makeEvent("evt_compliance_no_res", "compliance:violation_detected", null, JSON.stringify({ violationId: "viol_003", severity: "critical" }));
    const state = incidentProjectionHandler(null, event);
...
test("incidentProjectionHandler handles compliance:violation_detected when incidentId already set", () => {
    const event1 = makeEvent("evt_compliance_1", "incident:created", null, JSON.stringify({ incidentId: "inc_001", severity: "high" }), "2026-04-19T10:00:00.000Z");
    const event2 = makeEvent("evt_compliance_2", "compliance:violation_detected", null, JSON.stringify({ violationId: "viol_new", severity: "critical" }), "2026-04-19T10:01:00.000Z");
    const state1 = incidentProjectionHandler(null, event1);
    const state2 = incidentProjectionHandler(state1, event2);
...
test("incidentProjectionHandler handles compliance:violation_detected when severity already set", () => {
    const event1 = makeEvent("evt_compliance_sev_1", "incident:created", null, JSON.stringify({ severity: "low" }), "2026-04-19T10:00:00.000Z");
    const event2 = makeEvent("evt_compliance_sev_2", "compliance:violation_detected", null, JSON.stringify({ violationId: "viol_sev", severity: "critical" }), "2026-04-19T10:01:00.000Z");
    const state1 = incidentProjectionHandler(null, event1);
    const state2 = incidentProjectionHandler(state1, event2);
...
test("incidentProjectionHandler handles slo:breached when incidentId already set", () => {
    const event1 = makeEvent("evt_slo_id_1", "incident:created", null, JSON.stringify({ incidentId: "inc_001" }), "2026-04-19T10:00:00.000Z");
    const event2 = makeEvent("evt_slo_id_2", "slo:breached", null, JSON.stringify({ sloId: "slo_new" }), "2026-04-19T10:01:00.000Z");
    const state1 = incidentProjectionHandler(null, event1);
    const state2 = incidentProjectionHandler(state1, event2);
    // incidentId should not be overwritten
    assert.equal(state2.incidentId, "inc_001");
...
test("incidentProjectionHandler handles slo:breached when severity already set", () => {
    const event1 = makeEvent("evt_slo_sev_1", "incident:created", null, JSON.stringify({ severity: "critical" }), "2026-04-19T10:00:00.000Z");
    const event2 = makeEvent("evt_slo_sev_2", "slo:breached", null, JSON.stringify({ sloId: "slo_sev" }), "2026-04-19T10:01:00.000Z");
    const state1 = incidentProjectionHandler(null, event1);
    const state2 = incidentProjectionHandler(state1, event2);
    // severity should not be overwritten
...
test("incidentProjectionHandler handles slo:breached with partial metric info", () => {
    ...
    const event = makeEvent("evt_slo_partial", "slo:breached", null, JSON.stringify(payload));
    const state = incidentProjectionHandler(null, event);
    assert.match(state.description, /SLO breach/);
...
test("incidentProjectionHandler does not add duplicate entity on second event", () => {
    const event1 = makeEvent("evt_dup_1", "incident:created", null, JSON.stringify({ incidentId: "inc_dup", affectedWorkflows: ["wf_unique"] }));
    const event2 = makeEvent("evt_dup_2", "incident:acknowledged", null, JSON.stringify({ affectedWorkflows: ["wf_unique"] }));
    const state1 = incidentProjectionHandler(null, event1);
    const state2 = incidentProjectionHandler(state1, event2);
...
test("incidentProjectionHandler handles array with mixed types in payload", () => {
    ...
    const event = makeEvent("evt_mixed", "incident:created", null, JSON.stringify(payload));
    const state = incidentProjectionHandler(null, event);
...
test("incidentProjectionHandler handles non-array value for affected entities", () => {
    ...
    const event = makeEvent("evt_non_array", "incident:created", null, JSON.stringify(payload));
    const state = incidentProjectionHandler(null, event);
...
test("incidentProjectionHandler timeline preserves details null when actionDetails missing", () => {
    ...
    const event = makeEvent("evt_tl_no_details", "incident:investigating", null, JSON.stringify(payload));
    const state = incidentProjectionHandler(null, event);
    assert.equal(state.timeline[0].details, null);
});
test("incidentProjectionHandler timeline handles null actorId", () => {
    const event = makeEvent("evt_tl_null_actor", "incident:created", null, JSON.stringify({ incidentId: "inc_tl_null" }));
    const state = incidentProjectionHandler(null, event);
...
test("incidentProjectionHandler handles compliance violation then resolve flow", () => {
    const events = [
        makeEvent("evt_comp_1", "compliance:violation_detected", null, JSON.stringify({ violationId: "viol_flow", severity: "high", framework: "SOC2" }), "2026-04-19T11:00:00.000Z"),
        makeEvent("evt_comp_2", "incident:acknowledged", null, JSON.stringify({}), "2026-04-19T11:05:00.000Z"),
        makeEvent("evt_comp_3", "incident:resolved", null, JSON.stringify({ rootCause: "Misconfigured access policy" }), "2026-04-19T11:30:00.000Z"),
    ];
    let state = null;
    for (const event of events) {
        state = incidentProjectionHandler(state, event);
...
test("incidentProjectionHandler handles empty JSON object payload", () => {
    const event = makeEvent("evt_empty_obj", "incident:created", null, JSON.stringify({}));
    const state = incidentProjectionHandler(null, event);
    assert.equal(state.incidentId, null);
    assert.equal(state.severity, null);
    assert.equal(state.description, null);
...
test("incidentProjectionHandler does not overwrite existing detectedAt", () => {
    const event1 = makeEvent("evt_detected_1", "incident:created", null, JSON.stringify({}), "2026-04-19T08:00:00.000Z");
    const event2 = makeEvent("evt_detected_2", "compliance:violation_detected", null, JSON.stringify({ violationId: "viol_new" }), "2026-04-19T09:00:00.000Z");
    const state1 = incidentProjectionHandler(null, event1);
    const state2 = incidentProjectionHandler(state1, event2);
...
test("incidentProjectionHandler does not overwrite existing firstEventAt", () => {
    const event1 = makeEvent("evt_first_1", "incident:created", null, JSON.stringify({}), "2026-04-19T08:00:00.000Z");
    const event2 = makeEvent("evt_first_2", "incident:acknowledged", null, JSON.stringify({}), "2026-04-19T09:00:00.000Z");
    const state1 = incidentProjectionHandler(null, event1);
    const state2 = incidentProjectionHandler(state1, event2);
...
test("incidentProjectionHandler handles non-object JSON payload (string)", () => {
    const event = makeEvent("evt_str_payload", "incident:created", null, '"just a string"');
    const state = incidentProjectionHandler(null, event);
...
test("incidentProjectionHandler handles non-object JSON payload (array)", () => {
    const event = makeEvent("evt_arr_payload", "incident:created", null, '["array", "not", "object"]');
    const state = incidentProjectionHandler(null, event);
...
test("incidentProjectionHandler handles null JSON payload", () => {
    const event = makeEvent("evt_null_payload", "incident:created", null, "null");
    const state = incidentProjectionHandler(null, event);
...
test("incidentProjectionHandler does not duplicate resourceId in affectedExecutions", () => {
    const event1 = makeEvent("evt_res_dup_1", "incident:created", null, JSON.stringify({ incidentId: "inc_res", affectedExecutions: ["exec_123"] }), "2026-04-19T10:00:00.000Z");
    const event2 = makeEvent("evt_res_dup_2", "compliance:violation_detected", null, JSON.stringify({ violationId: "viol_res", resourceId: "exec_123" }), "2026-04-19T10:01:00.000Z");
    const state1 = incidentProjectionHandler(null, event1);
    const state2 = incidentProjectionHandler(state1, event2);
...
test("incidentProjectionHandler handles slo:breached with null sloName", () => {
    ...
    const event = makeEvent("evt_slo_null_name", "slo:breached", null, JSON.stringify(payload));
    const state = incidentProjectionHandler(null, event);
    assert.match(state.description, /SLO breach/);
    assert.match(state.description, /unknown/);
...
test("incidentProjectionHandler handles slo:breached with null metricName", () => {
    ...
    const event = makeEvent("evt_slo_null_metric", "slo:breached", null, JSON.stringify(payload));
    const state = incidentProjectionHandler(null, event);
    assert.match(state.description, /SLO breach/);
...
test("incidentProjectionHandler handles slo:breached with undefined values", () => {
    ...
    const event = makeEvent("evt_slo_undef", "slo:breached", null, JSON.stringify(payload));
    const state = incidentProjectionHandler(null, event);
    assert.match(state.description, /SLO breach/);
    assert.match(state.description, /unknown/);
});
test("incidentProjectionHandler handles slo:breached with string values for numeric fields", () => {
    ...
    const event = makeEvent("evt_slo_str_num", "slo:breached", null, JSON.stringify(payload));
    const state = incidentProjectionHandler(null, event);
    assert.match(state.description, /SLO breach/);
...
test("incidentProjectionHandler creates new array instances", () => {
    const event = makeEvent("evt_new_array", "incident:created", null, JSON.stringify({ incidentId: "inc_array", affectedWorkflows: ["wf_1"] }));
    const state = incidentProjectionHandler(null, event);
...
test("incidentProjectionHandler creates new timeline array", () => {
    const event = makeEvent("evt_new_timeline", "incident:created", null, JSON.stringify({ incidentId: "inc_timeline" }));
    const state = incidentProjectionHandler(null, event);
...
test("incidentProjectionHandler creates new processedEventIds array", () => {
    const event = makeEvent("evt_new_processed", "incident:created", null, JSON.stringify({}));
    const state = incidentProjectionHandler(null, event);
