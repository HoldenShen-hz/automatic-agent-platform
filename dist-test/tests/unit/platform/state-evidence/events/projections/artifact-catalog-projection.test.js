...
import { artifactCatalogProjectionHandler, createEmptyArtifactCatalogState, createArtifactCatalogProjectionHandler, } from "../../../../../../src/platform/state-evidence/events/projections/artifact-catalog-projection.js";
/**
...
test("artifactCatalogProjectionHandler initializes state correctly", () => {
    const event = makeEvent("evt_1", "artifact:created", "task_1", '{"artifactId":"artifact_1","artifactType":"file"}');
    const state = artifactCatalogProjectionHandler(null, event);
...
test("artifactCatalogProjectionHandler handles artifact:created", () => {
    const event = makeEvent("evt_create", "artifact:created", "task_1", '{"artifactId":"artifact_abc","artifactName":"output.txt","mimeType":"text/plain"}');
    const state = artifactCatalogProjectionHandler(null, event);
...
test("artifactCatalogProjectionHandler handles workflow:artifact_linked", () => {
    const event = makeEvent("evt_link", "workflow:artifact_linked", "task_1", '{"artifact_ref":"artifact_xyz","workflowRunId":"wf_run_1"}');
    const state = artifactCatalogProjectionHandler(null, event);
...
test("artifactCatalogProjectionHandler handles artifact:updated", () => {
    const event = makeEvent("evt_update", "artifact:updated", "task_1", '{"artifactId":"artifact_1","contentHash":"hash123","sizeBytes":1024}');
    const state = artifactCatalogProjectionHandler(null, event);
...
test("artifactCatalogProjectionHandler handles artifact:sealed", () => {
    const event = makeEvent("evt_seal", "artifact:sealed", "task_1", '{"artifactId":"artifact_1"}');
    const state = artifactCatalogProjectionHandler(null, event);
    assert.equal(state.status, "sealed");
});
test("artifactCatalogProjectionHandler handles artifact:deleted", () => {
    const event = makeEvent("evt_del", "artifact:deleted", "task_1", '{"artifactId":"artifact_1"}');
    const state = artifactCatalogProjectionHandler(null, event);
    assert.equal(state.status, "deleted");
});
test("artifactCatalogProjectionHandler handles artifact:archived", () => {
    const event = makeEvent("evt_arch", "artifact:archived", "task_1", '{"artifactId":"artifact_1"}');
    const state = artifactCatalogProjectionHandler(null, event);
    assert.equal(state.status, "archived");
});
test("artifactCatalogProjectionHandler adds unique references", () => {
    const event = makeEvent("evt_ref", "artifact:created", "task_1", '{"artifactId":"artifact_1","referenceType":"workflow","referenceId":"wf_123","referencePath":"/path/to/artifact"}');
    const state = artifactCatalogProjectionHandler(null, event);
...
test("artifactCatalogProjectionHandler does not duplicate references", () => {
    const event1 = makeEvent("evt_ref_1", "artifact:created", "task_1", '{"artifactId":"artifact_1","referenceType":"workflow","referenceId":"wf_123"}');
    const event2 = makeEvent("evt_ref_2", "artifact:updated", "task_1", '{"artifactId":"artifact_1","referenceType":"workflow","referenceId":"wf_123"}');
    const state1 = artifactCatalogProjectionHandler(null, event1);
    const state2 = artifactCatalogProjectionHandler(state1, event2);
...
test("artifactCatalogProjectionHandler is idempotent - same event twice", () => {
    const event = makeEvent("evt_idem_1", "artifact:created", "task_1", '{"artifactId":"artifact_1"}');
    const state1 = artifactCatalogProjectionHandler(null, event);
    const state2 = artifactCatalogProjectionHandler(state1, event);
...
test("artifactCatalogProjectionHandler accumulates multiple events", () => {
    const createEvent = makeEvent("evt_create", "artifact:created", "task_1", '{"artifactId":"artifact_1"}');
    const updateEvent = makeEvent("evt_update", "artifact:updated", "task_1", '{"artifactId":"artifact_1"}');
    const state1 = artifactCatalogProjectionHandler(null, createEvent);
    const state2 = artifactCatalogProjectionHandler(state1, updateEvent);
...
test("createArtifactCatalogProjectionHandler returns handler function", () => {
    ...
    const event = makeEvent("evt_1", "artifact:created", "task_1", '{"artifactId":"artifact_1"}');
    const state = factory(null, event);
    assert.equal(state.artifactId, "artifact_1");
});
test("artifactCatalogProjectionHandler parses payload with size field", () => {
    const event = makeEvent("evt_1", "artifact:created", "task_1", '{"artifactId":"artifact_1","size":2048}');
    const state = artifactCatalogProjectionHandler(null, event);
...
test("artifactCatalogProjectionHandler timeline tracks events in order", () => {
    const events = [
        makeEvent("evt_1", "artifact:created", "task_1", '{"artifactId":"artifact_1"}', "2026-04-19T10:00:00.000Z"),
        makeEvent("evt_2", "artifact:updated", "task_1", '{"artifactId":"artifact_1"}', "2026-04-19T10:01:00.000Z"),
        makeEvent("evt_3", "artifact:sealed", "task_1", '{"artifactId":"artifact_1"}', "2026-04-19T10:02:00.000Z"),
    ];
    let state = null;
    for (const evt of events) {
        state = artifactCatalogProjectionHandler(state, evt);
    }
...
test("artifactCatalogProjectionHandler handles invalid JSON payload gracefully", () => {
    const event = makeEvent("evt_1", "artifact:created", "task_1", "not valid json");
    const state = artifactCatalogProjectionHandler(null, event);
...
test("artifactCatalogProjectionHandler skips traceContext in details", () => {
    const event = makeEvent("evt_1", "artifact:created", "task_1", '{"artifactId":"artifact_1","traceContext":{"traceId":"abc"}}');
    const state = artifactCatalogProjectionHandler(null, event);
...
test("artifactCatalogProjectionHandler extracts actorId from payload", () => {
    const event = makeEvent("evt_1", "artifact:created", "task_1", '{"artifactId":"artifact_1","actorId":"user_456"}');
    const state = artifactCatalogProjectionHandler(null, event);
...
test("artifactCatalogProjectionHandler uses artifactKey as fallback for artifactId", () => {
    const event = makeEvent("evt_1", "artifact:created", "task_1", '{"artifactKey":"key_123"}');
    const state = artifactCatalogProjectionHandler(null, event);
    assert.equal(state.artifactId, "key_123");
});
//# sourceMappingURL=artifact-catalog-projection.test.js.map