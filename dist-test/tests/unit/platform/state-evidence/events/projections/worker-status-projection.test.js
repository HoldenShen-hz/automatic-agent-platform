...
import { workerStatusProjectionHandler, createEmptyWorkerStatusState, createWorkerStatusProjectionHandler, } from "../../../../../../src/platform/state-evidence/events/projections/worker-status-projection.js";
/**
...
test("workerStatusProjectionHandler initializes state correctly", () => {
    ...
    const state = workerStatusProjectionHandler(null, event);
...
test("workerStatusProjectionHandler handles worker:claim_accepted", () => {
    const event = makeEvent("evt_claim_1", "worker:claim_accepted", "task_1", '{"workerId":"worker_abc","tenantId":"tenant_1"}');
    const state = workerStatusProjectionHandler(null, event);
...
test("workerStatusProjectionHandler handles worker:claim_rejected", () => {
    const event = makeEvent("evt_rej_1", "worker:claim_rejected", "task_1", '{"workerId":"worker_1"}');
    const state = workerStatusProjectionHandler(null, event);
    assert.equal(state.status, "rejected");
    assert.equal(state.claimsRejected, 1);
});
test("workerStatusProjectionHandler handles worker:heartbeat_recorded", () => {
    const event = makeEvent("evt_hb_1", "worker:heartbeat_recorded", "task_1", '{"workerId":"worker_1"}');
    const state = workerStatusProjectionHandler(null, event);
    assert.equal(state.status, "active");
...
test("workerStatusProjectionHandler transitions status from idle to active on heartbeat", () => {
    ...
    const idleEvent = makeEvent("evt_idle_1", "worker:heartbeat_recorded", "task_1", '{"workerId":"worker_1","status":"idle"}');
    const stateAfterIdle = workerStatusProjectionHandler(null, idleEvent);
    assert.equal(stateAfterIdle.status, "active");
});
test("workerStatusProjectionHandler handles worker:writeback_recorded", () => {
    const event = makeEvent("evt_wb_1", "worker:writeback_recorded", "task_1", '{"workerId":"worker_1"}');
    const state = workerStatusProjectionHandler(null, event);
...
test("workerStatusProjectionHandler handles worker:writeback_rejected", () => {
    const event = makeEvent("evt_wb_rej_1", "worker:writeback_rejected", "task_1", '{"workerId":"worker_1"}');
    const state = workerStatusProjectionHandler(null, event);
    assert.equal(state.status, "rejected");
...
test("workerStatusProjectionHandler handles worker:lease_released_after_writeback", () => {
    const event = makeEvent("evt_lease_1", "worker:lease_released_after_writeback", "task_1", '{"workerId":"worker_1"}');
    const state = workerStatusProjectionHandler(null, event);
    assert.equal(state.status, "completed");
    assert.equal(state.leaseReleasesAfterWriteback, 1);
});
test("workerStatusProjectionHandler is idempotent - same event twice", () => {
    const event = makeEvent("evt_idem_1", "worker:claim_accepted", "task_1", '{"workerId":"worker_1"}');
    const state1 = workerStatusProjectionHandler(null, event);
    const state2 = workerStatusProjectionHandler(state1, event);
...
test("workerStatusProjectionHandler accumulates multiple events", () => {
    const claimEvent = makeEvent("evt_claim", "worker:claim_accepted", "task_1", '{"workerId":"worker_1"}');
    const heartbeatEvent = makeEvent("evt_hb", "worker:heartbeat_recorded", "task_1", '{"workerId":"worker_1"}');
    const state1 = workerStatusProjectionHandler(null, claimEvent);
    const state2 = workerStatusProjectionHandler(state1, heartbeatEvent);
    assert.equal(state2.claimsAccepted, 1);
...
test("workerStatusProjectionHandler parses payload with executionId", () => {
    const event = makeEvent("evt_1", "worker:claim_accepted", "task_1", '{"workerId":"worker_1","executionId":"exec_xyz"}');
    const state = workerStatusProjectionHandler(null, event);
...
test("workerStatusProjectionHandler timeline tracks events in order", () => {
    ...
    for (const evt of events) {
        state = workerStatusProjectionHandler(state, evt);
    }
...
test("workerStatusProjectionHandler skips traceContext in details", () => {
    const event = makeEvent("evt_1", "worker:claim_accepted", "task_1", '{"workerId":"worker_1","traceContext":{"traceId":"abc"}}');
    const state = workerStatusProjectionHandler(null, event);
...
//# sourceMappingURL=worker-status-projection.test.js.map