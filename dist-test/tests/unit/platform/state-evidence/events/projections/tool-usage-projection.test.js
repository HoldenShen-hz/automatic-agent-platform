...
import { toolUsageProjectionHandler, createEmptyToolUsageState, createToolUsageProjectionHandler, } from "../../../../../../src/platform/state-evidence/events/projections/tool-usage-projection.js";
/**
...
test("toolUsageProjectionHandler initializes state correctly", () => {
    ...
    const state = toolUsageProjectionHandler(null, event);
...
test("toolUsageProjectionHandler handles plugin:invocation_started", () => {
    const event = makeEvent("evt_inv_1", "plugin:invocation_started", "task_1", '{"pluginId":"plugin_abc","toolName":"code_executor"}');
    const state = toolUsageProjectionHandler(null, event);
...
test("toolUsageProjectionHandler handles skill:step_succeeded", () => {
    const event = makeEvent("evt_succ_1", "skill:step_succeeded", "task_1", '{"skillId":"skill_1","stepId":"step_1","durationMs":150}');
    const state = toolUsageProjectionHandler(null, event);
...
test("toolUsageProjectionHandler handles skill:step_failed", () => {
    const event = makeEvent("evt_fail_1", "skill:step_failed", "task_1", '{"skillId":"skill_1","stepId":"step_1","errorCode":"TIMEOUT"}');
    const state = toolUsageProjectionHandler(null, event);
...
test("toolUsageProjectionHandler is idempotent - same event twice", () => {
    const event = makeEvent("evt_idem_1", "skill:step_succeeded", "task_1", '{"skillId":"skill_1"}');
    const state1 = toolUsageProjectionHandler(null, event);
    const state2 = toolUsageProjectionHandler(state1, event);
...
test("toolUsageProjectionHandler accumulates multiple events", () => {
    const startedEvent = makeEvent("evt_start", "skill:execution_started", "task_1", '{"skillId":"skill_1"}');
    const succeededEvent = makeEvent("evt_succ", "skill:step_succeeded", "task_1", '{"skillId":"skill_1"}');
    const state1 = toolUsageProjectionHandler(null, startedEvent);
    const state2 = toolUsageProjectionHandler(state1, succeededEvent);
...
test("toolUsageProjectionHandler parses payload with multiple fields", () => {
    const event = makeEvent("evt_1", "plugin:invocation_started", "task_1", '{"pluginId":"plugin_x","toolName":"bash","executionId":"exec_123"}');
    const state = toolUsageProjectionHandler(null, event);
...
test("toolUsageProjectionHandler timeline tracks events in order", () => {
    ...
    for (const evt of events) {
        state = toolUsageProjectionHandler(state, evt);
    }
...
//# sourceMappingURL=tool-usage-projection.test.js.map