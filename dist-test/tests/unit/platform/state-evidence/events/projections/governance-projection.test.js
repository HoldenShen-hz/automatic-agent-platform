...
import { governanceProjectionHandler, createEmptyGovernanceState, createGovernanceProjectionHandler, } from "../../../../../../src/platform/state-evidence/events/projections/governance-projection.js";
/**
...
test("governanceProjectionHandler initializes state correctly", () => {
    ...
    const state = governanceProjectionHandler(null, event);
...
test("governanceProjectionHandler handles policy:created", () => {
    const event = makeEvent("evt_policy_created", "policy:created", "task_1", '{"policyId":"pol_1","policyName":"Test Policy"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.entityId, "pol_1");
    assert.equal(state.entityKind, "policy");
    assert.equal(state.status, "active");
...
test("governanceProjectionHandler handles policy:updated", () => {
    const event = makeEvent("evt_policy_updated", "policy:updated", "task_1", '{"policyId":"pol_1","policyName":"Updated Policy"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.status, "active");
    assert.equal(state.actionType, "policy_updated");
});
test("governanceProjectionHandler handles policy:deleted", () => {
    const event = makeEvent("evt_policy_deleted", "policy:deleted", "task_1", '{"policyId":"pol_1"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.status, "cancelled");
    assert.equal(state.actionType, "policy_deleted");
});
test("governanceProjectionHandler handles approval_flow:approved", () => {
    const event = makeEvent("evt_approved", "approval_flow:approved", "task_1", '{"approvalId":"apr_1","actorId":"user_1","reason":"Approved"}');
    const state = governanceProjectionHandler(null, event);
...
test("governanceProjectionHandler handles approval_flow:rejected", () => {
    const event = makeEvent("evt_rejected", "approval_flow:rejected", "task_1", '{"approvalId":"apr_1","actorId":"user_1","reasonCode":"denied"}');
    const state = governanceProjectionHandler(null, event);
...
test("governanceProjectionHandler handles decision:approved", () => {
    const event = makeEvent("evt_decision_approved", "decision:approved", null, '{"approvalId":"dec_1","principal":"system"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.status, "approved");
    assert.equal(state.approved, true);
    assert.equal(state.actionType, "approval_granted");
...
test("governanceProjectionHandler handles decision:rejected", () => {
    const event = makeEvent("evt_decision_rejected", "decision:rejected", null, '{"approvalId":"dec_1","principal":"system"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.status, "denied");
    assert.equal(state.approved, false);
    assert.equal(state.actionType, "approval_denied");
...
test("governanceProjectionHandler handles delegation:created", () => {
    const event = makeEvent("evt_delegation_created", "delegation:created", "task_1", '{"delegationId":"del_1","actorId":"user_1"}');
    const state = governanceProjectionHandler(null, event);
...
test("governanceProjectionHandler handles delegation:completed", () => {
    const event = makeEvent("evt_delegation_completed", "delegation:completed", "task_1", '{"delegationId":"del_1"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.status, "completed");
    assert.equal(state.actionType, "delegation_completed");
});
test("governanceProjectionHandler handles delegation:failed", () => {
    const event = makeEvent("evt_delegation_failed", "delegation:failed", "task_1", '{"delegationId":"del_1","reasonCode":"timeout"}');
    const state = governanceProjectionHandler(null, event);
...
test("governanceProjectionHandler handles compliance:violation_detected", () => {
    const event = makeEvent("evt_compliance_violation", "compliance:violation_detected", "task_1", '{"violationId":"vio_1","framework":"SOC2","reasonCode":"data_leak"}');
    const state = governanceProjectionHandler(null, event);
...
test("governanceProjectionHandler handles compliance:resolved", () => {
    const event = makeEvent("evt_compliance_resolved", "compliance:resolved", "task_1", '{"violationId":"vio_1","framework":"SOC2"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.status, "resolved");
    assert.equal(state.actionType, "compliance_resolved");
});
test("governanceProjectionHandler handles permission:granted", () => {
    const event = makeEvent("evt_permission_granted", "permission:granted", "task_1", '{"principal":"user_1","resource":"doc_1"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.status, "approved");
    assert.equal(state.approved, true);
...
test("governanceProjectionHandler handles permission:revoked", () => {
    const event = makeEvent("evt_permission_revoked", "permission:revoked", "task_1", '{"principal":"user_1","resource":"doc_1"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.status, "denied");
    assert.equal(state.approved, false);
...
test("governanceProjectionHandler handles role:assigned", () => {
    const event = makeEvent("evt_role_assigned", "role:assigned", "task_1", '{"actorId":"admin_1","role":"admin"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.status, "approved");
...
test("governanceProjectionHandler handles role:removed", () => {
    const event = makeEvent("evt_role_removed", "role:removed", "task_1", '{"actorId":"admin_1","role":"admin"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.status, "denied");
    assert.equal(state.actionType, "role_removed");
});
test("governanceProjectionHandler handles config:changed", () => {
    const event = makeEvent("evt_config_changed", "config:changed", "task_1", '{"configId":"cfg_1","actorId":"admin"}');
    const state = governanceProjectionHandler(null, event);
...
test("governanceProjectionHandler handles approval_flow:escalated", () => {
    const event = makeEvent("evt_escalated", "approval_flow:escalated", "task_1", '{"approvalId":"apr_1","actorId":"manager"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.status, "pending");
    assert.equal(state.actionType, "escalation_triggered");
});
test("governanceProjectionHandler handles config:policy_created", () => {
    const event = makeEvent("evt_policy_created_alt", "config:policy_created", null, '{"policyId":"pol_alt","policyName":"Alt Policy"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.status, "active");
    assert.equal(state.actionType, "policy_created");
});
test("governanceProjectionHandler handles config:policy_changed", () => {
    const event = makeEvent("evt_policy_changed_alt", "config:policy_changed", null, '{"policyId":"pol_alt"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.status, "active");
    assert.equal(state.actionType, "policy_updated");
});
test("governanceProjectionHandler is idempotent - same event applied twice", () => {
    const event = makeEvent("evt_idempotent", "approval_flow:approved", "task_1", '{"approvalId":"apr_1"}');
    const state1 = governanceProjectionHandler(null, event);
    const state2 = governanceProjectionHandler(state1, event);
...
test("governanceProjectionHandler deduplicates event_ids", () => {
    ...
    const state1 = governanceProjectionHandler(null, event);
    const state2 = governanceProjectionHandler(state1, event);
    const state3 = governanceProjectionHandler(state2, event);
    // Should only count once
...
test("governanceProjectionHandler extracts tenantId from payload", () => {
    const event = makeEvent("evt_tenant", "policy:created", "task_1", '{"policyId":"pol_1","tenantId":"tenant_abc"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.tenantId, "tenant_abc");
});
test("governanceProjectionHandler extracts executionId from payload", () => {
    const event = makeEvent("evt_exec", "approval_flow:approved", "task_1", '{"approvalId":"apr_1","executionId":"exec_xyz"}');
    const state = governanceProjectionHandler(null, event);
...
test("governanceProjectionHandler infers entityKind from eventType when not in payload", () => {
    const event = makeEvent("evt_infer", "policy:created", null, '{"policyId":"pol_1"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.entityKind, "policy");
});
test("governanceProjectionHandler infers entityKind for approval events", () => {
    const event = makeEvent("evt_approval", "decision:confirmed", null, '{"approvalId":"apr_1"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.entityKind, "approval");
});
test("governanceProjectionHandler infers entityKind for delegation events", () => {
    const event = makeEvent("evt_delegation", "delegation:created", null, '{"delegationId":"del_1"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.entityKind, "delegation");
});
test("governanceProjectionHandler infers entityKind for compliance events", () => {
    const event = makeEvent("evt_compliance", "compliance:violation_detected", null, '{"violationId":"vio_1"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.entityKind, "compliance");
});
test("governanceProjectionHandler infers entityKind for permission events", () => {
    const event = makeEvent("evt_permission", "permission:granted", null, '{"principal":"user_1"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.entityKind, "permission");
});
test("governanceProjectionHandler infers entityKind for role events", () => {
    const event = makeEvent("evt_role", "role:assigned", null, '{"actorId":"user_1"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.entityKind, "role");
});
test("governanceProjectionHandler infers entityKind for config events", () => {
    const event = makeEvent("evt_config", "config:changed", null, '{"configId":"cfg_1"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.entityKind, "config");
});
...
test("governanceProjectionHandler preserves occurredAt from payload", () => {
    const event = makeEvent("evt_occurred", "approval_flow:approved", "task_1", '{"approvalId":"apr_1","occurredAt":"2026-04-18T09:00:00.000Z"}', "2026-04-19T10:00:00.000Z");
    const state = governanceProjectionHandler(null, event);
...
test("governanceProjectionHandler uses event createdAt when occurredAt not in payload", () => {
    const event = makeEvent("evt_no_occurred", "approval_flow:approved", "task_1", '{"approvalId":"apr_1"}', "2026-04-19T10:00:00.000Z");
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.occurredAt, "2026-04-19T10:00:00.000Z");
});
test("governanceProjectionHandler prioritizes policyId for entityId", () => {
    const event = makeEvent("evt_priority", "policy:created", null, '{"policyId":"pol_1","approvalId":"apr_1"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.entityId, "pol_1");
});
test("governanceProjectionHandler uses approvalId when policyId not available", () => {
    const event = makeEvent("evt_approval_only", "approval_flow:approved", null, '{"approvalId":"apr_1"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.entityId, "apr_1");
});
test("governanceProjectionHandler uses delegationId when policyId and approvalId not available", () => {
    const event = makeEvent("evt_delegation_only", "delegation:created", null, '{"delegationId":"del_1"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.entityId, "del_1");
});
test("governanceProjectionHandler uses configId when policyId, approvalId, and delegationId not available", () => {
    const event = makeEvent("evt_config_only", "config:changed", null, '{"configId":"cfg_1"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.entityId, "cfg_1");
});
test("governanceProjectionHandler uses violationId when other IDs not available", () => {
    const event = makeEvent("evt_violation_only", "compliance:violation_detected", null, '{"violationId":"vio_1"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.entityId, "vio_1");
});
test("governanceProjectionHandler prefers actorId over principal for principal field", () => {
    const event = makeEvent("evt_actor", "approval_flow:approved", null, '{"approvalId":"apr_1","actorId":"actor_1","principal":"principal_1"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.principal, "actor_1");
});
test("governanceProjectionHandler uses principal when actorId not available", () => {
    const event = makeEvent("evt_principal", "approval_flow:approved", null, '{"approvalId":"apr_1","principal":"principal_1"}');
    const state = governanceProjectionHandler(null, event);
...
test("governanceProjectionHandler uses respondedBy when actorId and principal not available", () => {
    const event = makeEvent("evt_responded", "approval_flow:approved", null, '{"approvalId":"apr_1","respondedBy":"responder_1"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.principal, "responder_1");
});
test("governanceProjectionHandler prefers reason over reasonCode for reason field", () => {
    const event = makeEvent("evt_reason", "approval_flow:approved", null, '{"approvalId":"apr_1","reason":"Approved","reasonCode":"approved"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.reason, "Approved");
});
test("governanceProjectionHandler uses reasonCode when reason not available", () => {
    const event = makeEvent("evt_reason_code", "approval_flow:approved", null, '{"approvalId":"apr_1","reasonCode":"approved"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.reason, "approved");
});
test("governanceProjectionHandler prefers framework over complianceFramework", () => {
    const event = makeEvent("evt_framework", "compliance:violation_detected", null, '{"violationId":"vio_1","framework":"HIPAA","complianceFramework":"SOC2"}');
    const state = governanceProjectionHandler(null, event);
...
test("governanceProjectionHandler uses complianceFramework when framework not available", () => {
    const event = makeEvent("evt_compliance_framework", "compliance:violation_detected", null, '{"violationId":"vio_1","complianceFramework":"SOC2"}');
    const state = governanceProjectionHandler(null, event);
    assert.equal(state.complianceFramework, "SOC2");
});
test("governanceProjectionHandler handles invalid JSON payload gracefully", () => {
    const event = makeEvent("evt_invalid_json", "policy:created", null, 'not valid json');
    const state = governanceProjectionHandler(null, event);
...
test("governanceProjectionHandler excludes traceContext from details", () => {
    const event = makeEvent("evt_trace", "policy:created", null, '{"policyId":"pol_1","traceContext":{"traceId":"abc"}}');
    const state = governanceProjectionHandler(null, event);
...
test("createGovernanceProjectionHandler returns handler function", () => {
    ...
    assert.equal(state.entityId, "pol_test");
});
test("governanceProjectionHandler timeline entry contains correct fields", () => {
    const event = makeEvent("evt_timeline_fields", "approval_flow:approved", "task_1", '{"approvalId":"apr_1","actorId":"user_1"}', "2026-04-19T10:00:00.000Z");
    const state = governanceProjectionHandler(null, event);
...
//# sourceMappingURL=governance-projection.test.js.map