...
import { EVENT_SCHEMA_REGISTRY, hasEventSchema, getRegisteredConsumers, getEventSchema, validateEventPayload, } from "../../../../../src/platform/state-evidence/events/event-registry.js";
...
test("getEventSchema throws ValidationError for unknown event", () => {
    assert.throws(() => getEventSchema("unknown:event"), (error) => {
...
test("validateEventPayload throws ValidationError for invalid payload", () => {
    ...
    assert.throws(() => validateEventPayload("task:status_changed", invalidPayload), (error) => {
...
test("validateEventPayload throws ValidationError for unknown event type", () => {
    const payload = { fromStatus: "queued", toStatus: "in_progress" };
    assert.throws(() => validateEventPayload("unknown:event_type", payload), (error) => {
...
test("validateEventPayload rejects subtask payload missing both stepId and subtaskId", () => {
    ...
    assert.throws(() => validateEventPayload("subtask:completed", invalidPayload), (error) => {
