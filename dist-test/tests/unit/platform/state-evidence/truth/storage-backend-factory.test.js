...
import { openAuthoritativeStorageBackend, openAuthoritativeStorageContext, openAsyncAuthoritativeStorageBackend, openAsyncAuthoritativeStorageContext, openPostgresAuthoritativeStorageBackend, planAuthoritativeStorageBackend, requirePostgresAuthoritativeStorageBackend, requireSyncCompatibleAuthoritativeSqlDatabase, requireSqliteAuthoritativeStorageBackend, } from "../../../../../src/platform/state-evidence/truth/storage-backend-factory.js";
...
test("storage backend factory sync compatibility helper returns shadow sqlite and fail-closes without it", () => {
    ...
    try {
        ...
        assert.throws(() => requireSyncCompatibleAuthoritativeSqlDatabase(postgresLike), /storage\.postgres_shadow_sqlite_required_for_sync_compatibility/);
        sqliteStorage.close();
    }
    finally {
