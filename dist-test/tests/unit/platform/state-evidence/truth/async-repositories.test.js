...
function costReportRecord(overrides = {}) {
    return {
        reportId: "report-1",
        tenantId: "tenant-a",
...
function budgetAlertRecord(overrides = {}) {
    return {
        alertId: "alert-1",
        tenantId: "tenant-a",
...
function tokenUsageDailyRecord(overrides = {}) {
    return {
        usageId: "usage-1",
        tenantId: "tenant-a",
        packId: "pack-1",
...
function delegationEventRecord(overrides = {}) {
    return {
...
test("AsyncCostManagementRepository writes and reads budget alerts", async () => {
    ...
    await repo.insertBudgetAlert({ ...alert, alertId: "alert-2" });
...
test("AsyncEvolutionRepository writes and reads evolution proposals", async () => {
    ...
    await repo.insertEvolutionProposal(proposal);
...
test("AsyncEvolutionRepository lists evolution proposals by status", async () => {
    ...
    assert.deepEqual(await repo.listEvolutionProposals("approved"), [proposal]);
...
test("AsyncEvolutionRepository writes and reads evolution policies and logs", async () => {
    ...
    const repo = new AsyncEvolutionRepository(connection);
    await repo.insertEvolutionPolicy(policy);
    ...
    assert.deepEqual(await repo.listEvolutionPolicies({ kind: "routing" }), [policy]);
    await repo.insertEvolutionLog(log);
...
test("AsyncEvolutionRepository writes and reads PMF validation reports", async () => {
    ...
    const repo = new AsyncEvolutionRepository(connection);
    await repo.insertPmfValidationReport(report);
...
test("AsyncOrganizationRepository upserts workspace and membership records", async () => {
    ...
    await repo.upsertWorkspaceMembershipRecord(membership);
...
test("AsyncOrganizationRepository upserts and reads organizations", async () => {
    ...
    await repo.upsertOrganizationMembershipRecord(membership);
...
test("AsyncPromptRepository writes and reads prompt bundles", async () => {
    ...
    const updated = await repo.updatePromptBundle({ bundleId: "bundle-1", version: "1.1.0", updatedAt: now });
    assert.equal(updated, 1);
