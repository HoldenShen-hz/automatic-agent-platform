...
import { isPgWriteError, PgDatabase, PgWriteError } from "../../../../../../src/platform/state-evidence/truth/postgres/pg-database.js";
...
test("PgDatabase.open throws when postgres driver is not installed", async () => {
    ...
    await assert.rejects(async () => PgDatabase.open(options), /postgres\.connection_failed|postgres\.driver_not_available|cannot find module/);
});
test("PgDatabase throws helpful error before connection", async () => {
    ...
    await assert.rejects(async () => { await db.migrate(); }, /postgres\.not_connected/);
    await assert.rejects(async () => { await db.getSchemaStatus(); }, /postgres\.not_connected/);
    await assert.rejects(async () => { await db.integrityCheck(); }, /postgres\.not_connected/);
    // Sync method - connection.exec throws synchronously
    assert.throws(() => { db.connection.exec("SELECT 1"); }, /postgres\.not_connected/);
});
...
test("PgDatabase connection.prepare returns stub before connect", () => {
    ...
    assert.throws(() => { stmt.all(); }, /postgres\.not_connected/);
    assert.throws(() => { stmt.run(); }, /postgres\.not_connected/);
});
...
test("factory openPostgresAuthoritativeStorageBackend throws when postgres driver not installed", async () => {
    ...
    try {
        ...
        await assert.rejects(async () => openPostgresAuthoritativeStorageBackend({
            dbPath,
            environment: "staging",
            env,
...
//# sourceMappingURL=pg-database.test.js.map