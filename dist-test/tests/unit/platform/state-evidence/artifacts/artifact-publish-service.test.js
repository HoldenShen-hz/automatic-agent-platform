...
function createMockBundle(overrides = {}) {
    ...
    return {
        ...
        taskId: "task_001",
        artifacts: [
            { artifactId: "a1", taskId: "t1", stepId: "s1", agentRole: "agent", type: "source_code", path: "/file.js", contentHash: "hash1", version: 1, parentArtifactId: null, size: 100, createdAt: now, status: "committed" },
            { artifactId: "a2", taskId: "t1", stepId: "s1", agentRole: "agent", type: "config", path: "/config.json", contentHash: "hash2", version: 1, parentArtifactId: null, size: 50, createdAt: now, status: "committed" },
        ],
        links: [],
...
test("ArtifactPublishService throws error when publishing already published bundle", () => {
    ...
    assert.throws(() => service.publish(bundle), (err) => err.code === "artifact.publish_already_published");
});
test("ArtifactPublishService throws error when publishing recalled bundle", () => {
    ...
    assert.throws(() => service.publish(bundle), (err) => err.code === "artifact.publish_recalled_bundle");
});
...
//# sourceMappingURL=artifact-publish-service.test.js.map