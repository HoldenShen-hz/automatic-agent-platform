...
test("ArtifactPlaneService records publish history to ledger", () => {
    ...
    try {
        const plane = new ArtifactPlaneService(undefined, undefined, undefined, new ArtifactPublishService(new ArtifactPublishLedger({
...
test("ArtifactPlaneService blocks publishing bundles with sensitive metadata", () => {
    ...
    assert.throws(() => plane.publishBundle(prepared.bundle), (error) => typeof error === "object" && error != null && "code" in error && error.code === "artifact_plane.publish_blocked");
});
//# sourceMappingURL=artifact-plane-service.test.js.map