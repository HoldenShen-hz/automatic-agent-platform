...
function createProviderWithMemoryScope(scope) {
...
test("buildView includes queryText tag when provided", async () => {
    const provider = createMockProvider();
    const cacheMock = {
        getOrComputeMemoryRetrieval: async (_query, loader, tags) => {
...
test("buildView categorizes multiple memories into correct layers", async () => {
    const provider = {
        ...createMockProvider(),
        prefetch: async () => createMockPrefetchResult({
            memories: [
...
test("syncTurn returns rememberedMemories when present", async () => {
    ...
    const provider = createMockProvider();
    provider.syncTurn = async () => createMockSyncResult({
...
//# sourceMappingURL=memory-plane-service.test.js.map