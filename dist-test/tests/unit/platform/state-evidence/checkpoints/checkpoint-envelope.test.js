...
import { createCheckpointEnvelope, unpackCheckpointEnvelope, wrapWorkflowStepCheckpoint, unwrapWorkflowStepCheckpoint, getEnvelopeOriginalSize, getEnvelopeCompressedSize, getEnvelopeCompressionRatio, CheckpointSizeExceededError, CheckpointEnvelopeInvalidError, DEFAULT_MAX_CHECKPOINT_SIZE_BYTES, CHECKPOINT_ENVELOPE_SCHEMA_VERSION, } from "../../../../../src/platform/state-evidence/checkpoints/index.js";
...
describe("CheckpointEnvelope", () => {
    ...
    describe("unpackCheckpointEnvelope", () => {
        ...
        it("should preserve all checkpoint fields after round-trip", async () => {
            ...
            assert.deepStrictEqual(unpacked.data.output.nested, { deeply: { value: true } });
        });
        ...
        it("should throw CheckpointEnvelopeInvalidError for invalid envelope", async () => {
            ...
            await assert.rejects(async () => unpackCheckpointEnvelope(invalidEnvelope), CheckpointEnvelopeInvalidError);
        });
        it("should throw CheckpointEnvelopeInvalidError for corrupted checksum", async () => {
            ...
            await assert.rejects(async () => unpackCheckpointEnvelope(envelope), CheckpointEnvelopeInvalidError);
        });
    ...
    describe("CheckpointSizeExceededError", () => {
        it("should throw when checkpoint exceeds default size limit", async () => {
            ...
            await assert.rejects(async () => createCheckpointEnvelope(checkpoint, "test.v1"), CheckpointSizeExceededError);
        });
        it("should throw when checkpoint exceeds custom size limit", async () => {
            const checkpoint = createTestCheckpoint({
                output: { data: "x".repeat(1000) },
            });
            await assert.rejects(async () => createCheckpointEnvelope(checkpoint, "test.v1", { maxSizeBytes: 100 }), CheckpointSizeExceededError);
        });
    ...
    describe("JSON serialization", () => {
        it("should serialize and deserialize envelope as JSON", async () => {
            ...
            const unpacked = await unpackCheckpointEnvelope(parsed);
            assert.deepStrictEqual(unpacked.data, checkpoint);
        });
...
//# sourceMappingURL=checkpoint-envelope.test.js.map