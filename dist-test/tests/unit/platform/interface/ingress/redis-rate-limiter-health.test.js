...
const mockRedisErrors = [];
const mockHealthStatus = { healthy: true };
const mockCounterIncrements = [];
...
test("[SYS-REL-2.1] redis rate limiter increments error counter on Redis errors", async () => {
    ...
    incrementCounter("redis_connection_errors");
    incrementCounter("redis_connection_errors");
    assert.equal(mockCounterIncrements.filter((c) => c === "redis_connection_errors").length, 3, "Error counter should be incremented 3 times");
});
test("[SYS-REL-2.1] rate limiter checkAndConsume returns allowed=false when limit exceeded", async () => {
    ...
    const checkAndConsume = async (_key, limit, windowMs) => {
...
test("[SYS-REL-2.1] rate limiter health check reflects Redis connection state", () => {
    ...
    const createHealthTracker = (redisConnected) => ({
...
test("[SYS-REL-2.1] redis error updates health status to unhealthy", () => {
    ...
    // Trigger error
    onRedisError();
    assert.strictEqual(mockHealthStatus.healthy, false, "Health status should be false after Redis error");
});
//# sourceMappingURL=redis-rate-limiter-health.test.js.map