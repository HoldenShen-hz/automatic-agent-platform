...
test("registerEndpoint throws on empty endpointId", () => {
    ...
    }, (error) => error instanceof Error && "code" in error && error.code === "webhook.invalid_endpoint_id");
});
test("registerEndpoint throws on empty source", () => {
    ...
    }, (error) => error instanceof Error && "code" in error && error.code === "webhook.invalid_source");
});
test("registerEndpoint throws when sha256_hmac endpoint has no signing secret", () => {
    ...
    }, (error) => error instanceof Error && error.message.includes("signing secret"));
});
...
test("receive throws when endpoint not registered", () => {
    ...
    }, (error) => error instanceof Error && "code" in error && error.code === "webhook.endpoint_not_found");
});
test("receive throws when endpoint is disabled", () => {
    ...
    }, (error) => error instanceof Error && "code" in error && error.code === "webhook.endpoint_disabled");
});
// =============================================================================
...
test("receive throws when eventType is missing", () => {
    ...
    }, (error) => error instanceof Error && "code" in error && error.code === "webhook.event_type_required");
});
test("receive throws when idempotency key is missing", () => {
    ...
    }, (error) => error instanceof Error && "code" in error && error.code === "webhook.idempotency_key_required");
});
...
test("receive throws on invalid JSON body", () => {
    ...
    }, (error) => error instanceof Error && "code" in error && error.code === "webhook.invalid_json");
});
...
test("receive throws when signature is missing for signed endpoint", () => {
    ...
    }, (error) => error instanceof Error && "code" in error && error.code === "webhook.signature_required");
});
test("receive throws when signature is invalid", () => {
    ...
    }, (error) => error instanceof Error && "code" in error && error.code === "webhook.signature_invalid");
});
