...
test("mission control global views fail-close when a tenant scope is requested", () => {
    ...
    try {
        const context = createSeededApiContext(workspace);
        assert.throws(() => context.missionControlService.getSnapshot("tenant-scoped"), /mission_control\.snapshot_not_tenant_scoped/);
        assert.throws(() => context.missionControlService.getStabilityPanel(10, "tenant-scoped"), /mission_control\.stability_not_tenant_scoped/);
        assert.throws(() => context.missionControlService.getAdminTakeoverConsole(context.seededTaskId, "tenant-scoped"), /mission_control\.admin_console_not_tenant_scoped/);
        context.db.close();
    }
    finally {
        cleanupPath(workspace);
...
//# sourceMappingURL=mission-control-service.test.js.map