...
import { GraphQLAdapterService, GraphQLSchemaBuilder, GRAPHQL_ERROR_CODES, } from "../../../../../src/platform/interface/api/graphql-adapter-service.js";
...
test("GraphQLAdapterService execute returns error for missing schema", async () => {
    const adapter = new GraphQLAdapterService({ endpoint: "http://localhost:4000/graphql" });
    const request = { query: "query { hello }" };
...
test("GraphQLAdapterService execute returns error for subscription", async () => {
    ...
    const schema = createTestSchema();
    adapter.registerSchema("test", schema);
    const request = { query: "subscription { onMessage { data } }" };
...
//# sourceMappingURL=graphql-adapter-service.test.js.map