...
function createMockGatewayTargetDirectoryService() {
    return {
        listTargets: () => [
            {
                targetId: "target_1",
...
function createMockChannelGatewayDeliveryService() {
    return {
        verifySignature: (payload, signature, timestamp, config) => ({
            valid: true,
            error: null,
        }),
        verifyNonce: (nonce, toleranceSeconds) => ({
            valid: true,
            error: null,
        }),
        createDeliveryMessage: (channel, targetId, payload) => ({
...
function createMockCoordinatorLoadBalancingService() {
    return {
        buildSummary: () => ({
            coordinators: [],
            totalActive: 0,
...
function createMockPrometheusMetricsExporter() {
    return {
        ...
        recordHttpRequest: (_method, _route, _statusCode, _durationMs) => { },
    };
}
// ─── Test Server Factory ─────────────────────────────────────────────────────
...
test("generates request-id when not provided", async () => {
    ...
    try {
        ...
        assert.ok(response.headers["x-request-id"].startsWith("req_"));
    }
    finally {
        await server.stop();
    }
...
test("HttpApiServer enforces request timeout budget", async () => {
    ...
    try {
        ...
        assert.equal(response.json().error.code, "api.request_timeout");
    }
    finally {
        await server.stop();
    }
