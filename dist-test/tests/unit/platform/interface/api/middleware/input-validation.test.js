...
test("readValidatedJsonBody parses valid JSON and validates", () => {
    ...
    const result = readValidatedJsonBody(body, (p) => {
...
test("readValidatedJsonBody applies sanitizer before parser", () => {
    ...
    assert.throws(() => readValidatedJsonBody(body, (p) => p), (err) => isAppError(err) && err.message.includes("reserved key"));
});
...
test("readValidatedJsonBody throws when JSON is invalid", () => {
    ...
    assert.throws(() => readValidatedJsonBody(body, (p) => p), (err) => isAppError(err) && err.message.includes("valid JSON"));
});
