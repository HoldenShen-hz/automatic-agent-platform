...
import { readRequestId, readLimit, readStatusFilter, readQueryParam, readJsonBody, resolveTenantScope, assertGlobalTenantScopeSupported, assertTaskTenantAccess, validateTaskId, buildJsonResponse, buildJsonErrorResponse, buildJsonDocumentResponse, buildHtmlResponse, buildTextResponse, normalizeSegments, matchNormalizedSegments, } from "../../../../../../src/platform/interface/api/http-server/utils.js";
...
function makePrincipal(tenantId) {
...
test("resolveTenantScope throws when principal tenant does not match requested", () => {
    const principal = makePrincipal("tenant_xyz");
    assert.throws(() => resolveTenantScope(principal, "tenant_abc"), /cannot access another tenant/);
});
...
test("assertGlobalTenantScopeSupported throws for tenant-scoped principal", () => {
    const principal = makePrincipal("tenant_abc");
    assert.throws(() => assertGlobalTenantScopeSupported(principal, "admin"), /cannot access/);
});
...
test("assertTaskTenantAccess throws when tenant does not match", () => {
    const principal = makePrincipal("tenant_xyz");
    assert.throws(() => assertTaskTenantAccess(principal, "tenant_abc", "not_found", "Not found"), /Not found/);
});
...
test("validateTaskId rejects null taskId", () => {
    assert.throws(() => validateTaskId(null, "path"), /requires taskId/);
});
