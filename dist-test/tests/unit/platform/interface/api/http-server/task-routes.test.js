...
function createMockContext(pathname = "/tasks", segments = [], headers = {}) {
...
test("GET /v1/tasks returns task list", async () => {
    ...
    assert.deepEqual(body.data.tasks.map((task) => task.taskId), ["task-3", "task-2", "task-1"]);
...
test("GET /v1/workflows returns workflow list", async () => {
    ...
    assert.equal(response.statusCode, 200);
    const body = JSON.parse(response.body);
    assert.deepEqual(body.data.workflows.map((workflow) => workflow.taskId), ["task-2", "task-1"]);
});
test("GET /v1/tasks supports cursor pagination", async () => {
    ...
    assert.deepEqual(firstBody.data.tasks.map((task) => task.taskId), ["task-3", "task-2"]);
    assert.equal(firstBody.data.hasMore, true);
    assert.equal(typeof firstBody.data.nextCursor, "string");
    const secondPage = await callRoute(routes, createMockContext(`/v1/tasks?limit=2&cursor=${encodeURIComponent(firstBody.data.nextCursor)}`, ["v1", "tasks"]));
    if (!secondPage)
        throw new Error("Handler returned null");
    const secondBody = JSON.parse(secondPage.body);
    assert.deepEqual(secondBody.data.tasks.map((task) => task.taskId), ["task-1"]);
...
test("GET /v1/workflows supports cursor pagination", async () => {
    ...
    if (!response)
        throw new Error("Handler returned null");
    const body = JSON.parse(response.body);
    assert.deepEqual(body.data.workflows.map((workflow) => workflow.taskId), ["task-2"]);
...
//# sourceMappingURL=task-routes.test.js.map