...
import { parseAllowedOrigins, normalizeCorsConfig, isOriginAllowed, buildPreflightHeaders, decorateResponseHeaders, DEFAULT_CORS_CONFIG, } from "../../../../../../src/platform/interface/api/http-server/response-hardening.js";
...
test("isOriginAllowed returns false for undefined origin", () => {
    const config = { ...DEFAULT_CORS_CONFIG, allowedOrigins: ["https://app.com"] };
...
test("buildPreflightHeaders includes credentials header when enabled", () => {
    const config = { ...DEFAULT_CORS_CONFIG, credentials: true, allowedOrigins: ["https://app.com"] };
    const headers = buildPreflightHeaders("https://app.com", config);
    assert.equal(headers["access-control-allow-credentials"], "true");
});
test("buildPreflightHeaders uses origin when credentials and wildcard", () => {
    const config = { ...DEFAULT_CORS_CONFIG, credentials: true, allowedOrigins: ["*"] };
    const headers = buildPreflightHeaders("https://app.com", config);
...
test("buildPreflightHeaders uses wildcard without credentials", () => {
    const config = { ...DEFAULT_CORS_CONFIG, credentials: false, allowedOrigins: ["*"] };
    const headers = buildPreflightHeaders("https://app.com", config);
...
//# sourceMappingURL=response-hardening.test.js.map