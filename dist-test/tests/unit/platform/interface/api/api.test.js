...
test("api barrel facade interfaces createNoOpIncidentFacadeService", () => {
    ...
    assert.throws(() => service.openIncident({ severity: "high", title: "Test incident" }), /not configured/);
    assert.throws(() => service.acknowledge("any_id", "any_owner"), /not configured/);
    assert.throws(() => service.startMitigation("any_id"), /not configured/);
    assert.throws(() => service.resolve("any_id"), /not configured/);
});
test("api barrel oidc oauth service provider management", async () => {
    const service = new ApiModule.OidcOAuthService([], ["https://issuer.example.com"], "test-audience", undefined, true);
...
test("api barrel oidc oauth service api key management", () => {
    const service = new ApiModule.OidcOAuthService([], [], "test-audience", undefined, true);
    // Register an API key
...
test("api barrel oidc oauth service build authorization url", () => {
    ...
    const url = service.buildAuthorizationUrl(provider, "client_123", "https://app.example.com/callback", "state_abc", "challenge_xyz");
...
test("api barrel oidc oauth service build authorization url with custom scopes", () => {
    ...
    const url = service.buildAuthorizationUrl(provider, "client_123", "https://app.example.com/callback", "state_abc", "challenge_xyz", ["openid", "profile", "email", "custom_scope"]);
...
test("api barrel oidc oauth service validate federated token - untrusted issuer", async () => {
    const service = new ApiModule.OidcOAuthService([], ["https://trusted.example.com"], "test-audience", undefined, true);
...
test("api barrel pack catalog service duplicate packId throws", () => {
    ...
    assert.throws(() => service.createPack({
        ...
        domainId: "domain",
        createdBy: "user",
    }), (err) => err?.code === "pack.already_exists");
});
