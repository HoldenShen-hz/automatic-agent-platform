...
test("decodeJwtJsonSegment throws AuthError for invalid base64", () => {
    assert.throws(() => decodeJwtJsonSegment("not-valid-base64!!!", "header"), (err) => err instanceof AuthError && err.code === "jwt.header_invalid");
});
...
test("parseJwtHeader throws for non-object", () => {
    assert.throws(() => parseJwtHeader("not an object"), (err) => err instanceof ValidationError && err.code === "jwt.header_invalid");
});
test("parseJwtHeader throws for array", () => {
    assert.throws(() => parseJwtHeader(["array"]), (err) => err instanceof ValidationError && err.code === "jwt.header_invalid");
});
test("parseJwtHeader throws for null", () => {
    assert.throws(() => parseJwtHeader(null), (err) => err instanceof ValidationError && err.code === "jwt.header_invalid");
});
test("parseJwtHeader throws when kid is not string", () => {
    assert.throws(() => parseJwtHeader({ kid: 123 }), (err) => err instanceof ValidationError && err.code === "jwt.header_invalid");
});
test("parseJwtHeader throws when alg is not string", () => {
    assert.throws(() => parseJwtHeader({ alg: 123 }), (err) => err instanceof ValidationError && err.code === "jwt.header_invalid");
});
...
test("parseFederatedTokenClaims throws for non-object", () => {
    assert.throws(() => parseFederatedTokenClaims("not an object"), (err) => err instanceof ValidationError && err.code === "jwt.payload_invalid");
});
test("parseFederatedTokenClaims throws for missing sub", () => {
    assert.throws(() => parseFederatedTokenClaims({
        iss: "https://issuer.example.com",
        aud: "client-id",
        exp: 1700000000,
        iat: 1600000000,
    }), (err) => err instanceof ValidationError && err.code === "jwt.payload_invalid");
});
