...
import { listInterfaceCapabilityBaselines, resolveInterfaceCapabilityBaseline, INTERFACE_CAPABILITY_BASELINES, } from "../../../../src/platform/interface/interface-plane-baseline.js";
test("interface plane baseline covers interface entry modules", () => {
    const baselines = listInterfaceCapabilityBaselines();
    assert.deepEqual(baselines.map((item) => item.capabilityId), ["api", "channel-gateway", "console-backend", "ingress", "scheduler", "webhook"]);
...
test("INTERFACE_CAPABILITY_BASELINES is frozen and immutable", () => {
    assert.ok(Object.isFrozen(INTERFACE_CAPABILITY_BASELINES));
    baselines: for (const baseline of INTERFACE_CAPABILITY_BASELINES) {
        assert.ok(Object.isFrozen(baseline));
        assert.ok(Object.isFrozen(baseline.baselineServices));
    }
});
...
test("resolveInterfaceCapabilityBaseline throws for unknown capability", () => {
    assert.throws(() => resolveInterfaceCapabilityBaseline("unknown"), (err) => err instanceof Error && err.message.includes("interface_capability.not_found"));
});
test("resolveInterfaceCapabilityBaseline throws with correct error format", () => {
    try {
        resolveInterfaceCapabilityBaseline("invalid");
...
//# sourceMappingURL=interface-plane-baseline.test.js.map