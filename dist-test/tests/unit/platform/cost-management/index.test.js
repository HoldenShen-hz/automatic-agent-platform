/**
 * Unit Tests: Cost Management Module
 *
 * Tests the re-exports from the cost-management module which wraps
 * cost estimation services from scale-ecosystem/marketplace/.
 */
import assert from "node:assert/strict";
import test from "node:test";
import { DatabaseSync } from "node:sqlite";
import { CostEstimationService, } from "../../../../src/platform/cost-management/index.js";
/**
...
test("CostEstimate type accepts null divisionId", () => {
    const estimate = {
...
test("CostEstimate basedOn accepts all valid values", () => {
    const basedOnValues = ["division_avg", "global_avg", "default"];
    assert.equal(basedOnValues.length, 3);
});
test("CostEstimationConfig type accepts optional fields", () => {
    const config = {
...
test("CostEstimationConfig type accepts partial configuration", () => {
    const config = {
        defaultCostUsd: 0.15,
    };
...
// Tests for CostEstimationService functionality
...
test("CostEstimationService accepts null divisionId parameter", () => {
    const db = createTestDb();
    db.connection
        .prepare("INSERT INTO tasks (id, division_id, status, created_at) VALUES (?, ?, ?, ?)")
        .run("task_1", "division_a", "done", "2026-04-01T00:00:00.000Z");
    db.connection
        ...
        .run("cost_1", "task_1", "anthropic", "claude-3", 0.10, "2026-04-01T00:00:00.000Z");
    const service = new CostEstimationService(db);
    // Explicitly passing null should use global average
    const result = service.estimate(null);
    assert.equal(result.basedOn, "global_avg");
    assert.equal(result.sampleCount, 1);
});
test("CostEstimationService estimate cost is rounded to 4 decimal places", () => {
    const db = createTestDb();
    db.connection
        .prepare("INSERT INTO tasks (id, division_id, status, created_at) VALUES (?, ?, ?, ?)")
        .run("task_1", "division_test", "done", "2026-04-01T00:00:00.000Z");
