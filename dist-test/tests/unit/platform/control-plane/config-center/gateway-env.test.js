...
import { readTrimmedEnv, readRequiredTrimmedEnv, parseStringRecordJson, loadGatewayEnv, } from "../../../../../src/platform/control-plane/config-center/gateway-env.js";
...
test("readRequiredTrimmedEnv throws ValidationError for missing key", () => {
    const env = {};
    assert.throws(() => readRequiredTrimmedEnv(env, "MISSING"), (err) => err instanceof ValidationError && err.code === "missing_env:MISSING");
});
test("readRequiredTrimmedEnv throws ValidationError for empty string", () => {
    const env = { KEY: "   " };
    assert.throws(() => readRequiredTrimmedEnv(env, "KEY"), (err) => err instanceof ValidationError && err.code === "missing_env:KEY");
});
...
test("parseStringRecordJson throws for empty string (not null)", () => {
    assert.throws(() => parseStringRecordJson("", "test"), (err) => err instanceof SyntaxError);
});
...
test("parseStringRecordJson throws for non-object JSON", () => {
    assert.throws(() => parseStringRecordJson('"string"', "err_code"), (err) => err instanceof ValidationError && err.code === "err_code");
});
test("parseStringRecordJson throws for array JSON", () => {
    assert.throws(() => parseStringRecordJson('["array"]', "err_code"), (err) => err instanceof ValidationError && err.code === "err_code");
});
test("parseStringRecordJson throws SyntaxError for invalid JSON", () => {
    assert.throws(() => parseStringRecordJson("not-json", "err_code"), (err) => err instanceof SyntaxError);
});
test("parseStringRecordJson throws when values are not strings", () => {
    assert.throws(() => parseStringRecordJson('{"key":123}', "err_code"), (err) => err instanceof ValidationError && err.code === "err_code");
});
...
test("loadGatewayEnv uses custom error code for invalid webhook headers structure", () => {
    ...
    assert.throws(() => loadGatewayEnv({
        AA_GATEWAY_WEBHOOK_DEFAULT_HEADERS_JSON: '"not-an-object"',
    }, { invalidWebhookHeadersCode: "custom_error" }), (err) => err instanceof ValidationError && err.code === "custom_error");
});
//# sourceMappingURL=gateway-env.test.js.map