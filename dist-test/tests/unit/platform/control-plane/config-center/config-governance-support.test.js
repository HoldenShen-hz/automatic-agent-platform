...
import { parseJsonObject, normalizeJsonc, validateLayerSchema, validateConfigField, addConfigIssue, stripJsonComments, stripTrailingCommas, stableStringify, mergeConfigObjects, sha256, isPositiveNumber, diffObjects, isPlainObject, BOOTSTRAP_LAYER_SCHEMA, } from "../../../../../src/platform/control-plane/config-center/config-governance-support.js";
...
test("parseJsonObject throws for invalid JSON", () => {
    assert.throws(() => parseJsonObject("not valid json", "test.json"), (error) => error.code.startsWith("config.invalid_json"));
});
test("parseJsonObject throws for non-object JSON", () => {
    assert.throws(() => parseJsonObject('"just a string"', "test.json"), (error) => error.code.startsWith("config.invalid_shape"));
});
test("parseJsonObject throws for arrays", () => {
    assert.throws(() => parseJsonObject("[]", "test.json"), (error) => error.code.startsWith("config.invalid_shape"));
});
test("parseJsonObject throws for null", () => {
    assert.throws(() => parseJsonObject("null", "test.json"), (error) => error.code.startsWith("config.invalid_shape"));
});
...
test("validateConfigField validates string type", () => {
    const schema = { kind: "string", issue: "config.invalid" };
    const issues = [];
...
test("validateConfigField validates string minLength", () => {
    const schema = { kind: "string", issue: "config.invalid", minLength: 3 };
    const issues = [];
...
test("validateConfigField validates boolean type", () => {
    const schema = { kind: "boolean", issue: "config.invalid" };
    const issues = [];
...
test("validateConfigField validates number type", () => {
    const schema = { kind: "number", issue: "config.invalid" };
    const issues = [];
...
test("validateConfigField validates integer constraint", () => {
    const schema = { kind: "number", issue: "config.invalid", integer: true };
    const issues = [];
    assert.equal(validateConfigField(42, schema, issues), true);
    assert.equal(validateConfigField(42.5, schema, issues), false);
});
test("validateConfigField validates minExclusive constraint", () => {
    const schema = { kind: "number", issue: "config.invalid", minExclusive: 0 };
    const issues = [];
...
test("validateConfigField validates object shape", () => {
    const schema = {
        ...
        issue: "config.invalid",
        shape: {
            name: { kind: "string", issue: "config.invalid.name", minLength: 1 },
        },
    };
    const issues = [];
...
test("validateConfigField skips optional undefined fields", () => {
    const schema = {
        ...
        issue: "config.invalid",
        shape: {
            optionalField: { kind: "string", issue: "config.invalid", optional: true },
        },
    };
    const issues = [];
...
test("validateConfigField treats undefined as optional only", () => {
    ...
    assert.equal(schema.optional, undefined);
});
test("validateConfigField allows optional undefined fields", () => {
    const schema = { kind: "string", issue: "config.invalid", optional: true };
    const issues = [];
...
//# sourceMappingURL=config-governance-support.test.js.map