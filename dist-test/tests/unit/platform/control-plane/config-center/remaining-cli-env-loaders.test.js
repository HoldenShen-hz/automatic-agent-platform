...
import { loadTenantPlatformCliEnv, loadEnterpriseCapabilityCliEnv, loadMarketplaceCliEnv, loadDeploymentExecutionCliEnv, loadControlPlaneBalancerCliEnv, loadOpsGovernanceCliEnv, loadSecretManagementCliEnv, loadWorkerRegisterCliEnv, loadGatewayTargetsCliEnv, loadInspectCliEnv, loadSkillCreatorCliEnv, loadShadowSnapshotCliEnv, loadMemoryCliEnv, buildMemoryProviderQuery, buildStructuredMemoryContentFromCliEnv, loadModelRoutingCliEnv, } from "../../../../../src/platform/control-plane/config-center/remaining-cli-env-loaders.js";
...
test("loadTenantPlatformCliEnv throws on missing AA_DB_PATH", () => {
    assert.throws(() => loadTenantPlatformCliEnv({}), (e) => e.code === "missing_env:AA_DB_PATH" && e instanceof ValidationError);
});
test("loadTenantPlatformCliEnv throws on invalid isolation mode", () => {
    assert.throws(() => loadTenantPlatformCliEnv({ AA_DB_PATH: "/data/db", AA_ISOLATION_MODE: "invalid_mode" }), (e) => e.code === "invalid_env:AA_ISOLATION_MODE" && e instanceof ValidationError);
});
...
test("loadEnterpriseCapabilityCliEnv throws on missing AA_DB_PATH", () => {
    assert.throws(() => loadEnterpriseCapabilityCliEnv({}), (e) => e.code === "missing_env:AA_DB_PATH" && e instanceof ValidationError);
});
...
test("loadMarketplaceCliEnv throws on missing AA_DB_PATH", () => {
    assert.throws(() => loadMarketplaceCliEnv({}), (e) => e.code === "missing_env:AA_DB_PATH" && e instanceof ValidationError);
});
...
test("loadDeploymentExecutionCliEnv throws on missing AA_DB_PATH", () => {
    assert.throws(() => loadDeploymentExecutionCliEnv({}), (e) => e.code === "missing_env:AA_DB_PATH" && e instanceof ValidationError);
});
...
test("loadControlPlaneBalancerCliEnv throws on missing AA_DB_PATH", () => {
    assert.throws(() => loadControlPlaneBalancerCliEnv({}), (e) => e.code === "missing_env:AA_DB_PATH" && e instanceof ValidationError);
});
...
test("loadOpsGovernanceCliEnv throws on missing AA_DB_PATH", () => {
    assert.throws(() => loadOpsGovernanceCliEnv({}), (e) => e.code === "missing_env:AA_DB_PATH" && e instanceof ValidationError);
});
test("loadOpsGovernanceCliEnv throws on missing AA_ENVIRONMENT", () => {
    assert.throws(() => loadOpsGovernanceCliEnv({ AA_DB_PATH: "/data/db" }), (e) => e.code === "missing_env:AA_ENVIRONMENT" && e instanceof ValidationError);
});
...
test("loadSecretManagementCliEnv throws on missing AA_DB_PATH", () => {
    assert.throws(() => loadSecretManagementCliEnv({}), (e) => e.code === "missing_env:AA_DB_PATH" && e instanceof ValidationError);
});
...
test("loadWorkerRegisterCliEnv throws on missing AA_DB_PATH", () => {
    assert.throws(() => loadWorkerRegisterCliEnv({}), (e) => e.code === "missing_env:AA_DB_PATH" && e instanceof ValidationError);
});
test("loadWorkerRegisterCliEnv throws on missing AA_WORKER_REGISTER_ACTION", () => {
    assert.throws(() => loadWorkerRegisterCliEnv({ AA_DB_PATH: "/data/db" }), (e) => e.code === "missing_env:AA_WORKER_REGISTER_ACTION" && e instanceof ValidationError);
});
...
test("loadGatewayTargetsCliEnv throws on missing AA_GATEWAY_TARGET_ACTION", () => {
    assert.throws(() => loadGatewayTargetsCliEnv({}), (e) => e.code === "missing_env:AA_GATEWAY_TARGET_ACTION" && e instanceof ValidationError);
});
...
test("loadInspectCliEnv throws on missing AA_INSPECT_KIND", () => {
    assert.throws(() => loadInspectCliEnv({}), (e) => e.code === "missing_env:AA_INSPECT_KIND" && e instanceof ValidationError);
});
...
test("loadSkillCreatorCliEnv throws on missing AA_SKILL_CREATOR_ACTION", () => {
    assert.throws(() => loadSkillCreatorCliEnv({}), (e) => e.code === "missing_env:AA_SKILL_CREATOR_ACTION" && e instanceof ValidationError);
});
...
test("loadShadowSnapshotCliEnv throws on missing AA_WORKSPACE_ROOT", () => {
    assert.throws(() => loadShadowSnapshotCliEnv({ AA_SHADOW_ROOT: "/shadow", AA_SHADOW_SNAPSHOT_ACTION: "create" }), (e) => e.code === "missing_env:AA_WORKSPACE_ROOT" && e instanceof ValidationError);
});
test("loadShadowSnapshotCliEnv throws on missing AA_SHADOW_ROOT", () => {
    assert.throws(() => loadShadowSnapshotCliEnv({ AA_WORKSPACE_ROOT: "/workspace", AA_SHADOW_SNAPSHOT_ACTION: "create" }), (e) => e.code === "missing_env:AA_SHADOW_ROOT" && e instanceof ValidationError);
});
test("loadShadowSnapshotCliEnv throws on missing AA_SHADOW_SNAPSHOT_ACTION", () => {
    assert.throws(() => loadShadowSnapshotCliEnv({ AA_WORKSPACE_ROOT: "/workspace", AA_SHADOW_ROOT: "/shadow" }), (e) => e.code === "missing_env:AA_SHADOW_SNAPSHOT_ACTION" && e instanceof ValidationError);
});
...
test("loadMemoryCliEnv throws on missing AA_MEMORY_ACTION", () => {
    assert.throws(() => loadMemoryCliEnv({}), (e) => e.code === "missing_env:AA_MEMORY_ACTION" && e instanceof ValidationError);
});
...
test("loadModelRoutingCliEnv throws on invalid provider health status", () => {
    assert.throws(() => loadModelRoutingCliEnv({ AA_MODEL_HEALTH_JSON: '{"bad":"unknown"}' }), (e) => e.code === "invalid_env:AA_MODEL_HEALTH_JSON" && e instanceof ValidationError);
});
test("loadModelRoutingCliEnv throws on invalid model health JSON string status", () => {
    assert.throws(() => loadModelRoutingCliEnv({ AA_MODEL_HEALTH_JSON: '{"p":"invalid_status"}' }), (e) => e.code === "invalid_env:AA_MODEL_HEALTH_JSON" && e instanceof ValidationError);
});
test("loadModelRoutingCliEnv throws on invalid fallback lease JSON", () => {
    assert.throws(() => loadModelRoutingCliEnv({ AA_MODEL_ROUTE_FALLBACK_LEASE_JSON: "not-json" }), (e) => e.code.startsWith("invalid_json:") && e instanceof ValidationError);
});
//# sourceMappingURL=remaining-cli-env-loaders.test.js.map