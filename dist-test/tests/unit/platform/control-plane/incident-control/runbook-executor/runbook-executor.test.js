...
import { RunbookExecutor, IncidentDrillService, parseRunbookMarkdown, } from "../../../../../../src/platform/control-plane/incident-control/runbook-executor/index.js";
...
`;
    ...
    const backtickStep = mitigation.steps.find((s) => s.command.includes("kubectl"));
...
`;
    ...
    const mitigation = runbook.sections.find((s) => s.name === "Mitigation");
    assert.ok(mitigation);
    const cliStep = mitigation.steps.find((s) => s.command.includes("kubectl"));
    assert.ok(cliStep);
...
`;
    const runbook = parseRunbookMarkdown(markdown);
    const symptoms = runbook.sections.find((s) => s.name === "Symptoms");
    assert.ok(symptoms);
...
`;
    ...
    const diagnosis = runbook.sections.find((s) => s.name === "Diagnosis");
    assert.ok(diagnosis);
...
`;
    const runbook = parseRunbookMarkdown(markdown);
    const notes = runbook.sections.find((s) => s.name === "Notes");
    assert.ok(notes);
...
test("IncidentDrillService recordObservation creates observation with all fields", () => {
    ...
    drillService.initializeDrill(scenario, ["operator-1"], "initiator");
    drillService.startDrill();
    const observation = drillService.recordObservation("operator-1", "decision", "Correct root cause identified", "good");
...
test("IncidentDrillService generateDrillReport formats correctly", () => {
    ...
    const result = drillService.completeDrill(["Detection time could be improved"], ["Add more monitoring alerts"], [{ criterion: "Detection", passed: true, notes: "OK" }]);
...
//# sourceMappingURL=runbook-executor.test.js.map