...
function seedHappyPathDb(dbPath) {
    const script = `
...
function createWorkspaceScopedStorageQuotaService(workspaceRoot) {
...
function writeDependencyFixtures(workspace) {
...
function createHarness(prefix) {
    ...
    const diagnostics = new DiagnosticsService(new InspectService(store), healthService, new StructuredLogger(), retentionService);
    const stalledDetector = new StalledExecutionDetector(store);
    const doctor = new DoctorService(healthService, checker, new RuntimeRecoveryService(store), stalledDetector, new SqliteReliabilityService(db), `${dbPath}.backup`, new ProtectedGovernanceIntegrityService(), createWorkspaceScopedStorageQuotaService(workspace), new WorkerRegistryService(store), retentionService, new StalledExecutionEscalationService(stalledDetector, diagnostics), new ExecutionResourceMonitor(store), { store });
...
test("EnterpriseGovernanceService returns empty history when no reports exist", () => {
    ...
    try {
        const store = new AuthoritativeTaskStore(db);
        const governance = new OperationsGovernanceService(db, new MetricsService(db, new HealthService(db, store)), new DoctorService(new HealthService(db, store), new StartupConsistencyChecker(db, store, createDefaultStartupConsistencyCheckerOptions()), new RuntimeRecoveryService(store), new StalledExecutionDetector(store), new SqliteReliabilityService(db), `${dbPath}.backup`, new ProtectedGovernanceIntegrityService(), createWorkspaceScopedStorageQuotaService(workspace), new WorkerRegistryService(store), new ObservabilityRetentionService(db), new StalledExecutionEscalationService(new StalledExecutionDetector(store), new DiagnosticsService(new InspectService(store), new HealthService(db, store), new StructuredLogger(), new ObservabilityRetentionService(db))), new ExecutionResourceMonitor(store), { store }), new DiagnosticsService(new InspectService(store), new HealthService(db, store), new StructuredLogger(), new ObservabilityRetentionService(db)));
...
//# sourceMappingURL=enterprise-governance-service.test.js.map