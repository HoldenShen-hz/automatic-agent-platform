...
import { maskSecretValue, validateSecretRef, deriveSecretScope, deriveSecretEnvName, EnvSecretProvider, } from "../../../../../src/platform/control-plane/iam/env-secret-provider.js";
...
test("validateSecretRef rejects invalid format", () => {
    assert.throws(() => validateSecretRef("invalid"), (error) => error.code.startsWith("secret.invalid_ref"));
});
test("validateSecretRef rejects missing scheme", () => {
    assert.throws(() => validateSecretRef("my-service/api-key"), (error) => error.code.startsWith("secret.invalid_ref"));
});
test("validateSecretRef rejects empty string", () => {
    assert.throws(() => validateSecretRef(""), (error) => error.code.startsWith("secret.invalid_ref"));
});
...
test("EnvSecretProvider.requireSecret throws for missing secret", async () => {
    const provider = new EnvSecretProvider({ env: {} });
    await assert.rejects(async () => provider.requireSecret("secret://test/key"), (error) => error.code.startsWith("secret.missing_value"));
});
...
test("EnvSecretProvider.requireSecretSync throws for missing secret", () => {
    const provider = new EnvSecretProvider({ env: {} });
    assert.throws(() => provider.requireSecretSync("secret://test/key"), (error) => error.code.startsWith("secret.missing_value"));
});
