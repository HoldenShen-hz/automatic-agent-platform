...
test("[SYS-PERF-3.1] log write does not block event loop > 1ms", async () => {
    ...
    try {
        ...
        assert.ok(avgLatency < 1.0, `Average log write latency must be < 1ms to avoid blocking event loop. Got: ${avgLatency.toFixed(3)}ms`);
    }
    finally {
        // Clean up
        StructuredLogger.configureGlobalFileSink(null);
...
test("[SYS-PERF-3.1] individual log write completes in reasonable time", async () => {
    ...
    try {
        ...
        assert.ok(latencyMs < 10, `Single log write should complete in < 10ms, got ${latencyMs.toFixed(3)}ms`);
    }
    finally {
        StructuredLogger.configureGlobalFileSink(null);
        await rm(tempDir, { recursive: true, force: true });
...
test("[SYS-PERF-3.1] concurrent log writes don't block excessively", async () => {
    ...
    try {
        ...
        assert.ok(avgLatencyPerWrite < 1.0, `Average per-write latency should be < 1ms with async I/O. Got: ${avgLatencyPerWrite.toFixed(3)}ms`);
    }
    finally {
        StructuredLogger.configureGlobalFileSink(null);
        await rm(tempDir, { recursive: true, force: true });
...
test("[SYS-PERF-3.1] file sink uses async appendFile not sync", async () => {
    ...
    try {
        ...
        assert.ok(maxLatency < 5.0, `Max write latency should be < 5ms (indicating async I/O). Got: ${maxLatency.toFixed(3)}ms`);
    }
    finally {
        StructuredLogger.configureGlobalFileSink(null);
        await rm(tempDir, { recursive: true, force: true });
...
test("[SYS-PERF-3.1] ring buffer insertion is O(1)", async () => {
    ...
    try {
        ...
        assert.ok(ratio < 5, `Large buffer should not be significantly slower. Ratio: ${ratio.toFixed(2)}`);
    }
    finally {
        StructuredLogger.configureGlobalFileSink(null);
        await rm(tempDir, { recursive: true, force: true });
...
//# sourceMappingURL=structured-logger-perf.test.js.map