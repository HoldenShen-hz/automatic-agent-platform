...
import { PagerDutyAlertChannel, WebhookAlertChannel, } from "../../../../src/platform/shared/observability/slo-alerting-service.js";
import { runtimeMetricsRegistry } from "../../../../src/platform/shared/observability/runtime-metrics-registry.js";
function createEvent(channelKind) {
    return {
...
test("[SYS-REL-2.5] WebhookAlertChannel increments runtime failure counter on async delivery failure", async () => {
    ...
    assert.deepEqual(counters.map((series) => ({ labels: series.labels, value: series.value })), [{ labels: { channel: "webhook" }, value: 1 }]);
});
test("[SYS-REL-2.5] PagerDutyAlertChannel increments runtime failure counter on async delivery failure", async () => {
    ...
    await new Promise((resolve) => setTimeout(resolve, 0));
    const counters = runtimeMetricsRegistry.getCounters("alert_delivery_failures_total");
    assert.deepEqual(counters.map((series) => ({ labels: series.labels, value: series.value })), [{ labels: { channel: "pagerduty" }, value: 1 }]);
});
test("[SYS-REL-2.5] PagerDuty delivery failure logs error and increments counter", async () => {
    const logs = [];
    const counters = { alert_delivery_failures_total: 0 };
    ...
    const deliverPagerDuty = async (event) => {
    ...
    assert.strictEqual(logs[0]?.message, "alert.delivery_failed", "Log message should be 'alert.delivery_failed'");
...
test("[SYS-REL-2.5] Slack delivery failure logs error and increments counter", async () => {
    ...
    const deliverSlack = async (event) => {
...
test("[SYS-REL-2.5] Webhook delivery failure logs error and increments counter", async () => {
    ...
    const deliverWebhook = async (event) => {
...
test("[SYS-REL-2.5] OpsGenie delivery failure logs error and increments counter", async () => {
    ...
    const deliverOpsGenie = async (event) => {
...
test("[SYS-REL-2.5] alert dispatcher must track delivery failures", async () => {
    ...
    const trackDeliveryFailure = (alertId, channelKind, error) => {
    ...
    assert.strictEqual(deliveryFailures.filter((f) => f.channelKind === "pagerduty").length, 1, "Should have 1 PagerDuty failure");
    assert.strictEqual(deliveryFailures.filter((f) => f.channelKind === "slack").length, 1, "Should have 1 Slack failure");
});
test("[SYS-REL-2.5] alert service summary includes delivery failure metrics", () => {
    ...
    assert.strictEqual(result.deliverySuccessRate, 0.8, "Delivery success rate should be 80%");
});
//# sourceMappingURL=slo-alerting-delivery.test.js.map