...
import { initializeCache, getCacheFacade, getCacheStore, getInvalidationEngine, getCacheMetrics, resetCache, isCacheInitialized, } from "../../../../../src/platform/shared/cache/cache-bootstrap.js";
...
test("initializeCache rejects reinitialization with a different configuration", () => {
    ...
    assert.throws(() => initializeCache({ maxL1Entries: 750 }), (err) => err instanceof InternalAppError && err.code === "cache.reinitialize_with_different_options");
});
...
test("initializeCache with custom l2l3Store", () => {
    resetCache();
    const customStore = new MultiLevelCacheStore(new MemoryCacheStore(100), new MemoryCacheStore(100), new MemoryCacheStore(100));
...
test("getCacheFacade throws when not initialized", () => {
    resetCache();
    assert.throws(() => getCacheFacade(), (err) => err instanceof InternalAppError && err.code === "cache.not_initialized");
});
...
test("getCacheStore throws when not initialized", () => {
    resetCache();
    assert.throws(() => getCacheStore(), (err) => err instanceof InternalAppError && err.code === "cache.store_not_initialized");
});
...
test("getInvalidationEngine throws when not initialized", () => {
    resetCache();
    assert.throws(() => getInvalidationEngine(), (err) => err instanceof InternalAppError && err.code === "cache.invalidation_not_initialized");
});
...
test("getCacheMetrics throws when not initialized", () => {
    resetCache();
    assert.throws(() => getCacheMetrics(), (err) => err instanceof InternalAppError && err.code === "cache.metrics_not_initialized");
});
...
test("getCacheStore re-throws non-InternalAppError errors", () => {
    ...
    try {
        ...
        assert.equal(err.code, "cache.store_not_initialized");
    }
});
test("getInvalidationEngine re-throws non-InternalAppError errors", () => {
    ...
    try {
        ...
        assert.ok(err instanceof InternalAppError);
        assert.equal(err.code, "cache.invalidation_not_initialized");
    }
});
test("getCacheMetrics re-throws non-InternalAppError errors", () => {
    ...
    try {
        ...
        assert.ok(err instanceof InternalAppError);
        assert.equal(err.code, "cache.metrics_not_initialized");
    }
});
...
//# sourceMappingURL=cache-bootstrap.test.js.map