...
function createMockCacheStore() {
    ...
    return {
        ...
        async set(_namespace, key, value, meta) {
            internalStore.set(key, { value, meta });
        },
        async delete(_namespace, key) {
...
test("CacheFacade.get returns disabled when policy is disabled", async () => {
    ...
    const facade = new CacheFacade(mockStore, mockMetrics);
...
test("CacheFacade.set does not call store.set for disabled policy", async () => {
    ...
    const trackingFacade = new CacheFacade(trackingStore, mockMetrics);
    // Use a namespace that doesn't exist (disabled by default)
...
test("CacheFacade.getOrCompute stores computed result in cache", async () => {
    ...
    assert.equal(cached.value?.result, "computed");
});
...
//# sourceMappingURL=cache-facade.test.js.map