...
function createIsolatedRegistry() {
...
test("ServiceRegistry returns same instance on multiple gets", async () => {
    ...
    const instance1 = registry.get("singleton-service");
    const instance2 = registry.get("singleton-service");
...
test("ServiceRegistry throws for unregistered service", async () => {
    const registry = ServiceRegistry.getInstance();
    await registry.reset();
    assert.throws(() => registry.get("nonexistent"), (error) => error.code === "service_registry.not_registered");
});
