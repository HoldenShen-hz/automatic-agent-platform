...
import { createResourceQuota, canAllocate, calculateBurstCapacity, inheritQuota, DEFAULT_RESOURCE_ALLOCATION, } from "../../../../src/platform/shared/scaling/resource-quota.js";
import { PriorityScheduler, PRIORITY_CLASSES, canPreempt, findTaskToPreempt, parseTimeoutToMs, hasExceededTimeout, } from "../../../../src/platform/shared/scaling/priority-scheduler.js";
...
test("canAllocate admits when within guaranteed limits", () => {
    ...
    const requested = { maxConcurrentWorkflows: 2 };
...
test("canAllocate rejects when exceeding max limit", () => {
    ...
    const requested = { maxConcurrentWorkflows: 5 };
    const result = canAllocate(quota, usage, requested);
...
test("canAllocate rejects when exceeding LLM token limit", () => {
    ...
    const requested = { llmTokensPerMinute: 2000 };
    const result = canAllocate(quota, usage, requested);
    assert.strictEqual(result.admitted, false);
...
test("canAllocate returns correct availableQuota on admission", () => {
    ...
    const requested = { maxConcurrentWorkflows: 1 };
    const result = canAllocate(quota, usage, requested);
    assert.strictEqual(result.admitted, true);
