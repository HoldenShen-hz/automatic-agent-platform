...
import { PagerDutyAlertChannel, SlackAlertChannel, OpsGenieAlertChannel, WebhookAlertChannel, } from "../../../../../src/platform/shared/observability/slo-alerting-service.js";
...
function getCounterValue(name, labels) {
    ...
    const match = counters.find(c => Object.entries(labels).every(([k, v]) => c.labels[k] === v));
...
async function captureErrorLogs(action) {
    const entries = [];
...
function findDeliveryFailureLog(entries, channel, alertId) {
    return entries.find((entry) => entry.message === "alert.delivery_failed"
...
test("[SYS-REL-2.5] PagerDuty delivery failure increments failure counter", async () => {
    ...
    assert.equal(afterCount, initialCount + 1, "PagerDuty failure must increment alert_delivery_failures_total counter");
});
test("[SYS-REL-2.5] PagerDuty delivery failure logs error via StructuredLogger", async () => {
    ...
    assert.ok(findDeliveryFailureLog(logs, "pagerduty"), "PagerDuty delivery failure must be logged via StructuredLogger");
});
...
test("[SYS-REL-2.5] Slack delivery failure increments failure counter", async () => {
    ...
    assert.equal(afterCount, initialCount + 1, "Slack failure must increment alert_delivery_failures_total counter");
});
...
test("[SYS-REL-2.5] OpsGenie delivery failure increments failure counter", async () => {
    ...
    assert.equal(afterCount, initialCount + 1, "OpsGenie failure must increment alert_delivery_failures_total counter");
});
...
test("[SYS-REL-2.5] Webhook delivery failure increments failure counter", async () => {
    ...
    assert.equal(afterCount, initialCount + 1, "Webhook failure must increment alert_delivery_failures_total counter");
});
...
test("[SYS-REL-2.5] Multiple concurrent delivery failures all increment counter", async () => {
    ...
    const events = Array.from({ length: 5 }, (_, i) => createTestEvent({ id: `concurrent-${i}`, channelKind: "pagerduty" }));
    events.forEach(event => {
        channel.deliver(event, { routingKey: "test-key" });
    });
    await new Promise((resolve) => setTimeout(resolve, 100));
    const afterCount = getCounterValue("alert_delivery_failures_total", { channel: "pagerduty" });
    assert.equal(afterCount, initialCount + 5, "All 5 concurrent failures must increment counter (actual delta: " + (afterCount - initialCount) + ")");
});
//# sourceMappingURL=slo-alerting-error.test.js.map