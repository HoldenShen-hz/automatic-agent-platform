...
test("metrics service returns empty metrics structure when no data exists", () => {
    ...
    try {
        ...
        const summary = new MetricsService(db, new HealthService(db, store, {
...
function seedWorkflowState(store, input) {
...
test("metrics service summarizes success recovery retry cost and backlog signals", () => {
    ...
    try {
        ...
        db.connection.prepare(`UPDATE tasks SET status = ?, updated_at = ? WHERE id = ?`).run("awaiting_decision", "2026-04-06T08:10:00.000Z", "task-metrics-awaiting");
        db.connection.prepare(`UPDATE executions SET status = ?, updated_at = ? WHERE id = ?`).run("created", "2026-04-06T08:10:00.000Z", "exec-metrics-awaiting");
        store.insertStepOutput({
...
test("metrics service calculates approval task trigger rate correctly", () => {
    ...
    try {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            db.connection.prepare(`UPDATE tasks SET status = ?, updated_at = ? WHERE id = ?`).run("awaiting_decision", "2026-04-06T08:00:00.000Z", `task-approval-${i}`);
        }
...
test("metrics service calculates recovery success rate correctly", () => {
    ...
    try {
        ...
        db.connection.prepare(`UPDATE tasks SET status = ?, updated_at = ? WHERE id = ?`).run("done", "2026-04-06T08:00:00.000Z", "task-recovered");
        store.insertEvent({
            id: newId("evt"),
            taskId: "task-recovered",
            executionId: "exec-recovered",
        ...
        db.connection.prepare(`UPDATE tasks SET status = ?, updated_at = ? WHERE id = ?`).run("failed", "2026-04-06T08:00:00.000Z", "task-dead-lettered");
        store.insertEvent({
            id: newId("evt"),
            taskId: "task-dead-lettered",
            executionId: "exec-dead-lettered",
