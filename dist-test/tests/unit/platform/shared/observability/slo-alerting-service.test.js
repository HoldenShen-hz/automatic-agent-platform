...
import { SloAlertingService, LogAlertChannel, OpsGenieAlertChannel, PagerDutyAlertChannel, SlackAlertChannel, WebhookAlertChannel, SLO_ALERTING_DDL, } from "../../../../../src/platform/shared/observability/slo-alerting-service.js";
...
test("webhook alert channel delivers to mock URL and returns success", async () => {
    ...
    assert.equal(receivedBody.id, "alert_test_1");
    assert.equal(receivedBody.title, "Critical Alert");
});
...
test("slack alert channel sends structured payload", async () => {
    ...
    assert.equal(result.delivered, true);
    assert.match(String(receivedBody.text), /Provider spike/);
});
...
test("pagerduty alert channel sends events v2 payload", async () => {
    ...
    assert.equal(receivedBody.routing_key, "routing-key-1");
    assert.equal(receivedBody.payload.summary, "Queue backlog breach");
});
...
test("pagerduty alert channel uses custom dedup key when provided", async () => {
    ...
    assert.equal(receivedBody.dedup_key, "custom-dedup-key");
});
test("pagerduty alert channel generates dedup key from rule and event when not provided", async () => {
    ...
    assert.equal(receivedBody.dedup_key, "rule_auto_dedup:alert_pd_auto_dedup");
});
test("pagerduty alert channel maps severity correctly", async () => {
    ...
    assert.equal(receivedBody.payload.severity, "critical");
    ...
    assert.equal(receivedBody.payload.severity, "warning");
});
test("pagerduty alert channel resolves alert with event_action resolve", async () => {
    ...
    channel.deliver(event, { routingKey: "routing-key-1" });
    assert.equal(receivedBody.event_action, "resolve");
});
...
test("slack alert channel sends correct severity in payload", async () => {
    ...
    assert.ok(receivedBody.text.includes("[CRITICAL]"));
});
...
//# sourceMappingURL=slo-alerting-service.test.js.map