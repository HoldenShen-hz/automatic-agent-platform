...
import { buildIncidentTimelineMarkdown, extractRemoteAuthorityViolations, toIncidentTimelineEntry, countIncidentEntries, resolveDurationMs, inferTimelineSeverity, previewText, safeParseRecord, buildDiagnosticWarningSummary, classifyDiagnosticWarning, resolveHighestWarningSeverity, warningSeverityRank, selectRemoteTimelineEntries, extractRemoteWorkerIds, buildTraceSummary, } from "../../../../../src/platform/shared/observability/diagnostics-support.js";
...
test("inferTimelineSeverity returns warning for blocked dispatch", () => {
    const entry = { kind: "dispatch", title: "", summary: "", status: "blocked", data: {}, occurredAt: "", id: "" };
    assert.equal(inferTimelineSeverity(entry), "warning");
});
test("inferTimelineSeverity returns critical for remote_log error", () => {
    const entry = { kind: "remote_log", title: "", summary: "", data: { level: "error" }, occurredAt: "", id: "" };
    assert.equal(inferTimelineSeverity(entry), "critical");
});
test("inferTimelineSeverity returns warning for remote_log warn", () => {
    const entry = { kind: "remote_log", title: "", summary: "", data: { level: "warn" }, occurredAt: "", id: "" };
    assert.equal(inferTimelineSeverity(entry), "warning");
});
test("inferTimelineSeverity returns info for remote_log info", () => {
    const entry = { kind: "remote_log", title: "", summary: "", data: { level: "info" }, occurredAt: "", id: "" };
...
test("inferTimelineSeverity returns warning for requested approval", () => {
    const entry = { kind: "approval", title: "", summary: "", status: "requested", data: {}, occurredAt: "", id: "" };
    assert.equal(inferTimelineSeverity(entry), "warning");
});
test("inferTimelineSeverity returns info by default", () => {
    const entry = { kind: "event", title: "other", summary: "", data: {}, occurredAt: "", id: "" };
    assert.equal(inferTimelineSeverity(entry), "info");
});
...
test("extractRemoteAuthorityViolations extracts violations", () => {
    ...
    const result = extractRemoteAuthorityViolations(events);
...
//# sourceMappingURL=diagnostics-support.test.js.map