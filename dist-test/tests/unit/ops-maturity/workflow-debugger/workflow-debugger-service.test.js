...
function createProdActor() {
    return { actorId: "sre-1", canDebugProduction: true };
}
function createNonProdActor() {
    return { actorId: "viewer-1", canDebugProduction: false };
}
function createBreakpoint(id = "bp-1", workflowId = "wf-123", stepSelector = "deploy") {
    return {
        breakpointId: id,
        workflowId,
        stepSelector,
...
test.describe("WorkflowDebuggerService", () => {
    test.describe("registerBreakpoint", () => {
        ...
        test("throws when non-authorized actor registers prod breakpoint", () => {
            const service = new WorkflowDebuggerService();
            const bp = createBreakpoint();
            assert.throws(() => service.registerBreakpoint(createNonProdActor(), "prod", bp), /workflow_debugger\.prod_breakpoint_forbidden/);
        });
        test("throws with correct actorId in error message", () => {
            const service = new WorkflowDebuggerService();
            const actor = { actorId: "unauthorized-user", canDebugProduction: false };
            const bp = createBreakpoint();
            assert.throws(() => service.registerBreakpoint(actor, "prod", bp), /unauthorized-user/);
        });
    ...
    test.describe("evaluateTrace", () => {
        ...
        test("uses correct action from breakpoint definition", () => {
            const service = new WorkflowDebuggerService();
            const bp = { ...createBreakpoint("bp-action", "wf-123", "deploy"), action: "snapshot" };
    ...
    test.describe("buildComparisonReport", () => {
        ...
        test("includes workflowId in report", () => {
            const service = new WorkflowDebuggerService();
            const report = service.buildComparisonReport("wf-special", [], []);
...
//# sourceMappingURL=workflow-debugger-service.test.js.map