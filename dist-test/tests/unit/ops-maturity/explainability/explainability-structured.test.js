...
renderStageExplanation, renderPlainTextExplanation, renderStructuredExplanation, renderForAudience, buildDecisionTree, 
// From simplified-explainer
simplifyExplanation, formatAsMarkdown, formatAsNotification, } from "../../../../src/ops-maturity/explainability/index.js";
describe("ExplanationRenderer - Structured Format", () => {
    ...
    describe("renderStructuredExplanation", () => {
        test("should return valid JSON", () => {
            const json = renderStructuredExplanation("execute", "Task completed", [{ source: "a", target: "b", rationale: "reason" }], ["evidence1"], ["factor1"]);
    ...
    describe("renderPlainTextExplanation", () => {
        test("should render multi-line plain text", () => {
            const result = renderPlainTextExplanation("assess", "Analysis completed", [{ source: "input", target: "output", rationale: "transformed" }], ["data_source_1"], ["performance_metric"]);
    ...
    describe("renderForAudience", () => {
        test("should use structured format for technical audience", () => {
            const result = renderForAudience("execute", "Task completed", [], [], [], "technical");
            // Should be JSON
            const parsed = JSON.parse(result);
            assert.strictEqual(parsed.format, "decision_tree");
        });
        test("should use plain text for business audience", () => {
            const result = renderForAudience("execute", "Task completed", [], [], [], "business");
            assert.ok(!result.startsWith("{"));
            assert.ok(result.includes("Task completed"));
        });
        test("should use structured format for audit audience", () => {
            const result = renderForAudience("execute", "Task completed", [], [], [], "audit");
            const parsed = JSON.parse(result);
            assert.strictEqual(parsed.format, "decision_tree");
        });
    });
...
describe("SimplifiedExplainer - Non-Technical Users", () => {
    describe("simplifyExplanation", () => {
        test("should generate headline with stage name", () => {
            const result = simplifyExplanation("execute", "Task completed successfully with all validations passed", ["cost_under_budget", "risk_level_low"], [], "low");
        ...
        test("should calculate confidence based on factors and links", () => {
            ...
            const withManyFactors = simplifyExplanation("execute", "Task done", ["factor1", "factor2", "factor3", "factor4", "factor5"], [], "low");
        ...
        test("should suggest action for approval stages", () => {
            const result = simplifyExplanation("approval", "Review required for production deployment", [], [], "medium");
        ...
        test("should explain why it matters for cost-related factors", () => {
            const result = simplifyExplanation("execute", "Budget analysis completed", ["cost_under_budget", "resource_usage_normal"], [], "low");
    ...
    describe("formatAsMarkdown", () => {
        test("should format as markdown with sections", () => {
            const explanation = simplifyExplanation("execute", "Task completed successfully", ["performance_good"], [], "low");
    ...
    describe("jargon simplification", () => {
        test("should replace technical terms", () => {
            const result = simplifyExplanation("execute", "The workflow has been deployed and is now running in production", ["deployment_successful", "no_errors"], [], "low");
            // Should use simplified terms
            assert.ok(result.whatHappened.includes("process") || result.whatHappened.includes("release") || !result.whatHappened.includes("workflow"));
        });
    });
});
describe("DecisionTreeNode structure", () => {
    test("should have correct node types", () => {
        const tree = buildDecisionTree("Test Decision", [{ source: "因子A", target: "结果B", rationale: "原因说明" }], ["证据1"], ["因素1"]);
...
//# sourceMappingURL=explainability-structured.test.js.map