...
test("ops-maturity support modules provide contract-aligned helpers", () => {
    ...
    assert.deepEqual(mapEvidenceByType([
        { evidenceId: "ev_1", evidenceType: "audit_log" },
        { evidenceId: "ev_2", evidenceType: "audit_log" },
    ]), { audit_log: ["ev_1", "ev_2"] });
    assert.match(renderComplianceReportMarkdown("SOC2", [{ title: "Controls", lines: ["AC-1", "AC-2"] }]), /SOC2/);
    assert.equal(findComplianceTemplate([{ templateId: "soc2", framework: "SOC2", reportType: "monthly" }], "soc2")?.framework, "SOC2");
    ...
    assert.equal(selectEdgeLocalModel([{ modelId: "local-vision", modalities: ["image", "text"] }], "image")?.modelId, "local-vision");
    assert.equal(orderEdgeSyncQueue([{ envelopeId: "a", priority: 1 }, { envelopeId: "b", priority: 3 }])[0]?.envelopeId, "b");
    assert.equal(buildForensicSnapshot({
        snapshotId: "snap_1",
        scope: "platform",
    ...
    assert.deepEqual(buildCausalChainSummary([{ source: "observe", target: "execute", rationale: "incident threshold exceeded" }]), ["observe -> execute: incident threshold exceeded"]);
    ...
    assert.deepEqual(compareWorkflowRuns([{ stepId: "step_1", status: "done" }], [{ stepId: "step_1", status: "failed" }]), ["step:step_1:done->failed"]);
    assert.deepEqual(renderWorkflowTimeline([{ timestamp: "2026-04-20T00:00:00.000Z", label: "started" }]), ["2026-04-20T00:00:00.000Z started"]);
});
