...
import { PackLifecycleOrchestrationService, PackPluginCompatibilityService, PackScaffoldService, PackTestLocalService, summarizeCapabilityMatrix, validateBusinessPackManifest, } from "../../../../src/sdk/pack-sdk/index.js";
// ============================================================================
// pack-manifest tests
// ============================================================================
...
test("validateBusinessPackManifest rejects packId with only whitespace", () => {
    assert.throws(() => validateBusinessPackManifest({
        packId: "   ",
        version: "1.0.0",
        domain: "testing",
        owner: "test@example.com",
    ...
    }), (error) => error instanceof ValidationError && error.code === "pack_sdk.invalid_pack_id");
});
test("validateBusinessPackManifest rejects empty capabilities array", () => {
    assert.throws(() => validateBusinessPackManifest({
        ...
        owner: "test@example.com",
        capabilities: [],
    }), (error) => error instanceof ValidationError && error.code === "pack_sdk.empty_capabilities");
});
...
test("PackScaffoldService.scaffold rejects invalid pack ID with uppercase", () => {
    ...
    assert.throws(() => service.scaffold({
        ...
        owner: "test@example.com",
        riskLevel: "low",
    }), (error) => error instanceof ValidationError && error.code === "pack_scaffold.invalid_pack_id_format");
});
...
test("PackScaffoldService.scaffold rejects empty name", () => {
    ...
    assert.throws(() => service.scaffold({
        ...
        owner: "test@example.com",
        riskLevel: "low",
    }), (error) => error instanceof ValidationError && error.code === "pack_scaffold.invalid_name");
});
test("PackScaffoldService.scaffold rejects empty owner", () => {
    ...
    assert.throws(() => service.scaffold({
        ...
        owner: "   ",
        riskLevel: "low",
    }), (error) => error instanceof ValidationError && error.code === "pack_scaffold.invalid_owner");
});
test("PackScaffoldService.scaffold accepts valid pack ID starting with number", () => {
    const service = new PackScaffoldService();
    assert.doesNotThrow(() => service.scaffold({
        packId: "123-pack",
        name: "Test Pack",
        template: "minimal",
        domain: "testing",
...
test("PackTestLocalService.validateTestOptions rejects empty packId", async () => {
    ...
    await assert.rejects(() => service.test(options), /Pack ID/i);
});
test("PackTestLocalService.validateTestOptions rejects empty version", async () => {
    ...
    await assert.rejects(() => service.test(options), /Version/i);
});
test("PackTestLocalService.validateTestOptions rejects negative timeout", async () => {
    ...
    await assert.rejects(() => service.test(options), /Timeout/i);
});
...
test("PackTestLocalService.test adds coverage_below_threshold finding when coverage low", async () => {
    ...
    const report = await service.test(options);
    assert.ok(report.findings.some((f) => f.includes("coverage_below_threshold")) || report.coveragePercent >= 80, "Expected coverage finding or acceptable coverage");
});
// ============================================================================
...
test("PackPluginCompatibilityService.inspectBuiltinPlugin returns valid entry for known plugin", () => {
    ...
    if (githubAdapter) {
        ...
        assert.ok(["initialize", "healthCheck", "shutdown"].some((h) => githubAdapter.lifecycleHooks.includes(h)));
    }
});
...
test("PackPluginCompatibilityService.evaluateManifest returns availablePlugins sorted", () => {
    ...
    for (let i = 1; i < report.availablePlugins.length; i++) {
        assert.ok(report.availablePlugins[i - 1].pluginId <= report.availablePlugins[i].pluginId, "Available plugins should be sorted");
    }
});
// ============================================================================
...
test("PackLifecycleOrchestrationService.registerPack rejects duplicate registration", () => {
    ...
    assert.throws(() => service.registerPack({ manifest, owner: "test@example.com", evalDatasetIds: ["ds1"] }), (error) => error instanceof ValidationError && error.code === "pack_lifecycle.already_registered:duplicate-pack@1.0.0");
});
test("PackLifecycleOrchestrationService.recordTesting rejects unknown pack", () => {
    const service = new PackLifecycleOrchestrationService();
    assert.throws(() => service.recordTesting({
        packId: "unknown-pack",
        version: "1.0.0",
        coveragePercent: 90,
    ...
    }), (error) => error instanceof ValidationError && error.code.startsWith("pack_lifecycle.not_found"));
});
...
test("PackLifecycleOrchestrationService.certifyPack rejects pack with a failing test report", () => {
    ...
    assert.throws(() => service.certifyPack({
        packId: "no-test-pack",
        version: "1.0.0",
    ...
    }), (error) => error instanceof ValidationError && error.code === "pack_lifecycle.testing_not_passed:no-test-pack@1.0.0");
});
...
test("PackLifecycleOrchestrationService.publishPack rejects packs before certification", () => {
    ...
    assert.throws(() => service.publishPack({
        packId: "unpublished-pack",
        version: "1.0.0",
        strategy: "shadow",
        owner: "release@example.com",
    }), (error) => error instanceof ValidationError && error.code === "pack_lifecycle.invalid_transition:unpublished-pack@1.0.0");
});
...
test("PackLifecycleOrchestrationService.deprecatePack rejects support window under 180 days", () => {
    ...
    assert.throws(() => service.deprecatePack({
        packId: "short-window-pack",
        version: "1.0.0",
        owner: "test@example.com",
    ...
    }), (error) => error instanceof ValidationError && error.code === "pack_lifecycle.support_window_too_short:short-window-pack@1.0.0");
});
test("PackLifecycleOrchestrationService.archivePack rejects non-deprecated pack", () => {
    ...
    assert.throws(() => service.archivePack("not-deprecated-pack", "1.0.0"), (error) => error instanceof ValidationError && error.code === "pack_lifecycle.archive_requires_deprecated:not-deprecated-pack@1.0.0");
});
