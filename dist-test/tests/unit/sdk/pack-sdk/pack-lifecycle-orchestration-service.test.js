...
test("PackLifecycleOrchestrationService blocks certification when compatibility fails", () => {
    ...
    assert.ok(certified.findings.includes("pack_lifecycle.license_blocked")
...
test("PackLifecycleOrchestrationService rejects publish before certification", () => {
    ...
    assert.throws(() => service.publishPack({
        ...
        strategy: "shadow",
        owner: "release@example.com",
    }), (error) => error instanceof ValidationError && error.code === "pack_lifecycle.invalid_transition:ops-pack@1.0.0");
});
test("PackLifecycleOrchestrationService enforces minimum deprecation support window", () => {
    ...
    }), (error) => error instanceof ValidationError && error.code === "pack_lifecycle.support_window_too_short:ops-pack@1.0.0");
});
//# sourceMappingURL=pack-lifecycle-orchestration-service.test.js.map