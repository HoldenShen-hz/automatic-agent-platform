...
import { deriveCliWorkspaceRoot, openCliAuthoritativeStorageContext, openCliAuthoritativeStorageContextAsync, withCliStorage, } from "../../../../src/sdk/cli/authoritative-storage.js";
...
test("cli authoritative storage async context fail-closes postgres-backed authoritative task store access", async () => {
    ...
    try {
        ...
        await assert.rejects(async () => openCliAuthoritativeStorageContextAsync(dbPath), /storage\.backend_config_invalid.*storage\.postgres/);
    }
    finally {
...
test("cli authoritative storage sync context fail-closes postgres mode without shadow sqlite", () => {
    ...
    try {
        ...
        delete process.env.AA_STORAGE_POSTGRES_DUAL_RUN;
        delete process.env.AA_STORAGE_POSTGRES_SHADOW_SQLITE_PATH;
        assert.throws(() => openCliAuthoritativeStorageContext(), /storage\.backend_config_invalid.*postgres.*dual_run_required/);
    }
    finally {
        if (previousEnv.AA_DB_PATH == null) {
            delete process.env.AA_DB_PATH;
...
test("deriveCliWorkspaceRoot returns workspace parent for standard sqlite layout", () => {
    assert.equal(deriveCliWorkspaceRoot("/tmp/aa-workspace/data/sqlite/runtime.db"), "/tmp/aa-workspace");
    assert.equal(deriveCliWorkspaceRoot("/tmp/custom/runtime.db"), "/tmp/custom");
});
test("withCliStorage migrates and closes the storage automatically", () => {
    ...
    try {
        ...
        const verify = openCliAuthoritativeStorageContext(dbPath);
        verify.migrate();
...
//# sourceMappingURL=authoritative-storage.test.js.map