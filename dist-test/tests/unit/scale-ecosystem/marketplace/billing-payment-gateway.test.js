...
import { ManualBillingPaymentGateway, StripeBillingPaymentGateway, PaddleBillingPaymentGateway, } from "../../../../src/scale-ecosystem/marketplace/billing-payment-gateway.js";
 ...
 */
function createCheckoutInput(invoice, account) {
    return {
...
test("StripeBillingPaymentGateway.createCheckoutSession handles API error response", async () => {
    ...
    const input = createCheckoutInput();
    await assert.rejects(async () => gateway.createCheckoutSession(input), (err) => {
...
test("StripeBillingPaymentGateway.fetchPaymentSessionStatus handles API errors with retryable flag", async () => {
    ...
    const invoice = createTestInvoice();
    const account = createTestAccount();
    await assert.rejects(async () => gateway.fetchPaymentSessionStatus({ session, invoice, account }), (err) => {
        assert.ok(err instanceof ProviderError);
...
test("All gateways implement BillingPaymentGateway interface kind property", () => {
    ...
    const expectedKinds = ["manual", "stripe", "paddle"];
...
//# sourceMappingURL=billing-payment-gateway.test.js.map