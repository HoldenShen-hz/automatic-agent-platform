...
test("pmf validation service rejects malformed run options", () => {
    ...
    try {
        ...
        assert.throws(() => service.buildReport({ profileName: "bad profile" }), /pmf\.invalid_profile_name/);
        assert.throws(() => service.buildReport({ divisionId: "../bad-scope" }), /pmf\.invalid_division_id/);
        assert.throws(() => service.buildReport({ windowDays: 0 }), /pmf\.invalid_window_days/);
        db.close();
    }
    finally {
        cleanupPath(workspace);
