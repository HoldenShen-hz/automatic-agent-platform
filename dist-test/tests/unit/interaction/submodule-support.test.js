...
test("interaction support modules provide deterministic helper behavior", () => {
    ...
    assert.deepEqual(topologicallySortTaskIds(["a", "b", "c"], [{ fromTask: "a", toTask: "b" }, { fromTask: "b", toTask: "c" }]), ["a", "b", "c"]);
    assert.deepEqual(buildExecutionBatches(["a", "b", "c"], [{ fromTask: "a", toTask: "c" }]), [["a", "b"], ["c"]]);
    ...
    assert.deepEqual(resolveRequiredSlots([{ entityType: "date", value: "2026-04-20", normalized: "2026-04-20", sourceSpan: [0, 10] }], ["date", "money"]).missing, ["money"]);
    ...
    assert.equal(canAdvanceWizard({ sessionId: "wiz_1", currentStepId: "step_1", steps: [{ stepId: "step_1", title: "Step 1", completed: true }] }), true);
});
//# sourceMappingURL=submodule-support.test.js.map