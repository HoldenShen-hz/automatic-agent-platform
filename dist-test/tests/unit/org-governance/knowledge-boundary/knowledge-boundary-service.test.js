...
test("KnowledgeBoundaryService evaluates access and redacts audit logs", () => {
    ...
    const denied = service.evaluateAccess(boundary, "user_1", "dept_hr", "investigate", [], undefined, "2026-04-20T00:00:00.000Z");
    assert.equal(denied.allowed, false);
    const granted = service.evaluateAccess(boundary, "user_2", "dept_hr", "audit", [{
            grantId: "grant_1",
            boundaryId: "kb_finance",
            requesterOrgNodeId: "dept_hr",
            purpose: "audit",
            expiresAt: "2026-04-21T00:00:00.000Z",
        }], undefined, "2026-04-20T00:00:00.000Z");
...
test("KnowledgeBoundaryService grants access via allowedOrgNodeIds", () => {
    ...
    const decision = service.evaluateAccess(boundary, "user_3", "dept_audit", "review", [], undefined, "2026-04-20T00:00:00.000Z");
...
test("KnowledgeBoundaryService grants access via ownerOrgNodeId", () => {
    ...
    const decision = service.evaluateAccess(boundary, "user_4", "dept_finance", "manage", [], undefined, "2026-04-20T00:00:00.000Z");
    assert.equal(decision.allowed, true);
});
test("KnowledgeBoundaryService denies access when chinese wall blocks", () => {
    ...
    const decision = service.evaluateAccess(boundary, "user_5", "dept_finance", "access", [], chineseWallPolicy, "2026-04-20T00:00:00.000Z");
...
test("KnowledgeBoundaryService allows access when chinese wall passes and boundary allows", () => {
    ...
    const decision = service.evaluateAccess(boundary, "user_6", "dept_hr", "access", [], chineseWallPolicy, "2026-04-20T00:00:00.000Z");
    assert.equal(decision.allowed, true);
...
test("KnowledgeBoundaryService denies when chinese wall passes but boundary denies", () => {
    ...
    const decision = service.evaluateAccess(boundary, "user_7", "dept_hr", "access", [], chineseWallPolicy, "2026-04-20T00:00:00.000Z");
...
test("KnowledgeBoundaryService returns correct access log record", () => {
    ...
    const decision = service.evaluateAccess(boundary, "user_log", "dept_hr", "testing", [], undefined, "2026-04-23T12:00:00.000Z");
...
test("KnowledgeBoundaryService uses default occurredAt when not provided", () => {
    ...
    const decision = service.evaluateAccess(boundary, "user_time", "dept_hr", "testing", [], undefined);
    assert.equal(decision.allowed, true);
...
test("KnowledgeBoundaryService deny reason codes include chinese wall codes plus access_denied", () => {
    ...
    const decision = service.evaluateAccess(boundary, "user_codes", "dept_finance", "access", [], chineseWallPolicy, "2026-04-20T00:00:00.000Z");
    assert.equal(decision.allowed, false);
    assert.ok(decision.reasonCodes.includes("knowledge_boundary.chinese_wall_blocked"));
    assert.ok(decision.reasonCodes.includes("knowledge_boundary.conflict_group:group_blocked"));
    assert.ok(decision.reasonCodes.includes("knowledge_boundary.access_denied"));
});
//# sourceMappingURL=knowledge-boundary-service.test.js.map