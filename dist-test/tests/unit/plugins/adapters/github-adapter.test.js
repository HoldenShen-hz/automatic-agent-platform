...
function createMockPolicy(allowed = true) {
    return {
        evaluate: (_url) => ({
            allowed,
...
test("GithubAdapter.authenticate throws on missing token", async () => {
    const adapter = createGithubAdapterPlugin();
    await assert.rejects(async () => adapter.authenticate({}), { message: /github_adapter\.missing_token/ });
});
test("GithubAdapter.execute throws when not authenticated", async () => {
    const adapter = createGithubAdapterPlugin();
    await assert.rejects(async () => adapter.execute("create_issue", { repository: "test/repo" }), { message: "github_adapter.not_authenticated" });
});
...
test("GithubAdapter.execute throws PolicyDeniedError when egress blocked", async () => {
    ...
    await adapter.authenticate({ token: "ghp_test1234567890" });
    await assert.rejects(async () => adapter.execute("create_issue", {
        repository: "owner/repo",
        title: "Test",
        body: "Body",
    }), (err) => {
...
test("GithubAdapter.execute throws on missing repository", async () => {
    const adapter = createGithubAdapterPlugin({ policy: createMockPolicy() });
    await adapter.authenticate({ token: "ghp_test1234567890" });
    await assert.rejects(async () => adapter.execute("create_issue", {}), { message: /github_adapter\.missing_repository/ });
});
