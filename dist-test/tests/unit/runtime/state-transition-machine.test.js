...
test("StateTransitionMachine constructor sets entity kind and transitions", () => {
    ...
    assert.equal(machine.entityKind, "task");
});
...
test("StateTransitionMachine assertTransition throws on invalid transitions", () => {
    ...
    assert.throws(() => machine.assertTransition("queued", "done"), (err) => err instanceof WorkflowStateError && err.code === "task.invalid_transition");
});
...
test("StateTransitionMachine works with different entity kinds", () => {
    ...
    () => sessionMachine.assertTransition("active", "completed"), WorkflowStateError);
});
test("StateTransitionMachine handles empty transitions map", () => {
    const machine = new StateTransitionMachine("empty", {});
    // No transitions allowed from any state
    assert.throws(
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    () => machine.assertTransition("any", "other"), WorkflowStateError);
});
...
//# sourceMappingURL=state-transition-machine.test.js.map