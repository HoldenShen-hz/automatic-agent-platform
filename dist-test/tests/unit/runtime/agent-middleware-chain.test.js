...
import { createMiddlewareChain, } from "../../../src/platform/execution/execution-engine/agent-middleware-chain.js";
...
test("AgentMiddlewareChain before_model hooks execute and preserve warnings", async () => {
    ...
    const result = await chain.beforeModel({ messages: [], model: "test" });
...
test("AgentMiddlewareChain wrap_model_call wraps the actual call", async () => {
    ...
    const result = await chain.wrapModelCall({ messages: [] }, async () => {
        callCount++;
        return "model-result";
    });
...
test("AgentMiddlewareChain wrap_tool_call wraps tool execution", async () => {
    ...
    const result = await chain.wrapToolCall({ toolName: "test_tool", args: {} }, async () => "tool-result");
...
test("AgentMiddlewareChain throws when failOpen is false and hook throws", async () => {
    ...
    chain.registerBeforeAgent(throwingHook);
    await assert.rejects(() => chain.beforeAgent({ request: "test", history: [] }), (err) => err instanceof Error && err.message.includes("Hook exception"));
});
test("AgentMiddlewareChain wraps multiple tool hooks in sequence", async () => {
    ...
    await chain.wrapToolCall({ toolName: "test", args: {} }, async () => {
...
test("AgentMiddlewareChain wrapModelCall catches and re-throws model call errors", async () => {
    ...
    await assert.rejects(() => chain.wrapModelCall({ messages: [] }, async () => {
        throw new Error("Model call failed");
    }), (err) => err instanceof Error && err.message === "Model call failed");
});
test("AgentMiddlewareChain wrapToolCall catches and re-throws tool call errors", async () => {
    ...
    await assert.rejects(() => chain.wrapToolCall({ toolName: "test_tool", args: {} }, async () => {
        throw new Error("Tool call failed");
    }), (err) => err instanceof Error && err.message === "Tool call failed");
});
...
//# sourceMappingURL=agent-middleware-chain.test.js.map