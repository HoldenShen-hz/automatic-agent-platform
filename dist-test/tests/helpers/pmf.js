...
export function seedPmfValidationDataset(db, store) {
    db.transaction(() => {
        insertTask(store, {
            id: "task-alpha-1",
            rootId: "root-alpha",
...
function insertTask(store, input) {
    store.insertTask({
        id: input.id,
        parentId: null,
        rootId: input.rootId,
...
function insertSession(store, input) {
    store.insertSession({
        id: input.id,
        taskId: input.taskId,
        channel: "cli",
...
function insertStepOutput(store, input) {
...
function insertApproval(store, input) {
