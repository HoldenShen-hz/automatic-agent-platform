...
test("golden: workflow validator rejects duplicate step IDs", () => {
    ...
    const invalidWorkflow = {
        ...
        steps: [
            { stepId: "same-id", roleId: "general_executor", outputKey: "out1", outputSchemaPath: "test", timeoutMs: 60000, maxAttempts: 1, compensationModel: "idempotent_replay" },
            { stepId: "same-id", roleId: "general_executor", outputKey: "out2", outputSchemaPath: "test", timeoutMs: 60000, maxAttempts: 1, compensationModel: "idempotent_replay" },
        ],
    };
...
test("golden: workflow validator rejects negative timeout values", () => {
    ...
    const invalidWorkflow = {
        ...
        divisionId: "general_ops",
        steps: [
            { stepId: "step-1", roleId: "general_executor", outputKey: "out", outputSchemaPath: "test", timeoutMs: -1, maxAttempts: 1, compensationModel: "idempotent_replay" },
        ],
    };
    const result = validator.validate(invalidWorkflow);
...
test("golden: workflow validator rejects zero maxAttempts", () => {
    ...
    const invalidWorkflow = {
        ...
        divisionId: "general_ops",
        steps: [
            { stepId: "step-1", roleId: "general_executor", outputKey: "out", outputSchemaPath: "test", timeoutMs: 60000, maxAttempts: 0, compensationModel: "idempotent_replay" },
        ],
    };
    const result = validator.validate(invalidWorkflow);
...
test("golden: workflow validator detects dependency cycles", () => {
    ...
    const cyclicWorkflow = {
        ...
        divisionId: "general_ops",
        steps: [
            { stepId: "step-1", roleId: "general_executor", outputKey: "out1", outputSchemaPath: "test", timeoutMs: 60000, maxAttempts: 1, compensationModel: "idempotent_replay", dependsOnStepIds: ["step-3"] },
            { stepId: "step-2", roleId: "general_executor", outputKey: "out2", outputSchemaPath: "test", timeoutMs: 60000, maxAttempts: 1, compensationModel: "idempotent_replay", dependsOnStepIds: ["step-1"] },
            { stepId: "step-3", roleId: "general_executor", outputKey: "out3", outputSchemaPath: "test", timeoutMs: 60000, maxAttempts: 1, compensationModel: "idempotent_replay", dependsOnStepIds: ["step-2"] },
        ],
    };
...
test("golden: workflow validator reports missing output schema path", () => {
    ...
    const workflowWithoutSchema = {
        ...
        divisionId: "general_ops",
        steps: [
            { stepId: "step-1", roleId: "general_executor", outputKey: "out", outputSchemaPath: "", timeoutMs: 60000, maxAttempts: 1, compensationModel: "idempotent_replay" },
        ],
    };
