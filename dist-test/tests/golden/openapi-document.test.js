...
test("golden: OpenAPI document has task endpoints", () => {
    ...
    assert.deepEqual(document.paths["/v1/tasks"].get.parameters?.map((parameter) => parameter.name), ["limit", "cursor"]);
    ...
    assert.deepEqual(document.paths["/v1/workflows"]?.get?.parameters?.map((parameter) => parameter.name), ["limit", "cursor"]);
});
