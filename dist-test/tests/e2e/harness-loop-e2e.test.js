...
test("E2E: guardrail assessment blocks tool and suggests abort when risk is too high", (t) => {
    ...
    try {
        ...
        assert.ok(run.guardrailAssessment?.suggestedAction === "abort" ||
            run.status === "aborted", "should suggest abort or be aborted due to guardrail");
        assert.ok(run.guardrailAssessment?.findings.some((f) => f.code === "harness.guardrail.max_risk_exceeded"), "should have max_risk_exceeded finding");
    }
    finally {
        harness.cleanup();
    }
