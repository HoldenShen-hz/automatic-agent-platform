...
test("E2E: delegation chain rejects depth exceeded", async () => {
    ...
    await assert.rejects(async () => service.delegate(level2, createDelegationSpec({
        targetAgentId: "depth-3",
        targetPackId: "pack-depth-3",
    })), /depth/i, "Should reject delegation that exceeds max depth");
});
test("E2E: delegation chain narrows permissions", async () => {
    ...
    assert.ok(delegation.permissions.actions.includes("action-read"), "Should include required action");
    // Parent actions not in required should be filtered
    assert.ok(!delegation.permissions.actions.includes("action-write") ||
        delegation.permissions.actions.length <= parent.permissions.actions.length, "Actions should be narrowed");
});
...
test("E2E: delegation chain cannot cancel completed delegation", async () => {
    ...
    await assert.rejects(async () => service.cancel(handle.delegationId), Error, "Should not be able to cancel completed delegation");
});
...
test("E2E: delegation chain completes with evidence", async () => {
    ...
    const spec = createDelegationSpec();
    const handle = await service.delegate(parent, spec);
    await service.completeWithEvidence(handle.delegationId, ["artifact:proof-1", "artifact:proof-2"], "artifact:final-result");
    const delegation = service.getDelegation(handle.delegationId);
...
//# sourceMappingURL=delegation-chain-flow.test.js.map