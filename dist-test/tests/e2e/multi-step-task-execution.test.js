...
function insertTaskWithExecution(db, store, taskId, executionId, traceId, workflowId, status = "pending", executionStatus = "executing") {
    const now = nowIso();
    db.transaction(() => {
        store.insertTask({
            id: taskId,
...
test("E2E: multi-step task executes step 0 and transitions to step 1", () => {
    ...
    try {
        ...
        h.db.transaction(() => {
            h.store.updateWorkflowState(taskId, "running", 1, JSON.stringify({ step_0_output: "data_from_step_0" }), now, null);
        });
...
test("E2E: multi-step task completes all steps and reaches terminal state", () => {
    ...
    try {
        ...
        // Step 0 -> Step 1
        h.db.transaction(() => {
            h.store.updateWorkflowState(taskId, "running", 1, JSON.stringify({ step_0: "result_0" }), now, null);
        });
        // Step 1 -> Step 2
        h.db.transaction(() => {
            h.store.updateWorkflowState(taskId, "running", 2, JSON.stringify({ step_0: "result_0", step_1: "result_1" }), now, null);
        });
        // Step 2 -> Step 3 (final)
        h.db.transaction(() => {
            h.store.updateWorkflowState(taskId, "completed", 3, JSON.stringify({ step_0: "result_0", step_1: "result_1", step_2: "result_2", final: "done" }), now, null);
        });
...
test("E2E: multi-step task with dependencies waits for prerequisite", () => {
    ...
    try {
        ...
        h.db.transaction(() => {
            h.store.updateWorkflowState(taskId, "running", 1, JSON.stringify({ step_0_data: "prerequisite_for_step_2" }), now, null);
        });
        workflow = h.store.getWorkflowState(taskId);
...
test("E2E: multi-step task resumes from checkpoint after failure", () => {
    ...
    try {
        ...
        // Resume and complete
        h.db.transaction(() => {
            h.store.updateWorkflowState(taskId, "completed", 3, JSON.stringify({ step_0: "done", step_1: "done", step_2: "completed_on_retry" }), now, null);
        });
        workflow = h.store.getWorkflowState(taskId);
...
test("E2E: multi-step task with parallel branches converges", () => {
    ...
    try {
        ...
        h.db.transaction(() => {
            h.store.updateWorkflowState(taskId, "running", 2, // After parallel section
            JSON.stringify({
...
//# sourceMappingURL=multi-step-task-execution.test.js.map