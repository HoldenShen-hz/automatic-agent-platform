...
import { HarnessRuntimeService, HarnessLoopController } from "../../src/platform/orchestration/harness/index.js";
...
describe("HarnessLoopController Performance Benchmarks", () => {
    describe("recordIteration throughput", () => {
        it("should handle 10000 recordIteration calls at > 100000 ops/sec", () => {
            ...
            assert.ok(opsPerSec > 100000, `recordIteration throughput ${opsPerSec.toFixed(0)} ops/sec should be > 100000 ops/sec`);
        });
    });
    describe("recordReplan throughput", () => {
        it("should handle 10000 recordReplan calls", () => {
            ...
            const durationMs = end - start;
            const opsPerSec = (iterations / durationMs) * 1000;
            assert.ok(opsPerSec > 80000, `recordReplan throughput ${opsPerSec.toFixed(0)} ops/sec should be > 80000 ops/sec`);
        });
    });
    describe("getGuardViolation latency", () => {
        it("should evaluate getGuardViolation 50000 times with avg latency < 1µs", () => {
            ...
            assert.ok(avgLatencyNs < 1000, `getGuardViolation avg latency ${avgLatencyNs.toFixed(2)}ns should be < 1000ns (1µs)`);
        });
    });
    describe("evaluateProgress throughput", () => {
        it("should handle 10000 evaluateProgress calls with different actions", () => {
            ...
            const durationMs = end - start;
            const opsPerSec = (iterations / durationMs) * 1000;
            assert.ok(opsPerSec > 50000, `evaluateProgress throughput ${opsPerSec.toFixed(0)} ops/sec should be > 50000 ops/sec`);
        });
    });
    describe("getState/getGuards latency", () => {
        it("should measure readout performance for getState", () => {
            ...
            const durationNs = (end - start) * 1_000_000;
            const avgLatencyNs = durationNs / iterations;
            assert.ok(avgLatencyNs < 500, `getState avg latency ${avgLatencyNs.toFixed(2)}ns should be < 500ns`);
        });
        it("should measure readout performance for getGuards", () => {
            ...
            const durationNs = (end - start) * 1_000_000;
            const avgLatencyNs = durationNs / iterations;
            assert.ok(avgLatencyNs < 500, `getGuards avg latency ${avgLatencyNs.toFixed(2)}ns should be < 500ns`);
        });
    });
    describe("Full loop iteration performance", () => {
        it("should simulate 100 complete loop cycles efficiently", () => {
            ...
            assert.ok(cyclesPerSec > 1000, `Full loop cycles ${cyclesPerSec.toFixed(0)} cycles/sec should be > 1000 cycles/sec`);
        });
    });
    describe("runLoop iteration throughput", () => {
        const createLoopInput = (overrides = {}) => ({
            taskId: newId("task"),
        ...
        it("should execute runLoop iterations at > 500 ops/sec", () => {
            ...
            const durationMs = end - start;
            const opsPerSec = (iterations / durationMs) * 1000;
            assert.ok(opsPerSec > 500, `runLoop iteration throughput ${opsPerSec.toFixed(0)} ops/sec should be > 500 ops/sec`);
        });
        it("should handle replan-triggering runLoop at > 300 ops/sec", () => {
            ...
            const durationMs = end - start;
            const opsPerSec = (iterations / durationMs) * 1000;
            assert.ok(opsPerSec > 300, `runLoop replan throughput ${opsPerSec.toFixed(0)} ops/sec should be > 300 ops/sec`);
        });
        it("should handle multi-iteration runLoop at > 100 cycles/sec", () => {
            ...
            assert.ok(cyclesPerSec > 100, `runLoop multi-iteration throughput ${cyclesPerSec.toFixed(0)} cycles/sec should be > 100 cycles/sec`);
        });
        it("should handle runLoop with human escalation at > 400 ops/sec", () => {
            ...
            const durationMs = end - start;
            const opsPerSec = (iterations / durationMs) * 1000;
            assert.ok(opsPerSec > 400, `runLoop escalation throughput ${opsPerSec.toFixed(0)} ops/sec should be > 400 ops/sec`);
        });
    });
});
//# sourceMappingURL=harness-loop-performance.test.js.map