...
function createTestFeedbackSignals(count) {
    const signals = [];
...
test("performance: signal-preprocessor P99 < 10ms", () => {
    ...
    assert.ok(p99 < 10, `signal-preprocessor P99 latency ${p99.toFixed(3)}ms exceeds 10ms target`);
    // Sanity check median is reasonable
    assert.ok(p50 < 5, `signal-preprocessor P50 latency ${p50.toFixed(3)}ms seems unexpectedly high`);
});
test("performance: signal-preprocessor deduplicate P99 < 5ms", () => {
    ...
    assert.ok(p99 < 5, `signal-preprocessor.deduplicate P99 latency ${p99.toFixed(3)}ms exceeds 5ms target`);
});
test("performance: signal-preprocessor mergeCorrelated P99 < 8ms", () => {
    ...
    latencies.sort((a, b) => a - b);
    const p99 = latencies[Math.floor(iterations * 0.99)];
    assert.ok(p99 < 8, `signal-preprocessor.mergeCorrelated P99 latency ${p99.toFixed(3)}ms exceeds 8ms target`);
});
//# sourceMappingURL=feedback-perf.test.js.map