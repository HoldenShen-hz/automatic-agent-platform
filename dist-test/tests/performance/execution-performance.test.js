...
import { computeWorkerLoadScore, computeEffectiveActiveLeaseCount, summarizeWorkerLoadSkew, } from "../../src/platform/execution/worker-pool/worker-load-balancing.js";
...
test("performance: computeWorkerLoadScore() P99 <0.05ms", (t) => {
    ...
    try {
        assert.ok(p99 < 0.05, `computeWorkerLoadScore P99 latency ${p99.toFixed(4)}ms exceeds 0.05ms target`);
    }
    catch (err) {
...
test("performance: computeWorkerLoadScore() throughput >100000 ops/sec", (t) => {
    ...
    try {
        const result = runBenchmark("worker_load_score", () => computeWorkerLoadScore(signal), 10000);
        console.log(`computeWorkerLoadScore() throughput: ${result.opsPerSec.toFixed(0)} ops/sec, P99: ${result.p99.toFixed(4)}ms`);
        try {
            assert.ok(result.opsPerSec > 100000, `computeWorkerLoadScore throughput ${result.opsPerSec.toFixed(0)} ops/sec must be >100000 ops/sec`);
        }
        catch (err) {
            if (err instanceof assert.AssertionError) {
                t.skip(err.message);
...
test("performance: computeEffectiveActiveLeaseCount() P99 <0.01ms", (t) => {
    ...
    try {
        assert.ok(p99 < 0.01, `computeEffectiveActiveLeaseCount P99 latency ${p99.toFixed(4)}ms exceeds 0.01ms target`);
    }
    catch (err) {
        if (err instanceof assert.AssertionError) {
            t.skip(err.message);
...
test("performance: summarizeWorkerLoadSkew() with 5 workers P99 <0.5ms", (t) => {
    ...
    try {
        assert.ok(p99 < 0.5, `summarizeWorkerLoadSkew P99 latency ${p99.toFixed(4)}ms exceeds 0.5ms target`);
    }
    catch (err) {
        if (err instanceof assert.AssertionError) {
            t.skip(err.message);
...
test("performance: summarizeWorkerLoadSkew() with 10 workers P99 <1ms", (t) => {
    ...
    try {
        assert.ok(p99 < 1, `summarizeWorkerLoadSkew(10 workers) P99 latency ${p99.toFixed(4)}ms exceeds 1ms target`);
    }
    catch (err) {
        if (err instanceof assert.AssertionError) {
            t.skip(err.message);
...
test("performance: routeComplexity() passthrough path P99 <0.1ms", (t) => {
    ...
    try {
        assert.ok(p99 < 0.1, `routeComplexity(passthrough) P99 latency ${p99.toFixed(4)}ms exceeds 0.1ms target`);
    }
    catch (err) {
        if (err instanceof assert.AssertionError) {
            t.skip(err.message);
...
test("performance: routeComplexity() full path P99 <0.1ms", (t) => {
    ...
    try {
        assert.ok(p99 < 0.1, `routeComplexity(full) P99 latency ${p99.toFixed(4)}ms exceeds 0.1ms target`);
    }
    catch (err) {
        if (err instanceof assert.AssertionError) {
            t.skip(err.message);
...
test("performance: routeComplexity() fast path P99 <0.1ms", (t) => {
    ...
    try {
        assert.ok(p99 < 0.1, `routeComplexity(fast) P99 latency ${p99.toFixed(4)}ms exceeds 0.1ms target`);
    }
    catch (err) {
        if (err instanceof assert.AssertionError) {
            t.skip(err.message);
...
test("performance: routeComplexity() throughput >50000 ops/sec", (t) => {
    try {
        const result = runBenchmark("route_complexity", () => routeComplexity("List all the files in the project"), 10000);
        console.log(`routeComplexity() throughput: ${result.opsPerSec.toFixed(0)} ops/sec, P99: ${result.p99.toFixed(4)}ms`);
        try {
            assert.ok(result.opsPerSec > 50000, `routeComplexity throughput ${result.opsPerSec.toFixed(0)} ops/sec must be >50000 ops/sec`);
        }
        catch (err) {
            if (err instanceof assert.AssertionError) {
                t.skip(err.message);
...
test("performance: routeComplexity() with custom config P99 <0.2ms", (t) => {
    ...
    try {
        assert.ok(p99 < 0.2, `routeComplexity(custom config) P99 latency ${p99.toFixed(4)}ms exceeds 0.2ms target`);
    }
    catch (err) {
        if (err instanceof assert.AssertionError) {
            t.skip(err.message);
...
test("performance: createKvCachePrefixConfig() P99 <0.05ms", (t) => {
    ...
    try {
        assert.ok(p99 < 0.05, `createKvCachePrefixConfig P99 latency ${p99.toFixed(4)}ms exceeds 0.05ms target`);
    }
    catch (err) {
        if (err instanceof assert.AssertionError) {
            t.skip(err.message);
...
test("performance: estimateTokens() throughput >200000 ops/sec", (t) => {
    ...
    try {
        const result = runBenchmark("estimate_tokens", () => estimateTokens(testText), 10000);
        console.log(`estimateTokens() throughput: ${result.opsPerSec.toFixed(0)} ops/sec, P99: ${result.p99.toFixed(4)}ms`);
        try {
            assert.ok(result.opsPerSec > 200000, `estimateTokens throughput ${result.opsPerSec.toFixed(0)} ops/sec must be >200000 ops/sec`);
        }
        catch (err) {
            if (err instanceof assert.AssertionError) {
                t.skip(err.message);
...
test("performance: estimateTokens() with large text P99 <0.1ms", (t) => {
    ...
    try {
        assert.ok(p99 < 0.1, `estimateTokens(large text) P99 latency ${p99.toFixed(4)}ms exceeds 0.1ms target`);
    }
    catch (err) {
        if (err instanceof assert.AssertionError) {
            t.skip(err.message);
...
test("performance: isWithinFixedPrefixBudget() P99 <0.05ms", (t) => {
    ...
    try {
        assert.ok(p99 < 0.05, `isWithinFixedPrefixBudget P99 latency ${p99.toFixed(4)}ms exceeds 0.05ms target`);
    }
    catch (err) {
        if (err instanceof assert.AssertionError) {
            t.skip(err.message);
...
test("performance: isWithinDomainBlockBudget() P99 <0.05ms", (t) => {
    ...
    try {
        assert.ok(p99 < 0.05, `isWithinDomainBlockBudget P99 latency ${p99.toFixed(4)}ms exceeds 0.05ms target`);
    }
    catch (err) {
        if (err instanceof assert.AssertionError) {
            t.skip(err.message);
...
test("performance: AdmissionController.snapshot() P99 <0.1ms", (t) => {
    const store = new MockAuthoritativeTaskStore();
    ...
    try {
        assert.ok(p99 < 0.1, `AdmissionController.snapshot P99 latency ${p99.toFixed(4)}ms exceeds 0.1ms target`);
    }
    catch (err) {
        if (err instanceof assert.AssertionError) {
            t.skip(err.message);
...
test("performance: AdmissionController.evaluate() P99 <0.2ms", (t) => {
    ...
    try {
        assert.ok(p99 < 0.2, `AdmissionController.evaluate P99 latency ${p99.toFixed(4)}ms exceeds 0.2ms target`);
    }
    catch (err) {
        if (err instanceof assert.AssertionError) {
            t.skip(err.message);
...
test("performance: AdmissionController.evaluate() throughput >5000 ops/sec", (t) => {
    ...
    const controller = new AdmissionController(store, DEFAULT_ADMISSION_POLICY, null);
    try {
        const result = runBenchmark("admission_evaluate", () => controller.evaluate({ priority: "normal" }), 10000);
        console.log(`AdmissionController.evaluate() throughput: ${result.opsPerSec.toFixed(0)} ops/sec, P99: ${result.p99.toFixed(4)}ms`);
        try {
            assert.ok(result.opsPerSec > 5000, `AdmissionController.evaluate throughput ${result.opsPerSec.toFixed(0)} ops/sec must be >5000 ops/sec`);
        }
        catch (err) {
            if (err instanceof assert.AssertionError) {
                t.skip(err.message);
...
test("performance: AdmissionController.evaluate() with budget check P99 <0.2ms", (t) => {
    ...
    try {
        assert.ok(p99 < 0.2, `AdmissionController.evaluate(budget) P99 latency ${p99.toFixed(4)}ms exceeds 0.2ms target`);
    }
    catch (err) {
        if (err instanceof assert.AssertionError) {
            t.skip(err.message);
...
test("performance: AdmissionController.evaluate() with backpressure P99 <0.3ms", (t) => {
    ...
    try {
        assert.ok(p99 < 0.3, `AdmissionController.evaluate(backpressure) P99 latency ${p99.toFixed(4)}ms exceeds 0.3ms target`);
    }
    catch (err) {
        if (err instanceof assert.AssertionError) {
            t.skip(err.message);
...
test("performance: bulk load balancing operations throughput >10000 ops/sec", (t) => {
    try {
        ...
        const result = runBenchmark("bulk_load_balancing", () => {
        ...
        console.log(`Bulk load balancing throughput: ${totalOpsPerSec.toFixed(0)} ops/sec, P99: ${result.p99.toFixed(4)}ms`);
        try {
            assert.ok(totalOpsPerSec > 10000, `Bulk load balancing throughput ${totalOpsPerSec.toFixed(0)} ops/sec must be >10000 ops/sec`);
        }
        catch (err) {
            if (err instanceof assert.AssertionError) {
                t.skip(err.message);
...
test("performance: bulk routing operations throughput >20000 ops/sec", (t) => {
    ...
    try {
        const result = runBenchmark("bulk_routing", () => {
            for (const task of tasks) {
                routeComplexity(task);
            }
        }, 1000);
        const totalOpsPerSec = result.opsPerSec;
        console.log(`Bulk routing throughput: ${totalOpsPerSec.toFixed(0)} ops/sec, P99: ${result.p99.toFixed(4)}ms`);
        try {
            assert.ok(totalOpsPerSec > 20000, `Bulk routing throughput ${totalOpsPerSec.toFixed(0)} ops/sec must be >20000 ops/sec`);
        }
        catch (err) {
            if (err instanceof assert.AssertionError) {
                t.skip(err.message);
...
//# sourceMappingURL=execution-performance.test.js.map