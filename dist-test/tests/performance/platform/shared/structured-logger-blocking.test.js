...
function createRelativeWorkspace(prefix) {
    const workspace = join(".tmp", `${prefix}${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}`);
...
test("[SYS-PERF-3.1] structured logger write does not block event loop > 1ms", () => {
    ...
    try {
        ...
        assert.ok(avgMs < 1, `Average log write ${avgMs.toFixed(3)}ms must be < 1ms. Defect: appendFileSync blocks event loop. Max: ${maxMs.toFixed(3)}ms, P95: ${p95Ms.toFixed(3)}ms`);
        // P99 should be under 5ms (allow some variance)
        assert.ok(p99Ms < 5, `P99 log write ${p99Ms.toFixed(3)}ms must be < 5ms. Defect: synchronous I/O blocks event loop.`);
...
test("[SYS-PERF-3.1] logger does not block during high-frequency writes", () => {
    ...
    try {
        ...
        for (let batch = 0; batch < batches; batch++) {
            ...
            assert.ok(avgPerMsg < 1, `Batch ${batch} average ${avgPerMsg.toFixed(3)}ms per message must be < 1ms`);
        }
        // Overall average
        ...
        assert.ok(overallAvg < 0.5, `Overall average ${overallAvg.toFixed(3)}ms must be < 0.5ms`);
        assert.ok(overallP99 < 5, `Overall P99 ${overallP99.toFixed(3)}ms must be < 5ms`);
        StructuredLogger.configureGlobalFileSink(null);
    }
    finally {
        cleanupPath(workspace);
...
test("[SYS-PERF-3.1] concurrent logger writes do not block each other excessively", () => {
    ...
    try {
        ...
        assert.ok(avgPerMessage < 1, `Average per message ${avgPerMessage.toFixed(3)}ms must be < 1ms. Defect: appendFileSync blocks event loop.`);
        assert.ok(maxTiming < 10, `Max timing ${maxTiming.toFixed(3)}ms should be < 10ms even with contention`);
        // P99 should be reasonable
        assert.ok(p99Timing < 5, `P99 timing ${p99Timing.toFixed(3)}ms must be < 5ms`);
        StructuredLogger.configureGlobalFileSink(null);
    }
    finally {
        cleanupPath(workspace);
...
//# sourceMappingURL=structured-logger-blocking.test.js.map