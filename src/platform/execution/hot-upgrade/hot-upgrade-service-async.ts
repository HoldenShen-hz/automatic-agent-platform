...
export class HotUpgradeServiceAsync {
  ...
  constructor(
    ...
    private readonly repo: HotUpgradeRepository,
    options?: HotUpgradeServiceAsyncOptions,
  ) {
    this.defaultPolicy = {
  ...
  async registerVersionCompatibility(compat: VersionCompatibility): Promise<void> {
  ...
  async isUpgradeSafe(fromVersion: string, toVersion: string): Promise<{
    safe: boolean;
  ...
  async getUpgradePlansByStatus(status: UpgradeStatus): Promise<UpgradePlan[]> {
  ...
  async startUpgrade(planId: string): Promise<{
    started: boolean;
  ...
  async startBatch(batchId: string): Promise<{
    started: boolean;
    ...
    return this.db.transaction(async () => {
      ...
      const now = nowIso();
      const updatedBatch: UpgradeBatch = {
        ...batch,
        status: "in_progress",
        startedAt: now,
        healthChecks: [],
  ...
  async triggerRollback(upgradeId: string, reason: RollbackTrigger["reasonCode"], message: string): Promise<{
    triggered: boolean;
  ...
  async recordAudit(upgradeId: string, eventType: string, actor: string, message: string, details: Record<string, unknown>): Promise<void> {
  ...
  async getUpgradeAuditLog(upgradeId: string, limit = 100): Promise<Array<{ eventType: string; actor: string; message: string; occurredAt: string }>> {
  ...
  private async getUpgradePlanByBatch(batchId: string): Promise<UpgradePlan | null> {
    const batch = await this.repo.getUpgradeBatch(batchId);
