...
import {
  ...
  recordRejectedEvent,
  toWorkerStatus,
} from "./execution-worker-handshake-support.js";
...
export class ExecutionWorkerHandshakeService {
   ...
   */
  public claimExecution(input: WorkerClaimExecutionInput): WorkerHandshakeDecision {
    ...
    const remoteAuthorityBlockReason = resolveRemoteAuthorityBlockReason({
      ...
      remoteSessionStatus:
        input.remoteSessionStatus === undefined ? (workerSnapshot.remoteSessionStatus ?? null) : input.remoteSessionStatus,
    ...
    this.db.transaction(() => {
      ...
      this.upsertAgentExecutionRecord(execution, occurredAt, {
        ...
        status: "executing",
        currentStepId:
          input.currentStepId === undefined ? (this.store.worker.getAgentExecutionRecord(execution.id)?.currentStepId ?? null) : input.currentStepId,
        lastToolName:
          input.lastToolName === undefined ? (this.store.worker.getAgentExecutionRecord(execution.id)?.lastToolName ?? null) : input.lastToolName,
        toolCallCount:
   ...
   * @returns Decision indicating acceptance or rejection with reason code
   */
  public recordHeartbeat(input: WorkerExecutionHeartbeatInput): WorkerHandshakeDecision {
    const occurredAt = input.occurredAt ?? nowIso();
