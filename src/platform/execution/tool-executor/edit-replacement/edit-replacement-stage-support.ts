...
import type { EditInstruction, EditReplacementAttempt, StageEvaluation } from "./edit-replacement-types.js";
import { normalizeIndentationAware, normalizeWhitespace } from "./string-utils.js";

export function evaluateEditStages(content: string, request: EditInstruction): StageEvaluation {
  const attempts: EditReplacementAttempt[] = [];
  const exact = buildAttempt("exact", matchExact(content, request.oldString), content);
  attempts.push(exact);
  if (exact.candidateCount > 1) return { attempts, matchedCandidate: null, errorCode: "tool.edit_multiple_candidates", similarityScore: null };
  if (exact.matched) return { attempts, matchedCandidate: matchExact(content, request.oldString).candidate, errorCode: null, similarityScore: null };
  ...
  if (whitespace.candidateCount > 1) return { attempts, matchedCandidate: null, errorCode: "tool.edit_multiple_candidates", similarityScore: null };
  ...
  if (indentation.candidateCount > 1) return { attempts, matchedCandidate: null, errorCode: "tool.edit_multiple_candidates", similarityScore: null };
  ...
  if (fuzzy.matched) return { attempts, matchedCandidate: fuzzyOutcome.candidate, errorCode: null, similarityScore: fuzzy.similarityScore };
  ...
  if (anchored.matched) return { attempts, matchedCandidate: anchoredOutcome.candidate, errorCode: null, similarityScore: anchored.similarityScore };
