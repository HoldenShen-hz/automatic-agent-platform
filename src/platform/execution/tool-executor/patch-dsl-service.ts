...
} from "../../control-plane/iam/file-freshness.js";
import { ToolExecutionError } from "../../contracts/errors.js";
import { checkSandboxPath } from "../../control-plane/iam/sandbox-policy.js";
import type { SandboxPathCheckResult, SandboxPolicy } from "../../control-plane/iam/sandbox-policy.js";
...
export class PatchDslService {
    ...
    for (const [index, patch] of request.patches.entries()) {
      ...
      if ("errorCode" in preparedPatch.result && preparedPatch.result.status === "failed") {
        ...
        return this.buildResult(
          request,
          preparedPatch.result.errorCode?.startsWith("sandbox.") || preparedPatch.result.errorCode?.startsWith("tool.path_scope")
            ? "blocked"
            : "failed",
          false,
          mergedResults,
   ...
   */
  public parsePatchString(patchContent: string): FilePatch[] {
   ...
   */
  private preparePatch(patch: FilePatch, request: PatchApplicationRequest): PreparedPatch {
      ...
      if (!sourceCheck.allowed) {
        return {
          ...
          result: this.failedResult(
            targetPathInput ?? sourcePathInput,
            patch,
            "sandbox.write_path_denied",
            sourceCheck.reasonCode ?? "sandbox.write_path_denied",
          ),
          operation,
          sourcePath: null,
          targetPath: null,
      ...
      if (!targetCheck.allowed) {
        return {
          ...
          result: this.failedResult(
            ...
            patch,
            "sandbox.write_path_denied",
            targetCheck.reasonCode ?? "sandbox.write_path_denied",
          ),
          operation,
          sourcePath,
          targetPath: null,
    ...
    if (operation === "create" && !request.allowCreation) {
      return {
        ...
        result: this.failedResult(
          targetPathInput ?? sourcePathInput ?? "",
          patch,
          "patch.file_not_found",
          `File does not exist: ${targetPathInput ?? sourcePathInput ?? ""}`,
        ),
        operation,
        sourcePath,
        targetPath,
      ...
      if (sourcePath == null || !existsSync(sourcePath)) {
        return {
          ...
          result: this.failedResult(
            ...
            patch,
            "patch.file_not_found",
            `File does not exist: ${patch.oldPath}`,
          ),
          operation,
          sourcePath,
          targetPath,
    ...
    if (!sourceExists) {
      ...
      if (request.allowCreation && targetPath != null && sourcePath === targetPath) {
        ...
        return {
          patch,
          result: this.failedResult(
            patch.oldPath || patch.newPath,
            patch,
            "patch.file_not_found",
            `File does not exist: ${patch.oldPath || patch.newPath}`,
          ),
          operation,
          sourcePath,
          targetPath,
    ...
    if (operation === "move" && targetPath != null && existsSync(targetPath) && targetPath !== sourcePath) {
      return {
        ...
        result: this.failedResult(
          patch.newPath,
          patch,
          "patch.move_target_exists",
          `Target already exists: ${patch.newPath}`,
        ),
        operation,
        sourcePath,
        targetPath,
    ...
    if (originalContent.includes("\u0000")) {
      return {
        ...
        result: this.failedResult(
          ...
          patch,
          "tool.edit_binary_file_denied",
          "Binary files are not supported by apply_patch",
        ),
        operation,
        sourcePath,
        targetPath,
    ...
    if (!applied.success) {
      return {
        ...
        result: this.failedResult(
          patch.newPath || patch.oldPath,
          patch,
          "patch.hunk_not_found",
          applied.errorMessage ?? "Unable to apply patch hunk",
          applied.hunksApplied,
        ),
        operation,
        sourcePath,
        targetPath,
      ...
      if (!freshnessResult.fresh) {
        return {
          ...
          result: this.failedResult(
            patch.newPath || patch.oldPath,
            patch,
            "tool.file_stale_modification_denied",
            `File was modified externally: ${freshnessResult.reason}`,
            applied.hunksApplied,
          ),
          operation,
          sourcePath,
   ...
   */
  private classifyOperation(patch: FilePatch): PreparedPatch["operation"] {
  ...
  private resolveInitialSnapshot(
    ...
    freshnessConfig: FreshnessConfig | undefined,
  ): FileSnapshot | null {
  ...
  private applyHunks(
    ...
    strictMode: boolean,
  ): { success: boolean; content: string; hunksApplied: number; errorMessage?: string } {
    ...
    for (const hunk of hunks) {
      ...
      if (!applied.success) {
        const result: { success: boolean; content: string; hunksApplied: number; errorMessage?: string } = {
          success: false,
  ...
  ): Array<{ kind: "context" | "add" | "delete"; value: string }> {
    const operations: Array<{ kind: "context" | "add" | "delete"; value: string }> = [];
   ...
   */
  private captureAffectedPaths(prepared: readonly PreparedPatch[]): Map<string, FileStateSnapshot> {
    const snapshots = new Map<string, FileStateSnapshot>();
   ...
   */
  private restoreAffectedPaths(snapshots: Map<string, FileStateSnapshot>): void {
   ...
   */
  private commitPreparedPatch(prepared: PreparedPatch): void {
  ...
  private buildResult(
    request: PatchApplicationRequest,
    status: PatchApplicationResult["status"],
    success: boolean,
    results: PatchResult[],
    ...
    return {
      ...
      success,
      output: success
        ? [`Applied ${changedCount} patches`, formatDiagnosticsFeedback(diagnostics)].filter((part): part is string => part != null).join("\n")
        : null,
      results,
      diagnostics,
      durationMs,
