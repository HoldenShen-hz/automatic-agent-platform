...
export interface StableRollingUpgradeScenarioResult {
  scenarioId:
    | "repo_version_canary_routes_to_upgraded_worker"
    | "lease_handover_supports_step_boundary_upgrade";
  passed: boolean;
  durationMs: number;
  summary: string;
...
async function measureScenario(
  scenarioId: StableRollingUpgradeScenarioResult["scenarioId"],
  run: () => Promise<Omit<StableRollingUpgradeScenarioResult, "scenarioId" | "durationMs">>,
): Promise<StableRollingUpgradeScenarioResult> {
...
function buildRollingUpgradeRuntimeVersionSnapshot(outputDir: string): RuntimeVersionSnapshot {
...
async function runRepoVersionCanaryScenario(outputDir: string): Promise<StableRollingUpgradeScenarioResult> {
    ...
    return {
      passed:
        ...
        && decision.trace?.evaluations.some(
          (evaluation) =>
            evaluation.workerId === "worker-old-version"
            && evaluation.accepted === false
            && evaluation.rejectionReason === "worker_repo_version_mismatch",
        ) === true,
...
async function runStepBoundaryHandoverScenario(outputDir: string): Promise<StableRollingUpgradeScenarioResult> {
...
export async function runStableRollingUpgradeRehearsal(
  options: StableRollingUpgradeRehearsalOptions,
): Promise<StableRollingUpgradeRehearsalReport> {
...
export function writeStableRollingUpgradeRehearsalReport(
  outputFile: string,
  report: StableRollingUpgradeRehearsalReport,
): void {
