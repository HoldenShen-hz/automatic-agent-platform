...
export function getPolicyForNamespace(
  namespace: string,
...
export function isCacheableNamespace(namespace: string): boolean {
  return DEFAULT_CACHE_POLICIES[namespace]?.enabled ?? false;
}

export function getTTLForNamespace(namespace: string): number {
  return DEFAULT_CACHE_POLICIES[namespace]?.ttlMs ?? 0;
}

export function getScopeForNamespace(namespace: string): string {
