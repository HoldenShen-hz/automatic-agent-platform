...
} from "./stable-evidence-bundle.js";
import {
...
} from "./stable-runtime-validator.js";
...
function buildStatePath(outputDir: string): string {
  return join(outputDir, "stable-evidence-campaign-state.json");
}

/** Loads campaign state from disk */
function loadState(outputDir: string): StableEvidenceCampaignState | null {
  return safeReadJson<StableEvidenceCampaignState>(buildStatePath(outputDir));
}

/** Persists campaign state to disk */
function persistState(outputDir: string, state: StableEvidenceCampaignState): void {
  writeJson(buildStatePath(outputDir), state);
}

/** Loads validation reports for all completed segments */
function loadSegmentValidationReports(state: StableEvidenceCampaignState): StableValidationReport[] {
  return state.segments.map((segment) => {
    const report =
      safeReadJson<StableValidationReport>(segment.validationReportPath)
      ?? safeReadJson<StableValidationReport>(
        join(dirname(segment.validationReportPath), "validation", "stable-validation-report.json"),
      )
      ?? synthesizeValidationReportFromSegment(segment);
    if (!report) {
      throw new ValidationError(
        `missing validation segment report: ${segment.validationReportPath}`,
        `missing validation segment report: ${segment.validationReportPath}`,
        {
          retryable: false,
...
function loadSegmentSoakReports(state: StableEvidenceCampaignState): StableSoakReport[] {
  return state.segments.map((segment) => {
    const report =
      safeReadJson<StableSoakReport>(segment.soakReportPath)
      ?? safeReadJson<StableSoakReport>(
        join(dirname(segment.soakReportPath), "soak", "stable-soak-report.json"),
      )
      ?? synthesizeSoakReportFromSegment(segment);
    if (!report) {
      throw new ValidationError(
        `missing soak segment report: ${segment.soakReportPath}`,
        `missing soak segment report: ${segment.soakReportPath}`,
        {
          retryable: false,
...
function buildInitialState(outputDir: string, profile: StableEvidenceProfile, targetDurationMs: number): StableEvidenceCampaignState {
...
function resolveDurationMode(options: StableEvidenceCampaignOptions, profileName: StableEvidenceProfileName): "virtual" | "wall_clock" {
...
function normalizeLoadedState(
  state: StableEvidenceCampaignState,
  durationMode: "virtual" | "wall_clock",
): StableEvidenceCampaignState {
  const accumulatedWallClockDurationMs =
    state.accumulatedWallClockDurationMs
 ...
 */
function synthesizeValidationReportFromSegment(
  segment: StableEvidenceCampaignSegment,
): StableValidationReport | null {
  if (!segment.passed) {
    return null;
  }
 ...
 */
function synthesizeSoakReportFromSegment(
  segment: StableEvidenceCampaignSegment,
): StableSoakReport | null {
 ...
 */
export async function runStableEvidenceCampaign(
  options: StableEvidenceCampaignOptions,
): Promise<StableEvidenceCampaignReport> {
  ...
  const state = normalizeLoadedState(
    loadState(options.outputDir) ?? buildInitialState(options.outputDir, configuredProfile, targetDurationMs),
    durationMode,
  );
  ...
  if (state.completed) {
    return {
      state,
      finalEvidenceReport:
        state.finalEvidenceReportPath ? safeReadJson<StableEvidenceBundleReport>(state.finalEvidenceReportPath) : null,
    };
  }

  // Calculate segment parameters
  const segmentDurationMs = Math.min(
    options.segmentDurationMs ?? Math.max(state.profile.soakIntervalMs, 60_000),
  ...
  let finalEvidenceReport: StableEvidenceBundleReport | null = null;
  const durationSatisfied =
    state.durationMode === "wall_clock"
