 ...
 */
export function mergeStableSoakReports(reports: readonly StableSoakReport[]): StableSoakReport {
  ...
  const memoryGrowthRatio =
    initialHeapUsedBytes != null && initialHeapUsedBytes > 0 && peakHeapUsedBytes != null
 ...
 */
export async function runStableSoak(options: StableSoakRunnerOptions): Promise<StableSoakReport> {
  mkdirSync(options.outputDir, { recursive: true });

  const startedAtMs = Date.now();
  const startedAt = new Date().toISOString();
  const deadline = Date.now() + options.durationMs;
  const cycles: StableSoakCycle[] = [];

  let initialHeapUsedBytes: number | undefined;
  ...
  const memoryGrowthRatio =
    initialHeapUsedBytes != null && initialHeapUsedBytes > 0
      ? peakHeapUsedBytes / initialHeapUsedBytes
      : undefined;
 ...
 */
export function writeStableSoakReport(outputFile: string, report: StableSoakReport): void {
