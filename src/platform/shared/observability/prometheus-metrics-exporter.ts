...
export class PrometheusMetricsExporter {
  ...
  public export(): string {
    ...
    lines.push(
      `# HELP ${this.metricName("process_cpu_seconds_total")} Total user and system CPU time spent in seconds.`,
    );
    lines.push(
      `# TYPE ${this.metricName("process_cpu_seconds_total")} counter`,
    );
    lines.push(
      `${this.metricName("process_cpu_seconds_total")} ${this.getCpuSeconds()}`,
    );

    lines.push("");
    lines.push(
      `# HELP ${this.metricName("process_meminfo_bytes")} Process memory information in bytes.`,
    );
    lines.push(`# TYPE ${this.metricName("process_meminfo_bytes")} gauge`);
    const memUsage = this.getMemoryUsage();
    lines.push(
      `${this.metricName("process_meminfo_bytes")}{type="rss"} ${memUsage.rss}`,
    );
    lines.push(
      `${this.metricName("process_meminfo_bytes")}{type="heap_total"} ${memUsage.heapTotal}`,
    );
    lines.push(
      `${this.metricName("process_meminfo_bytes")}{type="heap_used"} ${memUsage.heapUsed}`,
    );
    lines.push(
      `${this.metricName("process_meminfo_bytes")}{type="external"} ${memUsage.external}`,
    );

    lines.push("");
    lines.push(
      `# HELP ${this.metricName("http_requests_total")} Total number of HTTP requests.`,
    );
    ...
    for (const metric of this.getHttpRequestCounts()) {
      ...
      lines.push(
        `${this.metricName("http_requests_total")}{method="${metric.method}",path="${path}",status="${metric.status}"} ${metric.count}`,
      );
    }
    ...
    lines.push("");
    lines.push(
      `# HELP ${this.metricName("http_request_duration_ms")} HTTP request duration in milliseconds.`,
    );
    lines.push(
      `# TYPE ${this.metricName("http_request_duration_ms")} histogram`,
    );
    ...
    lines.push("");
    lines.push(
      `# HELP ${this.metricName("task_executions_total")} Total number of task executions by status.`,
    );
    ...
    for (const [status, count] of Object.entries(executionMetrics)) {
      lines.push(
        `${this.metricName("task_executions_total")}{status="${status}"} ${count}`,
      );
    }

    lines.push("");
    lines.push(
      `# HELP ${this.metricName("agent_rounds_total")} Total number of agent rounds (workflow step completions).`,
    );
    lines.push(`# TYPE ${this.metricName("agent_rounds_total")} counter`);
    lines.push(
      `${this.metricName("agent_rounds_total")} ${this.getAgentRoundsCount()}`,
    );

    lines.push("");
    lines.push(
      `# HELP ${this.metricName("process_start_time_seconds")} Start time of the process since unix epoch in seconds.`,
    );
    lines.push(`# TYPE ${this.metricName("process_start_time_seconds")} gauge`);
    lines.push(
      `${this.metricName("process_start_time_seconds")} ${Math.floor(this.processStartTime / 1000)}`,
    );

    lines.push("");
    lines.push(
      `# HELP ${this.metricName("process_uptime_seconds")} Process uptime in seconds.`,
    );
    lines.push(`# TYPE ${this.metricName("process_uptime_seconds")} gauge`);
    lines.push(
      `${this.metricName("process_uptime_seconds")} ${Math.floor((Date.now() - this.processStartTime) / 1000)}`,
    );
    ...
    lines.push("");
    lines.push(
      `# HELP ${this.metricName("active_executions")} Current active executions.`,
    );
    lines.push(`# TYPE ${this.metricName("active_executions")} gauge`);
    lines.push(
      `${this.metricName("active_executions")} ${summary.runtimeMetrics.activeExecutions}`,
    );

    lines.push("");
    lines.push(
      `# HELP ${this.metricName("queued_tasks")} Current queued tasks.`,
    );
    lines.push(`# TYPE ${this.metricName("queued_tasks")} gauge`);
    lines.push(
      `${this.metricName("queued_tasks")} ${summary.runtimeMetrics.queuedTasks}`,
    );

    lines.push("");
    lines.push(
      `# HELP ${this.metricName("dead_letter_count")} Current dead letter count.`,
    );
    lines.push(`# TYPE ${this.metricName("dead_letter_count")} gauge`);
    lines.push(
      `${this.metricName("dead_letter_count")} ${summary.recoveryMetrics.deadLetterCount}`,
    );

    lines.push("");
    lines.push(
      `# HELP ${this.metricName("outbox_pending")} Current unpublished transactional outbox entries.`,
    );
    lines.push(`# TYPE ${this.metricName("outbox_pending")} gauge`);
    lines.push(
      `${this.metricName("outbox_pending")} ${this.getOutboxPendingCount()}`,
    );

    lines.push("");
    lines.push(
      `# HELP ${this.metricName("provider_success_rate")} Rolling provider success rate.`,
    );
    lines.push(`# TYPE ${this.metricName("provider_success_rate")} gauge`);
    lines.push(
      `${this.metricName("provider_success_rate")} ${summary.runtimeMetrics.providerSuccessRate}`,
    );

    lines.push("");
    lines.push(
      `# HELP ${this.metricName("memory_rss_bytes")} Resident set size in bytes.`,
    );
    lines.push(`# TYPE ${this.metricName("memory_rss_bytes")} gauge`);
    lines.push(
      `${this.metricName("memory_rss_bytes")} ${Math.round(summary.runtimeMetrics.memoryRssMb * 1024 * 1024)}`,
    );

    lines.push("");
    lines.push(
      `# HELP ${this.metricName("memory_heap_used_bytes")} Heap used in bytes.`,
    );
    lines.push(`# TYPE ${this.metricName("memory_heap_used_bytes")} gauge`);
    lines.push(
      `${this.metricName("memory_heap_used_bytes")} ${memUsage.heapUsed}`,
    );

    lines.push("");
    lines.push(
      `# HELP ${this.metricName("event_loop_lag_ms")} Event loop lag in milliseconds.`,
    );
    lines.push(`# TYPE ${this.metricName("event_loop_lag_ms")} gauge`);
    lines.push(
      `${this.metricName("event_loop_lag_ms")} ${summary.runtimeMetrics.eventLoopLagMs ?? 0}`,
    );

    lines.push("");
    lines.push(
      `# HELP ${this.metricName("redis_connection_errors")} Total Redis connection errors observed by runtime components.`,
    );
    ...
    lines.push("");
    lines.push(
      `# HELP ${this.metricName("queue_enqueue_failures_total")} Total queue enqueue write failures observed by backend and mode.`,
    );
    lines.push(
      `# TYPE ${this.metricName("queue_enqueue_failures_total")} counter`,
    );
    for (const line of this.renderCounterSeries(
    ...
    lines.push("");
    lines.push(
      `# HELP ${this.metricName("alert_delivery_failures_total")} Total alert delivery failures observed by channel.`,
    );
    lines.push(
      `# TYPE ${this.metricName("alert_delivery_failures_total")} counter`,
    );
    for (const line of this.renderCounterSeries(
    ...
    lines.push("");
    lines.push(
      `# HELP ${this.metricName("healthy_workers")} Healthy workers available.`,
    );
    lines.push(`# TYPE ${this.metricName("healthy_workers")} gauge`);
    lines.push(
      `${this.metricName("healthy_workers")} ${summary.runtimeMetrics.workerHealth.healthyWorkers}`,
    );

    lines.push("");
    lines.push(
      `# HELP ${this.metricName("total_workers")} Total workers registered.`,
    );
    lines.push(`# TYPE ${this.metricName("total_workers")} gauge`);
    lines.push(
      `${this.metricName("total_workers")} ${summary.runtimeMetrics.workerHealth.totalWorkers}`,
    );

    const diskUsage = this.getDiskUsage();
    lines.push("");
    lines.push(
      `# HELP ${this.metricName("disk_total_bytes")} Total bytes available in the current workspace filesystem.`,
    );
    lines.push(`# TYPE ${this.metricName("disk_total_bytes")} gauge`);
    lines.push(
      `${this.metricName("disk_total_bytes")} ${diskUsage.totalBytes}`,
    );

    lines.push("");
    lines.push(
      `# HELP ${this.metricName("disk_free_bytes")} Free bytes available in the current workspace filesystem.`,
    );
    ...
    lines.push("");
    lines.push(
      `# HELP ${this.metricName("disk_used_ratio")} Used ratio of the current workspace filesystem.`,
    );
    ...
    lines.push("");
    lines.push(
      `# HELP ${this.metricName("oapeflir_stage_duration_ms")} OAPEFLIR stage duration in milliseconds.`,
    );
    lines.push(
      `# TYPE ${this.metricName("oapeflir_stage_duration_ms")} histogram`,
    );
    for (const line of this.renderHistogramSeries(
    ...
    lines.push("");
    lines.push(
      `# HELP ${this.metricName("oapeflir_loop_duration_ms")} Backward-compatible alias for OAPEFLIR loop duration in milliseconds.`,
    );
    lines.push(
      `# TYPE ${this.metricName("oapeflir_loop_duration_ms")} histogram`,
    );
    for (const line of this.renderHistogramSeries(
    ...
    lines.push("");
    lines.push(
      `# HELP ${this.metricName("oapeflir_stage_outcome_total")} OAPEFLIR stage outcomes by result.`,
    );
    lines.push(
      `# TYPE ${this.metricName("oapeflir_stage_outcome_total")} counter`,
    );
    for (const line of this.renderCounterSeries(
    ...
    lines.push("");
    lines.push(
      `# HELP ${this.metricName("oapeflir_stage_entry_total")} OAPEFLIR stage entry count.`,
    );
    lines.push(
      `# TYPE ${this.metricName("oapeflir_stage_entry_total")} counter`,
    );
    ...
    lines.push("");
    lines.push(
      `# HELP ${this.metricName("llm_ttfb_seconds")} LLM time-to-first-token latency in seconds.`,
    );
    ...
    lines.push("");
    lines.push(
      `# HELP ${this.metricName("llm_total_seconds")} LLM total request latency in seconds.`,
    );
    ...
    lines.push("");
    lines.push(
      `# HELP ${this.metricName("knowledge_query_duration_ms")} Knowledge query duration in milliseconds.`,
    );
    lines.push(
      `# TYPE ${this.metricName("knowledge_query_duration_ms")} histogram`,
    );
    for (const line of this.renderHistogramSeries(
    ...
    lines.push("");
    lines.push(
      `# HELP ${this.metricName("knowledge_query_total")} Knowledge queries by operation and result.`,
    );
  ...
  private getDiskUsage(): {
    ...
    try {
      ...
      const usedRatio =
        totalBytes > 0
          ? Number(((totalBytes - freeBytes) / totalBytes).toFixed(6))
          : 0;
  ...
  private getHttpRequestCounts(): HttpRequestMetric[] {
    return runtimeMetricsRegistry
      .getCounters("http_requests_total")
      .map((series) => ({
  ...
  private renderHistogramSeries(name: string): string[] {
    ...
    for (const series of runtimeMetricsRegistry.getHistograms(name)) {
      for (let index = 0; index < series.buckets.length; index += 1) {
        lines.push(
          `${this.metricName(name)}_bucket${formatPrometheusLabels({ ...series.labels, le: String(series.buckets[index]) })} ${series.bucketCounts[index] ?? 0}`,
        );
      }
      lines.push(
        `${this.metricName(name)}_bucket${formatPrometheusLabels({ ...series.labels, le: "+Inf" })} ${series.count}`,
      );
      lines.push(
        `${this.metricName(name)}_sum${formatPrometheusLabels(series.labels)} ${series.sum}`,
      );
      lines.push(
        `${this.metricName(name)}_count${formatPrometheusLabels(series.labels)} ${series.count}`,
      );
    }
    return lines;
  }
...
function isRuntimeMetricNameAllowed(name: string): boolean {
  return (
    ALLOWED_RUNTIME_METRIC_NAMES.has(name) ||
