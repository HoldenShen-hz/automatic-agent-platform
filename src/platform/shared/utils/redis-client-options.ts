...
export function readRedisConnectionConfigFromEnv(
  prefix: string,
...
export function buildRedisClientOptions(
  ...
  const common: RedisClientOptions = {
...
function inferDeploymentMode(config: RedisConnectionConfig): RedisDeploymentMode {
  if (config.mode != null) {
    return config.mode;
  }
...
function parseNullableNonNegativeInteger(raw: string | undefined): number | null | undefined {
  const value = readTrimmedEnvValue(raw);
  if (value == null) {
    return undefined;
  }
...
function readDeploymentMode(raw: string | undefined, fallback?: RedisDeploymentMode): RedisDeploymentMode | null {
  const value = readTrimmedEnvValue(raw);
  if (value == null) {
...
function parseSentinelEndpoints(raw: string | undefined): RedisSentinelEndpoint[] {
  const value = readTrimmedEnvValue(raw);
  if (value == null) {
    return [];
  }
