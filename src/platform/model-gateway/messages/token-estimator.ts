 ...
 */
function readNumericUsage(payload: Record<string, unknown>): number | null {
...
export function estimateMessageTokens(
  message: Pick<MessageRecord, "content" | "partsJson">,
  options: {
  ...
  if (!options.trimmed) {
    ...
    const explicitUsage = parts.reduce((sum, part) => {
      try {
        const parsed = JSON.parse(part.contentJson) as unknown;
        if (!isRecord(parsed)) {
          return sum;
        }
