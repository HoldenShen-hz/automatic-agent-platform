...
// Credential status reflects the current state for selection purposes
...
export interface ProviderCredentialRecordInput {
  credentialId: string;
  apiKey?: string | null;
  // Reference to an external secret store (used with managed secrets)
  secretRef?: string | null;
  label?: string | null;
  status?: ProviderCredentialStatus;
  // Timestamp when cooldown ends (ISO 8601)
  cooldownUntil?: string | null;
  // When the rate limit resets (ISO 8601)
  resetAt?: string | null;
  // Last failure error code for debugging
  lastFailureCode?: string | null;
  // Retry after duration in milliseconds
  retryAfterMs?: number | null;
}

// Context provided to secret lease issuers/revokers
...
export interface ProviderCredentialManagedSecretLease {
  ...
  leaseId: string;
  expiresAt: string;
  // Whether the lease was issued by the provider or wrapped from external store
...
export interface ProviderCredentialManagedSecretAccess {
  // Resolves a secret reference to an actual API key
  ...
  // Revokes an active lease when request completes
  secretLeaseRevoker?: ((
    leaseId: string,
    context: ProviderCredentialManagedSecretContext & { reasonCode: string },
  ) => void) | null;
}

// Internal credential record with all fields normalized
export interface ProviderCredentialRecord extends Omit<ProviderCredentialRecordInput, "apiKey" | "secretRef"> {
  ...
  lastFailureCode: string | null;
  retryAfterMs: number | null;
  // Active lease information for managed secrets
  activeLeaseId: string | null;
  activeLeaseExpiresAt: string | null;
  activeLeaseSource: "provider_issued" | "wrapped_secret" | null;
}

// Current state of a credential including effective status
...
export interface ProviderCredentialSelection {
  ...
  // Why this credential was selected
...
export interface ProviderCredentialPoolOptions {
  ...
  // Default cooldown duration in milliseconds (default: 60 seconds)
...
export interface ProviderCredentialEnvLoadOptions extends ProviderCredentialManagedSecretAccess {
  // If true, secret refs are preserved rather than resolved immediately
  preserveManagedSecretRefs?: boolean;
}

// Error when all credentials are unavailable
 ...
 */
export function deriveProviderApiKeysJsonEnvName(providerId: string): string {
 ...
 */
export function deriveProviderApiKeySecretRefEnvName(providerId: string): string {
 ...
 */
export function deriveProviderApiKeySecretRefsJsonEnvName(providerId: string): string {
...
export function throwProviderCredentialValidation(
  provider: string,
  suffix: string,
...
export function throwProviderCredentialRuntimeError(
  provider: string,
  suffix: string,
  details: Record<string, unknown> = {},
): never {
 ...
 */
export function normalizeCredentialRecord(input: ProviderCredentialRecordInput): ProviderCredentialRecord {
...
export function addMilliseconds(timestamp: string, deltaMs: number): string {
 ...
 */
export function computeEffectiveStatus(record: ProviderCredentialRecord, now: string): ProviderCredentialStatus {
 ...
 */
export function isRetryableCredentialFailure(statusCode: number, retryAfterMs: number | null, resetAt: string | null): boolean {
 ...
 */
function parseJsonCredentialRecord(
  provider: string,
  item: unknown,
  index: number,
  ...
  const status: ProviderCredentialStatus | undefined =
    statusValue === "active" || statusValue === "cooling_down" || statusValue === "disabled"
      ? statusValue
      : undefined;
...
export function requireResolvedSecretValue(
  provider: string,
  secretRef: string,
  resolver: ((secretRef: string) => string) | null | undefined,
): string {
  if (resolver == null) {
 ...
 */
function parseJsonSecretRefCredentialRecord(
  provider: string,
  item: unknown,
  index: number,
  options: ProviderCredentialEnvLoadOptions,
): ProviderCredentialRecordInput {
 ...
 */
export function loadProviderCredentialRecordsFromEnv(
  provider: string,
  providerEnv: NodeJS.ProcessEnv,
  options: ProviderCredentialEnvLoadOptions = {},
): ProviderCredentialRecordInput[] {
  const records: ProviderCredentialRecordInput[] = [];
  ...
  if (typeof rawJson === "string" && rawJson.trim().length > 0) {
    ...
    try {
      ...
      throw new ValidationError(
        `provider.credentials_invalid:${provider}:json_parse`,
        `provider.credentials_invalid:${provider}:json_parse`,
        {
          retryable: false,
          source: "provider",
  ...
  if (typeof rawSecretRefsJson === "string" && rawSecretRefsJson.trim().length > 0) {
    ...
    try {
      ...
      throw new ValidationError(
        `provider.credentials_invalid:${provider}:secret_refs_json_parse`,
        `provider.credentials_invalid:${provider}:secret_refs_json_parse`,
        {
          retryable: false,
          source: "provider",
