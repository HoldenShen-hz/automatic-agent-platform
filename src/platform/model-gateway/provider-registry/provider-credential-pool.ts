...
import {
  ...
  type ProviderCredentialFailureSignal,
  type ProviderCredentialManagedSecretAccess,
  type ProviderCredentialManagedSecretContext,
  type ProviderCredentialPoolExhaustion,
  type ProviderCredentialPoolOptions,
  type ProviderCredentialPoolState,
  type ProviderCredentialRecord,
  type ProviderCredentialSelection,
  type ProviderCredentialStatus,
} from "./provider-credential-pool-support.js";
...
export class ProviderCredentialPool {
  ...
  private readonly managedSecretAccess: ProviderCredentialManagedSecretAccess;
  private disposed = false;

  public constructor(options: ProviderCredentialPoolOptions) {
   ...
   */
  public getStates(now: string = new Date().toISOString()): ProviderCredentialPoolState[] {
   ...
   */
  public async selectCredential(options: {
    ...
    const preferred =
      options.preferredCredentialId != null
        ? states.find(
            (state) => state.credentialId === options.preferredCredentialId && state.available && !excluded.has(state.credentialId),
          )
        : null;
    if (preferred != null) {
    ...
    for (const state of states) {
      ...
      return await this.buildSelection(
        state.credentialId,
        original?.status === "cooling_down" ? "reactivated_after_cooldown" : "first_active_credential",
        now,
      );
    }
   ...
   */
  public markSuccess(credentialId: string): ProviderCredentialRecord | null {
    const record = this.credentials.get(credentialId);
    if (record == null) {
      return null;
    }
   ...
   */
  public markFailure(signal: ProviderCredentialFailureSignal): ProviderCredentialRecord | null {
    ...
    let status: ProviderCredentialStatus = record.status;
   ...
   */
  public async canFailoverAfter(signal: {
    statusCode: number;
  ...
  public releaseCredential(
    selection: Pick<ProviderCredentialSelection, "credentialId" | "leaseId">,
    reasonCode: string = "provider.request_completed",
   ...
   */
  public getExhaustion(now: string = new Date().toISOString()): ProviderCredentialPoolExhaustion {
    this.assertNotDisposed();
    const states = this.getStates(now);
    if (states.length === 0) {
      return {
   ...
   */
  public dispose(reasonCode: string = "provider.pool_disposed"): void {
    if (this.disposed) {
      return;
    }
  ...
  private async buildSelection(
    credentialId: string,
    routeReason: ProviderCredentialSelection["routeReason"],
    now: string,
   ...
   */
  private async materializeCredential(credentialId: string, now: string): Promise<ProviderCredentialRecord> {
    const record = this.credentials.get(credentialId);
    if (record == null) {
  ...
  private buildManagedSecretContext(record: ProviderCredentialRecord): ProviderCredentialManagedSecretContext {
    return {
      provider: this.provider,
      credentialId: record.credentialId,
      label: record.label,
  ...
  private releaseLeaseState(record: ProviderCredentialRecord, reasonCode: string): ProviderCredentialRecord {
  ...
  private assertNotDisposed(): void {
    if (this.disposed) {
      throw new ProviderError(
        "provider.credential_pool.disposed",
        `Provider credential pool for ${this.provider} has been disposed.`,
        {
          source: "provider",
          retryable: false,
