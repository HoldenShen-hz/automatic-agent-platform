...
} from "../../five-plane-state-evidence/memory-gateway/index.js";
...
import {
  ...
  requiredEnv,
  requiredEnumValue,
  type ControlPlaneBalancerCliEnvConfig,
  type DeploymentExecutionCliEnvConfig,
  type EnterpriseCapabilityCliEnvConfig,
  type GatewayTargetsCliEnvConfig,
  type InspectCliEnvConfig,
  type MarketplaceCliEnvConfig,
  type MemoryCliEnvConfig,
  type ModelRoutingCliEnvConfig,
  type OpsGovernanceCliEnvConfig,
  type SecretManagementCliEnvConfig,
  type ShadowSnapshotCliEnvConfig,
  type SkillCreatorCliEnvConfig,
  type TenantPlatformCliEnvConfig,
  type WorkerRegisterCliEnvConfig,
} from "./remaining-cli-env-support.js";

export function loadTenantPlatformCliEnv(env: NodeJS.ProcessEnv = process.env): TenantPlatformCliEnvConfig {
  return {
    ...
    setAsOrganizationDefault: parseBoolean(env, "AA_SET_DEFAULT_TENANT") ?? false,
 ...
 */
export function loadEnterpriseCapabilityCliEnv(env: NodeJS.ProcessEnv = process.env): EnterpriseCapabilityCliEnvConfig {
  return {
    dbPath: requiredEnv(env, "AA_DB_PATH"),
    ...
    credentialReady: parseBoolean(env, "AA_CREDENTIAL_READY") ?? false,
    ...
    isActive: parseBoolean(env, "AA_IS_ACTIVE") ?? true,
 ...
 */
export function loadMarketplaceCliEnv(env: NodeJS.ProcessEnv = process.env): MarketplaceCliEnvConfig {
  return {
    dbPath: requiredEnv(env, "AA_DB_PATH"),
    ...
    signatureVerified: parseBoolean(env, "AA_SIGNATURE_VERIFIED") ?? false,
    manifestChecksum: optionalEnv(env, "AA_MANIFEST_CHECKSUM"),
    sbomVerified: parseBoolean(env, "AA_SBOM_VERIFIED") ?? false,
    sandboxCertVerified: parseBoolean(env, "AA_SANDBOX_CERT_VERIFIED") ?? false,
    egressPolicyCompliant: parseBoolean(env, "AA_EGRESS_POLICY_COMPLIANT") ?? false,
    ...
    reviewRequired: parseBoolean(env, "AA_REVIEW_REQUIRED") ?? true,
 ...
 */
export function loadDeploymentExecutionCliEnv(
  ...
  return {
    ...
    execute: parseBoolean(env, "AA_DEPLOYMENT_EXECUTE") ?? false,
  };
}
 ...
 */
export function loadControlPlaneBalancerCliEnv(env: NodeJS.ProcessEnv = process.env): ControlPlaneBalancerCliEnvConfig {
 ...
 */
export function loadOpsGovernanceCliEnv(env: NodeJS.ProcessEnv = process.env): OpsGovernanceCliEnvConfig {
  return {
    dbPath: requiredEnv(env, "AA_DB_PATH"),
 ...
 */
export function loadSecretManagementCliEnv(env: NodeJS.ProcessEnv = process.env): SecretManagementCliEnvConfig {
  return {
    dbPath: requiredEnv(env, "AA_DB_PATH"),
    ...
    breakGlass: parseBoolean(env, "AA_SECRET_BREAK_GLASS") ?? false,
 ...
 */
export function loadWorkerRegisterCliEnv(env: NodeJS.ProcessEnv = process.env): WorkerRegisterCliEnvConfig {
  return {
    dbPath: requiredEnv(env, "AA_DB_PATH"),
...
export function loadGatewayTargetsCliEnv(env: NodeJS.ProcessEnv = process.env): GatewayTargetsCliEnvConfig {
  return {
...
export function loadInspectCliEnv(env: NodeJS.ProcessEnv = process.env): InspectCliEnvConfig {
  return {
    dbPath: optionalEnv(env, "AA_DB_PATH") ?? undefined,
...
export function loadSkillCreatorCliEnv(env: NodeJS.ProcessEnv = process.env): SkillCreatorCliEnvConfig {
  return {
    ...
    resourceDirectories: (parseJsonValue(env, "AA_SKILL_RESOURCE_DIRS_JSON") as Array<Record<string, unknown>> | null) ?? undefined,
...
export function loadShadowSnapshotCliEnv(env: NodeJS.ProcessEnv = process.env): ShadowSnapshotCliEnvConfig {
  return {
...
export function loadMemoryCliEnv(env: NodeJS.ProcessEnv = process.env): MemoryCliEnvConfig {
  return {
    dbPath: optionalEnv(env, "AA_DB_PATH") ?? undefined,
    ...
    contentJson: parseTypedJson<Record<string, unknown>>(env, "AA_MEMORY_CONTENT_JSON"),
    ...
    memoryLayer: optionalEnv(env, "AA_MEMORY_LAYER") as MemoryLayer | null ?? undefined,
    sourceTrustLevel: optionalEnv(env, "AA_MEMORY_SOURCE_TRUST") as MemorySourceTrustLevel | null ?? undefined,
    ...
    memoryLayers: (parseStringArrayFromCsv(env, "AA_MEMORY_LAYERS") as MemoryLayer[] | null) ?? undefined,
    classifications: parseStringArrayFromCsv(env, "AA_MEMORY_CLASSIFICATIONS") ?? undefined,
    sourceTrustLevels: (parseStringArrayFromCsv(env, "AA_MEMORY_SOURCE_TRUST_LEVELS") as MemorySourceTrustLevel[] | null) ?? undefined,
    ...
    targetMemoryLayer: optionalEnv(env, "AA_MEMORY_TARGET_LAYER") as Exclude<MemoryLayer, "layer_3"> | null ?? undefined,
    ...
    facts: parseTypedJson<StructuredMemoryContent["facts"]>(env, "AA_MEMORY_FACTS_JSON"),
    ...
    experienceTools: parseTypedJson<MemoryCliEnvConfig["experienceTools"]>(env, "AA_EXPERIENCE_TOOLS_JSON"),
    experienceOutcome: optionalEnv(env, "AA_EXPERIENCE_OUTCOME") as MemoryCliEnvConfig["experienceOutcome"] ?? undefined,
...
export function buildMemoryProviderQuery(config: MemoryCliEnvConfig): MemoryProviderQuery {
  const query: MemoryProviderQuery = {};
  if (config.taskId != null) query.taskId = config.taskId;
  if (config.sessionId != null) query.sessionId = config.sessionId;
  if (config.agentId != null) query.agentId = config.agentId;
  if (config.executionId != null) query.executionId = config.executionId;
  if (config.scopes != null) query.scopes = config.scopes;
  if (config.memoryLayers != null) query.memoryLayers = config.memoryLayers;
  if (config.classifications != null) query.classifications = config.classifications;
  if (config.sourceTrustLevels != null) query.sourceTrustLevels = config.sourceTrustLevels;
  if (config.minQualityScore != null) query.minQualityScore = config.minQualityScore;
  if (config.limit != null) query.limit = config.limit;
  if (config.maxPromptMemories != null) query.maxPromptMemories = config.maxPromptMemories;
  if (config.maxFewShotExamples != null) query.maxFewShotExamples = config.maxFewShotExamples;
  if (config.queryText != null) query.queryText = config.queryText;
  if (config.taskIntent != null) query.taskIntent = config.taskIntent;
  if (config.toolNames != null) query.toolNames = config.toolNames;
  if (config.evaluatedAt != null) query.evaluatedAt = config.evaluatedAt;
  if (config.includeExperienceExamples != null) query.includeExperienceExamples = config.includeExperienceExamples;
  return query;
}

export function buildStructuredMemoryContentFromCliEnv(
  config: MemoryCliEnvConfig,
): StructuredMemoryContent | undefined {
  if (
    config.workContext == null
...
export function loadModelRoutingCliEnv(env: NodeJS.ProcessEnv = process.env): ModelRoutingCliEnvConfig {
  return {
    ...
    fallbackLease: parseTypedJson<ModelRouteFallbackLease>(env, "AA_MODEL_ROUTE_FALLBACK_LEASE_JSON"),
    ...
    governanceSnapshot: parseTypedJson<ModelGovernanceSnapshot>(env, "AA_MODEL_ROUTE_GOVERNANCE_SNAPSHOT_JSON"),
