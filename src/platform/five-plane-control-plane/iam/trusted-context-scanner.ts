...
import { checkSandboxPath, createWorkspaceWritePolicy, type SandboxPolicy } from "./sandbox-policy.js";
...
export interface TrustedContextFinding {
  ...
  code:
    | "context_trust.prompt_injection_phrase"
    | "context_trust.hidden_unicode"
    | "context_trust.control_character"
    | "context_trust.encoded_instruction"
    | "context_trust.sandbox_denied";
...
const PROMPT_INJECTION_PATTERNS: ReadonlyArray<{
  code: TrustedContextFinding["code"];
  severity: TrustedContextFindingSeverity;
  pattern: RegExp;
  message: string;
}> = [
 ...
 */
function computeTrustLevel(findings: readonly TrustedContextFinding[]): TrustedContextTrustLevel {
 ...
 */
function buildSnippet(line: string, column: number): string {
 ...
 */
function scanLine(line: string, lineNumber: number): TrustedContextFinding[] {
  const findings: TrustedContextFinding[] = [];
 ...
 */
export function scanTrustedContextContent(filePath: string, content: string): TrustedContextFileReport {
 ...
 */
function collectDivisionFiles(divisionsRoot: string): string[] {
 ...
 */
function readTrustedContextFile(filePath: string, policy: SandboxPolicy): TrustedContextFileReport {
 ...
 */
export function scanTrustedContextWorkspace(
  workspaceRoot: string,
  options: TrustedContextScannerOptions = {},
): TrustedContextWorkspaceReport {
