...
import { IndustrialOpsProgramService, type IndustrialOpsProgramReport } from "./industrial-ops-program-service.js";
import { OperationsGovernanceService, type OperationsGovernanceReport } from "./operations-governance-service.js";
import {
...
import {
  ...
  sha256,
  summarizeVerdict,
  type ApmMetricSample,
  type EnterpriseGovernanceApmExportBundle,
  type EnterpriseGovernanceExportResult,
  type EnterpriseGovernanceInput,
  type EnterpriseGovernanceReport,
  type EnterpriseGovernanceRunResult,
  type EnterpriseGovernanceSchemaGateReport,
  type EnterpriseGovernanceServiceOptions,
  type IncidentHandoffPackage,
  type PackageLockDocument,
  type PackageManifest,
  type SupplyChainFinding,
  type SupplyChainSbomComponent,
  type SupplyChainSecurityReport,
} from "./enterprise-governance-support.js";
export type {
  EnterpriseGovernanceStatus,
  IncidentHandoffStatus,
  SupplyChainFindingSeverity,
  EnterpriseGovernanceInput,
  EnterpriseGovernanceSchemaGateReport,
  SupplyChainSbomComponent,
  SupplyChainExtensionSummary,
  SupplyChainFinding,
  SupplyChainSecurityReport,
  ApmMetricSample,
  EnterpriseGovernanceApmExportBundle,
  IncidentHandoffPackage,
  EnterpriseGovernanceReport,
  EnterpriseGovernanceRunResult,
  EnterpriseGovernanceExportResult,
  EnterpriseGovernanceServiceOptions,
} from "./enterprise-governance-support.js";
...
export class EnterpriseGovernanceService {
  ...
  private readonly opsProgramService: IndustrialOpsProgramService;
  private readonly cveIntelligence: CveIntelligenceService | null;

  public constructor(
    private readonly governanceService: OperationsGovernanceService,
    private readonly store: AuthoritativeTaskStore,
    options: EnterpriseGovernanceServiceOptions = {},
  ) {
   ...
   */
  public buildReport(input: EnterpriseGovernanceInput): EnterpriseGovernanceReport {
    ...
    const status = summarizeVerdict(
      opsProgram.status === "fail" || schemaGate.verdict === "fail" || supplyChain.verdict === "fail",
      opsProgram.status === "warning" || supplyChain.verdict === "warning" || incidentHandoff.status === "warning",
    );
   ...
   */
  public runReport(input: EnterpriseGovernanceInput): EnterpriseGovernanceRunResult {
   ...
   */
  public exportReport(input: EnterpriseGovernanceInput): EnterpriseGovernanceExportResult {
   ...
   */
  public listHistory(limit = 20): EnterpriseGovernanceReportRecord[] {
   ...
   */
  public listIncidentHandoffs(limit = 20): IncidentHandoffRecord[] {
   ...
   */
  private buildSchemaGateReport(): EnterpriseGovernanceSchemaGateReport {
   ...
   */
  private buildSupplyChainReport(input: {
    generatedAt: string;
    ...
    if (!existsSync(normalizedManifestPath)) {
      throw new StorageError(
        `enterprise_governance.manifest_not_found:${normalizedManifestPath}`,
        `enterprise_governance.manifest_not_found:${normalizedManifestPath}`,
        {
          statusCode: 404,
          retryable: false,
    ...
    if (!existsSync(normalizedLockfilePath)) {
      throw new StorageError(
        `enterprise_governance.lockfile_not_found:${normalizedLockfilePath}`,
        `enterprise_governance.lockfile_not_found:${normalizedLockfilePath}`,
        {
          statusCode: 404,
          retryable: false,
    ...
    const lockfile = JSON.parse(lockfileText) as PackageLockDocument;

    // Validate lockfile version (only v3 supported)
    if (lockfile.lockfileVersion !== 3 || lockfile.packages == null) {
      throw new ValidationError(
        `enterprise_governance.unsupported_lockfile:${normalizedLockfilePath}`,
        `enterprise_governance.unsupported_lockfile:${normalizedLockfilePath}`,
        {
          retryable: false,
    ...
    const components: SupplyChainSbomComponent[] = [];
    ...
    Object.entries(lockfile.packages)
      ...
      .forEach(([packagePath, packageInfo]) => {
        ...
        const component: SupplyChainSbomComponent = {
          packageName,
          packagePath,
    ...
    let cveReport: { totalCves: number; matchedCves: number; matches: CveMatchResult[] } | undefined = undefined;
  ...
  private buildApmExport(
    ...
    handoff: IncidentHandoffPackage,
    generatedAt: string,
    ...
    const metricSamples: ApmMetricSample[] = [
      {
   ...
   */
  private toHandoffRecord(handoff: IncidentHandoffPackage): IncidentHandoffRecord {
    return {
   ...
   */
  private toRecord(report: EnterpriseGovernanceReport): EnterpriseGovernanceReportRecord {
    return {
   ...
   */
  private ensureArtifactTask(taskId: string | null, createdAt: string): string {
    if (taskId != null) {
      return taskId;
    }
