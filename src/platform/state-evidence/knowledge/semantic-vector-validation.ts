...
import {
  ...
  resolveSemanticVectorBackend,
} from "./semantic-vector-store.js";

export interface SemanticVectorReadinessCheck {
  name:
    | "storage_driver_postgres"
    | "semantic_backend_pgvector"
    | "database_available"
    | "pgvector_extension_installed"
    | "semantic_table_present"
    | "semantic_ivfflat_index_present"
    | "semantic_roundtrip";
  ok: boolean;
  required: boolean;
...
export async function validateSemanticVectorReadiness(
  options: SemanticVectorReadinessOptions,
): Promise<SemanticVectorReadinessReport> {
  ...
  const report: SemanticVectorReadinessReport = {
    validatedAt: nowIso(),
  ...
  const pushCheck = (check: SemanticVectorReadinessCheck): void => {
    checks.push(check);
  };
  const finalize = (): SemanticVectorReadinessReport => {
  ...
  const extensionInstalled = Boolean((await options.database.asyncConnection.queryOne<BooleanRow>(
  ...
  const tablePresent = Boolean((await options.database.asyncConnection.queryOne<RegclassRow>(
  ...
  const ivfflatIndexPresent = Boolean((await options.database.asyncConnection.queryOne<BooleanRow>(
    `
      SELECT EXISTS(
        SELECT 1
