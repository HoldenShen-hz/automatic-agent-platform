...
export class AsyncWorkflowRepository {
  ...
  public async insertWorkflowState(workflow: WorkflowStateRecord): Promise<void> {
    await asyncExecute(
      this.conn,
      ...
      workflow.taskId, workflow.divisionId, workflow.workflowId, workflow.currentStepIndex,
      workflow.status, workflow.outputsJson, workflow.lastErrorCode, workflow.retryCount,
      workflow.resumableFromStep, workflow.startedAt, workflow.updatedAt,
    );
  }

  public async insertStepOutput(stepOutput: StepOutputRecord): Promise<void> {
    await asyncExecute(
      this.conn,
  ...
  public async getWorkflowState(taskId: string, tenantId?: string | null): Promise<WorkflowStateRecord | null> {
    const scopedTenantId = resolveTenantScope(tenantId);
    if (scopedTenantId !== undefined) {
      const result = await asyncQueryOne<WorkflowStateRecord>(
        this.conn,
        `SELECT w.task_id AS "taskId", w.division_id AS "divisionId", w.workflow_id AS "workflowId",
         ...
         FROM workflow_state w INNER JOIN tasks t ON t.id = w.task_id WHERE w.task_id = $1 AND t.tenant_id = $2`,
    ...
    const result = await asyncQueryOne<WorkflowStateRecord>(
      this.conn,
      `SELECT task_id AS "taskId", division_id AS "divisionId", workflow_id AS "workflowId",
       ...
       FROM workflow_state WHERE task_id = $1`,
      taskId,
    );
    return result ?? null;
  }

  public async listWorkflowStates(tenantId?: string | null): Promise<WorkflowStateRecord[]> {
    const scopedTenantId = resolveTenantScope(tenantId);
    if (scopedTenantId !== undefined) {
      return asyncQueryAll<WorkflowStateRecord>(
        this.conn,
        `SELECT w.task_id AS "taskId", w.division_id AS "divisionId", w.workflow_id AS "workflowId",
          w.current_step_index AS "currentStepIndex", w.status, w.outputs_json AS "outputsJson",
          w.last_error_code AS "lastErrorCode", w.retry_count AS "retryCount",
          w.resumable_from_step AS "resumableFromStep", w.started_at AS "startedAt", w.updated_at AS "updatedAt"
         FROM workflow_state w WHERE w.task_id IN (SELECT id FROM tasks WHERE tenant_id = $1)`,
        scopedTenantId,
      );
    }
    return asyncQueryAll<WorkflowStateRecord>(
  ...
  public async updateWorkflowState(
    taskId: string, status: string, currentStepIndex: number, outputsJson: string, updatedAt: string, resumableFromStep: string | null = null,
  ): Promise<void> {
    await asyncExecute(
      this.conn,
      `UPDATE workflow_state SET status = $1, current_step_index = $2, outputs_json = $3, updated_at = $4, resumable_from_step = $5 WHERE task_id = $6`,
      status, currentStepIndex, outputsJson, updatedAt, resumableFromStep, taskId,
    );
  }

  public async updateWorkflowRecoveryState(input: {
    taskId: string; status: string; currentStepIndex: number; outputsJson: string; updatedAt: string;
    resumableFromStep: string | null; retryCount: number; lastErrorCode: string | null;
  }): Promise<void> {
    await asyncExecute(
      this.conn,
      `UPDATE workflow_state SET status = $1, current_step_index = $2, outputs_json = $3, updated_at = $4,
        resumable_from_step = $5, retry_count = $6, last_error_code = $7 WHERE task_id = $8`,
      input.status, input.currentStepIndex, input.outputsJson, input.updatedAt,
