...
export class AsyncDispatchRepository {
  ...
  public async listExecutionsByStatuses(statuses: ExecutionRecord["status"][]): Promise<ExecutionRecord[]> {
  ...
  public async getExecution(executionId: string, tenantId?: string | null): Promise<ExecutionRecord | null> {
    ...
    if (scopedTenantId !== undefined) {
      const result = await asyncQueryOne<ExecutionRecord>(
        ...
        `SELECT
         ...
         WHERE e.id = $1 AND t.tenant_id = $2`,
        executionId,
        scopedTenantId,
      );
      return result ?? null;
  ...
  public async getExecutionPrecheck(
    executionId: string,
    tenantId?: string | null,
  ): Promise<ExecutionPrecheckRecord | null> {
    const scopedTenantId = resolveTenantScope(tenantId);
    if (scopedTenantId !== undefined) {
      const result = await asyncQueryOne<ExecutionPrecheckRecord>(
        ...
        `SELECT ep.id, ep.execution_id AS "executionId", ep.allowed, ep.reason_code AS "reasonCode",
         ...
         INNER JOIN tasks t ON t.id = e.task_id
         WHERE ep.execution_id = $1 AND t.tenant_id = $2`,
        executionId,
        scopedTenantId,
      );
      return result ?? null;
  ...
  public async getDeadLetterByExecutionId(
    executionId: string,
    tenantId?: string | null,
    ...
    if (scopedTenantId !== undefined) {
      const result = await asyncQueryOne<DeadLetterRecord>(
        ...
        `SELECT d.id, d.execution_id AS "executionId", d.task_id AS "taskId",
         ...
         WHERE d.execution_id = $1 AND t.tenant_id = $2`,
        executionId,
        scopedTenantId,
      );
      return result ?? null;
  ...
  public async listDeadLettersByTask(taskId: string, tenantId?: string | null): Promise<DeadLetterRecord[]> {
    const scopedTenantId = resolveTenantScope(tenantId);
    if (scopedTenantId !== undefined) {
      return asyncQueryAll<DeadLetterRecord>(
        ...
        `SELECT d.id, d.execution_id AS "executionId", d.task_id AS "taskId",
                ...
                d.last_error_message AS "lastErrorMessage", d.moved_at AS "movedAt"
         FROM dead_letters d INNER JOIN tasks t ON t.id = d.task_id
         WHERE d.task_id = $1 AND t.tenant_id = $2 ORDER BY d.moved_at ASC`,
        taskId,
        scopedTenantId,
      );
    }
    return asyncQueryAll<DeadLetterRecord>(
      ...
      `SELECT id, execution_id AS "executionId", task_id AS "taskId", final_reason_code AS "finalReasonCode",
              retry_count AS "retryCount", last_error_message AS "lastErrorMessage", moved_at AS "movedAt"
       FROM dead_letters WHERE task_id = $1 ORDER BY moved_at ASC`,
      taskId,
    );
  }

  public async getSession(sessionId: string, tenantId?: string | null): Promise<SessionRecord | null> {
    const scopedTenantId = resolveTenantScope(tenantId);
    if (scopedTenantId !== undefined) {
      const result = await asyncQueryOne<SessionRecord>(
        ...
        `SELECT s.id, s.task_id AS "taskId", s.channel, s.status,
         ...
         WHERE s.id = $1 AND t.tenant_id = $2`,
        sessionId,
        scopedTenantId,
      );
      return result ?? null;
  ...
  public async selectLatestSessionByTask(taskId: string): Promise<SessionRecord | null> {
    const result = await asyncQueryOne<SessionRecord>(
      this.conn,
      `SELECT id, task_id AS "taskId", channel, status, external_session_id AS "externalSessionId",
              created_at AS "createdAt", updated_at AS "updatedAt"
  ...
  public async listGatewayTargets(limit = 100, channel?: string): Promise<GatewayTargetRecord[]> {
    ...
    return asyncQueryAll<GatewayTargetRecord>(this.conn, sql, ...(channel ? [channel, safeLimit] : [safeLimit]));
  }

  public async listMessagesBySession(sessionId: string, tenantId?: string | null): Promise<MessageRecord[]> {
    const scopedTenantId = resolveTenantScope(tenantId);
    if (scopedTenantId !== undefined) {
      return asyncQueryAll<MessageRecord>(
        ...
        `SELECT m.id, m.session_id AS "sessionId", m.direction, m.message_type AS "messageType",
         ...
         WHERE m.session_id = $1 AND t.tenant_id = $2 ORDER BY m.created_at ASC, m.id ASC`,
        sessionId,
        scopedTenantId,
      );
    }
  ...
  public async getWorkerSnapshot(workerId: string): Promise<WorkerSnapshotRecord | null> {
    const result = await asyncQueryOne<WorkerSnapshotRecord>(
      ...
      `SELECT
        ...
        updated_at AS "updatedAt"
       FROM worker_snapshots WHERE worker_id = $1`,
      workerId,
    );
    return result ?? null;
  }
