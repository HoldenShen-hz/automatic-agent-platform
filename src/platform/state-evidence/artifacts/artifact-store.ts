...
} from "./sensitive-content-scanner.js";
...
export class ArtifactStore {
  ...
  private readonly sensitiveContentScanner: SensitiveContentScanner;
  ...
  */
  public writeTextArtifact(input: ArtifactWriteInput): ArtifactWriteResult {
   ...
   * @returns The created artifact record and a lightweight reference
   */
  public writeJsonArtifact(input: Omit<ArtifactWriteInput, "mimeType" | "content"> & { content: unknown }): ArtifactWriteResult {
  ...
  private writeArtifact(
    input: Omit<ArtifactWriteInput, "content"> & {
      sanitizedContent: SanitizedToolOutput;
    ...
    const taskPolicy: SandboxPolicy = {
      ...rootPolicy,
   ...
   */
  private ensureWritableDirectory(path: string, policy: SandboxPolicy): string {
 ...
 */
function sanitizeArtifactContent(content: string): SanitizedToolOutput {
 ...
 */
function sanitizeStructuredArtifactContent(content: unknown): SanitizedToolOutput & { changed: boolean } {
