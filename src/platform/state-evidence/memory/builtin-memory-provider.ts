...
import type {
  ...
  FewShotExample,
} from "./experience-cache-service.js";
import { ExperienceCacheService } from "./experience-cache-service.js";
import type { MemoryService, RememberMemoryInput } from "./memory-service.js";
...
import type { SimilarExperienceQuery } from "./experience-cache-service.js";

// Represents an in-flight prefetch request
interface PrefetchJob {
  requestId: string;
  queuedAt: string;
  state: "queued" | "completed" | "failed";
  promise: Promise<MemoryProviderPrefetchResult>;
}
...
function summarizeMemory(record: MemoryRecord, tokenBudget: number = 256): string {
  ...
  if (fragments.length === 0) {
    for (const fact of structured.facts) {
      if (!tryAddFragment(fact.content)) break;
    }
  }
...
export class BuiltInMemoryProvider implements MemoryProvider {
   ...
   */
  public async initialize(): Promise<MemoryProviderInitializeResult> {
   ...
   */
  public async systemPromptBlock(query: MemoryProviderQuery): Promise<MemoryProviderPromptBlock> {
   ...
   */
  public async prefetch(query: MemoryProviderQuery): Promise<MemoryProviderPrefetchResult> {
   ...
   */
  public async queuePrefetch(query: MemoryProviderQuery): Promise<QueuedMemoryPrefetch> {
    await this.initialize();
   ...
   */
  public async awaitQueuedPrefetch(requestId: string): Promise<MemoryProviderPrefetchResult | null> {
    const job = this.prefetchJobs.get(requestId);
    if (job == null) {
      return null;
    }
  ...
  public async syncTurn(input: MemoryTurnSyncInput): Promise<MemoryTurnSyncResult> {
    ...
    const rememberedMemoryIds: string[] = [];
    const rememberedMemories: MemoryRecord[] = [];

    // Store and index each memory
    for (const memoryInput of input.memories ?? []) {
      const remembered = this.memoryService.remember(memoryInput as RememberMemoryInput);
    ...
    let recordedExperienceId: string | null = null;
   ...
   */
  public async shutdown(): Promise<MemoryProviderShutdownResult> {
   ...
   */
  private async computePrefetch(
    query: MemoryProviderQuery,
    queued: boolean,
    forcedRequestId?: string,
  ): Promise<MemoryProviderPrefetchResult> {
   ...
   */
  private resolveMemories(query: MemoryProviderQuery): MemoryRecord[] {
    if (query.queryText && query.queryText.trim().length > 0) {
      // Build recall query from provider query
      const memoryQuery: MemoryRecallQuery = {};
      if (query.taskId != null) memoryQuery.taskId = query.taskId;
      if (query.sessionId != null) memoryQuery.sessionId = query.sessionId;
      if (query.agentId != null) memoryQuery.agentId = query.agentId;
      if (query.executionId != null) memoryQuery.executionId = query.executionId;
      if (query.scopes != null) memoryQuery.scopes = query.scopes;
      if (query.memoryLayers != null) memoryQuery.memoryLayers = query.memoryLayers;
      if (query.classifications != null) memoryQuery.classifications = query.classifications;
      if (query.sourceTrustLevels != null) memoryQuery.sourceTrustLevels = query.sourceTrustLevels;
      if (query.minQualityScore != null) memoryQuery.minQualityScore = query.minQualityScore;
      if (query.limit != null) memoryQuery.limit = query.limit;
      if (query.evaluatedAt != null) memoryQuery.evaluatedAt = query.evaluatedAt;
   ...
   */
  private resolveExperiences(query: MemoryProviderQuery): ExperienceRetrievalResult {
    if (
      query.includeExperienceExamples === false
    ...
    const experienceQuery: SimilarExperienceQuery = {
    ...
    const filteredExamples: FewShotExample[] = [];
    const filteredExperienceIds: string[] = [];
