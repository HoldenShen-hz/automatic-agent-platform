...
export class WorkerServiceIdentityRegistry {
  ...
  public register(identity: WorkerServiceIdentity): WorkerServiceIdentity {
    ...
    const snapshot = {
      ...
      isolationLevel: existing?.isolationLevel ?? null,
      repoVersion: existing?.repoVersion ?? null,
      remoteSessionStatus: existing?.remoteSessionStatus ?? null,
      lastAcknowledgedStreamOffset: existing?.lastAcknowledgedStreamOffset ?? null,
      streamResumeSuccessRate: existing?.streamResumeSuccessRate ?? null,
      credentialRefreshSuccessRate: existing?.credentialRefreshSuccessRate ?? null,
      sessionConsistencyCheckStatus: existing?.sessionConsistencyCheckStatus ?? null,
      sessionConsistencyCheckedAt: existing?.sessionConsistencyCheckedAt ?? null,
      workspaceSyncStatus: existing?.workspaceSyncStatus ?? null,
      workspaceSyncCheckedAt: existing?.workspaceSyncCheckedAt ?? null,
      saturation: existing?.saturation ?? null,
      activeLeaseCount: existing?.activeLeaseCount ?? 0,
      meanStartupLatencyMs: existing?.meanStartupLatencyMs ?? null,
      sandboxSuccessRate: existing?.sandboxSuccessRate ?? null,
      repoCacheHitRate: existing?.repoCacheHitRate ?? null,
      registrationVerifiedAt: existing?.registrationVerifiedAt ?? null,
      registrationChallengeId: existing?.registrationChallengeId ?? null,
      ...
      queueAffinity: existing?.queueAffinity ?? null,
      ...
      cpuPct: existing?.cpuPct ?? null,
      memoryMb: existing?.memoryMb ?? null,
