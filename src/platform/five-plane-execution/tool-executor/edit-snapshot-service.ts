...
export class EditSnapshotService {
   ...
   */
  public getPreviousContent(stepId: string, filePath: string): string | null {
    const history = this.editHistory.get(stepId);
   ...
   */
  public getUndoContent(stepId: string, filePath: string): string | null {
    const history = this.editHistory.get(stepId);
    if (!history || history.length === 0) {
      return null;
    }
   ...
   */
  public getRedoContent(stepId: string, filePath: string): string | null {
   ...
   */
  public undo(stepId: string): EditHistoryEntry | null {
    const history = this.editHistory.get(stepId);
    if (!history || history.length === 0) {
      return null;
    }
   ...
   */
  public redo(stepId: string): EditHistoryEntry | null {
    const stack = this.redoStack.get(stepId);
    if (!stack || stack.length === 0) {
      return null;
    }
   ...
   */
  public getState(stepId: string): UndoRedoState {
    const history = this.editHistory.get(stepId) ?? [];
   ...
   */
  public getHistory(stepId: string): readonly EditHistoryEntry[] {
...
export class EditSnapshotManager {
  private services: Map<string, EditSnapshotService> = new Map();

  /**
   * Gets or creates a snapshot service for a session
   */
  public getService(sessionId: string): EditSnapshotService {
