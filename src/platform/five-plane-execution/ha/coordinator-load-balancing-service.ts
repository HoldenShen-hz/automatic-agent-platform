...
function normalizeQueueAffinity(value: string | null | undefined): string | null {
  if (value == null) {
    return null;
  }
  const normalized = value.trim();
...
function normalizeShards(value: string[] | undefined): string[] {
  return [...new Set((value ?? []).map((item) => item.trim()).filter((item) => item.length > 0))].sort();
}

function readShards(record: CoordinatorInstanceRecord): string[] {
  try {
    const parsed = JSON.parse(record.shardJson) as unknown;
...
export class CoordinatorLoadBalancingService {
   ...
   */
  public registerHeartbeat(input: RegisterCoordinatorHeartbeatInput): CoordinatorInstanceRecord {
   ...
   */
  public listSnapshots(limit = 100): CoordinatorInstanceRecord[] {
   ...
   */
  public buildSummary(generatedAt: string = nowIso()): CoordinatorLoadBalancingSummary {
   ...
   */
  public selectCoordinator(input: CoordinatorSelectionInput = {}): CoordinatorSelectionDecision {
    const records = this.store.worker.listCoordinatorInstanceSnapshots(200);
    const evaluations: CoordinatorSelectionEvaluation[] = records.map((record) => {
    ...
    const requestBias = stableHash(
      `${input.requestKey ?? ""}|${input.queueName ?? ""}|${input.preferredRegion ?? ""}|${input.tenantId ?? ""}`,
    );
