...
export class ChannelGatewayDeliveryService {
  ...
  constructor(
    ...
    config?: Partial<DeliveryGuaranteeConfig> & { rateLimit?: Partial<RateLimitConfig> },
  ) {
    this.deliveryConfig = {
   ...
   */
  verifyNonce(nonce: string, ttlSeconds = 300): ReplayProtectionResult {
    const now = new Date();
  ...
  createDeliveryMessage(
    ...
    this.db.connection
      ...
      .run(
        ...
        "pending",
        0,
        maxRetries ?? this.deliveryConfig.maxRetries,
        now,
        now,
      );
  ...
  recordAttempt(
    ...
    this.db.connection
      ...
      .run(
        status === "success" ? "delivered" : status === "failed" ? "failed" : "retrying",
        attemptNumber,
        now,
  ...
  public recordDeliveryFailure(
    ...
    this.moveToDeadLetter(
      messageId,
      options.retryable ? "gateway.delivery_retries_exhausted" : "gateway.delivery_non_retryable_failure",
      errorMessage,
      responseStatus,
    );
    return {
  ...
  getDeliveryReceipt(messageId: string): DeliveryReceipt | null {
    ...
    const attempts = this.db.connection
      .prepare(
        `SELECT response_status, provider_message_id
  ...
  moveToDeadLetter(
    ...
    this.db.connection
      .prepare(
        `INSERT OR REPLACE INTO gateway_dead_letters
      ...
      .run(
        ...
        lastResponseStatus ?? (lastAttempt?.response_status as number | null ?? null),
        Number(row.attempts),
        ...
        (lastAttempt?.provider_message_id as string | null) ?? null,
      );
  ...
  getRetryableMessages(limit = 100): Array<{
    ...
    const rows = this.db.connection
      .prepare(
        `SELECT m.message_id, m.channel, m.target_id, m.payload_json, m.attempts, m.max_retries,
                a.next_retry_at
   ...
   */
  private readDeliveryMessageRecord(messageId: string): DeliveryMessageRecord | null {
    const row = this.db.connection
      .prepare(
      ...
      .get(messageId) as { attempts: number; max_retries: number } | undefined;
