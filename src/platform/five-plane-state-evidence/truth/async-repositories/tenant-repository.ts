...
export class AsyncTenantRepository {
  ...
  public async updateTenant(input: {
    ...
    if (input.billingPlan !== undefined) { sets.push(`billing_plan = $${idx++}`); values.push(input.billingPlan); }
    if (input.slaLevel !== undefined) { sets.push(`sla_level = $${idx++}`); values.push(input.slaLevel); }
    if (input.allowedRegionsJson !== undefined) { sets.push(`allowed_regions_json = $${idx++}`); values.push(input.allowedRegionsJson); }
    if (input.quotasJson !== undefined) { sets.push(`quotas_json = $${idx++}`); values.push(input.quotasJson); }
    if (input.metadataJson !== undefined) { sets.push(`metadata_json = $${idx++}`); values.push(input.metadataJson); }

    values.push(input.tenantId);
    return asyncExecute(
      this.conn,
      `UPDATE tenants SET ${sets.join(", ")} WHERE tenant_id = $${idx}`,
      ...values,
    );
  }
  ...
  public async upsertTenantQuota(quota: TenantQuotaRecord): Promise<void> {
    await this.conn.execute(
  ...
  public async getTenantQuota(quotaId: string): Promise<TenantQuotaRecord | null> {
    const result = await asyncQueryOne<TenantQuotaRecord>(
      this.conn,
      `SELECT
        quota_id AS "quotaId",
        tenant_id AS "tenantId",
        ...
        created_at AS "createdAt",
        updated_at AS "updatedAt"
       FROM tenant_quotas WHERE quota_id = $1`,
      quotaId,
    );
    return result ?? null;
  }

  public async listTenantQuotas(tenantId: string): Promise<TenantQuotaRecord[]> {
    return asyncQueryAll<TenantQuotaRecord>(
      this.conn,
      `SELECT
        quota_id AS "quotaId",
        tenant_id AS "tenantId",
        ...
        created_at AS "createdAt",
        updated_at AS "updatedAt"
       FROM tenant_quotas WHERE tenant_id = $1 ORDER BY resource_type`,
      tenantId,
    );
  }

  public async updateQuotaUsage(quotaId: string, currentUsage: number, updatedAt: string): Promise<number> {
    return asyncExecute(
      this.conn,
      `UPDATE tenant_quotas SET current_usage = $1, updated_at = $2 WHERE quota_id = $3`,
      currentUsage,
      updatedAt,
      quotaId,
    );
  ...
  public async insertTenantBilling(billing: TenantBillingRecord): Promise<void> {
    await this.conn.execute(
  ...
  public async updateTenantBillingStatus(input: {
    billingId: string;
    status: string;
    totalCostUsd?: number;
    invoiceUrl?: string | null;
    ...
    const values: unknown[] = [input.status, input.updatedAt];
    let idx = 3;

    if (input.totalCostUsd !== undefined) { sets.push(`total_cost_usd = $${idx++}`); values.push(input.totalCostUsd); }
    if (input.invoiceUrl !== undefined) { sets.push(`invoice_url = $${idx++}`); values.push(input.invoiceUrl); }
    if (input.paidAt !== undefined) { sets.push(`paid_at = $${idx++}`); values.push(input.paidAt); }

    values.push(input.billingId);
    return asyncExecute(
      this.conn,
      `UPDATE tenant_billing SET ${sets.join(", ")} WHERE billing_id = $${idx}`,
      ...values,
    );
  }

  public async getTenantBilling(billingId: string): Promise<TenantBillingRecord | null> {
    const result = await asyncQueryOne<TenantBillingRecord>(
      this.conn,
      `SELECT
        ...
        created_at AS "createdAt",
        updated_at AS "updatedAt"
       FROM tenant_billing WHERE billing_id = $1`,
      billingId,
    );
    return result ?? null;
  }

  public async listTenantBillingHistory(tenantId: string, limit = 10): Promise<TenantBillingRecord[]> {
    return asyncQueryAll<TenantBillingRecord>(
      this.conn,
      `SELECT
        billing_id AS "billingId",
        tenant_id AS "tenantId",
