...
export class AsyncMemoryRepository {
  ...
  public async insertMemory(memory: MemoryRecord): Promise<void> {
    await this.conn.execute(
      ...
      memory.expiresAt,
      memory.revokedAt,
      memory.revocationReason,
      memory.kind,
      memory.status,
      memory.importanceScore,
      memory.freshnessScore,
      memory.contentHash,
    );
  }
   ...
   */
  public async listMemories(query: MemoryRecallQuery = {}): Promise<MemoryRecord[]> {
    ...
    const rows = await asyncQueryAll<MemoryRecord>(
      this.conn,
      `SELECT
        id,
        task_id AS "taskId",
   ...
   * Get a memory by ID.
   */
  public async getMemory(memoryId: string): Promise<MemoryRecord | null> {
    const result = await asyncQueryOne<MemoryRecord>(
      this.conn,
      `SELECT
        id,
        task_id AS "taskId",
   ...
   */
  public async recordMemoryAccess(memoryId: string, accessedAt: string): Promise<number> {
    return asyncExecute(
      this.conn,
      `UPDATE memories
   ...
   * Revoke a memory.
   */
  public async revokeMemory(memoryId: string, revokedAt: string, reason: string): Promise<number> {
    return asyncExecute(
      this.conn,
      `UPDATE memories
       SET revoked_at = $1,
   ...
   */
  public async findMemoryByContentHash(contentHash: string, scope: string): Promise<MemoryRecord | null> {
    const result = await asyncQueryOne<MemoryRecord>(
      this.conn,
      `SELECT
        id,
        ...
        content_hash AS "contentHash"
       FROM memories
       WHERE content_hash = $1 AND scope = $2 AND status = 'active'`,
      contentHash,
      scope,
    );
    return result ?? null;
   ...
   */
  public async getMemoryQualityReport(query: Omit<MemoryRecallQuery, "includeExpired" | "includeRevoked"> = {}): Promise<MemoryQualityReport> {
    const evaluatedAt = query.evaluatedAt ?? nowIso();
    return buildMemoryQualityReport(
