...
export class MemoryRepository {
   ...
   */
  public listMemories(query: MemoryRecallQuery = {}): MemoryRecord[] {
    ...
    const rows = queryAll<MemoryRecord>(
      this.conn,
      `SELECT
        id,
        task_id AS taskId,
   ...
   */
  public recordMemoryAccess(memoryId: string, accessedAt: string): void {
    execute(
      this.conn,
      `UPDATE memories
   ...
   * Revoke a memory.
   */
  public revokeMemory(memoryId: string, revokedAt: string, reason: string): void {
    execute(
      this.conn,
      `UPDATE memories
       SET revoked_at = ?,
   ...
   */
  public findMemoryByContentHash(contentHash: string, scope: string): MemoryRecord | null {
    return queryOne<MemoryRecord>(
      this.conn,
      `SELECT
        id,
   ...
   */
  public getMemoryQualityReport(query: Omit<MemoryRecallQuery, "includeExpired" | "includeRevoked"> = {}): MemoryQualityReport {
