...
function toKind(node: ts.Node): AstIndexedSymbol["symbolKind"] | null {
...
function buildSnippet(sourceText: string, startLine: number): string {
  const lines = sourceText.split("\n");
  return lines.slice(startLine, startLine + 3).join("\n").trim();
}

export class AstStructuralIndex {
  private readonly symbolsById = new Map<string, AstIndexedSymbol>();
  private readonly symbolsByNamespace = new Map<string, Set<string>>();

  public upsertDocument(input: {
    documentId: string;
    sourceUri: string;
    namespace: string;
    content: string;
    ...
    const sourceFile = ts.createSourceFile(
      basename(input.sourceUri),
      input.content,
      ts.ScriptTarget.Latest,
      true,
      this.inferScriptKind(input.sourceUri, input.language),
    );

    const discovered: AstIndexedSymbol[] = [];
    ...
    const namespaceSymbols = this.symbolsByNamespace.get(input.namespace) ?? new Set<string>();
  ...
  public query(input: AstIndexQuery): AstIndexedSymbol[] {
    ...
    const candidateIds = input.namespace
      ? [...(this.symbolsByNamespace.get(input.namespace) ?? new Set<string>())]
      : [...this.symbolsById.keys()];
    return candidateIds
      .map((symbolId) => this.symbolsById.get(symbolId))
      .filter((symbol): symbol is AstIndexedSymbol => symbol != null)
      .map((symbol) => ({
        symbol,
  ...
  public list(namespace?: string): AstIndexedSymbol[] {
    if (!namespace) {
      return [...this.symbolsById.values()];
    }
    return [...(this.symbolsByNamespace.get(namespace) ?? new Set<string>())]
      .map((symbolId) => this.symbolsById.get(symbolId))
      .filter((symbol): symbol is AstIndexedSymbol => symbol != null);
  }
  ...
  private computeScore(symbol: AstIndexedSymbol, query: string): number {
  ...
  private shouldParse(sourceUri: string, language?: string | null): boolean {
  ...
  private inferScriptKind(sourceUri: string, language?: string | null): ts.ScriptKind {
    const normalizedLanguage = language?.toLowerCase() ?? "";
