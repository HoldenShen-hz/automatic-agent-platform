...
export class NamespacePolicyStore {
  ...
  /**
   * Creates or retrieves a namespace for a specific tenant.
   * R20-3 FIX: Per-tenant namespace creation ensures each tenant has isolated
   * knowledge boundaries. If namespace doesn't exist for tenant, it creates one
   * with default settings.
   *
   * @param tenantId - The tenant identifier
   * @param domainId - The domain that owns this namespace
   * @param pathPrefix - Optional path prefix for the namespace (default: tenant id)
   * @returns The existing or newly created namespace
   */
  public createNamespaceForTenant(tenantId: string, domainId: string, pathPrefix?: string): KnowledgeNamespace {
    const namespacePath = pathPrefix ?? tenantId;

    // Check if namespace already exists for this tenant
    const existing = this.get(namespacePath);
    if (existing != null) {
      return existing;
    }

    // Create new namespace for tenant with default settings
    const namespace: KnowledgeNamespace = {
      namespaceId: `ns_${tenantId}`,
      path: namespacePath,
      description: `Tenant namespace for ${tenantId}`,
  ...
  /**
   * Gets or creates namespace, ensuring tenant isolation.
   * R20-4 FIX: Enforces namespace isolation - rejects cross-tenant access.
   *
   * @param namespacePath - The namespace path to retrieve/create
   * @param tenantId - The tenant requesting access
   * @returns The namespace or null if access denied
   */
  public getNamespaceForTenant(namespacePath: string, tenantId: string): KnowledgeNamespace | null {
    const namespace = this.get(namespacePath);
    if (namespace == null) {
      return null;
    }

    // R20-4 FIX: Enforce namespace isolation - reject cross-tenant access
    // unless cross-namespace retrieval is allowed and principal has permission
    if (!this.strategyConfig.crossNamespaceRetrieval) {
      // Strict isolation: tenant can only access their own namespace
      if (!namespace.path.startsWith(tenantId) && namespace.ownerDomainId !== tenantId) {
        return null;
      }
    }
