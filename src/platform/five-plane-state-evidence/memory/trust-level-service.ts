...
export interface LearningObjectMetadata {
  ...
  rollbackNote?: string;
  /** R20-11 FIX: Usefulness score computed from feedback signals (0-1 scale) */
  usefulnessScore?: number | null;
  /** R20-10 FIX: Feedback signals recorded for this learning object */
  feedbackSignals?: readonly FeedbackSignal[];
  /** R20-12 FIX: Pruning threshold - objects below this score are candidates for removal */
  pruningThreshold?: number;
}

/**
 * R20-10 FIX: Feedback signal recorded for learning objects.
 * Tracks user feedback to compute usefulness over time.
 */
...
/**
 * R20-11 FIX: Usefulness computation input
 */
export interface UsefulnessScoreInput {
  quality?: number;
  relevance?: number;
  accuracy?: number;
  usageCount?: number;
...
export class TrustLevelService {
  ...
  /**
   * R20-10 FIX: Records a feedback signal for a LearningObject.
   * Feedback signals are used to compute usefulness scores over time.
   *
   * @param learningObjectId - The LearningObject to record feedback for
   * @param signal - The feedback signal to record
   * @returns The updated LearningObject or null if not found
   */
  public recordFeedbackSignal(learningObjectId: string, signal: Omit<FeedbackSignal, "signalId" | "recordedAt">): LearningObject | null {
    const obj = this.learningObjects.get(learningObjectId);
    if (!obj) {
      return null;
    }

    // Create full feedback signal with ID and timestamp
    const fullSignal: FeedbackSignal = {
      ...signal,
      signalId: newId("fbsig"),
      recordedAt: nowIso(),
    };

    // Initialize feedback signals array if not present
    if (!obj.metadata.feedbackSignals) {
      obj.metadata.feedbackSignals = [];
    }

    // Append new signal (keep last 100 signals per object)
    const signals = [...obj.metadata.feedbackSignals, fullSignal];
    if (signals.length > 100) {
      signals.splice(0, signals.length - 100);
    }
    obj.metadata.feedbackSignals = signals;

    // Update usage count and last used timestamp
    obj.metadata.usageCount = (obj.metadata.usageCount ?? 0) + 1;
    obj.metadata.lastUsedAt = nowIso();

    // Recompute usefulness score after feedback
    const usefulness = this.computeUsefulnessScore({
      ...(signal.quality != null ? { quality: signal.quality } : {}),
      ...(signal.relevance != null ? { relevance: signal.relevance } : {}),
      ...(signal.accuracy != null ? { accuracy: signal.accuracy } : {}),
      usageCount: obj.metadata.usageCount,
      validationScore: obj.validationScore,
    });
    obj.metadata.usefulnessScore = usefulness;
  ...
  /**
   * R20-11 FIX: Computes usefulness score from feedback signals and other metrics.
   * Returns a 0-1 score where higher indicates more useful.
   *
   * Uses weighted combination of:
   * - Average feedback quality/relevance/accuracy (if available)
   * - Usage count (more usage = more validated)
   * - Validation score
   * - Age penalty (older objects decay slightly)
   */
  public computeUsefulnessScore(input: UsefulnessScoreInput): number {
    let score = 0.5; // Default neutral score

    // Factor 1: Feedback signals (weighted 40%)
    const feedbackWeight = 0.4;
    if (input.quality != null || input.relevance != null || input.accuracy != null) {
      const feedbackScore = ((input.quality ?? 0.5) + (input.relevance ?? 0.5) + (input.accuracy ?? 0.5)) / 3;
      score = score * (1 - feedbackWeight) + feedbackScore * feedbackWeight;
    }

    // Factor 2: Usage count (weighted 20%)
    // Logarithmic scale: usageCount of 10 = 0.5, 100 = 0.67, 1000 = 0.75
    const usageWeight = 0.2;
    const usageScore = Math.min(1, Math.log10((input.usageCount ?? 0) + 1) / 3);
    score = score * (1 - usageWeight) + usageScore * usageWeight;

    // Factor 3: Validation score (weighted 30%)
    const validationWeight = 0.3;
    if (input.validationScore != null) {
      score = score * (1 - validationWeight) + input.validationScore * validationWeight;
    }

    // Factor 4: Age penalty (10% max deduction over 90 days)
    const ageWeight = 0.1;
    const ageDays = input.ageDays ?? 0;
    const agePenalty = Math.min(0.1, ageDays / 90 * 0.1);
    score = score * (1 - ageWeight) + (score - agePenalty) * ageWeight;
  ...
  /**
   * R20-12 FIX: Prunes low-score LearningObjects from the store.
   * Objects with usefulnessScore below the pruningThreshold are removed.
   *
   * @param threshold - Minimum usefulness score to keep (default: 0.3)
   * @returns Number of objects pruned
   */
  public pruneLowScoreObjects(threshold: number = 0.3): number {
    let prunedCount = 0;

    for (const [id, obj] of this.learningObjects) {
      // Get the pruning threshold for this object (or use default)
      const objThreshold = obj.metadata.pruningThreshold ?? threshold;

      // Skip objects that are approved or rolled_out (high value)
      if (obj.status === "approved" || obj.status === "rolled_out") {
        continue;
      }

      // Get usefulness score (compute if not set)
      const usefulnessScore = obj.metadata.usefulnessScore ?? this.computeUsefulnessScore({
        usageCount: obj.metadata.usageCount ?? 0,
        validationScore: obj.validationScore,
        ageDays: (Date.now() - new Date(obj.createdAt).getTime()) / (1000 * 60 * 60 * 24),
      });

      // Prune if below threshold
      if (usefulnessScore < objThreshold) {
        this.learningObjects.delete(id);
        prunedCount++;
      }
    }
  ...
  /**
   * Gets the usefulness score for a LearningObject.
   * Computes on demand if not stored.
   */
  public getUsefulnessScore(learningObjectId: string): number | null {
    const obj = this.learningObjects.get(learningObjectId);
    if (!obj) {
      return null;
    }

    if (obj.metadata.usefulnessScore != null) {
      return obj.metadata.usefulnessScore;
    }

    // Compute on demand
    return this.computeUsefulnessScore({
      usageCount: obj.metadata.usageCount ?? 0,
      validationScore: obj.validationScore,
      ageDays: (Date.now() - new Date(obj.createdAt).getTime()) / (1000 * 60 * 60 * 24),
    });
