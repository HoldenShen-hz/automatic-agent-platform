...
export class DriftDetectorService implements IDriftDetector {
  ...
  public constructor(changepointConfig?: DriftDetectorConfig) {
    const config = changepointConfig ?? {
      minSampleSize: 10,
      samplesPerHour: 1,
      zscoreThreshold: 2.0,
  ...
  public detect(input: DriftDetectionInput): DriftDetectorResult {
    ...
    const primarySignal = this.buildPrimarySignal(windowResults, input.currentFingerprint.fingerprintId);
    ...
    const overallSeverity = this.worseSeverity(windowSeverity, crossAgentSeverity);
  ...
  private estimateBaselineWindowDays(fingerprints: readonly import("./fingerprint-builder/index.js").BehaviorFingerprint[]): number {
    ...
    return 7; // Default baseline window
  }
}
