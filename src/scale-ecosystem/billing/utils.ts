...
export function buildBillingMarkdown(summary: BillingAccountSummary): string {
  const lines = [
    ...
    ...summary.quotas.map(
      (quota) =>
        `- ${quota.metricType}: used=${quota.usedQuantity}, limit=${quota.limitQuantity ?? "unlimited"}, remaining=${quota.remainingQuantity ?? "n/a"}, type=${quota.limitType ?? "n/a"}, window=${quota.windowStart} -> ${quota.windowEnd}`,
    ),
  ];
  return lines.join("\n");
}
