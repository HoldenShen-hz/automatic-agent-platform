...
export class LicenseEnforcementService {
  private readonly featureGates: BoundedCache<string, FeatureGate> = new BoundedCache(100);
  private readonly usageMeters: BoundedCache<string, UsageMeter> = new BoundedCache(200);
  private readonly violations: LicenseViolation[] = [];
  private readonly config: LicenseEnforcementConfig;
  // C-08: TTL-based eviction for usage meters
  private readonly MAX_USAGE_METERS = 10000;
  private readonly METER_TTL_MS = 24 * 60 * 60 * 1000; // 24 hours
  private lastMeterEvictionTime = 0;
  private readonly METER_EVICTION_INTERVAL_MS = 60 * 60 * 1000; // Once per hour

  constructor(
    private readonly store: AuthoritativeTaskStore,
    config?: Partial<LicenseEnforcementConfig>,
  ) {
    this.config = {
  ...
  recordFeatureUsage(
    ...
    if (!this.config.enableUsageMetering) return;

    const gate = this.featureGates.get(featureKey);
    if (!gate || !gate.meterUsage) return;
  ...
  ): { count: number; limit: number | null; usageRatio: number | null } | null {
    const meterKey = this.buildMeterKey(featureKey, context);
    const meter = this.usageMeters.get(meterKey);
    if (!meter) {
      const gate = this.featureGates.get(featureKey);
      if (!gate || !gate.meterUsage) return null;
  ...
  getViolations(limit = 100): LicenseViolation[] {
