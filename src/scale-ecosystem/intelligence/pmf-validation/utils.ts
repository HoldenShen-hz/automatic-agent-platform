 ...
 */
function parseIsoTimestamp(value: string, errorCode: string): Date {
...
export function safeDividePercent(numerator: number, denominator: number): number | null {
  if (denominator <= 0) {
    return null;
  }
...
export function validateDivisionId(divisionId: string | null | undefined): string | null {
  if (divisionId == null) {
    return null;
  }
...
export function mergeThresholds(
  thresholds: Partial<PmfValidationThresholds> | undefined,
...
export function buildSummary(
  ...
  checks: readonly PmfMetricCheck[],
): string {
