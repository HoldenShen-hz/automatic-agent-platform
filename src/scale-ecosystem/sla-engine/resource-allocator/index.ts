 ...
 * Validates that total reservedPercent does not exceed 100 before allocation.
...
export function allocateReservedCapacity(totalUnits: number, allocations: readonly ReservedCapacityAllocation[]): Record<string, number> {
  ...
  if (totalReservedPercent > 100) {
    throw new Error(`resource_allocator.total_reserved_exceeds_100: total reservedPercent=${totalReservedPercent} exceeds 100`);
  }
