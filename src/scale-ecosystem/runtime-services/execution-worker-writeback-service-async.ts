...
/**
 * Coalesced writeback entry
 */
interface CoalescedWriteback {
  input: WorkerWritebackInput;
  count: number;
...
export type ExecutionWorkerWritebackServiceAsyncEvent =
  | { type: "writeback_start"; writebackId: string; executionId: string }
  | { type: "writeback_complete"; writebackId: string; executionId: string; durationMs: number; accepted: boolean }
  | { type: "writeback_retry"; writebackId: string; executionId: string; attempt: number; delayMs: number }
  | { type: "writeback_timeout"; writebackId: string; executionId: string }
  | { type: "writeback_coalesced"; executionId: string; count: number }
...
export class ExecutionWorkerWritebackServiceAsync extends LocalTypedEventEmitter<Record<string, unknown>> {
  ...
  private readonly coalescedWritebacks = new Map<string, CoalescedWriteback>();
  ...
  // Metrics
  private metrics: WritebackMetrics = {
    totalWritebacks: 0,
  ...
  public constructor(
    ...
    syncOptions: ExecutionWorkerWritebackServiceOptions = {},
    asyncOptions: ExecutionWorkerWritebackServiceAsyncOptions = {},
  ) {
    super();
   ...
   */
  private tryCoalesceWriteback(input: WorkerWritebackInput): Promise<WorkerWritebackDecision> | null {
   ...
   */
  public getMetrics(): WritebackMetrics {
    return { ...this.metrics };
  }
   ...
   * @internal
   */
  public getSyncService(): ExecutionWorkerWritebackService {
    return this.sync;
  }
   ...
   */
  private async executeOperation<T>(operation: () => T, operationId: string): Promise<T> {
   ...
   */
  private flushCoalescedWritebacks(): void {
    const now = Date.now();
