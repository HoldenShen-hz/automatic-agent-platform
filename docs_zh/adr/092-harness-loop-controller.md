...
- 每轮 iteration 都必须记录 `NodeRun / NodeAttempt`、decision、context snapshot 与 timeline
...
- A-33: 本 ADR 原先用 `step / decision` 记录主时间线，根因是 loop controller ADR 继承了语义 step 叙事，没有随着 `NodeRun / NodeAttempt` 成为执行真相对象而改写。修复：正文现将时间线主语收敛到 `NodeRun / NodeAttempt`，step 只保留为语义投影。
