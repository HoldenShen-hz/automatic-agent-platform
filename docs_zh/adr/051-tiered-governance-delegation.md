...
| 层级 | 可委托权限 | 约束 |
|------|-----------|------|
| 平台级 | 仅限 NonOverridableInvariant 之外的权限 | 必须遵守安全边界、审计要求等不可覆盖约束 |
| 事业群级 | 事业群内权限 | 受上级委托约束 |
| 部门级 | 部门内权限 | 受上级委托约束 |
| 团队级 | 有限权限 | 受上级委托约束 |

注：NonOverridableInvariant 为平台级安全不变量，任何委托不得覆盖此类约束（如安全边界、审计要求等）。平台级委托并非"全部权限"，而是排除 NonOverridableInvariant 之后的受限全部权限。
