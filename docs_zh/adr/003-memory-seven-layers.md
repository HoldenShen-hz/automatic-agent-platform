...
- Phase 1：优先落 L1/L2，配合 handoff、context compaction、plan 进度管理。
- Phase 2：Observe / Assess / Plan 走显式 DTO，session/project 级引用开始稳定。
- Phase 3：Feedback / Learn 把证据链转为 `LearningObject`，进入 L6。
- Phase 4：Improve / Release 消费 L6 的 validated/promoted 学习对象，且仅允许 `off/suggest/shadow` 三档 release。
