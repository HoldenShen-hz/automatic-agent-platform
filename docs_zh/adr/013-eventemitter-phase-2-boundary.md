# ADR-013 EventEmitter 是否继续使用到 Ring 2 Readiness
...
当前平台需要事件驱动的状态投影、gateway 流式反馈、恢复扫描和运维观测。但 Ring 1 仍以单机、单进程和最小多 Agent 编排为主。
...
Ring 1 允许继续使用内存事件分发机制作为本进程内分发工具。

但同时冻结以下边界：

- Tier 1 事件的事实源必须是持久化事件表与 per-consumer ack。
- EventEmitter 只负责进程内 fan-out，不承担可靠性交付语义。
- 是否在 Ring 2 readiness 之后替换为更正式的 queue / bus，届时再单独决策。
...
- Ring 2 readiness 后再评估是否升级为更重的队列系统
...
- `platform.feedback.signal_received`、`platform.learn.object_promoted`、`platform.release.rollout_*` 这类事件若被定义为高价值事实事件，必须优先满足持久化与 ack 约束，而不是依赖纯内存订阅成功。
...
- 单机为主
- 进程内分发为主
- Ring 1 MVP / Ring 2 readiness orchestration 为主
...
- [ADR-012 SQLite 是否作为 Ring 1-2 默认主存储](./012-sqlite-phase-1-2-primary-store.md)
...
- R8-72: 本 ADR 原先仍以 `Phase 1a / 1b / Phase 2` 描述适用边界和退出触发器，根因是事件 transport ADR 的历史命名没有随 ring 口径回写。修复：正文现统一收敛到 `Ring 1 MVP / Ring 2 readiness`，并把“是否替换”表述改为 ring readiness 之后的单独决策。
