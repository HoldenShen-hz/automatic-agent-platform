...
实现层必须显式区分 `InteractionAutonomyLevel` 与 `UnifiedRuntimeMode`：前者决定建议/人审/自动交互边界，后者决定运行时降级、暂停和 `incident_mode`。

自主权采用 ADR-042 定义的 4 级交互自治命名体系（与 ADR-042 保持一致）：

| 等级 | 名称 | 说明 |
|------|------|------|
| **L1** | `suggestion` | 只生成建议 |
| **L2** | `supervised` | 人工确认后执行 |
| **L3** | `semi_auto` | 低风险自动执行，高风险升级 |
| **L4** | `full_auto` | 在显式治理边界内自动执行 |
...
- R3-55: 本 ADR 原先定义第三套自治命名体系，与 ADR-042 互不兼容。修复：正文现明确声明采用 ADR-042 的 `suggestion / supervised / semi_auto / full_auto` 四级体系，两者保持一致。
