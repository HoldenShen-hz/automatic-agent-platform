...
自主权等级必须与 §42 规范模式集（5 级 canonical autonomy mode）保持一致：

注：§42 定义了 5 级自主权模型（manual_only/suggestion/supervised/semi_auto/full_auto），
ADR-083 与 ADR-042 均须与 §42 的 5 级模型对齐。
...
注：`full_auto` 等级不代表无限制自动化，高危域默认不得进入 `full_auto`，除非存在显式 `DomainRiskSpec` / `DomainRiskProfile` 允许并附带人工责任边界。
