...
1. **不可追踪**：无显式 Plan DTO，无法对计划内容进行独立验证。
...
interface Plan {
  planId: string;
  taskId: string;
  version: number;           // 每次 replan +1
  strategy: PlanStrategy;    // 8 种策略枚举
  steps: PlanStep[];         // DAG 节点列表
  dag: DAGStructure;         // 步骤间依赖关系
...
| 约束 | 说明 |
|------|------|
| **R3-SINGLE** | Execute 层只能接收 Plan DTO，不允许旁路 raw task 直接执行 |
...
通过 `RuntimeExecuteBridge` 接口实现 Plan 到执行引擎的解耦：
...
- `RuntimeExecuteBridge` 作为 Plan→Execute 解耦层。
