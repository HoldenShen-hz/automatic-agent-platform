...
> **文档版本**: v1.2
> **适用项目**: automatic-agent-system
> **测试框架**: Node.js built-in test runner (`node:test`) + `node:assert/strict`
> **覆盖率工具**: c8 (V8 native coverage) + Istanbul reporter
> **变异测试**: Stryker Mutator v9.6
...
│   ├── plugins/           # 插件测试
│   └── ...
├── integration/           # 跨服务/运行时测试（245 文件）
│   ├── security/          # 安全边界测试（50+ 文件）
│   ├── runtime/           # 运行时集成
│   ├── storage/           # DB 集成
│   └── ...
...
### 6.4 目录级基线（OAPEFLIR 相关）

| 目录 | Lines | Functions | Branches |
|------|-------|-----------|----------|
| `src/platform/orchestration` | 87.3% | 71.4% | 73.6% |
| `src/platform/state-evidence` | 90.8% | 98.2% | 76.5% |
| `src/platform/execution` | 87.9% | 82.6% | 69.6% |
| `src/platform/control-plane` | 90.4% | 88.9% | 85.7% |
| `src/platform/interface` | 76.5% | 69.2% | 72.3% |
| `src/domains` | 88.5% | 84.6% | 52.4% |
| `src/interaction` | 86.9% | 83.7% | 84.0% |
| `src/ops-maturity` | 93.4% | 93.8% | 78.4% |
| `src/plugins` | 57.3% | 60.0% | 86.7% |

**薄弱环节**：`improvement` branches 仅 52.4%，`plugins` lines 仅 57.3%，`planning` lines 仅 76.5%。
...
- [ ] 创建 `tests/unit/core/<module>/` 目录
- [ ] 每个 service class 创建对应 `<service-name>.test.ts`
- [ ] 如需 DB → 创建 `tests/integration/core/<module>/` 目录
...
import assert from "node:assert/strict";
import { TRANSITION_MAP, ALL_STATES } from "../../src/platform/execution/state-transition/types.js";
...
全局 82% 行覆盖率可能掩盖关键模块的低覆盖。需要对不同模块定义 **硬性最低门槛**。
...
| `locking` | Critical | 70.2% | 90% | 81.3% | 80% | ❌ Lines 差 19.8% |
| `agent-loop` | High | 87.3% | 85% | 73.6% | 75% | ❌ Branches 差 1.4% |
| `planning` | Standard | 76.5% | 80% | 72.3% | 70% | ❌ Lines 差 3.5% |
| `improvement` | Standard | 88.5% | 80% | 52.4% | 70% | ❌ Branches 差 17.6% |
| `plugins` | Baseline | 57.3% | 75% | 86.7% | 60% | ❌ Lines 差 17.7% |
...
> **文档结束 (v1.1)** — 本手册从 v1.0 的"代码覆盖治理"升级到 v1.1 的"架构语义覆盖治理"。
>
> **Part I** 保证：测试不少、质量不差、不会明显遗漏。
> **Part II** 保证：系统关键设计语义（状态机、事件、并发、阶段契约）都被覆盖到。
>
> 核心理念升级为：**覆盖率棘轮保证数量，变异测试保证质量，Traceability Matrix 保证完整性，PR Review 保证上下文，架构语义矩阵保证设计契约。五者缺一不可。**
