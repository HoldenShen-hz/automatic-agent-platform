...
8. **Receipt / Outbox 原子性补强目标已明确**：高风险副作用 commit 前必须写入 `PrepareReceipt` 或 durable outbox record；当前文档把它收口为工程目标，不再表述成仓内已经完成。
9. **Tool 可逆性进入风险决策**：`not_reversible` / `forward_fix_only` 等可逆性等级参与 risk resolution，不再只是 metadata。
10. **Memory Gateway 目标接口补齐**：补充 `ManagedMemoryMinimal`、`MemoryProposal`、`MemoryProjection`、`MemoryCommitDecision`、`MemoryRevokeDecision`，并明确哪些仍待工程实现。
11. **Security & Privacy Baseline 补齐**：覆盖 secret、PII、cross-tenant isolation、retention、export/delete、restricted evidence access；当前仅发布架构与 contract 目标，不宣称统一组件已全部存在。
...
| 状态 | 含义 | 处理方式 |
|---|---|---|
| existing | 当前仓库已存在（允许是 aggregate 命令） | 可以直接作为评审版验收命令；工程冻结前应尽量收敛为 dedicated script |
| to_add | 当前未确认存在，v1.9 要求新增 | 对应 TODO 必须创建脚本和测试集合 |
| TBD | 需要扫描仓库后确认 | 阻断工程基线冻结 |

### 7.2 P0 必需命令

当前仓库已经存在一批可直接复用的 aggregate 命令；v1.9 评审版先显式绑定这些现有命令，工程冻结前再按能力面补齐 dedicated script。

重要边界：

- `existing` 只表示脚本入口存在，不表示对应能力已经按本文 contract 全量实现。
- dedicated script 当前多为最小真实测试集合；能证明“入口不为空”，但不能被表述为“prepare/commit/verify/compensate / proposal-only / stable gate 全合同覆盖已经完成”。
...
| `AA_VALIDATION_ITERATIONS=2 npm run validate:stable:compiled` | 覆盖稳定性验证与 eval-style compiled validation | existing | EVAL-001 |
...
| `npm run test:tool-gateway` | 覆盖 ToolGateway 当前最小 facade / no-go / receipt 行为，不代表 transactional outbox 或完整 prepare-commit 契约 | existing | TOOL-001,TOOL-002 |
| `npm run test:memory-gateway` | 覆盖 MemoryGateway 当前最小 facade / proposal-only / projection，不代表冲突裁决与隐私工作流已全部到位 | existing | MEM-001 |
| `npm run test:release-gate` | 覆盖 ReleaseManifestDraft facade 与 stable gate 当前最小契约，不代表 lockfile/hash/canary/eval-threshold contract 已全部落地 | existing | REL-001 |
...
当前仓库已具备 `test:integration`、`test:e2e`、`gate:stable`、`validate:stable:compiled`、`prompt-injection:stable`、`security:tenant`、`observability:smoke` 等 aggregate 命令，并已补齐 `test:receipt-store`、`test:tool-gateway`、`test:memory-gateway`、`test:release-gate` 四个 dedicated script。注意：这些脚本名只是“入口存在”的验收契约，允许先包装最小真实测试集合，但不允许空壳脚本，也不得被当作“相关能力已经完整实现”的证据。
...
> 本节到 §18 的 TypeScript 片段默认都是 **target contract / gap-closing spec**，用于约束后续工程实现边界；除非在 TODO 表或代码映射中明确标成 `implemented`，否则不得把这些接口视为“当前仓库已经存在的源码事实”。
...
| REC-001 | BaseReceiptMinimal schema + receipt shadow write | P0a | State | TBD | TBD | `src/platform/five-plane-state-evidence/receipts/`、`src/platform/five-plane-state-evidence/side-effect-ledger/`、`src/platform/shared/outbox/` | `tests/unit/platform/state-evidence/receipts/` | `npm run test:receipt-store` | existing | shadow | partial | 当前仅有 `BaseReceiptMinimal` 与最小 receipt store 测试；`BaseReceiptFull` factory/persistence、RBAC receipt 子类与 repair worker 仍未落地 | yes | DOC-014 | TBD |
| TOOL-001 | ToolGateway shadow adapter | P0a | Runtime | TBD | TBD | `src/platform/five-plane-execution/tool-executor/`、`src/platform/five-plane-orchestration/harness/toolbelt/` | `tests/unit/platform/execution/tool-executor/`、`tests/integration/platform/execution/tool-executor/` | `npm run test:integration` | existing | shadow | partial | 现有 tool-executor/toolbelt 可复用，但统一 `ToolGatewayShadow` contract、ActionRiskInput 与 dry-run path 仍是目标接口 | yes | REC-001 | TBD |
| GOV-001 | H2 Action Gate dry-run | P0a | Control | TBD | TBD | `src/platform/five-plane-control-plane/risk-control/`、`src/platform/five-plane-control-plane/approval-center/` | `tests/integration/platform/control-plane/` | `npm run test:integration` | existing | dry-run | partial | 风控/审批主干已存在，但 `PolicyDryRunDecision`、统一 action risk matrix 与 reversibility rule 尚未工程化 | yes | REC-001 | TBD |
| GOV-002 | H4 Approval enforce for high-risk tool | P0b-1 | Control | TBD | TBD | `src/platform/five-plane-control-plane/approval-center/`、`src/org-governance/approval-routing/` | `tests/e2e/approval-*.test.ts`、`tests/integration/platform/control-plane/approval-center/` | `npm run test:e2e` | existing | enforce | partial | approval-center + routing 存在，但尚未以 `H4` 语义强制绑定高风险 tool prepare/commit contract 或 break-glass receipt 闭环 | yes | TOOL-001,GOV-001 | TBD |
| TOOL-002 | prepare/commit/verify/compensate with durable outbox | P0b-1 | Runtime | TBD | TBD | `src/platform/five-plane-execution/tool-gateway/`、`src/platform/shared/outbox/`、`src/platform/five-plane-state-evidence/receipts/` | `tests/unit/platform/execution/tool-gateway/` | `npm run test:tool-gateway` | existing | enforce | partial | 当前 `test:tool-gateway` 只覆盖最小 facade / no-go / receipt 行为；`PrepareReceipt`、`DurableOutboxRecord`、transactional outbox、commit journal、idempotent repair 仍未落地 | yes | REC-001,TOOL-001 | TBD |
| MEM-001 | MemoryGateway facade + L4-L7 proposal-only | P0b-2 | State | TBD | TBD | `src/platform/five-plane-state-evidence/memory-gateway/`、`src/platform/five-plane-orchestration/harness/memory-manager.ts` | `tests/unit/platform/state-evidence/memory-gateway/` | `npm run test:memory-gateway` | existing | enforce | partial | `ManagedMemoryMinimal` / `MemoryProposal` / projection 接口已存在，但 proposal reject/quarantine、冲突优先级、projection invalidation 与 token budget enforce 仍未完成 | yes | REC-001 | TBD |
| REL-001 | ReleaseManifestDraft + ReleaseGate enforce for prod | P0b-3 | Release | TBD | TBD | `src/platform/shared/stability/release-gate.ts`、`src/sdk/cli/release-pipeline.ts`、`src/platform/five-plane-control-plane/config-center/` | `tests/unit/platform/shared/stability/`、`tests/integration/platform/execution/stable-release-gate.test.ts` | `npm run gate:stable` | existing | enforce | partial | 当前 stable gate 已存在，但 `Harness Lockfile`、canonical hash、`EvalThresholdVersion`、canary plan 与 compatibility matrix 仍未落地 | yes | REC-001 | TBD |
| GOV-003 | H3 Observation Gate for untrusted tool output | P0b-4 | Control | TBD | TBD | `src/platform/five-plane-execution/tool-executor/tool-output-sanitizer.ts`、`src/platform/five-plane-execution/tool-executor/mcp-tool-guard.ts` | `tests/unit/platform/execution/tool-executor/`、`tests/e2e/prompt-injection-guard*.test.ts` | `npm run prompt-injection:stable` | existing | enforce | partial | 现有 sanitizer / guard 可提供基础防护，但 `ObservationGateDecision` 类型、trust label 和统一 H3 注册仍未存在 | yes | TOOL-002 | TBD |
| UI-001 | Minimal Approval UI/API | P0b-5 | UI | TBD | TBD | `ui/packages/features/approval/`、`src/platform/five-plane-interface/api/http-server/approval-routes.ts` | `ui/tests/unit/ui/packages/features/approval/`、`tests/e2e/approval-*.test.ts` | `npm run test:e2e` | existing | enforce | partial | 现有 approval UI/API 可复用，但不应表述成已完成 H4 + break-glass + policy dry-run + receipt 的最小闭环 | conditional | GOV-002 | TBD |
| GOV-004 | H1-lite untrusted input marking | P0c | Control | TBD | TBD | `src/platform/prompt-engine/prompt-injection-guard.ts`、`src/platform/shared/stability/stable-prompt-injection-red-team.ts` | `tests/e2e/prompt-injection-guard*.test.ts`、`tests/integration/platform/prompt-engine/prompt-injection-guard.integration.test.ts` | `npm run prompt-injection:stable` | existing | dry-run/enforce for external | partial | 当前 prompt injection guard 可复用，但 `InputTrustLabel`、H1 sample set、context-assembler / projection 强制 redaction 尚未全部到位 | yes | DOC-014 | TBD |
| SEC-001 | P0 security suite：prompt/tool/memory injection | P0c | Security | TBD | TBD | `src/platform/prompt-engine/prompt-injection-guard.ts`、`src/platform/five-plane-control-plane/incident-control/tenant-execution-isolation-service.ts` | `tests/e2e/prompt-injection-guard*.test.ts`、`tests/integration/platform/security/` | `npm run test:e2e` | existing | enforce for high-risk | partial | 当前安全测试可覆盖部分 prompt / tenant 边界，但 SecretScanner、TenantIsolationGuard、EvidenceAccessPolicy、RetentionPolicyEnforcer 仍未组件化 | yes | GOV-003,GOV-004,MEM-001 | TBD |
| EVAL-001 | Minimal eval report and release blocking threshold | P0c | Eval | TBD | TBD | `src/platform/five-plane-orchestration/harness/evaluation/`、`src/platform/five-plane-orchestration/harness/eval-harness/` | `tests/unit/platform/orchestration/harness/evaluation/`、`tests/integration/platform/orchestration/harness/` | `AA_VALIDATION_ITERATIONS=2 npm run validate:stable:compiled` | existing | enforce for release | partial | 当前 compiled validation 可复用，但 trajectory rubric、sample audit record、`EvalThresholdVersion` 和场景校准协议仍未落地 | yes | REL-001 | TBD |
| OBS-001 | Basic cost/latency tracing advisory | P0c | Observability | TBD | TBD | `src/platform/shared/observability/` | `tests/integration/platform/shared/observability/` | `npm run observability:smoke` | existing | advisory | partial | observability smoke 存在，但 scenario threshold matrix、advisory-to-blocking wiring 与 release consumption path 仍未完成 | conditional | REC-001 | TBD |

### 20.3 P1/P2 TODO 摘要

| ID | Task | Priority | TeamOwner | Status | Notes |
|---|---|---|---|---|---|
| SAN-001 | SandboxProvider abstraction | P1 | Runtime | partial | `sandbox-provider/` 已存在 provider 抽象，但本文要求的高风险 DB operations end-to-end sandbox policy / audit 样例仍未建立 |
| EVAL-002 | Full trajectory evaluator | P1 | Eval | partial | `full-trajectory-evaluator.ts` 已存在，但 trajectory rubric 8 维评分、LLM judge 校准凭证和 reviewer drift 报告仍未落地 |
| OBS-002 | Cost/latency release blocking gate | P1 | Observability | partial | `cost-latency-release-gate.ts` 已存在，但尚未与 ReleaseManifestDraft / stable gate / scenario threshold table 全量串联 |
| MEM-002 | Memory export/delete/revoke privacy workflow | P1 | State/Security | partial | privacy workflow 存在，但 projection rebuild、index invalidation 与 user/team scope tests 仍未收口 |
| UI-002 | Memory Review Console frontend | P1 | UI | partial | feature 目录存在，但 reviewer / approve / revoke / risk highlight 的完整治理台尚未闭环 |
| UI-003 | Release Console frontend | P1 | UI | partial | leadership claims 子页已存在，但本文描述的 release / rollback / publish / secondary approval 并未全部实现 |
| UI-004 | Trace Explorer frontend | P1 | UI | partial | feature 目录存在，但 RBAC、receipt 写入路径和 dry-run / policy / incident consoles 仍以占位为主 |
| OPS-001 | Operational runbook automation | P1 | Ops | partial | runbook automation service 存在，但 5 个指定 runbook 尚未一一工程化为可执行流程 |
...
| Security & Privacy Baseline 已定义 | 已完成（定义级）；组件化与闭环实现未完成 |
...
5. H1-lite 样例集、ToolRiskMetadata、Transactional Outbox、ReleaseManifest hash、ManagedMemoryMinimal 都已从“文档接口”收敛为可执行代码或最小测试。
...
| 事项 | 为什么不能在文档内直接完成 |
|---|---|
| 真实代码路径基线 | 需要生成可复现扫描产物并固化到 review artifact |
...
| Receipt / durable outbox 原子性 | 已定义为目标 contract，工程实现未完成 |
| Tool reversibility risk rule | 已定义为目标 contract，工程实现未完成 |
| ManagedMemoryMinimal schema | 已有最小接口，但高级治理逻辑未完成 |
| Security & Privacy Baseline | 已定义，组件化与闭环实现未完成 |
| 首批业务场景展开 | 已完成 |
| TODO 去重 | 已完成 |
| 真实代码扫描 | 未完成，需工程执行 |
| 真实 owner 填写 | 未完成，需项目排期 |
| CI 命令落地 | 未完成，需工程执行 |
| H1-lite 最小测试集定义 | 已定义，仓内样例与 trust label contract 未完成 |
| ToolRiskMetadata 必填契约 | 已定义，仓内强制元数据未完成 |
| Transactional Outbox 事务边界 | 已定义，仓内事务实现未完成 |
| Eval baseline 校准协议 | 已定义，`EvalThresholdVersion` 与场景校准未完成 |
| Security Gate 组件化 | 已定义，统一组件未完成 |
