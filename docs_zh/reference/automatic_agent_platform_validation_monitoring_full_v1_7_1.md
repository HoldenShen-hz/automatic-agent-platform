...
| 单次可完成工作                 | Task           | dispatch 前必须经 MissionResolver 解析 Mission；低/中风险可绑定已有 Mission 或创建 ad hoc Mission |
...
| P0 incident 修复               | Task           | 归属显式 Incident Mission；无授权 Mission 时拒绝 dispatch                                         |
| 高风险或带副作用的一次性请求   | Task           | 必须显式 Mission 绑定，禁止 missionless dispatch                                                  |
| 低/中风险无归属一次性请求      | Task           | 允许 MissionResolver 自动创建 ad hoc Mission，禁止 orphan task                                    |

## 4.2 禁止对象

| 禁止项                     | 原因                                                                 |
| -------------------------- | -------------------------------------------------------------------- |
| Orphan Task                | 无法做预算、证据、归档、追责；当前 intake 应在解析失败时 fail-closed |
...
本文的“全覆盖”指 **风险、契约、生命周期、观测和验收证据覆盖**，不是声称当前仓库已经达到 100% 代码覆盖率。代码覆盖率事实、测试排除审计和补测路线以 `docs_zh/quality/00-full-coverage-test-manual.md`、覆盖率报告和 CI gate 为准。
...
## 33.2 当前仓内可执行 CI / 验证基线

当前仓库已经提供的主验证入口如下。它们是 review 时可直接执行或接入 CI 的 **当前基线**；更细的 validation job 可以在此基础上拆分，但不得把尚未落地的命令写成已存在脚本。

| 基线入口                        | 当前命令                                                                                       | 当前用途                                        |
| ------------------------------- | ---------------------------------------------------------------------------------------------- | ----------------------------------------------- |
| 类型与静态检查                  | `npm run typecheck`、`npm run lint`                                                            | 类型、静态规则、基础漂移发现                    |
| 分层测试                        | `npm run test:unit`、`npm run test:integration`、`npm run test:e2e`、`npm run test:invariants` | 当前单元、集成、E2E、不变量验证                 |
| 输出与性能                      | `npm run test:golden`、`npm run test:performance`                                              | Golden 输出稳定性和性能回归                     |
| 全层验证                        | `npm run test:layers:full`                                                                     | 当前仓内全层测试聚合入口                        |
| 覆盖与变异                      | `npm run coverage:gate`、`npm run test:mutation`                                               | 覆盖率棘轮与变异测试                            |
| 测试排除与仓库卫生              | `npm run audit:test-exclusions`、`npm run audit:repo-hygiene`                                  | skip/exclusion、审查样例、供应链检查            |
...
| Registry         | 必须满足                                                                                        |
| ---------------- | ----------------------------------------------------------------------------------------------- |
| Gate Registry    | 正文引用的所有 Gate 均登记，且能导出机器快照                                                    |
| Metric Registry  | 正文/Runbook/Dashboard/RTM 所有 aa.\* 目标指标均登记，并有当前 exporter 或采集映射              |
| Event Registry   | 当前源码 registry 与导出快照一致；canonical 事件有 payload schema，legacy 事件有兼容策略        |
| CI Job Registry  | Gate 中引用的所有 CI job 均登记，并能映射到真实脚本或 CI workflow                               |
| Runbook Registry | 每个 P0 Gate 绑定 runbook，机器 registry 与 `deploy/runbooks/` / 附录 D 无漂移                  |
...
CI Job Registry 中每个 required job 均有真实执行映射
Metric Registry 与 Prometheus exporter/alert mapping 已复核
```
...
# 57. 附录 E：机器可执行工件清单 done

## E.1 当前仓内已有基线

review 时可直接定位的实现和配置基线包括：
