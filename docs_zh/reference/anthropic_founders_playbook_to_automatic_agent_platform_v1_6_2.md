# Anthropic《The Founder’s Playbook》对 Automatic Agent Platform 的架构借鉴与落地方案

> 文档状态：reviewed-reference / architecture-patch-candidate  
> Registry 状态：repo_validation_patch_present / main_baseline_acceptance_pending  
...
> 目标：把 Playbook 中的 AI-native startup 方法论抽象为可落地、可验证、可审计、可回滚的 Agent 平台架构能力，并与现有 Mission、Task、Session、Skill、Workflow、Evidence、Validation、Runtime Governance 体系闭环。

---

## 审阅结论与本次修订

本文适合作为“外部方法论到平台能力”的参考设计稿，但不应直接被读成当前仓库已经接受的契约或已经存在的实现基线。本次审阅按当前仓库文档与代码状态做了以下收紧：

| 发现 | 风险 | 本次修订 |
|---|---|---|
| 文档头把自身写成 `release-ready`，同时又声明 Registry、Evidence、Runtime 仍待 closure | 容易把参考稿误读为已通过主基线验收 | 状态改为 `reviewed-reference / architecture-patch-candidate`，明确当前仓库尚未注册这些条目 |
| `npm run playbook:*`、`stage-exit`、`mission-outcome` 等命令被写成“必须执行” | 审阅时 `package.json` 尚无这些命令，验收路径会变成纸面命令 | 先改成 proposed command inventory；2026-05-21 已补仓内 closure 命令、registry patch 与报告产物，主基线接受仍需单独签收 |
| 文档新增 MissionPlaybook / StageExit 模型，但权威关系没有放在最前面 | 可能和现有 Mission、PlanGraph、HarnessRun、NodeRun 主链形成第二执行运行时 | 新增“权威边界与落地差距”，固定 Mission Playbook 只能做业务阶段治理，不得替代 canonical runtime |
| Source Traceability 只有 PDF 页码，缺官方源固定规则 | PDF 换版后来源映射漂移 | 头部补官方源，保留页码置信度并要求正式合并时复核 sourceRef |
| Metric 表允许 `missionId` 进入标签 | 与 Mission 高基数治理不一致 | 删除高基数 `missionId` label，改为低基数标签和 trace/evidence 回链 |
...
| 版本 | 状态 | 主要变更 |
|---|---|---|
| v1.0 | proposal / ready-for-review | 基于 Anthropic Founder’s Playbook 提出 MissionPlaybookRegistry、StageExitGateService、FailureModeRegistry、MissionOutcomeMeasurementService、Workflow-to-Skill Candidate Pipeline。 |
| v1.1 | proposal / integration-ready after review | 修复 v1.0 review 中发现的问题：补齐 Gate / Metric / Event / Runbook Registry 接入；新增 Mission / Task / Session 生命周期对齐；补 StageExit 的 CAS/RSM/transaction 语义；将 ExitCriterion.expression 改为安全 DSL；明确 MissionStageGraph 可受控循环、PlanGraphBundle 必须 DAG；补 FailureMode 检测治理、Outcome 延迟度量、WorkflowRecordingPolicy、SkillCandidate 生命周期对齐、Source Traceability Matrix、Native Surface Matrix，并将 Phase A-D 改名为 Implementation Track A-D。 |
| v1.2 | proposal / baseline-integration-candidate | 修复 v1.1 review 中的定板阻断项：明确 proposed registry 状态与 runtime enforcement 边界；新增 v2.0 Baseline Integration Patch Plan；补 StageExitGateService 事务伪代码与禁止分离事务规则；补 ExitCriterion DSL Validator；修复 MissionStageStatus 与状态机中的 HITL 不一致；拆分 SkillCandidate 与 SkillPack 生命周期；补 WorkflowRecordingPolicy 默认最小权限配置；补 MissionPlaybook 版本治理、生命周期、rollout/rollback/signature；补 Outcome score 分层、FailureMode vs Gate severity 关系、Native Surface 动作权限矩阵、Implementation Track owner/dependency/exit/rollback；明确并入 v2.0 主文档方式。 |
| v1.3 | proposal / registry-closure-candidate | 修复 v1.2 review 中的闭环问题：真正拆分 SkillCandidate 与 SkillPack 生命周期；补 StageExit 的 nextStageInstance 原子创建、Mission currentStage 指针更新、idempotencyKey 与重复调用语义；新增 MissionPlaybookResolutionPolicy 与 MissionPlaybookMigrationPlan；WorkflowRecordingPolicy 改为安全枚举并默认 redacted_summary；统一 SkillCandidate 事件聚合边界；Runbook severity 改为 defaultSeverity + escalationRules；补 ExitCriterion snapshot binding、actual/expected value 记录；补 OutcomeObservation/source/confidence；补 MissionStageGraph edge-level guard；Implementation Tracks 增加 observe_only→enforcing 策略；Native Surface Matrix 绑定 Policy Action / Required Capability；Source Traceability Matrix 补章节/页码范围；补 Benchmark Monitoring Mission playbook 示例、Outcome→Knowledge/Artifact promotion、SkillPack policy/eval/rollback 示例。 |
| v1.4 | proposal / baseline-acceptance-candidate | 修复 v1.3 review 中的 acceptance blocker：拆分 SkillCandidate conversion gate 与 SkillPack activation gate；补 SkillPack 异常/终态事件和违规指标；修正 StageExit 事件因果顺序，补 held/rollback/terminate 分支事件；StageExitGateInput 增加 playbookVersion、remediationRef、HITL 重新评估强制校验；FailureMode 类型补 triggerCondition；补 MissionPlaybook governance gates/metrics；WorkflowRecordingPolicy 增加 retention deletion proof、sweep、deletion events/metrics；ExitCriterion window 改为 BoundedTimeWindow；StageEdgeGuard 增加 risk/data/evidence/outcome 约束；MissionOutcome 第一阶段用 BusinessImpactObservation；Runbook/CI/Track/Traceability 进一步机器闭环。 |
| v1.5 | proposal / final-acceptance-candidate | 修复 v1.4 review 中的最终接受问题：修复 Markdown code fence；MissionStageInstance 使用 version 字段，expectedVersion 只保留在 input/command；StageEdgeGuard 补齐 risk/data/evidence/outcome；StageExit event payload 补 playbookVersion/idempotencyKey/criterionResults/sequence/correlationId；Runbook Registry 补 ack/resolve SLA 和人工审批字段；CI Job Registry 增加 Track A-D observe_only/enforcing 模式；收紧 BoundedTimeWindow、use_last_active、fencingToken、MissionOutcome score 来源、SkillPack Event 顺序和 SkillCandidate/SkillPack gate 边界。 |
| v1.6 | proposal / ready-to-merge-candidate | 修复 v1.5 review 中的最后 integration closure 问题：补 v1.5 版本记录并清理残留 v1.4 表述；Patch Plan 补 GATE-SKILLPACK-001、skillpack-validate、SkillPack evidence refs；统一 StageTransitionEventBase 与 EventEnvelope/Payload 边界；补 MissionPlaybook validation_failed/rejected/suspended 事件；收紧 use_last_active fallback；新增 GATE-WORKFLOW-RECORDING-003；Runbook D.35/D.40 补迁移兼容与 retention/deletion metrics；MissionOutcome 类型去重；明确 mapDecisionToStageStatus、canonical enum 引用、SkillPack rollback 语义、结构化 metric alert 与 gate-level CI mode。 |
| v1.6.1 | proposal / final-release-closure-candidate | 修复 v1.6 review 中的 release closure 问题：删除未注册的 mission.stage.rollback_completed 引用；补齐 merge checklist 中 mission-outcome / skill-candidate / skillpack 验证命令；mission.stage.completed 补 decisionId 并继承 StageTransitionEventBase；§14 改造点同步 SkillPack、WorkflowRecording retention、SkillPack metrics 与 skillpack-validate；Evidence Bundle Patch Plan 补 playbookId / playbookResolutionRef / playbookMigrationPlanRefs；明确 Metric Alert 表格为 human-readable summary，主 registry 必须生成 metric-alert-policy.yaml；Source Traceability Matrix 增加逐行 sourceRefConfidence。 |
| v1.6.2 | reviewed-reference / architecture-patch-candidate | 补充 Release Evidence、Release Scope、Release Decision 与 Merge Closure 候选命令；2026-05-21 审阅修订后，本文定位收紧为参考设计与主基线 patch 候选，runtime enforcement 与 release blocking 只在主仓 Gate / Metric / Event / Runbook / CI / Evidence Bundle closure 通过并写入证据包后生效。 |

---

## 0. 一页结论

Anthropic 这份 Playbook 的核心不是“创业建议”，而是一种清晰的 **AI-native operating model**：
...
```text
目标 → 退出标准 → 常见失败模式 → 合适的 AI surface → 可执行 exercise → 进入下一阶段的判断
```

这对 Automatic Agent Platform 的直接启发是：

> 我们不应该只把 Agent 当成一个“任务执行器”，而应该把系统升级为 **AI-native Mission Operating System**：每个 Mission 都有阶段、退出标准、失败模式、证据要求、指标度量、Skill 编排、HITL 决策、学习与改进闭环。
...
### 0.1 Registry 状态与执行边界

本文提出的 Gate / Metric / Event / Runbook / CI Job 在并入主 `v2.0 Validation Baseline` 前，属于 **proposed registry entries**；主 Registry Closure 通过后才能转为 accepted entries。当前仓库的主运行时、主契约和主测试不得把本参考文档中的 proposed entry 当作已接受事实。
...
```text
1. 本文中的 proposed Gate 不得在主基线接受前作为正式 release blocker。
2. 本文中的 proposed Metric 不得在主 Metric Registry 接受前作为唯一告警依据。
3. 本文中的 proposed Event 必须先生成机器可执行 payload schema，才能进入 Event Registry。
4. 本文中的 proposed Runbook 必须先绑定 accepted Gate 与 accepted Metric，才能进入 Runbook Registry。
5. 一旦合并进主文档，所有 proposed 条目必须通过 Registry Closure 校验。
```
...
```text
从“Agent 会执行”升级为“Mission 会经营、会度量、会学习、会沉淀”。
```

### 0.2 Release Scope / Evidence / Decision

本版本的作用范围是 **参考文档层与架构基线补丁候选层**，用于评估是否并入 `v2.0 Validation Baseline`。在主基线 closure 前，不称为代码 release，也不称为 runtime release。
...
| 主基线合并 | repo closure passed / main acceptance pending | 仓内已执行 Registry Closure、Markdown Render Closure 和 Track B/C/D validation 命令；正式主基线 acceptance 仍需纳入对应 validation profile。 |
| Runtime enforcement | pending accepted registry | 只有主 Registry 接收并启用 validation profile 后，新增 Gate 才能成为 release blocker。 |
...
下列命令最初是本文建议的 **closure 命令契约**。2026-05-21 的仓内基线实现已把命令入口写入 `package.json`，由 `scripts/validation/mission-operating-model-closure.mjs` 生成机器报告；它们可以作为 registry patch 证据，但主 Validation Baseline 是否把这些 gate 提升为 release blocker 仍需单独接受：
...
| 证据 | 最低要求 |
|---|---|
| 命令存在性 | `package.json` 或 CI workflow 能定位命令入口，命令失败会返回非零状态 |
| 工件存在性 | 每个 closure 命令定义机器可读 artifact path、hash、owner |
| 主基线引用 | Gate / Metric / Event / Runbook / CI Job Registry 引用这些 artifact，而不是仅引用本文 |
...
本文处于 `docs_zh/reference/`，它的定位是参考设计输入。若本文与当前平台权威对象冲突，按下表处理：

| 主题 | 当前权威 | 本文允许做什么 | 本文不得做什么 |
|---|---|---|---|
| Mission 定位 | `docs_zh/reference/mission_architecture_design_review_v1_4_full_merged.md` 与现有 Mission contracts | 为 Mission 增加 Playbook / Stage 治理候选 | 把 MissionStage 变成 Harness/Node 的第二执行状态机 |
| 执行主链 | `PlanGraphBundle`、`HarnessRun`、`NodeRun`、`NodeAttempt` canonical runtime | 提供业务阶段 gate、outcome、evidence 约束 | 复活 Step-centric 或线性 workflow truth |
...
| Mission StageExit 审计事件类型 | `src/platform/contracts/mission/index.ts` | `tests/unit/platform/control-plane/mission-services.test.ts` |
...
| Track B 失败模式检测、Outcome 度量和 Mission 审计事件 | `src/platform/five-plane-control-plane/mission/operating-model.ts` | `tests/unit/platform/control-plane/mission-operating-model.test.ts` |
| Track C Workflow Trace retention 与 Candidate→SkillPack 安全链 | `src/platform/five-plane-control-plane/mission/operating-model.ts` | `tests/unit/platform/control-plane/mission-operating-model.test.ts` |
...
1. 不把 `MissionStageInstance` 持久化成第二套执行 truth；它当前是 Mission 治理契约，后续持久化必须走 Mission truth/event 设计评审。
2. 不把 StageExit 直接改写 `HarnessRun`、`PlanGraphBundle` 或 `NodeRun` 状态；执行面继续由 canonical runtime 控制。
...
| 阶段 | 核心问题 | 关键产物 | 退出条件 |
|---|---|---|---|
| Idea | 这个问题是否真实、具体、值得做？ | problem hypothesis、customer discovery、competitive map、solution concept | problem-solution fit |
| MVP | 应该先构建什么最小可验证产品？ | MVP scope、architecture context、security review、measurement framework | 真实 PMF 证据 |
| Launch | 业务是否能稳定增长并承受生产压力？ | production hardening、ops workflows、security/compliance、product management process | 可重复增长、可生产运行、运营不再依赖 founder |
| Scale | 组织和产品是否能被审计、可持续、难复制？ | governance、SLA、support infra、GTM system、data/workflow moat | 系统化增长、治理成熟、护城河成立 |

对我们来说，这个阶段模型不应该被直接照搬，而应该抽象为：
...
Playbook 明确指出，在 AI-native startup 中，founder 的角色从亲自写代码、运营、管理日常任务，转向 **orchestrator of agents**。这正好对应我们系统中的：
...
| 角色 | 应看到的对象 | 不应直接暴露的对象 |
|---|---|---|
| 普通业务用户 | Mission goal、progress、risk、evidence、decision needed、final output | NodeAttempt、lease、fencing、CAS、raw events |
| Operator | Mission、Task、PlanGraph、NodeRun、HITL Queue、Runbook、SLO | 低层存储实现细节 |
| 平台工程 | EventLog、TruthStore、CAS、Lease、Fencing、DLQ、Projection、Metric、Trace | 无 |

这再次支持系统设计中“弱化 step 概念”的方向：用户不应面对 Step，而应面对 Mission / Stage / Outcome / Evidence。

---

### 1.3 AI-native 的关键不是更快执行，而是证据化执行

Playbook 在 Idea 阶段反复强调：AI 降低了 build 门槛，但不能把“能 build”误认为“已验证”。在 MVP 阶段，它也强调 AI coding 会带来 agentic technical debt，需要架构上下文、scope 文档、安全审查和 measurement framework。

对应到 Automatic Agent Platform：

```text
AI-native ≠ 跳过流程
AI-native = 自动执行 + 自动证据 + 自动评估 + 自动反馈 + 人类处理高价值判断
```
...
> 注：页码/章节范围基于当前 PDF 目录与段落结构人工校对，`sourceRefConfidence=medium`；正式并入主文档前应由文档 owner 做最终页码复核。

> 页码范围以用户上传的 36 页 PDF 为参考，用于内部追溯。若 PDF 后续换版，应重新生成本表的 `sourceRef`。

> 转译边界：下表的“平台对象 / Gate / Registry”是 Automatic Agent Platform 的架构推导，不代表 Anthropic 原文定义了这些对象。若后续引用原文做产品或市场结论，应回到官方原文复核，不应反向把本表当成原文替代。

| Playbook Source | PDF 章节 / 页码范围 | sourceRefConfidence | 原文主题 / 设计意图 | 我们的抽象 | 平台对象 | 对应 Gate / Registry |
|---|---|---|---|---|---|---|
| Startup lifecycle rebooted | Ch.1 / p.3-4 | medium | Idea / MVP / Launch / Scale 重新定义 AI-native startup 生命周期 | Mission Stage Model | MissionPlaybook、MissionStageGraph | GATE-MISSION-PLAYBOOK-001 |
...
| Scale Stage | Ch.6 / p.25-30 | medium | 可审计治理、组织成熟度、数据飞轮 | 平台护城河来自 workflow/evidence/knowledge 复利 | Knowledge Graph、Skill Catalog、MissionOutcome | Metric / Evidence / Knowledge Registry |
...
```text
Mission Stage 不是 Step。
Stage 是业务生命周期状态；NodeRun 才是执行时节点；Task 是 Mission 下的工作单元。
```
...
| 图 | 是否允许循环 | 目的 | 约束 |
|---|---:|---|---|
| MissionStageGraph | 允许受控循环 | 表达业务生命周期，例如 monitor → improve → review | 每次循环必须创建新的 `stageInstanceId`，受 maxCycle / dwellTime / budget / approval guard 约束 |
| PlanGraphBundle | 不允许循环，必须 DAG | 表达一次 HarnessRun 的执行计划 | 必须通过 DAG 校验、依赖校验、worst-path budget 校验 |

示例：

```text
MissionStageGraph:
  publish → monitor → improve → review   # 合法，代表业务持续改进循环

PlanGraphBundle:
  nodeA → nodeB → nodeA                   # 非法，执行计划不能循环
```
...
Edge-level guard 用于区分不同阶段边的风险。例如 `validate → publish` 应比 `monitor → improve` 更严格。所有 stage transition 必须先检查 edge guard，再评估 exit criteria。

Canonical enum requirement:

```text
RiskLevel / DataClass / RuntimeMode 必须引用 v2.0 canonical contracts，本文不得重新定义这些 enum。
所有 Playbook DSL、StageEdgeGuard、Policy Action 与 Native Surface Matrix 都必须复用同一组 canonical enum，避免 RiskLevel / RuntimeMode 再次出现多处定义与排序冲突。
```
...
v1.0 中 `rolled_back` 被定义为长期状态。v1.1 修正为：rollback 是 transition result，不是稳定长期状态。
...
rollback 完成由恢复/迁移流程通过独立 Runbook 或 MigrationPlan 表达；StageExit 本身只产生 `rollback_requested`，不定义 `mission.stage.rollback_completed`，避免把 rollback completion 误认为 StageExit 的同步结果。

---

### 3.4 每个阶段必须有 Exit Criteria，且必须使用安全 DSL
...
这会引入任意代码执行、不可静态分析、不可审计、不可回放等风险。v1.1 改为安全 DSL。
...
#### 3.4.1 ExitCriterion snapshot binding 与验证器

ExitCriterion 只能基于 StageExit 输入中的 immutable snapshot 计算。每次计算必须记录实际值、期望值、表达式 hash 与 snapshot 引用，保证后续 replay 可解释。
...
| operator | 必须与 value 类型兼容 |
| window | 必须使用 `BoundedTimeWindow`，不允许裸 string 或无限窗口 |
...
```text
stage window 的 maxDurationMs 必须显式提供；active stage 不允许无上限窗口。
...
Playbook 在每个阶段都给出常见失败模式。我们应把它系统化：
...
```text
同一 failure mode 必须按 dedupeKey 去重。
...
### 3.6 引入 Mission Outcome Measurement Framework

Playbook 提到 MVP 阶段应提前建立 measurement framework，避免把早期热度误认为 PMF。对我们来说，Agent 完成任务不等于 Mission 成功。

本文将 Outcome 拆成四层，并增加 `OutcomeObservation`，防止系统“自称成功”但没有采用证据：

```text
Execution Success        # 是否执行完成
Immediate Outcome        # 产出是否合格
Delayed Outcome          # 一段时间后是否被采纳
Business Impact          # 是否形成业务价值 / 实验 / 决策 / 知识资产
```
...
type MissionOutcomeScores = {
  ...
  // 第一阶段不直接手写抽象 businessImpactScore；
...
```text
Mission 成功 = 产出被采纳 / 证据完整 / 成本可控 / 风险可控 / 能沉淀改进。
不是仅仅“跑完”。
MissionOutcomeScores 是标准化结果；MissionOutcomeReport.quality/evidence/cost/latency/risk 是原始评分明细。
Metric Registry 只采集 MissionOutcomeScores 的标准化字段，并通过 scoresGeneratedFromRefs 回溯原始来源。
```
...
SkillCandidate 只表达“候选能力已被接受”，不能承载 SkillPack / SkillRuntime 生命周期。
SkillPack 必须独立经过 manifest、policy、SBOM、eval、signature、canary、activation。
```
...
### 4.1 三者都需要生命周期，但不能共用一套生命周期

```text
Mission 生命周期 = 业务目标 / 长期计划 / 阶段推进
Task 生命周期    = 可执行工作项 / 排队执行 / 阻塞恢复
Session 生命周期 = 人机交互上下文 / 对话窗口 / 审批上下文
```

| 对象 | 生命周期类型 | 作用 |
|---|---|---|
| Mission | 业务生命周期 + 阶段生命周期 | 管理长期目标、阶段推进、成果沉淀 |
| Task | 工作项生命周期 | 管理单个可执行任务的创建、执行、阻塞、完成 |
| Session | 交互生命周期 | 管理人机对话、审批、上下文窗口 |
| HarnessRun | 执行生命周期 | 管理一次受控运行 |
| PlanGraphBundle | 计划生命周期 | 管理 DAG 计划生成、验证、替换 |
| NodeRun | 运行节点生命周期 | 管理单个节点执行 |
| NodeAttempt | 尝试生命周期 | 管理一次具体执行尝试 |
...
Session 是交互入口，不是长期生命周期主轴。Session 可以创建 Task，但 Task 不应依赖 Session 存活。
...
```text
INV-LIFECYCLE-001:
Mission、Task、Session 必须有独立生命周期，不能共用同一状态枚举。

INV-LIFECYCLE-002:
Session terminal 不得自动终止 Task 或 Mission。

INV-LIFECYCLE-003:
Task terminal 不得自动完成 Mission，必须经 StageExitGate 判断。

INV-LIFECYCLE-004:
Mission stage advance 必须生成 StageExitDecision。

INV-LIFECYCLE-005:
所有 terminal state 必须 immutable，除非通过显式 repair / reopen 协议生成新 run。

INV-LIFECYCLE-006:
所有状态转换必须产生 PlatformFactEvent。

INV-LIFECYCLE-007:
跨层状态传播必须通过事件和决策服务，不允许直接 mutate 上层对象。

INV-LIFECYCLE-008:
Session memory 不是 Truth，不能作为 Mission / Task 权威状态来源。
```
...
```text
管理不同 MissionType 的阶段图、默认技能、退出标准、失败模式、度量框架、运行边界。
```
...
### 5.1.1 MissionPlaybook 版本治理与生命周期

MissionPlaybook 自身必须按配置/策略对象治理，不能作为普通 Markdown 文档静态引用。
...
canary playbook 不得作为全租户默认 playbook。
revoked playbook 不得继续创建新 Mission。
已有 Mission 使用 revoked playbook 时必须进入 controlled migration / hold。
```

### 5.1.2 MissionPlaybookResolutionPolicy

Mission 创建时必须解析并锁定 `playbookId + playbookVersion`，运行中的 Mission 不得随 active playbook 自动漂移。
...
运行中的 Mission 不得自动漂移到新 playbook。
revoked playbook 不得创建新 Mission。
已有 Mission 使用 revoked playbook 时进入 hold / controlled migration。
canary playbook 只能通过明确 rollout policy 命中。
没有可用 playbook 时 fail-closed，不得回退到无 playbook 运行。
`use_last_active` 只允许在以下条件全部满足时使用：last active playbook 未 revoked / suspended / archived；compatibility 仍然通过；tenant 仍有授权；rollback / migration policy 允许；并生成 resolution auditRef。否则必须 `fail_closed`。
任何 fallback 命中必须产生 `aa.mission.playbook.use_last_active.count`；任何被阻断的不安全 fallback 必须产生 `aa.mission.playbook.unsafe_fallback_blocked.count`。
```
...
```text
不允许直接替换运行中 Mission 的 playbookVersion。
不兼容迁移必须 HITL 批准。
...
```text
在每个 MissionStage 结束前，读取 playbook、metrics、evidence、risk、budget、HITL 决策，判断是否允许进入下一阶段。
```
...
StageExitGateService 失败必须 fail-closed。
Stage advance 不允许绕过 RSM / CAS。
所有 metric/evidence/risk/budget/HITL 输入必须来自不可变 snapshot。
input.playbookId / input.playbookVersion 必须与 Mission 锁定版本、StageInstance 版本完全一致。
held 状态重新 evaluate 必须提供 remediationRef 或 approved hitlDecisionRef。
...
| 场景 | 行为 |
|---|---|
| 同 `idempotencyKey` 重试 | 返回原 decision，不重复创建 stage / event |
...
| rollback / advance 并发 | CAS 只允许一个成功，另一个返回 version conflict |
...
`mapDecisionToStageStatus` 必须按下表实现，禁止把 `require_hitl` 重新加入 `MissionStageStatus`：
...
| `require_hitl` | `held` | `requiredActions` 必须包含 `hitl_required` | `mission.stage.held` |
| `rollback` | `held` | 仅生成 rollback request；是否创建 previous/new stage 由 Migration/Recovery policy 决定 | `mission.stage.rollback_requested` |
| `terminate` | `terminated` | MissionStage 终止，不再自动推进 | `mission.stage.terminated` |

`StageExitDecision` 是业务决策对象，`EventEnvelope` / event payload 是事件事实对象。事件 append 时必须补齐 `sequence / correlationId / causationId`，不得直接把裸 decision 当成完整事件 payload。

禁止事项:

```text
禁止 StageExitDecision 先写入、stage state 后异步写入的分离事务。
禁止先推进 stage 再异步补 decision。
禁止只完成 current stage 而不原子创建 nextStageInstance。
禁止 mission.currentStageInstanceId 与 stage truth 分离更新。
禁止不锁定 stage version 的并发 evaluate。
禁止从可变 dashboard 当前值直接读取 exit metric。
...
### 5.3.1 FailureMode Severity 与 Gate Severity 的关系

FailureMode severity 和 Gate severity 分工不同：

```text
Gate severity 控制 release / runtime 阻断级别。
FailureMode severity 控制响应级别、Runbook 路由和学习优先级。
```
...
P0 Gate 触发时，即使 FailureMode 为 P1，也按 P0 响应。
FailureMode suppression 必须有 owner、expiry、auditRef，不得 suppress P0 gate failure。
同一 dedupeKey 在 recurrenceWindow 内重复触发，应升级 recurrence signal，而不是重复刷告警。
```
...
```text
判断 Mission 的实际业务价值，而不仅是运行成功与否。
```

输出对象见 §3.6。Outcome service 必须支持：
...
MissionOutcome 必须区分执行质量、内容质量、证据质量、采用情况和业务影响，不能用一个 `quality_score` 覆盖全部。完整数据模型以 §3.6 为唯一来源，§5.4 只描述 service 职责、输入输出、measurement window 和 delayed observation，避免 Outcome 类型重复定义。
...
| 层级 | 含义 | 是否立即可得 |
|---|---|---:|
| Execution Success | 系统是否跑完、是否无 P0 gate failure | 是 |
| Immediate Outcome | 产物是否可读、证据是否完整、质量分是否达标 | 是 |
| Delayed Outcome | 7d / 30d 后是否被采纳、是否产生下游任务 | 否 |
| Business Impact | 是否节省人力、产生实验、支持决策 | 否 |
...
```text
把高质量、重复出现、可自动化的人类或 Agent workflow 转换成可治理的 SkillPack。
```
...
Workflow Recording 必须有独立数据治理边界。浏览器 / IDE / 桌面录制尤其不能默认采集完整 DOM、屏幕或 request body。
...
```text
任何未显式设置的 recording policy 均按更严格值处理。
生产环境默认不记录 request body / response body。
full DOM capture 只能在 sandbox / internal tenant / explicit consent / short retention 下启用。
DOM 持久化前必须生成 redactionReportRef。
redactionReportRef 必须进入 WorkflowTrace。
过期 recording 必须由 retention sweeper 删除，并生成 deletion proof；删除失败必须进入 Runbook D.40。
...
```text
Recording 不能绕过 Policy Engine。
Recording 不能绕过 Data Governance Gate。
Recording 不能捕获 secret / credential。
Recording 不能直接生成 active Skill。
...
```text
Mission:
  长期目标与业务阶段的容器。必须绑定 MissionPlaybook。

MissionStage:
  Mission 生命周期中的业务阶段。不是 Step，不直接执行工具。

Task:
  MissionStage 下的可管理工作单元。可以由人创建，也可以由 StageExitGate 或 Playbook 自动派生。

Session:
  人机交互上下文。不是权威执行对象，不应承载长期真相。

PlanGraphBundle:
  Task/HarnessRun 的执行计划。必须是 DAG，不允许退回线性 PlanStep[]。

HarnessRun:
  一次受控执行运行，负责预算、风险、证据、状态机、调度。

NodeRun:
  PlanGraph 中一个可执行节点的运行实例。

NodeAttempt:
  NodeRun 的一次尝试，记录工具调用、模型调用、错误与 receipt。

Workflow:
  可复用流程模板或业务流程描述，不应替代 Mission / Task / HarnessRun。
```

---

### 6.2 新对象与 OAPEFLIR 的关系
...
```text
MissionStage 决定“当前业务处于什么阶段、目标是什么、退出条件是什么”。
OAPEFLIR 决定“为了完成当前阶段任务，Agent 如何观察、评估、计划、执行、反馈、学习、改进、发布”。
```

| MissionStage | OAPEFLIR 主要阶段 | 说明 |
|---|---|---|
| discover | Observe / Assess | 来源摄入、风险判断、数据治理 |
| review | Assess / Plan / Execute / Feedback | 论文阅读、claim extraction、证据链接 |
| validate | Feedback / Learn | 交叉验证、冲突检测、质量评估 |
| publish | Release | 产物发布、artifact lifecycle、权限检查 |
| monitor | Observe / Feedback | 监控新论文、新数据、指标变化 |
...
Anthropic Playbook 区分 Chat / Claude Cowork / Claude Code 等不同 surface。我们的平台也应该明确入口、允许动作、Policy Action 与后端对象关系。

| Surface | 适合任务 | 后端对象 | 风险边界 |
|---|---|---|---|
| Session Chat | 快速问答、需求澄清、单次咨询 | Session only / Task optional | 默认不产生 side effect |
| Operator Cockpit | Mission 管理、HITL、Runbook、SLO | Mission / Task / HITL / Incident | 所有写操作经 Policy / Audit |
| CLI / SDK | 工程自动化、批处理、验证任务 | Task / HarnessRun | 必须携带 principal、tenant、idempotencyKey |
| IDE Surface | Code Agent、代码修改、测试 | Code Mission / PlanGraph / NodeRun | 写文件 / shell 必须 sandbox + approval |
| Browser Extension | Workflow recording、web evidence capture | WorkflowTrace / Evidence | 默认只读，写操作必须 HITL |
| Mobile | 审批、告警、轻量审查 | HITL / Incident / Notification | 不允许高风险配置修改 |
| API / Integration | 系统间调用 | RequestEnvelope / Task | 必须有 ContractEnvelope / version / signature |
...
```text
Native Surface 不得直接写 Truth Store。
Native Surface 不得绕过 Interface Gateway / Policy Engine / Tool Registry。
Browser Extension 默认只读，写操作必须经 Gateway + HITL + SideEffectRecord。
CLI / SDK 写操作必须携带 principal、tenantId、correlationId、idempotencyKey。
所有 Surface operation 必须映射到 Policy Action 与 Required Capability。
```
...
|---|---|---|---|---|---|
| GATE-MISSION-PLAYBOOK-001 | P0 | 无 | MissionType 未绑定 MissionPlaybook | playbook-validate | D.35 |
| GATE-MISSION-PLAYBOOK-002 | P0 | 无 | MissionStage 缺 exit criteria | playbook-validate | D.35 |
| GATE-MISSION-PLAYBOOK-003 | P1 | P0 if production mission | MissionStage 缺 failure mode 声明 | playbook-validate | D.36 |
| GATE-MISSION-PLAYBOOK-004 | P0 | 无 | Stage advance 未生成 StageExitDecision | stage-exit-e2e | D.37 |
...
| GATE-MISSION-PLAYBOOK-005 | P0 | 无 | Active playbook 缺 signature / compatibility / rollback | playbook-validate | D.35 |
| GATE-MISSION-PLAYBOOK-006 | P0 | 无 | Mission 从 revoked/suspended playbook 创建 | playbook-validate | D.35 |
| GATE-MISSION-PLAYBOOK-007 | P0 | 无 | Running mission 未经 migration plan 自动漂移到新 playbookVersion | playbook-validate | D.35 |
...
| GATE-SKILL-CANDIDATE-002 | P0 | 无 | SkillCandidate 未经 HITL approval / owner / policyDraft / evalSuite 转换为 SkillPack | skill-candidate-validate | D.39A |
| GATE-SKILLPACK-001 | P0 | 无 | SkillPack 未经 manifest/policy/SBOM/eval/signature/canary/rollback 进入 active | skillpack-validate | D.39B |
| GATE-WORKFLOW-RECORDING-001 | P0 | 无 | Workflow recording 绕过 policy | workflow-recording-policy-validate | D.40 |
| GATE-WORKFLOW-RECORDING-002 | P0 | 无 | Workflow recording 采集 restricted data 且未脱敏 / 未授权 | workflow-recording-data-boundary-validate | D.40 |
| GATE-WORKFLOW-RECORDING-003 | P1 | P0 if restricted/credential data | Workflow recording 过期未删除、缺 deletion proof 或删除失败 | workflow-recording-retention-validate | D.40 |

说明：

```text
GATE-SKILL-CANDIDATE-001 用于阻断 SkillCandidate definition incomplete，发生在创建 / review 前。
GATE-SKILL-CANDIDATE-002 用于阻断 SkillCandidate unsafe conversion，发生在 approved → converted_to_skillpack 前。
GATE-SKILLPACK-001 用于阻断 SkillPack unsafe activation，发生在 canary / active 前。
```

v1.0 示例中的 `GATE-QUALITY-001` 已移除，不再使用未登记 Gate。Research quality 统一归入 `GATE-MISSION-OUTCOME-001`、`GATE-EVIDENCE-001`、`GATE-TEST-003` 的组合。

---

### 9.2 新增 Metric Registry 条目

> 以下指标是待评审的 v2.0 Metric Registry patch。正式定义应包含 name、type、formula、window、labels、source、dashboard、alert、owner、target。
...
P0/P1 混合表达不得只写在人类可读文本中；进入主 Metric Registry 时必须转成 `defaultSeverity + escalationRules`，并能被 alert/router 自动解析。

> 说明：上表 `Alert` 列是 human-readable summary。合并进入主 Metric Registry 时，必须生成机器可读的 `metric-alert-policy.yaml`，并把该文件引用写入 Release Evidence Bundle，其中每个 P0/P1 指标都使用 `defaultSeverity + escalationRules` 表达；禁止只保留自然语言阈值。
...
> 以下事件是待评审的 v2.0 Event Registry patch。正式 registry 需要补 payload schema 文件、producer / consumer、compatibility policy、replay behavior。
...
```text
mission.stage.exit_evaluated 必须先于 mission.stage.advanced / held / rollback_requested / terminated 写入同一事务。
所有 stage transition 事件必须包含 `MissionStageTransitionEventBase` 字段：missionId、stageInstanceId、stageId、playbookId、playbookVersion、decisionId、sequence、correlationId、causationId、auditRef。
StageExitDecision 与 EventEnvelope / EventPayload 边界必须清晰：Decision 不直接等同于事件；event append 时必须由 EventAppender 补齐 sequence、correlationId、causationId。
criterionResults 必须包含 actualValue / expectedValue / expressionHash / snapshotRefs，保证 replay 可解释。
```
...
| Event Registry | 加入 `mission.stage.*`、`mission.playbook.*`、`mission.outcome.*`、`failure_mode.*`、`workflow.*`、`skill_candidate.*`、`skillpack.*` | payload schema 存在且可生成 TS 类型 |
| Runbook Registry | 加入 D.35-D.40 | 每个 P0/P1 Gate 绑定 Runbook |
| CI Job Registry | 加入 `playbook-validate`、`stage-exit-e2e`、`mission-outcome-validate`、`skill-candidate-validate`、`skillpack-validate`、`workflow-recording-policy-validate`、`workflow-recording-data-boundary-validate`、`workflow-recording-retention-validate` | Gate 引用的 CI job 全部存在 |
...
4. 本文中 registryStatus 从 proposed_for_v2_0 改为 accepted，仅在主 registry closure 通过后执行。
```
...
| Anthropic Exercise / Workflow | 我们的 SkillPack / Mission Template | 说明 |
|---|---|---|
...
| Customer discovery synthesis | interview-synthesis | 整理访谈、痛点、需求聚类、证据强度 |
| MVP scope definition | scope-boundary-builder | 生成最小范围、非目标、风险边界、延期项 |
| Architecture context | architecture-context-builder | 为 Code Agent / Claude Code / OpenCode 准备架构上下文 |
| Security review first pass | security-review-first-pass | 初步威胁建模、敏感能力识别、配置风险检查 |
| Measurement framework | outcome-measurement-designer | 生成可度量目标、指标、baseline、数据源 |
...
| Scale moat mapping | workflow-data-moat-analysis | 分析专有 workflow、数据飞轮、知识沉淀 |
...
适用 benchmark：OpenCompass、AIME、GPQA、MMLU-Pro、LiveCodeBench、BFCL、τ-Bench、τ²-Bench。Benchmark Mission 的输出不得直接进入对外发布，必须先经过 evidence / contamination / source reliability gate。

### 11.2 Outcome → Knowledge / Artifact Promotion 规则

| Outcome 类型 | 默认处理 | 进入 Knowledge 条件 | 进入 Artifact 条件 |
|---|---|---|---|
...
| skill improvement suggestion | learning object | 通过 eval / HITL / rollout | 可生成 SkillCandidate |
| external-facing report | requires approval | 需 copyright / data governance / review | 需 artifact signature + recall plan |

Promotion 必须记录：`outcomeReportRef`、`evidenceRefs`、`reviewerRefs`、`promotionDecisionRef`、`artifactHash`。

---

## 12. 不应照搬的内容

### 12.1 不要直接照搬 Idea / MVP / Launch / Scale

这套阶段适合 startup 生命周期，但我们的平台是通用 Agent 平台，应抽象为：
...
Playbook 反复说明 AI 降低了执行成本，但更容易导致错误加速、技术债、scope creep、false signal、security exposure。
...
### 12.3 不要让 Workflow Recording 直接产生生产自动化
...
|---|---|---|---|---|---|
| A：最小可落地版本 | Orchestration Owner | Mission / Task / Session lifecycle; Event Registry | Research Mission 可阶段化推进，StageExitDecision 事务闭环 | disable StageExit enforcement，保留 locked playbook read-only，停止新 stage transition；已有 Mission 仅可在 operator approval 后通过 legacy Task flow 完成 | playbook model, DSL validator, stage-exit report |
| B：Failure Mode 与 Outcome 闭环 | Reliability Owner + Quality Owner | Track A; Metric Registry; Runbook Registry | unsupported claim 可检测，OutcomeReport 可生成 | 关闭 detector enforcement，保留 metrics observe-only | failure mode pack, outcome report, runbook mapping |
| C：Workflow-to-Skill Candidate | Skill Owner + Security Owner | Track A/B; WorkflowRecordingPolicy; Skill Registry | Candidate 不可绕过 HITL/eval/policy；recording 无 restricted capture | 禁用 recording-to-candidate promotion，保留 trace archive | workflow trace, candidate, eval suite, redaction report |
...
原则：proposed gate 先以 observe_only 方式采集指标与误报，只有 Runbook、Metric、Event、CI、Owner 全闭环后才能进入 enforcing。

### Implementation Track A：最小可落地版本

目标：让 Research Intelligence Mission 支持阶段化运行。
...
```text
Research Mission 必须通过 discover → review → validate → publish 阶段推进。
每次 stage advance 必须生成 StageExitDecision。
所有 publish 必须满足 evidence coverage = 100%。
StageExitDecision append 与 stage state update 必须同事务。
```

---

### Implementation Track B：Failure Mode 与 Outcome 闭环
...
```text
重复人工流程可生成 SkillCandidate。
SkillCandidate 不能绕过 owner/policy/eval/rollback/HITL。
Workflow recording 不能采集 restricted data without redaction / consent。
SkillCandidate approved 不能直接 active，必须进入 SkillPack 生命周期。
```
...
```text
Code Agent Mission 不允许无 PlanGraph 执行。
...
| MissionStage | 新增 MissionStageInstance、cycleIndex、version；expectedVersion 仅用于 transition command | P0 |
...
| Skill Registry | 拆分 SkillCandidate / SkillPack 生命周期，增加 candidate→skillpack 转换 gate 与 SkillPack activation gate | P1 |
| Workflow Recorder | 新增 WorkflowRecordingPolicy、redaction report、retention sweeper、deletion proof 与 deletion failure handling | P1 |
...
Anthropic 这份 Playbook 对我们的最大价值是把 AI-native 抽象成一种“阶段化、证据化、可度量、可编排”的操作系统。
...
```text
Mission 不只是长期任务。
Mission 是有 playbook、有阶段、有 exit criteria、有 failure modes、有 outcome measurement、有 learning loop 的业务操作系统单元。
```
...
```text
文档状态：reviewed-reference / architecture-patch-candidate
建议下一步：先把 Release Evidence 中列出的 proposed closure 命令、artifact、CI job 与 registry patch 真正实现到主仓；通过 Gate / Metric / Event / Runbook / CI / Evidence Bundle / Markdown Render closure 后，再作为 v2.0 Validation Baseline 的新增章节 + 附录并入主文档，并将 registryStatus 改为 accepted。
```
...
2. Mission / Task / Session Lifecycle Alignment 写入主架构文档和 contract。
3. FailureModeRegistry 与 Gate / Metric / Runbook 打通。
4. UI Cockpit 增加 MissionStage Timeline 和 Exit Criteria 面板。
5. Workflow-to-Skill Candidate 在 Track C 进入 observe_only / internal enforcing；生产全量启用必须等待 SkillPack activation gate、Workflow Recording policy/data-boundary/retention gates、Runbook D.39/D.40 全闭环。
```

这会让 Automatic Agent Platform 从“可执行 Agent 平台”升级为：

> **AI-native Mission Operating System**  
> 一个能定义目标、验证阶段、执行任务、收集证据、度量结果、识别失败、沉淀技能、持续改进的 Agent 生态系统。
