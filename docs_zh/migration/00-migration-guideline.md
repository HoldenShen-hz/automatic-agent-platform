...
> **文档版本**：v1.1
> **文档状态**：Draft
> **评估范围**：`docs_zh/`（不含 `docs_zh/automatic_agent_platform/`）+ `src/` + `config/` + `divisions/` + `tests/`
...
| 类别 | 文件数 | 🟢 直接 | 🟡 改造 | 🔵 参考 | ⚪ 归档 |
|------|--------|---------|---------|---------|---------|
| 主干文档 (docs_zh/architecture/) | 5 | 0 | 5 | 3 | 0 |
| 技术分析文档 (docs_zh/analysis/) | 3 | 0 | 2 | 0 | 0 |
| 架构与序列图 | 4 | 0 | 3 | 1 | 0 |
| 合约文档 (docs_zh/contracts/) | 113 | 22 | 38 | 20 | 10 |
| ADR (docs_zh/adr/) | 38 | 15 | 8 | 3 | 2 |
| 运维文档 (docs_zh/operations/) | 16 | 5 | 10 | 8 | 7+ |
| 评审文档 (docs_zh/reviews/) | 1 | 0 | 3 | 12 | 6 |
| 治理文档 (docs_zh/governance/) | 7 | 4 | 3 | 1 | 0 |
| 指南文档 (docs_zh/guides/) | 4 | 2 | 2 | 0 | 0 |
| 参考文档 (docs_zh/reference/) | 0 | 0 | 0 | 8 | 9 |
| 研究文档 (docs_zh/research/) | 0 | 0 | 0 | 28 | 0 |
| 归档文档 (docs_zh/archive/) | 0 | 0 | 0 | 0 | 3 |
| **合计** | **~243** | **~48** | **~74** | **~84** | **~37** |
...
### 4.1 主干文档 (docs_zh/architecture/)

| 文件 | 行数 | 等级 | 目标架构层 | 移植说明 |
|------|------|------|-----------|---------|
| `00-platform-architecture.md` | ~2,000 | 🟡 B | 跨层 | 文档分层治理模型（L0-L10）可复用，需更新为七层架构的文档体系 |
| `01-code-structure.md` | ~500 | 🟡 B | Layer 1-2 | 目录结构 + control-plane 角色定义可复用，需与 v2.7 §1-§5 对齐 |
| `02-code-architecture-reference.md` | ~800 | 🟡 B | Layer 5 | Agent 分层、权限、安全模型与 v2.7 §11 安全体系兼容，需扩展组织治理部分 |
| `03-module-diagrams.md` | ~400 | 🟡 B | Layer 2,4 | 六层模块图与反馈循环兼容，需更新 KV cache 对齐细节 |
| `04-runtime-sequence.md` | ~300 | 🔵 C | 跨层 | 约束和反模式列表作为新平台设计参考 |

### 4.2 技术分析文档 (docs_zh/analysis/)

| 文件 | 行数 | 等级 | 移植说明 |
|------|------|------|---------|
| `00-architecture-coverage-matrix.md` | ~150 | 🟡 B | 覆盖率矩阵，需更新以反映五平面模块重组 |
| `01-codebase-vs-design-review.md` | ~2,000 | 🟡 B | 代码与设计差异分析手册 |
| `02-implementation-progress-tracker.md` | ~100 | 🔵 C | 实施进度追踪作为参考 |

### 4.3 架构与序列图文档

| 文件 | 行数 | 等级 | 移植说明 |
|------|------|------|---------|
| `00-platform-architecture.md` | ~2,000 | 🟡 B | 主架构入口文档，SLO 定量指标（95%/90%/100%）可复用，需与 v2.7 §27 对齐 |
| `04-runtime-sequence.md` | ~300 | 🟡 B | 4 组核心运行时序图（Intake/Dispatch/Writeback/Recovery）可直接移植，需补充 OAPEFLIR 全循环序列 |

### 4.4 合约文档 (docs_zh/contracts/) — 113 文件
...
| `003-memory-six-layers.md` | 记忆分层 | §3.5 记忆 |
...
### 4.6 治理文档 (docs_zh/governance/) — 7 文件
...
### 4.8 运维文档 (docs_zh/operations/) — 16 文件
...
### 4.10 参考文档 (docs_zh/reference/) — 0 文件
...
### 4.11 研究文档 (docs_zh/research/) — 0 文件

| 等级 | 说明 |
|------|------|
| 🔵 C（全部 28 文件） | 竞品分析（Claude Code/Codex/Goose/Aider/MetaGPT/LangGraph/Temporal/DeerFlow 等）和参考对齐评审。不直接移植但对新平台设计决策有高参考价值。建议保留 `docs_zh/research/` 目录整体移入新项目 |

### 4.12 归档文档 (docs_zh/archive/) — 0 文件
...
| `golden-tasks.test.ts` | 30 | 🟢 A | 金色任务 |
...
`AuthoritativeTaskStore`（`src/platform/five-plane-state-evidence/truth/sqlite/authoritative-task-store-core.ts` 及同目录拆分模块）是当前系统的全局数据访问门面：
...
| 内容 | 原因 |
|------|------|
| `docs_zh/archive/` 全部 | 历史归档 |
| `docs_zh/reference/` 中 9 个 ⚪ D 文件 | 被 v2.7 替代 |
| `docs_zh/automatic_agent_platform/agent_platform.md` (92K行) | 未删节旧版，已被 v2.7 (6.7K行) 替代 |
| `docs_zh/automatic_agent_platform/` 中间翻译碎片文件 | chunk_b-j、part1-6 为翻译中间产物 |
| `docs_zh/reviews/` 中 6 个 ⚪ D 文件 | 旧版评审 |
| `docs_zh/contracts/` 中 10 个 ⚪ D 合约 | 早期 v1.x 合约 |
