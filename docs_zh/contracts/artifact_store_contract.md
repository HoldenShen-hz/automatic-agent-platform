...
> **OAPEFLIR 相关**：本 contract 定义 OAPEFLIR Artifact Plane 的存储机制，对应 ADR-016 §11 和设计文档 §D。
> **更新日期**：2026-04-17

## 1. 范围

本 contract 定义文件型产出物的存储布局、元数据索引、生命周期和引用语义。
...
- DB 中只保存索引和引用，不保存大体积 BLOB 主体。
- artifact 路径必须稳定且可重建。
- 删除策略不得破坏已完成任务的审计性。
- 对外暴露 artifact 时必须经过权限检查。
...
- 元数据以 DB authoritative 索引为准

### 5.2 对象存储边界

- 对象存储负责 artifact 主体，不负责任务真相状态。
- `artifact_id`、`storage_key`、`checksum` 必须能相互映射。
- 迁移到对象存储后，读取接口语义不变。

### 5.3 GC 与冷存储

- 已完成任务的核心 artifact 不得在审计窗口内被直接删除。
- 可重建或低价值 artifact 可进入冷存储或过期删除。
- GC 必须按 retention policy 执行，并产生日志与审计记录。
