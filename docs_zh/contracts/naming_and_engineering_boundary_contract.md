...
本 contract 补充产品叙事与工程实现的命名边界，并收口审计主体、渠道能力和运行环境能力的统一抽象方向。
...
- 协议、schema、状态机、事件注册表中使用 canonical id。
- 叙事 alias 仅用于产品表达、示意图和对外说明。
- 不允许把 `CEO / VP / Lead` 直接写成调度主键、schema enum 或权限对象 id。

### 2.3 OAPEFLIR 与扩展对象 canonical 写法

以下对象在 contract / schema / API / event 中应使用 canonical 工程命名：
...
命名边界收紧后，产品叙事和工程实现就不会互相绑架；而 actor、channel、environment、resource 这几组抽象，是后续扩展性真正的共用基础。
