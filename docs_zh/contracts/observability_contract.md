...
| `stageMetrics` | observe / assess / plan / execute / feedback / learn / improve / release 的 count / duration / failure / timeout |
...
- step duration 指标必须支持百分位计算（至少 p95）。
- cost 指标必须区分全量任务平均和成功任务平均。
- recovery 指标必须覆盖从恢复事件（`recovery:*`）到最终任务成功率的完整链路。
- 数值精度统一四舍五入到四位小数。

### 4.2 传统核心指标（概括）

以下概括性指标名仍然有效，作为 `RuntimeMetricsSummary` 各维度的简化视图：

- 任务成功率（taskMetrics.successRate）
- 工作流恢复率（recoveryMetrics.successRate）
- token 使用量（stepMetrics.totalTokenCost）
...
| `rollout_record_refs` | `string[]` | rollout 记录引用 |
...
- Improve / Release 指标必须能关联到 guardrail、approval 和 rollout 证据，不能只记录最终成功或失败。
...
- T-47: RuntimeMetricsSummary用oapeflirMetrics.convergenceRate为顶层指标，架构§5.5/§13声明OAPEFLIR仅为投影/trace非truth。修复：该语义收敛到 v4.3 canonical contract；旧字段、旧状态、旧 DTO 或旧术语仅允许作为 legacy/deprecated/projection/migration input，不得作为新实现入口。
