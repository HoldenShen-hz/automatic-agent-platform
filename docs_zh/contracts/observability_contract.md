...
- T-22（observability）: 原 `LogEvent` 字段列表已包含 `harness_run_id`（必填）和 `node_run_id?` / `attempt_id?`（可选），满足架构 §25.8 预算关联键要求。R2-22 指出"缺 harness_run_id/node_run_id"系 audit 误判；本文§3 已正确对齐，无须修改。
