...
- 指标命名统一使用 `oapeflir_<stage>_<metric>_<unit?>` 风格；未实现的示例指标不得提前写成 frozen contract。
...
- T-22（observability）: LogEvent 现以 `harness_run_id / node_run_id / attempt_id` 作为 canonical 主关联键，`task_id` 仅保留为 legacy 查询投影。
