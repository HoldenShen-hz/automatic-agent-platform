...
: Plan Hub 产出 `Plan` DTO 进入 execution plane；execution plane 执行后把 `DualChannelStepOutput` 与 `FeedbackSignal` 回写到 Feedback Hub（对应 ADR-079）。
...
- `WorkerHeartbeat`
- `TakeoverManager`
- `Plan`（OAPEFLIR Plan Hub 输出）
- `FeedbackSignal`（OAPEFLIR Feedback Hub 输入）
...
| `task_id` | `string` | 关联任务 |
| `plan_ref` | `string?` | 关联 PlanDTO 或 execution graph 引用 |
| `stage` | `observe \| assess \| plan \| execute \| feedback \| learn \| improve \| release?` | 生成该票据的闭环阶段 |
...
- 一个 execution 在同一 attempt 下应只对应一个 active ticket。
- ticket 失效后不得再次被 worker 消费。
- execute plane 接到的 authoritative 输入应来自 `PlanDTO`，而不是仅靠非结构化 prompt 拼接。

## 8A. OAPEFLIR Plan → Execute → Feedback 边界

### 8A.1 Plan Hub → Execute（对应 ADR-060）

`Plan` 进入 execution plane 时最少应提供（对应 ADR-060 §2）：
...
**R3 约束**（ADR-060 §3）：
- Execute 层只能接收 Plan DTO，不允许旁路 raw task 直接执行（R3-NOBYPASS）
- Plan 版本链必须保持稳定 lineage，不得被新 worker 静默改写

### 8A.2 Execute → Feedback Hub（对应 ADR-079）

execution plane 完成单次 run 后，除 `DualChannelStepOutput` 外还应能产出：
...
- `FeedbackSignal` 是 Execute → Feedback 的正式输出，不得只通过日志侧带。
- 若某次 run 没有产生 feedback，也应显式记录 `feedback_count=0` 或等价 evidence，避免后续 Learn / Improve 误判链路缺失。
- FeedbackSignal 必须关联到原始 Plan version 和 executionId（用于 Learn Hub evidence 链接，R4-EVIDENCE 约束）
...
- 同一时刻同一 `execution_id` 只能有一个 `active` lease。
...
1. `execution` 由 control plane 注册。
...
7. run 结束后写回结果并释放 lease。
8. 若 lease 过期、worker 消失或 run 卡死，则进入 recovery scan。
...
- queue 选择至少考虑：优先级、能力、隔离级别、队列拥塞度。
...
- worker 心跳和 execution 心跳是不同层级：
  - worker heartbeat 用于说明 worker 存活与容量。
  - execution heartbeat 用于说明某个 run 的进度和存活。
...
`takeover`
: 由于异常、人工接管或治理需要，强制把 execution 交给新执行主体或人工流程。
...
- recovery scan 至少能识别 `running but stale`、`ticket lost`、`duplicate claimant` 三类异常。
...
- `storage_schema_contract.md` 继续负责 `executions` 的 authoritative 基线，execution plane 只是在其上增加调度层。
