...
| 包管理                  | pnpm workspace    | Yarn 4 workspace | Nx            | **pnpm**        | 与后端保持一致；严格依赖隔离；速度最快                                           |
| 构建编排                | Turborepo         | Nx               | 自研          | **Turborepo**   | 与 pnpm 集成好；增量构建缓存；配置简洁                                           |
...
```text
ui/                                    # UI Monorepo 根目录（独立 Git 仓库）
├── package.json                       # 根 workspace 声明
├── pnpm-workspace.yaml
├── turbo.json                         # Turborepo 构建编排
├── tsconfig.base.json                 # 共享 TypeScript 基准配置（与后端对齐：strict + noUncheckedIndexedAccess）
├── .eslintrc.cjs
├── .prettierrc                        # 双引号/分号/2空格，与后端一致
├── .env.example                       # UI 环境变量模板
│
├── packages/
│   ├── shared/                        # L3 共享核心层
│   │   ├── api-client/                # REST + WebSocket 通信
│   │   │   ├── src/
│   │   │   │   ├── rest-client.ts     # fetch 封装 + TanStack Query 集成
│   │   │   │   ├── ws-client.ts       # WebSocket 管理（自动重连/心跳/订阅）
│   │   │   │   ├── interceptors.ts    # JWT 刷新/tenant 注入/签名/去重/离线排队/trace
│   │   │   │   ├── endpoints/         # 按资源分组的 API 函数
│   │   │   │   │   ├── tasks.ts       # → /api/v1/tasks [Implemented/Contracted]
│   │   │   │   │   ├── approvals.ts   # → /api/v1/approvals [Implemented/Contracted]
│   │   │   │   │   ├── dashboard.ts   # → /api/v1/dashboard [Implemented/Contracted]
│   │   │   │   │   ├── mission-control.ts # → MissionControlService 聚合视图 [Implemented/Internal]
│   │   │   │   │   ├── console.ts     # → /console/* [Implemented/Internal]
│   │   │   │   │   ├── admin.ts       # → /admin/v1/* [Implemented/Contracted]
│   │   │   │   │   ├── workflows.ts   # → [Planned]
│   │   │   │   │   ├── agents.ts      # → [Planned]
│   │   │   │   │   ├── marketplace.ts # → [Planned]
│   │   │   │   │   ├── costs.ts       # → [Planned]
│   │   │   │   │   ├── explanations.ts# → [Planned]
│   │   │   │   │   └── index.ts
│   │   │   │   └── types/             # 从后端 Zod Schema 生成
│   │   │   └── package.json
│   │   │
│   │   ├── auth/                      # 认证模块
│   │   │   └── src/
│   │   │       ├── auth-service.ts    # 登录/登出/刷新/SSO 流程
│   │   │       ├── token-manager.ts   # Token 生命周期（依赖 L4 SecureStorage）
│   │   │       ├── session-guard.ts   # 过期/多设备/二次认证
│   │   │       └── types.ts
│   │   │
...
│   │   │       │   ├── mission-control-queries.ts
│   │   │       │   └── index.ts
│   │   │       └── query-client.ts    # QueryClient 工厂（含离线 persister）
│   │   │
│   │   ├── sync/                      # 离线同步引擎
│   │   │   └── src/
│   │   │       ├── offline-queue.ts
...
│   │   ├── domain/                    # 领域逻辑（风险/权限/成本/状态机/域 UI 配置）
│   │   ├── nl-client/                 # NL 交互客户端
│   │   ├── telemetry/                 # 前端遥测
│   │   └── types/                     # 共享类型（从后端 Zod 生成）
│   │
│   ├── ui-core/                       # L2 共享 UI 组件（Web + 桌面）
│   │   ├── design-tokens/
│   │   ├── components/                # Button / Input / Modal / Table / Toast / Sidebar / CommandPalette
│   │   ├── charts/                    # ECharts 封装
│   │   ├── layouts/                   # 响应式布局
│   │   ├── business/                  # TaskCard / ApprovalCard / AgentHealthIndicator / RiskBadge / DAGViewer / DiffViewer
│   │   └── themes/                    # Light / Dark / HighContrast
│   │
│   ├── ui-mobile/                     # L2 React Native 组件
│   │   ├── components/                # 对应 ui-core 接口的 RN 原生组件
│   │   ├── navigation/                # React Navigation 路由
│   │   └── native-modules/            # biometric / push / secure-storage / file-picker
│   │
│   ├── features/                      # L2 功能模块（按契约信息架构分组）
│   │   ├── dashboard/                 # Mission Control → Dashboard
│   │   ├── task-cockpit/              # Mission Control → TaskCockpit
│   │   ├── workflow-cockpit/          # Mission Control → WorkflowCockpit
│   │   ├── approval/                  # Mission Control → ApprovalCenter
│   │   ├── stability/                 # Mission Control → StabilityPanel
│   │   ├── alerts/                    # Mission Control → Alerts
...
│   │   ├── takeover/                  # Admin → AdminTakeoverConsole
│   │   ├── workers/                   # Admin → Workers
│   │   ├── queues/                    # Admin → Queues
│   │   ├── conversation/              # Extended → NL 对话
│   │   ├── workflow-builder/          # Extended → 可视化构建器（Web+桌面）
│   │   ├── workflow-debugger/         # Extended → 调试器（Web+桌面）
│   │   ├── agent-manager/             # Extended → Agent 管理
│   │   ├── hitl/                      # Extended → HITL 人机协作
│   │   ├── explainability/            # Shared → 可解释性查看器
│   │   ├── cost-center/               # Shared → 成本中心
│   │   ├── marketplace/               # Shared → Agent 市场
│   │   ├── domain-wizard/             # Shared → 域接入向导
│   │   ├── settings/                  # Shared → 配置管理中心
│   │   └── analytics/                 # Shared → 数据统计平台
│   │   #  每个 feature 模块内部结构：
│   │   #    ├── web/           # React DOM 实现
│   │   #    ├── mobile/        # React Native 实现
│   │   #    ├── hooks/         # 共享 hooks
│   │   #    └── package.json
│   │
│   └── apps/                          # L1 平台壳层入口
│       ├── web/                       # React 19 + Vite 6 SPA
│       ├── electron-win/              # Electron 34 (Windows)
│       ├── tauri-macos/               # Tauri 2 (macOS)
│       ├── tauri-linux/               # Tauri 2 (Linux)
│       └── mobile/                    # React Native 0.79 (Android + iOS)
│
├── tools/
│   ├── codegen/                       # 后端 Zod → 前端类型生成器
│   ├── mock-server/                   # API Mock（基于后端 OpenAPI spec）
│   └── e2e/                           # 跨平台 E2E 测试
│
└── docs/
    ├── storybook/
...
| ------------------------------------ | --------------------- |
| `pnpm install`                       | 安装所有依赖          |
| `pnpm turbo build`                   | 全量构建              |
| `pnpm turbo typecheck`               | 全量类型检查          |
| `pnpm turbo lint`                    | 全量 lint             |
| `pnpm --filter @aa/web dev`          | 启动 Web 开发服务器   |
| `pnpm --filter @aa/electron-win dev` | 启动 Electron 开发    |
| `pnpm --filter @aa/mobile start`     | 启动 RN Metro bundler |
| `pnpm --filter @aa/codegen generate` | 生成前端类型定义      |
...
interface PlatformAdapter {
  ...
  notifications: {
    ...
    show(notification: AppNotification): Promise<void>;
  ...
  // 文件系统
  fileSystem: {
    pickFile(options?: FilePickerOptions): Promise<FileHandle | undefined>;
  ...
  lifecycle: {
    ...
    onNetworkChange(handler: (online: boolean) => void): () => void;
    isOnline(): boolean;
  };
  ...
  // [Planned] 窗口管理（桌面端）
  windowing: {
    openWindow(url: string, options?: WindowOptions): Promise<WindowHandle>;
    closeWindow(handle: WindowHandle): Promise<void>;
    focusWindow(handle: WindowHandle): Promise<void>;
    isMainWindow(): boolean;
    listWindows(): Promise<WindowHandle[]>;
  };

  // [Planned] 系统 Shell 交互（桌面端）
  shell: {
  ...
  process: {
    ...
    getBuildChannel(): "stable" | "beta" | "nightly";
    getSystemLocale(): string;
  };

  // [Planned] 分析与遥测同意
  analyticsConsent: {
    getStatus(): Promise<"granted" | "denied" | "undecided">;
...
**PlatformAdapter 能力状态总览**：

| 能力组           | 方法数 | 状态          | 平台适用                   | 说明                      |
| ---------------- | ------ | ------------- | -------------------------- | ------------------------- |
| fetch            | 1      | [Implemented] | 全平台                     | 网络请求抽象              |
| secureStorage    | 3      | [Implemented] | 全平台                     | 密钥/Token 安全存储       |
| notifications    | 4      | [Implemented] | 全平台                     | 推送与角标                |
| biometric        | 2      | [Implemented] | 全平台（Web via WebAuthn） | 生物识别认证              |
| offlineStore     | 4      | [Implemented] | 全平台                     | 离线数据持久化            |
| fileSystem       | 2      | [Implemented] | 全平台                     | 文件选择与保存            |
| clipboard        | 2      | [Implemented] | 全平台                     | v2.1 新增                 |
| lifecycle        | 4      | [Implemented] | 全平台                     | v2.1 新增                 |
| deepLink         | 2      | [Implemented] | 全平台                     | v2.1 新增                 |
| haptics          | 3      | [Implemented] | 移动端（桌面/Web no-op）   | v2.1 新增                 |
| platform         | 6      | [Implemented] | 全平台                     | 平台探测与特性查询        |
| windowing        | 5      | [Planned]     | 桌面端                     | v2.2 新增，Phase 2 交付   |
| shell            | 2      | [Planned]     | 桌面端                     | v2.2 新增，Phase 2 交付   |
| process          | 4      | [Planned]     | 全平台                     | v2.2 新增，Phase 2 交付   |
| analyticsConsent | 4      | [Planned]     | 全平台                     | v2.2 新增，Phase 1 Gate 2 |
| screenSecurity   | 3      | [Planned]     | 桌面端 + 移动端            | v2.2 新增，Phase 3 交付   |
...
| 接口            | Web                    | Electron (Win)        | Tauri (Mac/Linux)     | RN (Android)     | RN (iOS)            |
| --------------- | ---------------------- | --------------------- | --------------------- | ---------------- | ------------------- |
| `fetch`         | `window.fetch`         | `net.fetch`           | `tauri::http::fetch`  | RN `fetch`       | RN `fetch`          |
| `secureStorage` | HttpOnly Cookie (受限) | Credential Manager    | Keychain / libsecret  | Android Keystore | iOS Keychain        |
| `notification`  | Web Notification API   | Electron Notification | Tauri notification    | FCM              | APNs                |
| `biometric`     | WebAuthn               | Windows Hello         | Touch ID (macOS only) | BiometricPrompt  | LocalAuthentication |
| `fileSystem`    | File System Access API | Node.js fs (sandbox)  | Tauri fs (scope)      | SAF/MediaStore   | UIDocumentPicker    |
| `clipboard`     | Clipboard API          | Electron clipboard    | Tauri clipboard       | RN Clipboard     | RN Clipboard        |
| `lifecycle`     | visibilitychange event | BrowserWindow events  | Tauri window events   | AppState         | AppState            |
| `deepLink`      | URL routing            | Protocol handler      | Universal Links       | App Links        | Universal Links     |
...
L1 App 启动
  ...
  │   ├─ AuthService(adapter.secureStorage)
  │   ├─ SyncEngine(adapter.secureStorage, adapter.lifecycle)
  │   └─ NotificationService(adapter.notification)
  │
  └─ 渲染 L2 功能模块 UI
```
...
| 供应链   | 依赖锁定（pnpm-lock.yaml）；CI 自动 npm audit / Snyk 扫描               |
...
Push / PR
  ...
  │   ├── pnpm turbo test --filter=packages/shared/*
  │   ├── pnpm turbo test --filter=packages/ui-core
  │   ├── pnpm turbo test --filter=packages/ui-mobile
  │   └── pnpm turbo test --filter=packages/features/*
  │
...
| `GET /api/v1/incidents`                | [Planned]                | Layer C   | alerts, stability (Incident 面板)                                  |
| `GET /api/v1/knowledge`                | [Planned]                | Layer C   | explainability (知识引用查看)                                      |
...
| `GET /api/v1/plugins`                  | [Planned]                | Layer C   | marketplace (市场列表)                                             |
| `GET /api/v1/prompts`                  | [Planned]                | Layer C   | agent-manager (Prompt 版本)                                        |
| `GET /api/v1/cost-reports`             | [Planned]                | Layer C   | cost-center (成本数据)                                             |
| `GET/POST /api/v1/webhooks`            | [Planned]                | Layer C   | settings (Webhook 管理)                                            |
