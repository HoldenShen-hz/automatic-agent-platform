...
> **说明**: 本文档以 ASCII 框架图形式呈现系统全景及各层/各模块的内部结构与交互关系；v1.3 已同步 `01-code-structure.md` v1.3 中的 UI Monorepo、Mission/Yono、五平面新增子域与测试/部署支撑结构。
...
> 本文档历史图中仍保留部分规划口径；v1.3 新增或改写的统计为 **2026-05-18 当前工作区结构快照**。精确文件数应以后续结构盘点脚本为准。
...
| §二五 | 结构图 + 约束图 | 跨平台 UI Monorepo 与前后端边界 |
...
│  │ 跨平台 UI    │  │ 自动化验收    │  │ 版本化配置   │  │ 部署与运维   │        │
│  └──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘        │
│  ┌──────────────┐  ┌──────────────┐                                          │
│  │ src/testing/ │  │src/benchmarks│   ← 测试基础设施与性能基准                │
│  │ 测试公共设施  │  │ 性能基准入口 │                                          │
│  └──────────────┘  └──────────────┘                                          │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
| Outbox/副作用 | 关键状态变化必须通过 outbox 或 side-effect-ledger 可见，不允许静默外部写入 |
...
│  │ Yono Business│ │ services/    │ │ 量化交易域   │  不属于框架基础设施    │
│  └──────────────┘ └──────────────┘ └──────────────┘                       │
│                                                                              │
...
## §二五 跨平台 UI Monorepo 与前后端边界图

> **图类型: 结构图 + 约束图** — 表达 `ui/` Monorepo 的模块归属、六平台壳层和前后端依赖边界。不表达具体页面布局。
...
前后端边界:
  ...
  feature ──禁止──▶ 直接消费后端 DTO 或直接调用 Electron/Tauri/RN API
```
...
> **图类型: 结构图** — 表达 v1.3 代码结构 review 发现的新增权威模块，以及它们与原七层/五平面的归属关系。
...
| `src/testing/` / `src/benchmarks/` | 支撑 | 新增/校准 | 测试基础设施与性能入口 |
...
| §二三 | 时序图 | **新增**: 迁移波次路线图 |
| §二四 | 数据流图 | **新增**: 三轴协作图 |
| §二五 | 结构图 + 约束图 | **新增**: 跨平台 UI Monorepo 与前后端边界 |
