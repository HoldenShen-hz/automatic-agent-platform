...
Phase 1 ─→ Phase 2 ─┬→ Phase 3 ──→ Phase 4 ──→ Phase 5 ──→ Phase 6 ──→ Phase 7
                     └→ Phase 8a ┬→ Phase 8b ─┬→ Phase 8c ──┘(8c 须在 Phase 5 前完成)
                                  └→ Phase 8d ─┘
...
每次任务运行都通过 HarnessRuntime 统一入口，装配约束、工具、上下文，驱动 Planner→Generator→Evaluator 多轮闭环，产出最终结果与证据链。
...
| P4 Execution     | 下发执行计划                 | ExecutionPlan、ToolCall、HITLWait、AsyncDispatch               |
| P5 Evidence      | 写入运行证据                 | HarnessRun、HarnessStep、ContextSnapshot、Evidence             |
...
| Task Context         | 当前任务目标、步骤状态、ExecutionPlan、已完成步骤的 Receipt | 任务级   |
...
| System 级   | 累积足够反馈信号后（异步） | 是否需要更新 prompt/policy/tool_config | ImprovementCandidate → P2 Release   |
...
| replan         | 触发 Planner 重新规划     | replan_count < max_replans  |
...
**现有可复用模块**：IntakeRouter · AssessmentService · PlanBuilder · GoalDecomposer · PolicyCenterService
...
- **目标偏离**：当前结果与 PlanBundle.success_criteria 的距离
- **质量门禁**：复用 §17 模型评估 + DomainEvalFramework(§37.5)
- **风险变化**：执行后风险是否升高（对比 PlanBundle.risk_profile）
...
| 现有组件            | Harness 集成方式                                                                          |
| ------------------- | ----------------------------------------------------------------------------------------- |
| §5 平面间契约       | HarnessRuntime 作为 P3 Orchestration 的统一入口，接收 RequestEnvelope，输出 ExecutionPlan |
...
| §14 Execution Plane | Generator Agent 通过标准 ExecutionPlan → P4 dispatch → ExecutionReceipt 路径执行          |
...
| §59 可解释性        | 每轮 loop 的 PlanBundle/WorkProduct/EvaluationReport 自动纳入解释管线                     |
...
| inputs       | 步骤输入（上下文快照引用）                                                |
| outputs      | 步骤输出（PlanBundle / WorkProduct / EvaluationReport / HarnessDecision） |
...
| replan            | 触发 Planner 重新规划     | 目标偏离、风险升高、replan_count < max | Planner 生成新 PlanBundle                       |
...
v3.1 覆盖范围极广（94 节 · 24 域 · 11 Part），为避免项目实施面过宽，本节定义三环实施法，明确"先做什么 → 再做什么 → 最后做什么"。三环与 §33 分阶段路线对应，但从**能力维度**而非**时间维度**划分优先级。

### 第一环：平台生存环（Platform Survival Ring）

> **没有第一环，不谈大规模接入。** 对应 §33 Phase 1-2 + Phase 8a（Harness 核心可并行，见 §33 依赖图）。
...
**第一环验收门**：可在受控环境中端到端运行一个 Agent 任务（从 NL 输入到结果产出），且任务可被中断、恢复、审计。

### 第二环：平台可用环（Platform Usability Ring）

> **做到第二环，平台能支撑真实业务试点。** 对应 §33 Phase 3-5 + Phase 8b-8c。
...
第一环（生存）    第二环（可用）         第三环（扩张）
 ...
 │ 心+风控  │      │ 协作协议      │      │ Drift+24域全覆盖       │
 └─────────┘      └───────────────┘      └────────────────────────┘
 约 16 周            约 24 周               约 40+ 周
```
...
Phase 8a → Phase 8b；Phase 8a → Phase 8d；Phase 8b + Phase 8d → Phase 8c。Phase 8a 可与 Phase 3-7 并行推进。Phase 5 之前必须完成 Phase 8c。

## Phase 8d：OAPEFLIR v4.4 可执行运行规范（8 周）

> OAPEFLIR v4.4 Runtime Contract。依赖 Phase 8a，可与 Phase 8b 并行，Phase 8c 的治理评测依赖本阶段完成。
...
Phase 3 (企业可靠化)    Phase 8a (统一运行协议)
    │                              ├──────────────┐
    ▼                              ▼              ▼
Phase 4 (规模化扩展)    Phase 8b (长时与人机)  Phase 8d (OAPEFLIR v4.4)
    │                              │              │
    ▼                              └──────┬───────┘
Phase 5 (智能交互与组织治理) ◄──────── Phase 8c (治理与评测)
    │                              │
    ├──────────────────────────────┘
    │
    ▼
...
| DLQ                      | Dead Letter Queue                                          | 死信队列，无法处理的消息/事件的暂存区(§28.7)                                                                                                        |
