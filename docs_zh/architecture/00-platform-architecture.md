...
| downgrade_mode | 降级运行模式              | 信任不足 / 预算紧张 / 风险接近阈值 |
...
| 字段           | 说明                                         |
| -------------- | -------------------------------------------- |
| node_run_id    | 当前执行步骤 ID（NodeRun 主键）              |
| artifacts      | 产出的工件引用列表                           |
...
| finalDecision    | accept / retry_same_plan / replan / escalate_to_human / downgrade_mode / abort / quarantine / revoke_approval / pause_for_external / require_revalidation |
...
**HarnessDecision** 裁决类型（基础裁决 6 种 + 生产扩展裁决 4 种，详见 §58.6）：accept · retry_same_plan · replan · escalate_to_human · downgrade_mode · abort · quarantine · revoke_approval · pause_for_external · require_revalidation。
...
> LoopController(§45.7) 的裁决升级为一级协议。基础裁决为 accept/retry_same_plan/replan/escalate_to_human/downgrade_mode/abort；生产扩展裁决用于隔离、撤销授权、等待外部系统和重新校验。共十种裁决（基础裁决 6 种 + 生产扩展裁决 4 种）。
...
用户问"为什么？"
    │
    ▼
ExplanationRequest { harness_run_id, node_run_id?, depth }
    │
    ▼
┌─────────────────┐
...
src/
  ...
  platform/
    ...
    contracts/          # 平面间契约
      request-envelope/
      control-directive/         # deprecated：替代为 OperationalDirective/DecisionDirective
      execution-plan/           # deprecated：替代为 PlanGraphBundle
      execution-receipt/        # deprecated：替代为 NodeAttemptReceipt
      state-command/
      delegation-request/
      model-request/
...
v4.2 以 Ring 1/2/3（三环）为权威交付模型，具体定义见 §33。Phase 1-9 是历史遗留映射，仅用于与 v4.1 路线对比；Phase 8/9 是 Enterprise 扩张 gate，不阻塞核心平台投产。所有成功标准必须能映射到自动化测试、演练记录、审计证据或人工签核记录；不能验证的描述只能作为目标说明，不得作为 gate。

平台成功标准与域成功标准分离：平台核心只看 Harness / State / Evidence / Governance 的可运行闭环；24 域、12 原型和域 GA 数量按 domain wave 验收，不得反向阻塞核心平台投产。

Ring 1（平台生存环）成功标准对应 Phase 1-2 + Phase 8a/8d 的 MVP slice，是核心投产 gate。Ring 2/3 成功标准对应 Phase 3-9，按需启用。

### Ring 1（平台生存环）— 对应 Phase 1-2 + Phase 8a/8d
...
- 50 并发 workflow 稳定运行，Load test 符合 SLO
- 容灾演练 RTO < 10min
- 非技术用户可通过自然语言创建和管理任务
- 目标分解引擎自动将业务目标拆解为可执行任务图
- 主动式 Agent 按 TriggerPolicy 自动触发且无风暴
- 渐进式自主权 suggestion/supervised/semi_auto/full_auto 四级升级路径端到端验证
...
- 1000 并发 workflow 下高优先级任务不饥饿
- SLA Tier P0 任务 99.95% 在承诺时间内完成（99.99% 仅在专用部署档单独承诺）
...
- 行为漂移检测覆盖 success_rate_drop/override_rate_spike/cost_spike/tool_usage_shift/incident_count 五类指标
