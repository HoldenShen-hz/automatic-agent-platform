...
P4 必须通过 ExecutionReceipt 向 P3/P5 回报执行结果，Receipt 以 `nodeRunId` 为主键，包含 harnessRunId / planGraphId / status / duration / side_effects / evidence_refs / error_detail。旧 `stepId` 只允许出现在 legacy projection，不得作为 P4 执行回执字段。
...
| P4 → P5         | `StateMutationCommand` | 同步  | 写 truth，必须同事务追加 event                              |
...
P4 → P3/P5 的执行回执，记录 NodeRun 执行结果与遥测数据。Receipt 不再以 step 为权威对象；HarnessStep 只是语义/展示层，NodeRun 才是执行层事实。
...
P3/P4 → P5 的 truth 写入指令，基于 CAS、active lease 和 fencing token 保证幂等写入。事件追加和审计追加使用独立命令对象，避免把 truth mutation、event append、audit append 和 artifact write 混成一个宽命令。
...
| ExecutionReceipt | canonical P4 result | NodeRun 结果回执 | 不使用 stepId |
| EventEnvelope | canonical event fact | 状态变更、审计、projection rebuild 输入 | `oapeflir.*` 只做投影事件 |
...
`/api/v1/workflow-runs/{id}/steps` 仅作为 legacy projection 保留；新控制台和 SDK 必须优先读取 NodeRun 与 PlanGraph。
...
P1 ──sync──> P2 ──sync/event──> P3 ──sync──> P4
                                              │
                                              ▼
                                    P4 ──sync──> P5 (StateMutationCommand)
...
自动切换必须声明 `cooldown_window` 和 `stable_recovery_window`，恢复条件满足前不得反复震荡。
...
- v4.2 第一批 migration 必须覆盖 HarnessRun、NodeRun、PlanGraphBundle、Event Registry Metadata、RunVersionLock。
...
v4.2 MVP 不以 86 表全量模型为交付目标，而以不超过 20 张表完成最小生产闭环：
...
v4.2 后 Event Registry 分为三层：`EventEnvelope` 是统一事件信封，`platform.*` 保留平台级事实事件，`oapeflir.*` 只承载 OAPEFLIR 语义投影事件。所有层共享 eventId、traceId、replayBehavior、schemaVersion 和 projection 订阅模型，但 truth projection 的权威输入只能是 platform facts。
...
| circuit_breaker.* / config.changed | platform.* | 无或 oapeflir.stage.* | 不迁入 OAPEFLIR truth，仅通过 OperationalDirective 影响 HarnessRun |
...
PromptBundle 必须声明 `PromptBundleCompatibilityMatrix`，覆盖 Tool schema、Evaluator schema、DomainDescriptor schema 和 Model routing profile。rollback 或 revocation 只影响新 run；in-flight run 默认受 RunVersionLock 保护，除非触发 `BundleRevocationEvent` 并经 HITL / P2 emergency override。
...
`PromptInjectionDefenseChain` 不只属于 Prompt 管理，必须联动 Tool Guardrails、Egress Control、Context Assembly 和 Output Validator。若 Injection Classifier 未有训练数据、误报率、漏报率和更新流程记录，则只能作为辅助信号；阻断必须以规则、隔离和最小权限工程防线为主。
...
热门租户预算账本必须支持 `budget_sub_ledger` / `reservation_shard`，例如 `tenant_monthly_budget → tenant_budget_bucket[0..N]` 并定期 reconciliation。对 streaming LLM 或分段工具执行，必须允许 partial settlement 并记录 `reservation_estimation_error_metric`；估算过低触发 overrun_policy，估算过高导致假耗尽时必须可自动 release 或重估。
...
2. 执行 version compatibility check、approval validity check、budget reservation refresh、secret lease reacquire、external callback signature verification
...
v4.2 保留执行模式字段，用于区分 LLM 辅助规划与确定性热路径执行：
...
核心方法 `decompose(goal, constraints) → GoalGraph → TaskGraph → PlanGraphBundle`，将高层目标拆解为可执行图。
...
目标分解必须传播 budget、risk、permission、data boundary 和 capability constraints；LLM 分解结果必须经过 deterministic DAG validation、capability validation、risk propagation 与 budget propagation 后才能进入 PlanGraphBundle。
...
规则：子节点**继承**父节点所有合规约束，可**追加**但不可**放松**。由于严格性并非总是全序，每类策略必须提供 `PolicyStrictnessComparator`；无法比较时按更严格路径执行或要求合规审批。
...
Chinese Wall 必须有 `WallExpiryPolicy` / reset 流程，避免用户长期使用后权限只收紧不可恢复。审计/debug 需求由 `ComplianceOfficerAuditView` 满足，不向普通调用者暴露知识存在性。controlled sharing 必须通过 `CrossBoundaryTransform` 做脱敏、摘要和字段级过滤。
...
RuntimeInvariant 不可被任何 admin 覆盖，包括 super_admin。治理委托必须声明 DelegationScope、DelegationExpiry、revocation policy 和 PolicyComparator；过期或撤销后所有派生权限即时失效。
...
- Global Control Plane 只保存 metadata、routing policy、region health 和 home-region mapping；不得保存业务 payload、PII、PHI、secret 或 tenant truth。每个 tenant/partition 必须声明 home region 与 failover tier。
...
`ResourceKind` 至少包含：`worker_concurrency`、`tool_qps`、`model_tpm`、`model_rpm`、`budget_amount`、`approval_capacity`、`storage_io`。Admission Controller 只有在整个 `resourceVector` 都可满足时才允许进入队列，避免 token、审批席位或预算任一维度被隐性打爆。
...
训练样本选择同时计算 `CandidateDiversityScore`，覆盖 tenant、domain、语言、任务难度、失败类型和风险等级，避免只从最活跃用户或单一失败模式学习。任何候选若命中 PII、secret、受限数据、holdout contamination 或 eval leakage，必须进入 quarantine 并阻断发布。
...
| compatibilityMatrix | `ComponentCompatibilityMatrix`           | Pack、Prompt、Model、Tool、Policy、Domain、Eval 版本兼容性 |
...
| Debug IDE | breakpoint、watchpoint、step_over、变量检查 | Enterprise，仅 replay sandbox |
...
生产 run 不支持交互式断点或 watchpoint。Debugger 权限等同高敏证据访问：必须双因素认证、最小权限、短期 session、全量审计和导出水印。任何 re-execution replay 都必须运行在 ReplaySandboxPolicy 下，真实外部 side effect 被强制替换为 mock/recorded adapter。
...
容量建议必须同时检查 SLA Tier、队列延迟、budget、approval capacity、provider quota 和 Region failover reserve。任一关键 provider quota 接近上限时，系统生成降级/限流建议，而不是只扩 Worker。
...
- 自运维不得成为唯一恢复路径。每个 P0/P1 平台故障必须有 out-of-band recovery runbook，可在 P1/P3/P4/P5 任一平面不可用时由人工和基础设施控制面执行
...
| HarnessStep | canonical | HarnessStep | false | §14 / §45 |
