...
> **v4.1 版本定位**：OAPEFLIR-Harness 收敛版 + 最小生产闭环版。本版本明确 HarnessRun 是唯一权威 Run，PlanGraph 归属于 Harness plannerOutput，OAPEFLIR 阶段只作为 StageRationale / TraceProjection / Audit View；默认使用 Trace Replay，不假设 LLM 可确定性重放；MVP 收敛为 8-12 周可交付的最小生产闭环。
...
- `approved` 只是授权窗口，不是提交事实；approval 过期、撤回或 scope 不匹配时必须进入 `expired` / `revoked`，不得继续 commit。
...
v4.1 不承诺 active-active truth writes。CAS、Lease、Fencing、Budget Ledger 和 SideEffect Commit 只在 partition leader 内有效：
...
v4.1 后 Event Registry 分为两层：`platform.*` 保留平台级事实事件，`oapeflir.*` 承载 OAPEFLIR 语义投影事件。两层共享 eventId、traceId、replayBehavior、schemaVersion 和 projection 订阅模型。
...
| oapeflir.* | stage、graph、node、attempt、budget、side_effect、reconciliation、decision、hitl、memory、eval、learning | OAPEFLIR 阶段语义、图执行投影、对账、回放和学习发布 |
...
多 Region 部署要求每个 Region 至少达到 §31 HA-3 等级（多 AZ 部署），但 v4.1 不承诺 active-active truth writes。每个 tenant / partition 同一时刻只能有一个写入 leader，其他 Region 提供 follower read、异步复制和受控 failover。
...
> **没有第一环，不谈大规模接入。** 对应 §33 Phase 1-2 + Phase 8a + Phase 8d 的最小切片，目标是 8-12 周交付可运行、可审计、可停止、可回放的最小生产闭环。
...
- **ADR-Multi-Region-Single-Leader-Per-Partition** — 多 Region 采用每 partition 单 leader 写入，follower read + 异步复制 + 受控 failover，禁止 active-active truth writes
