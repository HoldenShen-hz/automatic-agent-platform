...
> **文档定位**：企业级 / 平台级 Agent System 总体技术架构设计文档（稳定性优先 · AI 运营完备 · 业务域接入完备 · 垂直业务域深化完备（24 域） · 统一领域元模型 · 多 Agent 协作协议 · 智能交互完备 · 组织治理完备 · 规模化生态完备 · 运营成熟度完备 · Harness 工程化完备 · Harness 八支柱深化完备 · OAPEFLIR v4.4 可执行运行规范 · 三环实施优先级 · 落地导向版）
...
> **设计目标**：构建一套以稳定性、风险控制、安全可靠、异常处理为第一原则的企业级 Agent 平台，使 Agent 作为高风险自动化单元在企业环境中可控、可恢复、可审计地长期运行；同时具备完整的 AI 运营能力（LLM 抽象、Prompt 治理、模型质量、成本管控），确保平台在 AI 层面同样可控、可演进；提供结构化的业务域建模与接入框架；构建面向非技术用户的智能交互层；建立完整的组织治理体系和规模化运行生态层；补齐运营成熟度层；**并引入 Harness Runtime 统一抽象和 OAPEFLIR v4.4 可执行运行规范——将分散的约束、工具、上下文、反馈能力收敛为 PlanGraph + Event Sourcing + Deterministic Runtime + Governed SideEffect + HITL + Evaluation + Learning Release 的闭环系统，使 Agent 从"一次性模型调用"升级为"受约束、可执行、可记忆、可反馈、可恢复、可审计、可长期运行"的生产级运行时**
> **v4.0 版本定位**：OAPEFLIR v4.4 Executable Runtime Spec 集成版。本版本将 OAPEFLIR 从"受控认知内核"升级为"可执行运行规范"，引入 PlanGraph、Event Registry、确定性调度、SideEffect Reconciliation、Budget Ledger、Context Assembly、RunVersionLock、HITL Responsibility、Evaluation Gate 与 Learning Release Pipeline。
...
**Part I — 基础设施层（§4-§14, §24-§32）** 4. [总体架构：五平面 + 一横切控制织网](#4-总体架构五平面--一横切控制织网) 5. [平面间通信契约](#5-平面间通信契约) 6. [API 契约与版本化架构](#6-api-契约与版本化架构) 7. [服务通信架构](#7-服务通信架构) 8. [可扩展性架构](#8-可扩展性架构) 9. [稳定性架构](#9-稳定性架构) 10. [风险控制架构](#10-风险控制架构) 11. [安全可靠架构](#11-安全可靠架构) 12. [异常事件处理架构](#12-异常事件处理架构) 13. [OAPEFLIR v4.4 受控认知内核与可执行运行规范](#13-oapeflir-v44-受控认知内核与可执行运行规范) 14. [Runtime Execution Plane](#14-runtime-execution-plane) 24. [配置治理架构](#24-配置治理架构) 25. [数据与状态一致性架构](#25-数据与状态一致性架构) 26. [存储架构](#26-存储架构) 27. [性能架构与 SLO](#27-性能架构与-slo) 28. [Event Registry / Projection / Incident / DLQ 模型](#28-event-registry--projection--incident--dlq-模型) 29. [Knowledge / Memory / Artifact / Learning 边界](#29-knowledge--memory--artifact--learning-边界) 30. [业务接入约束与 Business Pack 模型](#30-业务接入约束与-business-pack-模型) 31. [容灾与高可用架构](#31-容灾与高可用架构) 32. [部署架构](#32-部署架构)
...
**Part VI — Harness 工程化与八支柱深化层（§45, §58）** 45. [Harness Runtime 架构](#45-harness-runtime-架构) 58. [Harness 横切关注面](#58-harness-横切关注面)
...
[附录 H：OAPEFLIR v4.4 Executable Runtime Spec 完整规范索引](#附录-hoapeflir-v44-executable-runtime-spec-完整规范索引)
...
Phase 1 ─→ Phase 2 ─┬→ Phase 3 ──→ Phase 4 ──→ Phase 5 ──→ Phase 6 ──→ Phase 7
                     └→ Phase 8a → Phase 8d ┬→ Phase 8b ─┬→ Phase 8c ──┘(8c 须在 Phase 5 前完成)
                                             └────────────┘
Phase 5+8c ──→ Phase 9a → 9b → 9c → 9d → 9e → 9f  (24 域, 48 周)
```

### 图 5 — OAPEFLIR v4.4 可执行运行内核

```text
RequestEnvelope
   │
   ▼
Observe ─→ Assess ─→ PlanGraph ─→ Graph Normalize / Validate / Risk Propagate
                                      │
                                      ▼
                           Deterministic Graph Scheduler
                                      │
...
| type        | enum          | 指令类型（approve / deny / pause / resume / abort / rollback / kill） |
...
P3 → P4 的标准执行计划。自 v4.0 / OAPEFLIR v4.4 起，`ExecutionPlan` 保留为平面间契约名称，但其内部结构从线性 `steps: Step[]` 升级为 `PlanGraphBundle`。所有复杂任务必须以 PlanGraph 形式下发给 P4，由 Graph Scheduler 按确定性策略调度 NodeRun。
...
> OAPEFLIR v4.4 将 Observe → Assess → Plan → Execute → Feedback → Learn → Improve → Release 从抽象流程升级为 P3/P4/P5 共同遵守的生产级 Runtime Contract。它不直接绕过 P4 执行工具，但定义什么图可以执行、什么状态可以迁移、什么事件必须记录、什么副作用可以提交、什么学习结果可以进入发布管线。
...
OAPEFLIR v4.4 是 P3 Orchestration Plane 的内核规范，也是 P4 Execution Plane 的执行约束来源、P5 State & Evidence Plane 的事件与回放事实来源。它定义：

- OapeflirRun / NodeRun 的生命周期和终态封闭规则
- PlanGraph、GraphPatch 与 Replan 的可执行语义
- Graph Normalize / Validate / Risk Propagation / Worst-Path Analysis 的准入门
- Deterministic Graph Scheduler 的 ready node 调度规则
- Event Registry、replayBehavior、causal lineage 与 deterministic replay
- Budget Ledger、SideEffect Manager、Reconciliation 与 Compensation
- ContextAssemblyContract、PromptExecutionContract、DecisionInputBundle
- HITL Responsibility、EvaluationGate、LearningCandidate 与 Release Pipeline
...
| P3 Orchestration | 承载 Observe / Assess / Plan / Feedback / Learn / Improve / Release 语义 | OapeflirRun、PlanGraphBundle、DecisionInputBundle |
...
5. Graph Scheduler 必须 deterministic；同一 graph + seed + policy 的 ready node 顺序可回放。
...
PlanGraph 是 OAPEFLIR v4.4 的主执行结构。它显式表达并发、依赖、join、terminal、补偿和风险传播边界，避免线性 steps 隐藏真实依赖。
...
| deterministic_seed | 同一 graph + seed + policy 产生同一调度顺序 |
...
OAPEFLIR v4.4 将 Run 与 Node 状态作为 P5 truth 的一级对象。所有状态迁移必须满足：
...
`OapeflirRun` 标准状态：created · admitted · observing · assessing · planning · ready · running · pausing · paused · resuming · replanning · compensating · completed · failed · aborted。
...
每个 OapeflirRun 在 admitted 时冻结 `RunVersionLock`，锁定本次运行使用的 Prompt、Policy、Tool、Domain、Model、Eval、Guardrail、RuntimeProfile 和 schema version。运行中配置发布不得改变已运行 run 的语义；只能通过显式 GraphPatch、ControlDirective 或 redrive 使用新版本。

RunVersionLock 目标：

- 支撑 deterministic replay
...
## 26.4 v4.0 OAPEFLIR Runtime 表落点

OAPEFLIR v4.4 新增的运行实体必须在存储层有明确落点。实现时优先复用现有 truth / event / execution 表；确需物化为新表时，必须同步更新 `SchemaInventoryService`、migration 版本、§26.3 表数量和回滚脚本。

| 运行实体 | 推荐表 / Repository | 存储策略 | 所属 Group | 迁移阶段 |
| --- | --- | --- | --- | --- |
| OapeflirRun | `oapeflir_runs` 或 `workflow_runs` 扩展列 | 新 run 使用专表；legacy workflow 可通过兼容 view 读取 | Group 1 | E1/E2 |
...
| RunVersionLock | `oapeflir_run_version_locks` | admitted 时冻结，后续只读 | Group 7 | E1/E2 |
...
- v4.0 第一批 migration 必须覆盖 OapeflirRun、NodeRun、PlanGraphBundle、Event Registry Metadata、RunVersionLock。
...
Event Registry 是 OAPEFLIR v4.4 的事件类型、payload schema、replay 行为和 projection 消费规则注册表。所有事件必须可校验、可回放、可去重、可追溯。
...
v4.0 后 Event Registry 分为两层：`platform.*` 保留平台级事实事件，`oapeflir.*` 承载 OAPEFLIR v4.4 Runtime Contract。两层共享 eventId、traceId、replayBehavior、schemaVersion 和 projection 订阅模型。

| 层级 | 事件范围 | 用途 |
| --- | --- | --- |
| platform.* | workflow_run、execution、tool_call、approval_flow、rollout、incident、dlq、cost、circuit_breaker、config | 兼容既有 projection、incident、dashboard、billing、ops 链路 |
| oapeflir.* | run、stage、graph、node、attempt、budget、side_effect、reconciliation、decision、hitl、memory、eval、learning | v4.4 运行时状态机、图执行、对账、回放和学习发布 |

**兼容映射**：

| legacy / platform 事件 | v4.0 映射 | 处理方式 |
| --- | --- | --- |
| workflow_run.created / failed / completed | oapeflir.run.created / failed / completed | 双写一个迁移窗口；projection 可消费任一命名空间 |
| step_run.* / execution.* | oapeflir.node.* | 新实现以 NodeRun 为 truth，旧 step projection 由 adapter 派生 |
| step_attempt.* | oapeflir.attempt.* | retry / redrive 统一转为 AttemptLineage |
| tool_call.succeeded / failed | platform.tool_call.* + oapeflir.node.* | 工具调用事实保留 platform 事件，节点状态由 OAPEFLIR 事件推进 |
| side_effect.proposed / committed | oapeflir.side_effect.* | committed 后仍需 confirmed / reconciliation 才能作为成功事实 |
| decision.requested / approved | oapeflir.decision.* + oapeflir.hitl.* | 自动裁决与人工责任记录拆分 |
| rollout.* / eval.* | platform.rollout.* + oapeflir.eval.* | 发布治理保留 platform rollout，评测门禁进入 OAPEFLIR eval |
| cost.* | platform.cost.* + oapeflir.budget.* | 财务账单保留 cost 事件，运行预算由 BudgetLedger 管理 |
| circuit_breaker.* / config.changed | platform.* | 不迁入 OAPEFLIR，仅通过 ControlDirective 影响 run |

迁移硬规则：新 projection 必须优先订阅 `oapeflir.*`；旧 projection 在迁移期通过 adapter 从 `oapeflir.*` 派生 legacy 事件；同一事实不得被 platform 与 oapeflir 双重计数。
...
| 命名空间 | 代表事件 | 说明 |
| --- | --- | --- |
| oapeflir.run | created / admitted / paused / resumed / completed / failed / aborted | Run 生命周期 |
...
| deterministic_replay | 事故复盘和一致性检查 | 使用 frozen input + seed，所有副作用 simulate only |
| what_if_simulation | 策略、Prompt、工具替换评估 | 隔离沙箱运行，不写 production truth |

Replay 永远不得真实发送邮件、付款、部署、写外部系统或修改生产环境。需要验证外部状态时只能创建 ReconciliationRecord 或人工 review 任务。
...
incident 必须链接到：affected workflows / executions / workers / rollout / repair jobs / replay jobs / evidence bundles / resolution record。OAPEFLIR v4.4 运行时 incident 还必须关联 runId、nodeRunId、attemptId、eventId、sideEffectId 或 reconciliationId（如适用）。
...
- **与目标分解的交互**：目标分解引擎(§40)递归深度上限 = 5，每层分解可能触发委托。最坏情况下总调用深度 = 分解深度 × 委托深度 = 5 × 3 = 15 层。为防止级联爆炸，平台实施**全局调用深度上限 = 10**（`global_call_depth` 字段随 trace 传播），超过上限时拒绝新委托并触发 escalation
...
| `depth`             | uint8        | 从 §19.2 全局调用深度继承 | 防止递归爆炸（≤ global_call_depth） |
...
| C7  | depth 不得超过 global_call_depth（§19.2 定义，默认 10）                               | task_request 时      | 消息被拒 + escalation    |
...
每次任务运行都通过 HarnessRuntime 统一入口，装配约束、工具、上下文，驱动 Planner→Generator→Evaluator 多轮闭环，产出最终结果与证据链。v4.0 后，Harness 中的 `PlanBundle` 是产品层 wrapper，内部 canonical execution contract 必须是 `PlanGraphBundle`；P4 只按 PlanGraph / NodeRun 语义执行复杂任务。
...
| P5 Evidence      | 写入运行证据                 | HarnessRun、HarnessStep、OapeflirRun、NodeRun、ContextSnapshot、Evidence |
...
| budget        | max_cost / max_steps / max_duration_ms                       | §18 成本管理 + §37.9 DomainGovernancePolicy      |
...
Harness 是 OAPEFLIR v4.4 的外部产品化入口；OAPEFLIR 是 Harness 内部可执行协议。两者的映射关系：

| Harness 对象 | OAPEFLIR v4.4 对象 | 说明 |
| --- | --- | --- |
| HarnessRun | OapeflirRun | 运行实体与生命周期对齐 |
| HarnessStep | NodeRun | 步骤级实体升级为图节点运行 |
| PlanBundle | PlanGraphBundle | Planner 输出必须图化、校验、风险传播 |
...
Harness Runtime 不得绕过 OAPEFLIR v4.4 的 PlanGraph、Event Registry、Budget Ledger、SideEffect Manager 和 EvaluationGate。
...
- deterministic replay 所需 seed 或不可确定性声明
...
基于 ContextSnapshot 序列(§45.5)，支持：

| 能力         | 说明                                              | 用途               |
| ------------ | ------------------------------------------------- | ------------------ |
| 离线 Replay  | 对已完成的 HarnessRun 完整回放                    | 故障定位、审计取证 |
| 策略对比     | 同一任务用不同 ConstraintPack 运行                | 约束调优           |
| Prompt A/B   | 同一任务用不同 Planner/Generator/Evaluator Prompt | Prompt 优化        |
| 工具替换模拟 | 在 Toolbelt 中替换工具后重放                      | 工具迁移评估       |
| What-if 分析 | 修改 ContextSnapshot 中某个值后继续运行           | 根因分析           |

Replay 在隔离沙箱中运行(§34 ADR-Workflow-Debug-Session-Isolated)，不影响生产环境。
...
| §31 HA 架构 vs §52 多 Region 架构范围重叠                              | §31, §52     | §31 定义单 Region 内 HA-1/HA-2/HA-3 分级；§52 定义跨 Region 部署。映射关系：HA-1 = 单节点单 Region，HA-2 = 双节点单 Region，HA-3 = 多 AZ 单 Region，§52 = 多 Region Active-Active（前提是每个 Region 至少 HA-3） |
...
多 Region 部署要求每个 Region 至少达到 §31 HA-3 等级（多 AZ 部署），确保单 Region 故障不影响全局可用性。
...
| availability   | number (%)                              | 承诺可用性，如 99.99%                   |
...
| ttlSeconds        | number                   | 熔断自动过期时间，过期后需显式续期或解除           |
...
v4.0 覆盖范围极广（94 节 · 24 域 · 11 Part · OAPEFLIR v4.4 Runtime Contract），为避免项目实施面过宽，本节定义三环实施法，明确"先做什么 → 再做什么 → 最后做什么"。三环与 §33 分阶段路线对应，但从**能力维度**而非**时间维度**划分优先级。

### 第一环：平台生存环（Platform Survival Ring）

> **没有第一环，不谈大规模接入。** 对应 §33 Phase 1-2 + Phase 8a + Phase 8d（Harness 核心与 OAPEFLIR v4.4 可执行规范可并行推进，见 §33 依赖图）。
...
**第一环验收门**：可在受控环境中端到端运行一个 Agent 任务（从 NL 输入到结果产出），复杂任务以 PlanGraph 执行，任务可被中断、恢复、审计、回放，且真实 side effect 不会在 replay 中重复发生。
...
第一环（生存）    第二环（可用）         第三环（扩张）
 ...
 │ 心+v4.4  │      │ 协作协议+评测 │      │ Drift+24域全覆盖       │
 └─────────┘      └───────────────┘      └────────────────────────┘
 约 20-24 周          约 24 周               约 40+ 周
```
...
> OAPEFLIR v4.4 Runtime Contract。依赖 Phase 8a，是 Phase 8b/8c 的运行语义基线；Phase 8b 的长时/HITL 能力可以局部并行开发，但验收必须以 Phase 8d 的 Run / Node / Event / SideEffect / Budget 契约为准。

### 交付物

- Run / Node State Machine
- Event Registry + Event Replay Semantics
...
Phase 3 (企业可靠化)    Phase 8a (统一运行协议)
    │                              │
    ▼                              ▼
Phase 4 (规模化扩展)    Phase 8d (OAPEFLIR v4.4)
    │                              ├──────────────┐
    ▼                              ▼              │
...
- **ADR-Global-Call-Depth-Limit** — 目标分解(depth≤5) × 委托链(depth≤3) 的全局调用深度上限 = 10，通过 trace 传播的 global_call_depth 字段强制
...
- **ADR-OAPEFLIR-Deterministic-Graph-Scheduler** — Graph Scheduler 必须可确定性回放，不依赖不可复现外部状态
- **ADR-OAPEFLIR-Terminal-State-Immutability** — Run / Node 终态不可迁出，修复只能追加 redrive、compensation 或 GraphPatch
...
src/
  ...
  platform/
    ...
    oapeflir/           # OAPEFLIR v4.4 可执行运行规范实现
      runtime/            # OapeflirRun / NodeRun 状态机
      ...
      context/            # ContextAssemblyContract
      prompt/             # PromptExecutionContract
      llm/                # LLM decision record / deterministic replay seed
      side-effects/       # SideEffect Manager / delivery semantics
      decision/           # DecisionInputBundle / precedence policy
      guardrails/         # OAPEFLIR 级护栏钩子
      hitl/               # HITL lock / responsibility record
...
- OAPEFLIR v4.4 Phase 8d 验收：Run / Node 状态机、PlanGraph、Event Registry、Budget Ledger、SideEffect Reconciliation、EvaluationGate、Runtime Test Matrix 均有自动化覆盖
- Replay 验证不产生真实 side effect，且同一 graph + seed 的调度顺序一致
...
- **OAPEFLIR 必须可执行——复杂任务以 PlanGraph、Event Registry、确定性调度、受控副作用、对账、评测门禁和学习发布管线落地，而不是停留在抽象认知阶段**
...
| PlanGraph                | OAPEFLIR Plan Graph                                        | OAPEFLIR v4.4 的图结构计划模型，替代复杂任务中的线性 steps(§13.7)                                                                                    |
| Graph Scheduler          | Deterministic Graph Scheduler                              | 基于 ready node 的确定性图调度器，同一 graph + seed 可回放(§14.9)                                                                                    |
| GraphPatch               | PlanGraph Patch                                            | 对运行中或暂停中 PlanGraph 的受控追加式变更(§13.13)                                                                                                  |
| Event Registry           | OAPEFLIR Event Registry                                    | OAPEFLIR 事件类型、payload、replay 行为和 projection 消费规则注册表(§28)                                                                              |
...
# 附录 H：OAPEFLIR v4.4 Executable Runtime Spec 完整规范索引

完整规范以 [oapeflir-v4.4-executable-spec.md](/Users/holden/Project/automatic_agent/automatic_agent_platform/docs_zh/architecture/oapeflir-v4.4-executable-spec.md) 为权威来源，本文档 v4.0 在正文中承载架构级规范和落地约束，避免在总览文档中重复维护完整 TypeScript contract。
...
| Run / Node 状态一致性、Budget Ledger、RunVersionLock | §25 |
...
| 4-5 | OapeflirRun、RunStatus、NodeRun 状态机 |
