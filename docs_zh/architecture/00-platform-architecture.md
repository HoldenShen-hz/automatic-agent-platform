...
> **文档版本**：v3.2
> **文档状态**：Release
> **文档定位**：企业级 / 平台级 Agent System 总体技术架构设计文档（稳定性优先 · AI 运营完备 · 业务域接入完备 · 垂直业务域深化完备（24 域） · 统一领域元模型 · 多 Agent 协作协议 · 智能交互完备 · 组织治理完备 · 规模化生态完备 · 运营成熟度完备 · Harness 工程化完备 · Harness 八支柱深化完备 · 三环实施优先级 · 落地导向版）
...
> **设计目标**：构建一套以稳定性、风险控制、安全可靠、异常处理为第一原则的企业级 Agent 平台，使 Agent 作为高风险自动化单元在企业环境中可控、可恢复、可审计地长期运行；同时具备完整的 AI 运营能力（LLM 抽象、Prompt 治理、模型质量、成本管控），确保平台在 AI 层面同样可控、可演进；提供结构化的业务域建模与接入框架；构建面向非技术用户的智能交互层；建立完整的组织治理体系和规模化运行生态层；补齐运营成熟度层；**并引入 Harness Runtime 统一抽象——将分散的约束、工具、上下文、反馈能力收敛为标准化的 Planner→Generator→Evaluator→Loop 闭环运行时，使 Agent 从"一次性模型调用"升级为"受约束、可执行、可记忆、可反馈、可恢复"的工程化闭环系统**
...
**Part I — 基础设施层（§4-§14, §24-§32）** 4. [总体架构：五平面 + 一横切控制织网](#4-总体架构五平面--一横切控制织网) 5. [平面间通信契约](#5-平面间通信契约) 6. [API 契约与版本化架构](#6-api-契约与版本化架构) 7. [服务通信架构](#7-服务通信架构) 8. [可扩展性架构](#8-可扩展性架构) 9. [稳定性架构](#9-稳定性架构) 10. [风险控制架构](#10-风险控制架构) 11. [安全可靠架构](#11-安全可靠架构) 12. [异常事件处理架构](#12-异常事件处理架构) 13. [OAPEFLIR 受控认知内核](#13-oapeflir-受控认知内核) 14. [Runtime Execution Plane](#14-runtime-execution-plane) 24. [配置治理架构](#24-配置治理架构) 25. [数据与状态一致性架构](#25-数据与状态一致性架构) 26. [存储架构](#26-存储架构) 27. [性能架构与 SLO](#27-性能架构与-slo) 28. [Event / Projection / Incident / DLQ 模型](#28-event--projection--incident--dlq-模型) 29. [Knowledge / Memory / Artifact / Learning 边界](#29-knowledge--memory--artifact--learning-边界) 30. [业务接入约束与 Business Pack 模型](#30-业务接入约束与-business-pack-模型) 31. [容灾与高可用架构](#31-容灾与高可用架构) 32. [部署架构](#32-部署架构)
...
本节以四张图概括整份架构文档的核心结构，读者可先建立全局画面再按需深入。
...
P3 输出标准化的 ExecutionPlan（见 §13 接口契约），所有决策都必须可序列化、可审计、可重放。
...
P3 → P4 的标准执行计划，描述编排层分解后的有序步骤与资源约束。

| 字段             | 类型             | 说明                                 |
| ---------------- | ---------------- | ------------------------------------ |
| planId           | string (UUID)    | 计划唯一标识                         |
| steps            | Step[]           | 有序步骤列表（含依赖关系）           |
...
输入：原始请求或事件（RequestEnvelope / WebhookEvent）。输出：StructuredObservation。
职责：从非结构化输入中提取意图、上下文、紧急程度，生成标准化的观察对象供后续阶段消费。

### Assess

输入：StructuredObservation。输出：RiskAssessment + FeasibilityReport。
职责：评估风险等级、检查策略合规性、判定执行模式（自动 / 人机协同 / 拒绝），为 Plan 阶段提供约束边界。

### Plan

输入：RiskAssessment + FeasibilityReport。输出：ExecutionPlan + PlanBundle。
职责：将目标分解为有序步骤、分配预算与工具、生成回滚策略，输出可由 P4 直接执行的标准计划。
...
输入：ExecutionReceipt。输出：FeedbackEnvelope。
职责：评估执行结果质量、检测目标漂移、比对预期与实际产出，生成反馈信号供 Learn 阶段消费。

### Learn（异步）

输入：FeedbackEnvelope 批次。输出：LearningInsight。
职责：异步提取失败模式与成功模式、分类故障根因、聚合跨任务统计，生成可操作的学习洞察。

### Improve（异步）

输入：LearningInsight。输出：ImprovementCandidate。
职责：基于学习洞察生成 prompt 优化补丁、策略规则调整或工具配置变更，提交至 Release 阶段等待治理审批。
...
**总计**：71 表，实现时**按 Group 分阶段建表**，不要求一次性全部到位。
...
workflow*run.* · loop*cycle.* · step*run.* · step*attempt.* · task._ · execution._ · execution*lease.* · worker._ · tool_call._ · side*effect.* · decision._ · artifact._ · memory._ · knowledge._ · rollout._ · incident._ · dlq._ · delegation._ · hibernation._ · prompt._ · eval._ · cost._ · approval*flow.* · agent*lifecycle.* · circuit_breaker.\*
...
Phase 8a → Phase 8b → Phase 8c。Phase 8a 可与 Phase 3-7 并行推进。Phase 5 之前必须完成 Phase 8c。
