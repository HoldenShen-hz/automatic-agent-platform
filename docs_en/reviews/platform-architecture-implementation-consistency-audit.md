...
| C-3 | HIGH   | `src/platform/execution/runtime-state-machine.ts` | RuntimeStateMachine lists `compensating` as a legitimate reachable state for NodeRun, architecture §14.3 explicitly models compensation as independent CompensationRun, not NodeRun sub-state | Unfixed | Pending |
...
| C-7 | LOW    | `src/platform/execution/budget-allocator.ts`      | `settle()` hardcoded `hardCapSatisfied: true`, skips architecture §18 required actual hard cap validation | Unfixed | Pending |
...
| G-2 | HIGH   | `src/platform/orchestration/harness/index.ts`          | Uses "sleeping" state to indicate pause, architecture uniformly uses "paused" (short-term)/"hibernated" (long-term), semantics completely different | Unfixed | Pending |
...
| G-5 | MED    | `src/platform/state-evidence/index.ts`                 | Only implements snapshot/timeline/audit three sub-modules, missing architecture required reconciliation/side-effect-ledger/outbox/compaction four sub-modules | Unfixed | Pending |
