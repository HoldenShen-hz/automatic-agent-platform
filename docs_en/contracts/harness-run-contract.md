...
- All status progression must go through `RuntimeStateMachine.transition(command)`.
...
- `replanning` can only be expressed through `GraphPatch` append, not by overwriting historical `PlanGraphBundle`.
...
- Every status progression must produce platform fact event and audit evidence.
...
- T-7: Contract §45.13 defines 6 states vs architecture §25.8 defines 13 states; architecture documents are also inconsistent internally (§25.4 lists 7 states vs §25.8 lists 13 states). Fix: Semantics converge to v4.3 canonical contract; old fields, old state, old DTOs, or old terminology are only allowed as legacy/deprecated/projection/migration input, not as new implementation entry points.
