...
This file defines the stable error code registry permitted for use in the current phase.
...
- Provider sub-codes must at minimum be细分: `provider.context_window_exceeded`, `provider.model_not_available`, `provider.output_truncated`, `provider.capability_unsupported`.
- Enterprise-specific error codes must at least reserve: `enterprise.environment_unhealthy`, `enterprise.release_guard_failed`, `enterprise.audit_export_denied`.