...
- T-37: Early version referenced `buildTaskResultEnvelope` as the main builder entry. v4.3 canonical builders are `buildHarnessRunResultEnvelope` / `buildNodeRunResultEnvelope`, projecting to `HarnessRun -> NodeRun` chain instead of the old `Task -> Execution` model.
