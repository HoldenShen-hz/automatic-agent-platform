...
- Reports must reference real evidence artifacts and must not fabricate compliance conclusions manually.
...
- `source_event_type` must be traceable back to `platform.*` truth fact or registered view/evidence events; if only `oapeflir.view.*` exists without source fact, the report must mark evidence as incomplete.
...
- `evidence_quality_gate` must validate `EvidenceRecord` completeness before report generation, not discover gaps manually after rendering.
