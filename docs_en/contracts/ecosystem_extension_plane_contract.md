...
It extends [tool_skill_plugin_contract.md](./tool_skill_plugin_contract.md), used to answer "how external extensions are securely integrated, registered, published, upgraded, disabled, and rolled back".
...
- Provide clear contract boundaries for `M2-EXT-01`'s `Knowledge Plane / Artifact Plane / Plugin SPI / Domain Registry`.
...
- Domain bundle binding is the authoritative entry point for exposing capability to a specific domain.
...
- `containerized_process` represents a launcher-based external isolation runtime interface, which can be carried by `docker` / `podman` / `bwrap` or equivalent independent sandbox launcher; communication between host and child is via stdio JSON protocol.
...
- Isolated failure can set plugin to `degraded` or `disabled`, with an optional cooldown window; cooldown status must be queryable by inventory, diagnostics, or API.
- If `forked_process`, `sandboxed_process`, or `containerized_process` is enabled, runtime process id should be queryable by inventory, diagnostics, or API, and the host process must be able to reap child processes on unload / shutdown.
- If `sandboxed_process` or `containerized_process` is enabled, runtime sandbox root should also be queryable by inventory, diagnostics, or API, for operators to do isolation root directory audits.
- Plugin invocation should publish at minimum `plugin:invocation_started` and `plugin:invocation_completed` typed audit events for consumption by audit and feedback projections.
- SPI registration results must be queryable by inventory, diagnostics, and audit systems.
- Plugins can only interact with core via public SDK surface, and must not reach into private implementations.
...
- `api_surface_contract.md`, `admin_console_and_human_takeover_contract.md` handle extension plane management entry points.
...
- Domain bundle, plugin SPI, marketplace can exist as design boundaries, but should not be stated as fully operational production plane
...
Therefore, this contract primarily bears the governance definition of target-state extension plane; current readiness can only treat it as a boundary document, not a completed delivery proof.