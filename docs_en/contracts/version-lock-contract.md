...
Each `HarnessRun` freezes `RunVersionLock` when admitted. Configuration releases during runtime must not change the semantics of completed runs; new versions can only be used via explicit GraphPatch, OperationalDirective, redrive, or new HarnessRun.
...
- After admitted, `RunVersionLock` is append-only and must not be overwritten in place.
...
- Re-execution Replay products must enter isolated namespace and must not overwrite original artifact locks.
...
- Configuration releases must not change active `HarnessRun` lock.
- GraphPatch lock conflicts must be rejected, re-validated, or restarted according to strategy.
- Artifact hash changes must be detected as new version.
...
- T-4: Only 3 locking strategies supported (pinned/floating/range), architecture §22.4 defines 4 including digest-locked. Fix: This semantics converges to v4.3 canonical contract; old fields, old state, old DTOs, or old terminology are only allowed as legacy/deprecated/projection/migration input and must not serve as new implementation entry points.

Mandatory rules: State migration must go through `RuntimeStateMachine.transition(command)`; execution plans must use `PlanGraphBundle`; execution results must use `NodeAttemptReceipt`; truth events can only use `platform.*`; OAPEFLIR can only be used as `oapeflir.view.*` / rationale projection; budgets must use `BudgetLedger` / `BudgetReservation` / `BudgetSettlement`.