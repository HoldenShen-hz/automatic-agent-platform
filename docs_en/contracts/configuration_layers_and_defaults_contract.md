...
## 3. Four-Type Responsibility Separation
...
- Runtime strong constraints must not be written only in prompts.
...
| --- | --- | --- |
| `bootstrap` | `config/bootstrap/default.json` | Application identifier, phase declaration, feature switches |
...
- The system generates `configVersion` by taking the first 16 characters of SHA256 from deterministic JSON serialization of the bundle.
...
| `runtime` | `defaultTaskTimeoutMs` | Must be positive |
| `runtime` | `defaultStepTimeoutMs` | Must be positive |
| `runtime` | `maxConcurrentTasks` | Must be positive integer |
| `security` | `sandboxMode` | Must be one of `read_only | workspace_write | danger_full_access` |
| `security` | `remoteWorkerRegistration.challengeTtlMs` | Must be positive |
| `security` | `remoteWorkerRegistration.allowedCapabilities` | Must be non-empty string array |
...
| `knowledge` | namespace / trust tier | Must satisfy enumeration and boundary constraints |
...
Configuration files support `//` line comments, `/* */` block comments, and trailing commas. Strip comments before JSON parsing during processing.

### 3A.4 Sandbox Path Constraints

Configuration file loading paths must be within the config root directory; reading files outside the config directory via `../` path traversal is prohibited.

## 4. Defaults Registry

At minimum uniformly manages:
...
Metadata involving model selection, budget, context limits, modalities, and provider authentication methods must not be scattered as hardcoded at call sites.

At minimum should have unified registry managing:
...
- Remote refresh is reserved for future extension (`remote_refresh`).
- When local file does not exist, silently falls back to built-in snapshot without error.

Rules:

- Registry can support local snapshots, offline use, and remote refresh, but authoritative field shapes must be stable.
- Runtime must not use string contains checks to replace formal capability metadata unless belonging to short-term compatibility layer.
...
The biggest risk of configuration system is not "too many configuration items" but defaults, prompts, YAML, and policies vying for power; this contract is to lock down their hierarchy.
