...
- Runtime hard constraints must not be written only into prompt.
...
| --- | --- | --- |
| `bootstrap` | `config/bootstrap/default.json` | Application identification, phase stage declaration, feature switches |
...
| `workflows` | `config/workflows/default.json` | Workflow definition and default step templates |

### 3A.1 Configuration Version

- The system generates `configVersion` by taking the first 16 characters of SHA256 after deterministic JSON serialization of the bundle.
- `configVersion` is used for tamper detection: if the runtime bundle recalculates a version inconsistent with the recorded version, doctor should report `config.version_tampered`.
...
| `providers` | provider / profile reference | Must have a matching entry in model metadata registry |
...
| `kvcache` | budget partition | Sum of fixed/domain/variable three-segment budget must be explainable |
| Production environment | `allowDestructiveActions` | Must not be `true` (fail-closed) |

### 3A.3 JSONC Support

Configuration files support `//` line comments, `/* */` block comments, and trailing commas. Parse by stripping comments before JSON parse.

### 3A.3A Schema Carrier

- "versioned schema" refers to authoritative and versioned configuration structure constraints; it does not mandate that it must be delivered as inline `$schema` or separate `*.schema.json` file.
- Using authoritative executable schema / validator in code is allowed, as long as it is version-managed together with `configSchemaVersion` and strongly validated when bundle is loaded.
...
Metadata involving model selection, budget, context limits, modalities, and provider authentication methods must not be hardcoded at call sites.

The unified registry should at least manage:
...
- When local file does not exist, silently fallback to built-in snapshot without error.

Rules:

- Registry can support local snapshots, offline use, and remote refresh, but authoritative field shapes must be stable.
- Runtime must not use string contains to replace formal capability metadata unless it belongs to a short-term compatibility layer.
- UI, CLI, server, policy, budget, and provider routing should prioritize consuming unified registry rather than each maintaining their own model lists.

## 6. Closure Conclusion

The biggest risk in configuration system is not "too many configuration items", but defaults, prompts, YAML, and policies competing for authority; this contract hardcodes their hierarchy.