...
It answers the question: Before entering staging, pre-prod, or prod, how does the system uniformly record whether external dependencies like providers, gateways, sandboxes, worker fleets, and artifact stores are ready.
...
- Unify modeling of credential, secondary gates, owner, and last verified time.
...
- When secondary gate fails, should block corresponding capability, not just log warning.
- When `last_verified_at` is stale, system can degrade readiness to `stale` and trigger review.
