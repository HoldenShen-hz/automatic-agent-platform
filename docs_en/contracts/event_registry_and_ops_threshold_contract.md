...
> **OAPEFLIR Related**: This contract defines the event registry for OAPEFLIR 8-stage pipeline, corresponding to ADR-016/ADR-079/ADR-080.
> **Last Updated**: 2026-04-17

## 1. Scope

This contract further freezes the event registry, consumer relationships, and operational thresholds for the current phase on top of `event_reliability_matrix_contract.md`.
...
- Who produces each event type, who consumes it, and whether ack is required.
- When backlog or loss becomes an operational alert.
...
- Tier 2 / 3 even if not mandating ack, must clarify usage scenarios to avoid event semantic drift.
...
Standard Tier 1 consumers include at least:
...
- One consumer's failure must not overwrite other consumers' acknowledged results.
- When adding a new Tier 1 consumer, must simultaneously evaluate startup scan and ack thresholds.
...
Current Tier 2 events' primary consumer is `inspect_projection`, used for maintaining structured projections for inspect / diagnostics / timeline.
...
- **skill events** (`skill:*`): Produced by `skill_execution_service`, projected to skill execution observability chain. Covers full skill lifecycle: start, cache hit/miss/store, step start/success/failure, retry scheduled, execution completed.

Rules:

- Tier 2 may skip persistent ack, but if undertaking critical projection functionality, should explicitly declare degradation strategy in implementation.

### 5.3 Tier 3 Consumers

- No persistent ack
- Must not impersonate recoverable factual source
...
| --- | --- | --- |
| Tier 1 unacknowledged event count | `> 0` for `5m` | Alert |
| Single consumer Tier 1 backlog | `>= 20` | Alert and trigger recovery scan |
| Tier 2 backlog | `>= 100` for `10m` | Optional alert |
| Tier 3 loss | Not individually alerted | Monitor trends only |
...
| --- | --- | --- |
| Tier 1 ack continuous failure | `>= 3` times | Mark `degraded` and enter recovery |
| Same event replay count | `>= 5` times | Human intervention |
| Single consumer long-term inactivity | `> 10m` | Suspend registration or degrade projection |
...
- Tier 2 may skip `event_consumer_acks`, but if undertaking critical projection in the future, should upgrade to Tier 1.
- Tier 3 must not impersonate recoverable factual source.

## 8. Startup Scan Linkage

Startup scan minimum checks:

- Whether Tier 1 events exceeding threshold are unacknowledged
- Whether event types produced but not registered in the registry exist
- Whether events with inconsistent consumer state and registry exist
...
Whether the event system is reliable depends not only on "whether events are sent", but on having a stable registry that clarifies who should receive, how long they should receive, and how the system will react when nothing is received.