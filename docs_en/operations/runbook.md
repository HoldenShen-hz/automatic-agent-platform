...
> Note: This runbook contains industrial-grade target operations items; items related to `canary / blue-green` correspond to subsequent industrial-grade release paths, and do not indicate that current phase1-4 authoritative release level supports running levels beyond `off / suggest / shadow`.

## 1. API Latency Breach
- Symptoms: `aa_http_request_duration_seconds` P95/P99 exceeds alert threshold, `/healthz` degrades, user requests timeout.
