...
> Source Material: Anthropic "The Founder's Playbook: Building an AI-Native Startup" PDF, 36 pages; official source at [Claude Blog原文](https://claude.com/blog/the-founders-playbook)  
...
| Source Traceability only has PDF page numbers, lacks official source fixed rules | PDF revision后会 cause source mapping drift | Added official source to header, preserving page number confidence level and requiring sourceRef复核 when formally merging |
...
| Proposals, patch plans, implementation tracks mixed together | Readers cannot determine "what can be used now" | Clearly separated "proposal, main baseline patch, implementation track"三层口径; remaining content still retained as design input |
...
The core of Anthropic's Playbook is not "startup advice," but a clear **AI-native operating model**:
...
> We should not just treat Agent as an "task executor," but upgrade the system to an **AI-native Mission Operating System**: each Mission has stages, exit criteria, failure modes, evidence requirements, metric measurement, Skill orchestration, HITL decisions, and learning/improvement loops.
...
2. Proposed Metrics in this document cannot be the sole alerting basis before accepted into main Metric Registry.
...
Release decision is in two layers:

| Layer | Current Status | Description |
|---|---|---|
| Reference Document Review | reviewed | This document's structure, Registry Patch, Gate/Metric/Event/Runbook/CI/Evidence Bundle integration relationships can serve as review input. |
| Main Baseline Merge | repo closure passed / main acceptance pending | Repository has executed Registry Closure, Markdown Render Closure, and Track B/C/D validation commands; formal main baseline acceptance still needs to be incorporated into corresponding validation profile. |
| Runtime enforcement | pending accepted registry | Only after main Registry accepts and enables validation profile can new Gates become release blockers. |

Before formally merging into main baseline, must form Release Evidence Bundle:
...
The following commands were originally the **closure command contract** suggested in this document. The 2026-05-21 repository baseline implementation has written command entries into `package.json`, generated by `scripts/validation/mission-operating-model-closure.mjs`; they can serve as registry patch evidence, but whether the main Validation Baseline promotes these gates to release blockers still requires separate acceptance:
...
After repository entry, three more types of evidence still need to be added:
...
After passing, the status in main baseline can be updated from:
...
### 0.3 Authority Boundary and Implementation Gap in Current Repository

This document is located in `docs_zh/reference/`, and its positioning is reference design input. If this document conflicts with current platform authority objects, handle according to the following table:

| Topic | Current Authority | What This Document Allows | What This Document Must Not Do |
|---|---|---|---|
| Mission positioning | `docs_zh/reference/mission_architecture_design_review_v1_4_full_merged.md` and existing Mission contracts | Provide Mission Playbook/Stage governance candidates for Mission | Make MissionStage into a second execution state machine alongside Harness/Node |
| Execution main chain | `PlanGraphBundle`, `HarnessRun`, `NodeRun`, `NodeAttempt` canonical runtime | Provide business stage gate, outcome, evidence constraints | Resurrect Step-centric or linear workflow truth |
| Validation baseline | Rules in `docs_zh/reference/automatic_agent_platform_validation_monitoring_full_v1_7_1.md` Registry closure rules | Provide Gate/Metric/Event/Runbook/CI patch to be merged | Claim proposed registry already accepted |
| Machine contracts | Zod/OpenAPI/Event schema/CI tests | Give candidate schema and command inventory | Only rely on Markdown text as release blocker |

Current repository review baseline:

| Object | Current Judgment |
|---|---|
| `MissionPlaybookRegistry`, `StageExitGateService` | Repository baseline implemented: Mission Playbook schema, stage instance schema, safe Exit Criterion DSL, Playbook reference validation, snapshot-based StageExit decision, and `platform.mission.stage_exit_evaluated` audit event; still not a main Validation Baseline release blocker |
| `MissionFailureModeRegistry`, `MissionOutcomeMeasurementService` | Track B repository baseline implemented: failure mode registration, P0 routing requirements, dedupe/suppression approval, Mission outcome layered score and outcome event audit have referenceable services and targeted tests |
| `WorkflowRecordingService`, `SkillCandidatePipeline` | Track C repository baseline implemented: recording policy, restricted consent/redaction fail-closed, retention deletion proof, Trace→Candidate→SkillPack approval/eval/policy/signature/canary/rollback gate have testable services |
| Research / Code / Ops Playbook | Track D repository baseline implemented: three active builtin playbooks contain stage evidence, default skills, HITL edge and gate refs; still should not mistakenly write Mission Stage as execution truth |
| `playbook:validate`, `test:e2e:stage-exit`, `mission-outcome:validate`, `skill-candidate:validate`, `skillpack:validate`, `workflow-recording:*` | Become repository script entries, with `config/validation/mission-operating-model-registry.json` and closure report forming registry patch evidence |
| Mission/Task/Session boundaries, PlanGraph DAG, Evidence/Policy/Budget/HITL constraints | Should continue to reuse existing platform canonical design, not start another set of factual models in this document |
...
| Validation registry patch, closure commands, and report generators | `config/validation/mission-operating-model-registry.json`, `scripts/validation/mission-operating-model-closure.mjs`, `package.json` | `npm run registry:closure`, `npm run playbook:validate`, `npm run mission-outcome:validate`, `npm run skill-candidate:validate`, `npm run skillpack:validate`, `npm run workflow-recording:*` |

This baseline deliberately does not do three things:

1. Does not persist `MissionStageInstance` as a second execution truth; it is currently a Mission governance contract, and future persistence must go through Mission truth/event design review.
2. Does not directly modify `HarnessRun`, `PlanGraphBundle`, or `NodeRun` state through StageExit; the execution plane continues to be controlled by canonical runtime.
3. Does not prematurely declare that main Validation Baseline has promoted Gate/Metric/Runbook/CI Registry to release blockers; this round only puts registry patch, commands, and closure artifacts into the repository.

---

## 1. Core Content Abstraction from the Playbook

### 1.1 Four-Stage Lifecycle

The Playbook divides AI-native startup into four stages:
...
| Scale | Can the organization and product be audited, sustainable, and hard to replicate? | governance, SLA, support infra, GTM system, data/workflow moat | systematized growth, governance maturity, moats established |
...
### 1.2 Founder Transitions from Individual Contributor to Orchestrator

The Playbook clearly states that in an AI-native startup, the founder's role shifts from personally writing code, operating, and managing daily tasks to being an **orchestrator of agents**. This exactly corresponds to our system's:
...
This again supports the direction in system design to "de-emphasize step concept": users should not face Step, but Mission / Stage / Outcome / Evidence.

---

### 1.3 The Key to AI-Native is Not Faster Execution, but Evidence-Based Execution

The Playbook repeatedly emphasizes in the Idea stage: AI lowers the build barrier, but cannot mistake "can build" for "already validated." In the MVP stage, it also emphasizes that AI coding accelerates but creates technical debt, requiring architecture context, scope documentation, security review, and measurement framework.

Corresponding to Automatic Agent Platform:

```text
AI-native ≠ skip process
AI-native = automatic execution + automatic evidence + automatic evaluation + automatic feedback + human handling high-value judgments
```
...
no policy decision, not allowed to side effect
no budget reservation, not allowed to model/tool call
no audit trail, not allowed to HITL / autonomy override
...
> Note: Page/section ranges are manually calibrated based on current PDF table of contents and paragraph structure, `sourceRefConfidence=medium`; should do final page number review with document owner before formal merge.

> Page ranges reference the uploaded 36-page PDF for internal traceability. If PDF is revised later, regenerate `sourceRef` for this table.

> Translation boundary: The "platform objects / Gates / Registries" in the table below are Automatic Agent Platform's architectural derivations and do not represent Anthropic originally defined these objects. If later citing original text for product or market conclusions, should go back to official original text for review, should not reversely use this table as a substitute for the original text.

| Playbook Source | PDF Section / Page Range | sourceRefConfidence | Original Topic / Design Intent | Our Abstraction | Platform Object | Corresponding Gate / Registry |
|---|---|---|---|---|---|---|
| Startup lifecycle rebooted | Ch.1 / p.3-4 | medium | Idea / MVP / Launch / Scale redefined AI-native startup lifecycle | Mission Stage Model | MissionPlaybook, MissionStageGraph | GATE-MISSION-PLAYBOOK-001 |
| Founder as orchestrator | Ch.2 / p.5-7 | medium | founder transitions from individual contributor to agent orchestrator | Mission Owner / Operator | Native Surface Matrix, Operator Cockpit | UI Authorization Matrix / Runbook Registry |
...
| MVP Stage | Ch.4 / p.15-20 | medium | Scope, architecture context, measurement framework | Mission cannot only produce output, must also be measurable | MissionOutcomeMeasurementService | GATE-MISSION-OUTCOME-001 |
...
| Launch Stage | Ch.5 / p.21-24 | medium | Security, compliance, production hardening | Governance cannot be an afterthought | Policy, Data Governance, SideEffect | GATE-DATA-001 / GATE-SIDEEFFECT-001 |
...
| Scale Stage | Ch.6 / p.25-30 | medium | Auditable governance, organizational maturity, data flywheel | platform moats come from workflow/evidence/knowledge compound interest | Knowledge Graph, Skill Catalog, MissionOutcome | Metric / Evidence / Knowledge Registry |

---

## 3. Core Inspirations for Automatic Agent Platform

### 3.1 Mission Must Be Staged

Existing Mission should not just be a long-term container. It should have:
...
Stage is business lifecycle state; NodeRun is the execution-time node; Task is the work unit under Mission.
```
...
Must clarify the different semantics of two "graphs":

| Graph | Loop Allowed | Purpose | Constraint |
|---|---:|---|---|
| MissionStageGraph | Controlled loops allowed | Express business lifecycle, e.g., monitor → improve → review | Each loop must create new `stageInstanceId`, constrained by maxCycle / dwellTime / budget / approval guard |
| PlanGraphBundle | No loops, must be DAG | Express one HarnessRun's execution plan | Must pass DAG validation, dependency validation, worst-path budget validation |
...
Edge-level guard distinguishes risk across different stage edges. For example, `validate → publish` should be stricter than `monitor → improve`. All stage transitions must first check edge guard, then evaluate exit criteria.
...
All Playbook DSL, StageEdgeGuard, Policy Action, and Native Surface Matrix must reuse the same canonical enum set to avoid RiskLevel / RuntimeMode appearing in multiple places with conflicting sort orders.
```
...
In v1.0, `rolled_back` was defined as a long-term status. v1.1 corrected: rollback is a transition result, not a stable long-term status.
...
Rollback completion is expressed by recovery/migration process through independent Runbook or MigrationPlan; StageExit itself only produces `rollback_requested` and does not define `mission.stage.rollback_completed`, avoiding misinterpreting rollback completion as a synchronous result of StageExit.
...
This introduces risks: arbitrary code execution, unanalyzable statically, unauditable, unreplayable, etc. v1.1 changed to safe DSL.
...
```text
No arbitrary JS / Python / shell execution.
No network IO.
No reading of real-time mutable state.
Can only read metric/evidence/risk/budget/HITL snapshots bound in StageExitGateInput.
All metric / event / evidence kind must exist in Registry.
Expression must be statically verifiable, serializable, replayable.
```

#### 3.4.1 ExitCriterion Snapshot Binding and Validator

ExitCriterion can only be computed based on immutable snapshot in StageExit input. Each computation must record actual value, expected value, expression hash, and snapshot reference to ensure later replay is explainable.
...
| window | Must use `BoundedTimeWindow`, no bare string or infinite window |
| not | P0 gate does not allow bare `not` wrapping external event absence |
| recursive depth | DSL nesting depth must be constrained to prevent DoS |

`BoundedTimeWindow` additional constraints:

```text
stage window's maxDurationMs must be explicitly provided; active stage does not allow unlimited window.
duration window's maxDurationMs must be less than or equal to MissionProfile.maxExitEvaluationWindowMs.
mission window only allows reading current mission's immutable snapshot, no cross-mission aggregation.
```
...
The Playbook gives common failure modes at each stage. We should systematize this:
...
The Playbook mentions that MVP stage should establish measurement framework early to avoid mistaking early momentum for PMF. For us, Agent completing a task does not equal Mission success.
...
type MissionOutcomeScores = {
  ...
  // Phase 1 does not directly handwrite abstract businessImpactScore;
  // business impact is obtained by aggregating BusinessImpactObservation.
...
```text
Mission success = output adopted / evidence complete / cost controlled / risk controlled / can沉淀 improve.
Not just "ran through."
MissionOutcomeScores is standardized result; MissionOutcomeReport.quality/evidence/cost/latency/risk are raw score details.
Metric Registry only collects standardized fields of MissionOutcomeScores, and traces back to raw sources through scoresGeneratedFromRefs.
```
...
The Playbook emphasizes that operational workflows can be taken over by AI, and in the Scale stage, founder's tacit knowledge becomes a reusable system. We can implement this as:
...
```text
Recording workflow must not directly become production Skill.
...
approved → rejected       # Review found risk
```
...
### 4.1 All Three Need Lifecycles, but Cannot Share One Lifecycle

```text
Mission lifecycle = business goal / long-term plan / stage progression
Task lifecycle    = executable work item / queue execution / block recovery
Session lifecycle = human-machine interaction context / conversation window / approval context
```

| Object | Lifecycle Type | Role |
|---|---|---|
| Mission | Business lifecycle + stage lifecycle | Manage long-term goals, stage progression, outcome沉淀 |
| Task | Work item lifecycle | Manage single executable task creation, execution, block, completion |
...
Stage advancement must be generated by `StageExitGateService` producing `StageExitDecision`; manual modification of `currentStageId` is prohibited.
...
Session is the interaction entry point, not the main axis of long-term lifecycle. Session can create Task, but Task should not depend on Session to survive.
...
```text
INV-LIFECYCLE-001:
Mission, Task, Session must have independent lifecycles and cannot share the same status enum.
...
INV-LIFECYCLE-007:
Cross-layer state propagation must go through events and decision services; direct mutation of upper-layer objects is not allowed.

INV-LIFECYCLE-008:
Session memory is not Truth and cannot serve as Mission/Task authority state source.
```
...
```text
Manage stage graphs, default skills, exit criteria, failure modes, measurement frameworks, and operational boundaries for different MissionTypes.
```
...
MissionPlaybook itself must be governed as a configuration/policy object, not statically referenced as a regular Markdown document.
...
canary playbook cannot serve as default playbook for all tenants.
revoked playbook cannot create new Missions.
...
Running Mission must not automatically drift to new playbook.
revoked playbook cannot create new Mission.
Existing Mission using revoked playbook enters hold / controlled migration.
canary playbook can only be hit through explicit rollout policy.
No available playbook means fail-closed; must not fall back to running without playbook.
`use_last_active` only allowed when all conditions are met: last active playbook not revoked/suspended/archived; compatibility still passes; tenant still authorized; rollback/migration policy allows; and generates resolution auditRef. Otherwise must `fail_closed`.
...
```text
Must not directly replace running Mission's playbookVersion.
Incompatible migration must have HITL approval.
Must generate compatibilityReportRef before migration.
Must retain original playbookVersion after migration for replay.
```
...
```text
Before each MissionStage ends, read playbook, metrics, evidence, risk, budget, HITL decisions to determine whether to allow entering the next stage.
```
...
All metric/evidence/risk/budget/HITL inputs must come from immutable snapshot.
input.playbookId / input.playbookVersion must exactly match Mission locked version, StageInstance version.
stage held re-evaluate must provide remediationRef or approved hitlDecisionRef.
...
| Scenario | Behavior |
|---|---|
| Retry with same `idempotencyKey` | Return original decision, do not recreate stage/event |
...
| rollback/advance concurrency | CAS allows only one success, the other returns version conflict |
...
`mapDecisionToStageStatus` must be implemented according to the following table; must not re-add `require_hitl` to `MissionStageStatus`:
...
| `require_hitl` | `held` | `requiredActions` must include `hitl_required` | `mission.stage.held` |
...
`StageExitDecision` is a business decision object; `EventEnvelope` / event payload is an event fact object. When appending events, must supplement `sequence / correlationId / causationId`; must not directly use bare decision as complete event payload.

Prohibited items:

```text
Prohibited: StageExitDecision written first, stage state written asynchronously later (separated transactions).
Prohibited: Advance stage first, supplement decision asynchronously later.
Prohibited: Only complete current stage without atomic nextStageInstance creation.
Prohibited: mission.currentStageInstanceId and stage truth updated separately.
Prohibited: Concurrent evaluate without locking stage version.
Prohibited: Reading exit metric directly from mutable dashboard current value.
Event order in same transaction must be: mission.stage.exit_evaluated → mission.stage.advanced / mission.stage.held / mission.stage.rollback_requested / mission.stage.terminated, with monotonic sequence persisted.
```
...
```text
Turn common failure modes into first-class objects that are detectable, governable, and learnable on the platform.
```
...
FailureMode suppression must have owner, expiry, auditRef; must not suppress P0 gate failure.
Same dedupeKey triggers repeatedly within recurrenceWindow, should escalate recurrence signal instead of spamming alerts.
```
...
Output objects are in §3.6. Outcome service must support:
...
MissionOutcome must distinguish execution quality, content quality, evidence quality, adoption, and business impact; cannot use one `quality_score` to cover everything. Complete data model takes §3.6 as the only source; §5.4 only describes service responsibilities, input/output, measurement window, and delayed observation to avoid Outcome type duplication.

`MissionOutcomeScores` is standardized result; `MissionOutcomeReport.quality`, `MissionOutcomeReport.evidence`, `MissionOutcomeReport.cost`, etc. are raw details; Metric Registry only collects standardized fields, and all score sources must trace back through `scoresGeneratedFromRefs`.

Layered explanation:

| Layer | Meaning | Immediately Available |
|---|---|---:|
| Execution Success | Whether system completed run, whether no P0 gate failure | Yes |
| Immediate Outcome | Whether output is readable, evidence complete, quality score meets threshold | Yes |
| Delayed Outcome | Whether adopted after 7d/30d, whether downstream tasks generated | No |
| Business Impact | Whether saved manpower, generated experiments, supported decisions | No |
...
Workflow Recording must have independent data governance boundaries. Browser / IDE / desktop recording especially cannot default to capturing complete DOM, screen, or request body.
...
Production environment defaults to not recording request body / response body.
Full DOM capture can only be enabled under sandbox / internal tenant / explicit consent / short retention.
DOM persistence must generate redactionReportRef before.
redactionReportRef must go into WorkflowTrace.
Expired recording must be deleted by retention sweeper, and deletion proof generated; deletion failure must enter Runbook D.40.
Trace containing credential / secret / regulated_pii must fail-closed and must not enter SkillCandidateGenerator.
```
...
```text
Mission:
  Long-term goal and business stage container. Must bind MissionPlaybook.

MissionStage:
  Business stage in Mission lifecycle. Not Step, does not directly execute tools.

Task:
  Manageable work unit under MissionStage. Can be created by humans, or derived automatically by StageExitGate or Playbook.

Session:
  Human-machine interaction context. Not an authority execution object and should not carry long-term truth.

PlanGraphBundle:
  Execution plan for Task/HarnessRun. Must be DAG; linear PlanStep[] not allowed.

HarnessRun:
  One controlled execution run, responsible for budget, risk, evidence, state machine, scheduling.

NodeRun:
  Executable node instance in PlanGraph.

NodeAttempt:
  One attempt of NodeRun, records tool calls, model calls, errors, and receipts.
...
```text
MissionStage determines "what business stage we are currently in, what the goal is, what the exit criteria are".
OAPEFLIR determines "how does Agent observe, assess, plan, execute, feedback, learn, improve, release to complete current stage tasks".
```
...
| monitor | Observe / Feedback | Monitoring new papers, new data, metric changes |
...
Anthropic Playbook distinguishes different surfaces like Chat / Claude Cowork / Claude Code. Our platform should also clarify entry points, allowed actions, Policy Action, and backend object relationships.

| Surface | Suitable Tasks | Backend Object | Risk Boundary |
|---|---|---|---|
| Session Chat | Quick Q&A, requirement clarification, single consultation | Session only / Task optional | Defaults to no side effect |
...
| Browser Extension | Workflow recording, web evidence capture | WorkflowTrace / Evidence | Defaults to read-only; writes must HITL |
| Mobile | Approval, alerts, lightweight review | HITL / Incident / Notification | No high-risk config modifications |
...
```text
Native Surface must not directly write Truth Store.
Native Surface must not bypass Interface Gateway / Policy Engine / Tool Registry.
Browser Extension defaults to read-only; write operations must go through Gateway + HITL + SideEffectRecord.
...
## 9. Recommended Chapters to纳入 v2.0 Validation Baseline after Review
...
> The following Gates are pending review as v2.0 Gate Registry patch. If not yet merged into main document, should be marked as `proposed_for_v2_0` and not used as already-frozen Gates in the main text.
...
| GATE-MISSION-OUTCOME-001 | P1 | P0 if final release missing outcome | Mission completed without outcome measurement | mission-outcome-validate | D.38 |
...
| GATE-MISSION-PLAYBOOK-007 | P0 | none | Running mission automatically drifted to new playbookVersion without migration | playbook-validate | D.35 |
...
| GATE-SKILLPACK-001 | P0 | none | SkillPack activated without manifest/policy/SBOM/eval/signature/canary/rollback | skillpack-validate | D.39B |
...
```text
GATE-SKILL-CANDIDATE-001 blocks SkillCandidate definition incomplete, occurs before creation / review.
GATE-SKILL-CANDIDATE-002 blocks SkillCandidate unsafe conversion, occurs before approved → converted_to_skillpack.
GATE-SKILLPACK-001 blocks SkillPack unsafe activation, occurs before canary / active.
```

`GATE-QUALITY-001` in v1.0 example has been removed; no unregistered Gate is used. Research quality is uniformly covered by combination of `GATE-MISSION-OUTCOME-001`, `GATE-EVIDENCE-001`, `GATE-TEST-003`.

---

### 9.2 New Metric Registry Entries

> The following metrics are pending review as v2.0 Metric Registry patch. Formal definition should include name, type, formula, window, labels, source, dashboard, alert, owner, target.
...
Metric Alert structured requirement:
...
P0/P1 mixed expressions must not be written only in human-readable text; when merging into main Metric Registry, must be converted to `defaultSeverity + escalationRules` and auto-parsable by alert/router; plain natural language thresholds are prohibited.

> Note: The `Alert` column in the above table is a human-readable summary. When merging into main Metric Registry, must generate machine-readable `metric-alert-policy.yaml` and write this file reference into Release Evidence Bundle, where each P0/P1 metric uses `defaultSeverity + escalationRules` expression; plain natural language threshold is prohibited.

---

### 9.3 New Event Registry Entries

> The following events are pending review as v2.0 Event Registry patch. Formal registry needs payload schema files, producer/consumer, compatibility policy, replay behavior.
...
```text
mission.stage.exit_evaluated must precede mission.stage.advanced / held / rollback_requested / terminated written in same transaction.
...
StageExitDecision and EventEnvelope/EventPayload boundary must be clear: Decision is not directly equivalent to event; when appending event, EventAppender must supplement sequence, correlationId, causationId.
criterionResults must include actualValue / expectedValue / expressionHash / snapshotRefs to ensure replay explainability.
```
...
CI mode structured requirement:
...
The same physical CI job can cover multiple gates, but when entering main CI Job Registry must expand to gate-level mode; using mixed text like "observe_only for X; enforcing for Y" that cannot be machine-parsed is prohibited.

---

## 10. v2.0 Baseline Integration Patch Plan

This section defines how this document merges into main `v2.0 Validation Baseline` after passing review. Before merging, all registry entries in this document remain `proposed_for_v2_0`; current repository should not prematurely declare these registry entries accepted just because this section exists.

| Main Document Location | Patch Content | Closure Check |
|---|---|---|
...
| Runbook Registry | Add D.35-D.40 | Each P0/P1 Gate bound to Runbook |
...
```text
1. Main document adds new chapter: AI-Native Mission Operating Model Validation.
2. This document as appendix: Appendix X — Anthropic Founder's Playbook Mapping.
3. Gate/Metric/Event/Runbook/CI entries merge into main Registry.
...
After candidate commands are in repository, main baseline merge process should execute:
...
|---|---|---|
| Pressure-test hypothesis | hypothesis-adversarial-review | Do counterexample, boundary, cost-benefit analysis on research/product hypotheses |
| Competitor landscape | competitive-research | Collect competitor, open-source solutions, pros/cons, reusable points |
...
| MVP scope definition | scope-boundary-builder | Generate minimum scope, non-goals, risk boundaries, deferral items |
...
| Workflow audit | workflow-automation-candidate-mining | Mine automatable workflows, generate SkillCandidate |
...
SkillPack entries should be supplemented:
...
| skill improvement suggestion | learning object | passed eval / HITL / rollout | Can generate SkillCandidate |
...
This stage set suits startup lifecycle, but our platform is a general-purpose Agent platform and should be abstracted as:
...
### 12.2 Do Not Interpret AI-Native as Canceling Governance

The Playbook repeatedly explains that AI lowers execution cost, but更容易导致错误加速, technical debt, scope creep, false signal, security exposure.

We must reverse strengthen:
...
Workflows recorded can only be candidates and must go through:
...
MissionPlaybook must be machine-readable, verifiable, versionable objects:
...
|---|---|---|---|---|---|
| A: Minimum Viable Implementation | Orchestration Owner | Mission / Task / Session lifecycle; Event Registry | Research Mission can advance by stages, StageExitDecision transaction closed | disable StageExit enforcement, retain locked playbook read-only, stop new stage transition; existing Mission can only complete via legacy Task flow after operator approval | playbook model, DSL validator, stage-exit report |
| B: Failure Mode and Outcome Closure | Reliability Owner + Quality Owner | Track A; Metric Registry; Runbook Registry | unsupported claim detectable, OutcomeReport generatable | close detector enforcement, retain metrics observe-only | failure mode pack, outcome report, runbook mapping |
...
| D: Extend to Code Agent / Ops Mission | Domain Owners | Track A/B/C; Code/Ops playbooks | Code/Ops Mission has independent playbook, stage gate, failure mode pack | rollback to Research-only playbook enablement | code playbook, ops playbook, SLO profiles |
...
Principle: Proposed gates first collect metrics and false positives in observe_only mode; only after Runbook, Metric, Event, CI, Owner fully closed can they enter enforcing.

### Implementation Track A: Minimum Viable Implementation

Goal: Enable Research Intelligence Mission to support staged operation.
...
### Implementation Track B: Failure Mode and Outcome Closure
...
quality score not meeting threshold must hold stage.
...
```text
Repeated human workflows can generate SkillCandidate.
...
| MissionStage | Add MissionStageInstance, cycleIndex, version; expectedVersion only for transition command | P0 |
...
The maximum value of Anthropic's Playbook for us is abstracting AI-native as a "staged, evidence-based, measurable, orchestratable" operating system.

For Automatic Agent Platform, absorb as:
...
Mission is a business operating system unit with playbook, stages, exit criteria, failure modes, outcome measurement, and learning loops.
```

v1.6.2 reviewed and revised recommendation status:

```text
Document Status: reviewed-reference / architecture-patch-candidate
Recommended Next Step: First implement the proposed closure commands, artifacts, CI jobs, and registry patches listed in Release Evidence into the main repository; after passing Gate/Metric/Event/Runbook/CI/Evidence Bundle/Markdown Render closure, merge as new chapter + appendix into main document as v2.0 Validation Baseline, and change registryStatus to accepted.
```

Priority implementation order:

```text
1. Research Intelligence Mission's MissionPlaybookRegistry + StageExitGateService.
2. Mission / Task / Session Lifecycle Alignment written into main architecture document and contract.
3. FailureModeRegistry connected with Gate/Metric/Runbook.
4. UI Cockpit adds MissionStage Timeline and Exit Criteria panel.
5. Workflow-to-Skill Candidate enters observe_only / internal enforcing in Track C; full production deployment must wait for SkillPack activation gate, Workflow Recording policy/data-boundary/retention gates, Runbook D.39/D.40 all closed.
```

This will upgrade Automatic Agent Platform from "executable Agent platform" to:

> **AI-native Mission Operating System**  
> An Agent ecosystem that can define goals, verify stages, execute tasks, collect evidence, measure results, identify failures,沉淀 skills, and continuously improve.