...
> **Related ADRs**: [ADR-016 OAPEFLIR Eight-Stage Model](../adr/016-oapeflir-loop-model.md), [ADR-075 Six-Level Controlled Rollout](../adr/075-controlled-rollout-release.md)

## 1. Objective

Define AI Agent autonomy permission boundaries at each OAPEFLIR stage to ensure:
...
| **L1** | `shadow` | Read-only execution records, no state modification | Not required |
...
| **L5** | `stable_100` | Fully autonomous (constrained by guardrail) | Exception escalation only |
...
| **Assess** | Assessment computation | Assess + recommend | Assess + recommend + confirm | Fully autonomous |
...
- Do not bypass Plan DTO for direct execution (must use R3-NOBYPASS)
...
|--------------|-----------------|
| Bypass Plan execution | Task immediately terminated, `R3-NOBYPASS` violation logged |
| Execute L2+ without approval | Execute rollback, notify administrator |
