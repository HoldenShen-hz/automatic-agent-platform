...
| step | 步骤 | Single execution step in a workflow |
...
| artifact | 产物/制品 | File-type or binary product, usually managed through artifact store |
...
| result envelope | 结果信封 | Unified result encapsulation of success, partial success, failure, warning, artifact, and metrics |
...
| stage | 阶段 | Stage-level state unit in the OAPEFLIR closed loop |
...
| RolloutRecord | 发布记录 | Controlled release record in Release stage |
...
| English | Chinese | Explanation |
|---------|---------|-------------|
| runtime | 运行时 | The layer where the system actually executes task/workflow/agent/tool |
...
| sub-agent | 子代理 | Secondary intelligent execution unit collaborating in the same task context |
| heartbeat | 心跳 | Periodic health/load reporting |
| stalled | 停滞 | Process may not be dead, but no effective progress within specified time |
| dead-letter | 死信 | Dead-letter record for failures that cannot be auto-recovered or should not continue retrying |
| checkpoint | 检查点 | State snapshot at recoverable boundary |
| partial result | 部分结果 | Task not yet整体 completed, but has retainable and auditable phased results |
| compensation | 补偿 | Action to roll back, reconcile, or manually repair steps that have already occurred side effects |
...
| English | Chinese | Explanation |
|---------|---------|-------------|
| running | 运行中 | Workflow main logic being advanced |
...
| blocked | 阻塞 | Temporarily unable to continue due to unmet dependencies, approvals, policies, or resources |
...
| degraded | 降级 | Worker capability is limited but still working |
| unavailable | 不可用 | Worker currently cannot accept tasks |
...
| handed_over | 已交接 | Lease handed over to other worker |
...
| ack | 确认 | Record that a consumer has acknowledged handling an event |
...
| compaction_marker | 压缩标记 | Context compression marker fragment |
...
| compare_and_swap_write | 比较并交换写入 | Strategy to achieve compensation through CAS write |
...
| replay_from_start | 从头重放 | Strategy to replay from workflow start |
...
| lease_handover | 租约交接 | Lease was handed over to other execution entity |
...
| worker_quarantined | 工作器被隔离 | Worker in quarantine state |
...
| validated | 已验证 | Passed validation |
...
- **blocked**: Unable to continue due to dependencies/approval/policy/resources
...
| English | Chinese | Explanation |
|---------|---------|-------------|
| Authoritative Store | 权威存储 | Storage system that has final interpretation rights for a certain type of fact, the ultimate source of truth for data, must not be confused with any cache |
| Transaction Store | 事务存储 | Storage responsible for transactional data such as tasks, states, approvals, events; in code, transactional data is stored in AuthoritativeSqlDatabase |
...
| Namespace | 命名空间 | Logical namespace under data, artifact, or tenant boundaries, different from OS path |
...
| Authoritative Task Store | 权威任务存储 | Phase 1a task state authoritative storage, delegated to sqlite or postgres implementation |
| Phase 1a Store | Phase 1a 存储 | Task storage abstraction for initial stable core stage |
| Read-after-write Consistency | 写后读一致性 | Guarantee that reading immediately after writing can obtain the written result |
...
| Dead-letter | 死信 | Dead-letter record for failures that cannot be auto-recovered or should not continue retrying |
| Partial Result | 部分结果 | Task not yet整体 completed, but has retainable and auditable phased results |
| Checkpoint | 检查点 | State snapshot at recoverable boundary, different from arbitrary temporary variable |
...
| English | Chinese | Explanation |
|---------|---------|-------------|
| Config Bundle | 配置包 | A set of configurations that take effect together, including bootstrap, gateways, providers, runtime, security, workflows, etc. |
...
| Canary Rollout | 金丝雀发布 | Config release strategy that first runs small比例 then gradually expands |
| Rollback Point | 回滚点 | Snapshot record of recoverable config version |
...
| Compatibility Window | 兼容性窗口 | Formally supported compatibility time interval between different runtimes/SDKs/protocols/plugins |
| Promote Criteria | 晋升标准 | Evidence threshold for a module to move from available to platform-ready/production-ready |
| Readiness Registry | 就绪注册表 | Formal registration surface for recording environment or module readiness state |
| Evidence Package | 证据包 | A set of evidence used to support promote/signoff/production-ready judgments |
| Production-ready | 生产就绪 | Has reached the comprehensive threshold required for production backing |
| Phase 1a Ready | Phase 1a 就绪 | Minimum usable threshold for reaching Phase 1a stable core |
...
| English | Chinese | Explanation |
|---------|---------|-------------|
| Fixed Prefix | 固定前缀 | System prompt fixed prefix shared across agents, not participating in ordinary compaction by default |
...
| English | Chinese | Explanation |
|---------|---------|-------------|
| Policy Engine | 策略引擎 | Code-level entry point for final ruling on permissions, risk, approval, budget, and runtime constraints, unified security decision center |
...
| Break-Glass | 紧急放行 | High-risk emergency release config marker, critical risk triggers break-glass approval type, used for emergency bypass of standard process |
| Sandbox | 沙箱 | Execution isolation boundary, isolating untrusted code or operations in a controlled environment |
...
| Secret | 密钥/凭证 | Sensitive credential information such as keys, tokens, credentials, including API Key, OAuth Token, database passwords, etc. |
| Secret Masking | 密钥脱敏 | Method to mask sensitive key information in logs and display, preventing credential leakage |
...
| Audit Evidence | 审计证据 | Traceable, verifiable, not easily disavowable behavioral evidence, used for compliance and accountability |
...
| Budget Guard | 预算防护 | Mechanism to monitor and control task execution costs, preventing budget overruns |
| Risk Category | 风险类别 | Risk types evaluated by PolicyEngine, including destructive/irreversible/prod_affecting, etc. |
| PII (Personally Identifiable Information) | 个人身份信息 | Information that can identify individuals, such as email, phone, SSN, credit card numbers, etc. |
...
| CVE Intelligence | CVE情报 | Known security vulnerability intelligence service, tracking and evaluating system vulnerability impact |
| Policy Decision Request | 策略决策请求 | Policy evaluation request containing context such as taskId, subjectId, action, risk category, etc. |
...
| PII Annotation | PII标注 | Marking detected PII locations and mask forms in content |
...
| Idempotent | 幂等性 | Property that repeated execution of the same operation yields consistent results |
...
| English | Chinese | Explanation |
|---------|---------|-------------|
| Stable Core | 稳定核心 | Deliberately contracted minimum capability range to first achieve stable operation, ensuring core functionality reliability |
| Golden Task | 黄金任务 | Fixed representative task used as version regression baseline, used to verify system basic functionality has not degraded |
| Fixture | 测试固件 | Pre-set fixed input/output samples for stable testing and VCR replay |
| VCR (Video Cassette Recorder) | VCR录制回放 | Testing mechanism for recording/replaying external LLM calls, achieving deterministic testing |
| Unit Test | 单元测试 | Fine-grained testing oriented toward single functions, single modules, single objects |
| Integration Test | 集成测试 | Testing of cross-module collaboration, verifying correctness of interactions between multiple components |
| E2E (End-to-End) | 端到端测试 | Complete process testing from entry to result, covering entire system链路 |
...
| Soak Test | 浸泡测试 | Long-duration continuous stability testing, verifying system behavior under long-term load |
| Recovery Drill | 恢复演练 | Recovery capability drills for scenarios such as crash, disconnection, lock conflict, restart |
| Chaos Smoke | 混沌冒烟测试 | Fast E2E test that verifies fault detection and repair capabilities at system startup |
| Admission Control | 准入控制 | Protection mechanism where system rejects, delays, or degrades before overload |
| Readiness | 就绪度 | Whether a stage, module, or environment has reached readiness state for the next action |
...
| Regression Detection | 回归检测 | Testing method to discover new code causing existing functionality degradation |
| Stable Runtime Validator | 运行时稳定验证器 | Core module that runs golden tasks and checks database integrity and backup validity |
| Stable Acceptance Line | 稳定验收线 | Quality standard line that must be met for environment to be promoted to production-ready state |
...
| Stable Evidence Campaign | 证据收集活动 | Set of evidence collected to support promote/signoff judgments |
| Stable Evidence Bundle | 证据包 | Evidence set containing test results, baseline comparison, regression analysis, etc. supporting release decisions |
...
| Stable Gray Release Rehearsal | 灰度发布演练 | Rehearsal for verifying gray release mechanism reliability |
| Stable Rolling Upgrade Rehearsal | 滚动升级演练 | Rehearsal for verifying rolling upgrade process reliability |
...
| Stable DB Writability Rehearsal | 数据库写入演练 | Rehearsal for verifying database write capability can be recovered under fault scenarios |
| Stable Dispatch Rehearsal | 调度演练 | Rehearsal for verifying task scheduling mechanism works normally under abnormal conditions |
| Stable Dispatch Reconciliation Rehearsal | 调度对账演练 | Rehearsal for verifying scheduling layer and execution layer state consistency |
| Stable Event Replay Rehearsal | 事件重放演练 | Rehearsal for verifying event replay mechanism reliability |
| Stable Lease Rehearsal | 租约演练 | Rehearsal for verifying task lease mechanism correctly releases and renews under fault scenarios |
...
| Environment Readiness | 环境就绪度 | Conditions that must be met for environment to be put into production use |
...
| English | Chinese | Explanation |
|---------|---------|-------------|
| ADR | 架构决策记录 | Architecture Decision Record, recording important architecture decisions |
...
| VCR | 录像回放 | Video Cassette Recorder, referring to test recording/replay |
...
## 40. Protocol, Model & Security Abbreviations
...
| TTL | 存活时间 | Time To Live, valid duration of data or cache |
...
## 41. Integration & Communication Abbreviations
...
## 42. Code-Level Variable Abbreviations
...
| ts | 时间戳 | timestamp, recording time |
...
## 43. Operations & Business Abbreviations

| English | Chinese | Explanation |
|---------|---------|-------------|
| SLA | 服务等级协议 | Service Level Agreement, external commitment agreement |
| SLO | 服务等级目标 | Service Level Objective, desired target |
...
## 45. Organization & Control Plane Roles
...
| `division_lead` | 部门主管 | Control plane role for in-division workflow autonomous orchestration (document definition, not implemented as independent service in code) |
...
| `model profile` | 模型画像 | Metadata such as model capabilities, limitations, pricing, default parameters |

---

## 47. Domain & Plugin Registry
...
## 48. Confusable Term Pairs with Distinctions

### permission vs policy | 权限 vs 策略
- **permission (权限)**: Authorization result or static capability boundary. In code, permission concept is implicitly implemented through PolicyEngine, no independent Permission type definition.
- **policy (策略)**: Ruling logic and rule system, code-level entry point for final ruling on permissions, risk, approval, budget, and runtime constraints.
- **Distinction**: Verbal limitations in prompts should not be treated as formal policy.
...
- **readiness (就绪度)**: Indicates reaching readiness for a certain gate or next action.
- **production-ready (生产就绪)**: Indicates reaching the comprehensive threshold required for production backing.
...
- **artifact (产物)**: File-type or binary product, usually managed through artifact store.
- **output (输出)**: Result oriented toward upstream steps or users, can be structured data or text, not necessarily a file.
- **step output (步骤输出)**: Structured result snapshot after a step completes.
...
## 49. Five-Plane Architecture

| English | Chinese | Explanation |
|---------|---------|-------------|
| P1 Interface Plane | P1 接口平面 | External access layer: API Gateway / Webhook / Scheduler / Console / Ingress, responsible for input validation, authentication, rate limiting, routing |
...
| P4 Execution Plane | P4 执行平面 | Unified execution layer: Dispatcher / Workers / Tools / Plugins / Recovery, responsible for actually executing actions, maintaining lease, writing results back, triggering recovery |
...
| ControlDirective | 控制指令 | Control directive from P2 → P3/P4, used for mode switching, pause, terminate, rollback, quota adjustment |
| ExecutionPlan | 执行计划 | Standard execution plan from P3 → P4, describing ordered steps and resource constraints |
| ExecutionReceipt | 执行回执 | Execution result receipt from P4 → P3/P5, containing status / duration / artifacts / telemetry / sideEffects / error |
...
## 50. OAPEFLIR Stage Types

| English | Chinese | Explanation |
|---------|---------|-------------|
| OapeflirStage | OAPEFLIR 阶段枚举 | State enum for eight-stage closed loop: `observe / assess / plan / execute / feedback / learn / improve / release / knowledge_promotion` |
| OapeflirStageStatus | OAPEFLIR 阶段状态 | Stage execution status: `completed / skipped` |
| OapeflirStageRecord | 阶段记录 | Record of single stage's execution status, duration, reference ID, and reason code |
| OapeflirStageTimelineBuilder | 阶段时间线构建器 | Tool class for building OAPEFLIR stage execution timeline |
...
| OapeflirLoopService | OAPEFLIR 循环服务 | Main service class for OAPEFLIR closed loop, coordinating stage execution |
...
## 52. Execution Assessment Types

| English | Chinese | Explanation |
|---------|---------|-------------|
| ExecutionAssessment | 执行评估 | Complete post-plan execution assessment, including result classification, quality score, deviation analysis, replan suggestions |
...
| CriterionResult | 标准结果 | Success criteria evaluation result, including pass/fail, actual value, and failure reason |

---

## 53. Recovery & Fault Tolerance

| English | Chinese | Explanation |
|---------|---------|-------------|
| runtime repair | 运行时修复 | Mechanism for automatically repairing execution when faults occur |
...
| auto-rollback | 自动回滚 | Automatically roll back to previous version on release failure |
| guardrail | 护栏 | Mechanism protecting production environment from improper changes |
| canary | 金丝雀发布 | First verify new version with small traffic before expanding |
