...
| workflow | 工作流 | Task's structured execution path, defining step, dependency, input/output, and failure paths |
...
| artifact | 产物/制品 | File or binary product, usually managed via artifact store |
| output | 输出 | Result oriented toward upstream steps or users, can be structured data or text, not necessarily a file |
...
| English | 中文 | 解释 |
|---------|------|------|
| runtime | 运行时 | Layer where the system actually executes task/workflow/agent/tool |
...
| lease owner | 租约持有者 | Entity currently holding execution rights |
| fencing token | 隔离令牌 | Version token to prevent old executors from writing dirty results |
| dispatch | 调度分发 | Assigning a task or execution rights to an execution carrier |
...
| partial result | 部分结果 | Task not yet entirely complete but has retainable, auditable phased results |
...
| in_progress | 进行中 | Main logic being advanced (Task state) |
...
| English | 中文 | 解释 |
|---------|------|------|
| running | 运行中 | Workflow is advancing main logic |
...
| executing | 执行中 | Main logic being advanced (Execution state) |
| blocked | 阻塞 | Temporarily unable to proceed due to unmet dependencies, approval, strategy, or resource reasons |
...
| event | 事件 | Structured factual notification within the system |
| tier_1 event | 一级事件 | Must be reliably persisted, must be recoverable, cannot be silently lost |
| tier_2 event | 二级事件 | At-least-once delivery event |
| tier_3 event | 三级事件 | Best-effort delivery event |
| ack | 确认 | Record that a consumer has confirmed processing an event |
| replay | 重放 | Re-sending events from memory buffer |
| stream | 流 | Incremental output stream oriented to channels/UI |
...
|---------|------|------|
| compensation | 补偿 | Action to rollback, reconcile, or manually repair steps that have already occurred side effects |
| idempotent_replay | 幂等重放 | Strategy to achieve compensation through replay |
| compare_and_swap_write | 比较并交换写入 | Strategy to achieve compensation through CAS writes |
| compensating_action | 补偿动作 | Strategy to achieve rollback through executing compensating actions |
| manual_reconciliation_required | 需要人工对账 | Manual intervention required for reconciliation repair |
| checkpoint | 检查点 | State snapshot at recoverable boundary |
| resume_from_checkpoint | 从检查点恢复 | Strategy to resume execution from checkpoint |
| replay_from_start | 从头重放 | Strategy to replay from the beginning of the workflow |
...
| termination_scope | 终止范围 | Scope of termination impact: step/workflow/task/session |
...
| user | 用户 | Task source: user created directly |
| perception | 感知 | Task source: system perception trigger |
...
| worker_quarantined | 工作器被隔离 | Worker is in quarantine |
...
| missing_capabilities | 缺少能力 | Worker lacks required capabilities |
...
- **execution**: A specific runtime attempt
...
- **blocked**: Cannot proceed due to dependencies/approval/strategy/resource
...
- Governance Level: Master version (glossary master version)
...
| English | 中文 | 解释 |
|---------|------|------|
| Authoritative Store | 权威存储 | Storage that has final interpretation rights for certain types of facts, the ultimate source of truth for data; must not be confused with any cache |
| Transaction Store | 事务存储 | Storage responsible for transactional data such as tasks, state, approval, events; in code, transactional data is stored in AuthoritativeSqlDatabase |
| Artifact Store | 产物存储 | Storage for file-type, large-volume, or export-type products; different nature from transaction store |
| Analytics Store | 分析存储 | Storage oriented toward projections and materialized views, not independent analytical report storage |
...
| Eventual Consistency | 最终一致性 | State that allows reaching consistency after a short delay; different from strong consistency guarantee |
| Reconciliation | 对账修复 | Action to perform account verification and repair for state, events, workers, locks, etc. |
...
| Authoritative Task Store | 权威任务存储 | Authoritative task storage for Phase 1a, delegated to sqlite or postgres implementation |
| Phase 1a Store | Phase 1a 存储 | Task storage abstraction for initial stable core stage |
| Read-after-write Consistency | 写后读一致性 | Guarantee that writes can be read immediately after completion |
| Lease | 租约 | Temporary ownership of an execution or worker dispatch; not permanent ownership |
| Fencing Token | 隔离令牌 | Version token to prevent old executors from writing dirty results; not regular sequence |
| Dead-letter | 死信 | Failure pocket record that cannot be automatically recovered or should not continue retrying |
| Partial Result | 部分结果 | Task not yet entirely complete but has retainable, auditable phased results |
...
| English | 中文 | 解释 |
|---------|------|------|
| Config Bundle | 配置包 | Set of configurations that take effect together, including bootstrap, gateways, providers, runtime, security, workflows, etc. levels |
| Config Version | 配置版本 | Version identifier after configuration change, used for tamper detection and cache management |
| Config Layer | 配置层级 | Vertical hierarchical structure of configuration, such as platform, tenant, pack, task_type |
| Feature Flag | 特性开关 | Switch controlling capability enable/disable or gradual rollout |
| Prompt Bundle | Prompt 包 | Group of prompts that are released and versioned together |
...
| Canary Rollout | 金丝雀发布 | Config release strategy of small-scale trial run before gradual expansion |
| Rollback Point | 回滚点 | Record of recoverable config version snapshot |
...
| Compatibility Window | 兼容性窗口 | Formally supported compatibility time range between different runtimes/SDKs/protocols/plugins |
...
| Evidence Package | 证据包 | Set of evidence used to support promote/signoff/production-ready judgments |
| Production-ready | 生产就绪 | Has reached comprehensive threshold for production backing |
| Phase 1a Ready | Phase 1a 就绪 | Minimum available threshold for Phase 1a stable core |
...
| English | 中文 | 解释 |
|---------|------|------|
| Fixed Prefix | 固定前缀 | Cross-agent shared system prompt fixed prefix that does not participate in normal compaction by default |
| Domain Block | 领域块 | Reusable prompt intermediate layer for same domain/profile |
| Variable Suffix | 变量后缀 | Prompt suffix that dynamically changes per task, role, plan, memory |
| KV Cache Fixed Prefix | KV 缓存固定前缀 | Pre-fill cache reuse mechanism based on same prefix hash |
...
| English | 中文 | 解释 |
|---------|------|------|
| Policy Engine | 策略引擎 | Code-level entry point that makes final rulings on permissions, risk, approval, budget, and runtime constraints; unified security decision center |
| HITL (Human In The Loop) | 人工介入 | Decision step requiring explicit human participation; high-risk operations must pass human approval to continue |
| Approval | 人工审批 | Decision node in workflow requiring explicit human confirmation to continue |
| Break-Glass | 紧急放行 | High-risk emergency release configuration marker; critical risk triggers break-glass approval type; used for emergency bypass of standard process |
| Sandbox | 沙箱 | Execution isolation boundary; limiting untrusted code or operations in a controlled environment |
| Exec Policy | 执行策略 | Rule set for tool/command execution; defines which operations are allowed or prohibited |
| Permission | 权限 | Authorization state where a subject can see or use a capability; implicitly implemented via PolicyEngine |
| Secret | 密钥/凭证 | Sensitive secret information such as keys, tokens, credentials, including API Key, OAuth Token, database password, etc. |
| Secret Masking | 密钥脱敏 | Method to mask sensitive key information in logs and displays; prevents credential leakage |
...
| Audit Evidence | 审计证据 | Traceable, verifiable, not easily rebuttable behavioral evidence; used for compliance and accountability |
| Field Encryption | 字段加密 | Encryption protection for specific fields; ensures security of sensitive data in storage and transmission |
...
| Kill Switch | 熔断开关 | Switch to comprehensively disable system functions in emergencies; once activated, all operations will be rejected |
...
| Policy Decision Request | 策略决策请求 | Strategy evaluation request containing task ID, subject ID, action, risk category, and other context |
...
| Data Handling Dimension | 数据处理维度 | Scenario dimensions for data flow, including prompt/logs/memory/artifact/cross_worker/debug |
...
| Secret Management Service | 密钥管理服务 | Service for unified management of all key lifecycle, rotation, and access |
...
| Idempotent | 幂等性 | Property where repeating the same operation yields consistent results |
...
| English | 中文 | 解释 |
|---------|------|------|
| Stable Core | 稳定核心 | Minimum capability range deliberately shrunk to achieve stable runnable state first; ensures core functions are reliable |
| Golden Task | 黄金任务 | Fixed representative task as version regression baseline; used to verify system basic functions have not degraded |
| Fixture | 测试固件 | Pre-set fixed input/output samples for stable testing and VCR playback |
| VCR (Video Cassette Recorder) | VCR录制回放 | Testing mechanism for recording/playback of external LLM calls; enables deterministic testing |
| Unit Test | 单元测试 | Fine-grained testing targeting single function, module, or object |
| Integration Test | 集成测试 | Cross-module collaboration testing; verifies correctness of interactions between multiple components |
| E2E (End-to-End) | 端到端测试 | Complete flow testing from entry to result; covers entire system chain |
| Chaos Test | 混沌测试 | Testing that actively injects faults to verify recovery and resilience, such as injecting network delays, service outages |
| Soak Test | 浸泡测试 | Long-duration continuous stability testing; verifies system performance under long-term load |
| Recovery Drill | 恢复演练 | Recovery capability drills for scenarios like crash, disconnect, lock conflict, restart |
| Chaos Smoke | 混沌冒烟测试 | Quick E2E test to verify fault detection and repair capabilities at system startup |
...
| Stable Validation | 稳定验证 | Verification process of running golden tasks and checking database integrity and backup round-trip |
...
| VCR Replay Mode | VCR回放模式 | Including fixture_only/vcr_replay/vcr_record three modes; controls whether actual calls are allowed |
| Drift Detection | 漂移检测 | Comparing current running results with baseline to detect performance or behavior anomalies |
| Regression Detection | 回归检测 | Testing method to discover functional degradation caused by new code |
| Stable Runtime Validator | 运行时稳定验证器 | Core module that runs golden tasks and checks database integrity and backup validity |
| Stable Acceptance Line | 稳定验收线 | Quality standard line that environment must meet to be promoted to production-ready state |
| Stable Release Gate | 发布门禁 | Stability and quality check that must be passed before version release |
| Stable Release Package | 稳定发布包 | Version product that has been verified and can be used for formal deployment |
...
| Stable Migration Compatibility Rehearsal | 迁移兼容性演练 | Rehearsal to verify system migration compatibility between different versions |
...
| Stable Concurrency Rehearsal | 并发演练 | Rehearsal to verify system stability under concurrent load |
...
| Stable DB Writability Rehearsal | 数据库写入演练 | Rehearsal to verify database write capability recovery under fault scenarios |
...
| Stable Lease Rehearsal | 租约演练 | Rehearsal to verify task lease mechanism correctly releases and renews under fault scenarios |
...
| Prompt Injection Guard | 提示词注入防护 | Protection mechanism to detect and block malicious prompt injection attacks |
| Prompt Injection Red Team | 提示词注入红队 | Security testing team for prompt injection attacks |
| Environment Readiness | 环境就绪度 | Conditions that environment must meet for production use |
...
| DLQ | 死信队列 | Dead Letter Queue,承接 unable-to-continue messages or tasks |
...
| English | 中文 | 解释 |
|---------|------|------|
| id | 标识符 | identifier, object's unique identifier |
...
| src | 源 | source, source |
| dest | 目标 | destination, destination |
...
| SLO | 服务等级目标 | Service Level Objective, desired goal |
...
| MTBF | 平均故障间隔 | Mean Time Between Failures, average time without failure |
| RCA | 根因分析 | Root Cause Analysis, incident analysis process |
| RTO | 恢复时间目标 | Recovery Time Objective, failure recovery duration goal |
...
*This document is read-only governance reference; terminology conflicts should be verified against corresponding authoritative contracts.*
...
| English | 中文 | 解释 |
|---------|------|------|
| `strategic_governor` | 战略总督 | Control layer role for strategic judgment, escalation governance, org-level approval (document definition, not implemented as independent service in code) |
| `intake_router` | 摄取路由器 | Control layer role for input triage, classification, routing, budget entry |
| `workflow_planner` | 工作流规划器 | Control layer role for cross-division splitting, dependency graph, aggregation, failure escalation |
| `division_lead` | 部门主管 | Control layer role for in-division workflow autonomous orchestration (document definition, not implemented as independent service in code) |
...
| English | 中文 | 解释 |
|---------|------|------|
| `channel` | 渠道 | User or system access interface such as CLI, Web, Telegram, API (note: code only implements telegram/slack/webhook) |
| `channel capability` | 渠道能力 | Capabilities supported by a channel such as text, button, stream, attachment (note: code has no corresponding capability enum type definition) |
| `plugin` | 插件 | Installation unit that extends platform capabilities through public SDK or controlled boundaries |
| `skill` | 技能 | Reusable orchestration capability for tools or steps |
| `MCP` | 模型上下文协议 | External capability access protocol/extension type (MCP tools are verified via mcp-tool-guard) |
| `recipe` | 配方 | Structured workflow or template definition that can be used as workflow author input layer |
| `template` | 模板 | Similar to recipe; structured workflow or step definition that can be reused |
...
- **permission（权限）**: Authorization result or static capability boundary. Permission concept in code is implicitly implemented via PolicyEngine; no independent Permission type definition.
- **policy（策略）**: Ruling logic and rule system; code-level entry point that makes final rulings on permissions, risk, approval, budget, and runtime constraints.
- **Distinction**: Descriptive limitations in prompts should not be written as formal policy.

### queue vs lease | 队列 vs 租赁
- **queue（队列）**: Task queuing mechanism that determines waiting order.
- **lease（租赁）**: Temporary ownership of an execution or worker dispatch; used to prevent duplicate execution; determines current execution rights.
- **Distinction**: When both exist, they should not replace each other.

### readiness vs production-ready | 就绪度 vs 生产就绪
- **readiness（就绪度）**: Indicates reaching a certain gate or readiness for next action.
- **production-ready（生产就绪）**: Indicates reaching comprehensive threshold for production backing.
...
- **Distinction**: One signoff conclusion should not be written as a permanent pass.
...
- **artifact（产物）**: File or binary product, usually managed via artifact store.
- **output（输出）**: Result oriented toward upstream steps or users, can be structured data or text, not necessarily a file.
- **step output（步骤输出）**: Structured result snapshot after a step completes.
...
- **execution（执行）**: A specific runtime attempt.
- **Distinction**: Same workflow can correspond to multiple execution attempts.

### agent vs worker | 智能体 vs 工作器
- **agent（智能体）**: Emphasizes responsibility and intelligent entity concept.
- **worker（工作器）**: Emphasizes execution carrying and resource position.
...
| RequestEnvelope | 请求信封 | Standard request wrapper from P1 → P2; contains requestId / tenantId / taskSpec / priority / traceContext / principal |
| ControlDirective | 控制指令 | Control directive from P2 → P3/P4; used for pattern switching, pause, termination, rollback, quota adjustment |
| ExecutionPlan | 执行计划 | Standard execution plan from P3 → P4; describes ordered steps and resource constraints |
| ExecutionReceipt | 执行回执 | Execution result return from P4 → P3/P5; contains status / duration / artifacts / telemetry / sideEffects / error |
...
| OapeflirStageRecord | 阶段记录 | Records individual stage execution status, duration, reference ID, and reason code |
| OapeflirStageTimelineBuilder | 阶段时间线构建器 | Tool class for building OAPEFLIR stage execution timelines |
...
| OapeflirLoopService | OAPEFLIR 循环服务 | Main service class for OAPEFLIR closed loop; coordinates execution of each stage |
...
| AssessmentPhase | 评估阶段 | Assessment timing: `pre-execution / post-execution` |
...
| TaskPhase | 任务阶段 | Task's current phase: `intake / planning / executing / reviewing / completed` |
...
| English | 中文 | 解释 |
|---------|------|------|
| ExecutionAssessment | 执行评估 | Complete post-plan execution assessment; contains result classification, quality score, deviation analysis, replan recommendation |
| ExecutionOutcome | 执行结果 | Execution result type: `completed / completed_with_deviations / repairable / failed / escalated` |
| ExecutionDeviation | 执行偏差 | Deviation from planned workflow during execution: `skipped / reordered / modified / added / substituted` |
| ExecutionError | 执行错误 | Error encountered during execution; contains step ID, error code, message, severity, and recoverability |
| CriterionResult | 标准结果 | Success criteria evaluation result; contains pass/fail, actual value, and failure reason |
...
| English | 中文 | 解释 |
|---------|------|------|
| runtime repair | 运行时修复 | Mechanism for automatic repair of execution when faults occur |
...
| stalled detection | 停滞检测 | Detect executions with no effective progress for extended periods |
...
| canary | 金丝雀发布 | First validate with small traffic before expanding |
