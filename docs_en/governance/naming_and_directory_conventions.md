...
- The evaluation asset root directory uniformly uses `eval/`; `evaluation/` is treated only as a historical term and is no longer used as a repository path, document entry name, or configuration reference name.
