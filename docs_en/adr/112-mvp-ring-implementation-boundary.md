...
2. Ring 1 is the first target for code implementation; Ring 2/3 must not reversely block Ring 1's API, schema, state machine, and test freeze.
3. v4.3 contracts in `docs_zh/contracts/` are the implementation entry points for Ring 1; old contracts can serve as historical compatibility description, but do not define MVP main chain.
...
- Can advance state through `RuntimeStateMachine.transition(command)` and append `platform.*` fact event.
