...
|------|------|
| latency_based | Route based on latency |
| geo_based | Route based on geographic location |
...
- Automatic failover must be built on async replication, lease takeover, and fencing validation, not assuming multi-master synchronous truth writes.
...
- Region-level failures do not impact globally
...
- A-24: This ADR originally listed `sync` replication with `RPO=0`, root cause: multi-region ADR reused traditional database deployment terminology without distinguishing append-only evidence replication from runtime truth single writer boundary. Fix: The body now clarifies that v4.2/v4.3 only acknowledges async/eventual replication, does not commit to multi-master truth writes or `RPO=0` sync replication.