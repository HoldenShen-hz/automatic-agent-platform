...
- **Plan**: Merge ConstraintPack and form execution cap
- **Execute**: Enforce apply before each run
...
- Constraint sources are merged in order: platform -> tenant -> domain -> task
...
- A-37: This ADR originally reduced `ConstraintPack` to `risk_policy + output_policy`. Root cause: Constraint engine ADR draft only covered risk and output governance and did not include budget, sandbox, and approval requirements in the unified constraint package. Fix: The text now adds `budget_envelope / sandbox_requirement / approval_requirement` to the minimum set.