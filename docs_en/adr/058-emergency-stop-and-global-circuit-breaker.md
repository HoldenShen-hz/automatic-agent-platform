...
When security incidents occur, the platform needs to instantly stop all Agent operations. Global circuit breaker mechanism prevents fault propagation.
...
interface GlobalCircuitBreaker {
  ...
  open_duration_ms: number; // Circuit breaker duration (does not include automatic release semantics)
}

Constraints:
- `open_duration_ms` only defines circuit breaker duration, does not represent TTL automatic release.
- Circuit breaker transitioning from `open` to `half_open` must be done via explicit call to `circuit_breaker.half_open()` or manual intervention.
- Prohibited from automatically recovering to `closed` from `open` state without transition.
- In `half_open` state, if probe request success rate reaches threshold, may transition to `closed`; otherwise fallback to `open`.
```
...
- Emergency stop must be implemented through official mechanisms: `PlatformPanicDirective` or `OperationalDirective`. Must not be replaced by bare switch variables or bypass scripts.
...
- A-25: This ADR originally only defined `L0-L4` level names without binding them to official control plane mechanisms. The root cause is that the emergency stop ADR was first drafted from operations playbook, and later did not align with `PlatformPanicDirective / OperationalDirective` main contract. Fix: The body now explicitly binds each level to `PlatformPanicDirective` or `OperationalDirective(type=kill_run / pause_run)`.