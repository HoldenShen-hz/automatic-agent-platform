...
| 深度         | Applicable Scenario                 | Content                                                  |
| ------------ | ------------------------ | ----------------------------------------------------- |
| L1 Summary   | Non-technical user daily viewing       | One-sentence summary: "Because abnormal traffic was detected, automatically scaled up 2 instances" |
| L2 Reasoning | Business owner review           | Causal chain + key data points + alternative plans                        |
...
## 59.5 NL Entry Integration

§39 NL interaction pipeline adds `why` Intent type:

Users can ask "Why was the last release rolled back?" in natural language; the system parses it as WhyQuery and calls the explanation pipeline.

## 59.6 Explanation Cache and Security

- L1/L2 explanation cache TTL = 24h; L3 not cached (ensures latest evidence)
- Explanation content subject to §50 knowledge domain isolation — can only see evidence with permissions
- Explanation logs themselves are纳入 audit (§23), recording who viewed what explanation when
- L3 forensic explanation must first be reserved through `forensic_explanation_budget`; when over budget, return evidence index and staged generation plan; must not crowd out P4 execution budget during incident
- Explanation renderer only works based on `versionLockRef` corresponding Prompt/Policy/PlanGraph version; policy changes after execution cannot rewrite history explanations, can only append new version explanation view

---

# 60. Emergency Brake and Global Circuit Breaker Architecture

> Provides a single atomic operation to quickly block the entry point and silence the execution plane, used for security incidents, Prompt injection attacks, Agent escape, and other emergency scenarios.
...
| severity          | full / partial           | full = full platform stop; partial = scoped stop          |
| scope             | global / tenant / domain | Circuit breaker effective scope                                       |
| reason            | string                   | Trigger reason (security incident description)                           |
| issuedBy          | string                   | Issuer identity                                         |
| requiredApprovers | string[] (min 2)         | Dual-approval requirement, prevents single-point misoperation                       |
| reconfirmationAfterSeconds | number          | Triggers re-confirmation reminder after expiration, does not automatically lift the circuit breaker             |
| rollbackStrategy  | freeze / graceful_drain  | freeze 立即冻结；graceful_drain 等待运行中Step完成 |

Platform Panic is effective indefinitely by default.`reconfirmationAfterSeconds` only triggers re-confirmation, escalation reminders, and evidence snapshot refresh; recovery must go through §60.3 `PlatformResumeDirective` with dual-approval.
...
PlatformPanicDirective
    ...
    ├──▶ P2 Control Plane: Revoke all active Agent tokens
    │
    ├──▶ P3 Orchestration Plane: Suspend all in-flight HarnessRun and OAPEFLIR projection updates
    │
    ├──▶ P4 Execution Plane: Abort new side effects, perform reconciliation on ambiguous side effects, execute compensation when supported
    │
    ├──▶ P5 State Plane: Generate ForensicSnapshot, set read-only mode
    │
    └──▶ X1 Fabric: Block all egress, trigger alerts on all channels
```

**SLA**: Split into two metrics: `ingress_block_time < 5s` (same Region) / `<15s` (cross Region); `execution_quiescence_time` defined by deployment form, D1 < 10s, D2 < 30s, D3/S4 < 120s. Ingress blocking and all workers stopped must not be conflated into one metric.
...
| localStopState | Actual stop scope and incomplete items for this plane |
| timestamp | Acknowledgment timestamp |
| evidenceRef | Local forensics snapshot or log reference |

If any plane fails / times out, a `panic_incomplete` P0 incident must be generated, and external止血 actions such as infrastructure-level kill, network isolation, or credential revocation must be triggered.

## 60.3 Safety Recovery Protocol

| Step | Operation                         | Requirement                                               |
| ---- | ---------------------------- | -------------------------------------------------- |
| 1    | ForensicSnapshot Review        | Security team confirms threat is eliminated                             |
| 2    | PlatformResumeDirective Publish | Requires ≥ 2 platform_admin dual approval                |
| 3    | Gradual Recovery                     | First restore read-only queries → low-risk workflow → full recovery |
| 4    | Post-Incident Report                     | Publish Post-Incident Report within 72h                    |

**Admin Unavailability Fallback Plan**: If fewer than 2 platform_admin are online for more than 4 hours, enable the following degraded recovery path:

1. System sends multi-channel emergency notifications to all platform_admin (SMS + phone + enterprise WeChat/Feishu/DingTalk)
2. If no response after 4h, authorize `break_glass` mechanism — any 1 platform_admin + 1 security_team member combination approval can replace dual admin approval
3. If still no response after 8h, only allow recovery of forensic / monitoring / read-only inspection; prohibit new workflow, write operations, external side effects, and policy relaxation; full recovery still requires dual approval
4. All `break_glass` recovery operations are recorded as P0-level audit events; platform_admin review must be supplemented within 72h
...
- Each drill outputs `PanicDrillReport`, containing at minimum ingress_block_time, execution_quiescence_time, egress_block_time, credential_revoke_time, plane_ack_success_rate, manual_recovery_time, and unresolved_findings.
...
| active→paused       | 手动/自动暂停     | 行为漂移检测(§63)触发或手动Operation      |
...
| 审批挂起   | 需要审批的Step进入 pending 状态，等待连接恢复                   |
...
| 模式          | 硬件Requirement                | Applicable Scenario                 |
| ------------- | ----------------------- | ------------------------ |
...
| avg_step_count          | number               | 平均Step数，检测复杂度变化                  |
...
所有阈值必须声明样本量、分布假设和误报处理策略；不得把“偏移 > 2σ”作为非正态业务数据的唯一成功标准。漂移响应默认先告警、降速或Requirement人工复核；自动降级必须考虑业务连续性和 SLA。
...
BehaviorDriftAlert { agent_id, dimension, severity, drift_score }
    ...
    │
    └── severity=high → 暂停 Agent(§61 paused) + 触发 Incident(§12) + Requirement人工审查
```
...
防博弈Requirement：行为指纹必须区分真实用户任务、合成评测任务和低价值“保活”任务；TrustScore、漂移基线和晋升统计不得由未验证的假任务单独推动。
...
| 能力          | 运行中 Workflow | 已完成 Workflow | Description                                          |
| ------------- | --------------- | --------------- | --------------------------------------------- |
...
断点命中后只有 sandbox session 进入 paused，通过 WebSocket 推送 `breakpoint_hit` 事件。调试者可检查 ContextSnapshot 后执行 `resume_replay`、`step_replay` 或 `abort_replay`；这些Operation不得修改 production truth、不得触发真实 Tool/LLM/SideEffect，也不得改变原 HarnessRun 状态。
...
| sections            | `Section[]`                      | 报告章节定义（控制点映射 + 证据Requirement）         |
...
`ComplianceTemplateRegistry` 必须额外保存 legal_version、change_source、effective_date 和 migration_rule。法律或框架变更不得只更新模板文本；必须生成影响分析、迁移任务和旧报告适用性Description。
...
- **权限控制**：仅 read Operation，无法修改、删除或导出原始数据
- **PII 保护**：返回数据经过 §23 数据分级检查，未通过分级审核的Field自动脱敏
- **审计追踪**：审计员的每次查询Operation本身纳入审计日志(§23)，记录查询人、时间、范围
- **最小权限**：AuditorAccess 按 framework、tenant、timeRange、controlId 和 evidence type 授权，默认不可见业务原文、secret、PHI 和跨租户证据
- **版本冻结**：已签核报告只能追加补充Description或勘误，不得原地覆盖；重新生成必须产生新 report version
...
| ------------- | ------------------------------------------------------------------- |
| domain        | `platform_ops`                                                      |
| riskProfile   | high（涉及生产环境写Operation）                                          |
| tools         | `metrics_query`、`config_patch`、`restart_service`、`scale_replica` |
| evalFramework | SLO-based（以 §27 SLO 达成率作为 Agent 表现评估标准）               |
| autonomy_cap  | 只读Operation最高 auto；写Operation最高 supervised（§42）                     |
...
| L1   | Agent 生成诊断报告、日报、容量趋势和只读问答 | 不执行生产写Operation |
...
MVP Eval Gate 只Requirement deterministic contract tests + small golden set + denial-path regression；完整 Prompt canary、跨模型评测和大规模 holdout 进入 Hardening。
...
- [ ] HarnessRun/HarnessStep 契约完整覆盖所有运行和Step
...
- [ ] HITL Runtime 的 inspect/patch/override 在 §43 看板可Operation
...
选择标准：高 HITL Requirement、监管密集、需律师/审计师参与验证。
...
### Phase 9d：高优先级新域（8 周）——客户服务 · IT 运维 SRE/DevOps · Content审核与安全 · 在线直播

选择标准：运营刚需、实时性Requirement高、已有成熟工具生态可集成。
...
选择标准：高合规Requirement、强 HITL 域、需领域专家深度参与认证。
...
Phase 9c (完善域：调研·学术·财务·法务)
    │
    ▼
Phase 9d (高优先级新域：客服·IT运维·Content审核·直播)
    │
    ▼
...
每个 ADR 必须包含以下Field，缺少 runtime/schema/test 映射的 ADR 只能处于 `draft`：
...
| P3 Domain | 高风险域边界、监管证据、HITL/资质Requirement | 改动需域 owner 签核 |
...
- **ADR-No-Code-UX-Maps-To-Standard-API** — 非技术用户界面Operation必须映射到标准 Public API，禁止旁路
...
- **ADR-Governance-Delegation-Requires-Scope** — 治理权委托必须限定 scope（资源Type + OrgNode 范围），禁止全局委托
...
- **ADR-Multimodal-Safety-Check-Before-Output** — 多模态输出（图片/语音）必须经过Content安全检查后才能交付给用户
- **ADR-PlatformOps-Agent-Read-Only-Default** — 平台自运维 Agent 默认只读，生产写Operation必须经过人工审批
...
- **ADR-HarnessDecision-Six-Way** — HarnessDecision 固定六种裁决（accept/retry_same_plan/replan/escalate_to_human/downgrade_mode/abort），禁止自定义裁决Type
...
- **ADR-HITL-As-Runtime-Primitive** — HITL 作为 Harness 原生运行时StepType（phase=hitl），不仅是 escalation 路径
...
- **ADR-Finance-Accounting-Segregation-Of-Duties** — 财务域必须执行职责分离（创建人≠审批人），符合 SOX 内控Requirement
...
- **ADR-Live-Streaming-Realtime-Moderation-SLA** — 在线直播域实时流审核延迟必须 < 2s，违规Content必须在检测后 5s 内执行下架/断流
- **ADR-Game-Publishing-Multi-Platform-Compliance** — 游戏上架域每个目标平台必须独立通过合规检查（年龄分级/Content审核/支付合规），禁止跨平台复用审核结果
- **ADR-Customer-Service-Escalation-Timeout** — 客户服务域 Agent 无法在 3 轮对话内解决问题必须自动转接人工坐席，禁止无限循环
- **ADR-IT-Operations-Blast-Radius-Limit** — IT 运维域自动修复Operation爆炸半径限制为单节点/单服务，跨域Operation必须人工审批
- **ADR-Education-Minor-Data-Protection** — 教育培训域涉及未成年人数据必须遵循 COPPA/未成年人保护法，数据收集最小化且需监护人同意
- **ADR-Creative-Production-IP-Verification** — 广告素材制作域所有 AI 生成Content必须通过版权/商标侵权检查，禁止使用未授权素材
- **ADR-Game-Dev-IP-Similarity-Check** — 游戏开发域 AI 生成美术资产必须通过已知 IP 相似度检测，防止版权侵权
- **ADR-Marketing-Brand-Consistency-Check** — 市场营销域所有对外发布Content必须通过品牌调性一致性检查和广告法合规检测
...
- `src/domains/` 中额外存在的目录属于孵化域、元域或平台性支撑目录，不Requirement与 `§71-§94` 一一对应。
...
- **多模态输出必须经过Content安全检查**
- **PlatformOps Agent 默认只读，生产写Operation需人工审批**
...
所有成功标准必须能映射到自动化测试、演练记录、审计证据或人工签核记录；不能验证的描述只能作为目标Description，不得作为 gate。

平台成功标准与域成功标准分离：平台核心只看 Harness / State / Evidence / Governance 的可运行闭环；24 域、12 原型和域 GA 数量按 domain wave 验收，不得反向阻塞核心平台投产。

Phase 8 / Phase 9 是独立能力 gate，不是 MVP 主线阻塞条件。MVP 只Requirement Phase 8 中与 Harness 可执行闭环有关的最小切片；24 域 GA 属于 Enterprise 扩张 gate，以 `N/24 domains GA` 作为进度指标，不阻塞核心平台投产。
...
- HarnessRun / HarnessStep 全部Field持久化且可查询
...
> **一个把 Agent 当作高风险自动化单元进行严格控制、隔离、恢复、审计和治理的企业Operation系统——从一人公司到万人企业，以十层架构覆盖基础设施、AI 运营、业务域接入、垂直业务域深化、智能交互、Harness 工程化、Harness 八支柱深化、组织治理、规模化生态、运营成熟度的全栈能力。**
...
- **多模态输入输出必须纳入统一安全管控，不可绕过Content审查**
...
| 运行和Step有一级契约吗？ | 仅 HarnessRunRequest                                  | §45.13 HarnessRun/HarnessStep 完整生命周期                          |
...
只有同时具备**基础设施层的稳定性**、**AI 运营层的可控性**、**业务域接入层的结构化**、**垂直业务域深化层的领域专精**、**智能交互层的易用性**、**Harness 工程化层的标准化**、**Harness 八支柱的深化**、**组织治理层的适配性**、**规模化运行层的可扩展性**和**运营成熟度层的可投产性**，企业才能把 Agent 平台从架构设计，升级为真正覆盖一人公司到万人企业、24 个垂直业务线的企业级生产力Operation系统。
...
| UoW                      | Unit of Work                                               | 工作单元，事务性Operation的原子边界                                                                                                                      |
...
| HarnessStep              | Harness Step实体                                           | 单个执行Step契约，含 phase/role/inputs/outputs/rationale(§45.13)                                                                                    |
...
| SAR/STR                  | Suspicious Activity/Transaction Report                     | 可疑活动/交易报告，反洗钱法规Requirement(§74)                                                                                                              |
...
| CSAM                     | Child Sexual Abuse Material                                | 儿童性虐待材料，Content审核领域法定强制上报Content(§92)                                                                                                   |
| AIR                      | Adverse Impact Ratio                                       | 不利影响比率，HR 招聘公平性指标，合规Requirement ≥ 0.8(§87)                                                                                                |
...
| ACP                      | Agent Collaboration Protocol                               | 多 Agent 协作协议，定义 8 种消息Type + 7 条不可违反规则(§19.5)                                                                                      |
...
OAPEFLIR v4.4 Executable Spec 是 v4.2 的历史输入与迁移来源，不再作为直接实现权威。v4.2 的运行契约以正文、Executable Runtime Contract、Schema / Zod / OpenAPI / Event Registry 为准；附录 H 只用于Description v4.4 能力已经落到哪些正文章节。
...
| v2.3 | 2026-04-19 | **智能交互完备版**：新增自然语言任务入口架构(§39)、目标分解引擎架构(§40)、主动式 Agent 框架(§41)、渐进式自主权模型(§42)、统一运营看板架构(§43)、非技术用户体验架构(§44)；新增 6 个 ADR；使平台从"Agent 基础设施"升级为面向非技术用户的"Agent Operation系统"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| v3.1 | 2026-04-22 | **24 域全覆盖版**：新增 12 个垂直业务域架构章节(§83-§94)——在线直播·广告素材制作·游戏开发·游戏上架·人力资源·供应链与物流·医疗健康·教育培训·客户服务·Content审核与安全·IT运维SRE/DevOps·市场营销与品牌；DomainRecipe 从 8 种原型扩展为 12 种（新增 Moderation/Logistics/Conversational/IncidentOps）；§37.1 问题陈述表扩展为 24 域×8 维度全景对比（4 张表）；§33 路线图 Phase 9 从 3 批扩展为 6 批（9a-9f，总计 48 周）含依赖图更新；§34 新增 12 个域专属 ADR（共 105 个）；§35 代码目录新增 12 个域实例目录；§36 新增 11 项域专属风险和 11 项域专属硬约束；§36.3 Phase 9 成功标准扩展至 24 域；§38 接入 Runbook 三个 Gate 扩展 Critical/High 风险域检查清单；§70 结论 12→24 垂直域；附录 G 新增 13 项域专属术语（4 种新原型 + CSAM/AIR/MTTR/MTTD/FCR/AHT/COPPA/SOV/SOV）；**结构重组**：全文按十层架构重排为 Part I-XI 分篇结构——基础设施层(§4-§14,§24-§32)整合为 Part I，§58 Harness横切合并到 §45 之后(Part VI)，§71-§94 垂直域移至 §38 之后(Part IV)，§33-§36 落地汇总移至 Part X，§70 结论移至全文末尾(Part XI)；章节编号保持稳定以兼容历史引用                                                                                                                                                                                                                                                                                                                                                                                                                |
| v3.2 | 2026-04-22 | **架构深化版**：新增统一领域元模型(§37.11)——Canonical Domain Meta-Model 定义 12 问标准模板，24 域填充矩阵（Q1-Q6 + Q7-Q12 两张表），使新域接入模板化、平台内核配置驱动、看板/审批/评测统一生成；新增多 Agent 协作协议(§19.5)——Agent Collaboration Protocol 定义 8 种消息Type（task_request/task_offer/task_accept/task_reject/partial_result/escalation_request/completion_report/takeover_notice）、9 个强制Field、7 条不可违反规则（权限不扩大/风险不提升/约束不绕过/输出可复核/接管必审计/预算不超支/深度不超限），将 §19.1-19.4 委托模型从约定升级为强制协议；新增三环实施优先级（Part X 前言）——第一环平台生存环（P1-P5+ConstraintPack+HarnessRun+Risk/Audit+Lease/Recovery+Panic/Incident+ModelGateway，对应 Phase 1-2+8a，约 16 周）、第二环平台可用环（NL入口+GoalDecomposition+HITL+AsyncHarness+Dashboard+Org/SSO+DomainDescriptor+元模型+协作协议，对应 Phase 3-5+8b/8c，约 24 周）、第三环平台扩张环（Marketplace+MultiRegion+Edge+CostOptimizer+BehaviorDrift+ComplianceReporter+24 DomainPacks，对应 Phase 6-9，约 40+ 周）；更新 §70 结论新增 3 条核心原则（元模型统一·协作协议强制·分环实施）；**Review 修正**：§38 接入 Runbook 新增元模型Step和门禁项(§37.11)、§45 Harness Runtime 新增协作协议关联(§19.5)、§33 Phase 5 交付物新增元模型+协作协议、§33 依赖 Phase 8b→8c 修正、§36.3 补全 Phase 8 成功标准、§34 域 ADR 标签 12→24+补全游戏开发域 ADR(共 105)、§36.1 补全游戏开发域风险(共 11 项)、§82 法务域 12→24 域修正、三环 Phase 映射修正 |
| v3.3 | 2026-04-22 | **域章节深化版**：24 个垂直业务域章节(§71-§94)从 ~28 行模板扩展至 ~65 行，每域新增 5 个子节：Agent 工作流（详细）——每个 Agent 的完整多Step流程；关键工具/集成——按类别列出具体产品和 API；数据敏感度分级——按机密等级划分数据Type；性能/延迟预算——逐Operation SLA 指标；常见故障与恢复——5 行故障模式×恢复策略表。Content源自 v3.0-domain-research.md 调研数据，总计新增 ~895 行                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
