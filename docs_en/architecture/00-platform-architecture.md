...
OAPEFLIR v4.4 Executable Spec is the historical input and migration source for v4.2, no longer used as direct implementation authority. v4.2's execution authority is the main text, Executable Runtime Contract, Schema / Zod / OpenAPI / Event Registry; Appendix H only explains where v4.4 capabilities have landed in the main text sections.
...
Structural conflicts resolved by above order; security, risk, compliance, and data protection class conflicts, without changing authority object attribution, follow the stricter one.

## H.1 Main Text Landing

| v4.4 Capability | Main Doc Landing |
| --- | --- |
| Eight-stage responsibilities, PlanGraph, GraphPatch, risk propagation, worst-path analysis | §13 |
...
## H.2 v4.4 Complete Specification Section Index
...
| 39-42 | Implementation directory, minimum implementation roadmap, ADR, final judgments |

## H.3 v4.4 Object Convergence Mapping

v4.4 objects do not directly become v4.2 authoritative implementation objects by original name; the following mapping confirms contract boundaries for "absorbed but renamed/downgraded/projection-ized" items.

| v4.4 Object | v4.2 Convergence Object / Status | Main Doc Landing |
| --- | --- | --- |
| OapeflirRun | HarnessRun; old name no longer authoritative execution entity | §13, §45.13, §25.9 |
...
| AttemptLineage | Append-only lineage for retry / redrive | §14.14, §28 |
...
| GraphPatch / GraphPatchOperation / GraphPatchCompatibilityReport | Append-style Replan contract | §13.13 |
...
| HarnessDecision | Unified adjudication protocol, with production extended adjudication | §58.6 |
...
| RunVersionLock | Version frozen at admitted | §25.10 |
...
| RepairRecord | Append evidence for repair / redrive / recovery | §12, §14.14, §31 |
...
## H.4 Non-Downgradable Clauses

The following clauses are non-downgradable; among them, Run authoritative objects converge to HarnessRun per v4.2, and have been moved to §2.4 ArchitectureInvariantRegistry:

1. Complex task Plan must be PlanGraph.
2. Graph Scheduler decisions must be Trace Replay capable.
3. HarnessRun / NodeRun terminal states are non-transitional.
4. Event append and truth update must be in same transaction.
5. SideEffect ambiguous must not be treated as success.
6. Trace Replay must not re-invoke LLM / Tool; any Replay must not produce real side effects.
7. LearningCandidate must not directly go online.
8. LLM-as-Judge must not override deterministic failures.
...
| ---- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| v1.0 | 2026-04    | Initial five-plane architecture + stability seven-layer + OAPEFLIR conceptual design |
| v1.1 | 2026-04    | Added risk matrix, DLQ model, deployment recommendations |
| v1.2 | 2026-04    | Added data model with 44 tables, event namespace, ADR recommendations, recommended directory |
...
| v3.3 | 2026-04-22 | **Domain Chapter Deepening Version**: 24 vertical business domain chapters (§71-§94) expanded from ~28-line templates to ~65 lines each, each domain added 5 subsections: Agent workflow (detailed) — complete multi-step process for each Agent; Key tools/integrations — specific products and APIs listed by category; Data sensitivity classification — data type classification by confidentiality level; Performance/latency budget — per-operation SLA metrics; Common failures & recovery — 5-row failure mode×recovery strategy table. Content sourced from v3.0-domain-research.md research data, total ~895 lines added |
| v4.0 | 2026-04-27 | **OAPEFLIR v4.4 Executable Runtime Specification Integration Version**: Upgraded OAPEFLIR from "controlled cognitive kernel" to "executable Runtime Spec". Added PlanGraph as internal structure of ExecutionPlan; introduced Graph Normalization, Graph Validation, Graph Risk Propagation, Worst-Path Analysis, Deterministic Graph Scheduler, GraphPatch; added Run / Node state machine terminal state closure rules, AttemptLineage, Event Registry, Event Replay Semantics, Budget Ledger, SideEffect Delivery Semantics, Reconciliation State Machine, ContextAssemblyContract, PromptExecutionContract, DecisionInputBundle, HITL Responsibility Record, Memory Write Governance, LearningCandidate, EvaluationGate, Runtime Test Matrix; completed Phase 8d three-ring mapping, PlanBundle/PlanGraphBundle compatible semantics, platform.* and oapeflir.* event compatible layer, v4.0 Runtime table landing, LLM-as-Judge cannot override failures; updated §5, §13, §14, §17, §25, §26, §28, §45, §58, §33, §34, §35, §36, §70, and Appendix G/H. |
| v4.1 | 2026-04-27 | **OAPEFLIR-Harness Convergence Version + Minimum Production Closed Loop Version**: Clarified HarnessRuntime as the only executable runtime, HarnessRun as the only authoritative Run; OAPEFLIR only provides StageRationale / TraceProjection / Audit View; converged Replay to default Trace Replay and isolated Re-execution Replay; strengthened Budget atomic reservation, SideEffect revoked/expired and commit pre-revalidation, Panic ack/resume rules, TrustScore does not reduce inherent risk, hot path deterministic execution, multi-Region single leader writes, SLA default 99.95 ceiling, global call depth 8; updated §5, §10, §14, §18, §19, §28, §31, §37, §42, §45, §52, §54, §58, §60, §33-§36, and Appendix H. |
| v4.2 | 2026-04-27 | **Implementable Specification Convergence Version**: Unified v4.1/v4.4 authority relationship, clarified main text and runtime/schema/event registry as implementation authority; deprecated `ExecutionPlan` canonical name and converged to `PlanGraphBundle`; split `ControlDirective` into `OperationalDirective` / `DecisionDirective`; completed core runtime object API, Webhook retry, Outbox poller failover, SLO P95/P99 caliber, §25 read path matrix, §26 MVP 20-table ceiling, §29 Memory/Knowledge/Artifact/Learning contract, §33 MVP/Hardening/Enterprise three-ring roadmap and Appendix H conflict resolution rules. |
