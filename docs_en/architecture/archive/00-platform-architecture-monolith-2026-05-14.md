...
文档拆分目标：当前 `00-platform-architecture.md` 作为主索引和人类架构权威；后续权威内容应收敛为 `00-core-architecture.md`、`01-runtime-executable-contract.md`、`02-state-event-evidence.md`、`03-ai-ops-governance.md`、`04-domain-framework.md`、`05-enterprise-operations.md` 六份文档。拆分完成前，本文件保留稳定章节号和迁移索引。
...
RuntimeInvariant、SecurityInvariant、AuditInvariant 进入 `NonOverridableInvariantRegistry` 后不得被任何管理员、域 owner、Pack 或 emergency override 关闭。`super_admin` 只能提交 policy proposal 或收紧策略；临时例外必须通过 break-glass、dual control、forensic logging、expiry 和 post-review，并且不得绕过 RuntimeStateMachine、SideEffectManager、BudgetAllocator、Event/Audit append。

### Invariant Coverage Matrix

每条设计宪法和硬目标必须映射到可执行检查。`ArchitectureInvariantRegistry` 中缺少 `test_ref`、`failure_behavior`、`owner` 或 `phase` 的条目只能作为设计意图，不能作为验收项或发布门禁。

| 原则 / 硬目标 | Invariant ID | Enforcement point | Test ref | Failure behavior | Owner | Phase |
| --- | --- | --- | --- | --- | --- | --- |
| 状态与证据同等重要 | INV-STATE-001 | StateStore.UnitOfWork | `tests/invariants/truth-event-atomicity.test.ts` | reject mutation + incident | Runtime | MVP |
| HarnessRuntime 唯一执行入口 | INV-RUN-001 | AdmissionController + DispatchGuard | `tests/invariants/harness-run-authority.test.ts` | reject bypass dispatch | Runtime | MVP |
| PlanGraphBundle 是 P3→P4 唯一执行契约 | INV-GRAPH-001 | PlanGraph validator + dispatch schema | `tests/invariants/plan-graph-only-dispatch.test.ts` | reject dispatch | Orchestration | MVP |
| Budget reserve 先于任何执行成本 | INV-BUDGET-001 | BudgetLedger guard | `tests/invariants/budget-reserve-before-execute.test.ts` | fail closed + release partial reservation | Finance Platform | MVP |
...
| 默认安全收敛 | INV-POLICY-001 | PolicyEngine + CapabilityGate | `tests/invariants/deny-by-default.test.ts` | deny / require approval | Security | MVP |
| 高风险域责任边界 | INV-DOMAIN-001 | DomainRiskSpec validator | `tests/invariants/high-risk-domain-boundary.test.ts` | block domain release | Domain Platform | Hardening |
...
`docs_zh/reviews/architecture-design-review.md` 中的设计审查项必须按“权威契约 → enforcement point → test gate”吸收。本文不以散落建议作为实现依据；下表是后续 ADR、contract、代码与测试的入口索引。

| Review 主题 | 架构决议 | 正文落点 | 必需门禁 |
| --- | --- | --- | --- |
| OAPEFLIR vs Harness 状态权威 | HarnessRuntime 是唯一运行时状态权威；OAPEFLIR 只是语义投影 | §13、§45.22、§58.6 | 状态机 invariant + projection consumer test |
| MVP 与长期路线 | 先交付 P1 + P3 + P4 + P5 最小闭环，Enterprise / Marketplace / Multi-Region 后置 | §1.4、§33、三环优先级 | MVP closure checklist |
| SLA 与故障转移 | 默认 99.95；99.99 只能绑定自动 failover、quorum 与演练证据 | §31、§52、§54 | DR drill pass/fail + SLA eligibility test |
| 调用深度矛盾 | `call_depth` 全局硬上限 = 8，不与目标分解和委托局部上限相乘 | §19.2、§40 | delegation depth test |
| Phase 1 存储范围 | SQLite 只承载 MVP 表子集；完整逻辑模型随 Phase 启用 | §26.2、§26.3、§33 | migration subset test |
| 跨 Region truth 写入 | truth / budget / side effect 只允许单 leader 写；CRDT 仅用于非关键统计 | §25.11、§52.3 | fencing epoch + failover reconciliation test |
| 低延迟业务 vs LLM 调用 | 实时/量化热路径使用离线规划 + 确定性执行，LLM 不在微秒级热路径内 | §15、§37、§71 | domain latency mode validation |
| Replay 确定性 | Trace Replay 只重放记录事实；Re-execution Replay 必须声明不可确定性 | §45.24、§58.4、§65 | no-side-effect replay test |
| 预算竞态 | 所有成本资源先原子 reserve，再执行，再 settle/release | §18.3、§25.9、§36.2 | concurrent hard-cap test |
| TrustScore 权限边界 | TrustScore 只减少审批摩擦，不降低 inherent risk 或扩大权限 | §42.5、§10 | risk policy test |

### Systemic Failure Mode Guardrails

以下系统性主题是不可忽略的架构缺口，必须在实现中显式建模：

- **多步治理操作必须 Saga 化**：OrgTree 级联、Chinese Wall 解除、治理委托撤销、SCIM deprovisioning、approval reroute 均必须包含 prepare / commit / compensate / audit 四段语义，禁止“部分完成但无补偿”的治理写入。
- **正反馈环路必须可断路**：Memory 自增强、Guardrail replan 振荡、Priority auto-upgrade 膨胀、Trust 维护型低价值任务必须有 `loop_counter`、`cooldown`、`promotion_budget`、`human_breaker` 或统计异常检测。
- **瓶颈必须有背压模型**：Evaluator、BudgetAllocator、ApprovalQueue、DispatchQueue 都必须声明 `max_queue_depth`、准入拒绝语义、降级模式和 DLQ/incident 路径。
- **资源释放必须终态驱动**：Run 终态、NodeRun 终态、Plugin 崩溃、TaskDraft 过期、Secret lease、ContextSnapshot retention 均必须由 sweeper 或 terminal cleanup 协议关闭。
...
所有 high/critical 域必须在 DomainRiskSpec 中显式声明 `advisory_only`、`human_accountable`、`deterministic_hot_path_only` 或等价责任边界；未声明时平台按更保守模式处理，默认不允许 full_auto。
...
P1 必须暴露标准化的 API 契约（见 §6）。自然语言、Webhook、UI 表单或外部事件不得直接生成可执行 RequestEnvelope；必须先进入 `RawInput → TaskDraft → ClarificationSession → ConfirmedTaskSpec → RequestEnvelope` intake pipeline。只有 `ConfirmedTaskSpec` 可生成 RequestEnvelope，且必须包含 trace_id、idempotency_key、principal、tenant_id。
...
P2 通过 `OperationalDirective` 或 `DecisionDirective` 向 P3/P4 发送指令，而非直接操作底层状态。旧名 `ControlDirective` 仅作为 deprecated alias，不得出现在新 schema、API 或事件中。

## 4.4 P3 Orchestration Plane

编排与决策层。

**包含**：P3a HarnessRuntime 协调入口 / P3b Planning Services / P3c Evaluation Services / P3d Routing & Escalation Services / OAPEFLIR trace projection adapter

**职责**：决定做什么 · 决定下一步谁执行 · 决定何时暂停 · 决定何时转人工 · 决定何时重规划、降级、终止

P3 输出标准化的 `PlanGraphBundle`（见 §13）。`ExecutionPlan` 是 deprecated alias，只允许出现在历史兼容适配器、术语表和迁移说明中；新实现不得消费线性 `steps`。
...
P4 必须通过 `NodeAttemptReceipt` 向 P3/P5 回报执行结果，Receipt 以 `attemptId` + `nodeRunId` 为主键，包含 harnessRunId / planGraphId / graphVersion / nodeRunId / attemptId / status / duration / side_effects / evidence_refs / error_detail。旧 `ExecutionReceipt` 与 `stepId` 只允许出现在 legacy adapter 或 projection，不得作为 P4 执行回执字段。
...
P5 内部必须按生命周期和一致性边界分模块：P5a truth-store、P5b event-store、P5c projection-store、P5d artifact-store、P5e audit-store。共享 Repository facade 可以统一暴露，但事务语义、retention、GC、legal hold 和 rebuild 策略不得混写。
...
**定位**：这不是辅助能力，而是平台的基础生命支持系统。X1 能力必须声明落地形态：library middleware、sidecar / interceptor 或 central service。默认优先 library / interceptor；只有跨进程共享状态、全局配额、Panic、密钥租约等需要集中协调的能力才做 central service。
...
| P2 → P3         | `OperationalDirective` | 同步/事件 | 模式切换、暂停、恢复、配额调整、rollout 控制                |
| HITL/Approval → P3/P4 | `DecisionDirective` | 同步/事件 | approve / deny / override / expire_approval 等业务裁决      |
| P3 → P4         | `PlanGraphDispatch` (`PlanGraphBundle`) | 同步 | 编排层输出给执行层的 canonical 图计划                       |
| P4 → P3         | `NodeAttemptReceipt` | 同步      | NodeAttempt 执行结果回报给编排层                           |
| P4 → P5         | `TransitionCommand` (`RuntimeStateMachine.transition`) | 同步  | 推进 truth 状态，必须同事务追加 platform fact event          |
| P4/P5 → EventLog | `EventAppendCommand` | 同步/事务 | 追加事实事件或投影事件                                     |
| P4/P5 → Audit   | `AuditAppendCommand` | 同步/事务 | 追加审计记录                                               |
| P3 → P5         | `EvidenceRecord`   | 异步      | 决策证据写入                                               |
| P2 → P4         | `OperationalDirective(type=kill)` | 同步 | 仅限紧急制动直达执行层（§60）                              |
| P5 → P2         | `ProjectionUpdate` | 事件      | Projection 变化通知控制面                                  |
| 任意 → X1       | middleware 注入    | 切面      | 不通过显式调用，通过装饰器/拦截器                          |

### ContractEnvelope

所有平面间命令、事件和审计追加均使用统一 envelope，业务 payload 只放在 `body` 中：
...
| issuedBy / issuedAt / expiresAt | 必填，过期命令必须拒绝 |
| idempotencyKey | 写操作必填 |
| signature | 跨进程或跨信任边界必填 |

## 5.3 核心契约对象定义

### Intake Pipeline 与 RequestEnvelope

P1 → P2 的标准请求信封只接收已确认任务。入站请求必须按以下状态推进：

```text
RawInput → TaskDraft → ClarificationSession → ConfirmedTaskSpec → RequestEnvelope
```

`RawInput` 可以来自自然语言、Webhook、UI、CLI 或定时触发；`TaskDraft` 只用于澄清、风险预览和草稿保存；`ClarificationSession` 收集缺失意图与高风险确认；`ConfirmedTaskSpec` 是唯一可转换为 RequestEnvelope 的前置对象。high/critical 任务必须有显式 `UserConfirmationReceipt`，low 风险可按 `ambiguity_policy` 使用安全默认值，medium 需要确认关键参数。

RequestEnvelope 封装已确认任务的元信息与任务规格。

| 字段         | 类型          | 说明                                     |
| ------------ | ------------- | ---------------------------------------- |
| requestId    | string (UUID) | 请求唯一标识                             |
| tenantId     | string        | 租户 ID，用于多租户隔离                  |
| confirmedTaskSpecId | string | 关联 ConfirmedTaskSpec，不直接承接 RawInput |
| taskSpec     | ConfirmedTaskSpec | 已确认任务规格（目标、输入、约束）       |
...
v4.2 起 `ControlDirective` 仅为 deprecated alias。新契约必须明确使用以下两类对象，避免把运行控制与业务裁决混在同一枚举中：
...
P3 → P4 的标准执行计划。`PlanGraphBundle` 是 canonical contract；`ExecutionPlan` 不再作为新实现契约名称，只能作为 deprecated alias 映射到 `PlanGraphBundle`。所有任务，包括简单任务，都以 PlanGraph 形式下发给 P4；简单任务退化为单节点图。Graph Scheduler 按确定性策略调度 NodeRun。
...
P4 → P3/P5 的执行回执，记录单次 NodeAttempt 的执行结果与遥测数据。Receipt 不再以 step 为权威对象；HarnessStep 只是语义/展示层，NodeRun / NodeAttempt 才是执行层事实。旧 `ExecutionReceipt` 仅由 legacy adapter 派生。
...
`RuntimeStateMachine.transition(command)` 是 HarnessRun / NodeRun / SideEffect / Budget 状态推进的唯一正式入口。P2/P3/P4/Recovery/HITL 不得直接更新 truth 表；它们只能提交 TransitionCommand，由 RuntimeStateMachine 统一校验状态机、CAS、active lease、fencing token、RunVersionLock、policy guard、budget precondition 和 side-effect safety。
...
| transition      | enum          | 状态迁移动作                                  |
| expectedStatus  | enum          | 期望状态                                      |
| nextStatus      | enum          | 目标状态                                      |
| leaseId         | string?       | 执行态迁移必填                                |
| fencingToken    | string?       | 执行态迁移必填                                |
| event           | EventEnvelope | 与 truth mutation 同事务追加的事实事件        |
| payload         | JSON          | 写入数据                                      |
| expectedVersion | number        | 期望版本号（CAS 乐观锁）                      |
| principal       | Principal     | 操作发起人身份                                |
| traceId         | string        | 关联追踪 ID                                   |

旧 `StateMutationCommand` 只允许作为 `RuntimeStateMachine.transition(command)` 的内部兼容 wrapper，不得作为新模块直接写 truth 的公共 API。

### EventAppendCommand / AuditAppendCommand / ArtifactWriteCommand

| 命令 | 事务语义 | 说明 |
| --- | --- | --- |
| EventAppendCommand | 与 truth mutation 同事务或 outbox 同事务 | 事实事件和 OAPEFLIR 投影事件都必须注册到 Event Registry |
| AuditAppendCommand | append-only，允许与业务事务同提交 | 记录 who / what / why / policy outcome |
| ArtifactWriteCommand | content-addressed，先写 artifact 后写 ref | 大对象不得内联到 event 或 truth payload |

## 5.4 契约遵守规则

1. **不可绕过**：P1 不可跳过 P2 直接调 P4
2. **P5 被动原则**：P5 不可向 P3/P4 发指令（只能被读/被写）；P5 → P2 仅限 ProjectionUpdate 事件通知，不可发送 OperationalDirective 或触发状态变更
3. **P2 → P4 仅限紧急通道**：P2 在紧急制动(§60)场景下可绕过 P3 直达 P4 发送 `OperationalDirective(type=kill)`，此路径仅限 `PlatformPanicDirective` 场景，常规指令仍须经 P3 编排
4. **必须签名**：每个契约对象必须包含 principal 和 trace_id
5. **必须幂等**：所有 StateMutationCommand 必须基于 expected_version 做 CAS
6. **必须可重放**：所有契约对象必须可序列化为 JSON

## 5.5 Canonical Runtime Object Map

| 对象 | 权威状态 | 唯一职责 | 非权威/legacy 用法 |
| --- | --- | --- | --- |
| TaskDraft | canonical pre-admission | NL / UI 草稿、澄清、风险预览 | 不得进入 P4 |
| RequestEnvelope | canonical ingress | 已确认请求进入 P1/P2 的标准信封 | 不承载执行状态 |
| HarnessRun | canonical run truth | 一次完整运行的唯一权威 Run | workflow_run 仅为 query projection |
| HarnessStep | semantic projection | 面向用户/解释/产品的语义步骤 | P4 不消费 HarnessStep 执行 |
| PlanBundle | product/debug wrapper | goal、taskGraph、successCriteria 的产品包装 | 不作为 P3→P4 执行契约 |
| PlanGraphBundle | canonical execution contract | P3→P4 唯一执行计划 | ExecutionPlan 为 deprecated alias |
| NodeRun | canonical execution truth | 可租约、可重试、可审计的最小执行单元 | stepId 仅 legacy projection |
| NodeAttempt | canonical attempt lineage | retry/redrive 的追加式 attempt | 不覆盖 NodeRun 历史 |
| SideEffectRecord | canonical side effect truth | 外部副作用生命周期与对账事实 | 工具成功不等于 side effect 成功 |
| BudgetReservation | canonical budget gate | LLM/Tool/SideEffect/Eval 前置预算硬门 | 发票 reconciliation 不是准入依据 |
| NodeAttemptReceipt | canonical P4 result | NodeAttempt 结果回执 | 旧 ExecutionReceipt 仅 legacy adapter |
| EventEnvelope | canonical event fact | 状态变更、审计、projection rebuild 输入 | `oapeflir.view.*` / `oapeflir.rationale.*` 只做投影事件 |
...
- 资源命名使用 kebab-case 复数形式；canonical 执行入口使用 `/api/v1/harness-runs`
...
| 资源                               | 方法                | 说明                   |
| ---------------------------------- | ------------------- | ---------------------- |
| `/api/v1/harness-runs`             | POST / GET          | canonical run 创建与查询 |
| `/api/v1/harness-runs/{id}`        | GET                 | 查询 HarnessRun truth / summary |
| `/api/v1/harness-runs/{id}/abort-requests` | POST      | 请求安全 abort，常规取消语义 |
| `/api/v1/harness-runs/{id}/pause-requests` | POST      | 请求 safe pause，不等同 panic kill |
| `/api/v1/tasks`                    | POST / GET          | compatibility layer；创建后必须转为 HarnessRun |
| `/api/v1/tasks/{id}`               | GET                 | 查询 TaskDraft / compatibility projection |
| `/api/v1/workflow-runs`            | GET                 | legacy query projection，只读 |
| `/api/v1/workflow-runs/{id}`       | GET                 | legacy query projection，只读 |
| `/api/v1/harness-runs/{id}/plan-graph` | GET             | 查询权威 PlanGraphBundle |
| `/api/v1/harness-runs/{id}/node-runs` | GET              | 查询 NodeRun 列表与状态 |
| `/api/v1/harness-runs/{id}/side-effects` | GET           | 查询副作用记录与对账状态 |
| `/api/v1/harness-runs/{id}/budget-reservations` | GET    | 查询预算预留、settlement 与释放 |
| `/api/v1/harness-runs/{id}/node-runs/{nodeRunId}/attempts` | GET | 查询 NodeAttempt 与 NodeAttemptReceipt |
| `/api/v1/harness-runs/{id}/forensic-snapshot` | GET      | 查询取证快照引用       |
| `/api/v1/replay-sessions`          | POST                | 创建 Trace / Re-execution Replay |
| `/api/v1/replay-sessions/{id}`     | GET                 | 查询 ReplaySession     |
...
`/api/v1/workflow-runs/{id}/steps` 仅作为 legacy projection 保留；新控制台和 SDK 必须优先读取 NodeRun 与 PlanGraph。`DELETE /tasks/{id}` 不再表达模糊取消语义；调用方必须选择 abort、pause 或 panic kill 的显式控制端点。

## 6.8 Canonical API vs Legacy Projection

| 类别 | 端点 | 语义 |
| --- | --- | --- |
| Canonical execution | `POST /api/v1/harness-runs` | 唯一创建 HarnessRun 的执行入口 |
...
| Compatibility | `/api/v1/tasks` | 兼容旧调用方；必须落为 TaskDraft 或 HarnessRun |
| Legacy projection | `/api/v1/workflow-runs*` | 只读查询投影，不得作为 truth 或执行入口 |

API 兼容层不得把 `workflow_run`、`task` 或 `stepId` 重新提升为权威对象。所有 compatibility handler 必须在入口处生成或解析 `harnessRunId`、`planGraphId`、`nodeRunId`，并在响应中用 `legacyProjection=true` 标记旧字段来源。
...
- Schema diff gate 必须阻止 deprecated term 进入新写路径；CI 必须包含 `contract-naming-consistency.test.ts`，扫描 OpenAPI、Zod schema、Event Registry、SDK 类型和 runtime-contracts，确保 `ExecutionPlan`、`ControlDirective`、`StateCommand`、`stepId` 只出现在术语表、迁移说明或 legacy projection adapter 中。
...
| 场景         | 认证方式                               | 说明                         |
| ------------ | -------------------------------------- | ---------------------------- |
| 服务间调用   | API Key（Header: `X-API-Key`）         | 有到期时间、作用域和最后使用时间，按 tenant 颁发 |
...
**Token 生命周期**：access_token TTL = 15min, refresh_token TTL = 24h。API key 必须声明 `expiresAt`、`scopes`、`lastUsedAt`、`createdBy` 和 `rotationPolicy`；异常来源、泄露迹象或 scope 漂移触发自动吊销，手动轮换只是补充能力。
...
- 目标返回 2xx 视为成功，否则按 retry policy 处理

Webhook retry policy：

| 响应/错误 | 策略 |
| --- | --- |
| 429 | 遵守 `Retry-After`，缺失时按 5xx 策略 |
| 5xx / timeout | exponential backoff + jitter，最大间隔 15min |
| 4xx permanent | 连续 10 次禁用 subscription，通知 tenant 管理员 |
...
- `last_event_id` 在 24h 保留窗口内时执行 delta replay
- `last_event_id` 过期时返回 snapshot + delta 恢复指令
- 检测到事件 gap 时发送 `stream_gap` 事件，客户端必须重新同步
- WebSocket / SSE subscription 必须绑定 authenticated principal、tenantId、allowed domains 和 projection scope；服务端逐事件执行 tenantId / scope 过滤，不得只依赖客户端过滤。跨租户事件命中订阅过滤器时必须丢弃并生成 security telemetry。
...
standby poller 只能在 lease TTL 过期且 fencing epoch 更新后接管。Projection lag ≤5s 是正常路径目标；poller failover 后必须在 `max_delivery_gap_p99_seconds` 内恢复投递，否则触发 incident。

高吞吐部署必须使用 partitioned outbox：按 tenant / aggregate shard 分区，每个 shard 独立 lease、fencing token 和 retry cursor。ordering 只在同一 aggregate 内保证；跨 aggregate 不承诺全序，只通过 causationId、correlationId 和 occurredAt 做因果关联。
...
WorkerDrainProtocol 是扩缩容、发布和 panic 传播的必备协议：worker 进入 `draining` 后不得领取新 lease；可 checkpoint 的 NodeRun 先写 checkpoint 再释放 lease；处于 side effect commit window 的 NodeRun 只能完成当前 fenced commit 或进入 reconciliation，不得被硬杀；超过 drain deadline 后由 `RunTerminationCleanup` 和 RecoveryWorker 接管。每次 drain 必须记录 drain reason、deadline、active leases、forced handoff count 和 cleanup result。
...
| hot_partition_detection | 热点阈值、窗口、最小样本量和 owner |
...
| rebalance_write_strategy | rebalancing 期间写入 leader、fencing 和幂等处理 |
| rollback_policy | cutover 失败后的恢复、事件补偿和 projection rebuild 策略 |
...
| S3a 分布式 | 微服务 + PostgreSQL + event bus | 200 并发, 50 worker       | 对应 Hardening 压测基线                                                          |
| S3b 分布式 | 微服务 + PostgreSQL + event bus | 500 并发, 100 worker      | 对应 Enterprise 前置压测                                                         |
| S4 集群   | Kubernetes + PG 分片 + 多 AZ    | 5000+ 并发                 | 需要多租户调度器(§53) + 跨 Pod 协调机制 + 多 Region 部署(§52)，对应落地 Phase 6 |

分片再均衡必须遵循 `ShardRebalanceProtocol`：detect hot partition → create split plan → shadow route → dual read compare → cutover → cleanup。状态性 worker（Browser Session、long-running tool、EdgeRuntime）必须声明 `stateful=true`、`lease_migration_supported` 和 `checkpoint_required_before_preempt`，不得被普通无状态调度假设覆盖。
...
当某 tenant 的 failure rate > 30% 且满足 `min_sample_size` 时，自动将该 tenant 隔离到独立 worker pool，不影响其他 tenant。低流量租户不因 1-2 次失败被自动隔离；样本不足时只告警和降速。
...
限流必须按 endpoint class 声明，不得让昂贵端点和廉价查询共享同一桶。至少区分：`read_query`、`create_run`、`control_command`、`side_effect_commit`、`webhook_ingress`、`admin_mutation`、`debug_replay`。每类端点必须声明 `rate_limit`、`burst`、`cost_weight`、`max_queue_depth`、`overflow_policy` 和 `retry_after_policy`。
...
队列背压不得只看 lag；Admission Controller 必须同时检查 `queue_depth >= max_queue_depth`、oldest age、worker health、approval capacity、budget allocator latency 和 evaluator capacity。达到 `max_queue_depth` 时必须 fail closed：低优先级请求返回 429 / `retry_after`，不可丢弃的内部事件进入 bounded DLQ 并生成 incident。不同瓶颈使用不同降级：Evaluator 瓶颈降级到 deterministic validators + HITL；Budget 瓶颈拒绝新成本预留；Approval 瓶颈暂停高风险新 run；Dispatch 瓶颈停止非关键 workflow admission。
...
模式切换通过 `OperationalDirective` 发出，支持自动触发规则：
...
`RiskCalibrationGuide` 必须同时给出归一化 0 / 25 / 50 / 75 / 100 锚点和实现侧 0 / 1 / 3 / 5 打分表，含正反例、审批人、证据要求、领域覆写理由和复核 owner。跨域放松风险锚点必须走 P2 policy approval；high/critical 域每季度校准一次。

| factor_value | 归一化锚点 | 含义 | 证据要求 |
| --- | --- | --- | --- |
| 0 | 0 | 无实质影响、无敏感数据、无外部写 | 自动记录 |
| 1 | 25 | 可逆低影响或只读敏感上下文 | evidence_ref + policy check |
| 3 | 50/75 | 有限外部写、有限财务/合规影响、可补偿 | HITL 或 deterministic guard |
| 5 | 100 | 不可逆、高金额、受监管、跨租户或安全影响 | high/critical gate + 责任记录 |

## 10.3 风险自动控制引擎

v4.2 将固有风险、信任分与审批策略拆开建模：
...
TrustScore 只允许影响：low-risk confirmation frequency、同一 risk tier 内的 queue priority、post-execution sampling rate。不得影响：approval required threshold、sandbox level、egress policy、budget hard cap、不可逆副作用确认。
...
| high       | ❌       | error    | 必须        | 受限        | 完整     |
| critical   | ❌       | critical | break-glass | 默认禁止；仅允许受限可追责动作 | 法务级   |

critical 默认 deny。break-glass 只能在限时、限 scope、双人审批、forensic logging、事后复盘的条件下允许受控动作请求，不得绕过 SideEffectManager、BudgetAllocator、RuntimeStateMachine 或 NonOverridableInvariantRegistry；不可逆副作用仍需 confirmation / reconciliation / compensation 路径。
...
每个 sandbox tier 必须声明 Linux / macOS / Windows / Kubernetes 的可实现方式、不可用时降级策略和逃逸测试用例。无法提供等价隔离时只能降级到更严格模式或拒绝执行，不得静默使用 host full access。
...
插件崩溃或 quarantine 必须触发 cleanup hook：关闭文件句柄、socket、临时目录、浏览器会话、secret lease 和外部 callback subscription。cleanup hook 超时后由 PluginSupervisor 强制隔离进程，并向 `RunTerminationCleanup` 追加 plugin cleanup receipt；不得让崩溃插件继续持有外部资源至 TTL 自然过期。
...
| `harness.node_run.duration_ms`  | histogram | tenant, node_kind  | NodeRun 执行耗时 |
| `harness.node_attempt.failure_rate` | gauge | tenant, error_type | Attempt 失败率  |
| `harness.dispatch.queue_depth`  | gauge     | queue_class        | 队列深度        |
| `harness.dispatch.queue_max_depth` | gauge   | queue_class        | 队列硬上限      |
...
| `harness.circuit_breaker.state` | gauge     | target             | 断路器状态      |
| `harness.dlq.depth`             | gauge     | category           | DLQ 深度        |

Legacy `agent.*` / `workflow_run.*` / `step.*` metrics 只能由 compatibility adapter 从 harness metrics 派生，不得作为新告警、SLO 或容量规划的数据源。
...
| `crosscutting_fabric` | enum? | reliability / security / governance，仅 X1 日志使用 |
...
每个异常类必须绑定 `error_code_namespace`、`incident_severity`、`mode_switch_rule`、`owner_team`、`replay_allowed` 和 `side_effect_safe_to_replay`。DLQ redrive 可能重新触发外部副作用时，必须先进入 simulation 或完成幂等确认。
...
告警路由必须支持 dedupe、suppression、maintenance window 和 escalation cooldown；同一 root cause 的重复告警不得绕过冷却窗口直接升级。
...
> v4.2 收敛口径：OAPEFLIR 是 Observe → Assess → Plan → Execute → Feedback → Learn → Improve → Release 的认知与治理框架，不是执行引擎。唯一可执行运行时入口是 HarnessRuntime，唯一权威运行实体是 HarnessRun。OAPEFLIR 阶段只能作为 StageRationale、TraceProjection、Audit View 和解释视图存在，不拥有独立执行权。
...
OAPEFLIR 只允许产出以下投影对象：`StageRationale`、`AssessmentSummary`、`PlanRationale`、`FeedbackSummary`、`LearningCandidate`、`ImprovementProposal`、`ReleaseDecisionView`。禁止 OAPEFLIR 拥有 run status、step status、lease、retry counter、side effect commit state 或 budget state。
...
| Execute | 执行 Graph Node，调用工具 / LLM / HITL / Subgraph | NodeRun / NodeAttemptReceipt | 受控 |
...
| P2 Control | 提供策略、审批、预算、版本锁和发布治理 | EffectivePolicySnapshot、OperationalDirective、DecisionDirective、EvaluationGate |
...
受控循环必须显式建模为 `LoopNode`，不得用隐式 edge cycle 表达。LoopNode 必须声明 `max_iterations`、`budget_per_iteration`、`termination_condition`、`loop_evidence_ref`、`state_carryover_policy` 和 `timeout_action`；缺少任一字段时 Graph Validation 失败。
...
Worst-Path Analysis 在执行前估计最坏路径的时间、成本、token、工具调用、审批等待和补偿成本。未知概率按 worst-case；可选分支按最大成本路径；并行 join 按 max latency + sum cost；LoopNode 按 max_iterations 估算。若最坏路径超过 ConstraintPack 或 BudgetLedger 的硬上限，PlanGraph 不得执行，必须 replan 或 escalate。
...
| add_edge | 追加依赖边 | 不得形成未授权循环 |
| disable_edge | 禁用未执行路径上的边 | 必须证明不会跳过已提交副作用的补偿路径 |
| add_compensation_node | 追加补偿节点 | 必须关联 sideEffectId / compensationPlanRef |
| add_failure_path | 追加失败处理路径 | 必须连接到 failure terminal 或 HITL |
| mark_skipped | 标记未执行节点跳过 | 只能用于未 leased / running / terminal 节点 |
| append_subgraph | 追加子图 | 子图必须通过完整 Graph Validation |

GraphPatch schema 必须包含：`patchId`、`baseGraphVersion`、`newGraphVersion`、`operations[]`、`affectedExecutedNodes[]`、`affectedSideEffects[]`、`compatibilityClass`、`compensationPlanRef?`、`policyProofRef`、`auditRef`。`compatibilityClass` 只能是 `safe_append / requires_checkpoint_revalidation / requires_human_approval / incompatible_restart_required`。已执行节点、已有 NodeAttemptReceipt、confirmed/ambiguous SideEffect 的语义不得被改写。
...
HarnessRun / PlanGraph / NodeRun / NodeAttempt 生命周期 · dispatch / queue / worker 调度 · lease / fencing · executor 调用 · side effect 受控提交 · retry / timeout / recovery · mode-aware execution · NodeAttemptReceipt 回报 · platform fact 事件发射
...
| node risk class     | 高风险 node 分配到隔离 pool 或 HITL |
...
模式切换权归 P2 Control Plane，通过 `OperationalDirective(type: "mode_switch")` 下发。
...
DispatchQueue 必须是 bounded queue。`queue_depth >= max_queue_depth` 时不得继续接收普通 NodeRun；critical NodeRun 只能使用独立 emergency lane，且 lane 也必须有硬上限。超过硬上限的可重试调度请求进入 `dispatch_backpressure_rejected` 事件；不可丢弃事件写入 DLQ，等待 operator redrive。
...
终态集合为 `succeeded / failed / skipped / cancelled / dependency_failed / policy_blocked / aborted`。依赖未满足或调度未挑选的节点不得以 `blocked/queued` 伪装成可执行 NodeRun；此时应保持在 PlanGraph 调度视图中，待可执行后再实例化为 `NodeRun` 并进入 `ready`。`retry_wait` 是非终态，必须记录 `wakeAt`、`retryPolicyRef`、`attemptId` 和 backoff reason，到期后只能通过 RuntimeStateMachine 迁回 ready 或进入 failed/aborted。补偿不是 NodeRun 成功终态；补偿事实必须记录在 SideEffectRecord / CompensationRecord。终态不可迁出；retry / redrive 必须创建新 attempt 并追加 AttemptLineage。所有状态迁移必须基于 RuntimeStateMachine + CAS + active lease + fencing token。

实体边界：`HarnessStep` 是语义 step，可展开成一个或多个 `NodeRun`；`NodeRun` 是可执行图节点实例；`NodeAttempt` 是 `NodeRun` 下的 retry / redrive attempt。重试不得新建 NodeRun，除非 GraphPatch 明确追加新节点。

### RunTerminationCleanup

HarnessRun 或 NodeRun 进入终态后必须同步触发 `RunTerminationCleanup`，并以 cleanup receipt 作为终态证据的一部分。清理顺序固定为：
...
任一清理步骤失败不得回滚终态；必须生成 `cleanup_failed` incident，并由 RecoveryWorker / Sweeper 按幂等键重试。Secret lease 与 budget reservation 的释放必须与 cleanup event 同事务或通过 outbox 保证最终可见。

## 14.15 NodeRun Terminal Reason Codes

| reasonCode | 适用状态 | 说明 |
| --- | --- | --- |
| `success_criteria_met` | succeeded | 节点成功且验收条件满足 |
| `tool_error_retry_exhausted` | failed | 可重试工具错误已耗尽 |
| `schema_validation_failed` | failed / policy_blocked | 输出或输入 schema 不合法 |
| `policy_denied` | policy_blocked | P2 / Guardrail 明确拒绝 |
| `budget_exhausted` | failed / aborted | 预算耗尽且不可继续 |
| `dependency_failed` | dependency_failed | 上游必需节点失败 |
| `condition_not_met` | skipped | 分支条件未命中 |
| `human_cancelled` | cancelled | 人工取消或 takeover 后终止 |
| `panic_aborted` | aborted | PlatformPanic / kill 指令中止 |
| `compensation_completed_after_failure` | failed / aborted | 原动作失败或终止后，关联 CompensationRecord 已完成；NodeRun 状态不改写为 compensated |

失败、跳过、取消和策略阻断必须用 terminal reason code 区分，禁止把所有非成功统一塞入 `failed`。
...
补偿失败不得删除原始 side effect；必须保留原始证据、补偿尝试、外部响应和人工确认记录。
...
运行时必须周期性执行 `ConfigDriftReconciler`：对比 versioned defaults、环境覆盖、租户覆盖、实际进程加载值和 RunVersionLock 中的冻结值。发现未授权漂移、热加载失败、不同 worker pool 配置不一致或 emergency override 过期未恢复时，生成 `config.drift_detected` incident；影响安全、预算、egress、sandbox 或 approval 的漂移必须 fail closed。
...
`ConfigImpactAnalyzer` 是高风险配置发布前的强制门禁，必须列出受影响 tenant、active HarnessRun、domain、pack、connector、policy、budget 和 approval route。若影响 high/critical run、egress、sandbox、secret、budget 或 policy strictness，必须阻止直接全量发布，改走 canary / checkpoint revalidation / explicit override。
...
所有 truth 写入必须通过唯一事务入口：

```text
RuntimeStateMachine.transition(command)
```

该入口必须在同一事务内校验 expectedStatus、active lease、fencing token、RunVersionLock、policy guard、budget precondition 和 side-effect safety，并同步写入 truth update + platform fact event + outbox + audit ref。绕过该入口直接更新 truth 表视为 invariant violation。
...
CAS 热点重试策略：`max_retries=3`，backoff = `20ms, 50ms, 100ms + jitter`；仍失败则 requeue scheduler tick，不得忙等。

## 25.7 Schema 迁移策略

当前 `SchemaInventoryService` 会从 authoritative schema 与 extension DDL 汇总完整逻辑表 inventory（§26.3），但 MVP 交付不得以全量 inventory 为验收口径。Ring 1 只允许使用 §26.5 的 MVP 表集；其余表必须标注 Phase/Ring 后再迁移。
...
v4.2 将 HarnessRun 与 NodeRun 状态作为 P5 truth 的一级对象。OAPEFLIR 阶段状态只作为 `OapeflirTraceProjection` 派生，不参与可执行状态迁移。所有状态迁移必须满足：
...
`NodeRun` 标准状态：created · ready · leased · running · retry_wait · awaiting_hitl · reconciling · succeeded · failed · skipped · cancelled · dependency_failed · policy_blocked · aborted。

终态封闭规则：completed / failed / aborted / succeeded / skipped / cancelled / dependency_failed / policy_blocked 作为终态后不得迁出；任何修复必须以 redrive、compensation、GraphPatch 或 child run 追加方式表达。`retry_wait`、`awaiting_hitl`、`reconciling` 是非终态等待态，必须有 wake condition 或 external resolution 记录。
...
每个 HarnessRun 在 admitted 时冻结 `RunVersionLock`，锁定本次运行使用的 Prompt、Policy、Tool、Domain、Model、Eval、Guardrail、RuntimeProfile 和 schema version。运行中配置发布不得改变已运行 run 的语义；只能通过显式 GraphPatch、OperationalDirective 或 redrive 使用新版本。
...
> 当前仓内通过 `src/platform/five-plane-state-evidence/truth/schema-inventory-service.ts` 维护 authoritative table inventory。文档保留两种互补视图：一套服务于迁移执行，一套服务于架构沟通。
...
**完整 inventory 当前为 86 张逻辑表**。该数字只用于架构沟通和 schema inventory golden test，不是 MVP 建表目标。实现时按 `category`、`documentedGroup` 和 delivery ring 渐进迁移；未标注 Ring/Phase、owner、migration、rollback 和 test_ref 的表不得进入主线 migration。

## 26.4 v4.2 Runtime 表落点

v4.2 收敛后的 Harness / OAPEFLIR 运行对象必须在存储层有明确落点。实现时优先复用现有 truth / event / execution 表；确需物化为新表时，必须同步更新 `SchemaInventoryService`、migration 版本、§26.3 inventory 和回滚脚本。

| 运行实体 | 推荐表 / Repository | 存储策略 | 所属 Group | 迁移阶段 |
| --- | --- | --- | --- | --- |
| HarnessRun | `harness_run` | 唯一权威 run truth；OAPEFLIR 只生成 trace projection | Group 1 | E1 |
| NodeRun | `node_run` | NodeRun 作为最小执行 truth；旧 step projection 由 adapter 派生 | Group 1 | E1 |
...
| BudgetLedger | `budget_ledger` + `budget_reservation` | reservation / consume / release 同事务写 event；MVP 真实 LLM/tool 前置 | Group 2 / Group 5 | E1 |
| RunVersionLock | `run_version_lock` | admitted 时冻结，后续只读 | Group 7 | E1 |
| SideEffectRecord | `side_effect` | proposed→confirmed 状态机，外部幂等键唯一索引 | Group 1 / Group 4 | E1 |
| ReconciliationRecord | `reconciliation_record` | ambiguous 后创建，对账结果 append-only；真实外部写前必须上线 | Group 4 | E2 |
| DecisionInputBundle | artifact ref + `decision_record` | bundle 大对象入 artifact，裁决摘要入 truth | Group 2 / Group 3 | E1/E2 |
...
- v4.3 第一批 migration 必须覆盖 HarnessRun、NodeRun、NodeAttempt、PlanGraphBundle、GraphPatch、BudgetLedger、BudgetReservation、Event Registry Metadata、RunVersionLock。
- BudgetLedger、SideEffectRecord 必须在任何 LLM/tool/side effect 执行前完成；ReconciliationRecord 必须在任何真实外部写操作上线前完成。
- 旧 workflow / execution 表进入兼容期后只允许 adapter 读写，不得再新增绕过 NodeRun 的执行路径。
- 若新表导致 inventory 统计变化，必须在同一 PR 更新 §26.3、版本历史和 schema inventory golden 测试。

## 26.5 MVP 表集裁剪

v4.3 MVP 不以 86 表全量模型为交付目标，而以 22 张以内完成最小生产闭环：
...
`task` 不作为 MVP truth 表；`task_draft` 和 `confirmed_task_spec` 是 pre-admission intake truth，用于阻止未确认自然语言直接进入 RequestEnvelope。`lease_record` 是 MVP 必需 truth，用于 NodeRun active lease、fencing token、过期扫描和 worker crash recovery。若必须严格 ≤20 张表，可合并 `task_draft + confirmed_task_spec`、`event_inbox + event_outbox`、`approval_request + decision_record`，但不建议删除 `lease_record`、`graph_patch` 或 `human_responsibility_record`。
...
MVP 第一批 migration 必须围绕 Intake / HarnessRun / PlanGraphBundle / GraphPatch / NodeRun / NodeAttempt / Lease / Event Inbox-Outbox / Budget / SideEffect / HITL / Audit 建立真实物理边界。下表是首批 schema 的最小验收，不要求一次实现 86 张逻辑表。
...
| `harness_run` | `harness_run_id`、`tenant_id`、`status`、`confirmed_task_spec_id`、`request_hash`、`constraint_pack_ref`、`version_lock_id` | `(tenant_id, status)`、`request_hash` 幂等索引 | `RuntimeStateMachine.transition` |
...
任何 MVP schema PR 必须同时提供 migration、rollback、repository contract test 和 `runtime-contracts` 对应 schema；不能只修改数据库或只修改文档。
...
| User-visible E2E SLA | P95/P99 by tier | 用户可感知端到端体验，包含 LLM/tool provider 延迟 |
| Provider Observed SLO | provider latency/error rate | LLM、工具、外部系统可用性和路由依据 |
...
以下 SLO 是 D3/S3b 以上部署的目标；D1/D2 不得被 Enterprise P99 卡死。MVP 验收优先看功能正确、状态一致、replay 可用和稳态阈值。
...
| Trace Replay rebuild | 可重建单 run | 可重建批量 run | 可重建 projection slice | 可支撑审计批量 replay |
...
| 维度          | S1 单体    | S2 多进程   | S3a 分布式 | S3b 分布式 | S4 集群 |
| ------------- | ---------- | ----------- | ---------- | ---------- | ------- |
| 并发 workflow | 10         | 50          | 200        | 500        | 1000+   |
...
**LLM 延迟不计入 Internal Platform SLO**，但必须计入 User-visible E2E SLA，并独立作为 Provider Observed SLO 监控。当 LLM P99 延迟 > 基线 200% 时，触发 ModelGateway 降级策略（见 §15.4）。
...
Event Registry 是平台事实事件与 OAPEFLIR 语义视图事件的类型、payload schema、replay 行为和 projection 消费规则注册表。所有事件必须可校验、可回放、可去重、可追溯。

硬规则：

- 事实事件名采用 `platform.object.action`，例如 `platform.node_run.succeeded`；OAPEFLIR 只能使用 `oapeflir.view.*` 或 `oapeflir.rationale.*`，不得使用看似 truth 的 `oapeflir.node.*`、`oapeflir.side_effect.*`、`oapeflir.budget.*`。
- 每个事件必须声明 schemaVersion、aggregateId、runId、sequence、traceId。
- 每个事件类型必须声明 `source_of_truth`、`replayable`、`side_effect_safe_to_replay`、`schema_owner` 和 `consumer_contract_tests`。
- run 内 sequence 单调递增；consumer 必须基于 eventId 去重。
- replayBehavior 必须显式声明：`replay_as_fact`、`skip_side_effect`、`simulate`、`forbidden`。
- 事件 append 与 truth update 同事务；outbox 投递失败进入 DLQ，不影响 truth 一致性。

## 28.2 Event Registry 分层与兼容策略

v4.3 后 Event Registry 分为三层：`EventEnvelope` 是统一事件信封，`platform.*` 保留平台级事实事件，`oapeflir.view.*` / `oapeflir.rationale.*` 只承载 OAPEFLIR 语义投影事件。所有层共享 eventId、traceId、replayBehavior、schemaVersion 和 projection 订阅模型，但 truth projection 的权威输入只能是 platform facts。
...
| OapeflirViewEvent (`oapeflir.view.*` / `oapeflir.rationale.*`) | stage、graph、attempt、decision、hitl、memory、eval、learning、semantic_view | OAPEFLIR 阶段解释、审计视图、语义投影；不得作为 truth source |

**兼容映射**：

| legacy / platform 事件 | v4.2 事实事件 | OAPEFLIR 投影 | 处理方式 |
| --- | --- | --- | --- |
| workflow_run.created / failed / completed | platform.harness_run.created / failed / completed | oapeflir.view.run_lifecycle.* | truth projection 消费 platform.harness_run；OAPEFLIR run 事件仅作生命周期解释 |
| step_run.* / execution.* | platform.node_run.* | oapeflir.view.node_lifecycle.* | 新实现以 NodeRun platform fact 为 truth；旧 step projection 由 adapter 派生 |
| step_attempt.* | platform.node_attempt.* | oapeflir.view.attempt_lineage.* | retry / redrive 统一转为 AttemptLineage，OAPEFLIR 只解释因果链 |
| tool_call.succeeded / failed | platform.tool_call.* | oapeflir.view.tool_output.* | 工具调用事实保留 platform 事件，节点语义投影由 adapter 派生 |
| side_effect.proposed / committed | platform.side_effect.* | oapeflir.view.side_effect.* | committed 后仍需 confirmed / reconciliation 才能作为成功事实 |
| decision.requested / approved | platform.decision.* / platform.approval_flow.* | oapeflir.rationale.decision.* / oapeflir.view.hitl.* | 自动裁决与人工责任记录拆分 |
...
| circuit_breaker.* / config.changed | platform.* | 无或 oapeflir.view.stage.* | 不迁入 OAPEFLIR truth，仅通过 OperationalDirective 影响 HarnessRun |

迁移硬规则：truth projection 必须优先订阅 `platform.harness_run.*`、`platform.node_run.*`、`platform.side_effect.*`、`platform.budget.*` 等事实事件；`oapeflir.view.*` / `oapeflir.rationale.*` 只能用于阶段解释、审计视图和语义投影。旧 projection 在迁移期通过 adapter 从 platform facts 派生 legacy 事件，adapter 事件必须标记 `derivedFromEventId`；同一事实不得被 platform 与 OAPEFLIR view 双重计数。
...
sequence 是 aggregate-local sequence，不是全局序号。并发 NodeRun 只能在同一 aggregate 内依赖 sequence 判断顺序；跨 aggregate 只能使用 causationId、correlationId、traceId 和 occurredAt 推断因果，不承诺全序。

## 28.4 OapeflirViewEventType 注册表

| 命名空间 | 代表事件 | 说明 |
| --- | --- | --- |
| oapeflir.view.run_lifecycle | created / admitted / paused / resumed / completed / failed / aborted | HarnessRun 生命周期投影事件，不代表独立 OAPEFLIR 运行实体 |
...
| oapeflir.view.node_lifecycle | ready / leased / started / succeeded / failed / awaiting_hitl / reconciling | NodeRun 语义视图 |
| oapeflir.view.attempt_lineage | started / retry_scheduled / redriven / exhausted | AttemptLineage |
| oapeflir.view.budget | reserved / consumed / released / exhausted | Budget 阶段视图 |
| oapeflir.view.side_effect | proposed / approved / committed / ambiguous / confirmed / compensation_required | 副作用治理视图 |
| oapeflir.view.reconciliation | started / matched / diverged / unknown / expired | 对账状态视图 |
| oapeflir.rationale.decision | input_frozen / accepted / retry_requested / replan_requested / escalated / aborted | Decision Engine 理由 |
| oapeflir.view.llm | response_recorded / schema_validated / guardrail_blocked | 已记录的 LLM 输出与校验结果 |
| oapeflir.view.tool_output | recorded / tainted / rejected | 已记录的工具输出与污染传播 |
| oapeflir.view.scheduler | decision_recorded / lease_assigned / ready_set_evaluated | 可回放的调度决策视图 |
| oapeflir.view.hitl | lock_acquired / requested / approved / rejected / overridden / takeover / timed_out | HITL Runtime 视图 |
| oapeflir.view.memory | write_requested / approved / rejected / promoted | Memory Governance 视图 |
| oapeflir.view.eval | gate_started / gate_passed / gate_failed / regression_detected | EvaluationGate 视图 |
| oapeflir.view.learning | candidate_created / quarantined / approved / rejected / released | LearningCandidate 视图 |
...
Projection rebuild 流程固定为：build shadow projection → compare counts/hash → cutover → retain old projection for rollback。rebuild 期间 API 必须返回 freshness/stale 标记，不得静默读取半成品 projection。
...
本节按四个 contract 管理：`MemoryContract`、`KnowledgeTrustContract`、`ArtifactContract`、`LearningCandidateContract`。四者都可被 P3/P4 读取，但只有 P5 truth/event/audit 是控制事实来源。
...
Knowledge contested 状态下默认仍可被检索，但必须降低 trust、强制显示争议来源，并禁止作为 high/critical 自动执行的唯一依据。authoritative 知识降级必须发出 `knowledge.trust_downgraded` 事件通知消费者。

## 29.2 Memory

运行态短中期上下文。会衰减 · 会压缩 · 会被覆盖 · 用于上下文装配。

Memory 分层明确为 6 层：working → session → episodic → semantic → procedural → meta。每层有独立的 TTL、压缩和淘汰策略。
...
ContextAssembly 必须输出 `ContextTruncationReport`，记录被压缩、被排除、被降权的 context refs、原因、数据等级和是否影响 high/critical 决策。working facts 不能静默丢弃；若 token budget 不足，必须要求 replan、summarize-with-evidence 或 escalate。

## 29.3 Artifact

执行产物与大对象，不承担控制真相职责。通过引用（artifact_ref）关联到 HarnessRun / NodeRun / NodeAttempt，不内联到 event。

Artifact 必须 immutable、content-addressed，并记录 hash、签名、creator、data_class、retention、legal_hold、provenance 和 redaction policy。对象存储 GC 不得删除仍被 event/audit/legal hold 引用的 artifact。

## 29.4 Learning

从反馈中提炼候选模式。Learn 不直接改变线上行为。LearningObject 必须经过 Improve → Validation → Approval → Rollout 才能生效。

LearningCandidate 默认进入 quarantine。候选样本必须通过污染防护、PII/secret scan、holdout 去重、多样性检查和人工/评测审批；未发布候选不得影响 production routing、Prompt 或 Memory promotion。
...
PackCapabilityProfile 为强 schema，不得用自由文本替代：
...
Pack / Plugin / Connector 生命周期边界：Pack 是业务能力包，Plugin 是受沙箱约束的扩展代码，Connector 是外部系统动作边界。Pack 可依赖 Plugin 和 Connector，但不得绕过 Connector action-level risk profile 或 Plugin 安全认证。

`BusinessPackManifest.v1` 必须是强 schema，至少包含：

| 字段 | 说明 |
| --- | --- |
| pack_id / version | Pack 身份与 semver |
| domain_id | 关联 Active DomainCoreDescriptor / DomainDescriptor |
| capabilities | 结构化 PackCapabilityProfile，不允许自由文本 |
| tools / connectors / plugins | 依赖项、版本范围、权限和 sandbox 要求 |
| side_effects | action-level SideEffect 类型、幂等键、补偿和对账策略 |
| data_classes | 可处理数据等级、PII/secret/regulated 标签 |
| eval_requirements | 发布前 eval dataset、golden set 和 denial-path 测试 |
...
Pack 兼容性测试套件由 Platform SDK team 拥有，并由 `PackCompatibilityTestGenerator` 从 Manifest、OpenAPI、Event Registry、Contract Schema 和 declared capabilities 生成。每个 Pack 发布时必须保存生成器版本、fixture 版本和测试报告；平台升级若无法生成或运行兼容性套件，升级门禁 fail closed。
...
HA-3 的 RPO=0 只适用于 in-region multi-AZ 同步复制；cross-region failover 默认 RPO>0，且只允许 metadata-only 预复制或按数据驻留策略复制。不得把跨 Region 数据驻留场景写成 RPO=0。
...
- **恢复校验**：每次恢复必须对 truth/event/projection 计数、hash、水位线和抽样业务对象做校验，并把报告写入 Evidence Plane
...
Event replay 不假设无限日志保留。P5 必须分层保留：truth/event 最小可恢复窗口、audit legal hold、artifact retention、projection 可重建缓存。GDPR / PIPL 删除通过 tombstone、不可逆摘要和 crypto-shredding 执行；tombstone 保留最小审计字段，不含 PII/PHI/secret，replay 时用于证明对象已删除而不是恢复原文。
...
部署形态必须显式声明 security isolation level、tenant isolation level、worker isolation level、supported risk tier 和 not supported capabilities；不得把单体阶段的逻辑隔离表述为生产强隔离。
...
适用：开发、测试、小规模生产（≤10 并发）。D1 只提供逻辑隔离，不支持 regulated critical domains、untrusted third-party plugin、multi-tenant strong isolation、跨租户共享市场和高风险浏览器/数据库写入自动化。
...
适用：中规模生产（≤50 并发），worker 可水平扩展。D2 支持 worker 级隔离和部分高风险池，但不承诺多 AZ 容灾、跨 Region failover 或第三方插件强沙箱。
...
适用：大规模生产（≤500 并发），各平面独立扩展。D3 才允许承载强多租户隔离、受监管高风险域、untrusted plugin sandbox、browser executor 独立池和跨 AZ HA；进入 S4/Kubernetes 多 Region 前必须完成状态迁移演练和 rollback plan。

| 部署形态 | security isolation | tenant isolation | worker isolation | supported risk tier | not supported capabilities |
| --- | --- | --- | --- | --- | --- |
| D1 | 进程内逻辑隔离 | 单租户/弱多租户 | 共享 worker | low/medium | critical regulated、untrusted plugin、强多租户 |
| D2 | 进程/worker 隔离 | tenant 逻辑分区 | 独立 worker pool | low/high（受限） | 多 AZ RPO/RTO、跨 Region、第三方强沙箱 |
| D3 | 服务/网络/池隔离 | tenant 分区 + policy enforcement | 高风险/浏览器/插件池隔离 | high/critical（需专用池） | 全球多 Region truth failover |
| S4 | Kubernetes + 多 AZ + 分片 | 强多租户 + home region | Pod/Sandbox/专属池 | enterprise critical | 需要 §52 多 Region 规则约束 |
...
每次 promotion 必须绑定 rollback runbook：回滚目标版本、schema rollback/forward-fix 策略、config rollback、worker drain、event consumer compatibility、projection rebuild 和 customer impact notice。D1 → D2 → D3 演进不是单向线性流程；任一环境的 guardrail breach 必须允许回退到上一可用部署形态或进入 read-only degraded。

Emergency hotfix 可绕过常规等待窗口，但不得绕过 security scan、schema compatibility、contract tests、dual approval、blast radius 限制和 post-deploy verification。hotfix promotion 必须声明 expiry、follow-up release 和 evidence bundle；未在窗口内补齐正式 release 时自动触发 incident。
...
- Worker pool 与 P1/P2/P3/P5 之间的跨进程通信必须使用 mTLS + service identity；每个 worker lease 绑定 identity、pool、capability 和 fencing token。身份不匹配或证书过期时不得领取 NodeRun。
...
| `stream()`   | modelId, messages/prompt, maxTokens, timeout, abortSignal | AsyncIterable<ModelStreamChunk> | 流式输出，chunk 必须带 sequence、usageDelta、finishReason、validationState |
| `embed()`    | modelId, input (string \| string[]), timeout         | EmbeddingResponse (vectors + usage) | 向量化，用于检索/相似度 |

ModelResponse 统一包含：`requestId`、`model`、`choices`、`usage { promptTokens, completionTokens, totalTokens, estimatedCost }` 和 `latencyMs`。ModelStreamChunk 必须支持 abort、incremental reserve/settle 和 partial validation；resume 只能恢复传输，不得把未校验 partial 当作业务输出。所有调用自动附加 traceId、tenantId、costTag，并纳入 §18 成本计量。
...
| D2 缓存  | 所有 provider unhealthy, 缓存可用 | 对精确缓存命中返回缓存结果；语义缓存默认不用于 MVP        |
| D3 静态  | 缓存不可用                        | 使用预置的 static fallback plan（仅低风险任务）           |
| D4 暂停  | 所有降级不可用                    | 暂停所有新 workflow，保护在途 workflow checkpoint，转人工 |

**缓存设计**：

- `ExactPromptCache`：基于 `prompt_ref + canonical params hash + model routing constraints`，只在输入完全规范化一致时命中
- `SemanticCache`：基于 embedding similarity + safe class + human-approved domain；必须有相似阈值、租户/数据域隔离、cache poisoning 防护和人工批准
- MVP 默认只实现 ExactPromptCache，禁止把“语义相似请求复用”当作确定性缓存使用
- TTL 按 data_classification 分级：public=1h, internal=15min, confidential=不缓存
- 缓存命中必须标记 `cached: true`，不计入模型质量评估

Cache warming 是启动门禁的一部分：每次发布、provider failover 或模型路由变更后，平台必须预热 health prompt、critical static fallback plan、低风险模板和 contract test prompts。缓存未预热不得把 D2/D3 作为可用降级能力计入 SLA；只能标记为 `degradation_unready` 并进入 D4/HITL 路径。
...
| 关注点         | 处理策略                                                                                                                                     |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| 流中断         | 已接收 token 只能作为 partial artifact 保存；是否可用由 Output Completeness Validator 判定，不得用长度比例启发式                              |
...
Streaming partial response 只有通过格式感知完整性校验后才可进入后续执行；否则只能作为调试 artifact，不得用于 PlanGraph、SQL、代码、决策或副作用。

| 输出类型 | 完整性校验 |
| --- | --- |
| JSON / schema output | JSON parse 成功 + Zod/schema valid |
| PlanGraphBundle | graph schema valid + 至少一个 entry/terminal + terminal reachable |
| SQL | parser/AST parse 成功 + 只读/写入策略校验 + statement 完整结束 |
| 代码/Patch | AST 或 patch parser 成功 + 文件边界完整 + 测试/扫描门禁可运行 |
| Markdown / 文本 | terminal marker 或 provider finish reason 正常；低置信 partial 必须显式标注 |
| HarnessDecision | 枚举合法 + DecisionInputBundle ref 存在 + precedence policy 可验证 |
...
PromptBundle 必须声明 `PromptBundleCompatibilityMatrix`，覆盖 Tool schema、Evaluator schema、DomainDescriptor schema 和 Model routing profile。rollback 或 revocation 默认只影响新 run；in-flight run 受 RunVersionLock 保护，除非触发 `BundleRevocationEvent` 并经 HITL / P2 emergency override。

`BundleRevocationSeverity`：

| severity | 影响范围 | 适用场景 |
| --- | --- | --- |
| soft_new_only | 只阻止新 admission | 普通质量回退 |
| checkpoint_switch | 下个 checkpoint revalidation 后切换 | medium 风险兼容修复 |
| forced_pause | high/critical in-flight run 暂停并等待复核 | 注入、泄露、坏 canary |
| forced_abort | 立即 abort 受影响 run | 已确认安全/合规事故 |

forced_pause / forced_abort 必须生成 platform fact event、incident link 和 affected HarnessRun 列表；不得静默改变已运行节点的 Prompt 语义。

Full prompt logging 遵循 `PromptLogRedactionPolicy`：日志和 artifact 中的 secret、PII、客户专有 payload 必须先脱敏或引用化，禁止为了调试完整记录敏感 prompt。
...
`PromptInjectionDefenseChain` 不只属于 Prompt 管理，必须联动 Tool Guardrails、Egress Control、Context Assembly 和 Output Validator。若 Injection Classifier 未有训练数据、误报率、漏报率和更新流程记录，则只能作为辅助信号；阻断必须以规则、权限、数据分级、egress、tool capability、sandbox 和最小权限工程防线为主。
...
**管理要求**：eval 集按风险分级确定最小规模；holdout 集仅质量门禁自动调用，禁止在开发/调试阶段使用；数据集变更需经 domain_owner 审批。

| 风险等级 | 最小 eval 样本 | 附加要求 |
| --- | --- | --- |
| low | ≥ 50 | 覆盖主路径和常见失败 |
| medium | ≥ 200 | 覆盖边界输入、权限拒绝和成本异常 |
| high | ≥ 500 | 增加对抗样本、人工标注复核和回归 holdout |
| critical | 领域专家集 + adversarial set + holdout（建议 ≥ 1000） | 必须包含监管/安全/误用案例，发布前人工签核 |
...
- Judge 独立性按风险分级：low 可同模型；medium 必须不同模型；high 必须不同 model family；critical 必须不同 provider + deterministic checks
- Judge 结果缓存（同一 input+output 不重复评估）
- Judge 调用本身有成本预算限制（见 §18）
- Judge 评估结果纳入质量门禁，但只能补充质量判断，不得覆盖确定性失败
- LLM-as-Judge 不能作为监管级证据，只能作为辅助质量信号；监管、财务、医疗、法务等关键结论必须绑定确定性检查、人工签核或外部权威证据。

当 Judge provider、judge model family 或 Evaluation Harness 不可用时，canary 不得继续自动放量。low/medium 风险可降级为 D1 deterministic checks + 人工抽样；high/critical 必须暂停 canary、保持旧版本或 rollback，直到独立 judge 恢复并补齐评测证据。
...
**计量点**：ModelGateway 在每次 LLM 调用完成后同步写入 UsageRecord，作为 provider usage evidence。运行预算事实以 BudgetLedger / BudgetSettlement 为准；chargeback 以 settlement 为财务事实，provider invoice 与 UsageRecord 只用于对账，不得反向提高预算硬上限。

`UsageRecord` 必须支持多币种、内部算力、人审和 provider 发票对账：

| 字段 | 说明 |
| --- | --- |
| currency | 原始费用币种 |
| fx_snapshot | 换算为 base_currency 时使用的汇率、来源和时间 |
| cost_source | provider_invoice / internal_compute / human_review / storage / egress |
| provider_invoice_reconciliation_id | 与 provider 发票或内部账单对账的引用 |
...
关键字段：reservationId、subjectId、runId、nodeRunId、attemptId、resource_type、estimatedCost、estimatedInputTokens、estimatedOutputTokens、expiresAt、status、settledCost、releasedCost、traceId。`resource_type` 必须是 money / model_tokens / context_tokens / output_tokens / tool_calls / human_review / duration_ms。预留到期由 Sweeper 释放，释放与事件写入必须同事务。Sweeper 必须使用数据库时间或 monotonic lease time，并保留 clock skew safety margin；不得因单个 worker 本地时钟快进而提前释放活跃预留。
...
热门租户预算账本必须支持 `BudgetAllocator` + `budget_sub_ledger` / `reservation_shard`，例如 `tenant_monthly_budget → tenant_budget_bucket[0..N]`。全局 hard cap 的原子边界在 BudgetAllocator，不在各 bucket 内部；Allocator 先给 bucket 分配额度，bucket 只能在已分配额度内原子 reserve，周期 reconciliation 只用于校准和回收，不得替代硬上限检查。
...
任何 bucket 预留不得使 `allocated_total > tenant_limit`；Allocator 写热点可分片读，但分配额度的提交必须经单 leader / fencing 保护。对 streaming LLM 或分段工具执行，必须每 N tokens 或每 provider chunk 做 incremental reserve / settle；超限必须 stop generation 并记录 partial output 为调试 artifact，不得进入 PlanGraph、代码、SQL、决策或 side effect。所有 streaming partial settlement 必须记录 `reservation_estimation_error_metric`；估算过低触发 overrun_policy，估算过高导致假耗尽时必须可自动 release 或重估。
...
- 报表必须同时展示 original currency、base currency、FX snapshot 和 invoice reconciliation status，避免多币种成本在审计时不可追溯。

## 18.5 成本优化策略

| 策略           | 说明                                         | 适用场景               |
| -------------- | -------------------------------------------- | ---------------------- |
| Prompt 缓存    | ExactPromptCache 默认；SemanticCache 仅限人工批准安全域（见 §15.5） | read-only / 低变化场景 |
...
委托 deadline 到期必须进入 `timed_out` 终态，释放 child run 未使用预算、关闭 pending approval、取消未完成 bid，并向 parent run 追加 `delegation.timed_out` 事件。广播委托未达到 quorum 时按 `AggregationPolicy` 的 `quorum_failure_policy = abort | best_available | human_arbitration | retry_once` 处理；落选 bid 的 discovery / proposal token 成本计入 parent run，不得丢失成本归因。
...
1. NodeRun 进入等待状态 → 创建完整 checkpoint
2. 释放 worker lease（worker 不再占用）
3. 创建 HibernationRecord，注册 wake_conditions
4. HarnessRun 状态设为 `paused` 或 `hibernated`
5. 所有内存上下文持久化到 P5

**唤醒流程**：

1. wake_condition 满足 → WakeEngine 触发
2. 执行 `ResumeCompatibilityCheck`
3. 从 checkpoint 恢复 workflow 上下文
4. 重新申请 worker lease
5. 从断点处继续执行

`ResumeCompatibilityCheck` 必须覆盖：RunVersionLock、Prompt/Model/Tool/Policy 版本锁、DomainDescriptor/Domain Spec 版本、connector auth 与 action schema、secret lease reacquire、approval validity、budget reservation refresh、external callback signature、policy diff、provider/model/prompt deprecation。任一 high/critical 兼容性失败必须进入 `require_revalidation` 或 `abort_on_resume`，不得静默继续。

`ResumeCompatibilityCheck` 必须有总超时（默认 30s，可由域策略收紧或放宽到最大 5min）。检查超时不得默认恢复；必须进入 `resume_check_timed_out`，并按 risk tier 选择 supervised resume、require_revalidation 或 abort_on_resume。休眠超过 DomainDescriptor 兼容窗口的 run 必须强制生成 `ResumeDiffReport`，由 owner 决定迁移、重规划或终止。

MVP 只承诺 approval_request + checkpoint 支撑基础 HITL wait 和进程重启后的安全恢复；通用长时休眠、复杂 wake_conditions、calendar timer 和 provider callback resume 进入 Hardening，并需要独立 `hibernation_record` / timer 表。若某部署未启用 hibernation_record，超过 checkpoint TTL 的长时等待必须进入 `paused_requires_operator`，不得静默恢复。
...
- 需要 <30s SLO 的 IT 运维、客服或交易控制任务必须声明 `high_precision_timer=true`，使用独立高精度调度器和更小扫描窗口；未声明时不得承诺亚 30s 唤醒。
...
| notification_only | 仅通知，无需审批                 | no-side-effect 或低风险可逆动作 | 自动通过           |
...
协同编辑默认使用 strict turn-taking token，避免 Agent 与人工同时修改同一 artifact。人工介入命令必须标准化：
...
人工 `takeover` 后 Agent 不得继续自主执行，除非收到 scope 匹配且未过期的 `resume` DecisionDirective。

`notification_only` 不是审批绕过机制，只能用于无副作用或低风险可逆动作。任何写 truth、外发数据、提交 side effect、改变预算、扩大权限或不可逆操作都必须使用 approval / HITL 模式。

审批委托 TTL 重置必须受 `max_delegation_chain_length` 和 `max_total_approval_wait` 约束；每次委托都必须重新执行 ConflictOfInterestFilter、SoD 检查和 scope 收缩。达到链长或总等待上限后只能升级或 abort，不得无限转委托。
...
MVP SDK 范围限定为：`create-pack`、`validate-manifest`、`run-local-simulation`、`generate-contract-tests`、`publish-dry-run`。其他 IDE、Playground、市场发布体验进入 Hardening / Enterprise。
...
| platform_min_version / platform_max_version | 支持的平台版本窗口 |
| contract_test_generator | 基于 Manifest / OpenAPI / Event Registry 生成契约测试 |
| plugin_sandbox_test_harness | 插件沙箱、egress、secret、filesystem 和 symlink denial-path 测试 |
| deprecation_policy | SDK API 废弃窗口、迁移提示和 CI 警告规则 |

所有 SDK 请求必须发送 `X-Platform-Version`、`X-SDK-Version` 和 `X-Contract-Version`。平台在 admission 前执行版本握手：低于 min version 返回 `upgrade_required`，超过 max tested version 返回 `compatibility_warning` 或要求 dry-run；已废弃 API 只能在 deprecation window 内调用，并在响应中返回 migration hint。SDK 不得静默调用 legacy projection 写路径。
...
record/replay fixture 必须自动脱敏：redact secrets、hash PII、strip proprietary payload unless approved。本地模拟只能验证 contract 和确定性逻辑，不能证明生产 SideEffect 安全。
...
删除权与 immutable audit 的边界：destroy encrypted payload，retain non-PII audit envelope，retain hash/digest for integrity。删除后生成 `ErasureTombstone`，不可再被 Memory/Knowledge/Prompt fixture 引用。
...
`legal_hold_exception` 必须记录 hold reason、legal owner、review date 和可见性限制；解除 legal hold 后回到 `classified` 继续删除流程。backup 中的 payload 不要求立即物理覆盖，但必须有到期等待、密钥销毁证明和最终 verified 证据。
...
跨境与法律依据必须物化为证据对象：`DataTransferRecord`、`LegalBasisRecord`、`RetentionOverride`、`TenantNeutralAuditDigest`。Legal hold 优先于普通 retention/delete request，但只保留必要 envelope 和摘要，敏感 payload 按 legal basis 加密封存。
...
v4.3 将单一大 DomainDescriptor 拆为多份可独立版本化的 Domain Spec，避免一个 schema 承载过多语义。`DomainCoreDescriptor` 只保留领域身份和关联索引，其余能力由专用 spec 通过同一 `domainId` 关联：

| Spec | 职责 | 版本策略 |
| --- | --- | --- |
| DomainCoreDescriptor | domainId、owner、primary entities、recipe archetype、lifecycle | 2 个版本支持窗口 |
| DomainExecutionProfile | execution_mode、latency tier、hot path、compiled artifact | 热路径变更需重新认证 |
| DomainRiskSpec | 风险覆写、side effect、审批阈值、liability owner | high/critical 变更需 P2 approval |
| DomainKnowledgeSpec | knowledge source、ACL、freshness、conflict policy | 可独立灰度 |
| DomainEvalSpec | eval baseline、critical cases、acceptance threshold | 发布门禁输入 |
| DomainGovernanceSpec | HITL、policy、recertification、waiver | 只能收紧上级策略 |
| DomainInteractionSpec | NL、dashboard、proactive trigger、用户体验策略 | 不得绕过执行约束 |
...
| 数据/代码/知识库 | lineage / schema compatibility / CODEOWNER + SAST/license/dependency scan / principal-aware ACL |
| 医疗/法务/HR/教育 | professional signoff / bias audit / consent record / advisory_only boundary |
| 直播/内容审核/多模态创意 | StreamInterventionStateMachine / jurisdiction escalation / provenance / similarity/license evidence |
| 供应链/IT运维/市场营销 | export/hazmat policy / CMDB blast radius / rollback window / claim evidence source |

每个 Domain Spec 必须通过 `domain lint` 后才能进入 Gate 2：risk action coverage、HITL coverage、tool permission coverage、eval coverage、SLO profile、data boundary lint、critical action responsibility record 覆盖率均必须可机器校验。critical 域每个 critical action 必须绑定 HITL + HumanResponsibilityRecord；缺失时 DomainReleaseGate fail closed。

v4.3 保留执行模式字段，用于区分 LLM 辅助规划与确定性热路径执行：
...
`conflictResolutionPolicy` 必须引用平台支持枚举或注册的插件接口，不得填写自由文本。DomainDescriptor schema 支持窗口为 2 个版本；`cdm-v1`、`cdm-v2` 并行期结束前必须完成迁移，否则新 run admission 拒绝。

`CompiledPlanArtifact` 用于 deterministic hot path：

| 字段 | 说明 |
| --- | --- |
| sourceGraphRef | 来源 PlanGraph / strategy graph |
| compilerVersion | 编译器和规则版本 |
| signature | artifact 签名与发布者 |
| policyProofRef | 风险、权限、预算、数据边界证明 |
...
每个垂直业务域接入平台时，必须使用统一元模型回答以下 **15 个标准问题**。该元模型是平台"按域配置驱动"的基础，也是看板、审批、风险、评测统一生成的数据来源。新增第 25 个域时，只需填写同一模板即可完成接入定义。
...
| Q14 | 失败补偿/回滚模型是什么    | DomainRiskSpec.compensation_model         | refund、reversal、appeal、manual repair |
...
| 问题 | 机器契约落点 | 填写要求 |
| --- | --- | --- |
| Q13 责任主体是谁 | `DomainRiskSpec.liability_owner` | 指定业务 owner、法定资质责任人、审批责任人和事故响应 owner；critical 域不得使用团队邮箱作为唯一 owner |
| Q14 失败补偿/回滚模型是什么 | `DomainRiskSpec.compensation_model` | 每个不可逆或外部写动作必须声明 refund / reversal / appeal / manual repair / no_compensation，并绑定 SideEffect compensation policy |
| Q15 红队/对抗场景是什么 | `DomainEvalSpec.adversarial_scenarios` | 至少覆盖 prompt injection、越权访问、欺诈/滥用、极端输入、跨域数据泄漏和高成本触发；high/critical 域必须有独立 holdout |
...
接入路径按风险分层：low = fast-track，medium = standard，high = enhanced，critical = regulated。low-risk 内部域可合并部分人工会议审查；critical regulated 域必须增加领域专家、合规团队和生产演练签核。

| 风险路径 | 目标周期 | 说明 |
| --- | --- | --- |
| low | 1-2 周 | 只读/低风险内部流程，可合并人工会议 |
| medium | 3-6 周 | 标准四阶段门禁 |
| high | 6-10 周 | 增加安全、红队、回滚演练和 domain_owner 签核 |
| critical | 3-6 个月 | 领域专家、法务/合规、生产演练和监管证据齐备后才可灰度 |

每个 Gate 必须声明 `automated_check`、`human_signoff`、`evidence_required` 和 `waiver_policy`，并输出 machine-readable `DomainCertificationRecord`。waiver 只能有 expiry、owner、compensating control 和审计记录，不能豁免机器不变量。
...
- [ ] 风险画像已经过安全团队初审
- [ ] 知识源已确认可达且有授权
- [ ] eval dataset 满足 §17 风险分级下限：low ≥50、medium ≥200、high ≥500、critical 建议 ≥1000 + 专家签核 + holdout
- [ ] 治理契约已由 domain_owner 签署
- [ ] 跨域交互策略已与相关域确认（如有）
- [ ] 15 问元模型(§37.11)全部填写完成且通过校验
- [ ] 平台架构评审会议通过
...
持续治理状态包括：initial certification、periodic recertification、incident-triggered recertification、domain descriptor drift review。DomainDescriptor 与 Pack 行为出现 drift 时，平台可暂停新 run admission，直到复审完成。
...
| 域 | 必须吸收的专项约束 |
| --- | --- |
| 量化交易 | LLM 仅用于离线研究、策略解释和候选计划；下单热路径不得经过通用 Harness loop、LLM 或 HITL；上线物必须是签名的 compiled strategy artifact，带回测证据、盘前确定性风控、硬性限额、kill switch 和盘后审计 |
| 电商 | 定价、库存、退款全部作为 SideEffect 管理；必须有 price floor、库存预留、退款阈值、活动回滚窗口和 oversell incident workflow |
| 广告推广 | 广告消耗使用独立 `ad_spend_ledger`；创意审批与投放执行分离；实时竞价使用确定性策略和 bid adjustment bounds，不依赖 LLM 热路径 |
| 金融服务 | 不利信贷/授信决策必须生成 adverse action explanation、公平性评估包和监管证据包；LLM-as-Judge 不得作为最终合规裁决；高风险动作需持牌/授权复核人签核 |
| 数据处理 | Sink 必须声明 idempotency contract；Replay 必须感知 lineage；数据质量规则是版本化资产；破坏性迁移需 shadow compare 与人工审批 |
| 代码开发 | Agent 写代码默认 branch-only；merge 必须 PR、CI、SAST、dependency/license/secret scan 和 CODEOWNER review 通过 |
| 用户运营 | 触达频控是平台硬限制；分群需敏感属性/代理变量检测；实验需 consent、holdout protection 和伦理边界 |
| 行业调研 | 每个事实主张默认需要 citation；来源必须通过 license/ToS 检查；预测类输出必须给出置信区间或不确定性说明 |
| 学术调研 | 引用必须经 DOI/CrossRef/PubMed 等解析器验证；统计分析输出可复现 notebook artifact；必须执行 plagiarism/authorship policy |
| 企业知识库 | 查询时实时 ACL check；回答默认带引用；权限镜像有 freshness SLO；过期知识触发 stale alert 和 trust downgrade |
| 财务 | 多币种动作必须记录 base_currency 与 FX snapshot；四眼审批/SoD 为平台级能力；凭证、报表和财务证据包不可变且带签核链 |
| 法务 | 默认只提供 legal information；形成 legal advice、外发文本或可被采取行动的输出前必须 attorney review；privilege、jurisdiction、legal hold 是一等分类字段 |
| 在线直播 | 实时审核热路径优先 edge/deterministic moderation；断流是高风险副作用，必须有 stream kill switch、appeal/reinstate workflow 和未成年人保护策略 |
| 广告素材制作 | 生成资产必须带 provenance、C2PA、水印、版权/商标相似性扫描和品牌规则版本；不得直接发布到外部渠道 |
| 游戏开发 | 资产需 IP similarity scan；数值平衡配置不得自动写生产；QA 证据与 release gate 绑定 |
| 游戏上架 | 每个平台独立合规矩阵；年龄分级、防沉迷、支付、地区政策版本化；LiveOps 配置需审批 |
| 人力资源 | 招聘/晋升输出 recommendation-only，不得自动淘汰；必须进行 bias audit 和 protected attribute handling；HR 数据默认不进入长期/共享 memory |
| 供应链与物流 | 大额采购、危险品、出口管制触发硬审批；预测异常触发 circuit breaker；离线 side effect 按 dependency graph 拓扑提交 |
| 医疗健康 | 平台仅提供 clinical decision support，不是诊断主体；PHI 强隔离；诊疗建议需 physician signoff；急救/紧急路径不得依赖 LLM；医疗证据包不可篡改 |
| 教育培训 | 未成年人数据需 guardian/school consent；按年龄分级策略提供内容；默认 Socratic tutoring mode；学术诚信 guardrail 防止直接代做 |
| 客户服务 | 业务承诺需 promise checker；退款/补偿按阈值审批；`max_unresolved_turns = 3` 后转人工；负面情绪触发升级 |
| 内容审核与安全 | CSAM/极端内容按辖区流程上报；申诉与证据留存有专门状态机；审核员保护作为治理要求记录 |
| IT 运维 SRE/DevOps | 自动修复仅限 known-runbook-only；爆炸半径限制为单节点/单服务；遵守 change window；平台故障需 out-of-band break-glass |
| 市场营销与品牌 | 对外内容需品牌一致性、广告法/行业法检查、claim evidence 和危机公关升级路径 |
...
- 所有 NL 交互先转化为 `TaskDraft`；只有用户确认后的 `TaskSpec` 才能进入标准 `RequestEnvelope`(§5.3)，复用已有控制面和执行面
- 歧义必须显式消解，不猜测用户意图——宁可多问一句，不可误执行高风险动作
- 对话上下文持久化到 Memory(§29.2)，跨会话可恢复
- Intent Parser 输出必须经过 schema validation、risk preview 和 policy check；不得直接信任 LLM 生成的 TaskSpec
- high/critical 自然语言指令必须先生成 dry-run preview，用户确认后也仍需走 §10/§47 风险与审批
...
`TaskDraft` 和 `ClarificationState` 是 pre-admission 对象，不得进入 P4，也不得创建 HarnessRun。高风险、权限不完整、slot 缺失或 policy uncertain 的草稿必须停留在澄清/确认态。
...
| TaskSpecBuilder   | 将已确认的 TaskDraft 映射为 TaskSpec(§6)，填充域、参数与约束 |
...
| --- | --- | --- |
| TaskDraft | 保存解析出的目标、slot、候选域、风险预览和缺失项 | 无；只能生成 confirmed TaskSpec |
| ClarificationState | 保存追问轮次、未决问题、用户回答和置信度 | 所有 required slot resolved |
| UserConfirmationReceipt | 保存用户确认文本、风险预览版本、scope、time 和 actor | receipt 有效且 scope 与 TaskSpec 匹配 |

置信阈值默认值：`intent_confidence_threshold=0.80`，`slot_confidence_threshold=0.85`。低于阈值必须进入 Clarification Dialog；涉及高风险动作时阈值不得被域配置放松。
...
- 多轮上下文进入 memory 前必须按 §29 MemoryContract 做数据分级；restricted/regulated 对话默认只存 session memory，不进入 long-term/shared memory
...
核心方法 `decompose(goal, constraints) → GoalGraphDraft → TaskGraphDraft`，将高层目标拆解为可校验草稿。GoalDecomposer 不直接产出可执行 PlanGraphBundle；草稿必须交给 Harness Planner 执行 Normalize / Validate / Risk Propagation / Worst-Path Analysis 后，才能形成 PlanGraphBundle。

| 参数/特性   | 说明                                                    |
| ----------- | ------------------------------------------------------- |
| goal        | 结构化目标描述，含目标文本、所属域与优先级              |
| constraints | 分解约束：最大深度(默认 5)、预算上限(§18)、截止时间     |
| 返回值      | GoalGraphDraft / TaskGraphDraft —— 节点为 TaskSpec draft，边为依赖关系          |
| 环检测      | 构建 TaskGraph 时自动执行拓扑排序，检测到环即拒绝并报错 |
| 预算分配    | 按子任务预估成本比例将总预算分配到各节点                |

目标分解必须传播 budget、risk、permission、data boundary 和 capability constraints；LLM 分解结果必须经过 deterministic DAG validation、capability validation、risk propagation 与 budget propagation 后才能进入 Harness Planner。只有 Planner 输出的 PlanGraphBundle 是 P4 可执行输入。
...
- 主动任务不得抢占用户任务保留预算；`user_task_budget_reserve >= 60%`
- medium+ 主动动作默认 suggestion mode，不得直接 execution
...
- **proactive_budget_cap**：按 tenant/domain 设定主动任务成本上限，超出后只生成建议
- **ProactiveBudgetPool**：主动任务使用独立预算池，不得消耗用户发起任务保留预算
- **UserInitiatedReserveRatio**：用户发起任务预算保留比例默认 ≥ 60%，触发器不得突破
- **feedback loop detection**：触发器之间若形成互相触发闭环，自动禁用相关触发器并创建 incident
...
| historyWindow | duration (默认 90d)                             | 计算积分所用的滑动窗口长度，使用连续权重而非断崖窗口 |
...
| 条件 | 衰减行为 | 说明 |
| --- | --- | --- |
| 连续无执行 | 每日按 `decayRate` 平滑衰减，最低不低于当前级别下限 | 避免 90d 断崖式生产突变 |
| 30d 无执行 | 触发提醒 + 冻结晋升 | 不自动降级 |
| 90d 无执行且风险加权样本不足 | 降一级需 platform policy 允许并记录原因 | 高风险 Agent 默认先进入 supervised review |
| 180d 无执行 | 自主权降至 suggestion | Agent 视为"休眠态"，需 domain_owner 重新激活 |

衰减评估由 `TrustDecayWorker` 每日运行，变更记录 `agent.autonomy.decayed` 事件到 event_log(§28)。domain_owner 可通过 DomainGovernancePolicy(§37.9) 收紧衰减参数，但不得豁免平台级 max autonomy cap、risk-weighted trust score 或 high/critical side effect 的人工确认。

## 42.4 自主权变更审计

所有自主权变更记录到 event_log(§28)：

自动降级或暂停前必须生成 `AutonomyChangeImpactReport`，说明影响的 active run、SLA、审批队列、预算和业务 owner。关键业务可配置 grace period 或人工确认；P0/P1 安全事件除外，必须立即降级或暂停。
...
所有看板指标必须注册到 `MetricRegistry`，声明 metric_owner、source_of_truth、freshness_slo、actionability、permission_filter 和 stale_behavior。L1/L2 的自然语言摘要必须先做 redaction，禁止泄露隐藏知识、跨租户信息或审批细节。
...
看板操作按钮不得直接触发高风险 OperationalDirective 或 DecisionDirective。任何会写 truth、外发数据、提交副作用、变更预算或影响模式的操作必须走 dashboard action risk gate、二次确认和审计。

每条 LLM 生成摘要必须携带 evidence_refs、freshness、confidence、redaction_policy 和 source_projection_version；缺少证据或 freshness 过期时只能显示“证据不足”，不得生成确定性结论。
...
可视化域接入向导只适用于 low-risk fast-track。medium/high/critical 域只能展示只读配置、待办门禁和证据状态；UI 不得提供“风险滑块”来降低 inherent risk，风险只能由 DomainRiskSpec 和 PolicyEngine 计算。

## 44.3 可视化 Workflow 构建器

面向非技术用户的 workflow 编排界面：

可视化构建器必须是 PlanGraph 的 projection/editor，而不是另一套 DSL。保存前必须通过 Graph Normalize / Validate / Risk Propagation / Worst-Path Analysis；任何 UI 节点都必须能映射到 PlanGraph node、edge、ConstraintPack 或 HITL decision。
...
单人模式高风险动作仍必须 dry-run + cooldown + rollback/compensation window；不可逆或受监管动作不得通过自审批直接 full_auto。
...
**前端实现要求**：WCAG 2.1 AA 合规需要实际的前端 UI 实现（React/Vue/Angular 等框架）。平台 TypeScript 代码提供数据模型、颜色对比度令牌（`getSeverityColorTokens()`）和可访问性标签构建函数（`buildAccessibleLabel()`），但实际 UI 组件必须在具体前端框架中实现。§21 HITL 通知组件（`src/platform/five-plane-interface/console/hitl/notification.ts`）提供 TypeScript 逻辑，颜色值符合 WCAG AA 对比度要求（≥4.5:1），但渲染和交互实现由前端负责。
...
> 将平台分散的约束、工具、上下文、反馈能力收敛为统一的 Harness Runtime——标准化的 Agent 运行底座。融合 Anthropic 角色化闭环、LangGraph 持久运行时、OpenAI 治理与 Guardrails 原语三大行业流派的八支柱模型。HarnessRuntime 不替代业务模块；但替代所有分散执行入口，成为唯一执行入口，并把既有模块编排成闭环运行时。
...
每次任务运行都通过 HarnessRuntime 统一入口，装配约束、工具、上下文，驱动 Planner→Generator→Evaluator 多轮闭环，产出最终结果与证据链。v4.2 起 HarnessRuntime 是唯一可执行运行时，HarnessRun 是唯一权威 Run；OAPEFLIR 只提供 StageRationale、TraceProjection 与 Audit View，不创建独立运行实体。Harness 中的 `PlanBundle` 是产品层 wrapper，内部 canonical execution contract 必须是 `PlanGraphBundle`；P4 只按 PlanGraph / NodeRun 语义执行任务。

Planner、Generator、Evaluator 在本章默认表示 Runtime Role，不要求部署为三个独立 Agent。只有在多 Agent 部署中，它们才映射为独立 Agent 实例；无论部署形态如何，Prompt、ContextAssemblyContract、Evidence 和责任主体必须按 role 隔离。

## 45.1.1 Single Definition Table

为避免 §13、§14、§25、§28、§45、§58 重复定义造成漂移，以下对象只有一个规范落点，其他章节只能引用或说明 projection：

| 概念 | 规范落点 | 其他章节角色 |
| --- | --- | --- |
| HarnessRun / HarnessStep | §45.13 | §13/§58 只解释语义和观测 |
| PlanGraphBundle / PlanGraph | §5.3 / §13.8 | §40/§44 只产生 draft 或 projection |
| NodeRun / NodeAttempt | §14.10 / §14.15 | §25/§28 只描述状态持久化和事件 |
| SideEffectRecord | §14.11 | §57 只声明 connector action contract |
| BudgetReservation | §18.3 | §25/§58 只引用预算事实 |
| Event Registry | §28 | §58 Runtime Test Matrix 只定义测试覆盖 |
...
| P4 Execution     | 下发执行计划                 | PlanGraphBundle、NodeRun、ToolCall、HITLWait、AsyncDispatch |
...
| retry_same_plan | 重试当前步骤（同一 plan） | retry_count < max_retries   |
...
| downgrade_mode | 降级运行模式              | 成本/风险接近阈值           |
...
| 字段           | 说明                                         |
| -------------- | -------------------------------------------- |
| harnessStepId  | 当前执行语义步骤 ID（与 §45.13 一致）         |
| artifacts      | 产出的工件引用列表                           |
...
| 现有组件            | Harness 集成方式                                                                          |
| ------------------- | ----------------------------------------------------------------------------------------- |
| §5 平面间契约       | HarnessRuntime 作为 P3 Orchestration 的统一入口，接收 RequestEnvelope，输出 PlanGraphBundle |
...
| §14 Execution Plane | Generator Agent 通过标准 PlanGraphBundle → Graph Scheduler → NodeRun → NodeAttemptReceipt 路径执行 |
...
| 字段         | 说明                                                                      |
| ------------ | ------------------------------------------------------------------------- |
| harnessStepId | 语义步骤标识；不得作为 P4 执行或回执主键                                 |
| nodeRunRefs  | 关联的一个或多个 NodeRun；执行事实以 NodeRun 为准                         |
...
**HarnessDecision** 采用 §58.6 的统一裁决协议：基础六种裁决 `accept / retry_same_plan / replan / escalate_to_human / downgrade_mode / abort`，并允许生产扩展裁决 `quarantine / revoke_approval / pause_for_external / require_revalidation`。
...
**现有模块映射**：AuthoritativeTaskStore · NodeAttemptReceipt · OapeflirViewProjectionService · AuditService。旧 ExecutionReceipt / OapeflirLoopService 仅迁移期 adapter 使用。
...
- 防自增强：Evaluator 不能单独把自己刚产生的判断直接晋升为 Long-term Memory；必须经过 holdout 任务、不同模型/不同 judge 或人工审核之一。被标记 contested、低置信、来自失败 run 或来自未验证工具输出的记忆只能进入 quarantine，不得影响后续 Evaluator。
...
- retention 到期后执行 compaction，不删除 legal hold、audit evidence、HITL responsibility record 和 side effect evidence
- ContextEvictionPolicy 固定优先级：policy/approval/budget/risk > current truth > confirmed user spec > recent evidence > memory > knowledge retrieval > generated summary
...
Guardrail 冲突采用最严胜出：`abort > deny > escalate > replan > filter > allow`。每个 run 必须维护 `guardrail_action_count`、`last_guardrail_signature` 和 `guardrail_cooldown_until`；同一签名连续触发超过上限即判定为 guardrail vibration，必须 abort 或 escalate_to_human，不得继续 replan 消耗预算。
...
| Harness 对象 | OAPEFLIR v4.2 投影 / 契约 | 说明 |
| --- | --- | --- |
...
ContextAssemblyContract 必须声明输入源优先级、token budget、eviction / truncation report、taint propagation、PII/secret redaction 和 loss impact。默认输入优先级：policy / approval / budget / risk > current NodeRun truth > user confirmed TaskSpec > recent evidence > memory > knowledge retrieval > model-generated summary。
...
每次 Prompt 调用必须生成 `PromptExecutionRecord`：

| 字段 | 说明 |
| --- | --- |
| promptVersion | PromptBundle / role prompt 的冻结版本 |
| modelRoute | ModelGateway 路由、provider、model 和 fallback 路径 |
| inputHash / outputHash | 输入输出哈希，支撑 Trace Replay 与篡改检测 |
| contextSnapshotRef | 本次调用消费的上下文快照 |
| guardrailResult | 输入、输出和工具护栏结果 |
| usage | token、latency、cost、currency 和 BudgetReservation 引用 |
...
DecisionInputBundle 是 HITL / Evaluator / Audit 的统一 evidence artifact，必须在 §6 API、§26 表落点和 §58 Trace Replay 中可引用。
...
Guardrail 冲突优先级固定为：`abort > deny > escalate > replan > filter > allow`。每个 run 必须设置 `guardrail_action_count` 上限；超过上限说明循环无法收敛，必须 abort 或 escalate_to_human。
...
| 指标                              | 说明                              | SLO                                         |
| --------------------------------- | --------------------------------- | ------------------------------------------- |
| harness.run.duration              | 单次 HarnessRun 端到端耗时        | default P99 < 60s，或使用 DomainDescriptor.sloProfile 覆写 |
...
若 DomainDescriptor 未定义 SLO，默认使用 §27 deployment-tier SLO fallback；不得把“业务域未定义”解释为无 SLO 或 best effort。Harness 指标必须记录使用的 slo_source = domain_override / platform_tier_default / emergency_override。
...
关键约束：所有 candidate 只是建议，必须经过 §34 ADR-Quality-Gate-Before-Prompt-Release 和 P2 审批后才能生效。LearningCandidate 必须通过 quality gate：偏差检查、多样性检查、污染防护、PII/secret scan、holdout 隔离和 domain_owner 复核。
...
默认 Replay 是 Trace Replay：不重新调用 LLM / Tool，不写 production truth，不产生真实 SideEffect。ReplaySession 必须声明 `sideEffectMode=disabled/simulated/mock_only`，默认 `disabled`；任何未显式声明的 replay 都不得触达外部系统。策略对比、Prompt A/B、工具替换模拟和 What-if 属于 Re-execution Replay，必须标记 nondeterministic，在隔离沙箱中运行(§34 ADR-Workflow-Debug-Session-Isolated)，结果不得覆盖原 HarnessRun evidence。

ReplaySandboxPolicy：所有 Re-execution Replay 使用独立 budget、独立 data namespace、只读 production evidence、模拟 external side effect，并显式标记 `nondeterministic=true`。策略对比、Prompt A/B 和工具替换结果只能作为 proposal evidence。

## 58.5 架构遗留问题收口

收口以下跨章节遗留问题：

本表只保留为可读性索引；长期治理必须迁移到 `LegacyResolutionRegistry` / ADR / issue registry，避免正文永久承载历史债。
...
| 无统一错误分类体系                                                     | §6.2         | 错误码按层级组织：`PLATFORM.{plane}.{component}.{category}`，例如 `PLATFORM.P4.TOOL.TIMEOUT`、`PLATFORM.P2.POLICY.DENIED`。OAPEFLIR 只能作为 tag / trace projection 字段，不进入错误码命名空间。每个错误码关联 retryable/severity/user_message 三元组                                  |
...
> LoopController(§45.7) 的裁决升级为一级协议。基础六种裁决为 accept/retry/replan/escalate/downgrade_mode/abort；生产扩展裁决用于隔离、撤销授权、等待外部系统和重新校验。
...
v4.2 运行时指标以 HarnessRun / NodeRun 为主键，至少包含：
...
该矩阵是 Phase 8d 的验收基线，也必须纳入 executable runtime contract 和 CI 必跑发布门禁；任何 runtime schema、state machine、event registry、budget 或 side effect 变更都必须更新对应测试。

## 58.11 Executable Runtime Contract Package

机器验收入口必须落在 `runtime-contracts/`，文档只描述 intent；schema、状态机和 invariant test 才是实现冻结依据。最小目录：
...
| Gate | 必须检查 | 失败处理 |
| --- | --- | --- |
| Schema diff gate | OpenAPI / Zod / SDK 类型与 runtime-contracts 一致 | block merge |
| Naming consistency | deprecated term 未进入新写路径 | block merge |
| State-machine invariant | HarnessRun / NodeRun / SideEffect 终态封闭 | block release |
| Replay safety | Replay 不调用真实 LLM、Tool、Connector 或外部写 API | block release + incident |
| Budget precondition | LLM / Tool / SideEffect / Eval 前均有 active reservation | block release |
| Event atomicity | truth mutation 与 event append 同事务 | block merge |
| Event consumer safety | truth projector 只消费 platform facts，不消费 oapeflir view events | block merge |
| GraphPatch safety | 已执行节点不可删除；affectedSideEffects 必须声明 | block merge |
| HITL responsibility | approve / override / takeover / resume 均生成 HumanResponsibilityRecord | block release |
| Budget concurrency | 1000 并发 reserve 不穿透 tenant hard cap | block release |
...
`LegalEntityBoundary` 独立于 OrgNode 树存在，用于标记法人、国家/地区和监管边界。跨法人或跨国家的数据、审批、预算、知识共享默认按跨 tenant 处理，必须走 §50 controlled sharing、§52 跨境合规和 §47 审批路由。
...
组织变更必须生成 `OrgMergeConflictReport`、`ApprovalRerouteOnOrgChange` 或 `OrphanAgentFreezePolicy`（按需）。无人接管的 owned Agent 默认冻结新 run admission；in-flight approval 重新路由但保留原审批 evidence。

员工离职、停用或高风险调岗必须触发 revoke sessions、reassign pending approvals、cancel secret leases、freeze delegated authority 和 owned Agent admission freeze；完成后输出 `IdentityDeprovisioningReport`。

OrgTree 级联变更必须通过 `OrgGovernanceSaga` 执行：prepare 阶段冻结 orgVersion、影响范围、预算/审批/知识边界 diff；commit 阶段按固定顺序更新 identity、approval route、budget owner、domain owner 和 agent ownership；compensate 阶段回滚未完成子步骤或冻结受影响资源；audit 阶段输出 `OrgGovernanceSagaReceipt`。部门合并遇到域策略冲突时固定采用 stricter wins；无法比较的策略进入 compliance approval，不得自动合并。
...
引擎支持多审批人会签（parallel）与逐级审批（sequential）两种模式。每步设独立超时，超时后按 escalation_policy 自动升级到更高组织层级。职责分离（SoD）检查由策略引擎执行，不限于 requester != approver，还必须覆盖利益冲突、同一审批链互批、预算 owner 与执行 owner 冲突。
...
多币种审批必须统一为 `base_currency + FX snapshot` 后比较阈值；审批记录保存 FX rate、rate source 和 snapshot time。
...
peer delegate 必须经过 ConflictOfInterestFilter。所有 approval 必须有 expiry、revocation 和 commit-time revalidation；SideEffect commit 前必须确认 approval 仍在有效期且 scope 匹配。

审批创建时必须冻结 `ApprovalRouteSnapshot`，包含 org chart version、approver set、SoD/COI 检查结果、base_currency + FX snapshot、route policy version 和 evidence refs。组织变更只影响新审批或 checkpoint revalidation，不得原地改写历史审批链。
...
SCIM 同步自动维护 principal ↔ OrgNode 的关联关系。用户停用（deprovisioning）时立即撤销活跃 session 并暂停其名下 Agent，确保零残留访问。所有同步操作记录审计日志，冲突时以 IdP 为权威源。IdP sync 异常进入 `identity_sync_dlq`，并生成 SCIM conflict report。

`identity_sync_dlq` 必须有 retry/backoff 和周期对账：429 遵守 Retry-After，5xx 使用 exponential backoff，schema/conflict 错误进入人工处理；每日 reconciliation 对比 IdP group/user snapshot 与平台 principal/org mapping，发现漂移即生成 `IdentityReconciliationReport`。deprovisioning 属于安全关键路径，DLQ 不得阻塞 session revoke 和 secret lease cancel。
...
规则：子节点**继承**父节点所有合规约束，可**追加**但不可**放松**。策略合并语义固定为：deny overrides allow；安全/合规维度 stricter wins；配额、组织结构、数据边界等结构性冲突必须进入 ADR 或合规审批，不能靠“更严格”自动猜测。由于严格性并非总是全序，每类策略必须提供 `PolicyStrictnessComparator`；无法比较时按更严格路径执行或要求合规审批。
...
合规例外必须走 `ComplianceExceptionWorkflow`，包含 scope、expiresAt、approver、compensating controls 和 auditRef。合规运营指标包括 `EvidenceQualityScore` 与 `ControlCoverageReport`。
...
Chinese Wall 必须有 `WallExpiryPolicy` / reset 流程，避免用户长期使用后权限只收紧不可恢复。解除或到期必须经 compliance officer 审批、cooling period、full audit 和 data residue scan；审计/debug 需求由 `ComplianceOfficerAuditView` 满足，不向普通调用者暴露知识存在性。controlled sharing 必须通过 `CrossBoundaryTransform` 做脱敏、摘要和字段级过滤。

Chinese Wall grant/release 必须使用两阶段提交：prepare 先锁定 subject、conflict set、knowledge boundary 和 expiry；commit 时同时写 boundary state 与 audit event；失败时保持原隔离墙不变并生成 reconciliation task。release 不得先删除限制再补审计；审计写入失败时 release 失败。
...
RuntimeInvariant 不可被任何 admin 覆盖，包括 super_admin。super_admin 只能提交 policy proposal；不可降级 invariant 的临时例外必须通过 break-glass、dual control、post-review 和自动恢复窗口，不允许直接关闭。治理委托必须声明 DelegationScope、DelegationExpiry、revocation policy 和 PolicyComparator；过期或撤销后所有派生权限即时失效。

委托撤销必须有有界传播 SLO：普通治理权限 < 5min，高风险或安全事件 < 60s。撤销通过 `GovernanceDelegationRevocationSaga` 级联到派生委托、pending approval、active session、secret lease、worker lease 和 scheduled trigger；无法确认的下游资源进入 frozen/quarantined，不得继续执行。
...
| 用户在 CN，需要调用 US 的 LLM       | CN 执行，LLM 请求路由到 US      | 输入扫描 + 输出扫描均确认无 PII/regulated data 时允许跨境 |
...
- Global Control Plane 只保存 metadata、routing policy、region health 和 home-region mapping；不得保存业务 payload、PII、PHI、secret 或 tenant truth。每个 tenant/partition 必须声明 home region 与 failover tier。metadata 必须有 quorum/backup、home-region mapping restore 流程和 routing policy version lock；failover 后的路由不得读取未锁定策略。
...
├──────────────────┤
│ Output Scanner    │  对 LLM/tool 输出做 PII/regulated data 扫描；命中则阻断跨境返回并生成 incident
├──────────────────┤
...
| limit          | `Map<ResourceKind, number>`           | 各资源上限值                               |
...
`ResourceKind` 至少包含：`worker_concurrency`、`tool_qps`、`model_tpm`、`model_rpm`、`budget_amount`、`approval_capacity`、`storage_io`。Admission Controller 只有在整个 `resourceVector` 都可满足时才允许进入队列，避免 token、审批席位或预算任一维度被隐性打爆。`approval_capacity` 也必须 reserve/release；高风险 run 无审批容量时必须 admission deny 或排队，不得先执行再等人。
...
- **Weighted Aging**：排队时间按 `effective_priority = base_priority * sla_weight + aging_factor` 计算，aging 只提升调度顺序，不改变 SLA Tier 或风险等级
- **Promotion Budget**：每个 tenant 每日/每小时有独立 `promotion_budget`，排队升级消耗该预算；预算耗尽后只告警和扩容建议，防止高负载下所有任务都被升级为 high
- **Checkpoint-before-preempt**：抢占任何 durable run 前必须完成 checkpoint，并记录 `preempted_by`、`checkpoint_ref`、`resume_policy`；不可 checkpoint 的 side effect window 禁止抢占，只能限流或 drain
- **Starvation Prevention**：standard 任务排队超过阈值后进入 weighted aging，不得无配额地直接升级为 high
...
| externalP95Latency | number (ms)                         | 用户可见 P95 延迟上限                   |
| internalP99Latency | number (ms)                         | 平台内部 P99 延迟上限                   |
| approvalLatencySlo | duration                            | 审批等待目标                            |
| incidentResponseSlo | duration                           | Incident 响应目标                       |
...
| Tier         | 可用性 | E2E P95 延迟 | Internal P99 延迟 | 排队上限 | 恢复优先 | 适用场景           |
| ------------ | ------ | ------------ | ----------------- | -------- | -------- | ------------------ |
...
99.99% 只允许在自动 failover、quorum 写入、热备容量和跨 Region 演练均通过的专用部署档中单独承诺，不作为 v4.2 默认 SLA Tier。

SLA 必须按 workflow class 拆分：deterministic 可承诺低秒级；LLM-assisted 只承诺平台内部调度/队列 SLO，模型延迟单独统计；HITL-waiting 不把人工等待计入平台故障，但必须承诺通知、提醒和升级时限。
...
MVP 禁止把外部第三方 Pack 作为默认安装来源。第三方 Pack/Plugin/Connector 进入任何生产 tenant 前，必须通过 §11 安全基线：SBOM、artifact signing、provenance、sandbox certification、egress policy、secret access review 和最小权限 Capability Profile。
...
收益分成、计费结算、信用积分和外部商业规则不属于核心运行架构，不得影响 Pack 安装、执行、权限或安全裁决。Enterprise/Future Marketplace 可在独立商业规范中定义 `CommercialTerms`，但核心平台只消费认证状态、版本、依赖、权限和安全元数据。
...
| removed    | sunset 倒计时结束 + 迁移门槛达成                         | 从 Registry 移除；已安装实例冻结（不执行新任务），数据保留 90 天 |

进入 `removed` 前必须满足 `migration_threshold`：无 critical business Agent 仍依赖该条目；已迁移安装数 ≥ 95%；剩余安装均有显式风险接受记录；安全漏洞场景可紧急冻结执行，但仍须保留只读数据和迁移导出能力。
...
- Marketplace 必须维护 dependency graph、security advisory、forced patch、quarantine 和 tenant impact report。critical advisory 可强制冻结或补丁升级，但必须保留只读导出、回滚说明和租户影响清单。
...
上面 architecture 文档中的 `FeedbackSignalType` 使用扁平的 9 类型枚举。实际实现（`src/platform/five-plane-orchestration/oapeflir/types/feedback-signal.ts`）采用 3D 正交结构：
...
每条 FeedbackSignal 必须附带 `FeedbackTrustScore`：来源可信度、历史准确率、是否经过身份验证、是否来自同一攻击面、是否与 holdout/eval 数据重叠。低可信反馈可进入分析队列，但不得直接生成上线候选。

训练样本选择同时计算 `CandidateDataQualityScore`、`CandidateDiversityScore`、`ContaminationScanResult` 和 `HoldoutCheckResult`，覆盖 tenant、domain、语言、任务难度、失败类型和风险等级，避免只从最活跃用户或单一失败模式学习。任何候选若命中 PII、secret、受限数据、holdout contamination 或 eval leakage，必须进入 quarantine 并阻断发布。

Feedback Analysis 必须包含 collective anomaly detection：当同一租户、同一组织、同一外部来源或同一时间窗口内的反馈高度同质且推动同一策略放松时，候选进入 `bias_suspected` / quarantined，由人工审核。低信任反馈不得影响 holdout、risk threshold 或 autonomy promotion。
...
模型路由优化必须使用 `DataResidencyConstrainedOptimization`：候选 provider、region、cache、fallback 和训练/日志用途都先通过数据驻留、客户合同、模型独立性和合规策略过滤，再计算成本或延迟收益。优化建议不得绕过 §15 ModelGateway、§17 EvaluationGate 或 §23 数据治理。
...
`ImprovementCandidateStateMachine` 与 §16 Prompt Rollout、§17 EvaluationGate、§34 ADR 和 §42 自主权晋升共用同一发布门禁。任何候选上线前必须锁定输入样本、评估版本、policy 版本、Prompt/Model/Tool/Domain 版本和审批责任人。

released 不是不可逆终态。任何候选发布后必须保留 rollback target、release evidence、impact window 和 automatic rollback metrics；质量、成本、安全、人工升级率或投诉指标触发阈值时进入 `rollback_pending`，完成回滚后进入 `rolled_back` 并生成 postmortem。rollback 不得删除原 released 事件，只追加补偿事实。
...
`ConnectorManifest` 必须包含 `ConnectorCapabilityProfile`：

| 字段 | 说明 |
| --- | --- |
| actionRiskProfiles | `ActionRiskProfile[]`，每个 action 的 read/write/delete/payment/egress/irreversible 风险、审批要求和 SideEffect 语义 |
| permissionProbes | `PermissionProbe[]`，验证身份、scope、对象级权限、租户边界和最小权限是否仍有效 |
| quotaProbes | 验证外部 API quota、rate limit、并发和 provider 熔断状态 |
| businessCapabilityProbes | 验证关键业务能力是否可用，例如订单可写、退款开关、审批回调、schema 版本 |
| credentialRotationPolicy | `CredentialRotationPolicy`，与 §11 Secret Lease 对齐的轮换周期、双写窗口、失效检测和自动吊销规则 |

`healthCheck()` 不得只返回网络连通性；必须覆盖 permission、quota 和 business capability。任何 action-level profile 缺失的连接器不得被 ToolbeltAssembler 装配到生产任务。

每个 write action 必须声明 idempotency key semantics、external status query、compensation availability 和 reconciliation timeout。无法查询外部状态且无补偿路径的 action 默认 risk ≥ high，并强制 HITL。
...
| Database | 大范围读写、schema drift、事务边界 | 默认 read-only；写入需 action-level 审批和 row/table scope |
| Browser | 视觉误判、会话劫持、不可预测 DOM | 隔离 session、录屏审计、禁止默认保存 credential |
| MCP | 工具能力动态暴露、远端信任边界 | 必须固定 server identity、capability allowlist 和签名 manifest |
| File/S3/NFS | 数据外泄、路径穿越、批量删除 | 强制 content scan、path allowlist、delete threshold approval |
...
- 每个 OAPEFLIR 循环的每个阶段**必须在决策时**生成 `DecisionRationale` / `StageRationale` 记录，不允许事后让 LLM 猜测原因
- 解释按需生成（lazy），不增加正常执行路径开销
- 解释深度按领域配置：金融需要 forensic-level，客服需要 summary-level
- 解释缓存避免重复 LLM 调用
- 解释不可篡改，纳入 Evidence Plane
- 解释必须 permission-aware：同一 run 对不同查看者可返回不同证据可见集，但不得改变底层不可篡改 rationale
- 解释必须区分 recorded facts、model rationale 和 inferred summary。界面不得把 LLM 事后渲染的理由当作已记录事实；所有 inferred summary 必须显示 evidence_refs 与置信度。
...
| decisionInputRef    | string          | 决策时冻结的 DecisionInputBundle 引用                   |
| versionLockRef      | string          | Prompt/Policy/PlanGraph/Tool/Model/Domain 版本锁引用    |
| visibilityLabels    | `string[]`      | 权限过滤标签，解释渲染时按用户权限裁剪                  |
...
- L3 forensic 解释必须先通过 `forensic_explanation_budget` 预留，超预算时返回证据索引和分段生成计划，不得在 incident 期间挤占 P4 执行预算
- 解释渲染器只可基于 `versionLockRef` 对应的 Prompt/Policy/PlanGraph 版本工作；运行后策略变更不能重写历史解释，只能追加新版解释视图

---

# 60. 紧急制动与全局熔断架构

> 提供单一原子操作快速阻断入口并使执行面进入静默，用于安全事件、Prompt injection 攻击、Agent 逃逸等紧急场景。
...
PlatformPanicDirective
    ...
    │
    ├──▶ P4 Execution Plane: 中止新 side effect, 对 ambiguous side effect 做 reconciliation, 支持时执行 compensation
    │
...
**SLA**：拆分为两个指标：`ingress_block_time < 5s`（同 Region）/ `<15s`（跨 Region）；`execution_quiescence_time` 按部署形态定义，D1 < 10s、D2 < 30s、D3/S4 < 120s。不得把入口阻断和所有 worker 已停止混为一个指标。
...
3. 超过 8h 仍无响应，只允许恢复 forensic / monitoring / read-only inspection；禁止新 workflow、写操作、外部 side effect 和 policy 放松，完整恢复仍需双人审批
...
- 每次演练输出 `PanicDrillReport`，至少包含 ingress_block_time、execution_quiescence_time、egress_block_time、credential_revoke_time、plane_ack_success_rate、manual_recovery_time 和 unresolved_findings。
...
| status         | draft / testing / staging / canary / active / paused / deprecated / archived / removed | 版本状态，详见 §61.3 状态机 |
| compatibilityMatrix | `ComponentCompatibilityMatrix`           | Pack、Prompt、Model、Tool、Policy、Domain、Eval、connector action schema 版本兼容性 |
...
| paused→active/canary | 恢复运行          | resume revalidation：重新评估 trust、policy、DomainDescriptor、Connector 权限、budget 与 incident 状态 |
| active→deprecated   | 版本替代/业务变更 | 责任转移到新版本完成                 |
| deprecated→archived | TTL 过期          | 所有历史引用标记为 archived          |
| archived→removed    | 清理资源          | 无 critical business 依赖、迁移门槛达成、只读导出可用 |
...
关键业务 Agent 不允许从 `deprecated` 直接进入 `removed`。必须先完成迁移报告：下游依赖清单、未完成 run 处理方式、替代版本验证结果、回滚窗口、业务负责人签核和数据保留策略。
...
| 缓存计划   | EdgeRuntime 定期从 Central 预拉取 PlanGraphBundle 模板          |
| 设备身份   | 每台设备必须有硬件/软件绑定 device identity，证书吊销后禁止同步 |
| 本地加密   | Secret、PII、PHI 和 SyncQueue 在本地磁盘加密，密钥受租约和远程吊销控制 |
| 篡改防护   | 支持 secure boot / attestation 的设备必须启用；不支持时降级为 read-only 或低风险模式 |
| 离线时长   | 每个 EdgeRuntime 必须声明 offline max duration；超过后只读或冻结 |
| 密钥租约   | 本地 key lease 到期后禁止继续执行写动作；恢复连接前必须重新 attestation 和吊销检查 |

## 62.3 同步协议

**冲突解决原则**：Central 状态为权威源；离线期间的 side effect 如与 Central 冲突，默认 Central wins + 生成 Incident 供人工审查。

SyncQueue 必须是 signed append-only queue，每条记录包含 `device_id`、`sequence_no`、`prev_hash`、`side_effect_dependency_refs`、`signature` 和 `local_time_offset`。恢复连接后按 side effect dependency graph 拓扑提交；依赖未确认的后续副作用保持 pending，不得乱序提交。

模型、Prompt、规则和连接器 manifest 的边缘更新必须签名，支持 staged rollout 和 rollback。Central 可下发 `remote_wipe` 或 `edge_quarantine` 指令；设备在重新上线时必须先执行吊销检查，再上传业务变更。
...
MVP 只强制实现简单阈值和趋势指标：`success_rate_drop`、`override_rate_spike`、`cost_spike`、`tool_usage_shift`、`incident_count`。CUSUM、Bayesian Online Changepoint、KL/JS 等高级统计属于 Hardening/Enterprise，不得阻塞 MVP。

所有阈值必须声明样本量、分布假设和误报处理策略；不得把“偏移 > 2σ”作为非正态业务数据的唯一成功标准。漂移响应默认先告警、降速或要求人工复核；自动降级必须考虑业务连续性和 SLA。
...
BehaviorDriftAlert { agent_id, dimension, severity, drift_score }
    ...
    │
    ├── severity=medium → require_review + 降速/建议模式，默认不直接降级
    │
...
防博弈要求：行为指纹必须区分真实用户任务、合成评测任务和低价值“保活”任务；TrustScore、漂移基线和晋升统计不得由未验证的假任务单独推动。
...
| humanReviewCost | number                      | 审批、复核、审核员处理成本                  |
...
成本事实来源是 §18 `BudgetReservation`、settlement 和 release 记录；指标聚合不得直接从 provider 发票反推单次 run 成本。`totalCost = llm + tool + worker + storage + egress + human_review`。
...
| CacheHit       | 相同 query 重复调用                  | 启用 ExactPromptCache；SemanticCache 仅限人工批准安全域 | 40-80%   |
...
每次仿真输出 `projectedCost`、`qualityImpact`、`qualityRisk`、`slaImpact`、`regressionTestRequirement` 和 `recommendation`（建议 / 不建议 / 需进一步验证）。

所有自动优化建议都必须受 policy、data residency、quality gate、SLA 和合规约束。What-if 输出仅为 advisory，不得直接修改 ModelGateway 路由、Prompt、Tool 或预算配置；上线仍走 §16/§17/§24/§56 的发布门禁。
...
| 档位 | 能力边界 | 上线阶段 |
| --- | --- | --- |
| Debug Lite | timeline、evidence inspect、rationale 查看、只读日志 | MVP/Hardening |
| Debug Pro | Trace Replay、run compare、side effect diff、regression 标记 | Hardening |
| Debug IDE | replay breakpoint、replay variable inspect、step_replay、变量检查 | Enterprise，仅 replay sandbox |
...
| 断点调试      | ✗               | sandbox only    | 仅 Replay Sandbox 支持，不暂停生产 run        |
| 时间旅行      | ✗               | ✓               | 基于 Trace Replay 重建，不重写真相            |
| 运行对比      | ✗               | ✓               | 两次运行的并排对比                            |
...
断点 API 只属于 ReplaySandbox / Debug IDE，不作用于生产 HarnessRun。
...
| replay-variable | 仅观察 sandbox 中的变量或 context diff | inspect / diff |

断点命中后只有 sandbox session 进入 paused，通过 WebSocket 推送 `breakpoint_hit` 事件。调试者可检查 ContextSnapshot 后执行 `resume_replay`、`step_replay` 或 `abort_replay`；这些操作不得修改 production truth、不得触发真实 Tool/LLM/SideEffect，也不得改变原 HarnessRun 状态。

生产 run 只支持 `inspect`、`safe_pause_request` 和 `abort_request`，且必须走 §6 canonical control API、§21 HITL / §47 审批和审计。生产 run 不支持断点命中暂停、交互式单步、变量观察点或变量热修改。Debugger 权限等同高敏证据访问：必须双因素认证、最小权限、短期 session、全量审计和导出水印。任何 re-execution replay 都必须运行在 ReplaySandboxPolicy 下，真实外部 side effect 被强制替换为 mock/recorded adapter。
...
| reportVersionLock   | `ReportVersionLock`              | 锁定 schema、policy、data snapshot、template、renderer 版本 |

`ComplianceTemplateRegistry` 必须额外保存 legal_version、change_source、effective_date 和 migration_rule。法律或框架变更不得只更新模板文本；必须生成影响分析、迁移任务和旧报告适用性说明。
...
报告自动生成不等于合规确认。`ComplianceReport` 进入 `generated` 后必须经过 `HumanSignoff` 才能标记为 `attested` 或对外提供。报告同时输出 `EvidenceQualityScore`（完整性、freshness、来源可信度、不可篡改性）和 `ControlCoverageReport`（pass/fail/partial/not_applicable、证据缺口、补救 owner）。

`HumanSignoff` 必须声明 `signoff_due_at`、escalation owner 和 timeout action。超过期限仍停留在 `generated` 的报告进入 `signoff_overdue`，自动提醒责任人并升级给 compliance owner；超过最大等待窗口后标记 `not_attested_expired`，不得对外发布为已确认报告。
...
| freshness | 证据最大允许年龄和实际采集时间 |
| owner | 证据责任人或系统 owner |
| exception | 例外、补偿控制和过期时间 |
...
- **最小权限**：AuditorAccess 按 framework、tenant、timeRange、controlId 和 evidence type 授权，默认不可见业务原文、secret、PHI 和跨租户证据
- **版本冻结**：已签核报告只能追加补充说明或勘误，不得原地覆盖；重新生成必须产生新 report version
...
MVP 容量规划不依赖高级预测模型，只强制五项能力：threshold alert、trend projection、queue depth forecast、provider quota monitor、failover capacity reserve。复杂预测准确率指标属于 Hardening/Enterprise 验收，不得作为 MVP 出口条件。
...
容量建议必须同时检查 SLA Tier、队列延迟、budget、approval capacity、provider quota 和 Region failover reserve。Enterprise SLA tenant admission 必须把 N+1 / failover capacity reserve 作为硬门禁；任一关键 provider quota 接近上限时，系统生成降级/限流建议，而不是只扩 Worker。

每个预测窗口结束后必须记录 forecast vs actual：误差、低估/高估方向、触发的扩容动作和业务影响。连续两个窗口超过误差阈值时，ForecastModel 进入 `needs_recalibration`，容量建议降级为人工复核，不得继续自动扩容或自动收缩。
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| partId | string | 内容块唯一标识 |
| modality | text / image / audio / video / document | 模态 |
| text | string? | 仅允许小段文本内联 |
| artifactRef | string? | 二进制或大对象必须引用 §26 Artifact，不得内联 base64 |
| mimeType | string | 媒体类型 |
| provenance | `ProvenanceMetadata` | 来源、生成方式、C2PA/水印、许可证、hash |
| safetyLabels | `string[]` | 内容安全扫描标签 |
...
- 文档解析结果受 §50 知识域隔离约束
- 视频、音频、图像、文档分别使用 modality-specific guardrails，不得复用文本安全阈值
- 所有生成或转换后的二进制输出先写 Artifact Store，经安全扫描、provenance 写入和成本结算后再返回 artifact_ref
...
| labels | 检出的安全标签 |
| confidence | 每个标签的置信度 |
| provider | 安全检查 provider / model |
| policyDecision | allow / filter / escalate / deny |
| appealPath | 申诉或人工复核路径 |
...
PlatformOps 必须显式声明 ops data boundary：只能读取平台运维数据（metrics、logs、traces、config、deployment、incident、capacity），默认不可读取业务 payload、客户原文、PHI/PII 或跨租户证据。需要关联业务证据时只读取脱敏摘要和 evidence_ref。
...
- 默认 read-only；`config_patch`、`restart_service`、`scale_replica`、`failover` 等写动作必须绑定 runbook、审批、blast radius 和 rollback plan
- 自运维不得成为唯一恢复路径。每个 P0/P1 平台故障必须有 out-of-band recovery runbook，可在 Chat、Agent、ModelGateway 或 P1/P3/P4/P5 任一平面不可用时由人工和基础设施控制面执行

## 69.4 自运维成熟度等级

| 等级 | 描述 | 验收边界 |
| ---- | ---- | -------- |
| L0   | 纯手动运维，Agent 仅辅助文档查询 | 不接触生产控制面 |
| L1   | Agent 生成诊断报告、日报、容量趋势和只读问答 | 不执行生产写操作 |
| L2   | Agent 生成修复方案、配置补丁候选并预执行验证 | 人工审批后由标准变更系统执行 |
| L3   | Agent 自动处理受限 P3/P4 级别问题 | 仅 known-runbook、低爆炸半径、可回滚动作，且需事后复核 |
...
v4.2 将历史大平台蓝图收敛为 MVP / Hardening / Enterprise 三个生产交付环，明确"先做最小生产闭环 → 再做运行加固 → 最后做企业扩张"。三环是 §33 的权威实施模型，从**能力维度**而非**时间维度**划分优先级。
...
| Canonical Domain Meta-Model                           | §37.11            | 元模型 15 问模板可填写、可校验                                      |
...
> v4.2 以三环为权威交付模型：MVP 8-12 周、Hardening 3-6 个月、Enterprise 6-18 个月。旧 Phase 1-9 仅作为历史映射和详细拆解，不再作为并列路线；Phase 8a-8d 已拆入 Ring 1/2 交付包，Phase 9 域波次不阻塞平台核心 milestone。

本节的 Ring 是唯一权威实施路线；后续 Phase 1-9 是历史映射和详细拆解，不作为 backlog source 或并行路线。
...
平台 milestone 按能力验收，不按域数量验收。Phase 9 改为 independent domain waves：平台目标是 `N/24 domains GA`，单个域延期不得阻塞 Harness / State / Evidence / Governance 主链。

### 33.0.1 Operational Readiness Matrix

Operational readiness 按 Ring 渐进验收，避免 MVP 被 Enterprise 目标拖垮，也避免 Enterprise 宣称生产级但缺少运维闭环。
...
- [ ] Internal Pack Registry 至少 3 个核心 Pack 通过认证；外部 Marketplace 20 个认证 Pack 作为 Enterprise/Future 独立验收
- [ ] 用户反馈→改进闭环 < 7 天
...
## 33.1 Legacy Phase 依赖图（仅映射）

以下图仅保留历史引用兼容，不作为 v4.2 实施顺序权威；实际交付按 §33.0 三环验收。
...
ADR 不再按“越多越安全”冻结。v4.2 只冻结会影响运行时契约、状态真相、安全治理和域合规边界的决策；产品体验、商业条款、示例实现和未来能力不得进入 P0/P1 ADR 阻塞主线。

ADR 冻结分三类：`MVP-blocking` 必须在 Ring 1 前冻结；`Hardening-before-production` 必须在真实生产租户前冻结；`Enterprise-before-scale` 必须在多 Region / Marketplace / 24 域规模化前冻结。只冻结 MVP-blocking 不代表其余 ADR 可绕过，而是避免 ADR 数量反向阻塞最小生产闭环。

## 34.0 ADR 元数据模板与优先级

每个 ADR 必须包含以下字段，缺少 runtime/schema/test 映射的 ADR 只能处于 `draft`：
...
| 优先级 | 冻结范围 | 示例 |
| --- | --- | --- |
| P0 Runtime | HarnessRun、PlanGraph、NodeRun、SideEffect、Budget、Replay 等不可绕过运行契约 | 改动需同步 schema、API、测试 |
| P1 State/Evidence | truth/event/projection/artifact/audit 的一致性和证据语义 | 改动需 migration + rebuild 策略 |
| P2 Safety/Governance | policy、approval、panic、secret、egress、tenant isolation | 改动需 denial-path regression |
| P3 Domain | 高风险域边界、监管证据、HITL/资质要求 | 改动需域 owner 签核 |
| P4 Product | UI、商业、推荐、市场化体验 | 不阻塞 MVP runtime |
...
- **ADR-NL-Intent-Must-Resolve-To-Confirmed-TaskSpec** — 自然语言输入必须经过 Intent 解析生成 TaskDraft，经澄清和用户确认后才可形成 TaskSpec / RequestEnvelope(§5.3)，禁止将原始文本直接传递给 Agent
...
目录树是目标态索引，不是一次性 scaffold 清单。MVP 只创建实际交付所需目录，避免 24 域和未来模块产生空壳。
...
24 个垂直域目录按产品化接入节奏创建；未进入试点的域只保留文档规范和 DomainDescriptor 示例，不创建空实现目录。
...
- `src/platform/five-plane-orchestration/harness/` 是 P3 Harness Runtime 的 canonical implementation path，承载 HarnessRun、PlanGraph、NodeRun、Budget、SideEffect、Replay 等生产语义；新增 Harness runtime 实现不得写入 `src/platform/harness/`。
- `src/platform/five-plane-orchestration/oapeflir/` 是 OAPEFLIR 语义框架、StageRationale、TraceProjection 与审计视图的 canonical path。
...
以下列表是风险目录；进入实施 backlog 时必须转换为 `RiskRegister` 记录：
...
硬约束按执行方式分层，避免把机器可验证不变量、策略规则、人工流程和领域合规混成一类：

| 层级 | 执行方式 | 示例 |
| --- | --- | --- |
| machine-enforced invariant | 代码、schema、状态机、CI 自动拒绝 | CAS + Lease + Fencing、Replay 不产生真实副作用、终态不可迁出 |
| policy-enforced rule | Policy Engine / Approval Engine 判定 | 高风险动作审批、数据驻留、预算上限、egress allowlist |
| human-governed process | 人工签核、演练、复核、事故报告 | break-glass 复核、合规报告签核、律师/医师审核 |
| domain-specific compliance | 域规范和监管证据 | CSAM 上报、SOX SoD、PHI 隔离、交易热路径无 LLM |
...
- **NL 输入必须经过 Intent 解析生成 TaskDraft，并在用户确认后形成 TaskSpec / RequestEnvelope(§5.3)，禁止原始文本直传**
- **目标分解递归深度 ≤ 5**
...
所有成功标准必须能映射到自动化测试、演练记录、审计证据或人工签核记录；不能验证的描述只能作为目标说明，不得作为 gate。

平台成功标准与域成功标准分离：平台核心只看 Harness / State / Evidence / Governance 的可运行闭环；24 域、12 原型和域 GA 数量按 domain wave 验收，不得反向阻塞核心平台投产。

Ring 1 / Ring 2 / Ring 3a / Ring 3b / Ring 3c 是本节的权威 gate 命名；旧 Phase 1-9 只保留为历史映射。MVP 只要求 Ring 1 中与 Harness 可执行闭环有关的最小切片；24 域 GA 属于 Ring 3c 扩张 gate，以 `N/24 domains GA` 作为进度指标，不阻塞核心平台投产。
...
### Ring 3a 成功标准（历史 Phase 6）

- 双 Region 单 leader / follower read 部署，受控 failover 演练通过，单 Region 故障 RTO < 5min
- active-active 仅用于非 truth 的缓存、遥测或聚合统计
- 1000 并发 workflow 下高优先级任务不饥饿
- SLA Tier P0 任务 99.9% 在承诺时间内完成
- Internal Pack Registry 至少 3 个核心 Pack 通过认证；外部 Marketplace 20 个认证 Pack 属于 Enterprise/Future 独立 gate
- 用户反馈→改进闭环 < 7 天
- 预构建 Connector 覆盖 P0 类别全部系统

### Ring 3b 成功标准（历史 Phase 7）
...
- 行为漂移检测覆盖 success_rate_drop、override_rate_spike、cost_spike、tool_usage_shift、incident_count 五类 MVP 指标；高级统计阈值需声明样本量和误报处理
...
- **业务域必须以统一元模型描述(§37.11)——15 问模板确保 24 域结构一致、配置驱动、新域可模板化接入**
- **多 Agent 协作必须遵循强制协议(§19.5)——权限不扩大、风险不提升、约束不绕过、审计不断链**
- **实施必须分环推进——生存环保底、可用环试点、扩张环规模化，避免面面俱到导致无一落地**

v4.3 的成功不是一次性覆盖全部域、全部市场能力和全部运营成熟度，而是先形成稳定的 Harness + State + Evidence + Governance 最小闭环；只有这个闭环可测试、可审计、可恢复，24 域和生态扩张才有安全基础。
...
| OAPEFLIR 是否可执行？    | 受控认知流程                                          | 不作为独立执行运行时；执行权威为 HarnessRuntime，OAPEFLIR 仅提供 StageRationale / TraceProjection / Audit View |
...
新 schema、API、事件和代码目录不得新增 deprecated term；只能在兼容 adapter、迁移脚本、历史 ADR 和术语表中出现。
...
| CAS                      | Compare-And-Swap                                           | 乐观并发控制原语，用于 StateMutationCommand 幂等写入(§5.4)                                                                                          |
...
| WorkProduct              | Generator Agent 标准化输出                                 | 包含 nodeRunId/artifacts/observations/telemetry(§45.9)                                                                                              |
...
| CDM                      | Canonical Domain Meta-Model                                | 统一领域元模型，24 域使用同一 15 问模板描述(§37.11)                                                                                                 |
...
OAPEFLIR v4.4 Executable Spec 是 v4.2 的历史输入与迁移来源，不再作为直接实现权威。v4.2 的运行契约以正文、Executable Runtime Contract、Schema / Zod / OpenAPI / Event Registry 为准；附录 H 只用于说明 v4.4 能力已经落到哪些正文章节。
...
v4.4 对象不按原名直接成为 v4.2 权威实现对象；以下映射用于确认“已吸收但改名/降级/投影化”的契约边界。

| v4.4 对象 | v4.2 收敛对象 / 状态 | 主文档落点 |
| --- | --- | --- |
| OapeflirRun | HarnessRun；旧名不再作为权威运行实体 | §13, §45.13, §25.9 |
| RunStatus | HarnessRun status + OapeflirTraceProjection 阶段视图 | §25.9, §13 |
| NodeRun / NodeRunStatus | NodeRun + NodeAttempt 状态机 | §14.1, §25.9 |
| AttemptLineage | retry / redrive 的 append-only 谱系 | §14.14, §28 |
| ObservationBundle / AssessmentBundle | Observation/Assessment 投影对象，不拥有状态 | §13.2, §13.5 |
| PlanGraphBundle / PlanGraph / PlanNode / PlanEdge | canonical execution contract | §5.3, §13.6-§13.12 |
| GraphNormalizationReport | PlanGraph normalize 输出 | §13.8 |
| GraphValidationReport | PlanGraph validate 输出 | §13.9 |
| GraphRiskPropagationReport | 图风险传播输出 | §13.10 |
| GraphWorstPathAnalysis | 最坏路径分析输出 | §13.11 |
| ReadyNodeSchedulingPolicy | Graph Scheduler policy | §13.12, §14.3 |
| GraphPatch / GraphPatchOperation / GraphPatchCompatibilityReport | 追加式 Replan 契约 | §13.13 |
...
| SideEffectRecord / SideEffectType / SideEffectStatus | SideEffect truth + 状态机 | §14.5, §28 |
| SideEffectExecutionContract / ReversibilityProfile | SideEffect delivery semantics 与可逆性画像 | §14.5 |
| ReconciliationRecord / ReconciliationStatus | 对账状态机 | §14.6 |
| ContextAssemblyContract / ContextItemRef | 角色隔离上下文装配契约 | §45.23 |
| PromptExecutionContract | Prompt 执行契约 | §45.24 |
| LlmDecisionRecord / DeterministicRuntimeSeed | LLM 记录输出与 Trace Replay 种子；默认 replay 复用 recorded output | §45.24, §58.4 |
| ToolOutputTaint | 工具输出污染标签与 taint propagation | §45.23, §45.24, §45.26 |
...
| HitlLock / HitlEscalationPolicy | HITL lock、timeout、escalation 策略 | §21, §45.18 |
| HumanResponsibilityRecord / HumanDecision | 人工责任记录与 DecisionDirective | §45.27, §5.3 |
| FinalOutputContract | Final output / artifact / evidence 输出契约 | §45.9, §59, §68 |
| CausalLineageQuery | lineage / explanation / forensic 查询能力 | §23.6, §28, §59 |
| RunVersionLock | admitted 时冻结版本 | §25.10 |
| EffectivePolicySnapshot | 生效策略快照 | §13.5, §24, §45.25 |
| LearningCandidate / LearningCandidateType | LearningCandidate quarantine / release gate | §13.14, §29.4, §56 |
| EvaluationGate / EvaluationReport | 发布前与运行时评测门禁 | §17, §45.10, §58.10 |
| ReleaseRecord | P2 Release Governance / rollout record | §13.2, §16, §56 |
| RepairRecord | repair / redrive / recovery 的追加证据 | §12, §14.14, §31 |
| ResumeCompatibilityPolicy | pause/resume 版本兼容策略 | §20, §25.10, §45.15 |
| VerificationResult | deterministic verification / EvaluationReport 输入 | §13.9, §45.25 |
| PrincipalRef | Principal / issuedBy / actor 统一身份引用 | §5, §6.5, §45.27 |
...
| v4.2 | 2026-04-27 | **可实现规格收敛版**：统一 v4.1/v4.4 权威关系，明确正文与 runtime/schema/event registry 为实现权威；废弃 `ExecutionPlan` canonical 名称并收敛为 `PlanGraphBundle`；将 `ControlDirective` 拆为 `OperationalDirective` / `DecisionDirective`；补齐核心运行对象 API、Webhook retry、Outbox poller failover、SLO P95/P99 口径、§25 读路径矩阵、§26 MVP 20 表上限、§29 Memory/Knowledge/Artifact/Learning contract、§33 MVP/Hardening/Enterprise 三环路线与附录 H 冲突裁决规则。 |
