...
- Position, loss limits, and trading hot path must not be overwritten by Agent.
- Ultra-low-latency order path must not rely on general LLM/harness loop.

## Acceptance Criteria

- DomainDescriptor, DomainRiskProfile, DomainEvalFramework must first pass §38 four-stage gate.
- Prior to GA, must provide trading risk control, backtesting, human approval, audit, and kill-switch evidence.
