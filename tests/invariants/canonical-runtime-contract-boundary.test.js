...
import { CANONICAL_CONTRACT_NAMES, LEGACY_CONTRACT_NAMES, } from "../../src/platform/contracts/executable-contracts/index.js";
...
test("legacy contract directories are compatibility surfaces, not canonical runtime entries", () => {
    ...
    assert.equal(CANONICAL_CONTRACT_NAMES.includes("ExecutionPlan"), false);
    assert.equal(CANONICAL_CONTRACT_NAMES.includes("ExecutionReceipt"), false);
    assert.equal(CANONICAL_CONTRACT_NAMES.includes("ControlDirective"), false);
});
...
//# sourceMappingURL=canonical-runtime-contract-boundary.test.js.map