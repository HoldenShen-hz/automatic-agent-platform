...
test("INV-DOMAIN-001: Critical domains must have humanAccountable=true", () => {
  ...
  const criticalDomainSpec = {
    ...
    compensationModel: ["manual_repair", "appeal"] as const,
    sideEffectTypes: [],
    approvalThresholds: {},
  };
  ...
  if (result.success) {
    assert.equal(
      result.data.humanAccountable,
      true,
      "Critical domain healthcare must declare humanAccountable=true",
    );
    assert.equal(
      result.data.deterministicHotPathOnly,
      true,
      "Critical domain healthcare must declare deterministicHotPathOnly=true",
    );
  }
});
...
test("INV-DOMAIN-001: High-risk domains must have humanAccountable=true", () => {
  ...
  const highRiskDomainSpec = {
    ...
    compensationModel: ["reversal", "manual_repair"] as const,
    sideEffectTypes: [],
    approvalThresholds: {},
  };
  ...
  if (result.success) {
    assert.equal(
      result.data.humanAccountable,
      true,
      "High-risk domain quant-trading must declare humanAccountable=true",
    );
  }
});
...
test("INV-DOMAIN-001: DomainRiskSpec validation blocks incomplete high-risk spec", () => {
  ...
  const incompleteSpec = {
    ...
    compensationModel: ["manual_repair"] as const,
    sideEffectTypes: [],
    approvalThresholds: {},
  };
  ...
  assert.equal(
    result.success,
    true,
    "Schema accepts the structure but domain release gate should reject humanAccountable=false for high-risk",
  );
});
...
test("INV-DOMAIN-001: Critical domain without deterministicHotPathOnly is blocked", () => {
  ...
  const invalidCriticalSpec = {
    ...
    compensationModel: ["appeal"] as const,
    sideEffectTypes: [],
    approvalThresholds: {},
  };
  ...
  assert.equal(
    result.success,
    true,
    "Schema accepts but critical domain release must require deterministicHotPathOnly=true",
  );
});
...
test("INV-DOMAIN-001: Low-risk domains may omit human accountability flags", () => {
  const lowRiskSpec = {
    ...
    compensationModel: ["no_compensation"] as const,
    sideEffectTypes: [],
    approvalThresholds: {},
  };
  ...
  if (result.success) {
    assert.equal(
      result.data.humanAccountable,
      false,
      "Low-risk domain may declare humanAccountable=false",
    );
    assert.equal(
      result.data.deterministicHotPathOnly,
      false,
      "Low-risk domain may declare deterministicHotPathOnly=false",
    );
  }
});
...
test("INV-DOMAIN-001: Domain release blocked when liabilityOwner is missing for high-risk", () => {
  ...
  assert.equal(
    result.success,
    false,
    "High-risk domain must have at least one liabilityOwner",
  );
});
...
test("INV-DOMAIN-001: Domain release blocked when compensationModel is missing for high-risk", () => {
  ...
  assert.equal(
    result.success,
    false,
    "Critical domain must have at least one compensation model declared",
  );
});
