...
function stripRemediationSection(text, heading) {
    const index = text.indexOf(heading);
    return index >= 0 ? text.slice(0, index) : text;
}
const CONTRACT_FILE_BY_ISSUE = {
...
test("ADR remediation directly encodes the first architecture-aligned ADR fixes", () => {
    ...
    assert.match(adr012, /Ring 1 MVP/);
    ...
    assert.doesNotMatch(adr012, /Phase 2 以后/);
    const adr013 = readFileSync("docs_zh/adr/013-eventemitter-phase-2-boundary.md", "utf8");
    assert.match(adr013, /Ring 1-2 进程内分发边界/);
    assert.match(adr013, /Ring 2 Hardening/);
    assert.match(adr013, /Ring 3 Enterprise/);
    assert.doesNotMatch(adr013, /Phase 2 是否替换/);
    const adr002 = readFileSync("docs_zh/adr/002-division-system.md", "utf8");
    assert.match(adr002, /Superseded by ADR-037 \/ ADR-046 \/ ADR-081/);
    assert.match(adr002, /DomainDescriptor \+ OrgUnit/);
    ...
    const releaseDecisionContract = readFileSync("docs_zh/contracts/release_decision_view_and_channel_contract.md", "utf8");
    assert.match(releaseDecisionContract, /ReleaseDecisionView/);
    assert.match(releaseDecisionContract, /ReleaseChannel/);
    assert.match(releaseDecisionContract, /projection-only/);
    ...
    const typedEventBusContract = readFileSync("docs_zh/contracts/typed_event_bus_contract.md", "utf8");
    assert.match(typedEventBusContract, /harnessRunId/);
    assert.match(typedEventBusContract, /nodeRunId/);
    assert.match(typedEventBusContract, /planGraphId/);
    assert.match(typedEventBusContract, /禁止使用废弃的 `task_id`、`workflow_id`、`execution_id` 作为主键字段/);
    const runtimeStateMachineContract = readFileSync("docs_zh/contracts/runtime_state_machine_contract.md", "utf8");
    assert.match(runtimeStateMachineContract, /只表达 stage projection/);
    assert.match(runtimeStateMachineContract, /不是 workflow \/ HarnessRun truth 状态机/);
    const taskWorkflowContract = readFileSync("docs_zh/contracts/task_and_workflow_contract.md", "utf8");
    assert.match(taskWorkflowContract, /历史事业部别名投影；可为空/);
    assert.match(taskWorkflowContract, /不得把它当作 v4\.3 domain binding 的权威来源/);
    const quotaContract = readFileSync("docs_zh/contracts/quota_preemption_and_fair_scheduling_contract.md", "utf8");
    assert.match(quotaContract, /quota_dimension/);
    assert.match(quotaContract, /v4\.3 Contract Remediation/);
    const orgHierarchyContract = readFileSync("docs_zh/contracts/org_hierarchy_and_dynamic_approval_contract.md", "utf8");
    assert.match(orgHierarchyContract, /harness_run_id/);
    assert.match(orgHierarchyContract, /v4\.3 Contract Remediation/);
    const feedbackImprovementContract = readFileSync("docs_zh/contracts/feedback_improvement_pipeline_contract.md", "utf8");
    assert.match(feedbackImprovementContract, /harness_run_id/);
    assert.match(feedbackImprovementContract, /release_channel_id/);
    assert.match(feedbackImprovementContract, /v4\.3 Contract Remediation/);
    const adr079 = readFileSync("docs_zh/adr/079-feedback-hub-signals.md", "utf8");
    assert.match(adr079, /aggregate\(harnessRunId: string\)/);
    assert.doesNotMatch(adr079, /aggregate\(taskId: string\)/);
    const adr080 = readFileSync("docs_zh/adr/080-learn-hub-pattern-detection.md", "utf8");
    assert.match(adr080, /src\/platform\/five-plane-orchestration\/learn\//);
    assert.doesNotMatch(adr080, /src\/core\/learning\//);
});
//# sourceMappingURL=contract-and-oapeflir-remediation.test.js.map
