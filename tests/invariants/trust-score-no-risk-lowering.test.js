...
test("INV-RISK-001: Critical domain maintains at least medium risk regardless of score", () => {
    ...
    assert.equal(computedHighTrust, "medium", "Critical domain must remain at least medium even with very high score (simulating trust influence)");
    ...
    assert.equal(computedLowTrust, "medium", "Critical domain must remain at least medium even with low score");
});
test("INV-RISK-001: TrustScore cannot upgrade domain risk classification", () => {
    ...
    assert.equal(computed, "low", "TrustScore must not upgrade low-risk domain to medium");
});
test("INV-RISK-001: Risk level is computed from risk factors, not trust", () => {
    ...
    assert.equal(computed, "high", "Low trust score must not downgrade high-risk domain");
    ...
    assert.equal(computedHigh, "high", "High trust score must not downgrade high-risk domain");
});
test("INV-RISK-001: TrustScore only reduces approval friction, not risk level", () => {
    ...
    assert.equal(computed, "medium", "TrustScore must not reduce medium domain to low risk");
...
test("INV-RISK-001: Risk computation follows deterministic path regardless of trust", () => {
    ...
    for (const score of scoreValues) {
        ...
        assert.ok(computed === "critical" || computed === "medium", `Critical domain at score ${score} must be critical or medium, got: ${computed}`);
    }
});
test("INV-RISK-001: Trust downgrade attempts must be audited", () => {
    ...
    assert.equal(computed, "medium", "RiskEngine must ignore trust-based baseRisk when computing effective risk for critical domain");
});
test("INV-RISK-001: Blast radius classification is independent of trust", () => {
    ...
    assert.equal(computed, "medium", "Platform blast radius must not shrink based on trust score");
});
//# sourceMappingURL=trust-score-no-risk-lowering.test.js.map