...
test("golden: all CLI scripts have JSDoc documentation", () => {
  ...
  for (const script of CLI_SCRIPTS) {
    ...
    try {
      ...
      assert.ok(
        content.startsWith("/**"),
        `${script} should start with JSDoc comment`,
      );

      // Verify @fileoverview or @module tag
      assert.ok(
        content.includes("@fileoverview") || content.includes("@module") || content.includes("@doc"),
        `${script} should have @fileoverview or @module tag`,
      );

      // Verify Usage section or Environment Variables section
      assert.ok(
        content.includes("Usage:") || content.includes("Environment Variables:") || content.includes("Usage example:"),
...
test("golden: CLI error codes follow consistent format", () => {
  ...
  for (const { code, format } of errorCodes) {
    if (format === "colon") {
      assert.ok(
        code.includes(":"),
        `Error code ${code} should have colon separator`,
      );
    } else {
      assert.ok(
        code.includes("."),
        `Error code ${code} should have dot separator`,
      );
    }
    ...
    assert.ok(
      parts.length >= 2,
      `Error code ${code} should have at least domain and identifier`,
    );
  }
});
...
test("golden: ValidationError format is consistent", () => {
  ...
  for (const { code, pattern } of errorCodes) {
    assert.ok(
      pattern.test(code),
      `Error code ${code} should match expected pattern`,
    );
  }
});
...
test("golden: CLI output is JSON when successful", () => {
  ...
  assert.ok(
    inspectSource.includes("JSON.stringify"),
    "CLI should output JSON via JSON.stringify",
  );
});
...
test("golden: CLI scripts use consistent error types", () => {
  ...
  for (const file of cliFiles) {
    try {
      ...
      assert.ok(
        content.includes('"../core/errors.js"') || content.includes("'../core/errors.js'") || content.includes('from "../core/errors.js"'),
        `${file} should import errors from core/errors`,
      );
    } catch {
    ...
    try {
      ...
      if (matches) {
        for (const match of matches) {
          assert.ok(
            aaPrefixPattern.test(match),
            `Environment variable ${match} in ${file} should follow AA_ naming convention`,
          );
        }
      }
    } catch {
...
test("golden: CLI scripts close database connections properly", () => {
  ...
  for (const file of cliFiles) {
    try {
      ...
      assert.ok(
        content.includes("withCliStorage") || content.includes("withCliStorageAsync"),
        `${file} should use withCliStorage for DB connection management`,
      );
    } catch {
      // File might not exist
    }
  ...
  for (const file of cliFiles) {
    try {
      ...
      if (content.includes("@see")) {
        assert.ok(
          content.includes("docs_zh/") || content.includes("@see"),
          `${file} should reference documentation via @see tags`,
        );
      }
    } catch {
      // File might not exist
