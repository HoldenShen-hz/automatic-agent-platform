...
test("performance: basic-planner suggestWorkflow() P99 < 200ms (trivial)", () => {
  ...
  assert.ok(
    p99 < 200,
    `basic-planner (trivial) P99 latency ${p99.toFixed(3)}ms exceeds 200ms target`,
  );

  assert.ok(
    p50 < 100,
    `basic-planner (trivial) P50 latency ${p50.toFixed(3)}ms seems unexpectedly high`,
  );
});
...
test("performance: basic-planner suggestWorkflow() P99 < 200ms (moderate)", () => {
  ...
  assert.ok(
    p99 < 200,
    `basic-planner (moderate) P99 latency ${p99.toFixed(3)}ms exceeds 200ms target`,
  );
});
...
test("performance: basic-planner suggestWorkflow() P99 < 200ms (complex)", () => {
  ...
  assert.ok(
    p99 < 200,
    `basic-planner (complex) P99 latency ${p99.toFixed(3)}ms exceeds 200ms target`,
  );
});
...
test("performance: basic-planner plugin lifecycle (initialize + suggest + healthCheck) P99 < 300ms", async () => {
  ...
  assert.ok(
    p99 < 300,
    `basic-planner lifecycle P99 latency ${p99.toFixed(3)}ms exceeds 300ms target`,
  );
});
