...
test("query: Get workflow state latency P99 <2ms", (t) => {
  const db = createTempDb();
  const store = new AuthoritativeTaskStore(db);
  ...
  try {
    ...
    // Warmup
    for (let i = 0; i < 50; i++) {
      store.getWorkflowState(workflowIds[i % workflowIds.length]!);
    }
    ...
    try {
      assert.ok(
        p99 < 2,
        `Get workflow state P99 latency ${p99.toFixed(4)}ms exceeds 2ms target. P50: ${p50.toFixed(4)}ms`,
      );
    } catch (err) {
      if (err instanceof assert.AssertionError) {
        reportSoftPerformanceMiss(t, err);
...
test("query: List workflow states throughput >1000 ops/sec", (t) => {
  const db = createTempDb();
  const store = new AuthoritativeTaskStore(db);
  ...
  try {
    ...
    for (let i = 0; i < iterations; i++) {
      store.listWorkflowStates({ limit: 50, offset: 0 });
    }
    ...
    try {
      assert.ok(
        opsPerSec > 1000,
        `List workflow states throughput ${opsPerSec.toFixed(0)} ops/sec must be >1000 ops/sec. Avg: ${avgLatencyMs.toFixed(4)}ms`,
      );
    } catch (err) {
      if (err instanceof assert.AssertionError) {
        reportSoftPerformanceMiss(t, err);
...
test("query: Insert session throughput >2000 ops/sec", (t) => {
  const db = createTempDb();
  const store = new AuthoritativeTaskStore(db);
  ...
  try {
    ...
    try {
      assert.ok(
        opsPerSec > 2000,
        `Insert session throughput ${opsPerSec.toFixed(0)} ops/sec must be >2000 ops/sec. Avg: ${avgLatencyMs.toFixed(4)}ms`,
      );
    } catch (err) {
      if (err instanceof assert.AssertionError) {
        reportSoftPerformanceMiss(t, err);
