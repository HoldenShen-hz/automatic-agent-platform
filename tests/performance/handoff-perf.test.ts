...
test("performance: serializeHandoff P99 < 5ms", () => {
  ...
  assert.ok(
    p99 < 5,
    `serializeHandoff P99 latency ${p99.toFixed(3)}ms exceeds 5ms target`,
  );

  assert.ok(
    p50 < 2,
    `serializeHandoff P50 latency ${p50.toFixed(3)}ms seems unexpectedly high`,
  );
});

test("performance: serializeHandoff with large payload P99 < 10ms", () => {
  // Create a handoff with larger payload to stress test
  const largeHandoff: AgentHandoff = {
    handoffId: newId("handoff"),
    taskId: newId("task"),
    fromAgentId: newId("agent"),
    toAgentId: newId("agent"),
  ...
  assert.ok(
    p99 < 10,
    `serializeHandoff (large payload) P99 latency ${p99.toFixed(3)}ms exceeds 10ms relaxed target`,
  );
});
