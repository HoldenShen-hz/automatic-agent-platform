...
describe("normalizeTierFromPlan via buildMatrix", () => {
    ...
    test("maps professional planId to professional tier", () => {
        const mockStore = createMockStore();
        mockStore.billing.getBillingAccount = () => ({ planId: "professional" });
        const service = new EnterpriseCapabilityMatrixService(createMockDb(), mockStore);
        const result = service.buildMatrix({
            accountId: "acct_pro",
            environment: "production",
    ...
    test("maps null planId to community tier", () => {
        const mockStore = createMockStore();
        mockStore.billing.getBillingAccount = () => ({ planId: null });
        const service = new EnterpriseCapabilityMatrixService(createMockDb(), mockStore);
        const result = service.buildMatrix({
            accountId: "acct_null",
            environment: "production",
    ...
    test("maps undefined planId to community tier", () => {
        const mockStore = createMockStore();
        mockStore.billing.getBillingAccount = () => ({ planId: undefined });
        const service = new EnterpriseCapabilityMatrixService(createMockDb(), mockStore);
        const result = service.buildMatrix({
    ...
    test("maps unknown planId to community tier", () => {
        const mockStore = createMockStore();
        mockStore.billing.getBillingAccount = () => ({ planId: "unknown_plan" });
        const service = new EnterpriseCapabilityMatrixService(createMockDb(), mockStore);
        const result = service.buildMatrix({
...
describe("DEFAULT_ENTERPRISE_CAPABILITIES structure", () => {
    test("all capabilities have requiredTier defined", () => {
        for (const cap of DEFAULT_ENTERPRISE_CAPABILITIES) {
            ...
            assert.ok(["community", "professional", "enterprise"].includes(cap.requiredTier), `Invalid tier ${cap.requiredTier} for ${cap.capabilityKey}`);
        }
    });
...
//# sourceMappingURL=enterprise-capability-matrix-helpers.test.js.map