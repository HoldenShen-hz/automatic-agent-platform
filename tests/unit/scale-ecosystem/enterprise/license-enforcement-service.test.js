...
import { LicenseEnforcementService } from "../../../../src/scale-ecosystem/enterprise/license-enforcement-service.js";
...
test("LicenseEnforcementService allows access when enforcement disabled", () => {
    const service = new LicenseEnforcementService(createMockStore(), { enabled: false });
...
test("LicenseEnforcementService denies feature not in gates", () => {
    const service = new LicenseEnforcementService(createMockStore());
...
test("LicenseEnforcementService denies enterprise feature for community tier in strict mode", () => {
    const service = new LicenseEnforcementService(createMockStore(), { strictMode: true });
    const result = service.checkFeatureAccess("sso", "community");
    assert.equal(result.allowed, false);
    assert.equal(result.action, "deny");
...
test("LicenseEnforcementService warns community tier for enterprise feature in non-strict mode", () => {
    const service = new LicenseEnforcementService(createMockStore(), { strictMode: false });
    const result = service.checkFeatureAccess("sso", "community");
    assert.equal(result.allowed, false);
...
test("LicenseEnforcementService does not log violations when disabled", () => {
    const service = new LicenseEnforcementService(createMockStore(), { logViolations: false });
    service.checkFeatureAccess("sso", "community");
    const violations = service.getViolations();
    assert.equal(violations.length, 0);
});
test("LicenseEnforcementService meters usage correctly", () => {
    const service = new LicenseEnforcementService(createMockStore(), { enableUsageMetering: true });
...
test("LicenseEnforcementService isEnabled and setEnabled work", () => {
    const service = new LicenseEnforcementService(createMockStore(), { enabled: true });
...
//# sourceMappingURL=license-enforcement-service.test.js.map