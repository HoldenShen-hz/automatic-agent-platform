...
describe("EnterpriseCapabilityMatrixService buildMarkdownReport (via exportMatrix)", () => {
    ...
    test("exportMatrix uses correct scope when tenantId is present", () => {
        ...
        mockStore.billing.getBillingAccount = () => ({ accountId: "acct_123", workspaceId: "ws_123", planId: "enterprise" });
    ...
    test("exportMatrix uses correct scope when workspaceId is present without tenantId", () => {
        const mockStore = createMockStore();
        mockStore.billing.getBillingAccount = () => ({ accountId: "acct_456", workspaceId: "ws_456", planId: "enterprise" });
        const service = new EnterpriseCapabilityMatrixService(createMockDb(), mockStore);
        const result = service.exportMatrix({
            accountId: "acct_456",
            environment: "production",
    ...
    test("exportMatrix report summary counts match entries", () => {
        ...
        assert.equal(result.report.summary.available + result.report.summary.degraded + result.report.summary.blocked, result.report.summary.total);
    });
    ...
    test("exportMatrix with enterprise tier allows enterprise features", () => {
        const mockStore = createMockStore();
        mockStore.billing.getBillingAccount = () => ({ accountId: "acct_ent", planId: "enterprise" });
        const service = new EnterpriseCapabilityMatrixService(createMockDb(), mockStore);
        const result = service.exportMatrix({
            accountId: "acct_ent",
            environment: "production",
...
//# sourceMappingURL=enterprise-capability-matrix-build-markdown.test.js.map