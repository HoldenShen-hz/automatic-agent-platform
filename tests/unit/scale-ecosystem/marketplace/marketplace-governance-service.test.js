...
test("marketplace governance service blocks publishing packages with rejected or missing review state", () => {
    ...
    try {
        ...
        assert.throws(() => service.publishPackage({ packageId: pkg.packageId }), /marketplace\.review_required/);
        const review = service.submitReview({
            packageId: pkg.packageId,
        ...
        assert.throws(() => service.publishPackage({ packageId: pkg.packageId, reviewId: review.reviewId }), /marketplace\.review_not_approved/);
        const summary = service.buildCatalog();
...
test("marketplace governance service fail-closes malformed identifiers and checksums", () => {
    ...
    try {
        ...
        const store = new AuthoritativeTaskStore(db);
        const service = new MarketplaceGovernanceService(db, store);
        assert.throws(() => service.registerExtensionPackage({
...
test("marketplace governance service scopes duplicate extension versions across tenants", () => {
    ...
    try {
        ...
        assert.throws(() => service.publishPackage({ tenantId: "tenant-b", packageId: tenantAPackage.packageId, reviewId: reviewA.reviewId }), /marketplace\.package_not_found/);
        db.close();
    }
    finally {
        cleanupPath(workspace);
...
//# sourceMappingURL=marketplace-governance-service.test.js.map