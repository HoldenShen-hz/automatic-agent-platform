...
test("buildMarkdownReport generates header section [report-format]", () => {
...
test("buildMarkdownReport includes window information [report-format]", () => {
  const report = createMockPmfReport({
...
test("buildMarkdownReport shows division scope [report-format]", () => {
...
test("buildMarkdownReport includes summary [report-format]", () => {
...
test("buildMarkdownReport renders all required metrics [report-format]", () => {
  const report = createMockPmfReport({
    metrics: {
      taskCount: 100,
...
test("buildMarkdownReport handles null optional metrics as n/a [report-format]", () => {
  const report = createMockPmfReport({
    metrics: {
      taskCount: 100,
      terminalTaskCount: 95,
...
test("buildMarkdownReport handles checks with null observed/threshold [report-format]", () => {
  const report = createMockPmfReport({
    checks: [
      {
        checkId: "sample_size",
...
function createMockPmfReport(
  overrides: Partial<Omit<PmfValidationReport, "metrics" | "checks" | "runtimeSummary">> & {
    metrics?: Partial<PmfValidationReport["metrics"]>;
    checks?: PmfValidationReport["checks"];
  },
): PmfValidationReport {
  return {
