...
test("selectPreferredRegion handles undefined status", () => {
  const regions = [
    { regionId: "us-east", jurisdiction: "US", latencyScore: 5, residencyAllowed: true, status: undefined as any },
    { regionId: "eu-west", jurisdiction: "EU", latencyScore: 20, residencyAllowed: true },
  ];
...
test("selectPreferredRegion handles undefined residencyAllowed", () => {
  const regions = [
    { regionId: "us-east", jurisdiction: "US", latencyScore: 5, residencyAllowed: undefined as any },
    { regionId: "eu-west", jurisdiction: "EU", latencyScore: 20, residencyAllowed: true },
  ];
...
test("CDCReplicationService getRegisteredRegionPairs handles malformed keys", () => {
  ...
  (service as any).configs.set("invalid-key", {
    sourceRegionId: "us-east",
...
test("RegionFailoverOrchestrator removeFailoverListener removes listener", () => {
  ...
  assert.equal((orchestrator as any).failoverListeners.has(listener), false);
});
...
function createTestReplicationEvent(id: string): ReplicationEvent {
  return {
    eventId: `evt_${id}`,
    sourceRegionId: "us-east",
    targetRegionId: "eu-west",
