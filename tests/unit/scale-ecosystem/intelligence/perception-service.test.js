...
test("PerceptionService registers a perception source", () => {
    const service = new PerceptionService(createMockDb(), createMockStore());
    const input = {
        type: "api",
        name: "Test Source",
    };
...
test("PerceptionService ingests intel items", () => {
    ...
    const source = { sourceId: "src_1", tenantId: null, type: "api", name: "Test", enabled: 1, scheduleJson: null, filtersJson: null, priority: 0, createdAt: "", updatedAt: "" };
...
test("PerceptionService skips duplicate intel items", () => {
    ...
    const source = { sourceId: "src_1", tenantId: null, type: "api", name: "Test", enabled: 1, scheduleJson: null, filtersJson: null, priority: 0, createdAt: "", updatedAt: "" };
    mockStore.intelligence.getPerceptionSource = () => source;
    mockStore.intelligence.getIntelItemBySourceAndDedupeKey = () => ({ intelId: "existing" }); // Return existing item
    const service = new PerceptionService(createMockDb(), mockStore);
    const candidate = {
...
test("PerceptionService builds intel brief", () => {
    ...
    mockStore.intelligence.listPerceptionSources = () => [{ sourceId: "src_1", enabled: 1, tenantId: null }];
    const service = new PerceptionService(createMockDb(), mockStore);
...
test("PerceptionService returns existing proposals idempotently", () => {
    const mockStore = createMockStore();
    const existingProposals = [{ proposalId: "prop_1", briefId: "brief_1" }];
...
test("PerceptionService lists sources", () => {
    const mockStore = createMockStore();
    mockStore.intelligence.listPerceptionSources = (enabledOnly) => {
        const sources = [
            { sourceId: "src_1", enabled: 1, tenantId: null },
            { sourceId: "src_2", enabled: 0, tenantId: null },
        ];
...
//# sourceMappingURL=perception-service.test.js.map