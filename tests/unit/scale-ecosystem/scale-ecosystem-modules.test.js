...
import { resolveRegionFailover, getNextFencingEpoch, } from "../../../src/scale-ecosystem/multi-region/failover-controller/index.js";
...
import { isQuotaExceeded, evaluateQuota, evaluateMultiDimensionalQuota, } from "../../../src/scale-ecosystem/resource-manager/quota-enforcer/index.js";
...
import { ConnectorManifestSchema, listEnabledConnectors, } from "../../../src/scale-ecosystem/integration/connector-registry/index.js";
import { buildConnectorExecutionKey, ConnectorExecutionRequestSchema, } from "../../../src/scale-ecosystem/integration/connector-runtime/index.js";
...
function createResidencyPolicy(overrides = {}) {
    return {
...
function createSlaTierProfile(overrides = {}) {
    return {
...
function createManifest(lifecycleState = "enabled", extra = {}) {
    return {
...
test("shouldReplicateToRegion respects blocked residency mode", () => {
    const policy = { sourceRegionId: "us-east", targetRegionIds: ["eu-west"], residencyMode: "blocked" };
    const result = shouldReplicateToRegion(policy, "eu-west");
    assert.equal(result, false);
});
test("shouldReplicateToRegion allows replication when not blocked", () => {
    const policy = { sourceRegionId: "us-east", targetRegionIds: ["eu-west"], residencyMode: "allowed_cross_border" };
    const result = shouldReplicateToRegion(policy, "eu-west");
    assert.equal(result, true);
});
test("shouldReplicateToRegion returns false for non-target region", () => {
    const policy = { sourceRegionId: "us-east", targetRegionIds: ["eu-west"], residencyMode: "same_jurisdiction" };
...
test("detectSlaBreach returns empty array when no breaches", () => {
    const observation = { latencyMs: 100, successRate: 0.99, queueWaitMs: 500 };
    const commitment = { maxLatencyMs: 200, minSuccessRate: 0.95, maxQueueWaitMs: 1000 };
    const breaches = detectSlaBreach(observation, commitment);
    assert.deepEqual(breaches, []);
});
test("detectSlaBreach detects latency breach", () => {
    const observation = { latencyMs: 300, successRate: 0.99, queueWaitMs: 500 };
    const commitment = { maxLatencyMs: 200, minSuccessRate: 0.95, maxQueueWaitMs: 1000 };
    const breaches = detectSlaBreach(observation, commitment);
    assert.ok(breaches.includes("sla.latency_breach"));
});
test("detectSlaBreach detects success rate breach", () => {
    const observation = { latencyMs: 100, successRate: 0.90, queueWaitMs: 500 };
    const commitment = { maxLatencyMs: 200, minSuccessRate: 0.95, maxQueueWaitMs: 1000 };
    const breaches = detectSlaBreach(observation, commitment);
    assert.ok(breaches.includes("sla.success_rate_breach"));
});
test("detectSlaBreach detects queue wait breach", () => {
    const observation = { latencyMs: 100, successRate: 0.99, queueWaitMs: 1500 };
    const commitment = { maxLatencyMs: 200, minSuccessRate: 0.95, maxQueueWaitMs: 1000 };
    const breaches = detectSlaBreach(observation, commitment);
    assert.ok(breaches.includes("sla.queue_wait_breach"));
});
test("detectSlaBreach returns multiple breaches", () => {
    const observation = { latencyMs: 300, successRate: 0.90, queueWaitMs: 1500 };
    const commitment = { maxLatencyMs: 200, minSuccessRate: 0.95, maxQueueWaitMs: 1000 };
    const breaches = detectSlaBreach(observation, commitment);
...
test("allocateReservedCapacity handles zero total units", () => {
    const allocations = [{ tierId: "enterprise", reservedPercent: 40 }];
...
//# sourceMappingURL=scale-ecosystem-modules.test.js.map