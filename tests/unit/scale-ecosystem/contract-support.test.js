...
test("scale-ecosystem support modules provide contract-aligned helpers", () => {
    ...
    assert.equal(canPublisherReleaseArtifact({ publisherId: "pub_1", displayName: "Publisher", trustLevel: "verified", allowedArtifactTypes: ["plugin"], reputationScore: 0.5, publishedArtifactCount: 10 }, "plugin"), true);
    ...
    assert.deepEqual(resolveRegionFailover({ primaryHealthy: false, candidateRegionIds: ["us-west-2"] }), {
        shouldFailover: true,
