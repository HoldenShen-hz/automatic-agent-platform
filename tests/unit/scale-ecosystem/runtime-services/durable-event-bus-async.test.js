...
test("DurableEventBusAsync is an EventEmitter subclass", () => {
    assert.ok(new DurableEventBusAsync({}, {}) instanceof EventEmitter);
});
...
test("DurableEventBusAsync dispose marks service as disposed", async () => {
    ...
    await assert.rejects(() => bus.publish({ eventType: "test", payload: {} }), (err) => err.message.includes("disposed"));
});
