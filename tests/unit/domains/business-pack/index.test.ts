...
import {
  ...
  type SandboxTier,
} from "../../../../src/domains/business-pack/index.js";
...
test("type exports are usable", () => {
  const stage: BusinessPackLifecycleStage = "draft";
  assert.equal(stage, "draft");

  const riskLevel: BusinessPackRiskLevel = "medium";
  assert.equal(riskLevel, "medium");

  const approvalPoint: ApprovalPointDef = {
    pointId: "ap-1",
    name: "Test Approval",
    description: "Test",
  ...
  const riskEntry: RiskMatrixEntry = {
    riskId: "risk-1",
    level: "high",
    triggers: ["data_loss"],
    mitigation: "backup",
  ...
  const validationIssue: ManifestValidationIssue = {
    code: "test.error",
    field: "testField",
    message: "Test error",
    severity: "error",
