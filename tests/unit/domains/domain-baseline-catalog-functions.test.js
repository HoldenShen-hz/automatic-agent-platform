...
import { listVerticalDomainBaselines, listVerticalDomainIds, listLegacyVerticalDomainIds, resolveCanonicalVerticalDomainId, getVerticalDomainBaseline, listVerticalDomainBaselinesByPhase, } from "../../../src/domains/domain-baseline-catalog.js";
...
test("listLegacyVerticalDomainIds maps to correct canonical IDs", () => {
    ...
    for (const [legacyId, canonicalId] of Object.entries(legacyToCanonical)) {
        assert.ok(ids.includes(legacyId), `Missing legacy ID: ${legacyId}`);
...
test("getVerticalDomainBaseline throws for unknown domain ID", () => {
    assert.throws(() => getVerticalDomainBaseline("nonexistent-domain"), /vertical_domain\.not_found/);
});
...
//# sourceMappingURL=domain-baseline-catalog-functions.test.js.map