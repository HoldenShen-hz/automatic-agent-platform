...
test("PluginSpiRegistry enforces maxConcurrentInvocations limit", async () => {
    ...
    const promises = Array.from({ length: 5 }, (_, i) => registry.invokeRetriever("plugin.concurrent", {
        query: {
            taskId: `task_${i}`,
            intent: "test",
            context: {},
...
test("PluginSpiRegistry queues invocations up to maxQueuedInvocations", async () => {
    ...
    const results = [];
    for (let i = 0; i < 6; i++) {
        results.push(registry.invokeRetriever("plugin.queue", {
            query: {
                taskId: `task_q_${i}`,
                intent: "test",
                context: {},
...
test("PluginSpiRegistry applies cooldown after failures", async () => {
    ...
    await assert.rejects(registry.invokeRetriever("plugin.cooldown", {
        query: {
            taskId: "task_fail_0",
            intent: "fail",
            context: {},
...
test("PluginSpiRegistry clears cooldown when invocations succeed", async () => {
    ...
    registry.register({
        ...
        async retrieve() {
            ...
            return [{ knowledgeRef: "knowledge:test", snippet: "test", score: 1, namespace: "test", chunkId: "c1", documentId: "d1", matchType: "keyword" }];
        },
    }, {
        pluginId: "plugin.clear_cooldown",
...
test("PluginSpiRegistry transitions to degraded after invocation failure", async () => {
    ...
    assert.ok(record);
    assert.ok(record.lifecycleState === "degraded" || record.lifecycleState === "disabled", `Expected degraded or disabled, got ${record.lifecycleState}`);
});
...
//# sourceMappingURL=plugin-spi-registry-invocation.test.js.map