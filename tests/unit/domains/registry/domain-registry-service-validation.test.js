...
test("DomainRegistryService throws on duplicate workflow IDs", () => {
    ...
    assert.throws(() => service.register(makeMinimalDomain({
...
test("DomainRegistryService allows plugin when manifest has no domain restrictions", () => {
    ...
    service.register(makeMinimalDomain({
        domainId: "any_domain",
        pluginBindings: [
            {
                bindingId: "b1",
...
test("DomainRegistryService buildCapabilityEntry throws for unknown domain", () => {
    const service = new DomainRegistryService();
    assert.throws(() => service.buildCapabilityEntry("nonexistent"), /domain_not_found/);
});
test("DomainRegistryService activate throws for unknown domain", () => {
    const service = new DomainRegistryService();
    assert.throws(() => service.activate("nonexistent"), /domain_not_found/);
});
test("DomainRegistryService deprecate throws for unknown domain", () => {
    const service = new DomainRegistryService();
    assert.throws(() => service.deprecate("nonexistent"), /domain_not_found/);
});
test("DomainRegistryService validate throws for unknown domain", () => {
    const service = new DomainRegistryService();
    assert.throws(() => service.validate("nonexistent"), /domain_not_found/);
});
//# sourceMappingURL=domain-registry-service-validation.test.js.map