...
test("DomainPromptGovernanceService.review throws for unknown prompt", () => {
  ...
  assert.throws(
    () => service.review(library, "unknown_prompt"),
    /prompt_governance.prompt_not_found/,
  );
});
...
test("DomainPromptGovernanceService.rollback throws when target version not found", () => {
  ...
  assert.throws(
    () => service.rollback(library, release.releaseId, "0.8"),
    /prompt_governance.rollback_target_missing/,
  );
});
