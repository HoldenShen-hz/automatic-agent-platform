...
test("expectNonEmptyString throws for non-string", () => {
    assert.throws(() => expectNonEmptyString(123, "ERR"), /ERR/);
});
...
test("throwDivisionValidationError throws ValidationError", () => {
    assert.throws(() => throwDivisionValidationError("ERR_CODE", { detail: "value" }), (err) => err.message === "ERR_CODE");
});
test("throwDivisionWorkflowError throws WorkflowStateError", () => {
    assert.throws(() => throwDivisionWorkflowError("WF_ERR", { detail: "value" }), (err) => err.message === "WF_ERR");
});
test("throwDivisionSandboxError throws SandboxError", () => {
    assert.throws(() => throwDivisionSandboxError("SB_ERR", { detail: "value" }), (err) => err.message === "SB_ERR");
});
