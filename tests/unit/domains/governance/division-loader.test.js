...
function seedDivisionTree(root) {
    ...
    createFile(join(root, "general_ops/division.yaml"), [
        "id: general_ops",
        "version: 1",
    ...
    createFile(join(root, "general_ops/workflows/minimal.yaml"), [
    ...
    createFile(join(root, "general_ops/workflows/orchestration.yaml"), [
    ...
    createFile(join(root, "engineering_ops/division.yaml"), [
        "id: engineering_ops",
        "version: 1",
    ...
    createFile(join(root, "engineering_ops/workflows/minimal.yaml"), [
    ...
    createFile(join(root, "engineering_ops/workflows/orchestration.yaml"), [
...
function seedWorkflowConfig(root, allowCrossDivisionDag) {
    createFile(join(root, "workflows/default.json"), JSON.stringify({
...
test("division loader loads multiple divisions, prompts, and workflows", () => {
    ...
    try {
        ...
        assert.equal(registry.workflows.get("engineering_multi_step_delivery")?.steps[2]?.roleId, "reviewer");
        assert.equal(registry.workflows.get("single_agent_minimal")?.steps[0]?.outputSchemaPath?.endsWith("general_ops/schemas/minimal-output.json"), true);
        assert.match(registry.divisions.get("engineering_ops")?.roles[0]?.promptText ?? "", /engineer/);
    }
    finally {
...
test("division loader rejects workflows that reference undefined roles", () => {
    ...
    try {
        seedDivisionTree(divisionsRoot);
        createFile(join(divisionsRoot, "engineering_ops/workflows/orchestration.yaml"), [
            "id: engineering_multi_step_delivery",
            "division_id: engineering_ops",
            "steps:",
            "  - step_id: analyze_request",
        ...
        assert.throws(() => new DivisionLoader({
            divisionsRoot,
            sandboxPolicy: createWorkspaceWritePolicy(workspace),
        }).loadAll(), /workflow\.role_missing/);
    }
    finally {
        cleanupPath(workspace);
    }
...
test("division loader accepts cross-division workflow steps when explicitly enabled", () => {
    ...
    try {
        seedDivisionTree(divisionsRoot);
        createFile(join(divisionsRoot, "general_ops/workflows/cross-division.yaml"), [
        ...
        assert.equal(registry.workflows.get("cross_division_delivery")?.steps[1]?.divisionId, "engineering_ops");
        assert.deepEqual(registry.workflows.get("cross_division_delivery")?.steps[1]?.inputKeys, ["triage"]);
    }
    finally {
        cleanupPath(workspace);
    }
...
test("division loader rejects cross-division workflow steps when the DAG feature is disabled", () => {
    ...
    try {
        ...
        assert.throws(() => new DivisionLoader({
            divisionsRoot,
            sandboxPolicy: createWorkspaceWritePolicy(workspace),
        }).loadAll(), /workflow\.cross_division_disabled/);
    }
    finally {
        cleanupPath(workspace);
    }
...
test("division loader rejects orphan workflow steps that are disconnected from the DAG", () => {
    ...
    try {
        seedDivisionTree(divisionsRoot);
        createFile(join(divisionsRoot, "general_ops/workflows/orphan.yaml"), [
            "id: orphan_workflow",
            "division_id: general_ops",
            "steps:",
            "  - step_id: intake",
        ...
        assert.throws(() => new DivisionLoader({
            divisionsRoot,
            sandboxPolicy: createWorkspaceWritePolicy(workspace),
        }).loadAll(), /workflow\.orphaned_step/);
    }
    finally {
        cleanupPath(workspace);
    }
...
test("division loader rejects disconnected workflow components that never join the main DAG", () => {
    ...
    try {
        seedDivisionTree(divisionsRoot);
        createFile(join(divisionsRoot, "general_ops/workflows/disconnected.yaml"), [
        ...
        assert.throws(() => new DivisionLoader({
            divisionsRoot,
            sandboxPolicy: createWorkspaceWritePolicy(workspace),
        }).loadAll(), /workflow\.disconnected_step/);
    }
    finally {
        cleanupPath(workspace);
    }
...
test("division loader rejects cross-division workflow steps that reference a missing division", () => {
    ...
    try {
        seedDivisionTree(divisionsRoot);
        createFile(join(divisionsRoot, "general_ops/workflows/cross-division-missing-division.yaml"), [
        ...
        assert.throws(() => new DivisionLoader({
            ...
            sandboxPolicy: createWorkspaceWritePolicy(workspace),
            allowCrossDivisionDag: true,
        }).loadAll(), /workflow\.step_division_missing/);
    }
    finally {
        cleanupPath(workspace);
    }
});
//# sourceMappingURL=division-loader.test.js.map