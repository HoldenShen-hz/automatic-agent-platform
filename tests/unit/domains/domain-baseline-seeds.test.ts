...
import type { VerticalDomainPhase } from "../../../src/domains/domain-baseline-catalog.js";
...
test("each DomainSeed phase is valid", () => {
  const validPhases: VerticalDomainPhase[] = ["9a", "9b", "9c", "9d", "9e", "9f"];
  for (const seed of DOMAIN_SEEDS) {
...
test("each DomainSeed riskLevel is valid", () => {
  const validRiskLevels: DomainRiskLevel[] = ["low", "medium", "high", "critical"];
  for (const seed of DOMAIN_SEEDS) {
