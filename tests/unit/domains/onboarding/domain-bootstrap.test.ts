...
test("DOMAIN_PHASE_BOOTSTRAP_SERVICE_IDS contains all 6 phases", () => {
  const phases: VerticalDomainPhase[] = ["9a", "9b", "9c", "9d", "9e", "9f"];
...
test("DOMAIN_PHASE_BOOTSTRAP_SERVICE_IDS has correct format for each phase", () => {
  const expectedIds: Record<VerticalDomainPhase, string> = {
  ...
  for (const [phase, expectedId] of Object.entries(expectedIds)) {
    assert.equal(
      DOMAIN_PHASE_BOOTSTRAP_SERVICE_IDS[phase as VerticalDomainPhase],
      expectedId,
      `Phase ${phase} should have correct service ID`,
    );
  }
});
...
test("buildDomainsBootstrap phases are in correct order (9a to 9f)", () => {
  const result = buildDomainsBootstrap();
  const phaseOrder: VerticalDomainPhase[] = ["9a", "9b", "9c", "9d", "9e", "9f"];
...
test("registerDomainsBootstrap registers services in ServiceRegistry", () => {
  ...
  for (const phase of ["9a", "9b", "9c", "9d", "9e", "9f"] as VerticalDomainPhase[]) {
    assert.ok(
      registry.isInitialized(DOMAIN_PHASE_BOOTSTRAP_SERVICE_IDS[phase]),
      `Phase ${phase} service should be initialized`,
    );
  }
});
...
test("DomainPhaseBootstrap interface structure is correct", () => {
  const phase: DomainPhaseBootstrap = buildDomainPhaseBootstrap("9a");

  // Check the type has expected readonly properties
  const phaseValue: VerticalDomainPhase = phase.phase;
  const baselinesReadonly: readonly unknown[] = phase.baselines;
  const serviceId: string = phase.registeredServiceId;
...
test("DomainsBootstrap interface structure is correct", () => {
  const bootstrap: DomainsBootstrap = buildDomainsBootstrap();

  const capabilityGroupId: string = bootstrap.capabilityGroupId;
  const catalogReadonly: readonly unknown[] = bootstrap.catalog;
  const phasesReadonly: readonly unknown[] = bootstrap.phases;
  const registeredIdsReadonly: readonly string[] = bootstrap.registeredServiceIds;
  const phaseIdsReadonly: readonly string[] = bootstrap.phaseServiceIds;
...
test("phases baselines are specific to their phase", () => {
  ...
  for (const phaseBootstrap of bootstrap.phases) {
    ...
    assert.deepEqual(
      phaseBootstrap.baselines,
      baselines,
      `Phase ${phase} baselines should match listVerticalDomainBaselinesByPhase`,
    );
  }
});
