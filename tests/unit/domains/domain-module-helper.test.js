...
test("createDomainModulePreset includes required capabilities", () => {
    const taskTypes = ["code_review", "refactor", "testing"];
    const preset = mod.createDomainModulePreset("coding", taskTypes, []);
    assert.deepEqual(preset.requiredCapabilities, taskTypes);
});
test("createDomainModulePreset includes review required task types", () => {
    const reviewTypes = ["security_scan", "compliance_check"];
    const preset = mod.createDomainModulePreset("coding", [], reviewTypes);
...
test("requiresPresetReview returns true for review-required task type", () => {
    const preset = mod.createDomainModulePreset("coding", ["code_review"], ["security_scan"]);
...
//# sourceMappingURL=domain-module-helper.test.js.map