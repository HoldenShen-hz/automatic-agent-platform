...
test("RoadmapService updateRoadmapItemStatus throws for non-existent item", () => {
    const service = new RoadmapService();
    assert.throws(() => service.updateRoadmapItemStatus("non_existent_item", "in_progress"), (err) => {
...
test("RoadmapService completeRoadmapItem throws for non-existent item", () => {
    const service = new RoadmapService();
    assert.throws(() => service.completeRoadmapItem("non_existent_item", { completedAt: "2026-04-23T10:00:00Z" }), (err) => {
        if (err instanceof Error && "code" in err) {
            return err.code === "roadmap.item_not_found";
        }
        return false;
...
test("RoadmapService deferRoadmapItem throws for non-existent item", () => {
    const service = new RoadmapService();
    assert.throws(() => service.deferRoadmapItem("non_existent_item", "some reason"), (err) => {
        if (err instanceof Error && "code" in err) {
            return err.code === "roadmap.item_not_found";
        }
        return false;
...
//# sourceMappingURL=roadmap.test.js.map