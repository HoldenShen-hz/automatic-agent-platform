...
import { DomainManifestSchema, DomainDefinitionSchema, DomainCapabilityProfileSchema, PluginBindingSchema, StepTemplateConfigSchema, WorkflowConfigSchema, ToolBundleConfigSchema, OutputContractConfigSchema, } from "../../../src/domains/registry/domain-model.js";
// ─────────────────────────────────────────────────────────────────────────────
...
test("DomainManifestSchema accepts all trust levels", () => {
    const levels = ["internal", "trusted", "community", "unverified"];
...
test("PluginBindingSchema accepts all standard plugin types", () => {
    const types = ["tool", "adapter", "retriever", "evaluator"];
...
test("OutputContractConfigSchema accepts all validation levels", () => {
    const levels = ["strict", "lenient", "none"];
    for (const level of levels) {
        const contract = OutputContractConfigSchema.parse({
...
test("DomainCapabilityProfileSchema accepts all security levels", () => {
    const levels = ["standard", "elevated", "restricted"];
    for (const level of levels) {
