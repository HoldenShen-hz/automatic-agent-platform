...
function registerTestDomain(service, domainId = "coding") {
    service.register({
        domainId,
        name: "Coding",
...
test("DomainOnboardingService.advance() merges evidence artifact IDs when advancing through same phase", () => {
    ...
    const devValidationRecord = session2.records.find((r) => r.phase === "pack_development");
...
test("DomainOnboardingService.advance() throws when no active phase exists", () => {
    ...
    assert.throws(() => service.advance("coding", ["artifact:extra"]), (error) => error.code === "domain_onboarding.no_active_phase");
});
test("DomainOnboardingService.advance() throws when evidence is empty", () => {
    ...
    const service = new DomainOnboardingService(registry);
    service.start("coding");
    assert.throws(() => service.advance("coding", []), (error) => error.code === "domain_onboarding.evidence_required");
});
...
test("DomainOnboardingService.block() throws when no active phase exists", () => {
    ...
    service.advance("coding", ["artifact:security"]);
    service.advance("coding", ["artifact:canary"]);
    assert.throws(() => service.block("coding", "reason-artifact"), (error) => error.code === "domain_onboarding.no_active_phase");
});
...
test("DomainOnboardingService.rollback() throws when no active phase exists", () => {
    ...
    service.advance("coding", ["artifact:security"]);
    service.advance("coding", ["artifact:canary"]);
    assert.throws(() => service.rollback("coding", "domain_modeling", "checkpoint", "reason"), (error) => error.code === "domain_onboarding.no_active_phase");
});
...
test("DomainOnboardingService.get() throws for unregistered domain", () => {
    const registry = new DomainRegistryService();
    const service = new DomainOnboardingService(registry);
    assert.throws(() => service.get("unknown"), (error) => error.code === "domain_onboarding.domain_not_found");
});
...
test("DomainOnboardingService.start() throws for unregistered domain", () => {
    const registry = new DomainRegistryService();
    const service = new DomainOnboardingService(registry);
    assert.throws(() => service.start("unknown"), (error) => error.code === "domain_onboarding.domain_not_found");
});
test("DomainOnboardingService.advance() throws for unregistered domain", () => {
    const registry = new DomainRegistryService();
    const service = new DomainOnboardingService(registry);
    assert.throws(() => service.advance("unknown", ["artifact"]), (error) => error.code === "domain_onboarding.domain_not_found");
});
test("DomainOnboardingService.advance() throws when session not started", () => {
    const registry = new DomainRegistryService();
    registerTestDomain(registry);
    const service = new DomainOnboardingService(registry);
    assert.throws(() => service.advance("coding", ["artifact"]), (error) => error.code === "domain_onboarding.session_not_started");
});
test("DomainOnboardingService.block() throws for unregistered domain", () => {
    const registry = new DomainRegistryService();
    const service = new DomainOnboardingService(registry);
    assert.throws(() => service.block("unknown", "artifact"), (error) => error.code === "domain_onboarding.domain_not_found");
});
test("DomainOnboardingService.block() throws when session not started", () => {
    const registry = new DomainRegistryService();
    registerTestDomain(registry);
    const service = new DomainOnboardingService(registry);
    assert.throws(() => service.block("coding", "artifact"), (error) => error.code === "domain_onboarding.session_not_started");
});
test("DomainOnboardingService.rollback() throws for unregistered domain", () => {
    const registry = new DomainRegistryService();
    const service = new DomainOnboardingService(registry);
    assert.throws(() => service.rollback("unknown", "domain_modeling", "artifact", "reason"), (error) => error.code === "domain_onboarding.domain_not_found");
});
test("DomainOnboardingService.rollback() throws when session not started", () => {
    const registry = new DomainRegistryService();
    registerTestDomain(registry);
    const service = new DomainOnboardingService(registry);
    assert.throws(() => service.rollback("coding", "domain_modeling", "artifact", "reason"), (error) => error.code === "domain_onboarding.session_not_started");
});
