...
test("DomainOnboardingService.block throws when no session exists", () => {
  const registry = new DomainRegistryService();
  registerTestDomain(registry, "test_block_no_session");
...
test("DomainOnboardingService.promotes domain status to active when onboarding completes", () => {
  const registry = new DomainRegistryService();
  registerTestDomain(registry, "test_promote", "draft");

  const service = new DomainOnboardingService(registry);
  service.start("test_promote");

  service.advance("test_promote", ["modeling"]);
  service.advance("test_promote", ["validation"]);
  service.advance("test_promote", ["security"]);
  service.advance("test_promote", ["canary"]);

  const domain = registry.get("test_promote");
