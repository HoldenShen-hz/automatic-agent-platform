    ...
    return {
        writeTextArtifact(input) {
            const record = {
                ...
                stepId: null,
                kind: input.kind,
                storagePath: `/fake/path/${input.taskId}/artifact_test_123/${input.fileName}`,
...
function createMockFeedback(outcome, signals) {
    return { outcome, signals };
}
function createMockPlan(taskId) {
    return { taskId };
}
test("ExecutionOutcomeEvaluator.evaluate returns pass for completed high score", () => {
    ...
    const result = evaluator.evaluate(plan, feedback);
...
test("QualityGateEvidenceService.persistEvaluation returns empty when disabled", () => {
    ...
    const service = new QualityGateEvidenceService({ artifactStore: store, config });
    const evaluation = {
...
test("QualityGateEvidenceService.persistEvaluation computes fail verdict when blocked", () => {
    ...
    const stored = JSON.parse(store.artifacts[0].input.content);
