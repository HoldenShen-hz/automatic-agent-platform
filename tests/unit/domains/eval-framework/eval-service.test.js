                ...
                return {
                    run(...args) {
                        if (sql.includes("INSERT INTO eval_suites")) {
                            ...
                            const [status, passedCases, failedCases, averageScore, verdict, completedAt, id] = args;
                    ...
                    all(...params) {
                        ...
                        if (isRun) {
                            if (params.length >= 3 && sql.includes("model_id") && sql.includes("prompt_version")) {
                                ...
                                const [suiteId, modelId, promptVersion] = params;
                                return tables.eval_runs.filter(r => r.suite_id === suiteId && r.model_id === modelId && r.prompt_version === promptVersion);
                            }
                            if (params.length > 0 && typeof params[0] === "string")
                                return tables.eval_runs.filter(r => r.suite_id === params[0]);
                            return tables.eval_runs.slice(0, params[0] ?? 50);
                        }
                        if (isSuite)
                            return tables.eval_suites;
                        return [];
                    },
                };
            },
...
//# sourceMappingURL=eval-service.test.js.map