...
import { PLATFORM_MAINLINE_CAPABILITIES, listPlatformMainlineCapabilities, resolvePlatformMainlineCapability, } from "../../src/platform/platform-mainline-bootstrap.js";
...
test("resolvePlatformMainlineCapability throws for unknown capabilityId", () => {
    assert.throws(() => resolvePlatformMainlineCapability("unknown-capability"), (error) => error.message.includes("platform_mainline.not_found"));
});
test("resolvePlatformMainlineCapability error message includes the unknown ID", () => {
    const unknownId = "nonexistent-capability";
    assert.throws(() => resolvePlatformMainlineCapability(unknownId), (error) => error.message.includes(unknownId));
});
...
test("PlatformMainlineCapabilityId type accepts all valid IDs", () => {
    const ids = VALID_CAPABILITY_IDS;
