...
import { getPlatformApplicationKernel, PlatformApplicationKernel, registerPlatformApplicationKernel, } from "../../src/platform-application-kernel.js";
...
test("PlatformApplicationKernel getApp throws for unknown kind", async () => {
    ...
    await registry.reset();
    const kernel = new PlatformApplicationKernel();
    assert.throws(() => kernel.getApp("unknown"), (err) => err.message.includes("Unknown platform app kind"));
});
...
test("PlatformApplicationKernel buildStartupPlan throws for unknown target kind", async () => {
    ...
    await registry.reset();
    const kernel = new PlatformApplicationKernel();
    assert.throws(() => kernel.buildStartupPlan("unknown"), (err) => err.message.includes("Unknown platform startup target"));
});
...
//# sourceMappingURL=platform-application-kernel.test.js.map