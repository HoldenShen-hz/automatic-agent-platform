...
test("[SYS-SEC-4.2] knowledge snapshot store rejects path traversal", () => {
    assert.throws(() => new KnowledgeSnapshotStore({ snapshotPath: "../../etc/passwd" }), { message: /sandbox|path|denied/i }, "Must reject paths outside sandbox root");
});
test("[SYS-SEC-4.2] knowledge snapshot store rejects absolute paths outside sandbox", () => {
    assert.throws(() => new KnowledgeSnapshotStore({ snapshotPath: "/etc/shadow" }), { message: /sandbox|path|denied/i }, "Must reject absolute paths outside sandbox");
});
...
//# sourceMappingURL=knowledge-snapshot-path.test.js.map