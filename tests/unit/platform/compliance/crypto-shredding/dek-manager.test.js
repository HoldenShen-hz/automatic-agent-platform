...
import { DekStore, DekManager } from "../../../../../src/platform/compliance/crypto-shredding/dek-manager.js";
...
test("DekStore.create throws when active DEK already exists", async () => {
    ...
    await assert.rejects(async () => store.create({ subjectId: "user-123" }), (err) => err.code === "dek.active_exists");
});
...
test("DekStore.markRotated throws for non-existent DEK", async () => {
    const store = new DekStore();
    await assert.rejects(async () => store.markRotated("unknown_dek", "some_replacement"), (err) => err.code === "dek.not_found");
});
test("DekStore.markRotated throws for non-active DEK", async () => {
    const store = new DekStore();
    const result = await store.create({ subjectId: "user-123" });
    await store.destroy(result.metadata.dekId);
    await assert.rejects(async () => store.markRotated(result.metadata.dekId, "replacement"), (err) => err.code === "dek.not_active");
});
...
test("DekStore.destroy throws for non-existent DEK", async () => {
    const store = new DekStore();
    await assert.rejects(async () => store.destroy("unknown_dek"), (err) => err.code === "dek.not_found");
});
...
test("DekManager.createForSubject validates subject ID", async () => {
    const manager = new DekManager();
    await assert.rejects(async () => manager.createForSubject(""), (err) => err.code === "dek.missing_subject");
    await assert.rejects(async () => manager.createForSubject("   "), (err) => err.code === "dek.missing_subject");
});
...
test("DekManager.encryptForSubject throws when no DEK exists", async () => {
    const manager = new DekManager();
    await assert.rejects(async () => manager.encryptForSubject("unknown-user", "data"), (err) => err.code === "dek.not_found");
});
...
test("DekManager.decrypt throws for destroyed DEK", async () => {
    ...
    await assert.rejects(async () => manager.decrypt(created.metadata.dekId, "some:ciphertext"), (err) => err.code === "dek.destroyed");
});
test("DekManager.decrypt throws for invalid ciphertext format", async () => {
    const manager = new DekManager();
    await manager.createForSubject("user-123");
    const active = await manager.getActiveDek("user-123");
    await assert.rejects(async () => manager.decrypt(active.dekId, "invalid-format"), (err) => err.code === "dek.invalid_ciphertext");
});
...
//# sourceMappingURL=dek-manager.test.js.map