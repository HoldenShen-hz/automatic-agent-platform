...
test("FieldEncryptionService protectRecord skips non-string field values", () => {
    ...
    const protectedRec = result.protectedRecord;
...
test("FieldEncryptionService protectRecord throws for empty keyRef", () => {
    ...
    assert.throws(() => service.protectRecord({
        ...
        if (error instanceof Error && "code" in error) {
            return error.code === "field_encryption.missing_key_ref";
        }
        return false;
    });
});
...
test("FieldEncryptionService revealField throws for key mismatch", () => {
    ...
    assert.throws(() => service.revealField({
        ciphertext: result.protectedRecord.name,
        keyRef: "kms://tenant-a/wrong-key",
    }), (error) => {
        if (error instanceof Error && "code" in error) {
            return error.code === "field_encryption.key_mismatch";
        }
        return false;
    });
});
...
test("FieldEncryptionService protectRecord handles nested object creation", () => {
    ...
    const nested = result.protectedRecord;
...
test("FieldEncryptionService protectRecord handles multiple levels of nesting", () => {
    ...
    const protectedRecord2 = result.protectedRecord;
    const l1 = protectedRecord2.level1;
    const l2 = l1.level2;
    const l3 = l2.level3;
...
test("FieldEncryptionService protectRecord preserves non-protected fields", () => {
    ...
    const protectedRecord = result.protectedRecord;
