...
test("FieldEncryptionService protectRecord encrypts specified field", () => {
  ...
  const rules = [
    { fieldPath: "email", classification: "confidential" as const },
  ];
...
test("FieldEncryptionService protectRecord handles nested fields", () => {
  ...
  const rules = [
    { fieldPath: "user.email", classification: "confidential" as const },
  ];
  ...
  assert.notEqual((result.protectedRecord as any).user.email, "john@example.com");
  assert.equal((result.protectedRecord as any).user.name, "John Doe");
});
...
test("FieldEncryptionService revealField decrypts ciphertext", () => {
  ...
  const revealed = service.revealField({
    ciphertext: (protectedResult.protectedRecord as any).email,
    keyRef: "key_123",
  });
...
test("FieldEncryptionService protectRecord encrypts with correct classification", () => {
  ...
  const rules = [
    { fieldPath: "email", classification: "restricted" as const },
  ];
...
test("FieldEncryptionService protectRecord handles multiple rules", () => {
  ...
  const rules = [
    { fieldPath: "email", classification: "confidential" as const },
    { fieldPath: "phone", classification: "internal" as const },
  ];
...
test("FieldEncryptionService protectRecord does not modify original record", () => {
  ...
  service.protectRecord({
    record,
    rules: [{ fieldPath: "email", classification: "confidential" as const }],
    keyRef: "key_123",
  });
