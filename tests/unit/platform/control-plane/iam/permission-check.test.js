...
import { checkSandboxPath, createRestrictedExecPolicy, createScopedExternalAccessPolicy, createWorkspaceWritePolicy, createConfigReadPolicy, resolveSandboxPath, } from "../../../../../src/platform/control-plane/iam/sandbox-policy.js";
...
test("SandboxPolicy type accepts all four modes", () => {
    const modes = ["read_only", "workspace_write", "scoped_external_access", "restricted_exec"];
    modes.forEach((mode) => {
        const policy = makeSandboxPolicy({ mode });
        assert.equal(policy.mode, mode);
    });
});
//# sourceMappingURL=permission-check.test.js.map