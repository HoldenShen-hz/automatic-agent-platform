...
import { evaluateAuthorizationContext, capabilitiesForRole, roleGrantsCapabilities, listPlatformRoles, inferCapabilitiesForAction, } from "../../../../../src/platform/control-plane/iam/access-model.js";
// ---------------------------------------------------------------------------
...
test("PolicyEngine.evaluate handles system subjectType with high-risk action", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    const result = engine.evaluate(makeRequest({ subjectType: "system", riskCategory: "destructive", mode: "auto" }));
...
test("PolicyEngine.evaluate cost_sensitive risk category does not escalate in supervised mode", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    const result = engine.evaluate(makeRequest({ mode: "supervised", riskCategory: "cost_sensitive" }));
    assert.equal(result.decision, "allow_with_constraints");
    assert.equal(result.requiresApproval, false);
});
test("PolicyEngine.evaluate sensitive_data risk category does not escalate in supervised mode", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    const result = engine.evaluate(makeRequest({ mode: "supervised", riskCategory: "sensitive_data" }));
    assert.equal(result.decision, "allow_with_constraints");
    assert.equal(result.requiresApproval, false);
});
test("PolicyEngine.evaluate cost_sensitive does not escalate in auto mode", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    const result = engine.evaluate(makeRequest({ mode: "auto", riskCategory: "cost_sensitive" }));
    assert.equal(result.decision, "allow_with_constraints");
    assert.equal(result.requiresApproval, false);
});
// ---------------------------------------------------------------------------
...
test("PolicyEngine.evaluate invoke_model action in supervised mode", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    const result = engine.evaluate(makeRequest({ action: "invoke_model", mode: "supervised", riskCategory: "sensitive_data" }));
    assert.equal(result.decision, "allow_with_constraints");
});
test("PolicyEngine.evaluate write_file action with destructive risk in supervised mode", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    const result = engine.evaluate(makeRequest({ action: "write_file", mode: "supervised", riskCategory: "destructive" }));
    assert.equal(result.decision, "escalate_for_approval");
});
test("PolicyEngine.evaluate network_access action with cost_sensitive risk in auto mode", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    const result = engine.evaluate(makeRequest({ action: "network_access", mode: "auto", riskCategory: "cost_sensitive" }));
    assert.equal(result.decision, "allow_with_constraints");
});
test("PolicyEngine.evaluate org_change action in full-auto mode", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    const result = engine.evaluate(makeRequest({ action: "org_change", mode: "full-auto", riskCategory: "org_changing" }));
    assert.equal(result.decision, "allow_with_constraints");
    assert.equal(result.requiresApproval, false);
});
test("PolicyEngine.evaluate install_extension action in auto mode with org_changing risk", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    const result = engine.evaluate(makeRequest({ action: "install_extension", mode: "auto", riskCategory: "org_changing" }));
    assert.equal(result.decision, "escalate_for_approval");
    assert.equal(result.requiresApproval, true);
});
// ---------------------------------------------------------------------------
...
test("PolicyEngine.evaluate handles resourceRef in request", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    const result = engine.evaluate(makeRequest({ resourceRef: "file:///workspace/script.sh" }));
    assert.equal(result.decision, "allow_with_constraints");
});
...
test("PolicyEngine.evaluate kill switch applies to all actions regardless of risk", () => {
    ...
    const result = engine.evaluate(makeRequest({ action: "org_change", riskCategory: "org_changing", mode: "full-auto" }));
    assert.equal(result.decision, "deny");
    assert.equal(result.killSwitchApplied, true);
});
// ---------------------------------------------------------------------------
...
test("PolicyEngine.evaluate denies when currentTaskCostUsd + estimatedCostUsd exceeds limit", () => {
    ...
    const result = engine.evaluate(makeRequest({ estimatedCostUsd: 3, metadata: { currentTaskCostUsd: 3 } }));
    assert.equal(result.decision, "deny");
...
test("PolicyEngine.evaluate allows when currentTaskCostUsd + estimatedCostUsd equals limit", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy({ maxTaskCostUsd: 5 }) });
    const result = engine.evaluate(makeRequest({ estimatedCostUsd: 2, metadata: { currentTaskCostUsd: 3 } }));
    assert.equal(result.decision, "allow_with_constraints");
});
// ---------------------------------------------------------------------------
...
test("PolicyEngine.evaluate explainSummary describes escalation", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    const result = engine.evaluate(makeRequest({ mode: "supervised", riskCategory: "destructive" }));
...
//# sourceMappingURL=policy-engine-edge-cases.test.js.map