...
test("secret management service throws StorageError when resolving unknown secret", async () => {
    ...
    try {
        ...
        await assert.rejects(async () => service.resolveSecret({
            secretRef: "secret://system/unknown/secret",
            requestedBy: "test",
            grantedTo: "test",
            usagePurpose: "test",
        }), (err) => err.code === "secret.registry_not_found:secret://system/unknown/secret");
    }
    finally {
        harness.db.close();
...
test("secret management service throws PolicyDeniedError when resolving disabled secret", async () => {
    ...
    try {
        ...
        await assert.rejects(async () => service.resolveSecret({
            ...
            grantedTo: "test",
            usagePurpose: "test",
        }), (err) => err.code.startsWith("secret.registry_unavailable"));
    }
    finally {
        harness.db.close();
        cleanupPath(harness.workspace);
...
test("secret management service throws ProviderError when provider is not registered", async () => {
    ...
    try {
        ...
        await assert.rejects(async () => service.resolveSecret({
            ...
            grantedTo: "test",
            usagePurpose: "test",
        }), (err) => err.code === "secret.provider_not_registered:kms");
    }
    finally {
        harness.db.close();
        cleanupPath(harness.workspace);
...
test("secret management service throws StorageError when revoking unknown lease", () => {
    ...
    try {
        const service = new SecretManagementService(harness.db, harness.store);
        assert.throws(() => service.revokeSecretLease({
            leaseId: "unknown-lease-id",
            revokedBy: "ops.revoke",
            reasonCode: "test",
        }), (err) => err.code === "secret.lease_not_found:unknown-lease-id");
    }
    finally {
        harness.db.close();
        cleanupPath(harness.workspace);
...
test("secret management service throws ValidationError for invalid status on registration", () => {
    ...
    try {
        const service = new SecretManagementService(harness.db, harness.store);
        assert.throws(() => service.registerSecret({
        ...
        }), (err) => err.code === "secret.invalid_status");
    }
    finally {
        harness.db.close();
        cleanupPath(harness.workspace);
...
test("secret management service throws ValidationError for invalid category on registration", () => {
    ...
    try {
        ...
        assert.throws(() => service.registerSecret({
            ...
            rotationPolicy: { cadenceDays: 30, ttlMinutes: 60, breakGlass: false },
        }), (err) => err.code === "secret.invalid_category");
    }
    finally {
        harness.db.close();
        cleanupPath(harness.workspace);
...
test("secret management service throws ValidationError for invalid provider kind on registration", () => {
    ...
    try {
        ...
        assert.throws(() => service.registerSecret({
            ...
            category: "provider_api_key",
            providerKind: "invalid_provider",
            scopeType: "system",
            scopeRef: "system.invalid.provider",
            rotationPolicy: { cadenceDays: 30, ttlMinutes: 60, breakGlass: false },
        }), (err) => err.code === "secret.invalid_provider_kind");
    }
    finally {
        harness.db.close();
        cleanupPath(harness.workspace);
...
test("secret management service throws ValidationError for invalid scope type on registration", () => {
    ...
    try {
        ...
        assert.throws(() => service.registerSecret({
            ...
            rotationPolicy: { cadenceDays: 30, ttlMinutes: 60, breakGlass: false },
        }), (err) => err.code === "secret.invalid_scope_type");
    }
    finally {
        harness.db.close();
        cleanupPath(harness.workspace);
...
test("secret management service throws ValidationError for empty secret ref", () => {
    ...
    try {
        ...
        assert.throws(() => service.registerSecret({
            ...
            rotationPolicy: { cadenceDays: 30, ttlMinutes: 60, breakGlass: false },
        }), (err) => err.code === "secret.invalid_ref");
    }
    finally {
        harness.db.close();
        cleanupPath(harness.workspace);
...
test("secret management service throws ValidationError for empty display name", () => {
    ...
    try {
        ...
        assert.throws(() => service.registerSecret({
            ...
            rotationPolicy: { cadenceDays: 30, ttlMinutes: 60, breakGlass: false },
        }), (err) => err.code === "secret.invalid_display_name");
    }
    finally {
        harness.db.close();
        cleanupPath(harness.workspace);
...
test("secret management service throws ValidationError for invalid lease expiry", async () => {
    ...
    try {
        ...
        await assert.rejects(async () => service.issueSecretLease({
            secretRef: "secret://system/expiry/test",
            requestedBy: "ops.expiry",
            grantedTo: "test-worker",
            usagePurpose: "expiry_test",
            expiresAt: "2020-01-01T00:00:00.000Z",
        }), (err) => err.code.startsWith("secret.invalid_lease_expiry"));
    }
    finally {
        harness.db.close();
        cleanupPath(harness.workspace);
...
test("secret management service throws ValidationError for invalid rotation mode", () => {
    ...
    try {
        ...
        assert.throws(() => service.recordRotationEvent({
            secretRef: "secret://system/invalid/rotation/mode",
            rotationMode: "invalid_mode",
            status: "requested",
            reasonCode: "test",
            requestedBy: "test",
        }), (err) => err.code === "secret.invalid_rotation_mode");
    }
    finally {
        harness.db.close();
        cleanupPath(harness.workspace);
...
test("secret management service throws ValidationError for invalid rotation status", () => {
    ...
    try {
        ...
        assert.throws(() => service.recordRotationEvent({
            ...
            reasonCode: "test",
            requestedBy: "test",
        }), (err) => err.code === "secret.invalid_rotation_status");
    }
    finally {
        harness.db.close();
        cleanupPath(harness.workspace);
...
test("secret management service describeSecret throws on disabled secret", async () => {
    ...
    try {
        ...
        await assert.rejects(async () => service.describeSecret("secret://system/describe/disabled"), (err) => err.code.startsWith("secret.registry_unavailable"));
    }
    finally {
        harness.db.close();
        cleanupPath(harness.workspace);
...
test("secret management service requireSecret throws on revoked secret", async () => {
    ...
    try {
        ...
        await assert.rejects(async () => service.requireSecret("secret://system/require/revoked"), (err) => err.code.startsWith("secret.registry_unavailable"));
    }
    finally {
        harness.db.close();
        cleanupPath(harness.workspace);
...
//# sourceMappingURL=secret-management-service-edge-cases.test.js.map