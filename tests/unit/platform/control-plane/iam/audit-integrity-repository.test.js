...
import { createAuditIntegrityRepository, AUDIT_INTEGRITY_DDL } from "../../../../../src/platform/control-plane/iam/audit-integrity-repository.js";
...
test("insertIntegrityRecord stores and retrieves record", () => {
    const db = createMockDatabase();
    const repo = createAuditIntegrityRepository(db);
    repo.insertIntegrityRecord("evt-123", 1, "task:status_changed", "2026-04-07T00:00:00.000Z", "checksum-abc", null, "chain-hash-1");
...
test("getLatestChainHash returns most recent chain hash", () => {
    const db = createMockDatabase();
    const repo = createAuditIntegrityRepository(db);
    repo.insertIntegrityRecord("evt-1", 1, "task:created", "2026-04-07T00:00:00.000Z", "checksum-1", null, "hash-1");
    repo.insertIntegrityRecord("evt-2", 2, "task:started", "2026-04-07T00:01:00.000Z", "checksum-2", "hash-1", "hash-2");
...
test("insertIntegrityRecord and getIntegrityRecord handle string chain position", () => {
    const db = createMockDatabase();
    const repo = createAuditIntegrityRepository(db);
    repo.insertIntegrityRecord("evt-str", 42, "workflow:step_completed", "2026-04-07T00:00:00.000Z", "checksum-str", "prev-hash-str", "chain-hash-str");
...
//# sourceMappingURL=audit-integrity-repository.test.js.map