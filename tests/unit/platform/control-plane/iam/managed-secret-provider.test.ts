...
const createMockProvider = (): ManagedSecretProvider & {
  mockDescribe: () => Promise<SecretProviderMetadata>;
  mockRequire: () => Promise<SecretProviderMetadata & { value: string }>;
} => {
  const mockProvider: ManagedSecretProvider & {
    mockDescribe: () => Promise<SecretProviderMetadata>;
    mockRequire: () => Promise<SecretProviderMetadata & { value: string }>;
  } = {
    providerKind: "environment" as SecretProviderKind,
    mockDescribe: async () => ({
...
test("ManagedSecretProvider interface allows optional issueSecretLease", () => {
  ...
  const basicProvider: ManagedSecretProvider = {
...
test("ManagedSecretProvider with issueSecretLease implementation", () => {
  const providerWithLease: ManagedSecretProvider = {
    providerKind: "vault",
    async describeSecret(secretRef) {
      return {
        secretRef,
    ...
    async issueSecretLease(secretRef) {
      const result: SecretProviderIssuedLease = {
        secretRef,
        envName: "VAULT_SECRET",
        scope: "secret",
        source: "vault",
...
test("SecretProviderMetadata structure requirements", () => {
  const metadata: SecretProviderMetadata = {
...
test("SecretProviderIssuedLease extends metadata with lease info", () => {
  const lease: SecretProviderIssuedLease = {
...
test("ManagedSecretProvider works with different provider kinds", () => {
  const providers: Array<{ kind: SecretProviderKind; name: string }> = [
