...
// Minimal mock for SecretLeaseRecord (only fields needed for testing)
interface MockLeaseRecord {
...
test("assertNonEmpty throws ValidationError for empty string", () => {
  assert.throws(
    ...
    (e: any) => e instanceof ValidationError && e.code === "test.empty",
  );
});
...
test("assertNonEmpty throws ValidationError for whitespace-only string", () => {
  assert.throws(
    ...
    (e: any) => e instanceof ValidationError && e.code === "test.whitespace",
  );
});

test("assertEnum returns value when valid", () => {
  const result = assertEnum("red", ["red", "green", "blue"] as const, "test.enum");
  assert.equal(result, "red");
});

test("assertEnum throws ValidationError for invalid value", () => {
  assert.throws(
    () => assertEnum("yellow", ["red", "green", "blue"] as const, "test.invalid"),
    (e: any) => e instanceof ValidationError && e.code === "test.invalid",
  );
});
...
test("computeLeaseExpiry throws for invalid issuedAt", () => {
  assert.throws(
    ...
    (e: any) => e instanceof ValidationError && e.code === "secret.invalid_lease_issued_at",
  );
});

test("normalizeLeaseStatus returns record status when not active", () => {
  const record: MockLeaseRecord = {
    status: "revoked",
    expiresAt: "2026-04-14T00:01:00.000Z",
  };

  const result = normalizeLeaseStatus(record as any, "2026-04-14T00:00:30.000Z");
...
test("normalizeLeaseStatus returns expired when past expiresAt", () => {
  ...
  const result = normalizeLeaseStatus(record as any, "2026-04-14T00:02:00.000Z");
...
test("normalizeLeaseStatus returns expired when at exact expiry time", () => {
  ...
  const result = normalizeLeaseStatus(record as any, "2026-04-14T00:01:00.000Z");
