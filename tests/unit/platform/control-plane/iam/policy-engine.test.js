...
function makeBudgetPolicy(overrides = {}) {
    return {
        maxTaskCostUsd: 10,
        maxDailyCostUsd: 100,
...
test("PolicyEngine.evaluate throws ValidationError for empty decisionId", () => {
    ...
    assert.throws(() => engine.evaluate(makeRequest({ decisionId: "" })), (err) => err.code === "policy.invalid_decision_id");
});
test("PolicyEngine.evaluate throws ValidationError for empty taskId", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    assert.throws(() => engine.evaluate(makeRequest({ taskId: "" })), (err) => err.code === "policy.invalid_task_id");
});
test("PolicyEngine.evaluate throws ValidationError for empty subjectId", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    assert.throws(() => engine.evaluate(makeRequest({ subjectId: "" })), (err) => err.code === "policy.invalid_subject_id");
});
test("PolicyEngine.evaluate throws ValidationError for invalid action", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    assert.throws(() => engine.evaluate(makeRequest({ action: "" })), (err) => err.code === "policy.invalid_action");
});
test("PolicyEngine.evaluate throws ValidationError for invalid riskCategory", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    assert.throws(() => engine.evaluate(makeRequest({ riskCategory: "" })), (err) => err.code === "policy.invalid_risk_category");
});
test("PolicyEngine.evaluate throws ValidationError for invalid mode", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    assert.throws(() => engine.evaluate(makeRequest({ mode: "" })), (err) => err.code === "policy.invalid_mode");
});
...
test("PolicyEngine.evaluate escalates high-risk action in supervised mode", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    const result = engine.evaluate(makeRequest({ mode: "supervised", riskCategory: "destructive", estimatedCostUsd: 1 }));
...
test("PolicyEngine.evaluate escalates irreversible action in supervised mode", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    const result = engine.evaluate(makeRequest({ mode: "supervised", riskCategory: "irreversible", estimatedCostUsd: 1 }));
    assert.equal(result.decision, "escalate_for_approval");
    assert.equal(result.requiresApproval, true);
});
test("PolicyEngine.evaluate escalates prod_affecting action in supervised mode", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    const result = engine.evaluate(makeRequest({ mode: "supervised", riskCategory: "prod_affecting", estimatedCostUsd: 1 }));
    assert.equal(result.decision, "escalate_for_approval");
    assert.equal(result.requiresApproval, true);
});
test("PolicyEngine.evaluate escalates org_changing action in supervised mode", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    const result = engine.evaluate(makeRequest({ mode: "supervised", riskCategory: "org_changing", estimatedCostUsd: 1 }));
    assert.equal(result.decision, "escalate_for_approval");
    assert.equal(result.requiresApproval, true);
});
test("PolicyEngine.evaluate escalates high-risk action in auto mode", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    const result = engine.evaluate(makeRequest({ mode: "auto", riskCategory: "destructive", estimatedCostUsd: 1 }));
    assert.equal(result.decision, "escalate_for_approval");
    assert.equal(result.requiresApproval, true);
...
test("PolicyEngine.evaluate allows non-high-risk action in auto mode without escalation", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    const result = engine.evaluate(makeRequest({ mode: "auto", riskCategory: "sensitive_data", estimatedCostUsd: 1 }));
    assert.equal(result.decision, "allow_with_constraints");
    assert.equal(result.requiresApproval, false);
});
test("PolicyEngine.evaluate allows high-risk action in full-auto mode without approval", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    const result = engine.evaluate(makeRequest({ mode: "full-auto", riskCategory: "destructive", estimatedCostUsd: 1 }));
    assert.equal(result.decision, "allow_with_constraints");
    assert.equal(result.requiresApproval, false);
});
...
test("PolicyEngine.evaluate uses currentTaskCostUsd from metadata for budget check", () => {
    ...
    const result = engine.evaluate(makeRequest({ estimatedCostUsd: 1, metadata: { currentTaskCostUsd: 4 } }));
...
test("mapToolRiskToPolicyCategory maps low/other to sensitive_data", () => {
    ...
    assert.equal(mapToolRiskToPolicyCategory("unknown"), "sensitive_data");
});
...
test("PolicyEngine.evaluate handles exec_command as high-risk action in supervised mode", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    const result = engine.evaluate(makeRequest({ mode: "supervised", action: "exec_command", riskCategory: "destructive" }));
    assert.equal(result.decision, "escalate_for_approval");
    assert.equal(result.requiresApproval, true);
});
test("PolicyEngine.evaluate handles install_extension as org_changing in supervised mode", () => {
    const engine = new PolicyEngine({ budgetPolicy: makeBudgetPolicy() });
    const result = engine.evaluate(makeRequest({ mode: "supervised", action: "install_extension", riskCategory: "org_changing" }));
    assert.equal(result.decision, "escalate_for_approval");
    assert.equal(result.requiresApproval, true);
});
