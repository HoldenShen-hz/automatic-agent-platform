...
describe("RolloutStateMachine", () => {
    describe("transition", () => {
        ...
        test("throws error for invalid transition from draft to stable", () => {
            ...
            assert.throws(() => stateMachine.transition(candidate, "stable"), /Invalid rollout transition/);
        });
        ...
        test("throws error for rejected candidate transitioning to shadow", () => {
            ...
            assert.throws(() => stateMachine.transition(candidate, "shadow"), /Invalid rollout transition/);
        });
        ...
        test("rejected status is terminal (only rejects allowed)", () => {
            const stateMachine = new RolloutStateMachine();
            const candidate = createMockCandidate({ status: "rejected" });
            assert.throws(() => stateMachine.transition(candidate, "off"), /Invalid rollout transition: rejected -> rolled_back/);
        });
...
//# sourceMappingURL=rollout-state-machine.test.js.map