...
import { VoteType, createInitialQuorumStatus, calculateQuorumStatus, isQuorumMet, isDenied, getRemainingVotes, mergeVotes, createVote, determineFinalStatus, calculateWithVote, validateVote, countEffectiveVotes, hasApproverVoted, getApproverVote, QuorumCalculator, } from "../../../../../../src/platform/control-plane/approval-center/quorum-calculator.js";
...
test("calculateQuorumStatus denies with enough rejections", () => {
    const config = { minApprovals: 2, minRejectionsToDeny: 2 };
    const startTime = "2026-04-26T00:00:00.000Z";
    const currentTime = "2026-04-26T00:05:00.000Z";
    const votes = [
...
test("calculateQuorumStatus detects expired voting window", () => {
    const config = { minApprovals: 2, minRejectionsToDeny: 3, votingWindowMs: 60000 };
    const startTime = "2026-04-26T00:00:00.000Z";
...
test("calculateQuorumStatus with active window", () => {
    const config = { minApprovals: 2, minRejectionsToDeny: 3, votingWindowMs: 600000 };
    const startTime = "2026-04-26T00:00:00.000Z";
...
test("calculateQuorumStatus counts unique approvers", () => {
    const config = { minApprovals: 3, minRejectionsToDeny: 3 };
    const startTime = "2026-04-26T00:00:00.000Z";
    const currentTime = "2026-04-26T00:05:00.000Z";
    const votes = [
        { approverId: "user1", voteType: VoteType.APPROVE, votedAt: startTime },
...
test("mergeVotes adds new vote when not exists", () => {
    ...
    const newVote = { approverId: "user2", voteType: VoteType.REJECT, votedAt: "2026-04-26T00:01:00.000Z" };
...
test("mergeVotes updates existing vote", () => {
    ...
    const newVote = { approverId: "user1", voteType: VoteType.REJECT, votedAt: "2026-04-26T00:01:00.000Z" };
    const merged = mergeVotes(existing, newVote);
...
test("calculateWithVote combines merge and status calculation", () => {
    const existing = [];
    const newVote = { approverId: "user1", voteType: VoteType.APPROVE, votedAt: "2026-04-26T00:00:00.000Z" };
...
test("validateVote throws on empty approverId", () => {
    const vote = { approverId: "", voteType: VoteType.APPROVE, votedAt: "2026-04-26T00:00:00.000Z" };
...
test("validateVote throws on invalid vote type", () => {
    const vote = { approverId: "user1", voteType: "invalid", votedAt: "2026-04-26T00:00:00.000Z" };
    assert.throws(() => validateVote(vote), /Invalid vote type/);
});
test("validateVote throws on missing votedAt", () => {
    const vote = { approverId: "user1", voteType: VoteType.APPROVE };
    assert.throws(() => validateVote(vote), /votedAt timestamp/);
});
test("validateVote does not throw for valid vote", () => {
    const vote = { approverId: "user1", voteType: VoteType.APPROVE, votedAt: "2026-04-26T00:00:00.000Z" };
    assert.doesNotThrow(() => validateVote(vote));
});
test("validateVote does not throw for valid vote with delegation", () => {
    const vote = { approverId: "del", voteType: VoteType.APPROVE, votedAt: "2026-04-26T00:00:00.000Z", delegationSource: "orig" };
    assert.doesNotThrow(() => validateVote(vote));
});
