...
function createBundle(rules, enabled = true) {
    return {
...
test("evaluate matches rule with in operator", () => {
    ...
    const engine = new ApprovalPolicyEngine(bundle);
    const result = engine.evaluate(createMockContext({ action: "exec_command" }));
...
test("evaluate does not match rule with in when value not in array", () => {
    ...
    const engine = new ApprovalPolicyEngine(bundle);
    const result = engine.evaluate(createMockContext({ action: "read" }));
    assert.equal(result.deny, false);
});
// ---------------------------------------------------------------------------
...
test("evaluate requires all conditions to match with AND logic", () => {
    ...
    const result1 = engine.evaluate(createMockContext({ riskCategory: "destructive", mode: "supervised" }));
    assert.equal(result1.requiresApproval, true);
    // Only one condition matches
    const result2 = engine.evaluate(createMockContext({ riskCategory: "destructive", mode: "auto" }));
...
test("evaluate handles deny action", () => {
    ...
    const engine = new ApprovalPolicyEngine(bundle);
    const result = engine.evaluate(createMockContext({ action: "delete_all" }));
    assert.equal(result.deny, true);
    assert.equal(result.requiresApproval, false);
});
...
test("evaluate handles require_multi_party_approval action", () => {
    ...
    const engine = new ApprovalPolicyEngine(bundle);
    const result = engine.evaluate(createMockContext({ riskCategory: "critical" }));
    assert.equal(result.requiresApproval, true);
