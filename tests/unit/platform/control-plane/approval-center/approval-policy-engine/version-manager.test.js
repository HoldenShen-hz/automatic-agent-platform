...
import { PolicyVersionManager, createDefaultVersionManager, } from "../../../../../../src/platform/control-plane/approval-center/approval-policy-engine/index.js";
...
test("PolicyVersionManager createDraft throws for non-existent base version", () => {
    ...
    const manager = new PolicyVersionManager(bundle);
    assert.throws(() => manager.createDraft("nonexistent-bundle", "99.99.99", "user-1"), /Base version 99.99.99 not found/);
});
test("PolicyVersionManager updateDraft only works on draft bundles", () => {
    ...
    assert.throws(() => manager.updateDraft(active, "user-1"), /Cannot update non-draft bundle/);
});
...
test("PolicyVersionManager submitForApproval throws for non-draft", () => {
    ...
    assert.throws(() => manager.submitForApproval(active, "user-1", "Should fail"), /Cannot submit non-draft bundle/);
});
...
test("PolicyVersionManager approve throws for non-pending bundle", () => {
    ...
    assert.throws(() => manager.approve(draft, "approver-1", "approval-123"), /Cannot approve bundle with status/);
});
...
test("PolicyVersionManager deprecate cannot deprecate active version", () => {
    ...
    const manager = new PolicyVersionManager(bundle);
    assert.throws(() => manager.deprecate("deprecate-active", "1.0.0", "admin-1"), /Cannot deprecate the currently active version/);
});
...
test("PolicyVersionManager compareVersions throws for missing versions", () => {
    ...
    const manager = new PolicyVersionManager(bundle);
    assert.throws(() => manager.compareVersions("compare-missing", "1.0.0", "99.99.99"), /One or both versions not found/);
});
