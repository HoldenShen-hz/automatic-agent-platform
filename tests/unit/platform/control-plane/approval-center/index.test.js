...
createInitialQuorumStatus, calculateQuorumStatus, createVote, mergeVotes, isQuorumMet, isDenied, getRemainingVotes, determineFinalStatus, validateVote, hasApproverVoted, getApproverVote, countEffectiveVotes, VoteType, 
// From escalation-manager
EscalationManager, EscalationReason, DelegationStatus, NotificationChannelType, NotificationPriority, 
// From approval-flow-engine
ApprovalFlowEngine, FlowType, FlowStatus, } from "../../../../../src/platform/control-plane/approval-center/index.js";
// ============================================
...
test("calculateQuorumStatus detects denial", () => {
    ...
    const config = { minApprovals: 2, minRejectionsToDeny: 1 };
    const status = calculateQuorumStatus(votes, config, "2026-04-14T00:00:00.000Z", "2026-04-14T00:00:00.000Z");
...
test("calculateQuorumStatus tracks voting window expiration", () => {
    const votes = [];
    const config = { minApprovals: 2, minRejectionsToDeny: 2, votingWindowMs: 1000 };
...
test("validateVote throws for invalid vote", () => {
    assert.throws(() => validateVote({ approverId: "", voteType: VoteType.APPROVE, votedAt: "2026-04-14T00:00:00.000Z" }), /valid approverId/);
    assert.throws(() => validateVote({ approverId: "user_1", voteType: "invalid", votedAt: "2026-04-14T00:00:00.000Z" }), /Invalid vote type/);
});
