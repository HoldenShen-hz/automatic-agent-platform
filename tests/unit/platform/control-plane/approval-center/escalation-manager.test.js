...
import { EscalationManager, NotificationChannelType, NotificationPriority, DelegationStatus, EscalationReason, } from "../../../../../src/platform/control-plane/approval-center/escalation-manager.js";
function createTestRule() {
    return {
        escalateTo: { type: "role", identifier: "senior-engineer", can_delegate: true },
...
function createTestContext(approvalId = "approval-1", currentLevel = 0) {
    return {
        approvalId,
        taskId: "task-1",
        executionId: "exec-1",
...
test("EscalationManager createEscalation throws when max depth exceeded", () => {
    ...
    assert.throws(() => manager.createEscalation(context, rule), /Cannot escalate beyond max depth/);
});
...
test("EscalationManager createDelegation throws on self-delegation", () => {
    const manager = new EscalationManager();
    assert.throws(() => manager.createDelegation("approver-1", "approver-1", "approval-1"), /Cannot delegate to yourself/);
});
...
test("EscalationManager resetDelegationTtl throws when max resets exceeded", () => {
    ...
    assert.throws(() => manager.resetDelegationTtl(delegation), /Cannot reset TTL more than/);
});
test("EscalationManager resetDelegationTtl throws when not active", () => {
    ...
    assert.throws(() => manager.resetDelegationTtl(delegation), /Cannot reset TTL on inactive delegation/);
});
...
test("EscalationManager revokeDelegation throws for unknown delegation", () => {
    const manager = new EscalationManager();
    assert.throws(() => manager.revokeDelegation("unknown-delegation"), /Delegation not found/);
});
...
test("EscalationManager completeDelegation throws for unknown delegation", () => {
    const manager = new EscalationManager();
    assert.throws(() => manager.completeDelegation("unknown-delegation"), /Delegation not found/);
});
...
//# sourceMappingURL=escalation-manager.test.js.map