...
test("BudgetPeriod type accepts valid values", () => {
  const validPeriods: BudgetPeriod[] = ["monthly", "weekly", "per_run"];
...
test("CostAlertAction type accepts valid values", () => {
  const validActions: CostAlertAction[] = [
    "sev1_alert",
    "sev2_alert",
    "sev3_alert",
    "queue_slowdown",
...
test("CostAlertLevel type accepts valid values", () => {
  const validLevels: CostAlertLevel[] = ["ok", "warning", "critical", "exceeded"];
...
test("CostAlertReasonCode type accepts valid values", () => {
  const validCodes: CostAlertReasonCode[] = [
    "cost.ok",
...
test("eventTier values are correct for each alert level", () => {
  const eventTiers: Array<{ alertLevel: CostAlertLevel; tier: "tier_1" | "tier_2" | "tier_3" }> = [
