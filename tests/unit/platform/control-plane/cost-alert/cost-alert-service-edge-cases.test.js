...
function createPolicy(scope, scopeId, limit) {
    return {
        scope,
        scopeId,
        period: "monthly",
...
test("CostAlertService emits exceeded event at limit", (t, done) => {
    ...
    const service = new CostAlertService(mockDb, mockStore, config);
    const events = [];
    service.on("cost.threshold.exceeded", (event) => events.push(event));
...
//# sourceMappingURL=cost-alert-service-edge-cases.test.js.map