...
function createMockStore(): {
  ...
  event: {
    insertEvent: (event: { id: string; eventType: string; payloadJson: string; traceId: string | null; createdAt: string }) => void;
  };
  ...
  return {
    compliance: {
      ...
      listErasureReportsByTenant: (_tenantId: string): ErasureReport[] => {
...
test("ErasureRequestService.createRequest accepts all legal basis options", () => {
  ...
  const bases = ["gdpr_article_17", "gdpr_article_17_1", "gdpr_article_17_3", "other"] as const;
...
test("ErasureReportService.verifyCryptoShredding handles missing DEK (assumed destroyed)", () => {
  ...
  assert.ok(verification.messages.some((m: string) => m.includes("not found")));
});
...
test("DataEncryptionKeyService.createDek sets default algorithm when not provided", () => {
  ...
  const mockStore = createMockStore();
  const mockDb = createMockDb();
  const service = new DataEncryptionKeyService(mockDb as any, mockStore as any);
...
test("DataResidencyService.validateDataPlacement throws for EU target with cross-border restriction", () => {
  ...
  assert.throws(
    ...
    (err: any) => err.code === "residency.cross_border_not_allowed:eu-west-1",
  );
});
...
test("DataResidencyService handles all data categories", () => {
  ...
  const categories: DataCategory[] = ["personal", "financial", "health", "biometric", "children", "government", "business"];
