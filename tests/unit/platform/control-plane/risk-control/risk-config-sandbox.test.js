...
test("loadRiskConfig with sandbox policy denying path throws error", () => {
    ...
    try {
        ...
        try {
            ...
            assert.ok(errorMessage.includes("denied") ||
                errorCode === "config.risk_denied" ||
                errorCode?.includes("sandbox"), `Expected sandbox-related error, got: ${errorMessage} (code: ${errorCode})`);
        }
...
test("loadRiskConfig with sandbox policy denies path traversal", () => {
    ...
    try {
        ...
        try {
            ...
            const errorCode = error.code;
            assert.ok(errorCode === "config.risk_denied" ||
                errorCode?.includes("sandbox"), `Expected sandbox denial, got error code: ${errorCode}`);
        }
...
test("loadRiskConfig with sandbox policy handles missing file", () => {
    ...
    try {
        ...
        const policy = createConfigReadPolicy(configRoot);
        assert.throws(() => loadRiskConfig(configPath, policy), (error) => error instanceof Error);
    }
    finally {
        rmSync(tempDir, { recursive: true, force: true });
    }
...
//# sourceMappingURL=risk-config-sandbox.test.js.map