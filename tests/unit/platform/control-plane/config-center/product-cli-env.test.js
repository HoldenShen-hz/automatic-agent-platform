...
import { loadComplianceProgramCliEnv, loadEvolutionCliEnv, loadHaProgramCliEnv, loadPerceptionCliEnv, loadPmfCliEnv, } from "../../../../../src/platform/control-plane/config-center/product-cli-env.js";
...
test("product CLI env loaders reject malformed values", () => {
    assert.throws(() => loadPmfCliEnv({ AA_PMF_ACTION: "history", AA_PMF_LIMIT: "oops" }), /invalid_env:AA_PMF_LIMIT/);
    assert.throws(() => loadHaProgramCliEnv({ AA_ENVIRONMENT: "moon" }), /invalid_env:AA_ENVIRONMENT/);
    assert.throws(() => loadPerceptionCliEnv({ AA_PERCEPTION_ACTION: "sources", AA_SOURCES_ENABLED_ONLY: "yes" }), /invalid_env:AA_SOURCES_ENABLED_ONLY/);
});
//# sourceMappingURL=product-cli-env.test.js.map