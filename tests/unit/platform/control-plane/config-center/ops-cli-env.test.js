...
import { loadDispatchReconcileCliEnv, loadLeaseHandoverCliEnv, loadEventOpsCliEnv, loadOrphanCleanupCliEnv, loadReplayRecoveryCliEnv, loadProfileHomeCliEnv, loadAuthoritativeStorageAdminCliEnv, } from "../../../../../src/platform/control-plane/config-center/ops-cli-env.js";
...
test("loadDispatchReconcileCliEnv throws on invalid action", () => {
    assert.throws(() => loadDispatchReconcileCliEnv({ AA_DISPATCH_RECONCILE_ACTION: "invalid" }), (err) => err instanceof ValidationError && err.code === "dispatch_reconcile.invalid_action");
});
...
test("loadLeaseHandoverCliEnv throws on missing required env vars", () => {
    assert.throws(() => loadLeaseHandoverCliEnv({}), (err) => err instanceof ValidationError && err.code === "missing_env:AA_LEASE_ID");
});
...
test("loadLeaseHandoverCliEnv throws on invalid ttlMs", () => {
    assert.throws(() => loadLeaseHandoverCliEnv({
        AA_LEASE_ID: "lease_123",
        AA_WORKER_ID: "worker_456",
        AA_NEW_WORKER_ID: "worker_789",
        AA_LEASE_TTL_MS: "invalid",
    }), (err) => err instanceof ValidationError && err.code === "invalid_integer_env:AA_LEASE_TTL_MS");
});
...
test("loadReplayRecoveryCliEnv throws on missing kind", () => {
    assert.throws(() => loadReplayRecoveryCliEnv({}), (err) => err instanceof ValidationError && err.code === "missing_env:AA_RECOVERY_REPLAY_KIND");
});
...
test("loadReplayRecoveryCliEnv throws on invalid kind", () => {
    assert.throws(() => loadReplayRecoveryCliEnv({ AA_RECOVERY_REPLAY_KIND: "invalid" }), (err) => err instanceof ValidationError && err.code === "replay_recovery.invalid_kind");
});
...
//# sourceMappingURL=ops-cli-env.test.js.map