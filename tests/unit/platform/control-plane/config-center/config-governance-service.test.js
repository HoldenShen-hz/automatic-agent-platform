...
import { diffObjects, mergeConfigObjects, sha256, stableStringify, validateConfigField, } from "../../../../../src/platform/control-plane/config-center/config-governance-support.js";
...
test("validateConfigField accepts valid string with minLength", () => {
    const issues = [];
    const schema = { kind: "string", issue: "err", minLength: 1 };
...
test("validateConfigField rejects string below minLength", () => {
    const issues = [];
    const schema = { kind: "string", issue: "err", minLength: 3 };
...
test("validateConfigField accepts valid number with constraints", () => {
    const issues = [];
    const schema = { kind: "number", issue: "err", minExclusive: 0 };
...
test("validateConfigField accepts integer when required", () => {
    const issues = [];
    const schema = { kind: "number", issue: "err", integer: true };
    const result = validateConfigField(5, schema, issues);
    assert.equal(result, true);
});
...
test("validateConfigField accepts valid enum value", () => {
    const issues = [];
    const schema = { kind: "enum", issue: "err", values: ["a", "b", "c"] };
...
test("validateConfigField validates nested object shape", () => {
    ...
    const schema = {
        ...
        issue: "err",
        shape: {
            name: { kind: "string", issue: "err.name", minLength: 1 },
        },
    };
...
// ---------------------------------------------------------------------------
function seedMinimalConfig(root, env = "dev") {
...
function createServiceWithPolicy(configRoot) {
...
test("loadBundle applies environment-specific overlay", () => {
    ...
    try {
        ...
        createFile(join(configRoot, "security/prod.json"), JSON.stringify({ allowDestructiveActions: true }));
        const service = createServiceWithPolicy(configRoot);
...
test("loadBundle detects missing root directory", () => {
    ...
    try {
        ...
        assert.throws(() => service.loadBundle("dev"), /config\.root_missing/);
    }
    finally {
        cleanupPath(workspace);
    }
...
test("loadBundle version changes when config changes", () => {
    ...
    try {
        ...
        createFile(join(configRoot, "runtime/default.json"), JSON.stringify({ maxConcurrentTasks: 99, defaultTaskTimeoutMs: 300000, defaultStepTimeoutMs: 120000 }));
...
test("loadBundle parses JSONC with single-line comments", () => {
    ...
    try {
        seedMinimalConfig(configRoot);
        createFile(join(configRoot, "runtime/default.json"), `{
        // This is a comment
...
test("loadBundle throws on malformed JSON even after comment stripping", () => {
    ...
    try {
        ...
        const service = createServiceWithPolicy(configRoot);
        assert.throws(() => service.loadBundle("dev"), /config\.invalid_json/);
    }
    finally {
        cleanupPath(workspace);
    }
});
//# sourceMappingURL=config-governance-service.test.js.map