...
test("recordCreate generates audit entry with correct structure", () => {
    ...
    const entry = service.recordCreate("platform.runtime.config", "platform", null, content, "user-123", "Initial configuration");
...
test("recordCreate with approvalRequired sets pending status", () => {
    ...
    const entry = service.recordCreate("platform.protected.config", "platform", null, content, "user-123", "Protected config", { approvalRequired: true });
...
test("recordCreate with versionId stores version", () => {
    const service = new ConfigAuditService();
    const entry = service.recordCreate("platform.versioned.config", "platform", null, { value: 1 }, "user", null, { versionId: "v1.0.0" });
...
test("recordUpdate computes diff and stores hashes", () => {
    ...
    const entry = service.recordUpdate("tenant-1.runtime.timeout", "tenant", "tenant-1", before, after, "admin-001", "Increase timeout");
...
test("recordUpdate with approvalRequired sets pending status", () => {
    const service = new ConfigAuditService();
    const entry = service.recordUpdate("platform.protected", "platform", null, { old: 1 }, { new: 2 }, "user", null, { approvalRequired: true });
    assert.equal(entry.approvalStatus, "pending");
});
...
test("recordRollback generates entry with target version metadata", () => {
    ...
    const entry = service.recordRollback("tenant-1.config.feature", "tenant", "tenant-1", before, after, "v1-stable", "admin-001", null);
...
test("recordApproval updates entry with approval details", () => {
    const service = new ConfigAuditService();
    const createEntry = service.recordCreate("platform.protected", "platform", null, { value: 1 }, "user-123", "Test config", { approvalRequired: true });
...
test("query filters by action type", () => {
    ...
    const createResult = service.query({ action: "create" });
    const deleteResult = service.query({ action: "delete" });
...
test("query sorts by timestamp descending", () => {
    ...
    for (let i = 0; i < result.entries.length - 1; i++) {
        assert.ok(result.entries[i].timestamp >= result.entries[i + 1].timestamp, "entries should be sorted descending by timestamp");
    }
});
