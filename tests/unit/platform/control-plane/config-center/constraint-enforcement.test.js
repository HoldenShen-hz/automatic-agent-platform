...
test("createBreakGlassOverride creates correct override structure", () => {
    const override = createBreakGlassOverride("runtime.emergencyTimeout", 1000, "incident-response");
...
test("createTenantOverride creates correct override structure", () => {
    const override = createTenantOverride("workflows.maxSteps", 100, "tenant-456");
...
test("createEnvironmentOverride creates correct override structure", () => {
    const override = createEnvironmentOverride("runtime.concurrencyLimit", 50, "production");
...
test("ConfigOverrideGovernanceService rejects high-risk object not in allowed list for layer", () => {
    ...
    assert.equal(result.allowed, false);
    assert.equal(result.highRiskObject, "provider_profile");
    assert.ok(result.reason?.includes("config_override.path_not_allowed")
...
//# sourceMappingURL=constraint-enforcement.test.js.map