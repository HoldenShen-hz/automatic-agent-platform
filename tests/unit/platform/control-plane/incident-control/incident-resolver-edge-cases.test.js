...
import { IncidentResolver, } from "../../../../../src/platform/control-plane/incident-control/incident-resolver.js";
...
test("IncidentResolver determineStrategy for all category combinations", () => {
    ...
    for (const category of categories) {
        ...
        assert.ok(["self_heal", "automated", "assisted", "manual"].includes(strategy), `Expected valid strategy for category ${category}, got ${strategy}`);
    }
});
...
test("IncidentResolver buildActions for all strategies", () => {
    const resolver = new IncidentResolver();
    const strategies = ["self_heal", "automated", "assisted", "manual"];
...
test("IncidentResolver completeResolution preserves original fields", () => {
    ...
    const completed = resolver.completeResolution(resolution, "Root cause found", "Applied fix", "engineer1");
...
//# sourceMappingURL=incident-resolver-edge-cases.test.js.map