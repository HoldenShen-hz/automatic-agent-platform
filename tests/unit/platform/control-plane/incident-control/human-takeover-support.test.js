...
import { workflowTerminalForTask, sessionTerminalForTask, executionTerminalForTask, normalizeInputJson, normalizeJson, parseOutputs, resolveManualStepOutputSummary, serializeSnapshot, throwTakeoverStorageError, throwTakeoverWorkflowError, } from "../../../../../src/platform/control-plane/incident-control/human-takeover-support.js";
...
test("normalizeJson throws ValidationError for invalid JSON", () => {
    assert.throws(() => normalizeJson("not json", "test.invalid_json"), (e) => e.code === "test.invalid_json");
});
test("normalizeJson throws with provided error code", () => {
    assert.throws(() => normalizeJson("invalid", "takeover.invalid_input"), (e) => e.code === "takeover.invalid_input");
});
test("throwTakeoverStorageError throws StorageError with correct code", () => {
    assert.throws(() => throwTakeoverStorageError("takeover.storage_error", { taskId: "task_123" }), (e) => {
        return e.code === "takeover.storage_error" && e.statusCode === 404;
    });
});
test("throwTakeoverStorageError includes details", () => {
    assert.throws(() => throwTakeoverStorageError("takeover.error", { key: "value" }), (e) => {
        assert.deepEqual(e.details, { key: "value" });
        return true;
    });
});
test("throwTakeoverWorkflowError throws WorkflowStateError with correct code", () => {
    assert.throws(() => throwTakeoverWorkflowError("takeover.workflow_error", { workflowId: "wf_123" }), (e) => {
        return e.code === "takeover.workflow_error";
    });
});
test("throwTakeoverWorkflowError includes details", () => {
    assert.throws(() => throwTakeoverWorkflowError("takeover.error", { stepId: "step_1" }), (e) => {
...
//# sourceMappingURL=human-takeover-support.test.js.map