...
test("sanitizeRegistry throws for invalid registry", () => {
  assert.throws(
    () => sanitizeRegistry(""),
    (err: unknown) => (err as Error)?.message?.startsWith("release.invalid_registry:")
  );
  assert.throws(
    () => sanitizeRegistry("http://docker.io"),
    (err: unknown) => (err as Error)?.message?.startsWith("release.invalid_registry:")
  );
...
test("sanitizeSecretRef includes code in error", () => {
  ...
  assert.ok((caughtError as Error).message.startsWith("my.error.code:"));
});
...
test("buildMarkdown generates correct structure", () => {
  const bundle = {
    ...
    rolloutStrategy: "rolling" as const,
...
test("buildExecutionMarkdown generates correct structure", () => {
  ...
  const report = {
    ...
    registrySecret: {
      ...
      leaseExpiresAt: null,
      revokedAt: null,
      providerKind: "secret_manager" as const,
