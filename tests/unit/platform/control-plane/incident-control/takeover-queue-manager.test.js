...
test("getPendingRequests returns copy of queue", () => {
    ...
    const pending = manager.getPendingRequests();
    pending.push({}); // Mutate returned array
...
test("evictExpiredSessionEntries removes oldest when over capacity", () => {
    const emitter = createMockEventEmitter();
    const manager = new TakeoverQueueManager(createTestConfig({ maxQueueDepth: 200 }), emitter);
...
//# sourceMappingURL=takeover-queue-manager.test.js.map