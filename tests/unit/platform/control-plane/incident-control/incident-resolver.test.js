...
test("IncidentResolver completeResolution updates resolution correctly", () => {
    ...
    const completed = resolver.completeResolution(resolution, "Root cause identified", "Applied automated fix successfully", "automation");
...
test("IncidentResolver handles all severity levels for strategy determination", () => {
    ...
    for (const severity of severities) {
        for (const category of categories) {
            ...
            const strategy = resolver.determineStrategy(incident);
            assert.ok(["self_heal", "automated", "assisted", "manual"].includes(strategy), `Invalid strategy ${strategy} for severity=${severity}, category=${category}`);
        }
    }
});
...
//# sourceMappingURL=incident-resolver.test.js.map