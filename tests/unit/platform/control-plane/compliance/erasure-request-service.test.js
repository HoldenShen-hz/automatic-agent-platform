...
test("ErasureRequestService.failRequest throws on non-existent request", () => {
    ...
    const mockDb = createMockDb(mockStore);
    const service = new ErasureRequestService(mockDb, mockStore);
    assert.throws(() => service.failRequest("nonexistent_erasure", "Some reason"), (err) => err.code.startsWith("erasure.not_found"));
});
...
test("ErasureRequestService.completeRequest throws on non-existent request", () => {
    ...
    const mockDb = createMockDb(mockStore);
    const service = new ErasureRequestService(mockDb, mockStore);
    assert.throws(() => service.completeRequest("nonexistent_erasure", [
        { evidenceType: "dek_destruction", referenceId: "key-001", timestamp: "2026-04-21T00:00:00.000Z" },
    ]), (err) => err.code.startsWith("erasure.not_found"));
});
