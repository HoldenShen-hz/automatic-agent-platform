...
function createMockStore(): { compliance: ComplianceStore; event: { insertEvent: (event: { id: string; eventType: string; payloadJson: string; traceId: string | null; createdAt: string }) => void } } {
  return {
    compliance: {
      ...
      listDataEncryptionKeysByTenant: (tenantId: string): DataEncryptionKey[] => Array.from(dataEncryptionKeys.values()).filter(k => k.tenantId === tenantId),
...
test("DataEncryptionKeyService.createDek throws on invalid tenantId", () => {
  ...
  assert.throws(
    ...
    (err: any) => err.code === "dek.invalid_tenant_id",
  );
});
...
test("DataEncryptionKeyService.createDek throws on invalid key material", () => {
  ...
  assert.throws(
    ...
    (err: any) => err.code === "dek.invalid_key_material",
  );
});
...
test("DataEncryptionKeyService.createDek throws on invalid createdBy", () => {
  ...
  assert.throws(
    ...
    (err: any) => err.code === "dek.invalid_created_by",
  );
});
...
test("DataEncryptionKeyService.rotateDek throws on invalid rotatedBy", () => {
  ...
  assert.throws(
    ...
    (err: any) => err.code === "dek.invalid_rotated_by",
  );
});
...
test("DataEncryptionKeyService.destroyDek throws on non-existent DEK", () => {
  ...
  assert.throws(
    ...
    (err: any) => err.code.startsWith("dek.not_found"),
  );
});
...
test("DataEncryptionKeyService.requireActiveDek throws when no DEK", () => {
  ...
  assert.throws(
    () => service.requireActiveDek("tenant-nonexistent"),
    (err: any) => err.code.startsWith("dek.no_active_dek"),
  );
});
