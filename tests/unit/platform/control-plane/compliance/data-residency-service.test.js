...
function createMockStore() {
    return {
        compliance: {
            ...
            insertDataPlacement: (placement) => { dataPlacements.set(placement.placementId, placement); },
            listDataPlacementsByTenant: (tenantId) => Array.from(dataPlacements.values()).filter(p => p.tenantId === tenantId),
            insertResidencyViolation: (violation) => { residencyViolations.set(violation.violationId, violation); },
            updateResidencyViolation: (violation) => { residencyViolations.set(violation.violationId, violation); },
            listResidencyViolationsByTenant: (tenantId) => Array.from(residencyViolations.values()).filter(v => v.tenantId === tenantId),
            listAllResidencyViolations: () => Array.from(residencyViolations.values()),
        },
        event: {
            insertEvent: () => { },
        },
...
test("DataResidencyService.validateDataPlacement throws for data placement in EU", () => {
    ...
    assert.throws(() => service.validateDataPlacement({
        ...
        category: "business",
    }), (err) => err.code.startsWith("residency.cross_border_not_allowed"));
});
...
test("DataResidencyService.resolveViolation throws on non-existent violation", () => {
    ...
    const mockDb = createMockDb();
    const service = new DataResidencyService(mockDb, mockStore);
    assert.throws(() => service.resolveViolation("nonexistent_violation", "Some notes"), (err) => err.code.startsWith("residency.violation_not_found"));
});
...
//# sourceMappingURL=data-residency-service.test.js.map