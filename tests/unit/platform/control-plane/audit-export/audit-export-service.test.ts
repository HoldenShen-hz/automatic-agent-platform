...
import type { AuditIntegrityRepository } from "../../../../../src/platform/control-plane/iam/audit-integrity-repository.js";
...
function createMockStatement(): MockStatement {
  ...
  return {
    run: () => {},
    get: (..._args: unknown[]) => data[0],
    all: (..._args: unknown[]) => data,
    setData(rows: Record<string, unknown>[]) {
      data = rows;
    },
  };
}
...
function createMockDb(): {
  ...
  const mockPrepare = (sql: string): MockStatement => {
  ...
  const repo: AuditIntegrityRepository = {
    ...
    getIntegrityRecordsInRange() {
      return records.filter((r) => {
        const inRange =
          r.eventCreatedAt >= "2026-04-01T00:00:00.000Z" &&
          r.eventCreatedAt <= "2026-04-30T23:59:59.999Z";
        return inRange;
      });
    },
    setRecords(newRecords: Tier1AuditIntegrityRecord[]) {
      records = newRecords;
    },
  };
...
test("AuditExportService.generateExport updates pending export to completed", () => {
  ...
  }) as unknown as typeof getStmt.setData;
