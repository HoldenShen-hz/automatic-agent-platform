...
test("resolveHarnessCapabilityBaseline throws for unknown capabilityId", () => {
  assert.throws(
    () => resolveHarnessCapabilityBaseline("unknown-capability" as any),
    /harness_capability\.not_found/,
  );
});

test("resolveHarnessCapabilityBaseline throws with correct error message", () => {
  try {
    resolveHarnessCapabilityBaseline("invalid-id" as any);
    assert.fail("expected error to be thrown");
  } catch (err) {
    assert.ok(err instanceof Error);
    assert.equal((err as Error).message, "harness_capability.not_found:invalid-id");
  }
});
