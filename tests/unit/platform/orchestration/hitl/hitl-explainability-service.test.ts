...
test("HITLExplainabilityService generates explanation for task_escalation", () => {
  ...
  const explanation = service.explainTaskEscalation(
    "task-123",
    { complexity: 0.9, duration: 120, expectedDuration: 60, errorRate: 25 },
    { executionId: "exec-1" },
  );
...
test("HITLExplainabilityService generates explanation for approval_required", () => {
  ...
  const explanation = service.explainApprovalRequired(
    "task-456",
    { riskLevel: "high", policy: "sensitive-data-policy", classification: "PII" },
    { contextSnapshot: { userId: "user-1" } },
  );
...
test("HITLExplainabilityService isFeedbackDue checks elapsed time", () => {
  ...
  const oldSession = { id: "session-1", closedAt: new Date(Date.now() - 2000).toISOString() } as any;

  assert.equal(service.isFeedbackDue(oldSession), true);

  const recentSession = { id: "session-2", closedAt: new Date().toISOString() } as any;
