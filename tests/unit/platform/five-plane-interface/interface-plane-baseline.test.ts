...
test("INTERFACE_CAPABILITY_BASELINES contains ingress capability", () => {
  ...
  assert.equal(ing.entryModule, "src/platform/five-plane-interface/ingress/index.ts");
});
...
test("INTERFACE_CAPABILITY_BASELINES contains scheduler capability", () => {
  ...
  assert.equal(sched.entryModule, "src/platform/five-plane-interface/scheduler/index.ts");
});
...
test("resolveInterfaceCapabilityBaseline resolves valid capability", () => {
  ...
  assert.equal(baseline.entryModule, "src/platform/five-plane-interface/api/index.ts");
});
