...
} from "../../../../src/platform/prompt-registry/index.js";
...
test("SemanticVersion type is correctly structured", () => {
  const version: SemanticVersion = { major: 1, minor: 0, patch: 2 };
  assert.equal(version.major, 1);
...
test("VersionLineage type is correctly structured", () => {
  const lineage: VersionLineage = { current: "v1.1", previous: "v1.0", next: "v1.2" };
...
test("HierarchicalPromptRegistryConfig type is correctly structured", () => {
  const config: HierarchicalPromptRegistryConfig = {
