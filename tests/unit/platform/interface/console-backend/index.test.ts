...
function makeTaskBoard(overrides: Partial<ConsoleTaskSummary>[] = []): ConsoleTaskSummary[] {
...
function makeApprovalQueue(overrides: Partial<ConsoleApprovalSummary>[] = []): ConsoleApprovalSummary[] {
...
function makeWorkerPanel(overrides: Partial<ConsoleWorkerSummary>[] = []): ConsoleWorkerSummary[] {
...
function makeIncidentTimeline(overrides: Partial<ConsoleIncidentSummary>[] = []): ConsoleIncidentSummary[] {
...
test("planHumanTakeoverAction throws for empty taskId", () => {
  ...
  assert.throws(() => {
    ...
    return error instanceof Error && "code" in error && error.code === "console.task_id_required";
  });
});
...
test("planHumanTakeoverAction throws for empty reasonCode", () => {
  ...
  assert.throws(() => {
    ...
    return error instanceof Error && "code" in error && error.code === "console.reason_required";
  });
});
...
test("planHumanTakeoverAction throws for operator without operatorId", () => {
  ...
  assert.throws(() => {
    ...
    return error instanceof Error && "code" in error && error.code === "console.operator_id_required";
  });
});
...
test("incidentTimeline is sorted by createdAt desc and limited to 50", () => {
  const incidents: ConsoleIncidentSummary[] = [];
  for (let i = 0; i < 60; i++) {
    incidents.push({
      incidentId: `incident_${i}`,
      taskId: null,
