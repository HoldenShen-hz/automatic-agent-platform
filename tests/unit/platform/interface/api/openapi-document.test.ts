...
test("buildOpenApiDocument includes auth routes", () => {
  ...
  const route = doc.paths["/v1/auth/token"] as Record<string, unknown>;
...
test("buildOpenApiDocument route has correct method structure", () => {
  ...
  const healthz = doc.paths["/healthz"] as Record<string, unknown>;
  const get = healthz.get as Record<string, unknown>;
...
test("buildOpenApiDocument paths have both GET and POST where expected", () => {
  ...
  const tasks = doc.paths["/v1/tasks"] as Record<string, unknown>;
  assert.ok(tasks.get);
  
  const authToken = doc.paths["/v1/auth/token"] as Record<string, unknown>;
