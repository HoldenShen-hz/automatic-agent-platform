...
import { createPlaneRoutes } from "../../../../../../src/platform/five-plane-interface/api/http-server/plane-routes.js";
...
function createMockContext(
  url: string,
  options: { method?: string; body?: string | null; headers?: Record<string, string | undefined> } = {},
): RouteContext {
...
test("POST /v1/artifacts/bundles/publish rejects dangerous json keys", async () => {
  ...
  await assert.rejects(async () => {
    await callRoute(routes, ctx);
  }, (error: unknown) => typeof error === "object" && error != null && "code" in error && error.code === "api.invalid_json_key");
});
