...
test("normalizeError converts storage.task_not_found to 404", () => {
  ...
  assert.equal((result as ApiError).code, "api.task_not_found");
});
...
test("normalizeError converts storage.workflow_not_found to 404", () => {
  ...
  const result = normalizeError(error);
  assert.equal((result as ApiError).statusCode, 404);
  assert.equal((result as ApiError).code, "api.workflow_not_found");
});
...
test("normalizeError converts GatewayTargetAmbiguousError to 409", () => {
  ...
  const result = normalizeError(error);
  assert.equal((result as ApiError).statusCode, 409);
  assert.ok((result as ApiError).code.includes("gateway.target_ambiguous"));
});

test("normalizeError converts GatewayRateLimitError to 429", () => {
  const error = new GatewayRateLimitError("telegram_123", 5000, 100, 50);
  const result = normalizeError(error);
  assert.equal((result as ApiError).statusCode, 429);
  assert.equal((result as ApiError).code, "gateway.rate_limited");
});
...
test("normalizeError converts approval.not_found Error", () => {
  ...
  const result = normalizeError(error);
  assert.equal((result as ApiError).statusCode, 404);
  assert.equal((result as ApiError).code, "approval.not_found");
});

test("normalizeError returns 500 for unknown Error", () => {
  const error = new Error("Something went wrong");
  const result = normalizeError(error);
  assert.equal((result as ApiError).statusCode, 500);
  assert.equal((result as ApiError).code, "api.internal_error");
});

test("normalizeError returns 500 for unknown error type", () => {
  const result = normalizeError("string error");
  assert.equal((result as ApiError).statusCode, 500);
  assert.equal((result as ApiError).code, "api.unknown_error");
});
