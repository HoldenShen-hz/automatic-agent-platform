...
function makeResourceCost(overrides: Partial<{
  ...
  costUsd: number;
  currency: string;
}> = {}): { resourceId: string; resourceType: "compute" | "storage" | "network" | "api"; costUsd: number; currency: string } {
  return {
    resourceId: "res_001",
...
function makeCreateInput(overrides: Partial<CreateCostReportInput> = {}): CreateCostReportInput {
  return {
...
test("listBudgetSummaries aggregates by tenant and currency", () => {
  ...
  const usdTenantA = summaries.find((s: BudgetSummaryRecord) => s.budgetKey === "tenant_a:USD");
  assert.ok(usdTenantA);
  assert.equal(usdTenantA!.totalCostUsd, 80);
  assert.equal(usdTenantA!.reportCount, 2);

  const usdTenantB = summaries.find((s: BudgetSummaryRecord) => s.budgetKey === "tenant_b:USD");
  assert.ok(usdTenantB);
  assert.equal(usdTenantB!.totalCostUsd, 20);

  const eurTenantA = summaries.find((s: BudgetSummaryRecord) => s.budgetKey === "tenant_a:EUR");
...
test("listBudgetSummaries groups null tenantId as platform", () => {
  ...
  const platformSummary = summaries.find((s: BudgetSummaryRecord) => s.budgetKey === "platform:USD");
...
test("createReport stores multiple reports", () => {
  ...
  const reports = service.listReports();
  assert.equal(reports.length, 3);
  const totalCost = reports.reduce((sum: number, r: CostReportRecord) => sum + r.totalCostUsd, 0);
