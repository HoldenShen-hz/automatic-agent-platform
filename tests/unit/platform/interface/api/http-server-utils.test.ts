...
test("readJsonBody throws for invalid JSON", () => {
  assert.throws(
    () => readJsonBody("not valid json"),
    (e: any) => e.code === "api.invalid_json"
  );
});
...
test("validateTaskId throws for undefined", () => {
  assert.throws(
    ...
    (e: any) => e.code === "api.task_not_found" && e.message.includes("test")
  );
});
...
test("validateTaskId throws for empty string", () => {
  assert.throws(
    () => validateTaskId("", "test"),
    (e: any) => e.code === "api.task_not_found"
  );
});
...
test("validateTaskId throws when exceeds max length", () => {
  ...
  assert.throws(
    ...
    (e: any) => e.code === "api.invalid_task_id" && e.message.includes("128")
  );
});
...
test("validateTaskId rejects taskId of 129 chars", () => {
  ...
  assert.throws(
    () => validateTaskId(overByOne, "test"),
    (e: any) => e.code === "api.invalid_task_id"
  );
});

test("validateTaskId throws for invalid characters", () => {
  assert.throws(
    () => validateTaskId("task with space", "test"),
    (e: any) => e.code === "api.invalid_task_id"
  );
  assert.throws(
    () => validateTaskId("task@special", "test"),
    (e: any) => e.code === "api.invalid_task_id"
  );
});
...
test("validateTaskId throws for non-string", () => {
  assert.throws(
    () => validateTaskId(123 as any, "test"),
    (e: any) => e.code === "api.task_not_found"
  );
});
