...
test("GatewayRateLimitError details contain all fields", () => {
  ...
  assert.equal((error as any).internalDetails?.channel, "slack_456");
  assert.equal((error as any).internalDetails?.retryAfterMs, 3000);
  assert.equal((error as any).internalDetails?.limit, 200);
  assert.equal((error as any).internalDetails?.currentCount, 150);
});
...
test("normalizeGatewayDeliveryFailure handles GatewayRateLimitError", () => {
  const mockDeliveryService = {
    isRetryableStatus: (_status: number) => true,
  } as any;
...
test("normalizeGatewayDeliveryFailure uses deliveryService for retryable check", () => {
  const mockDeliveryService = {
    isRetryableStatus: (status: number) => status !== 404, // 404 is not retryable
  } as any;
