...
import { ChannelGatewayRetryExecutor, type ChannelGatewayRetryExecutorOptions } from "../../../../../src/platform/interface/channel-gateway/channel-gateway-retry-executor.js";
import type { ChannelGatewayService } from "../../../../../src/platform/interface/channel-gateway/channel-gateway-service.js";
import type { GatewayRetryQueueSummary } from "../../../../../src/platform/interface/channel-gateway/channel-gateway-service.js";

class MockChannelGatewayService implements Partial<ChannelGatewayService> {
  public processRetryQueueCalls: Array<{ batchSize: number }> = [];
  public processRetryQueueResult: GatewayRetryQueueSummary = {
    scanned: 0,
    delivered: 0,
    retryScheduled: 0,
    deadLettered: 0,
  ...
  processRetryQueue(batchSize: number): Promise<GatewayRetryQueueSummary> {
...
test("ChannelGatewayRetryExecutor accepts options with defaults", () => {
  ...
  const executor = new ChannelGatewayRetryExecutor(mock as ChannelGatewayService);
...
test("ChannelGatewayRetryExecutor accepts custom pollIntervalMs and batchSize", () => {
  ...
  const executor = new ChannelGatewayRetryExecutor(mock as ChannelGatewayService, options);
...
test("ChannelGatewayRetryExecutor.runOnce calls gatewayService.processRetryQueue with batchSize", async () => {
  ...
  const executor = new ChannelGatewayRetryExecutor(mock as ChannelGatewayService, { batchSize: 25 });
...
test("ChannelGatewayRetryExecutor.runOnce returns empty summary on error", async () => {
  const mock = new MockChannelGatewayService();
  (mock as unknown as { processRetryQueue: (batchSize: number) => Promise<GatewayRetryQueueSummary> }).processRetryQueue = () => {
...
test("ChannelGatewayRetryExecutor processes multiple batches via runOnce", async () => {
  ...
  const executor = new ChannelGatewayRetryExecutor(mock as ChannelGatewayService, { batchSize: 10 });
