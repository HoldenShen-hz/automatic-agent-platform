...
import type { ChannelGatewayService, GatewayRetryQueueSummary } from "../../../../../src/platform/interface/channel-gateway/channel-gateway-service.js";

test("channel gateway retry executor runs a retry pass through the gateway service", async () => {
  const passes: number[] = [];
  const gatewayService = {
    async processRetryQueue(limit?: number): Promise<GatewayRetryQueueSummary> {
      passes.push(limit ?? 0);
      return {
        scanned: 2,
        delivered: 1,
...
test("channel gateway retry executor returns busy result when already running", async () => {
  const gatewayService = {
    async processRetryQueue(): Promise<GatewayRetryQueueSummary> {
