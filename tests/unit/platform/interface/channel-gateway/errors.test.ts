...
test("GatewayRateLimitError extends PolicyDeniedError", () => {
  const err = new GatewayRateLimitError("webhook", 1000, 10, 5);
  assert.ok(err instanceof PolicyDeniedError);
});

test("GatewayDeliveryError creates error with correct properties", () => {
...
test("GatewayDeliveryError is retryable for 5xx errors", () => {
  const err = new GatewayDeliveryError("Internal error", 502, true);
  assert.ok(err.retryable === true);
});

test("GatewayDeliveryError is non-retryable for 4xx errors", () => {
  const err = new GatewayDeliveryError("Bad request", 400, false);
  assert.ok(err.retryable === false);
});

test("GatewayDeliveryError defaults to 502 status code", () => {
...
test("normalizeGatewayDeliveryFailure extracts status from error message ending with :NNN", () => {
...
test("normalizeGatewayDeliveryFailure handles plain Error with no status code", () => {
