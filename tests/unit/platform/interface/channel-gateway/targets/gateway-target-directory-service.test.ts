...
import {
  ...
  type RegisterGatewayTargetInput,
  type GatewayTargetDirectoryEntry,
...
function createMockStoragePort(overrides?: Partial<GatewayStoragePort>): GatewayStoragePort {
  ...
  return {
    ...
    listGatewayTargets(limit = 200, channel?: string): GatewayTargetRecord[] {
    ...
    listGatewaySessionTargetCandidates(limit = 200, channel?: string): GatewaySessionTargetCandidate[] {
...
function createTargetRecord(overrides?: Partial<GatewayTargetRecord>): GatewayTargetRecord {
...
function addTargetToStorage(storage: GatewayStoragePort, record: GatewayTargetRecord): void {
  storage.upsertGatewayTarget(record);
}

test("RegisterGatewayTargetInput structure", () => {
  const input: RegisterGatewayTargetInput = {
    channel: "telegram",
    targetKind: "user",
...
test("GatewayTargetDirectoryEntry structure", () => {
  const entry: GatewayTargetDirectoryEntry = {
    targetId: "telegram:user:123",
    channel: "telegram",
    targetKind: "user",
    source: "directory",
...
test("ListGatewayTargetsQuery structure", () => {
  const query: ListGatewayTargetsQuery = {
    channel: "telegram",
    query: "john",
    limit: 100,
  };
...
test("ResolveGatewayTargetQuery structure", () => {
  const query: ResolveGatewayTargetQuery = {
    query: "john",
    channel: "telegram",
  };
...
test("GatewayTargetResolution structure", () => {
  ...
  const resolution: GatewayTargetResolution = {
    entry,
...
test("GatewayTargetResolution matchedBy types", () => {
  ...
  const matchTypes: GatewayTargetResolution["matchedBy"][] = [
    "target_id_exact",
    "display_name_exact",
    "alias_exact",
    "target_id_prefix",
  ...
  for (const matchedBy of matchTypes) {
    const resolution: GatewayTargetResolution = { entry, matchedBy };
...
test("GatewayTargetAmbiguousError structure", () => {
  const entries: GatewayTargetDirectoryEntry[] = [
    {
...
test("GatewayTargetDirectoryService.registerTarget rejects empty externalTargetId", () => {
  ...
  assert.throws(
    ...
    (err: any) => err.code === "gateway.invalid_external_target_id",
  );
});
...
test("GatewayTargetDirectoryService.registerTarget rejects empty displayName", () => {
  ...
  assert.throws(
    () => service.registerTarget(input),
    (err: any) => err.code === "gateway.invalid_display_name",
  );
});
...
test("GatewayTargetDirectoryService.registerTarget rejects empty channel", () => {
  ...
  assert.throws(
    () => service.registerTarget(input),
    (err: any) => err.code === "gateway.invalid_channel",
  );
});
...
test("GatewayTargetDirectoryService.resolveTarget throws Ambiguous for multiple matches", () => {
  ...
  assert.throws(
    () => service.resolveTarget({ query: "john" }),
    GatewayTargetAmbiguousError,
  );
});
...
test("GatewayTargetDirectoryService.resolveTarget rejects empty query", () => {
  ...
  assert.throws(
    ...
    (err: any) => err.code === "gateway.target_query_required",
  );
});
