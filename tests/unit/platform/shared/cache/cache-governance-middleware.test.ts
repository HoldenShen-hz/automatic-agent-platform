...
const createMockCache = () => ({
  ...
  tags: [] as string[],
  computeFn: null as (() => Promise<unknown>) | null,
  getOrComputeResult: { fromCache: false, value: "computed" },
  getOrCompute: async function<T>(
    namespace: string,
    ...
    return this.getOrComputeResult as { fromCache: boolean; value: T };
  },
  invalidateByTag: async (tag: string): Promise<number> => tag.includes("valid") ? 1 : 0,
...
test("createCacheGovernanceMiddleware returns middleware with correct name and priority", () => {
  ...
  const middleware = createCacheGovernanceMiddleware({ cache: mockCache as any });
...
test("createCacheGovernanceMiddleware skips non-cacheable tools", async () => {
  ...
  const result = await middleware.run(
    ...
    async () => { nextCalled = true; return "bash-result" as unknown; }
  );
...
test("createCacheGovernanceMiddleware uses correct namespace for cacheable tool", async () => {
  ...
  await middleware.run(
    createMockContext(),
    { toolName: "read", args: { path: "/workspace/file.ts" } },
    async () => "read-result"
  );
...
test("createCacheGovernanceMiddleware normalizes path with workspaceRoot", async () => {
  ...
  await middleware.run(
    createMockContext(),
    { toolName: "read", args: { path: "relative/path.ts" } },
    async () => "result"
  );
...
test("createCacheGovernanceMiddleware returns cached value on cache hit", async () => {
  ...
  const middleware = createCacheGovernanceMiddleware({ cache: mockCache as any, logger: logger as any });
...
test("createCacheSummaryMiddleware returns success", async () => {
  const mockCache = createMockCache();
  const logger = createMockLogger();
  const summaryMiddleware = createCacheSummaryMiddleware({ cache: mockCache as any, logger: logger as any });
...
test("createCacheGovernanceMiddleware does not call next when cache hit", async () => {
  ...
  await middleware.run(
    createMockContext(),
    { toolName: "read", args: {} },
    async () => { nextCalled = true; return "fresh"; }
  );
