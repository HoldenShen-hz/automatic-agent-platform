...
function createMockDb(queryResults: MockQueryResult[] = [], executeResults: { changes: number }[] = []): {
  execute: (sql: string, params?: unknown[]) => Promise<{ changes?: number }>;
  query: <T>(sql: string, params?: unknown[]) => Promise<T[]>;
  callLog: { method: string; sql: string; params: unknown[] }[];
} {
  const callLog: { method: string; sql: string; params: unknown[] }[] = [];
  let queryIndex = 0;
  let execIndex = 0;
