...
import { readRedisConnectionConfigFromEnv, buildRedisClientOptions, } from "../../../../../src/platform/shared/utils/redis-client-options.js";
...
test("buildRedisClientOptions throws ValidationError for sentinel without endpoints", () => {
    ...
    assert.throws(() => buildRedisClientOptions(config), /redis.sentinel_endpoints_required/);
});
test("buildRedisClientOptions throws ValidationError for sentinel without name", () => {
    ...
    assert.throws(() => buildRedisClientOptions(config), /redis.sentinel_name_required/);
});
...
test("readRedisConnectionConfigFromEnv parseSentinelEndpoints throws on invalid endpoint format", () => {
    assert.throws(() => readRedisConnectionConfigFromEnv("R", { R_SENTINELS: "invalid-endpoint" }), /redis.sentinel_endpoint_invalid/);
});
test("readRedisConnectionConfigFromEnv parseSentinelEndpoints throws on missing port", () => {
    assert.throws(() => readRedisConnectionConfigFromEnv("R", { R_SENTINELS: "host-without-port" }), /redis.sentinel_endpoint_invalid/);
});
