...
class MockRuntimeMetricsRegistry {
  ...
  public gauges = new Map<string, number>();
  public histograms = new Map<string, { count: number; sum: number }>();

  public incrementCounter(name: string, _labels: Record<string, string>, delta: number = 1): void {
    const key = `counter:${name}`;
    this.counters.set(key, (this.counters.get(key) ?? 0) + delta);
  }

  public setGauge(name: string, _labels: Record<string, string>, value: number): void {
    this.gauges.set(`gauge:${name}`, value);
  }

  public observeHistogram(name: string, _labels: Record<string, string>, value: number): void {
...
test("MetricsCollector normalizes numeric labels to strings", () => {
  mockRegistry.reset();
  mockRegistry.incrementCounter("numeric_test", { port: 8080 as unknown as string, active: 1 as unknown as string }, 1);
