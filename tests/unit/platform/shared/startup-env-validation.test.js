...
import { StartupEnvSchema, validateStartupEnv, } from "../../../../src/platform/control-plane/config-center/startup-env-schema.js";
/**
...
test("[SYS-SEC-4.1] startup env schema validates required AA_DB_PATH rejects empty string", () => {
    ...
    try {
        ...
        const hasDbPathError = result.errors.some((e) => e.key === "AA_DB_PATH" && e.message.includes("non-empty"));
...
test("[SYS-SEC-4.1] startup env schema validates all critical AA_ vars are present", () => {
    ...
    const schemaKeys = Object.keys(StartupEnvSchema._def.schema.shape);
    const criticalVars = [
        "AA_STORAGE_DRIVER",
        "AA_API_HOST",
        "AA_API_PORT",
    ...
    assert.deepStrictEqual(missing, [], `Critical AA_* vars must be in startup env schema. Missing: ${missing.join(", ")}`);
});
test("[SYS-SEC-4.1] validateStartupEnv returns structured result with errors array", () => {
    ...
    try {
        ...
        const portError = result.errors.find((e) => e.key === "AA_API_PORT");
...
test("[SYS-SEC-4.1] validateStartupEnv succeeds with valid env", () => {
    ...
    // ZodEffects wraps the ZodObject; access inner shape via _def.schema.shape
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const aaVars = Object.keys(StartupEnvSchema._def.schema.shape);
    try {
        for (const key of aaVars) {
...
//# sourceMappingURL=startup-env-validation.test.js.map
