...
test("[SYS-PERF-3.4] anomaly detection metricBuffer has size limit", () => {
  ...
  const bufferSize = (service as unknown as { history: { size: number } }).history.size;
...
test("[SYS-PERF-3.4] metricBuffer eviction is FIFO (oldest entries removed)", () => {
  ...
  assert.equal(
    firstMetricHistory.length,
    0,
    "Oldest metric should be evicted when buffer is full",
  );

  // Recent metrics should still have data
  const recentMetricHistory = service.getHistory("other-metric-199");
  assert.ok(
    recentMetricHistory.length > 0,
    "Recent metrics should be retained after eviction",
  );
});
...
test("[SYS-PERF-3.4] each metric series has bounded point count", () => {
  ...
  assert.ok(
    history.length <= maxExpectedPoints,
    `History length ${history.length} exceeds max expected ${maxExpectedPoints}`,
  );
});
