...
test("runStableDbWritabilityRehearsal executes all three scenarios successfully [stable-db-writability-rehearsal]", async () => {
...
test("health_and_doctor_fail_close_when_db_is_not_writable scenario passes [stable-db-writability-rehearsal]", async () => {
  const outputDir = createTempDir();
  try {
    const report = await runStableDbWritabilityRehearsal({ outputDir });
...
test("multi_step_admission_rejects_new_work_in_read_only_mode scenario passes [stable-db-writability-rehearsal]", async () => {
  const outputDir = createTempDir();
  try {
    const report = await runStableDbWritabilityRehearsal({ outputDir });
    const scenario = report.scenarios.find(
...
test("dispatch_blocks_claims_without_dropping_pending_ticket_in_read_only_mode scenario passes [stable-db-writability-rehearsal]", async () => {
  const outputDir = createTempDir();
  try {
    const report = await runStableDbWritabilityRehearsal({ outputDir });
    const scenario = report.scenarios.find(
...
test("writeStableDbWritabilityRehearsalReport writes valid JSON [stable-db-writability-rehearsal]", async () => {
  const outputDir = createTempDir();
...
test("report contains valid startedAt and finishedAt timestamps [stable-db-writability-rehearsal]", async () => {
  const outputDir = createTempDir();
  try {
    const report = await runStableDbWritabilityRehearsal({ outputDir });
...
test("report outputDir matches options [stable-db-writability-rehearsal]", async () => {
  const outputDir = createTempDir();
  try {
    const report = await runStableDbWritabilityRehearsal({ outputDir });
...
test("each scenario has durationMs greater than zero [stable-db-writability-rehearsal]", async () => {
  const outputDir = createTempDir();
  try {
    const report = await runStableDbWritabilityRehearsal({ outputDir });
