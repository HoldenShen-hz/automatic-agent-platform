...
import { buildGoldenTaskInventoryBaseline, REQUIRED_GOLDEN_TASK_CLASSES, SINGLE_TASK_GOLDEN_TASKS, } from "../../../../../src/platform/shared/stability/golden-task-runner.js";
...
test("SINGLE_TASK_GOLDEN_TASKS has exactly one case per required class", () => {
    ...
    for (const cls of REQUIRED_GOLDEN_TASK_CLASSES) {
        assert.equal(classCount.get(cls) ?? 0, 1, `Expected exactly one task for class ${cls}`);
    }
});
...
test("GoldenTaskCase crash_recovery has requeue_supported", () => {
    const crashCases = SINGLE_TASK_GOLDEN_TASKS.filter((t) => t.metadata.expectedClass === "crash_recovery");
...
test("GoldenTaskCase cross_division has manual_takeover_supported", () => {
    const crossCases = SINGLE_TASK_GOLDEN_TASKS.filter((t) => t.metadata.expectedClass === "cross_division");
...
//# sourceMappingURL=golden-task-runner-coverage.test.js.map