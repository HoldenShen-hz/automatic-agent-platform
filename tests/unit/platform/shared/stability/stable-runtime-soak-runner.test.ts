...
function makeMockValidationReport(overrides: Partial<{
  totalRuns: number;
  failedRuns: number;
  integrityFailures: number;
  backupFailures: number;
}> = {}): import("../../../../../src/platform/shared/stability/stable-runtime-validator.js").StableValidationReport {
  return {
...
function makeMockSoakReport(overrides: Partial<StableSoakReport> & { cycles?: StableSoakCycle[] } = {}): StableSoakReport {
  const defaultReport: StableSoakReport = {
    startedAt: "2026-04-09T00:00:00.000Z",
...
test("StableSoakReport memory growth ratio below 2x threshold indicates healthy run [stable-runtime-soak-runner]", () => {
  ...
  assert.ok(
    report.memoryGrowthRatio! < 2.0,
    `Memory growth ratio ${report.memoryGrowthRatio} should be below 2.0 threshold`,
  );
});
