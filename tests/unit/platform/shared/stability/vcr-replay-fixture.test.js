...
import { VcrFixtureStore, buildRequestFingerprint, } from "../../../../../src/platform/shared/stability/vcr-replay-fixture.js";
...
test("VcrFixtureStore replay throws when fixture missing in vcr_replay mode", () => {
    ...
    assert.throws(() => store.replay(request), (err) => err instanceof ValidationError && err.code === "vcr.fixture_missing");
});
...
test("VcrFixtureStore loadFixture throws on invalid input", () => {
    assert.throws(() => VcrFixtureStore.loadFixture(null), (err) => err instanceof ValidationError && err.code === "vcr.fixture_schema_invalid");
    assert.throws(() => VcrFixtureStore.loadFixture(undefined), (err) => err instanceof ValidationError && err.code === "vcr.fixture_schema_invalid");
    assert.throws(() => VcrFixtureStore.loadFixture({ notInteractions: [] }), (err) => err instanceof ValidationError && err.code === "vcr.fixture_schema_invalid");
});
test("VcrFixtureStore loadFixture validates each interaction", () => {
    ...
    assert.throws(() => VcrFixtureStore.loadFixture(invalidInteractions), (err) => err instanceof ValidationError && err.code === "vcr.fixture_schema_invalid");
});
...
//# sourceMappingURL=vcr-replay-fixture.test.js.map