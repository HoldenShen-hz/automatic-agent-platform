...
import { runStableDbQueueDisconnectRehearsal, writeStableDbQueueDisconnectRehearsalReport, } from "../../../../../src/platform/shared/stability/stable-db-queue-disconnect-rehearsal.js";
...
test("missing_dispatch_ticket_rebuilt_after_queue_reconnect scenario passes", async () => {
    ...
    try {
        const report = await runStableDbQueueDisconnectRehearsal({ outputDir });
        const scenario = report.scenarios.find((s) => s.scenarioId === "missing_dispatch_ticket_rebuilt_after_queue_reconnect");
        if (!scenario) {
...
test("authoritative_writeback_failure_fails_closed_until_store_recovers scenario passes", async () => {
    ...
    try {
        const report = await runStableDbQueueDisconnectRehearsal({ outputDir });
        const scenario = report.scenarios.find((s) => s.scenarioId === "authoritative_writeback_failure_fails_closed_until_store_recovers");
        if (!scenario) {
            throw new Error("Missing authoritative_writeback_failure_fails_closed_until_store_recovers scenario");
        }
        if (!scenario.passed) {
            throw new Error(`Scenario failed: ${scenario.summary}`);
        }
...
//# sourceMappingURL=stable-db-queue-disconnect-rehearsal.test.js.map