...
function seedEmptyEvidenceProfile(evidenceRoot, profile) {
    const profileDir = join(evidenceRoot, profile);
    mkdirSync(profileDir, { recursive: true });
    // Create an empty report to simulate a passing smoke test
    writeFileSync(join(profileDir, "stable-evidence-report.json"), JSON.stringify({
...
function seedEmptyEvidenceProfileMissingPlaybooks(evidenceRoot, profile) {
    const profileDir = join(evidenceRoot, profile);
    mkdirSync(profileDir, { recursive: true });
...
test("StableReleasePackageReport verdict is conditional when profiles missing", () => {
    ...
    try {
        ...
        assert.ok(report.overallVerdict === "conditional" || report.overallVerdict === "promote_blocked", `Expected conditional or blocked, got ${report.overallVerdict}`);
...
test("StableReleasePackageReport nextActions includes smoke action when missing", () => {
    ...
    try {
        ...
        const hasSmokeAction = report.nextActions.some((action) => action.includes("smoke evidence") || action.includes("smoke"));
...
test("StableReleasePackageReport tenant_gray target includes tenant_gray criteria", () => {
    ...
    try {
        ...
        const tenantGrayCriterion = report.gate.criteria.find((c) => c.criterionId === "tenant_gray_rollout_tested");
...
test("StableReleasePackageReport production_ready target requires additional criteria", () => {
    ...
    try {
        ...
        const dbQueueCriterion = report.gate.criteria.find((c) => c.criterionId === "db_queue_disconnect_tested");
        assert.ok(dbQueueCriterion, "Expected db_queue_disconnect_tested criterion for production_ready");
        const acceptanceLineCriterion = report.gate.criteria.find((c) => c.criterionId === "stable_acceptance_line");
...
//# sourceMappingURL=stable-release-package.test.js.map