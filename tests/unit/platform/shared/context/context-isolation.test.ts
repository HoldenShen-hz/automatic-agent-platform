...
function createTestSnapshot(overrides: Partial<RuntimeContextSnapshot> = {}): RuntimeContextSnapshot {
  return {
    traceId: "trace-123",
    taskId: "task-456",
...
test("assertContext reports all missing fields in error details", () => {
  ...
  const details = error.details as Record<string, unknown> | null;
  assert.ok(details != null);
  assert.ok((details.missingFields as string[]).includes("taskId"));
  assert.ok((details.missingFields as string[]).includes("traceId"));
});
...
test("withContextPatch with multiple field patches", () => {
  ...
  provideContext(original, () => {
    const result = withContextPatch(
      { taskId: "patched-task", tenantId: "patched-tenant" },
      () => {
        const ctx = getContext();
        return {
          taskId: ctx.taskId,
...
test("context is preserved through async/await boundaries", async () => {
  ...
  async function innerAsync(): Promise<string> {
    return getContext().taskId;
  }
...
test("context isolation in nested concurrent operations", async () => {
  ...
  await provideContext(outer, async () => {
    const [r1, r2] = await Promise.all([
      provideContext(createTestSnapshot({ taskId: "inner-1" }), () =>
        Promise.resolve(getContext().taskId),
      ),
      provideContext(createTestSnapshot({ taskId: "inner-2" }), () =>
        Promise.resolve(getContext().taskId),
      ),
    ]);
...
test("getContextOrNull returns context with null fields when context exists", () => {
  const snapshot = createTestSnapshot({ tenantId: null as unknown as string });
  provideContext(snapshot, () => {
...
test("hasTenantContext returns false for undefined tenantId", () => {
  const snapshot = createTestSnapshot({ tenantId: undefined as unknown as string });
  provideContext(snapshot, () => {
...
test("hasWorkspaceContext returns false for undefined workspaceId", () => {
  const snapshot = createTestSnapshot({ workspaceId: undefined as unknown as string });
  provideContext(snapshot, () => {
...
test("provideContext returns promise rejected when inner function rejects", async () => {
  ...
  try {
    ...
    threw = true;
    assert.ok(err instanceof Error);
    assert.equal((err as Error).message, "inner rejection");
  }
  assert.equal(threw, true);
});

test("getWorkspaceId returns correct value when tenantId is set but workspaceId is null", () => {
  const snapshot = createTestSnapshot({ tenantId: "test-tenant", workspaceId: null as unknown as string });
  provideContext(snapshot, () => {
