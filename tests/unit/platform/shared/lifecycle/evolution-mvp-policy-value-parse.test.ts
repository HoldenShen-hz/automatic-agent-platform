...
test("parsePolicyValue parses budget adjustment policy", () => {
  ...
  const value = parsePolicyValue<{
    recommendedPolicy: { maxTaskCostUsd: number };
    baselinePolicy: { maxTaskCostUsd: number };
    appliedBy: string;
  }>(record);
...
test("parsePolicyValue handles complex nested structure", () => {
  ...
  const value = parsePolicyValue<{
    recommendedPolicy: { maxTaskCostUsd: number; features: { enableBulkOperations: boolean } };
    baselinePolicy: { maxTaskCostUsd: number; features: { enableBulkOperations: boolean } };
  }>(record);
...
test("parsePolicyValue parses simple string value", () => {
  ...
  const value = parsePolicyValue<string>(record);
