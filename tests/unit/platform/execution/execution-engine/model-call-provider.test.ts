...
test("ModelCallProviderService enforces maxSteps budget constraint before execution", async () => {
  const provider = new ModelCallProviderService({
    budgetPolicy: {
      maxSteps: 0,
      maxModelTokens: 1000,
      maxDurationMs: 60000,
  ...
  await assert.rejects(
    async () => provider.createCompletion({
      model: "test-model",
    ...
    (error: unknown) => {
      return Boolean(
        ...
        && typeof error === "object"
        && "code" in error
        && (error as { code?: string }).code === "budget.maxSteps_exceeded",
      );
    },
  );
});

test("ModelCallProviderService enforces maxModelTokens budget constraint before execution", async () => {
  const provider = new ModelCallProviderService({
    budgetPolicy: {
      maxSteps: 10,
      maxModelTokens: 5,
  ...
  await assert.rejects(
    async () => provider.createCompletion({
      model: "test-model",
      messages: [{ role: "user", content: "token heavy request" }],
      maxTokens: 10,
    }),
    (error: unknown) => {
      return Boolean(
        ...
        && typeof error === "object"
        && "code" in error
        && (error as { code?: string }).code === "budget.maxModelTokens_exceeded",
      );
    },
  );
});

test("ModelCallProviderService enforces maxDurationMs budget constraint during execution", async () => {
  const provider = new ModelCallProviderService({
    budgetPolicy: {
      maxSteps: 10,
      maxModelTokens: 1000,
  ...
  }).executeGovernedCompletion = async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => {
      return Boolean(
        ...
        && typeof error === "object"
        && "code" in error
        && (error as { code?: string }).code === "budget.maxDurationMs_exceeded",
      );
    },
  );
});
