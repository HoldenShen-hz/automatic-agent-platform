...
test("WorkerRegistryService recordHeartbeat creates snapshot for new worker [worker-registry-service]", () => {
...
test("WorkerRegistryService recordHeartbeat updates existing snapshot [worker-registry-service]", () => {
...
test("WorkerRegistryService recordHeartbeat preserves existing telemetry when not provided [worker-registry-service]", () => {
...
test("WorkerRegistryService recordHeartbeat merges running execution IDs [worker-registry-service]", () => {
...
test("WorkerRegistryService recordHeartbeat normalizes capabilities [worker-registry-service]", () => {
  const snapshots = new Map<string, WorkerSnapshotRecord>();
  const store = createMockStore(snapshots);
  const service = new WorkerRegistryService(store);
...
test("WorkerRegistryService recordHeartbeat handles remote placement [worker-registry-service]", () => {
  const snapshots = new Map<string, WorkerSnapshotRecord>();
  const store = createMockStore(snapshots);
  const service = new WorkerRegistryService(store);
...
test("WorkerRegistryService recordHeartbeat computes availableSlots [worker-registry-service]", () => {
...
test("WorkerRegistryService recordHeartbeat increments restartGeneration on runtime change [worker-registry-service]", () => {
  const existing = createMockWorkerSnapshot({
    workerId: "worker-1",
...
test("WorkerRegistryService recordHeartbeat updates lastProgressAt when progressMessage provided [worker-registry-service]", () => {
...
test("WorkerRegistryService recordHeartbeat normalizes rate values [worker-registry-service]", () => {
  const snapshots = new Map<string, WorkerSnapshotRecord>();
  const store = createMockStore(snapshots);
  const service = new WorkerRegistryService(store);
...
test("WorkerRegistryService recordHeartbeat normalizes activeLeaseCount [worker-registry-service]", () => {
  const snapshots = new Map<string, WorkerSnapshotRecord>();
  const store = createMockStore(snapshots);
  const service = new WorkerRegistryService(store);
...
test("WorkerRegistryService verifyRemoteWorkerRegistration creates remote worker [worker-registry-service]", () => {
  const snapshots = new Map<string, WorkerSnapshotRecord>();
  const store = createMockStore(snapshots);
  const service = new WorkerRegistryService(store);
...
test("WorkerRegistryService verifyRemoteWorkerRegistration sets registrationVerifiedAt [worker-registry-service]", () => {
  const snapshots = new Map<string, WorkerSnapshotRecord>();
  const store = createMockStore(snapshots);
  const service = new WorkerRegistryService(store);
...
test("WorkerRegistryService verifyRemoteWorkerRegistration preserves existing state [worker-registry-service]", () => {
  const existing = createMockWorkerSnapshot({
    workerId: "remote-worker-1",
    placement: "remote",
...
test("WorkerRegistryService getWorker returns worker view when exists [worker-registry-service]", () => {
...
test("WorkerRegistryService getWorker returns null when not exists [worker-registry-service]", () => {
  const snapshots = new Map<string, WorkerSnapshotRecord>();
  const store = createMockStore(snapshots);
  const service = new WorkerRegistryService(store);
...
test("WorkerRegistryService listEligibleWorkers includes degraded workers when requested [worker-registry-service]", () => {
  const snapshots = new Map([
    ["worker-healthy", createMockWorkerSnapshot({ workerId: "worker-healthy", status: "idle", maxConcurrency: 4 })],
    ["worker-degraded", createMockWorkerSnapshot({ workerId: "worker-degraded", status: "degraded", maxConcurrency: 4 })],
  ]);
...
test("WorkerRegistryService listEligibleWorkers respects isolation level requirement [worker-registry-service]", () => {
  const snapshots = new Map([
...
test("WorkerRegistryService listStaleWorkers returns workers with old heartbeats [worker-registry-service]", () => {
