...
test("HumanWaitExecutor throws for empty title", () => {
...
test("HumanWaitExecutor throws for empty reason", () => {
  const executor = new HumanWaitExecutor();
  const context = createTestContext();
...
test("HumanWaitExecutor throws for whitespace-only title and reason", () => {
  const executor = new HumanWaitExecutor();
  const context = createTestContext();
...
test("HumanWaitExecutor trims title and reason", () => {
  const executor = new HumanWaitExecutor({
...
test("HumanWaitExecutor uses provided approvalId if valid", () => {
  const executor = new HumanWaitExecutor();
  const context = createTestContext();
...
test("HumanWaitExecutor trims provided approvalId", () => {
  const executor = new HumanWaitExecutor();
  const context = createTestContext();
...
test("HumanWaitExecutor uses idFactory when approvalId is empty string", () => {
  const executor = new HumanWaitExecutor({
...
test("HumanWaitExecutor uses idFactory when approvalId is only whitespace", () => {
  const executor = new HumanWaitExecutor({
...
test("HumanWaitExecutor resolves to approved status", () => {
  const executor = new HumanWaitExecutor({
...
test("HumanWaitExecutor resolves to rejected status", () => {
  const executor = new HumanWaitExecutor({
    now: () => "2026-04-25T00:00:00.000Z",
...
test("HumanWaitExecutor resolves to expired status", () => {
  const executor = new HumanWaitExecutor({
    now: () => "2026-04-25T00:00:00.000Z",
...
test("HumanWaitExecutor resolves to cancelled status", () => {
  const executor = new HumanWaitExecutor({
    now: () => "2026-04-25T00:00:00.000Z",
...
test("HumanWaitExecutor resolves to cancelled (expired) status", () => {
  const executor = new HumanWaitExecutor({
    now: () => "2026-04-25T00:00:00.000Z",
...
test("HumanWaitExecutor uses provided resolvedAt timestamp", () => {
  const executor = new HumanWaitExecutor({
    now: () => "2026-04-25T00:00:00.000Z",
...
test("HumanWaitExecutor uses now() when resolvedAt not provided", () => {
  const executor = new HumanWaitExecutor({
    now: () => "2026-04-25T00:00:00.000Z",
...
test("HumanWaitExecutor uses provided resolvedBy", () => {
  const executor = new HumanWaitExecutor({
    now: () => "2026-04-25T00:00:00.000Z",
...
test("HumanWaitExecutor defaults resolvedBy to null when not provided", () => {
  const executor = new HumanWaitExecutor({
    now: () => "2026-04-25T00:00:00.000Z",
...
test("HumanWaitExecutor uses provided note", () => {
  const executor = new HumanWaitExecutor({
    now: () => "2026-04-25T00:00:00.000Z",
...
test("HumanWaitExecutor defaults note to null when not provided", () => {
  const executor = new HumanWaitExecutor({
    now: () => "2026-04-25T00:00:00.000Z",
...
test("HumanWaitExecutor calculates duration from requestedAt to resolvedAt", () => {
  const executor = new HumanWaitExecutor({
    now: () => "2026-04-25T00:00:00.000Z",
...
test("HumanWaitExecutor handles future resolvedAt with zero duration", () => {
  const executor = new HumanWaitExecutor({
    now: () => "2026-04-25T00:00:00.000Z",
...
test("HumanWaitExecutor uses requestedAt from request when provided", () => {
  const executor = new HumanWaitExecutor({
...
test("HumanWaitExecutor uses default options when empty array provided", () => {
  const executor = new HumanWaitExecutor({
...
test("HumanWaitExecutor preserves provided options", () => {
  const executor = new HumanWaitExecutor({
...
test("HumanWaitExecutor defaults requestedBy to null when not provided", () => {
  const executor = new HumanWaitExecutor({
...
test("HumanWaitExecutor uses provided requestedBy", () => {
  const executor = new HumanWaitExecutor({
...
test("HumanWaitExecutor defaults requestedBy to null when explicitly null", () => {
  const executor = new HumanWaitExecutor({
...
test("HumanWaitExecutor defaults metadata to empty object when not provided", () => {
  const executor = new HumanWaitExecutor({
...
test("HumanWaitExecutor preserves provided metadata", () => {
  const executor = new HumanWaitExecutor({
...
test("HumanWaitExecutor metadata is readonly", () => {
  const executor = new HumanWaitExecutor({
...
test("HumanWaitExecutor preserves timeoutPolicy: reject", () => {
  const executor = new HumanWaitExecutor({
...
test("HumanWaitExecutor preserves timeoutPolicy: approve", () => {
  const executor = new HumanWaitExecutor({
...
test("HumanWaitExecutor preserves timeoutPolicy: remain_pending", () => {
  const executor = new HumanWaitExecutor({
...
test("HumanWaitExecutor handles multiple simultaneous approvals", () => {
  const executor = new HumanWaitExecutor({
    idFactory: () => "auto-id",
  });
  const context = createTestContext();
...
test("HumanWaitExecutor resolves one approval without affecting others", () => {
  const executor = new HumanWaitExecutor({
    now: () => "2026-04-25T00:00:00.000Z",
    idFactory: () => "auto-id",
  });
...
test("HumanWaitExecutor.getApproval returns null for resolved approval", () => {
  const executor = new HumanWaitExecutor({
    now: () => "2026-04-25T00:00:00.000Z",
...
test("HumanWaitExecutor.getApproval returns null for non-existent approval", () => {
  const executor = new HumanWaitExecutor();

  assert.equal(executor.getApproval("non-existent"), null);
});

test("HumanWaitExecutor.listPendingApprovals returns empty when no approvals", () => {
  const executor = new HumanWaitExecutor();
...
test("HumanWaitExecutor.listPendingApprovals returns copy not reference", () => {
  const executor = new HumanWaitExecutor({
...
test("HumanWaitExecutor.execute() is an alias for requestApproval()", () => {
  const executor = new HumanWaitExecutor({
...
test("HumanWaitExecutor.resolveApproval() throws for unknown approvalId", () => {
  const executor = new HumanWaitExecutor();
...
test("HumanWaitExecutor uses custom now function", () => {
  let callCount = 0;
  const executor = new HumanWaitExecutor({
    now: () => {
      callCount++;
...
test("HumanWaitExecutor uses custom idFactory function", () => {
  let idCounter = 0;
  const executor = new HumanWaitExecutor({
