 ...
 *
 * Simple module that provides queue metrics types and classes.
 */
...
export class QueueMetricsService {
  ...
  public deriveFromStats(stats: { queueName: string; waiting: number; delayed: number; active: number; failed: number; deadLetter: number }): void {
  ...
  public getSnapshot(): QueueMetricsSnapshot {
    ...
    for (const [queueName, times] of this.waitTimeByQueue) {
      if (times.length > 0) {
  ...
  public getQueueMetrics(queueName: string): QueueMetrics | undefined {
    ...
    const avgWaitTime = waitTimes.length > 0 ? waitTimes.reduce((s, t) => s + t, 0) / waitTimes.length : 0;
    return { queueName, depth, enqueuedPerMinute: enqueued, dequeuedPerMinute: dequeued, averageWaitTimeMs: avgWaitTime };
  }
