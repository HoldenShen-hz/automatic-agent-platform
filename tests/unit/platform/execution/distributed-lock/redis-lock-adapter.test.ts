...
// =============================================================================
// Sync acquire method tests
// =============================================================================

test("RedisLockAdapter acquire() uses spawnSync to acquire lock", () => {
  ...
  (require("child_process") as any).spawnSync = () => ({
    status: 0,
    stdout: { trim: () => "OK" },
  });
  ...
  // Restore
  (require("child_process") as any).spawnSync = originalSpawnSync;
...
test("RedisLockAdapter acquire() returns false when spawnSync returns non-zero", () => {
  const adapter = new RedisLockAdapter({ host: "localhost", port: 6379 });
...
test("RedisLockAdapter acquire() returns false when spawnSync throws", () => {
  const adapter = new RedisLockAdapter({ host: "localhost", port: 6379 });
