...
test("lockLogger is available [locking-support]", () => {
  assert.ok(lockLogger);
  assert.equal(typeof lockLogger.log, "function");
});

test("inferPgSslFromDsn returns object for sslmode=require [locking-support]", () => {
  const result = inferPgSslFromDsn("postgres://user:pass@host/db?sslmode=require");
  assert.deepEqual(result, { rejectUnauthorized: true });
});

test("inferPgSslFromDsn returns null for non-sslmode [locking-support]", () => {
  const result = inferPgSslFromDsn("postgres://user:pass@host/db");
  assert.equal(result, null);
});

test("inferPgSslFromDsn returns null for sslmode=disable [locking-support]", () => {
  const result = inferPgSslFromDsn("postgres://user:pass@host/db?sslmode=disable");
  assert.equal(result, null);
});

test("inferPgSslFromDsn returns null for empty DSN [locking-support]", () => {
  const result = inferPgSslFromDsn("");
  assert.equal(result, null);
});

test("inferPgSslFromDsn handles DSN without query params [locking-support]", () => {
  const result = inferPgSslFromDsn("postgres://user:pass@host:5432/dbname");
  assert.equal(result, null);
});

test("inferPgSslFromDsn handles mixed case sslmode values [locking-support]", () => {
...
test("inferPgSslFromDsn trims whitespace from sslmode value [locking-support]", () => {
  const result = inferPgSslFromDsn("postgres://user:pass@host/db?sslmode=  require  ");
  assert.deepEqual(result, { rejectUnauthorized: true });
});

test("inferPgSslFromDsn returns null for invalid URL [locking-support]", () => {
  const result = inferPgSslFromDsn("not a valid url");
  assert.equal(result, null);
});

test("inferPgSslFromDsn returns null for missing sslmode param [locking-support]", () => {
...
test("defaultPostgresFactory returns a callable factory function [locking-support]", () => {
