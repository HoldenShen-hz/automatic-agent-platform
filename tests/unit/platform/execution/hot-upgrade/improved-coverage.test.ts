...
// Import types only - these don't require runtime database connections
import type {
  UpgradePlan,
  UpgradeBatch,
...
test("Version compatibility mapping preserves all fields", () => {
  ...
  const compat: VersionCompatibility = {
    ...
    compatibilityLevel: String(rawRow.compatibility_level) as "full" | "n_minus_1" | "incompatible",
...
test("Version compatibility roundtrip conversion", () => {
  ...
  const reconstructed: VersionCompatibility = {
    fromVersion: String(dbRow.from_version),
    toVersion: String(dbRow.to_version),
    compatibilityLevel: String(dbRow.compatibility_level) as VersionCompatibility["compatibilityLevel"],
...
test("Upgrade batch status transitions", () => {
  ...
  const inProgressBatch: UpgradeBatch = {
    ...pendingBatch,
    status: "in_progress",
    startedAt: "2024-01-01T00:00:00Z",
  };
  ...
  const completedBatch: UpgradeBatch = {
    ...inProgressBatch,
    status: "completed",
