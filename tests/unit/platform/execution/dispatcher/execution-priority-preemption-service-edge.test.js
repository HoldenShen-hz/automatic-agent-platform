...
import { ExecutionPriorityPreemptionService } from "../../../../../src/platform/execution/dispatcher/execution-priority-preemption-service.js";
// ---------------------------------------------------------------------------
// Helper types
// ---------------------------------------------------------------------------
...
function createMockTicket(priority = "normal", overrides = {}) {
    return {
        id: "ticket-1",
        executionId: "exec-1",
        taskId: "task-1",
...
function createMockAgentExecution(overrides = {}) {
    return {
        executionId: "exec-running-1",
        taskId: "task-1",
        agentId: "agent-1",
...
function createPreemptionRequest(ticket, overrides = {}) {
    return {
        ticket,
        dispatchTarget: "any",
        requiredIsolationLevel: "standard",
...
test("preemptForUrgentTicket filters by insufficient isolation level", () => {
    ...
    const service = new ExecutionPriorityPreemptionService(createMockDb(), store);
    const ticket = createMockTicket("urgent");
    const request = createPreemptionRequest(ticket, { requiredIsolationLevel: "hardened" });
    const decision = service.preemptForUrgentTicket(request);
    assert.equal(decision.outcome, "not_preempted");
    assert.equal(decision.trace.reasonCode, "no_safe_preemption_candidate");
});
...
test("preemptForUrgentTicket rejects standard isolation when required is strict", () => {
    ...
    const service = new ExecutionPriorityPreemptionService(createMockDb(), store);
    const ticket = createMockTicket("urgent");
    const request = createPreemptionRequest(ticket, { requiredIsolationLevel: "strict" });
    const decision = service.preemptForUrgentTicket(request);
    assert.equal(decision.outcome, "not_preempted");
    assert.equal(decision.trace.reasonCode, "no_safe_preemption_candidate");
});
...
//# sourceMappingURL=execution-priority-preemption-service-edge.test.js.map