...
import { normalizeStringArray, parseJsonArray, resolveDispatchTarget, resolveRequiredIsolationLevel, resolveRequiredRepoVersion, meetsIsolationRequirement, resolveRemoteAvailability, resolveRemoteRepoVersionReason, resolveRemoteSessionReason, resolveRemoteTrustReason, selectWorkersForDispatch, toWorkerEvaluation, isElevatedPriority, isRemoteSessionReadyForDispatch, resolveDispatchBackpressureReason, buildDispatchAgentExecutionRecord, AFFINITY_SELECTION_BONUS, LOAD_SKEW_SELECTION_PENALTY, DEFAULT_RUNTIME_BACKPRESSURE_HEALTH_OPTIONS, } from "../../../../../src/platform/execution/dispatcher/execution-dispatch-support.js";
// ---------------------------------------------------------------------------
...
test("resolveDispatchTarget returns any for empty string", () => {
    assert.equal(resolveDispatchTarget(""), "any");
});
...
test("resolveRequiredIsolationLevel returns standard for unknown string", () => {
    assert.equal(resolveRequiredIsolationLevel("unknown"), "standard");
});
test("resolveRequiredIsolationLevel returns standard for empty string", () => {
    assert.equal(resolveRequiredIsolationLevel(""), "standard");
});
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
function makeEvaluation(placement, accepted, rejectionReason = null) {
    return {
        workerId: "w1",
        status: "idle",
...
test("selectWorkersForDispatch prefer_remote with only remote workers", () => {
    const workers = [
        { workerId: "r1", placement: "remote" },
        { workerId: "r2", placement: "remote" },
    ];
...
test("selectWorkersForDispatch prefer_remote with only local workers and all reasons null", () => {
    const workers = [{ workerId: "l1", placement: "local" }];
    const result = selectWorkersForDispatch("prefer_remote", workers, null, null, null, null);
...
test("selectWorkersForDispatch prefer_remote priority of reasons: trust > session > repo > availability", () => {
    const workers = [{ workerId: "l1", placement: "local" }];
    const result = selectWorkersForDispatch("prefer_remote", workers, "available", "remote.untrusted", "remote.session_unready", "remote.repo_version_mismatch");
...
test("isElevatedPriority returns false for unknown priority", () => {
    assert.equal(isElevatedPriority("unknown"), false);
});
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
function createMockStoreForBuildRecord(existingAgentRecord) {
    return {
        worker: {
...
function createMockExecution(id = "exec-1") {
    return {
        id,
        taskId: "task-1",
        workflowId: "wf-1",
...
test("toWorkerEvaluation preserves all numeric fields", () => {
    ...
    const evaluation = toWorkerEvaluation(worker, false, "worker_unavailable", ["cap1", "cap2"]);
...
test("selectWorkersForDispatch local_only returns all workers (no filtering)", () => {
    const workers = [
        { workerId: "w1", placement: "local" },
        { workerId: "w2", placement: "remote" },
        { workerId: "w3", placement: "local" },
    ];
...
test("selectWorkersForDispatch require_remote returns all workers even when no remote", () => {
    const workers = [{ workerId: "w1", placement: "local" }];
    const result = selectWorkersForDispatch("require_remote", workers, null, null, null, null);
...
test("selectWorkersForDispatch prefer_remote uses first reason in priority chain", () => {
    const workers = [{ workerId: "l1", placement: "local" }];
    const result = selectWorkersForDispatch("prefer_remote", workers, "partial_available", null, "remote.session_unready", null);
...
test("selectWorkersForDispatch prefer_remote uses availability when no specific reasons", () => {
    const workers = [{ workerId: "l1", placement: "local" }];
    const result = selectWorkersForDispatch("prefer_remote", workers, "partial_available", null, null, null);
    assert.equal(result.reasonCode, "remote.fallback_local.partial_available");
});
//# sourceMappingURL=execution-dispatch-support-edge.test.js.map