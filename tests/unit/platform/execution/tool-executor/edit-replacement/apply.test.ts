...
test("reindentBlock reindents with 2-space indent [apply]", () => {
  const input = "function foo() {\n  return 1;\n}";
  const result = reindentBlock(input, "  ");
  assert.equal(result, "  function foo() {\n    return 1;\n  }");
});

test("reindentBlock handles empty string [apply]", () => {
  const result = reindentBlock("", "  ");
  assert.equal(result, "");
});

test("reindentBlock handles single line [apply]", () => {
  const result = reindentBlock("hello", "    ");
  assert.equal(result, "    hello");
});

test("reindentBlock strips common indent before reindenting [apply]", () => {
  const input = "    line1\n    line2\n    line3";
  const result = reindentBlock(input, "");
  assert.equal(result, "line1\nline2\nline3");
});

test("reindentBlock normalizes CRLF to LF [apply]", () => {
  const input = "line1\r\nline2";
  const result = reindentBlock(input, "");
  assert.equal(result, "line1\nline2");
});

test("reindentBlock preserves empty lines [apply]", () => {
  const input = "line1\n\nline3";
  const result = reindentBlock(input, "  ");
  assert.equal(result, "  line1\n\n  line3");
});
...
test("reindentBlockToMatch matches single line indentation [apply]", () => {
...
test("reindentBlockToMatch matches multiple line indentation [apply]", () => {
...
test("reindentBlockToMatch falls back to common indent for different line counts [apply]", () => {
  const value = "a\nb";
  const matchedText = "    a\n    b\n    c";
  const result = reindentBlockToMatch(value, matchedText);
...
test("reindentBlockToMatch handles empty value [apply]", () => {
