...
function createTestSession(db: SqliteDatabase, sessionId: string, taskId: string, now: string, status: SessionStatus = "open"): void {
...
test("SessionRepository updateSessionStatus creates terminal session summary", () => {
  ...
  try {
    ...
    const summary = repo.getLatestSessionSummary("session-terminal") as SessionSummaryRecord | undefined;
    assert.ok(summary);
...
test("SessionRepository writes dual-storage JSONL events when configured", () => {
  ...
  try {
    ...
    const repo = new SessionRepository(
      db.connection,
      new SessionDualStorageService({ jsonlRootDir: replayRoot }),
    );
