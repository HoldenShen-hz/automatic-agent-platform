...
test("async-sql-database helper functions delegate to AsyncSqlConnection", async () => {
  ...
  const rows = await asyncQueryAll<{ id: string }>(connection, "SELECT * FROM demo WHERE kind = $1", "alpha");
  const rowsOrEmpty = await asyncQueryAllOrEmpty<{ id: string }>(connection, "SELECT * FROM demo WHERE state = $1", "ready");
  const row = await asyncQueryOne<{ id: string }>(connection, "SELECT * FROM demo WHERE id = $1", "row-a");
...
test("asyncQueryAll returns multiple rows", async () => {
  ...
  const rows = await asyncQueryAll<{ id: string }>(connection, "SELECT id FROM items");
...
test("asyncExecute returns correct change count", async () => {
  const connection: AsyncSqlConnection = {
    async query<T>(sql: string, ...params: unknown[]): Promise<AsyncQueryResult<T>> {
      return { rows: [] as T[], rowCount: 0, changes: 0 };
    },
    async queryOne<T>(sql: string, ...params: unknown[]) {
      return undefined;
    },
