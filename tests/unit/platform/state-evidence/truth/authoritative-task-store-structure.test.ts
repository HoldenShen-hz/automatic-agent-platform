...
test("AuthoritativeTaskStore delegating core remains repository-backed and method-complete", () => {
  ...
  assert.ok(
    baseSource.includes("createAuthoritativeTaskStoreRepositories"),
    "delegating base should build the repository set",
  );
  assert.ok(
    coreSource.split("\n").length <= 20,
    `delegating core entrypoint should stay thin, got ${coreSource.split("\n").length} lines`,
  );
  ...
  assert.equal(
    combinedMethodSource.includes("this.callRepo("),
    false,
    "delegating method files should use helper delegation instead of direct callRepo",
  );
  ...
  assert.equal(
    overrideMethodCount,
    abstractMethodCount,
    "delegating method files should implement every abstract compatibility method",
  );
});
...
test("AuthoritativeTaskStore base exposes repository accessors and query aliases", () => {
  ...
  for (const accessor of EXPECTED_ACCESSORS) {
    assert.ok(
      source.includes(`public get ${accessor}()`),
      `delegating base should expose accessor ${accessor}`,
    );
  }
});
...
test("AuthoritativeTaskStore consumers use repository accessors instead of legacy store methods", () => {
  ...
  for (const root of CONSUMER_SCAN_ROOTS) {
    for (const filePath of listTypeScriptFiles(root)) {
      ...
      if (
        !source.includes("AuthoritativeTaskStore")
