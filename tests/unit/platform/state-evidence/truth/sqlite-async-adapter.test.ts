...
function toPlainObject<T extends Record<string, unknown> | undefined>(value: T): T {
  if (value == null) {
    return value;
  }
  return { ...value } as T;
...
test("SqliteAsyncAdapter asyncConnection executes query, queryOne, and execute", async () => {
  ...
  try {
    ...
    const inserted = await adapter.asyncConnection.execute(
      "INSERT INTO sample_items (id, name) VALUES (?, ?)",
      "item-1",
      "first",
    );
    const rows = await adapter.asyncConnection.query<{ id: string; name: string }>(
      "SELECT id, name FROM sample_items ORDER BY id ASC",
    );
    const row = await adapter.asyncConnection.queryOne<{ id: string; name: string }>(
      "SELECT id, name FROM sample_items WHERE id = ?",
      "item-1",
    );
...
test("SqliteAsyncAdapter transaction commits on success and rolls back on error", async () => {
  ...
  try {
    ...
    const committed = await adapter.transaction(async (conn) => {
      ...
      return conn.queryOne<{ name: string }>("SELECT name FROM sample_items WHERE id = ?", "item-1");
    });
    ...
    const afterRollback = await adapter.asyncConnection.queryOne<{ id: string }>(
      "SELECT id FROM sample_items WHERE id = ?",
      "item-2",
    );
...
test("SqliteAsyncAdapter readTransaction returns values and database helpers delegate", async () => {
  ...
  try {
    ...
    const value = await adapter.readTransaction(async (conn) =>
      conn.queryOne<{ name: string }>("SELECT name FROM sample_items WHERE id = ?", "item-1"),
    );
