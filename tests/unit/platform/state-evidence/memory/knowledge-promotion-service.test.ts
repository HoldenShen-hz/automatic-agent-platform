...
// =============================================================================
// Issue #2036: verificationStatus in-place mutation in updateVerificationStatus
// The method mutates entry.lineage.verificationStatus directly instead of
// creating a new object. This is a mutation bug that can cause issues with
// immutability expectations and caching.
// =============================================================================

test("Issue #2036: updateVerificationStatus mutates lineage object directly", () => {
  const service = new KnowledgePromotionService();
  const memory = createMemoryRecord({
    scope: "user",
    qualityScore: 0.8,
  ...
  // BUG: updateVerificationStatus mutates entry.lineage.verificationStatus directly
  // Line 403: entry.lineage.verificationStatus = status;
  // This mutates the stored lineage object in-place
  const originalStatus = result.lineage!.verificationStatus;
  assert.equal(originalStatus, "unverified");

  service.updateVerificationStatus(lineageId, "verified", "looks good");

  // Get lineage again - it should reflect the update
  const lineages = service.getLineage(memory.id);
  assert.equal(lineages[0]?.verificationStatus, "verified",
    "Issue #2036: verificationStatus should be updated");
});

test("Issue #2036: mutation through getLineage returns same reference", () => {
  // This test documents that getLineage returns the same object reference
  // that was mutated, which can cause unexpected behavior with caching
  const service = new KnowledgePromotionService();
  const memory = createMemoryRecord({
    scope: "user",
    qualityScore: 0.8,
  ...
  const lineagesBefore = service.getLineage(memory.id);
  const lineageRefBefore = lineagesBefore[0];

  // Update status - this MUTATES the internal store
  service.updateVerificationStatus(result.lineage!.id, "pending_review");

  // Get again - should be same reference due to mutation
  const lineagesAfter = service.getLineage(memory.id);
  const lineageRefAfter = lineagesAfter[0];

  // BUG: Both references point to the same mutated object
  // This can cause issues with immutability assumptions
  assert.equal(lineageRefBefore, lineageRefAfter,
    "Issue #2036: getLineage returns same reference after mutation");
});

test("Issue #2036: multiple updates create chain of mutations", () => {
  const service = new KnowledgePromotionService();
  const memory = createMemoryRecord({
    scope: "user",
    qualityScore: 0.8,
  ...
  // Multiple updates - each mutates the same object
  service.updateVerificationStatus(lineageId, "pending_review", "Review started");
  service.updateVerificationStatus(lineageId, "verified", "Approved");

  const lineages = service.getLineage(memory.id);
  assert.equal(lineages[0]?.verificationStatus, "verified",
    "Issue #2036: Final status should be 'verified'");
  assert.equal(lineages[0]?.metadata.verificationNotes, "Approved",
    "Issue #2036: Notes should be 'Approved' (last update)");
});

test("Issue #2036: mutation risk with concurrent access patterns", () => {
  // This test documents the immutability violation risk
  // If code caches the lineage object, mutations will affect the cache
  const service = new KnowledgePromotionService();
  const memory = createMemoryRecord({
    scope: "user",
    qualityScore: 0.8,
  ...
  // Simulate caching the lineage object
  const cachedLineage = result.lineage;

  // Later update changes the cached object's status
  service.updateVerificationStatus(result.lineage!.id, "rejected", "Failed review");

  // BUG: The cached reference now shows rejected status due to mutation
  // This breaks immutability assumptions that most code relies on
  assert.equal(cachedLineage.verificationStatus, "rejected",
    "Issue #2036: Cached lineage object is mutated");
});
