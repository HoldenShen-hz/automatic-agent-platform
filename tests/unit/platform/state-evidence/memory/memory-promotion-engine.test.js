...
function createMemory(id, scope, hitCount, qualityScore, importanceScore) {
    return {
        id,
        taskId: "task_1",
        sessionId: "session_1",
...
test("evaluatePromotion handles null qualityScore with ?? 0 fallback", () => {
    const engine = new MemoryPromotionEngine();
    const memory = createMemory("mem_006", "session", 10, 0, 0.5);
...
test("evaluatePromotion handles null importanceScore with ?? 0 fallback", () => {
    const engine = new MemoryPromotionEngine();
    const memory = createMemory("mem_007", "session", 10, 0.7, 0);
...
test("promote adds to projectEntries when targetLayer is project and projectId provided", () => {
    const engine = new MemoryPromotionEngine();
    const result = engine.promote([createMemory("mem_project", "agent", 9, 0.8, 0.7)], { projectId: "project_alpha" });
...
test("promote adds to userEntries when targetLayer is user and userId provided", () => {
    const engine = new MemoryPromotionEngine();
    const result = engine.promote([createMemory("mem_user", "project", 14, 0.85, 0.8)], { userId: "user_alpha" });
    assert.equal(result.promoted.length, 1);
...
test("promote does not promote when below all thresholds", () => {
    const engine = new MemoryPromotionEngine();
    const result = engine.promote([createMemory("mem_reject", "session", 1, 0.4, 0.2)], { projectId: "project_alpha", userId: "user_alpha" });
...
test("promote does not add to projectEntries when targetLayer is user (only adds to userEntries)", () => {
    const engine = new MemoryPromotionEngine();
    const result = engine.promote([createMemory("mem_user_only", "project", 15, 0.9, 0.8)], { projectId: "proj_001", userId: "user_001" });
    assert.equal(result.promoted.length, 1);
    assert.equal(result.promoted[0]?.targetLayer, "user");
    assert.equal(result.projectEntries.length, 0);
    assert.equal(result.userEntries.length, 1);
...
test("promote does not add to userEntries when targetLayer is project (only adds to projectEntries)", () => {
    const engine = new MemoryPromotionEngine();
    const result = engine.promote([createMemory("mem_proj_only", "agent", 10, 0.85, 0.75)], { projectId: "proj_001", userId: "user_001" });
    assert.equal(result.promoted.length, 1);
    assert.equal(result.promoted[0]?.targetLayer, "project");
    assert.equal(result.projectEntries.length, 1);
    assert.equal(result.userEntries.length, 0);
...
test("promote with no projectId context skips project store", () => {
    const engine = new MemoryPromotionEngine();
    const result = engine.promote([createMemory("mem_proj_no_context", "agent", 10, 0.85, 0.75)], { userId: "user_001" });
    assert.equal(result.promoted.length, 1);
    assert.equal(result.projectEntries.length, 0);
});
test("promote with no userId context skips user store", () => {
    const engine = new MemoryPromotionEngine();
    const result = engine.promote([createMemory("mem_user_no_context", "project", 15, 0.9, 0.8)], { projectId: "proj_001" });
    assert.equal(result.promoted.length, 1);
    assert.equal(result.userEntries.length, 0);
});
test("promote with null projectId skips project store", () => {
    const engine = new MemoryPromotionEngine();
    const result = engine.promote([createMemory("mem_proj_null", "agent", 10, 0.85, 0.75)], { projectId: null, userId: "user_001" });
    assert.equal(result.promoted.length, 1);
    assert.equal(result.projectEntries.length, 0);
});
test("promote with null userId skips user store", () => {
    const engine = new MemoryPromotionEngine();
    const result = engine.promote([createMemory("mem_user_null", "project", 15, 0.9, 0.8)], { projectId: "proj_001", userId: null });
    assert.equal(result.promoted.length, 1);
    assert.equal(result.userEntries.length, 0);
});
...
test("listProjectMemory returns entries after promotion", () => {
    const engine = new MemoryPromotionEngine();
    engine.promote([createMemory("mem_proj_1", "agent", 10, 0.9, 0.8)], { projectId: "proj_list_test" });
...
test("listProjectMemory returns multiple entries for same project", () => {
    const engine = new MemoryPromotionEngine();
    engine.promote([createMemory("mem_proj_a", "agent", 10, 0.9, 0.8)], { projectId: "proj_multi" });
    engine.promote([createMemory("mem_proj_b", "agent", 12, 0.85, 0.75)], { projectId: "proj_multi" });
    const entries = engine.listProjectMemory("proj_multi");
    assert.equal(entries.length, 2);
});
test("listProjectMemory only returns entries for specified project", () => {
    const engine = new MemoryPromotionEngine();
    engine.promote([createMemory("mem_proj_1", "agent", 10, 0.9, 0.8)], { projectId: "proj_a" });
    engine.promote([createMemory("mem_proj_2", "agent", 12, 0.85, 0.75)], { projectId: "proj_b" });
...
test("listUserMemory returns entries after promotion", () => {
    const engine = new MemoryPromotionEngine();
    engine.promote([createMemory("mem_user_1", "project", 15, 0.9, 0.8)], { userId: "user_list_test" });
...
test("listUserMemory returns multiple entries for same user", () => {
    const engine = new MemoryPromotionEngine();
    engine.promote([createMemory("mem_user_a", "project", 15, 0.9, 0.8)], { userId: "user_multi" });
    engine.promote([createMemory("mem_user_b", "project", 20, 0.85, 0.75)], { userId: "user_multi" });
    const entries = engine.listUserMemory("user_multi");
    assert.equal(entries.length, 2);
});
test("listUserMemory only returns entries for specified user", () => {
    const engine = new MemoryPromotionEngine();
    engine.promote([createMemory("mem_user_1", "project", 15, 0.9, 0.8)], { userId: "user_a" });
    engine.promote([createMemory("mem_user_2", "project", 20, 0.85, 0.75)], { userId: "user_b" });
...
//# sourceMappingURL=memory-promotion-engine.test.js.map