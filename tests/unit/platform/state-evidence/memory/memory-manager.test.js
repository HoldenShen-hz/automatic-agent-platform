...
function createMockProvider() {
    return {
        ...
        prefetch: async (_query) => createMockPrefetchResult(),
        queuePrefetch: async () => ({
            providerId: "test-provider",
            requestId: "req_123",
...
function createMockPrefetchResult(overrides = {}) {
    return {
        providerId: "test-provider",
        requestId: "req_123",
        generatedAt: "2026-04-01T00:00:00.000Z",
...
function createMockSyncResult(overrides = {}) {
    return {
        providerId: "test-provider",
...
test("MemoryManager accepts provider and cache", () => {
    const provider = createMockProvider();
    const cacheMock = {
        getOrComputeMemoryRetrieval: async (_query, loader, _tags) => {
...
test("buildView categorizes task_runtime scope into runtime layer", async () => {
    const provider = createMockProvider();
    provider.prefetch = async () => createMockPrefetchResult({ memories: [createMockMemoryRecord({ scope: "task_runtime" })] });
    const service = new MemoryPlaneService(provider);
...
test("buildView categorizes session scope into session layer", async () => {
    const provider = createMockProvider();
    provider.prefetch = async () => createMockPrefetchResult({ memories: [createMockMemoryRecord({ scope: "session" })] });
    const service = new MemoryPlaneService(provider);
    const result = await service.buildView({ sessionId: "session_1" });
...
test("buildView categorizes agent scope into agent layer and episodic architecture layer", async () => {
    const provider = createMockProvider();
    provider.prefetch = async () => createMockPrefetchResult({ memories: [createMockMemoryRecord({ scope: "agent" })] });
    const service = new MemoryPlaneService(provider);
    const result = await service.buildView({ sessionId: "session_1" });
...
test("buildView categorizes workspace scope into project layer", async () => {
    const provider = createMockProvider();
    provider.prefetch = async () => createMockPrefetchResult({ memories: [createMockMemoryRecord({ scope: "workspace" })] });
    const service = new MemoryPlaneService(provider);
    const result = await service.buildView({ sessionId: "session_1" });
...
test("buildView categorizes project scope into project layer and semantic architecture layer", async () => {
    const provider = createMockProvider();
    provider.prefetch = async () => createMockPrefetchResult({ memories: [createMockMemoryRecord({ scope: "project" })] });
    const service = new MemoryPlaneService(provider);
    const result = await service.buildView({ sessionId: "session_1" });
    assert.equal(result.layers.project.length, 1);
    assert.equal(result.architectureLayers.semantic.length, 1);
...
test("buildView categorizes user scope into user layer and procedural architecture layer", async () => {
    const provider = createMockProvider();
    provider.prefetch = async () => createMockPrefetchResult({ memories: [createMockMemoryRecord({ scope: "user" })] });
    const service = new MemoryPlaneService(provider);
    const result = await service.buildView({ sessionId: "session_1" });
...
test("buildView categorizes evolution scope into evolution layer and meta architecture layer", async () => {
    const provider = createMockProvider();
    provider.prefetch = async () => createMockPrefetchResult({ memories: [createMockMemoryRecord({ scope: "evolution" })] });
    const service = new MemoryPlaneService(provider);
    const result = await service.buildView({ sessionId: "session_1" });
...
test("buildView categorizes experience scope into evolution layer", async () => {
    const provider = createMockProvider();
    provider.prefetch = async () => createMockPrefetchResult({ memories: [createMockMemoryRecord({ scope: "experience" })] });
    const service = new MemoryPlaneService(provider);
    const result = await service.buildView({ sessionId: "session_1" });
    assert.equal(result.layers.evolution.length, 1);
});
test("buildView maps unknown scope to project layer", async () => {
    const provider = createMockProvider();
    provider.prefetch = async () => createMockPrefetchResult({ memories: [createMockMemoryRecord({ scope: "unknown_scope" })] });
    const service = new MemoryPlaneService(provider);
    const result = await service.buildView({ sessionId: "session_1" });
    assert.equal(result.layers.project.length, 1);
});
...
test("buildView includes memoryIds from prefetch result", async () => {
    const provider = createMockProvider();
    provider.prefetch = async () => createMockPrefetchResult({ memories: [createMockMemoryRecord({ id: "mem_abc" })] });
    const service = new MemoryPlaneService(provider);
    const result = await service.buildView({ sessionId: "session_1" });
...
test("buildView includes experienceIds from prefetch result", async () => {
    const provider = createMockProvider();
    provider.prefetch = async () => createMockPrefetchResult({ experienceIds: ["exp_1", "exp_2"] });
    const service = new MemoryPlaneService(provider);
    const result = await service.buildView({ sessionId: "session_1" });
...
test("buildView sets fewShotExampleCount correctly", async () => {
    const provider = createMockProvider();
    provider.prefetch = async () => createMockPrefetchResult({
        fewShotExamples: [
            { taskContext: "ctx1", taskIntent: "intent1", approach: "approach1", toolsUsed: ["tool1"], outcome: "succeeded", reasoning: null },
            { taskContext: "ctx2", taskIntent: "intent2", approach: "approach2", toolsUsed: ["tool2"], outcome: "succeeded", reasoning: null },
            { taskContext: "ctx3", taskIntent: "intent3", approach: "approach3", toolsUsed: ["tool3"], outcome: "succeeded", reasoning: null },
        ],
    });
    const service = new MemoryPlaneService(provider);
    const result = await service.buildView({ sessionId: "session_1" });
...
test("buildView returns empty layers for empty memories", async () => {
    const provider = createMockProvider();
    provider.prefetch = async () => createMockPrefetchResult({ memories: [] });
    const service = new MemoryPlaneService(provider);
    const result = await service.buildView({ sessionId: "session_1" });
    assert.equal(result.layers.runtime.length, 0);
    assert.equal(result.layers.session.length, 0);
...
test("buildView includes queryText in cache tags when provided", async () => {
    ...
    let capturedTags = [];
    const cacheMock = {
        getOrComputeMemoryRetrieval: async (_query, _loader, tags) => {
            capturedTags = tags;
...
test("syncTurn returns rememberedMemories when present", async () => {
    ...
    const provider = createMockProvider();
    provider.syncTurn = async () => createMockSyncResult({ rememberedMemoryIds: ["mem_remembered"], rememberedMemories });
    const service = new MemoryPlaneService(provider);
...
test("syncTurn returns result unchanged when no remembered memories", async () => {
    const provider = createMockProvider();
    provider.syncTurn = async () => createMockSyncResult({ rememberedMemoryIds: [] });
    const service = new MemoryPlaneService(provider);
    const result = await service.syncTurn({ memories: [], experience: null });
...
test("syncTurn handles null experience gracefully", async () => {
    const provider = createMockProvider();
    provider.syncTurn = async () => createMockSyncResult({ rememberedMemoryIds: ["mem_1"] });
    const service = new MemoryPlaneService(provider);
    const result = await service.syncTurn({ memories: [], experience: null });
