...
import { architectureLayerToScope, scopeToArchitectureLayer, getLayerTtlConfig, getLayerTtlConfigByArchitectureLayer, isMemoryStale, getEvictionPriority, shouldEvict, DEFAULT_LAYER_TTL_CONFIGS, DEFAULT_MEMORY_PROMOTION_RULES, } from "../../../../../src/platform/state-evidence/memory/memory-layer-model.js";
...
test("getLayerTtlConfig returns undefined for unknown", () => {
    const config = getLayerTtlConfig("unknown");
...
test("getEvictionPriority handles unknown trust level", () => {
    const memory = createTestMemory({
        scope: "project",
        sourceTrustLevel: "unknown_level",
    });
    const priority = getEvictionPriority(memory);
    // Should default to 0.5, so 1 - 0.5 = 0.5
    assert.ok(Math.abs(priority - 0.5) < 0.001);
...
test("Promotion rules require increasing hit counts", () => {
    for (let i = 1; i < DEFAULT_MEMORY_PROMOTION_RULES.length; i++) {
        ...
        assert.ok(current.minHitCount > previous.minHitCount, `${current.from} requires more hits than ${previous.from}`);
    }
});
test("Promotion rules require increasing quality scores", () => {
    for (let i = 1; i < DEFAULT_MEMORY_PROMOTION_RULES.length; i++) {
        const current = DEFAULT_MEMORY_PROMOTION_RULES[i];
        const previous = DEFAULT_MEMORY_PROMOTION_RULES[i - 1];
        assert.ok(current.minQualityScore > previous.minQualityScore, `${current.from} requires higher quality than ${previous.from}`);
    }
});
//# sourceMappingURL=memory-layer-model-functions.test.js.map