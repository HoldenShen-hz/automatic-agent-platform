...
import type { MemoryRecord, MemoryLayer, MemorySourceTrustLevel } from "../../../../../src/platform/contracts/types/domain.js";
import {
  getMemoryState,
...
test("getMemoryState returns expired when expiresAt is in the past", () => {
  const record = createMemoryRecord({
    expiresAt: "2026-04-23T00:00:00.000Z" as any,
  });
...
test("getMemoryState returns revoked when revokedAt is in the past", () => {
  const record = createMemoryRecord({
    revokedAt: "2026-04-23T00:00:00.000Z" as any,
  });
  const state = getMemoryState(record, "2026-04-24T00:00:00.000Z");
...
test("getMemoryState uses provided evaluatedAt timestamp", () => {
  const record = createMemoryRecord({
    expiresAt: "2026-04-25T00:00:00.000Z" as any,
  });
  const state = getMemoryState(record, "2026-04-24T00:00:00.000Z");
  assert.equal(state, "active");
});
...
test("filterAndSortMemories sorts by createdAt descending", () => {
  const records = [
    createMemoryRecord({ id: "mem_old", createdAt: "2026-04-20T00:00:00.000Z" as any }),
    createMemoryRecord({ id: "mem_new", createdAt: "2026-04-24T00:00:00.000Z" as any }),
    createMemoryRecord({ id: "mem_mid", createdAt: "2026-04-22T00:00:00.000Z" as any }),
  ];
  const result = filterAndSortMemories(records, {});
...
test("filterAndSortMemories respects limit parameter", () => {
  const records = [
    createMemoryRecord({ id: "mem_l1", createdAt: "2026-04-24T00:00:00.000Z" as any }),
    createMemoryRecord({ id: "mem_l2", createdAt: "2026-04-23T00:00:00.000Z" as any }),
    createMemoryRecord({ id: "mem_l3", createdAt: "2026-04-22T00:00:00.000Z" as any }),
  ];
...
test("filterAndSortMemories applies multiple filters", () => {
  const records = [
    createMemoryRecord({ id: "mem_m1", taskId: "task_1", scope: "project", memoryLayer: "layer_3" as MemoryLayer }),
    createMemoryRecord({ id: "mem_m2", taskId: "task_2", scope: "project", memoryLayer: "layer_3" as MemoryLayer }),
    createMemoryRecord({ id: "mem_m3", taskId: "task_1", scope: "agent", memoryLayer: "layer_3" as MemoryLayer }),
  ];
  const result = filterAndSortMemories(records, {
    taskId: "task_1",
    scopes: ["project"],
...
test("filterAndSortMemories applies limit with other filters", () => {
  const records = [
    createMemoryRecord({ id: "mem_lim1", taskId: "task_x", createdAt: "2026-04-24T00:00:00.000Z" as any }),
    createMemoryRecord({ id: "mem_lim2", taskId: "task_x", createdAt: "2026-04-23T00:00:00.000Z" as any }),
    createMemoryRecord({ id: "mem_lim3", taskId: "task_x", createdAt: "2026-04-22T00:00:00.000Z" as any }),
  ];
