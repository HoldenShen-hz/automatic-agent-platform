...
import { normalizeMemoryContent, parseStructuredMemoryContent, stringifyStructuredMemoryContent, extractStructuredMemoryText, buildFactProvenanceFromRecord, } from "../../../../../src/platform/state-evidence/memory/memory-schema.js";
...
test("normalizeMemoryContent handles reasonCode field", () => {
    ...
    assert.ok(result.facts.some((f) => f.content === "ERR_TIMEOUT" && f.category === "reason_code" && f.confidence === 1));
});
test("normalizeMemoryContent handles kind field as fact", () => {
    ...
    assert.ok(result.facts.some((f) => f.content === "recovery_action" && f.category === "kind" && f.confidence === 1));
});
test("normalizeMemoryContent handles array fields as facts", () => {
    ...
    assert.ok(result.facts.some((f) => f.content === "urgent" && f.category === "tags"));
    assert.ok(result.facts.some((f) => f.content === "backend" && f.category === "tags"));
});
test("normalizeMemoryContent handles primitive values as facts", () => {
    ...
    assert.ok(result.facts.some((f) => f.content === "priority=1" && f.category === "priority"));
    assert.ok(result.facts.some((f) => f.content === "active=true" && f.category === "active"));
});
...
test("extractStructuredMemoryText deduplicates results", () => {
    ...
    const text = extractStructuredMemoryText(content);
    const duplicates = text.filter((t) => t === "duplicate");
...
test("normalizeMemoryContent handles text/note fields as topOfMind", () => {
    ...
    assert.ok(result.facts.some((f) => f.content === "some text note" && f.category === "note"));
});
...
test("normalizeMemoryContent handles errorMessage field", () => {
    ...
    assert.ok(result.facts.some((f) => f.category === "error_message"));
});
...
test("normalizeMemoryContent filters null values from topOfMind array via normalizeStringList", () => {
    const result = normalizeMemoryContent({
        content: {
            schemaVersion: "memory.v2",
            workContext: null,
            topOfMind: ["valid item", null, undefined, "another valid"],
            recentHistory: [],
            longTermBackground: [],
            facts: [],
        },
...
test("normalizeMemoryContent filters null values from recentHistory array via normalizeStringList", () => {
    const result = normalizeMemoryContent({
        content: {
            ...
            workContext: null,
            topOfMind: [],
            recentHistory: [null, "valid history", undefined],
            longTermBackground: [],
            facts: [],
        },
    });
...
test("normalizeMemoryContent filters null values from longTermBackground array via normalizeStringList", () => {
    const result = normalizeMemoryContent({
        content: {
            ...
            topOfMind: [],
            recentHistory: [],
            longTermBackground: [null, "valid background", undefined],
            facts: [],
        },
    });
...
test("normalizeMemoryContent handles string fact in facts array via normalizeFact string branch", () => {
    ...
    const result = normalizeMemoryContent({
        content: {
            ...
            longTermBackground: [],
            facts: [
                "plain string fact", // String fact to trigger normalizeFact string branch
    ...
    assert.ok(result.facts.some((f) => f.content === "plain string fact" && f.category === null));
    assert.ok(result.facts.some((f) => f.content === "object fact"));
});
...
//# sourceMappingURL=memory-schema.test.js.map