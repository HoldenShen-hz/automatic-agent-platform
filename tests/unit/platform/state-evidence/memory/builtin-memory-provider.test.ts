 ...
 */
function createMockFewShotExample(overrides: Partial<FewShotExample> = {}): FewShotExample {
  return {
...
function createMockStore(): AuthoritativeTaskStore {
  ...
  const experiences: Map<string, any> = new Map();
 ...
 */
function createMockMemoryService(store: AuthoritativeTaskStore): MemoryService {
  const memories: Map<string, MemoryRecord> = new Map();
  let nextId = 1;
