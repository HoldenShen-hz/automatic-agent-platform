...
import { computeKeywordSimilarity, computeToolOverlap, ExperienceCacheService, ExperienceCacheManager, } from "../../../../../src/platform/state-evidence/memory/experience-cache-service.js";
...
test("ExperienceCacheService.evictByCapacity keeps all when under limit", () => {
    ...
    const mockConnection = store.withConnection.bind(store);
    mockConnection((conn) => {
...
test("ExperienceCacheService.retrieveForFewShot includes matched keywords in reasoning", () => {
    ...
    if (result.examples.length > 0 && result.examples[0].reasoning) {
        ...
        assert.ok(result.examples[0].reasoning.includes("REST") ||
...
test("ExperienceCacheManager.removeService actually removes the service", () => {
    ...
    const serviceId1 = service1.constructor?.name;
    manager.removeService("session_1");
    const service2 = manager.getService("session_1", store);
    const serviceId2 = service2.constructor?.name;
...
test("ExperienceCacheManager.clearAll removes all services", () => {
    ...
    assert.equal(manager.services.size, 3);
    manager.clearAll();
...
//# sourceMappingURL=experience-cache-service-eviction.test.js.map