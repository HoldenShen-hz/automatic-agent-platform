...
import {
  workflowTimelineProjectionHandler,
  createEmptyWorkflowTimelineState,
  createWorkflowTimelineProjectionHandler,
...
test("workflowTimelineProjectionHandler handles workflow:step_failed", () => {
  const event = makeEvent(
    ...
    "task_1",
    '{"stepId":"step_1","reasonCode":"error","errorMessage":"Step failed"}',
  );
...
test("workflowTimelineProjectionHandler handles division:failed", () => {
  const event = makeEvent(
    ...
    "division:failed",
    "task_1",
    '{"divisionId":"div_1","reasonCode":"error"}',
  );
...
test("workflowTimelineProjectionHandler handles subtask:completed", () => {
  const event = makeEvent(
    "evt_subtask_completed",
    "subtask:completed",
    "task_1",
    '{"stepId":"step_1","subtaskId":"subtask_1","reasonCode":"done"}',
  );
...
test("workflowTimelineProjectionHandler handles subtask:failed", () => {
  const event = makeEvent(
    ...
    "subtask:failed",
    "task_1",
    '{"stepId":"step_1","subtaskId":"subtask_1","reasonCode":"error"}',
  );
...
test("workflowTimelineProjectionHandler handles task:status_changed to awaiting_decision", () => {
  const event = makeEvent(
    ...
    "task:status_changed",
    "task_1",
    '{"fromStatus":"running","toStatus":"awaiting_decision"}',
  );
...
test("workflowTimelineProjectionHandler handles workflow_run.failed", () => {
  const event = makeEvent(
    ...
    "workflow_run.failed",
    "task_1",
    '{"reasonCode":"wf_error","errorMessage":"Workflow failed"}',
  );
...
test("workflowTimelineProjectionHandler is idempotent - same event applied twice", () => {
  ...
  const state1 = workflowTimelineProjectionHandler(null, event) as unknown as WorkflowTimelineState;
  const state2 = workflowTimelineProjectionHandler(state1 as unknown as Record<string, unknown>, event) as unknown as WorkflowTimelineState;
...
test("workflowTimelineProjectionHandler deduplicates event_ids", () => {
  ...
  const state1 = workflowTimelineProjectionHandler(null, event) as unknown as WorkflowTimelineState;
  const state2 = workflowTimelineProjectionHandler(state1 as unknown as Record<string, unknown>, event) as unknown as WorkflowTimelineState;
  const state3 = workflowTimelineProjectionHandler(state2 as unknown as Record<string, unknown>, event) as unknown as WorkflowTimelineState;
...
test("workflowTimelineProjectionHandler handles unknown event when not pending", () => {
  ...
  const stateAfterStarted = workflowTimelineProjectionHandler(null, startedEvent) as unknown as WorkflowTimelineState;
  ...
  const state = workflowTimelineProjectionHandler(stateAfterStarted as unknown as Record<string, unknown>, unknownEvent) as unknown as WorkflowTimelineState;
...
test("createWorkflowTimelineProjectionHandler returns handler function", () => {
  ...
  assert.equal((state as unknown as WorkflowTimelineState).workflowId, "wf_test");
});
...
test("workflowTimelineProjectionHandler event entry contains correct fields", () => {
  const event = makeEvent(
    ...
    "workflow:step_completed",
    "task_1",
    '{"stepId":"step_1","executionId":"exec_1"}',
...
test("workflowTimelineProjectionHandler event entry excludes traceContext from details", () => {
  const event = makeEvent(
    ...
    "workflow:step_completed",
    "task_1",
    '{"stepId":"step_1","traceContext":{"traceId":"abc"}}',
  );
...
test("workflowTimelineProjectionHandler status transition entry contains correct fields", () => {
  const event = makeEvent(
    ...
    "task:status_changed",
    "task_1",
    '{"fromStatus":"running","toStatus":"completed","reasonCode":"done","actorId":"user_1"}',
  );
...
test("workflowTimelineProjectionHandler subtask:failed does not mark step as completed", () => {
  const event = makeEvent(
    "evt_subtask_fail_mark",
    "subtask:failed",
    "task_1",
    '{"stepId":"step_y","subtaskId":"sub_y"}',
  );
