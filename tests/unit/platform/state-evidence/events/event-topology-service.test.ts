...
import { EventTopologyService, type EventTopologyNode, type EventTopologyEdge } from "../../../../../src/platform/state-evidence/events/event-topology-service.js";

test("EventTopologyService", () => {
  const service = new EventTopologyService();

  test("listEntries returns all registered event entries", () => {
  ...
  test("listEntries assigns reliableAckRequired based on tier", () => {
    const entries = service.listEntries();
    for (const entry of entries) {
  ...
  test("buildGraph creates nodes and edges from entries", () => {
  ...
  test("buildGraph includes producer, event, and consumer nodes", () => {
  ...
  test("buildGraph creates emits edges from producer to event", () => {
  ...
  test("buildGraph creates consumes edges from event to consumer", () => {
    const { edges } = service.buildGraph();
  ...
  test("buildSummary returns counts for events, namespaces, producers, consumers", () => {
    ...
    assert.equal(summary.totalEvents, summary.namespaces.length + Object.values(summary.tierCounts).reduce((a, b) => a + b, 0) - summary.namespaces.length);
  });

  test("buildSummary tierCounts has tier_1, tier_2, tier_3 keys", () => {
    const summary = service.buildSummary();
  ...
  test("buildSummary namespaces are sorted and unique", () => {
    const summary = service.buildSummary();
    const sorted = [...summary.namespaces].sort();
    assert.deepEqual(summary.namespaces, sorted);
  });

  test("listNamespaceEntries filters by namespace", () => {
  ...
  test("listNamespaceEntries returns empty array for unknown namespace", () => {
