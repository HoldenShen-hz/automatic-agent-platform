...
function createEvent(overrides: Partial<ProjectionInputEvent> = {}): ProjectionInputEvent {
  return {
    eventId: "evt_test_1",
...
test("EventProjectionService.applyEvent stores parsed payload in state", () => {
  ...
  const lastPayload = projection.state.lastPayload as Record<string, unknown>;
