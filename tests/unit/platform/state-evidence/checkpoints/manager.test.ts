...
/**
 * Timestamp counter for ensuring unique timestamps across rapid checkpoint creation
 */
...
class MockCheckpointManager implements CheckpointManager {
  ...
  async createCheckpoint(input: CreateCheckpointInput): Promise<{ artifactId: string; checkpoint: WorkflowStepCheckpoint }> {
    timestampCounter++;
    const producedAt = new Date(BASE_TIME + timestampCounter).toISOString();
  ...
  async getWorkflowProgress(workflowId: string): Promise<WorkflowProgress | null> {
    ...
    let latestCheckpoint: WorkflowStepCheckpoint | null = null;
    ...
    // Use the nextStepId from the latest checkpoint directly
