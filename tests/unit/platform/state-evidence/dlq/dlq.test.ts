...
import {
  ...
  type DeadLetterQueueSummary,
  type DeadLetterQueueRepository,
  type DeadLetterQueueRetryWorkerResult,
...
test("DeadLetterQueueSummary structure", () => {
  const summary: DeadLetterQueueSummary = {
    totalRecords: 10,
...
test("InMemoryDeadLetterQueueRepository update modifies record", () => {
  ...
  repo.insert(record);
  const updated = { ...record, status: "resolved" as const, updatedAt: "2026-04-26T12:00:00Z" };
  repo.update(updated);
...
test("DeadLetterQueueService scheduleRetry validates delay", () => {
  ...
  assert.throws(
    () => service.scheduleRetry(record.deadLetterId, -1),
    (err: any) => err.code === "dlq.invalid_retry_delay",
  );
  assert.throws(
    () => service.scheduleRetry(record.deadLetterId, Infinity),
    (err: any) => err.code === "dlq.invalid_retry_delay",
  );
  assert.throws(
    () => service.scheduleRetry(record.deadLetterId, NaN),
    (err: any) => err.code === "dlq.invalid_retry_delay",
  );
});
...
test("DeadLetterQueueRetryWorker runDueRetries with no records", async () => {
  const service = new DeadLetterQueueService();
  const worker = new DeadLetterQueueRetryWorker(service);
  const result = await worker.runDueRetries(() => ({ outcome: "resolved" as const }));
