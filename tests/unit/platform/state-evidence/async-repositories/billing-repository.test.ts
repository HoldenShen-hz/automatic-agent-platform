...
test("AsyncBillingRepository incrementQuotaCounter increments and returns updated counter", async () => {
  const counter = quotaCounterRecord();
  const { connection, calls } = createConnection({
    executeResults: [1],
    queryOneRows: [counter],
  });
  const repo = new AsyncBillingRepository(connection);

  const result = await repo.incrementQuotaCounter(counter, 100);

  assert.equal(result.counterId, "quota-1");
  assert.equal(calls[0]!.method, "execute");
  assert.match(calls[0]!.sql, /INSERT INTO quota_counters/);
  assert.match(calls[0]!.sql, /ON CONFLICT.*DO UPDATE SET/);
  assert.match(calls[0]!.sql, /used_quantity = ROUND\(quota_counters\.used_quantity \+ excluded\.used_quantity/);
});

test("AsyncBillingRepository incrementQuotaCounter throws when record not found after upsert", async () => {
  const counter = quotaCounterRecord();
  ...
  await assert.rejects(
    async () => await repo.incrementQuotaCounter(counter, 100),
    /billing_repository\.increment_quota_counter_missing_after_upsert/,
  );
});

test("AsyncBillingRepository getQuotaCounter returns counter when found", async () => {
  const counter = quotaCounterRecord();
  const { connection, calls } = createConnection({ queryOneRows: [counter] });
  const repo = new AsyncBillingRepository(connection);

  const result = await repo.getQuotaCounter("acct-1", "api_calls", "2024-01-01T00:00:00Z", "2024-01-31T23:59:59Z");

  assert.deepEqual(result, counter);
  assert.match(calls[0]!.sql, /FROM quota_counters/);
  assert.match(calls[0]!.sql, /WHERE account_id = \$1/);
});

test("AsyncBillingRepository getQuotaCounter returns null when not found", async () => {
  const { connection } = createConnection({ queryOneRows: [undefined] });
  const repo = new AsyncBillingRepository(connection);

  const result = await repo.getQuotaCounter("acct-missing", "api_calls", "2024-01-01T00:00:00Z", "2024-01-31T23:59:59Z");
