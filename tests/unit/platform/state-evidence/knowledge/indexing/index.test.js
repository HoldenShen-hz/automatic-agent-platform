...
import { HashEmbeddingProvider, OpenAIEmbeddingProvider, MiniMaxEmbeddingProvider, createEmbeddingProviderFromEnv, EMBEDDING_PROVIDER_TYPES, } from "../../../../../../src/platform/state-evidence/knowledge/indexing/embedding-provider.js";
/* ============================================================
...
test("HashEmbeddingProvider rejects null-like inputs via semantic-embedding", async () => {
    const provider = new HashEmbeddingProvider();
    await assert.rejects(() => provider.embed(""), /empty_input|cannot build/i);
});
...
test("OpenAIEmbeddingProvider embedBatch splits into batches", async () => {
    ...
    await assert.rejects(() => provider.embedBatch(["a", "b", "c", "d"]), /fetch|network|failed/);
});
/* ============================================================
...
test("createEmbeddingProviderFromEnv throws for OpenAI without API key", () => {
    assert.throws(() => createEmbeddingProviderFromEnv({ AA_KNOWLEDGE_EMBEDDING_PROVIDER: "openai" }), /missing_api_key/i);
});
test("createEmbeddingProviderFromEnv throws for MiniMax without API key", () => {
    assert.throws(() => createEmbeddingProviderFromEnv({ AA_KNOWLEDGE_EMBEDDING_PROVIDER: "minimax" }), /missing_api_key/i);
});
...
test("createEmbeddingProviderFromEnv defaults to hash for unknown type", () => {
    const provider = createEmbeddingProviderFromEnv({
        AA_KNOWLEDGE_EMBEDDING_PROVIDER: "unknown-type",
    });
