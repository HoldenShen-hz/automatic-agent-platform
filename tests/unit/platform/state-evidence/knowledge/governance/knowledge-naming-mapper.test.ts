...
test("toDocTrustLevel and toCodeTrustLevel are inverse operations", () => {
  const codeLevels: CodeTrustLevel[] = ["trusted", "external", "untrusted"];
...
test("toCodeTrustLevel and toDocTrustLevel are inverse operations", () => {
  const docLevels: DocTrustLevel[] = ["verified", "unverified", "deprecated"];
...
test("toDocMemoryLayer and toCodeMemoryLayer are inverse operations", () => {
  const codeLayers: CodeMemoryLayer[] = ["layer_3", "layer_4", "layer_5"];
...
test("toCodeMemoryLayer and toDocMemoryLayer are inverse operations", () => {
  const docLayers: DocMemoryLayer[] = ["working", "episodic", "semantic"];
...
test("isTrustLevelAtOrAbove handles unknown strings as-is", () => {
  ...
  const result = isTrustLevelAtOrAbove("unknown_level" as CodeTrustLevel, "trusted");
...
test("isMemoryLayerAtOrAbove handles unknown strings as-is", () => {
  const result = isMemoryLayerAtOrAbove("unknown_layer" as CodeMemoryLayer, "layer_3");
  assert.equal(typeof result, "boolean");
});
