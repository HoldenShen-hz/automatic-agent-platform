...
// mock factories
// =============================================================================
function createMinimalSource(overrides = {}) {
    return {
        sourceId: "source_1",
        type: "text",
        uri: "file:///test/path",
...
function createMinimalDocument(overrides = {}) {
    return {
        documentId: "doc_1",
        sourceId: "source_1",
        title: "Test Document",
...
function createMinimalChunk(overrides = {}) {
    return {
        chunkId: "chunk_1",
        documentId: "doc_1",
...
function createMinimalRecord(overrides = {}) {
    return {
...
test("KnowledgeSnapshotStore rejects path traversal attempt with encoded dots", () => {
    assert.throws(() => new KnowledgeSnapshotStore({ snapshotPath: "/tmp/aa-sandbox/../etc/passwd" }), /path_traversal_denied/);
});
...
test("KnowledgeSnapshotStore rejects path with null bytes", () => {
    ...
    assert.doesNotThrow(() => new KnowledgeSnapshotStore({ snapshotPath: "/tmp/aa-sandbox/\0invalid" }));
});
//# sourceMappingURL=knowledge-snapshot-store.test.js.map