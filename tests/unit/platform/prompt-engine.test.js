...
import { PromptRolloutService, PROMPT_ROLLOUT_STAGES, comparePromptRolloutStage, nextPromptRolloutStage, } from "../../../src/platform/prompt-engine/rollout/index.js";
import { ConversationTemplateRegistry, ConversationTemplateExecutor, } from "../../../src/platform/prompt-engine/conversation-template-service.js";
...
test("PromptVersionManager respects max versions limit", () => {
    ...
    manager.registerBundleVersion(createMockPromptBundle("bundle", "v1.0"));
    manager.registerBundleVersion(createMockPromptBundle("bundle", "v1.1"));
    manager.registerBundleVersion(createMockPromptBundle("bundle", "v1.2"));
    manager.registerBundleVersion(createMockPromptBundle("bundle", "v1.3"));
...
test("HierarchicalPromptRegistryService validates registration input", () => {
    const service = new HierarchicalPromptRegistryService();
    assert.throws(() => service.registerBundle({ ...createBundleInput("bundle", "v1"), name: "" }, "global"));
    assert.throws(() => service.registerBundle({ ...createBundleInput("bundle", "v1"), version: "" }, "global"));
    assert.throws(() => service.registerBundle({ ...createBundleInput("bundle", "v1"), domain: "" }, "global"));
    assert.throws(() => service.registerBundle({ ...createBundleInput("bundle", "v1"), taskType: "" }, "global"));
    assert.throws(() => service.registerBundle({ ...createBundleInput("bundle", "v1"), systemPrompt: { content: "", templateVariables: [], channel: "system" } }, "global"));
});
...
test("HierarchicalPromptRegistryService resolves bundle for traffic with A/B testing", () => {
    ...
    service.registerBundle({ ...createBundleInput("bundle", "v1"), metadata: { trafficAllocation: { weight: 30 } } }, "global");
    service.registerBundle({ ...createBundleInput("bundle", "v2"), metadata: { trafficAllocation: { weight: 70 } } }, "global");
...
test("CrossProviderJudgeService getProviderDiversityScore calculates correctly", () => {
    ...
    const service = new CrossProviderJudgeService(judgeService);
    const judges = [
        { judgeId: "j1", provider: "openai", providerFamily: "openai", capabilities: [], maxCostUsd: 0.01, status: "ready", modelId: "gpt-4", createdAt: "", updatedAt: "" },
        { judgeId: "j2", provider: "anthropic", providerFamily: "anthropic", capabilities: [], maxCostUsd: 0.02, status: "ready", modelId: "claude-3", createdAt: "", updatedAt: "" },
        { judgeId: "j3", provider: "minimax", providerFamily: "minimax", capabilities: [], maxCostUsd: 0.01, status: "ready", modelId: "m1", createdAt: "", updatedAt: "" },
    ];
...
test("CrossProviderJudgeService getProviderDiversityScore handles single provider", () => {
    ...
    const judges = [
        { judgeId: "j1", provider: "openai", providerFamily: "openai", capabilities: [], maxCostUsd: 0.01, status: "ready", modelId: "gpt-4", createdAt: "", updatedAt: "" },
        { judgeId: "j2", provider: "openai", providerFamily: "openai", capabilities: [], maxCostUsd: 0.02, status: "ready", modelId: "gpt-4", createdAt: "", updatedAt: "" },
    ];
    const score = service.getProviderDiversityScore(judges);
...
test("PlatformPromptReleaseOrchestrationService throws for missing dataset", () => {
    ...
    const rollouts = new PromptRolloutService();
    const service = new PlatformPromptReleaseOrchestrationService(templates, datasets, rollouts);
    assert.throws(() => service.createRelease({
        template: {
            templateKey: "test_template",
            version: "v1",
            owner: "test@example.com",
...
test("QualityGateEvidenceService returns empty string when evidence disabled", () => {
    ...
    const service = new QualityGateEvidenceService({ artifactStore: mockArtifactStore, config });
    const evaluation = createMockEvaluation();
    const decision = { accepted: false, releaseStage: "blocked", reasonCodes: [] };
...
test("QualityGateEvidenceService persists evaluation when enabled", () => {
    ...
    const service = new QualityGateEvidenceService({ artifactStore: mockArtifactStore, config });
    const evaluation = createMockEvaluation();
    const decision = { accepted: true, releaseStage: "released", reasonCodes: ["quality.accepted"] };
...
// ── Helper Functions ──────────────────────────────────────────────────
function createMockPromptBundle(name, version) {
    return {
        ...
        taskType: "test_task",
        packId: "test_pack",
        systemPrompt: { content: "Test prompt", templateVariables: [], channel: "system" },
        userPrompt: { content: "User prompt", templateVariables: [], channel: "user" },
        fewShotExamples: [],
        constraints: {},
        metadata: {
            owner: "test",
...
function createBundleInput(name, version) {
    return {
        name,
        version,
        domain: "test_domain",
        taskType: "test_task",
        packId: "test_pack",
        systemPrompt: { content: "Test system prompt", templateVariables: [], channel: "system" },
        userPrompt: { content: "Test user prompt", templateVariables: [], channel: "user" },
        fewShotExamples: [],
