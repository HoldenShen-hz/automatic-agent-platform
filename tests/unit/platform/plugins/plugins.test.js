...
// ---------------------------------------------------------------------------
function createMockAssessment(complexity, risk, approvalRequired = false) {
    return {
        taskId: "task-1",
        intent: "test intent",
        assessment: {
...
test("GithubAdapter throws when not authenticated", async () => {
    const plugin = createGithubAdapterPlugin();
    await assert.rejects(async () => plugin.execute("create_issue", { repository: "owner/repo", title: "Test", body: "Body" }), /github_adapter.not_authenticated/);
});
test("GithubAdapter throws on missing repository", async () => {
    const plugin = createGithubAdapterPlugin();
    await plugin.authenticate({ token: "test" });
    await assert.rejects(async () => plugin.execute("create_issue", { title: "Test", body: "Body" }), /github_adapter.missing_repository/);
});
test("GithubAdapter throws on missing required fields", async () => {
    const plugin = createGithubAdapterPlugin();
    await plugin.authenticate({ token: "test" });
    await assert.rejects(async () => plugin.execute("create_issue", { repository: "owner/repo" }), /github_adapter.missing_title/);
});
...
test("GithubAdapter builds endpoint for create_pr_comment", async () => {
    ...
    assert.ok(result.endpoint.includes("/issues/123/comments"));
});
test("GithubAdapter builds endpoint for dispatch_workflow", async () => {
    ...
    assert.ok(result.endpoint.includes("/actions/workflows/ci.yml/dispatches"));
});
test("GithubAdapter builds endpoint for get_file", async () => {
    ...
    assert.ok(result.endpoint.includes("/contents/README.md"));
});
...
test("GithubAdapter clears credentials on shutdown", async () => {
    ...
    await plugin.authenticate({ token: "test" });
    await plugin.shutdown();
    await assert.rejects(async () => plugin.execute("create_issue", { repository: "r", title: "t", body: "b" }), /github_adapter.not_authenticated/);
});
...
test("CrmAdapter returns structured response with action", async () => {
    ...
    assert.equal(result.ok, true);
    assert.equal(result.data.action, "campaigns");
});
...
test("AssetProductionAdapter execute returns success response", async () => {
    ...
    assert.equal(result.output.nodeId, "node456");
});
...
test("LivestreamAdapter execute returns success response", async () => {
    ...
    assert.equal(result.success, true);
    assert.equal(result.output.streamId, "stream123");
});
...
test("CodingPresenter includes artifacts section", async () => {
    ...
    assert.ok(result.sections.some((s) => s.includes("file1.ts")));
});
...
test("GrowthPresenter formats campaign output", async () => {
    ...
    assert.ok(result.sections.some((s) => s.includes("Summer Sale")));
    assert.ok(result.sections.some((s) => s.includes("10000")));
...
test("GrowthPresenter formats abtest output", async () => {
    ...
    assert.ok(result.sections.some((s) => s.includes("Button Color Test")));
    assert.ok(result.sections.some((s) => s.includes("blue")));
...
test("GrowthPresenter formats generic output", async () => {
    ...
    assert.ok(result.sections.some((s) => s.includes("generic-1")));
    assert.ok(result.sections.some((s) => s.includes("custom")));
});
...
test("OperationsPresenter formats incident output", async () => {
    ...
    assert.ok(result.sections.some((s) => s.includes("CRITICAL")));
    assert.ok(result.sections.some((s) => s.includes("payment-service")));
...
test("OperationsPresenter formats runbook output", async () => {
    ...
    assert.ok(result.sections.some((s) => s.includes("Database Backup")));
    assert.ok(result.sections.some((s) => s.includes("1. Connect")));
    assert.ok(result.sections.some((s) => s.includes("3. Verify")));
});
...
test("OperationsPresenter handles empty steps array in runbook", async () => {
    ...
    assert.ok(result.sections.some((s) => s.includes("Empty Runbook")));
});
test("OperationsPresenter handles missing fields with defaults", async () => {
    ...
    assert.ok(result.sections.some((s) => s.includes("step1")));
});
...
test("OperationsRetriever returns different match types", async () => {
    ...
    const matchTypes = result.map((r) => r.matchType);
