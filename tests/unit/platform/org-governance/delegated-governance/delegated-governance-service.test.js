...
function mockDelegation(overrides = {}) {
    return {
...
function mockGuardrail(overrides = {}) {
    return {
...
test("DelegatedGovernanceService checkOperation allows permitted operation", () => {
    const service = new DelegatedGovernanceService([mockDelegation()]);
    const result = service.checkOperation({ orgNodeId: "org-1", domainId: "domain-1", actorRole: "division_admin" }, "approve_task");
    assert.strictEqual(result.allowed, true);
});
test("DelegatedGovernanceService checkOperation denies operation for wrong role", () => {
    const service = new DelegatedGovernanceService([mockDelegation()]);
    const result = service.checkOperation({ orgNodeId: "org-1", domainId: "domain-1", actorRole: "team_lead" }, "approve_budget_increase");
    assert.strictEqual(result.allowed, false);
...
test("DelegatedGovernanceService checkOperation evaluates guardrails", () => {
    ...
    const result = service.checkOperation({ orgNodeId: "org-1", domainId: "domain-1", actorRole: "division_admin" }, "approve_task", 1000);
    assert.strictEqual(result.allowed, false);
