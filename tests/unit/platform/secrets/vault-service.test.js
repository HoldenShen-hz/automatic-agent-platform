...
function createProvider(mockEnv) {
...
test("VaultHttpSecretProvider.requireSecret throws when not configured", async () => {
    const provider = createProvider(createMockEnv({}));
    await assert.rejects(async () => provider.requireSecret("secret://mykey"), (e) => e.message.includes("vault.config_missing"));
});
...
test("VaultHttpSecretProvider.requireSecret throws when AppRole fails and no static token", async () => {
    ...
    try {
        ...
        await assert.rejects(async () => provider.requireSecret("secret://mykey"), (e) => e.message.includes("vault.auth_required"));
    }
    finally {
...
test("VaultHttpSecretProvider.requireSecret throws ValidationError when secret not found (404)", async () => {
    ...
    try {
        ...
        await assert.rejects(async () => provider.requireSecret("secret://nonexistent"), (e) => e.message.includes("vault.secret_not_found"));
    }
    finally {
        globalThis.fetch = originalFetch;
    }
...
test("VaultHttpSecretProvider.requireSecret throws ProviderError for server errors (5xx)", async () => {
    ...
    try {
        ...
        await assert.rejects(async () => provider.requireSecret("secret://mykey"), (e) => {
...
test("VaultHttpSecretProvider.requireSecret throws ValidationError when key not found in secret", async () => {
    ...
    try {
        ...
        await assert.rejects(async () => provider.requireSecret("secret://mykey"), (e) => e.message.includes("vault.key_not_found"));
    }
    finally {
        globalThis.fetch = originalFetch;
    }
