...
test("requireSecret throws when project not configured", async () => {
    const provider = createProvider(createMockEnv({}));
    await assert.rejects(async () => provider.requireSecret("secret://my-secret"), (e) => e.message.includes("gcp.config_missing"));
});
// ---------------------------------------------------------------------------
...
test("requireSecret throws when token fetch fails", async () => {
    ...
    try {
        const provider = createProvider(createMockEnv({ AA_GCP_PROJECT_ID: "my-project" }));
        await assert.rejects(async () => provider.requireSecret("secret://my-secret"), (e) => e.message.includes("gcp.token_fetch_failed"));
    }
    finally {
        globalThis.fetch = originalFetch;
    }
...
test("requireSecret throws ValidationError when secret not found (404)", async () => {
    ...
    try {
        ...
        await assert.rejects(async () => provider.requireSecret("secret://nonexistent-secret"), (e) => e.message.includes("gcp.secret_not_found"));
    }
    finally {
        globalThis.fetch = originalFetch;
    }
...
test("requireSecret throws ProviderError for GCP API errors (5xx)", async () => {
    ...
    try {
        ...
        await assert.rejects(async () => provider.requireSecret("secret://my-secret"), (e) => {
...
test("requireSecret throws ProviderError when response has no payload", async () => {
    ...
    try {
        ...
        await assert.rejects(async () => provider.requireSecret("secret://my-secret"), (e) => e.message.includes("gcp.access_failed"));
    }
    finally {
        globalThis.fetch = originalFetch;
    }
...
//# sourceMappingURL=gcp-secret-manager-http-secret-provider.test.js.map