...
test("ExternalSecretProviderAdapter.describeSecret delegates to provider", async () => {
    ...
    assert.equal(result.resolved, true);
    assert.equal(result.value, undefined); // describeSecret doesn't return value
});
...
test("ExternalSecretProvider throws for invalid JSON in inline source", async () => {
    ...
    await assert.rejects(async () => provider.requireSecret("secret://mykey"), (e) => e.message.includes("secret.provider_config_invalid"));
});
