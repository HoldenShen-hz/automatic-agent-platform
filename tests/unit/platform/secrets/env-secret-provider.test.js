...
import { EnvSecretProvider, maskSecretValue, validateSecretRef, deriveSecretScope, deriveSecretEnvName, } from "../../../../src/platform/control-plane/iam/env-secret-provider.js";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
function createProvider(mockEnv = createMockEnv()) {
...
test("validateSecretRef throws for invalid format without secret://", () => {
    assert.throws(() => validateSecretRef("mykey"), (e) => e.message.includes("secret.invalid_ref"));
});
test("validateSecretRef throws for empty string", () => {
    assert.throws(() => validateSecretRef(""), (e) => e.message.includes("secret.invalid_ref"));
});
test("validateSecretRef throws for whitespace-only string", () => {
    assert.throws(() => validateSecretRef("   "), (e) => e.message.includes("secret.invalid_ref"));
});
test("validateSecretRef throws for ref with invalid characters", () => {
    assert.throws(() => validateSecretRef("secret://my key"), (e) => e.message.includes("secret.invalid_ref"));
});
// ---------------------------------------------------------------------------
// deriveSecretScope
// ---------------------------------------------------------------------------
...
test("requireSecret throws ValidationError when secret not found", async () => {
    const provider = createProvider(createMockEnv({}));
    await assert.rejects(async () => provider.requireSecret("secret://mykey"), (e) => e.message.includes("secret.missing_value"));
});
test("requireSecret throws for empty env value", async () => {
    const provider = createProvider(createMockEnv({ AA_SECRET_SIMPLE: "" }));
    await assert.rejects(async () => provider.requireSecret("secret://simple"), (e) => e.message.includes("secret.missing_value"));
});
// ---------------------------------------------------------------------------
...
test("requireSecretSync throws for missing secret", () => {
    const provider = createProvider(createMockEnv({}));
    assert.throws(() => provider.requireSecretSync("secret://mykey"), (e) => e.message.includes("secret.missing_value"));
});
test("requireSecretSync throws for empty env value", () => {
    const provider = createProvider(createMockEnv({ AA_SECRET_SIMPLE: "" }));
    assert.throws(() => provider.requireSecretSync("secret://simple"), (e) => e.message.includes("secret.missing_value"));
});
// ---------------------------------------------------------------------------
...
//# sourceMappingURL=env-secret-provider.test.js.map