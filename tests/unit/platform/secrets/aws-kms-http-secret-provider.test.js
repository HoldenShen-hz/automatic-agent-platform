...
test("requireSecret throws when key ID cannot be extracted and no ARN configured", async () => {
    ...
    await assert.rejects(async () => provider.requireSecret("secret://other/path"), (e) => e.message.includes("kms.key_required"));
});
test("requireSecret throws when ciphertext env var not configured", async () => {
    ...
    await assert.rejects(async () => provider.requireSecret("secret://kms/mykey"), (e) => e.message.includes("kms.ciphertext_not_configured"));
});
// ---------------------------------------------------------------------------
...
test("requireSecret throws ProviderError when KMS returns error", async () => {
    ...
    try {
        ...
        await assert.rejects(async () => provider.requireSecret("secret://kms/mykey"), (e) => {
...
test("requireSecret throws when KMS response has no Plaintext", async () => {
    ...
    try {
        ...
        await assert.rejects(async () => provider.requireSecret("secret://kms/mykey"), (e) => e.message.includes("kms.decrypt_failed"));
    }
    finally {
        globalThis.fetch = originalFetch;
    }
...
//# sourceMappingURL=aws-kms-http-secret-provider.test.js.map