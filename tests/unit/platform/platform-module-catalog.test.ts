...
test("each surface manifest entryModule starts with 'src/platform/'", () => {
  ...
  for (const manifest of manifests) {
    assert.ok(
      manifest.entryModule.startsWith("src/platform/"),
      `${manifest.surfaceId}: entryModule should start with 'src/platform/', got ${manifest.entryModule}`,
    );
  }
});
...
test("resolvePlatformSurfaceManifest throws Error for unknown surfaceId", () => {
  assert.throws(
    () => resolvePlatformSurfaceManifest("non-existent" as PlatformSurfaceId),
    { message: /Unknown platform surface: non-existent/ },
  );
});
...
test("all ring statuses are valid ArchitectureReadinessStatus values", () => {
  ...
  for (const ring of rings) {
    assert.ok(
      validStatuses.includes(ring.status),
      `${ring.ringId}: status '${ring.status}' should be one of ${validStatuses.join(", ")}`,
    );
  }
});
...
test("evidence module paths are valid source paths", () => {
  ...
  for (const ring of rings) {
    for (const module of ring.evidenceModules) {
      assert.ok(
        module.startsWith("src/") || module.startsWith("tests/"),
        `${ring.ringId}: evidence module '${module}' should start with 'src/' or 'tests/'`,
      );
  ...
  for (const ring of rings) {
    for (const test of ring.verificationTests) {
      assert.ok(
        ...
        `${ring.ringId}: verification test '${test}' should start with 'tests/'`,
      );
...
test("resolveArchitectureReadinessRing throws Error for unknown ringId", () => {
  assert.throws(
    () => resolveArchitectureReadinessRing("non-existent" as any),
    { message: /Unknown architecture readiness ring: non-existent/ },
  );
});
...
test("gateMeaning fields are descriptive and non-empty", () => {
  ...
  for (const ring of rings) {
    ...
    assert.ok(
      /\b(evidence|verified|implemented|production|modules|tests)\b/i.test(ring.gateMeaning),
      `${ring.ringId}: gateMeaning should mention key concepts like evidence, verified, implemented`,
    );
  }
});
