...
function createController(options?: {
  ...
  const ctorOptions: {
    ...
    primaryProvider: (options?.primaryProvider as UnifiedChatProvider) ?? (mockProvider as unknown as UnifiedChatProvider),
    fallbackProvider: options?.fallbackProvider ?? null,
    fallbackService: options?.fallbackService ?? (mockFallbackService as unknown as ModelGatewayFallbackService),
    cacheService: options?.cacheService ?? (mockCacheService as unknown as ModelGatewayCacheService<string>),
  };
...
test("DegradationController escalates and records reason on D0 failure", async () => {
  ...
  const controller = createController({
    primaryProvider: mockProvider as unknown as UnifiedChatProvider,
  });
...
test("DegradationController caches response at D0 when semanticKey provided", async () => {
  const mockCacheService = new MockCacheService();
  const controller = createController({
    cacheService: mockCacheService as unknown as ModelGatewayCacheService<string>,
  });
...
test("DegradationController route at D4 throws ProviderError", async () => {
  ...
  await assert.rejects(async () => controller.route(request), (error: unknown) => {
