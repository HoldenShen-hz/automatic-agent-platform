...
import {
  ...
  createAnthropicChatService,
  createAnthropicChatServiceFromEnvironment,
...
function createSuccessResponse(body: unknown, status = 200): Response {
...
test("AnthropicChatService handles tool definitions in request", async () => {
  ...
  const toolChoice = requestBody.tool_choice as { type: string };
  ...
  const service = new AnthropicChatService({
    ...
    fetchImpl: async (_input, init) => {
      ...
      if (h instanceof Headers) {
        ...
        capturedApiKey = (h as Record<string, string>)["x-api-key"] ?? null;
      }
      return createSuccessResponse(ANTHROPIC_RESPONSE);
    },
  });
...
test("AnthropicChatService handles streaming refusal", async () => {
  let finalRefusal: string | null = null;
...
test("createAnthropicChatService creates service with provided API key", async () => {
  ...
  (service as unknown as { credentialPool: ProviderCredentialPool }).credentialPool;
  ...
  const customService = new AnthropicChatService({
    ...
    fetchImpl: async (_input, init) => {
      const h = init?.headers;
      apiKeyHeader = h instanceof Headers ? h.get("x-api-key") : (h as Record<string, string>)?.["x-api-key"] ?? null;
      return createSuccessResponse(ANTHROPIC_RESPONSE);
    },
  });
