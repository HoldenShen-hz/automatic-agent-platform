...
import { AgentDefinitionSchema } from "../../../src/ops-maturity/agent-lifecycle/agent-registry/index.js";
...
test("R25-24 projection-update is exported from the standalone contract barrel and validates instances", () => {
  const contractsBarrel = readRepoFile("src/platform/contracts/index.ts");
  const source = readRepoFile("src/platform/contracts/projection-update/index.ts");

  assert.match(contractsBarrel, /export \* from "\.\/projection-update\/index\.js";/);
...
test("R25-25 prompt bundles expose factory and validation helpers", () => {
...
test("R25-28 handoff serializer aligns L1 semantics and L4 budgeted full handoff", () => {
  ...
  assert.deepEqual(full.historyRefs, ["history:1", "history:2"]);
});
...
test("R25-31 stable evidence bundle includes dispatch, worker-handshake, and worker-writeback rehearsals", () => {
...
test("R25-33 handoff model exports explicit four-level budgets", () => {
  assert.deepEqual(HANDOFF_LEVEL_TOKEN_BUDGET, {
    L1_context_summary: 200,
    L2_state_delta: 500,
    L3_facts_plan_delta: 2_000,
...
test("R25-35 mobile app is implemented with React Native primitives rather than React DOM tags", () => {
  const mobileAppSource = readRepoFile("ui/apps/mobile/src/App.tsx");

  assert.match(mobileAppSource, /from "react-native"/);
  assert.match(mobileAppSource, /<View/);
  assert.match(mobileAppSource, /<Text/);
  assert.equal(existsSync(resolve(process.cwd(), "ui/tests/unit/ui/apps/mobile/App.test.tsx")), true);
  assert.doesNotMatch(mobileAppSource, /<div|<span/);
});
