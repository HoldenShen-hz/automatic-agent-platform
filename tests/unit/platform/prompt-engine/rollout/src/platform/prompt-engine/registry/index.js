...
export class PromptTemplateRegistryService {
    ...
    registerTemplate(input) {
        ...
        const now = nowIso();
        const templateVersions = this.templates.get(templateKey) ?? new Map();
        if (templateVersions.has(version)) {
            throw new ValidationError(`prompt_template.version_conflict:${templateKey}:${version}`, `Prompt template ${templateKey}@${version} is already registered.`);
        }
        const record = {
            templateKey,
            version,
...
export function hashPromptPrefix(fixedPrefix) {
...
function dedupeVariableSpecs(specs) {
    const seen = new Set();
    const result = [];
