...
export const EVAL_RISK_LEVEL_MIN_SAMPLES = {
    critical: 200,
    high: 100,
    medium: 50,
    standard: 20,
...
export class EvalDatasetJudgeService {
    ...
    registerDataset(input) {
        ...
        for (const [priority, count] of casesByPriority) {
            ...
            if (count < minRequired) {
                throw new ValidationError(`eval_dataset.insufficient_samples:${priority}`, `Evaluation dataset must have at least ${minRequired} ${priority} priority cases, got ${count}.`);
            }
        }
        const record = {
            datasetId,
    ...
    buildHoldoutSplit(cases, holdoutRatio) {
