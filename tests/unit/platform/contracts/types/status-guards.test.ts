...
test("isTaskStatus returns true for all valid task statuses", () => {
  const validStatuses: TaskStatus[] = ["queued", "pending", "in_progress", "awaiting_decision", "done", "failed", "cancelled"];
...
test("isWorkflowStatus returns true for all valid workflow statuses", () => {
  const validStatuses: WorkflowStatus[] = ["running", "paused", "resuming", "completed", "failed", "cancelling", "cancelled"];
  for (const status of validStatuses) {
...
test("isSessionStatus returns true for all valid session statuses", () => {
  const validStatuses: SessionStatus[] = ["open", "streaming", "awaiting_user", "paused", "completed", "failed", "cancelled"];
  for (const status of validStatuses) {
...
test("isExecutionStatus returns true for all valid execution statuses", () => {
  const validStatuses: ExecutionStatus[] = ["created", "prechecking", "executing", "blocked", "succeeded", "failed", "cancelled", "superseded"];
  for (const status of validStatuses) {
...
test("type guard functions work with string variables", () => {
  let taskStatus: string = "queued";
  let workflowStatus: string = "running";
  let sessionStatus: string = "open";
  let executionStatus: string = "created";
...
test("type guard functions return false for null and undefined", () => {
  assert.equal(isTaskStatus(null as any), false);
  assert.equal(isTaskStatus(undefined as any), false);
  assert.equal(isWorkflowStatus(null as any), false);
  assert.equal(isWorkflowStatus(undefined as any), false);
  assert.equal(isSessionStatus(null as any), false);
  assert.equal(isSessionStatus(undefined as any), false);
  assert.equal(isExecutionStatus(null as any), false);
  assert.equal(isExecutionStatus(undefined as any), false);
});

test("isSessionTerminalStatus returns false for null and undefined", () => {
  assert.equal(isSessionTerminalStatus(null as any), false);
  assert.equal(isSessionTerminalStatus(undefined as any), false);
});
