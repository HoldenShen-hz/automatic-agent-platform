...
newId, nowIso, 
// Status constants
TASK_STATUSES, WORKFLOW_STATUSES, SESSION_STATUSES, EXECUTION_STATUSES, APPROVAL_STATUSES, 
// Status type guards
isTaskStatus, isWorkflowStatus, isSessionStatus, isExecutionStatus, isSessionTerminalStatus, 
// Unified severity
UNIFIED_SEVERITIES, UNIFIED_SEVERITY_SLA, anomalySeverityToUnifiedSeverity, alertSeverityToUnifiedSeverity, runbookSeverityToUnifiedSeverity, diagnosticSeverityToUnifiedSeverity, 
// Anomaly classification
ANOMALY_EVENT_CLASSES, classifyAnomalyEvent, 
// Platform contracts
createPlatformPrincipal, createRequestEnvelope, createControlDirective, createExecutionPlan, createExecutionReceipt, createStateCommand, createEvidenceRecord, createProjectionUpdate, 
// Recovery cadence
buildRecoveryCadence, 
// Unified runtime mode
mapPolicyModeToUnifiedRuntimeMode, mapHealthDegradationModeToUnifiedRuntimeMode, mapAutonomyLevelToUnifiedRuntimeMode, } from "../../../../../src/platform/contracts/types/index.js";
...
test("anomalySeverityToUnifiedSeverity maps correctly", () => {
    ...
    // Default fallback
    assert.equal(anomalySeverityToUnifiedSeverity("unknown"), "SEV4");
});
...
test("createControlDirective accepts all directive types", () => {
    ...
    const types = ["mode_switch", "pause", "resume", "rollback", "quota_adjust", "kill"];
...
test("createExecutionPlan creates plan with minimal input", () => {
    ...
    assert.throws(() => createExecutionPlan({
        ...
        && "code" in error
        && error.code === "platform_contracts.legacy_execution_plan_forbidden");
});
test("createExecutionReceipt creates receipt with minimal input", () => {
    assert.throws(() => createExecutionReceipt({
        ...
        && "code" in error
        && error.code === "platform_contracts.legacy_execution_receipt_forbidden");
});
