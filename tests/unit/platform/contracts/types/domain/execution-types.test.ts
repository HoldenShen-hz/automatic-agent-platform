...
test("ExecutionRecord structure is correct", () => {
  ...
  assert.equal(record.nodeRunId, "nrun_123");
});
...
test("ExecutionRecord allows failed status with error", () => {
  const record: ExecutionRecord = {
    ...
    timeoutMs: 60000,
    budgetUsdLimit: 0.1,
    budgetReservationId: "bresv_failed",
    budgetLedgerId: "bledger_failed",
    requiresApproval: 0,
    sandboxMode: null,
    allowedToolsJson: null,
    allowedPathsJson: null,
