...
test("[SYS-OBS-5.1] OAPEFLIR files do not use console.* directly", () => {
    ...
    for (const file of oapeflirFiles) {
        ...
        assert.equal(consoleMatches?.length ?? 0, 0, `${file} has ${consoleMatches?.length ?? 0} console.* calls — use StructuredLogger`);
    }
});
test("[SYS-OBS-5.1] CDC replication uses StructuredLogger", () => {
    ...
    assert.ok(!content.match(/console\.(log|warn|error)\(/), "cdc-replication-service must use StructuredLogger instead of console.*");
});
...
//# sourceMappingURL=no-console-in-runtime.test.js.map