...
import { CANONICAL_CONTRACT_NAMES, CONTRACT_JSON_SCHEMAS, EXECUTABLE_CONTRACT_NAMES, EXECUTABLE_CONTRACT_PACKAGE, canTruthConsumerConsume, createBudgetLedger, createBudgetReservation, createBudgetSettlement, createCompensationRecord, createConfirmedTaskSpec, createDecisionInputBundle, createArtifactVersionLockSet, createAttemptLineage, createGraphPatch, createHarnessDecision, createHarnessRun, createHumanResponsibilityRecord, createNodeAttempt, createNodeAttemptReceipt, createNodeRun, createOapeflirViewEvent, createPlanGraphBundle, createPlatformFactEvent, createPrincipalRef, createReconciliationRecord, createRequestEnvelopeFromConfirmedTask, createRunVersionLock, createSideEffectRecord, createTaskDraft, reserveBudgetHardCap, validateExecutableContract, } from "../../../../../src/platform/contracts/executable-contracts/index.js";
...
test("v4.3 registry lists all frozen canonical contracts", () => {
    assert.deepEqual([...CANONICAL_CONTRACT_NAMES], [
        "TaskDraft",
        "ConfirmedTaskSpec",
        "RequestEnvelope",
        "HarnessRun",
...
test("intake factories enforce confirmation before request envelope", () => {
    ...
    assert.throws(() => createConfirmedTaskSpec({
...
test("GraphPatch requires version advance and at least one operation", () => {
    assert.throws(() => createGraphPatch({
        harnessRunId: "run-1",
        baseGraphVersion: 2,
        newGraphVersion: 2,
        operations: [
...
test("budget and HITL factories enforce v4.3 safety defaults", () => {
    ...
    assert.throws(() => createHumanResponsibilityRecord({
...
test("budget hard-cap reservation uses version CAS and rejects over-reservation", () => {
    ...
    assert.throws(() => reserveBudgetHardCap({
        ledger: first.ledger,
        amount: 10,
        resourceKind: "tool",
        expiresAt: "2026-04-27T01:00:00.000Z",
