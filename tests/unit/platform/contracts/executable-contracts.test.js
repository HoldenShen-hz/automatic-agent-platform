...
import { createNodeAttemptReceipt, createNodeAttempt, createNodeRun, createHarnessRun, createPrincipalRef, createPlanGraphBundle, CANONICAL_CONTRACT_NAMES, LEGACY_CONTRACT_NAMES, validateExecutableContract, CONTRACT_SCHEMA_VERSION, } from "../../../../src/platform/contracts/executable-contracts/index.js";
...
test("validateExecutableContract rejects invalid NodeAttemptReceipt", () => {
    ...
    assert.throws(() => validateExecutableContract("NodeAttemptReceipt", invalidReceipt), ValidationError);
});
...
test("CANONICAL_CONTRACT_NAMES does not contain any legacy contract names", () => {
    for (const legacyName of LEGACY_CONTRACT_NAMES) {
        assert.equal(CANONICAL_CONTRACT_NAMES.includes(legacyName), false, `${legacyName} should not be in CANONICAL_CONTRACT_NAMES`);
    }
});
//# sourceMappingURL=executable-contracts.test.js.map