...
test("validateGoalDecomposition handles multiple tasks with valid dependencies", () => {
  const decomposition = makeGoalDecomposition({
    tasks: [
      {
        taskId: "task1",
    ...
    dependencyGraph: [{ fromTask: "task1", toTask: "task2", type: "blocks" }],
  });
...
test("validateGoalDecomposition reports invalid depends_on reference", () => {
  const decomposition = makeGoalDecomposition({
    tasks: [
      {
        taskId: "task1",
        ...
        estimatedDuration: "1h",
        estimatedCost: { estimatedCostUsd: 0.01, confidence: "default", sampleCount: 0, divisionId: null, basedOn: "default" },
        dependsOn: ["task2"], // task2 does not exist
      },
    ],
  });

  const findings = validateGoalDecomposition(decomposition);
  assert.ok(findings.some((f) => f.includes("invalid_depends_on") && f.includes("task2")));
});

test("validateGoalDecomposition allows valid confidence 0 and 1", () => {
  const task = {
    taskId: "task1",
    domainId: "d1",
    description: "Task 1",
  ...
  const result0 = validateGoalDecomposition(makeGoalDecomposition({ decompositionConfidence: 0, tasks: [task] }));
  const result1 = validateGoalDecomposition(makeGoalDecomposition({ decompositionConfidence: 1, tasks: [task] }));
