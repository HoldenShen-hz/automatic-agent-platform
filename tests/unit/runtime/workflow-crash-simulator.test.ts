...
test("workflow crash simulator throws only for the matching point and step", () => {
  ...
  assert.equal((thrown as InjectedWorkflowCrashError).point, "tool_completed");
  assert.equal((thrown as InjectedWorkflowCrashError).stepId, "draft_solution");
...
test("workflow crash simulator handles all crash points", () => {
  ...
  for (const point of points) {
    ...
    assert.equal((thrown as InjectedWorkflowCrashError).point, point);
  }
});
