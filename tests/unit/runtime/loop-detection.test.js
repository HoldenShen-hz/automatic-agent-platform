...
import { LoopDetectionState, normalizeToolInputForHash, hashToolCall, SequenceLoopDetector, createLoopDetectionMiddleware, } from "../../../src/platform/execution/execution-engine/loop-detection.js";
...
test("createLoopDetectionMiddleware warns on warning threshold", async () => {
    const config = { warnThreshold: 2, escalateThreshold: 4 };
...
test("createLoopDetectionMiddleware fails on escalated pattern", async () => {
    const config = { warnThreshold: 1, escalateThreshold: 3 };
    const { middleware, state } = createLoopDetectionMiddleware(config);
    state.recordToolCall("read", { path: "/foo" });
    state.recordToolCall("read", { path: "/foo" });
    state.recordToolCall("read", { path: "/foo" });
