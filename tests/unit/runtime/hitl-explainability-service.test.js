...
test("generateExplanation creates a decision explanation", () => {
    ...
    try {
        const service = new HITLExplainabilityService(h.store);
        const explanation = service.generateExplanation("task_123", "task_escalation", [
...
test("explainTaskEscalation generates escalation explanation", () => {
    ...
    try {
        const service = new HITLExplainabilityService(h.store);
        const explanation = service.explainTaskEscalation("task_456", { complexity: 9, duration: 600, expectedDuration: 300, errorRate: 25 }, { executionId: "exec_456" });
        assert.ok(explanation !== undefined);
...
test("explainApprovalRequired generates approval explanation", () => {
    ...
    try {
        const service = new HITLExplainabilityService(h.store);
        const explanation = service.explainApprovalRequired("task_789", { riskLevel: "high", policy: "sensitive_data_policy", classification: "confidential" }, { takeoverSessionId: "session_123" });
        assert.ok(explanation !== undefined);
...
test("getExplanation retrieves explanation by id", () => {
    ...
    try {
        const service = new HITLExplainabilityService(h.store);
        const created = service.generateExplanation("task_get", "anomaly_detected", [{ name: "metric", weight: 1.0, value: 999, reason: "Anomaly" }]);
...
test("disabled service does not store explanations", () => {
    ...
    try {
        ...
        const explanation = service.generateExplanation("task_disabled", "task_escalation", [{ name: "c", weight: 1, value: 1, reason: "x" }]);
