...
import { EffectBuilder, EffectScope, EffectBuffer, } from "../../../src/platform/execution/execution-engine/effect-buffer.js";
...
test("EffectBuilder requires execute function", () => {
    assert.throws(() => EffectBuilder.create("event_publish", "test").build(), /effect_builder.missing_execute/);
});
...
test("EffectScope addEffect throws when already committed", () => {
    ...
    scope.commit();
    assert.throws(() => scope.addEffect(EffectBuilder.create("event_publish", "e2").withExecute(async () => { }).build()), /effect_scope.already_committed/);
});
test("EffectScope addEffect throws when rolled back", () => {
    const scope = new EffectScope({ scopeId: "test" });
    scope.rollback();
    assert.throws(() => scope.addEffect(EffectBuilder.create("event_publish", "e1").withExecute(async () => { }).build()), /effect_scope.already_rolled_back/);
});
