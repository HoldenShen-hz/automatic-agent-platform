...
function createTestDbPath(name) {
...
test("runMultiStepOrchestration transitions task from queued to in_progress", async () => {
    ...
    try {
        ...
        assert.ok(terminalStates.includes(result.snapshot.task.status), `Task should be in terminal state, got ${result.snapshot.task.status}`);
    }
    finally {
        cleanupDb(dbPath);
    }
...
test("runMultiStepOrchestration task status updatedAt changes on transition", async () => {
    ...
    try {
        ...
        assert.ok(new Date(result.snapshot.task.updatedAt) >= new Date(result.snapshot.task.createdAt), "updatedAt should be >= createdAt");
    }
    finally {
        cleanupDb(dbPath);
    }
...
test("runMultiStepOrchestration workflow status transitions from running", async () => {
    ...
    try {
        ...
        assert.ok(validStatuses.includes(result.snapshot.workflow.status), `Workflow should be in valid state, got ${result.snapshot.workflow.status}`);
    }
    finally {
        cleanupDb(dbPath);
    }
...
test("runMultiStepOrchestration session status transitions from open", async () => {
    ...
    try {
        const result = await runMultiStepOrchestration(input);
        assert.ok(result.snapshot.session);
        assert.ok(["open", "streaming", "failed", "done", "cancelled", "awaiting_user", "completed"].includes(result.snapshot.session.status), `Session should be in valid status, got ${result.snapshot.session.status}`);
    }
    finally {
        cleanupDb(dbPath);
    }
...
//# sourceMappingURL=multi-step-orchestration-comprehensive.test.js.map