...
test("acquireLeadership uses DEFAULT_LEASE_TTL_MS when ttlMs not specified", () => {
    ...
    try {
        ...
        assert.ok(result.lease.ttlMs >= 15000 - 100 && result.lease.ttlMs <= 15000 + 100, `Expected TTL ~15000, got ${result.lease.ttlMs}`);
    }
    finally {
        h.db.close();
        cleanupPath(h.workspace);
...
test("acquireLeadership throws RuntimeError when node is not registered", () => {
    ...
    try {
        const service = new HaCoordinatorService(h.db);
        assert.throws(() => service.acquireLeadership({ nodeId: "nonexistent-node" }), (err) => err instanceof Error &&
...
//# sourceMappingURL=ha-coordinator-service.test.js.map
