...
test("resolveMultiStepToolPath throws for path outside workspace", () => {
    const rootPath = "/workspace/project";
    assert.throws(() => resolveMultiStepToolPath(rootPath, "../secret"), (err) => err.code.startsWith("tool.path_outside_workspace"));
});
test("resolveMultiStepToolPath throws for absolute path outside workspace", () => {
    const rootPath = "/workspace/project";
    assert.throws(() => resolveMultiStepToolPath(rootPath, "/etc/passwd"), (err) => err.code.startsWith("tool.path_outside_workspace"));
});
