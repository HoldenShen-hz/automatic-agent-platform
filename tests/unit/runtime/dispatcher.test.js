...
import { executeMultiStepToolCallForTests, resetMultiStepToolRegistryForTests, } from "../../../src/platform/execution/dispatcher/index.js";
test.afterEach(() => {
...
test("executeToolCall wait-agent with unknown agent returns error", async () => {
    const result = await executeMultiStepToolCallForTests("wait-agent", JSON.stringify({ agentId: "nonexistent-agent" }));
    const parsed = JSON.parse(result);
    assert.equal(parsed.success, false);
    assert.equal(parsed.errorCode, "AGENT_NOT_FOUND");
});
test("executeToolCall send-message without agentId returns error", async () => {
    const result = await executeMultiStepToolCallForTests("send-message", JSON.stringify({ message: "hello" }));
    const parsed = JSON.parse(result);
    assert.equal(parsed.success, false);
    assert.equal(parsed.errorCode, "MISSING_ARGS");
});
test("executeToolCall send-message without message returns error", async () => {
    const result = await executeMultiStepToolCallForTests("send-message", JSON.stringify({ agentId: "agent-1" }));
    const parsed = JSON.parse(result);
    assert.equal(parsed.success, false);
    assert.equal(parsed.errorCode, "MISSING_ARGS");
});
...
test("executeToolCall batch-tool with empty toolCalls array returns error", async () => {
    const result = await executeMultiStepToolCallForTests("batch-tool", JSON.stringify({ toolCalls: [] }));
    const parsed = JSON.parse(result);
    assert.equal(parsed.success, false);
    assert.equal(parsed.errorCode, "MISSING_TOOL_CALLS");
});
...
test("executeToolCall unknown tool returns error", async () => {
    const result = await executeMultiStepToolCallForTests("nonexistent-tool", JSON.stringify({}));
    const parsed = JSON.parse(result);
    assert.equal(parsed.success, false);
    assert.equal(parsed.errorCode, "UNKNOWN_TOOL");
});
test("executeToolCall web_search with query formats result correctly", async () => {
    const result = await executeMultiStepToolCallForTests("web_search", JSON.stringify({ query: "test search" }));
    const parsed = JSON.parse(result);
...
test("executeToolCall web_fetch with valid url formats result correctly", async () => {
    const result = await executeMultiStepToolCallForTests("web_fetch", JSON.stringify({ url: "https://example.com" }));
    const parsed = JSON.parse(result);
    assert.ok(parsed.hasOwnProperty("success"));
});
...
test("executeToolCall wait-agent retrieves existing agent", async () => {
    ...
    const result = await executeMultiStepToolCallForTests("wait-agent", JSON.stringify({ agentId: "wait-test-agent" }));
    const parsed = JSON.parse(result);
    assert.equal(parsed.success, true);
...
test("executeToolCall git status command", async () => {
    const result = await executeMultiStepToolCallForTests("git", JSON.stringify({ args: ["status"] }));
    const parsed = JSON.parse(result);
    assert.ok(parsed.hasOwnProperty("success"));
...
test("executeToolCall git catches path_outside_workspace error from resolveMultiStepToolPath", async () => {
    ...
    const result = await executeMultiStepToolCallForTests("git", JSON.stringify({ args: ["status"], cwd: "/tmp/nonexistent_path_12345" }));
    const parsed = JSON.parse(result);
    assert.equal(parsed.success, false);
...
test("executeToolCall repo-map catches path_outside_workspace error from resolveMultiStepToolPath", async () => {
    ...
    const result = await executeMultiStepToolCallForTests("repo-map", JSON.stringify({ query: "test", rootPath: "/tmp/nonexistent_path_12345" }));
    const parsed = JSON.parse(result);
    assert.equal(parsed.success, false);
