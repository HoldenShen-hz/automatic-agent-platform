...
function seedTaskAndExecution(store, db, input) {
    const now = nowIso();
    db.transaction(() => {
        store.insertTask({
            id: input.taskId,
...
function registerWorker(workerRegistry, input) {
    ...
    const defaultStreamOffset = isRemote && input.lastAcknowledgedStreamOffset == null
        ? "stream:0"
...
test("dispatchNext blocks when no worker meets required capabilities", () => {
    ...
    try {
        ...
        assert.ok(decision.trace);
        assert.ok(decision.trace?.evaluations.some((e) => e.workerId === "worker-bash-only" && e.rejectionReason === "missing_capabilities"));
    }
    finally {
        harness.close();
    }
...
test("dispatchNext rejects unavailable workers", () => {
    ...
    try {
        ...
        const decision = harness.service.dispatchNext({ leaseTtlMs: 30000 });
        assert.equal(decision.outcome, "no_worker");
        assert.ok(decision.trace?.evaluations.some((e) => e.workerId === "worker-offline" && e.rejectionReason === "worker_offline"));
    }
    finally {
        harness.close();
    }
...
test("dispatchNext filters degraded workers by default", () => {
    ...
    try {
        ...
        const decision = harness.service.dispatchNext({ leaseTtlMs: 30000 });
        assert.equal(decision.outcome, "no_worker");
        assert.ok(decision.trace?.evaluations.some((e) => e.workerId === "worker-degraded" && e.rejectionReason === "worker_degraded_filtered"));
    }
    finally {
        harness.close();
    }
...
test("dispatchNext rejects workers at capacity", () => {
    ...
    try {
        ...
        const decision = harness.service.dispatchNext({ leaseTtlMs: 30000 });
        assert.equal(decision.outcome, "no_worker");
        assert.ok(decision.trace?.evaluations.some((e) => e.workerId === "worker-full" && e.rejectionReason === "worker_capacity_full"));
    }
    finally {
        harness.close();
    }
...
test("dispatchNext blocks on backpressure with read_only_operations_only", () => {
    ...
    try {
        ...
        registerWorker(harness.workerRegistry, { workerId: "worker-1" });
        harness.service.createTicket({ executionId });
        const serviceWithBackpressure = new ExecutionDispatchService(harness.db, harness.store, () => ({
            status: "unhealthy",
...
test("dispatchNext blocks on queue unavailability", () => {
    ...
    try {
        ...
        registerWorker(harness.workerRegistry, { workerId: "worker-1" });
        harness.service.createTicket({ executionId });
        const serviceWithQueueUnavailable = new ExecutionDispatchService(harness.db, harness.store, null, () => ({ state: "unavailable", reasonCode: "queue_shutdown" }));
...
test("dispatchNext filters out quarantined workers", () => {
    ...
    try {
        ...
        const decision = harness.service.dispatchNext({ leaseTtlMs: 30000 });
        assert.equal(decision.outcome, "no_worker");
        assert.ok(decision.trace?.evaluations.some((e) => e.workerId === "worker-quarantined" && e.rejectionReason === "worker_quarantined"));
    }
    finally {
        harness.close();
    }
...
test("dispatchNext filters out draining workers", () => {
    ...
    try {
        ...
        const decision = harness.service.dispatchNext({ leaseTtlMs: 30000 });
        assert.equal(decision.outcome, "no_worker");
        assert.ok(decision.trace?.evaluations.some((e) => e.workerId === "worker-draining" && e.rejectionReason === "worker_draining"));
    }
    finally {
        harness.close();
    }
...
test("dispatchNext rejects workers with mismatched queue affinity", () => {
    ...
    try {
        ...
        const decision = harness.service.dispatchNext({ leaseTtlMs: 30000 });
        assert.equal(decision.outcome, "no_worker");
        assert.ok(decision.trace?.evaluations.some((e) => e.workerId === "worker-mismatch" && e.rejectionReason === "queue_affinity_mismatch"));
    }
    finally {
        harness.close();
    }
