...
test("getDefaultBudgetPolicy returns expected policy", () => {
...
test("estimateLlmCallCost calculates correctly for known models", () => {
...
test("estimateLlmCallCost uses default rate for unknown models", () => {
...
test("estimateCostFromUsage sums prompt and completion tokens", () => {
  assert.equal(estimateCostFromUsage(100, 200, "MiniMax-M2.7"), estimateLlmCallCost(300, "MiniMax-M2.7"));
});

test("estimateCostFromUsage handles negative tokens by treating as 0", () => {
...
test("estimateActualLlmCallCost calculates cost from usage", () => {
  const mockResult = {
    id: "test",
    content: "response",
    refusal: null,
...
test("buildModelGovernanceKey formats correctly", () => {
...
test("readTrimmedEnvValue returns null for whitespace-only string", () => {
  assert.equal(readTrimmedEnvValue("   "), null);
  assert.equal(readTrimmedEnvValue("\t"), null);
  assert.equal(readTrimmedEnvValue("\n"), null);
});

test("readTrimmedEnvValue trims and returns non-empty string", () => {
...
test("parsePositiveInteger returns null for negative numbers", () => {
  assert.equal(parsePositiveInteger("-1"), null);
  assert.equal(parsePositiveInteger("-100"), null);
});

test("parsePositiveInteger returns parsed integer for valid positive numbers", () => {
...
test("parsePositiveInteger returns null for floats", () => {
...
test("parseNonNegativeInteger returns null for negative numbers", () => {
  assert.equal(parseNonNegativeInteger("-1"), null);
});

test("parseNonNegativeInteger returns 0 for zero", () => {
  assert.equal(parseNonNegativeInteger("0"), 0);
});

test("parseNonNegativeInteger returns parsed integer for valid non-negative numbers", () => {
  assert.equal(parseNonNegativeInteger("0"), 0);
...
test("parseBoolean returns true for truthy string values", () => {
...
test("parseBoolean returns false for falsy string values", () => {
...
test("parseBoolean returns null for non-boolean strings", () => {
  assert.equal(parseBoolean("maybe"), null);
  assert.equal(parseBoolean("yep"), null);
  assert.equal(parseBoolean("nope"), null);
});

test("isRetryableProviderError returns true for non-ProviderError", () => {
...
test("isRetryableProviderError returns retryable flag from ProviderError", () => {
...
test("isRetryableProviderError handles objects with retryable property", () => {
