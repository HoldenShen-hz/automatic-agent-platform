...
test("LoopDetectionState wouldEscalate checks threshold correctly", () => {
  const state = new LoopDetectionState({ warnThreshold: 2, escalateThreshold: 3 });
...
test("LoopDetectionState getPatterns returns all recorded patterns", () => {
...
test("LoopDetectionState removePattern deletes specific pattern", () => {
  const state = new LoopDetectionState();
...
test("normalizeToolInputForHash handles null/undefined", () => {
...
test("normalizeToolInputForHash handles objects with sorted keys", () => {
  const obj1 = { b: 2, a: 1 };
  const obj2 = { a: 1, b: 2 };
...
test("hashToolCall different inputs produce different hashes", () => {
