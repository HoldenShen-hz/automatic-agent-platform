 ...
 */
test("distributed-lock-service re-exports lock types via LockDataSchema [distributed-lock-service]", async () => {
...
test("distributed-lock-service re-exports transitionLock [distributed-lock-service]", async () => {
  const mod = await import("../../../src/core/runtime/distributed-lock-service.js");
  assert.ok("transitionLock" in mod, "should export transitionLock");
});

test("distributed-lock-service re-exports locking support via lockLogger [distributed-lock-service]", async () => {
  const mod = await import("../../../src/core/runtime/distributed-lock-service.js");
  assert.ok("lockLogger" in mod, "should export lockLogger");
});

test("distributed-lock-service re-exports SqliteLockAdapter [distributed-lock-service]", async () => {
  const mod = await import("../../../src/core/runtime/distributed-lock-service.js");
  assert.ok("SqliteLockAdapter" in mod || "sqliteLockAdapter" in mod, "should export SqliteLockAdapter");
});

test("distributed-lock-service re-exports PgAdvisoryLockAdapter [distributed-lock-service]", async () => {
  const mod = await import("../../../src/core/runtime/distributed-lock-service.js");
  assert.ok("PgAdvisoryLockAdapter" in mod || "pgAdvisoryLockAdapter" in mod, "should export PgAdvisoryLockAdapter");
});

test("distributed-lock-service re-exports RedisLockAdapter [distributed-lock-service]", async () => {
  const mod = await import("../../../src/core/runtime/distributed-lock-service.js");
  assert.ok("RedisLockAdapter" in mod || "redisLockAdapter" in mod, "should export RedisLockAdapter");
});

test("distributed-lock-service re-exports createLockAdapter factory [distributed-lock-service]", async () => {
  const mod = await import("../../../src/core/runtime/distributed-lock-service.js");
  assert.ok("createLockAdapter" in mod, "should export createLockAdapter");
});

test("distributed-lock-service exports are functions or objects [distributed-lock-service]", async () => {
  const mod = await import("../../../src/core/runtime/distributed-lock-service.js");
