...
test("[SYS-DEPLOY-6.1] terraform main.tf has remote backend configured", () => {
    ...
    assert.ok(hasBackendBlock, "main.tf must contain a backend block for remote state. Defect: no backend block found. This is critical for team collaboration and state locking.");
    assert.ok(!hasLocalBackend, "Backend must not be local. Found local backend configuration. Use S3, GCS, or Azure storage for remote state.");
    ...
    assert.ok(hasS3Backend || hasGcsBackend || hasAzBackend, "Must use S3, GCS, or Azure backend for remote state storage. Current config lacks remote backend configuration.");
});
test("[SYS-DEPLOY-6.1] terraform backend has required configuration", () => {
    ...
    if (content.includes("backend ")) {
        // For S3 backend
        if (content.includes('backend "s3"') || content.toLowerCase().includes("s3")) {
            assert.ok(content.includes("bucket") || content.toLowerCase().includes("s3_bucket"), "S3 backend must specify bucket");
            assert.ok(content.includes("region") || content.toLowerCase().includes("aws_region"), "S3 backend must specify region");
        }
        // For GCS backend
        if (content.includes('backend "gcs"') || content.toLowerCase().includes("gcs")) {
            assert.ok(content.includes("bucket") || content.toLowerCase().includes("gcs_bucket"), "GCS backend must specify bucket");
        }
    }
});
test("[SYS-DEPLOY-6.1] terraform state file not stored in git", () => {
    ...
    if (hasLocalBackend) {
        ...
        assert.ok(content.includes(".gitignore") || content.includes("terraform.tfstate"), "Local state file should be in .gitignore to prevent accidental commit");
    }
});
test("[SYS-DEPLOY-6.1] terraform required providers are defined", () => {
    ...
    assert.ok(content.includes("required_providers"), "main.tf must define required_providers block");
    // Verify AWS provider is configured
    assert.ok(content.includes("hashicorp/aws") || content.includes("registry.terraform.io/hashicorp/aws"), "AWS provider must be configured");
});
