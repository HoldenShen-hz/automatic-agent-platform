...
import { requiredEnv, optionalEnv, optionalNumber, optionalEnumValue, requiredEnumValue, parseStringArrayJson, parseObjectJson, parseBoolean, parseInteger, parseStringArrayFromCsv, parseCompatibilityJson, missingEnv, invalidEnv, } from "../../../src/platform/control-plane/config-center/remaining-cli-env-support.js";
...
test("parseCompatibilityJson throws on missing fields", () => {
    assert.throws(() => parseCompatibilityJson({ TEST_VAR: JSON.stringify({ apiContract: "v1" }) }, "TEST_VAR"), /invalid_env:TEST_VAR/);
});
