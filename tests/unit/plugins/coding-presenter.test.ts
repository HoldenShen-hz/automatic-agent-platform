...
test("formatOutput includes outputRef when present", async () => {
  ...
  const result = await plugin.formatOutput({
    ...
    artifacts: ["artifact:abc123"] as const,
    audience: "reviewer",
  });

  assert.ok(result.sections.some((s: string) => s.includes("outputRef: artifact:abc123")));
});
...
test("formatOutput includes artifacts section when artifacts present", async () => {
  ...
  const result = await plugin.formatOutput({
    ...
    artifacts: ["artifact:xyz", "artifact:abc"] as const,
    audience: "operator",
  });

  assert.ok(result.sections.some((s: string) => s.includes("Artifacts")));
  assert.ok(result.sections.some((s: string) => s.includes("artifact:xyz")));
  assert.ok(result.sections.some((s: string) => s.includes("artifact:abc")));
});
...
test("formatOutput formats payload as JSON code block", async () => {
  ...
  assert.ok(result.sections.some((s: string) => s.includes("```json")));
  assert.ok(result.sections.some((s: string) => s.includes('"files"')));
  assert.ok(result.sections.some((s: string) => s.includes('"count"')));
});

test("formatOutput citations contain all artifact refs", async () => {
  const plugin = createCodingPresenterPlugin();
  const artifacts = ["artifact:ref1", "artifact:ref2", "artifact:ref3"] as const;
