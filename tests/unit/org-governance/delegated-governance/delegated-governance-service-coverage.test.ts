...
} from "../../../../src/org-governance/delegated-governance/index.js";

function createMockDelegation(overrides: Partial<GovernanceDelegation> = {}): GovernanceDelegation {
  return {
...
function createMockScope(overrides: Partial<GovernanceActionScope> = {}): GovernanceActionScope {
  return {
...
function createMockContext(overrides: Partial<GovernanceOperationContext> = {}): GovernanceOperationContext {
  return {
    actorId: "actor-1",
    actorRole: "team_lead",
    orgNodeId: "org-node-1",
...
test("DelegatedGovernanceService checkOperation allows role permitted by role guardrail", () => {
  ...
  const result = service.checkOperation(
    createMockContext({ actorRole: "division_admin" }),
    "domain_onboarding",
  );
...
test("DelegatedGovernanceService getApplicableGuardrails filters by org node", () => {
  const service = new DelegatedGovernanceService([
    createMockDelegation({
      ...
      domainIds: [],
      guardrails: [{ guardrailId: "specific", type: "max_budget" as const, value: 1000 }],
    }),
  ]);
...
test("DelegatedGovernanceService getApplicableGuardrails filters by domain", () => {
  const service = new DelegatedGovernanceService([
    createMockDelegation({
      ...
      guardrails: [{ guardrailId: "domain-specific", type: "max_budget" as const, value: 1000 }],
    }),
  ]);
...
test("DelegatedGovernanceService getApplicableGuardrails ignores inactive delegations", () => {
  const service = new DelegatedGovernanceService([
    createMockDelegation({
      status: "inactive",
      guardrails: [{ guardrailId: "inactive", type: "max_budget" as const, value: 1000 }],
    }),
  ]);
...
test("DelegatedGovernanceService validateInheritanceRule denies unknown action", () => {
  ...
  const result = service.validateInheritanceRule("team_lead", "team_lead", "unknown" as "tighten");
...
test("DelegatedGovernanceService checkOperation evaluates guardrails for platform_team", () => {
  ...
  // Current service requires an attemptedValue whenever guardrails exist.
  assert.equal(result.allowed, false);
