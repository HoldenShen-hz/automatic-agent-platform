...
test("resolveGovernanceCapabilityBaseline throws for invalid capabilityId", () => {
    assert.throws(() => {
        OrgGovernanceExports.resolveGovernanceCapabilityBaseline("invalid-capability");
    });
});
...
test("all exported capability IDs are valid", () => {
    ...
    for (const id of capabilityIds) {
        const baseline = OrgGovernanceExports.resolveGovernanceCapabilityBaseline(id);
        assert.ok(baseline, `Should resolve capabilityId: ${id}`);
    }
});
//# sourceMappingURL=index-export.test.js.map