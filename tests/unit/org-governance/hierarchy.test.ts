...
test("hierarchy buildOrgChangeImpactArtifacts freezes reroute and deprovision outputs", () => {
  const artifacts = buildOrgChangeImpactArtifacts(hierarchy, [
    { orgNodeId: "team", nodeType: "team" as const, displayName: "Runtime", parentOrgNodeId: "missing", ownerUserIds: ["lead-2"], active: true, costCenter: "", metadata: {}, legalEntityBoundary: { boundaryId: "le-2", legalEntityId: "entity-2", jurisdictionCountry: "US", dataResidencyRegion: "us-east", crossBorderTransferPolicy: "approval_required" as const, crossEntityApprovalRoles: ["legal_reviewer"], restrictedDataClasses: [] } },
  ], [
    { principalId: "seat-1", userId: "engineer", homeNodeId: "team", managerUserId: "lead", active: true },
  ]);
