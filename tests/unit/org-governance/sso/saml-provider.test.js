...
import { SamlService, SamlProviderConfigSchema, buildSamlAudience, validateXmlSignature, } from "../../../../src/org-governance/sso-scim/saml/index.js";
...
test("SamlService rejects assertion with wrong issuer", () => {
    ...
    assert.throws(() => service.consumeAssertion("test-idp", assertion, now), /saml\.invalid_issuer/);
});
test("SamlService rejects assertion with wrong fingerprint", () => {
    ...
    assert.throws(() => service.consumeAssertion("test-idp", assertion, now), /saml\.invalid_fingerprint/);
});
test("SamlService rejects assertion with invalid audience", () => {
    ...
    assert.throws(() => service.consumeAssertion("test-idp", assertion, now), /saml\.invalid_audience/);
});
test("SamlService rejects assertion with empty subject", () => {
    ...
    assert.throws(() => service.consumeAssertion("test-idp", assertion, now), /saml\.invalid_subject/);
});
test("SamlService rejects expired assertion", () => {
    ...
    assert.throws(() => service.consumeAssertion("test-idp", assertion, now), /saml\.assertion_expired/);
});
...
test("SamlService rejects replayed assertion", () => {
    ...
    assert.throws(() => service.consumeAssertion("test-idp", assertion, now), /saml\.assertion_replayed/);
});
test("SamlService rejects assertion with wrong recipient", () => {
    ...
    assert.throws(() => service.consumeAssertion("test-idp", assertion, now), /saml\.invalid_recipient/);
});
test("SamlService requires signature when allowUnsignedAssertions is false", () => {
    ...
    assert.throws(() => service.consumeAssertion("test-idp", assertion, now), /saml\.signature_required/);
});
...
test("SamlService throws for missing provider on login", () => {
    const service = new SamlService();
    assert.throws(() => service.buildLoginRequest("non-existent"), /saml\.provider_not_found/);
});
test("SamlService throws for missing provider on logout", () => {
    const service = new SamlService();
    assert.throws(() => service.buildLogoutRequest("non-existent", {
        sessionId: "session-123",
        subjectId: "user@example.com",
        sessionIndex: null,
...
test("SamlService throws for missing provider on consume", () => {
    ...
    assert.throws(() => service.consumeAssertion("non-existent", assertion, now), /saml\.provider_not_found/);
});
...
test("validateXmlSignature returns invalid for malformed signature", () => {
    const result = validateXmlSignature("<invalid>signature</invalid>", "<xml>document</xml>");
    assert.equal(result.valid, false);
    assert.ok(result.error);
});
test("validateXmlSignature accepts custom key provider function", () => {
    const result = validateXmlSignature("<signature>test</signature>", "<xml>doc</xml>", {
