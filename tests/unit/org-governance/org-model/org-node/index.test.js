...
import { OrgNodeTypeSchema, OrgNodeSchema, isLeafOrgNode, getPlatformMapping, validateHierarchyDepth, createCrossOrgCollaborator, getLegalEntityApprovalRoles, requiresLegalEntityApproval, } from "../../../../../src/org-governance/org-model/org-node/index.js";
...
test("isLeafOrgNode returns true for team node", () => {
    const teamNode = OrgNodeSchema.parse({
        orgNodeId: "team-1",
        nodeType: "team",
        displayName: "Engineering",
...
test("requiresLegalEntityApproval detects cross-entity access", () => {
    ...
    assert.deepEqual(getLegalEntityApprovalRoles({
        ...source,
        crossBorderTransferPolicy: "approval_required",
        crossEntityApprovalRoles: ["legal_reviewer"],
        restrictedDataClasses: [],
        ...
        crossEntityApprovalRoles: ["compliance_officer"],
        restrictedDataClasses: [],
    }).sort(), ["compliance_officer", "legal_reviewer"]);
});
