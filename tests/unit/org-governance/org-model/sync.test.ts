...
import type { OrgSyncRecord } from "../../../../src/org-governance/org-model/sync/index.js";
...
function createOrgChart(nodes: OrgNode[], syncSource: OrgChart["syncSource"] = "manual"): OrgChart {
...
function createOrgChartWithRoot(root: OrgNode, nodes: OrgNode[], syncSource: OrgChart["syncSource"] = "manual"): OrgChart {
  return {
    root,
    nodes,
    syncSource,
...
test("buildOrgChart throws error when no root node found", () => {
  ...
  assert.throws(
    ...
    /Cannot build OrgChart: no root node found/,
  );
});

test("buildOrgChart throws error for empty nodes array", () => {
  assert.throws(
    () => buildOrgChart([], "hr_api"),
    /Cannot build OrgChart: no root node found/,
  );
});
