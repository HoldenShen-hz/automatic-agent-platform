...
import { SamlService, validateXmlSignature, SAML_SIGNATURE_ALGORITHMS, buildSamlAudience, SamlProviderConfigSchema, } from "../../../../src/org-governance/sso-scim/saml/index.js";
...
test("validateXmlSignature uses custom signature algorithm when provided", () => {
    const result = validateXmlSignature("<!-- mock -->", "<xml>test</xml>", { signatureAlgorithm: "http://www.w3.org/2000/09/xmldsig#rsa-sha1" });
...
test("validateXmlSignature uses custom keyProviderFn", () => {
    const keyProviderFn = (keyInfo) => {
        return "mock-key";
    };
    const result = validateXmlSignature("<!-- mock -->", "<xml>test</xml>", { keyProviderFn });
    assert.equal(typeof result.valid, "boolean");
});
...
test("validateXmlSignature returns a string error when signature parsing fails before key resolution", () => {
    const result = validateXmlSignature("<!-- mock -->", "<xml>test</xml>", {
        keyProviderFn: () => {
            throw "string error";
        },
    });
...
test("SamlService consumeAssertion with xmlSignature and rawXml triggers validation", () => {
    ...
    assert.throws(() => service.consumeAssertion("test-idp", assertion), /saml\.invalid_signature:/);
});
...
test("SamlService consumeAssertion rejects nameId with only whitespace", () => {
    ...
    assert.throws(() => service.consumeAssertion("test-idp", assertion), /saml\.invalid_subject:/);
});
...
test("SamlService fails closed for buildLoginRequest with unknown provider", () => {
    const service = new SamlService();
    assert.throws(() => service.buildLoginRequest("unknown-provider"), /saml\.provider_not_found:unknown-provider/);
});
test("SamlService fails closed for consumeAssertion with unknown provider", () => {
    const service = new SamlService();
    assert.throws(() => service.consumeAssertion("unknown-provider", {
...
test("SamlService fails closed for buildLogoutRequest with unknown provider", () => {
    const service = new SamlService();
    assert.throws(() => service.buildLogoutRequest("unknown-provider", {
        sessionId: "sess",
        subjectId: "user",
        sessionIndex: null,
    }), /saml\.provider_not_found:unknown-provider/);
...
test("SamlService consumeAssertion with only notOnOrAfter set expires when past", () => {
    ...
    assert.throws(() => service.consumeAssertion("test-idp", assertion, new Date("2026-04-23T10:00:00.000Z")), /saml\.assertion_expired:/);
});
...
//# sourceMappingURL=saml.test.js.map