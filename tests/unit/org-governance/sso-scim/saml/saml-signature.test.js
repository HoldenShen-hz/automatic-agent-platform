...
import { SamlService, validateXmlSignature, SAML_SIGNATURE_ALGORITHMS, buildSamlAudience, } from "../../../../../src/org-governance/sso-scim/saml/index.js";
/**
...
test("validateXmlSignature returns valid structure on success path", () => {
    ...
    const result = validateXmlSignature("<!-- mock signature -->", "<xml>test</xml>", { signatureAlgorithm: "http://www.w3.org/2001/04/xmldsig-more#rsa-sha256" });
...
test("SamlService consumeAssertion rejects invalid issuer", () => {
    ...
    assert.throws(() => service.consumeAssertion("corp-idp", assertion), /saml\.invalid_issuer:corp-idp/);
});
test("SamlService consumeAssertion rejects invalid fingerprint", () => {
    ...
    assert.throws(() => service.consumeAssertion("corp-idp", assertion), /saml\.invalid_fingerprint:corp-idp/);
});
test("SamlService consumeAssertion rejects invalid audience", () => {
    ...
    assert.throws(() => service.consumeAssertion("corp-idp", assertion), /saml\.invalid_audience:corp-idp/);
});
test("SamlService consumeAssertion rejects empty subject", () => {
    ...
    assert.throws(() => service.consumeAssertion("corp-idp", assertion), /saml\.invalid_subject:corp-idp/);
});
test("SamlService consumeAssertion rejects expired assertion", () => {
    ...
    assert.throws(() => service.consumeAssertion("corp-idp", assertion, new Date("2026-04-21")), /saml\.assertion_expired:corp-idp/);
});
...
test("SamlService consumeAssertion validates signature when provided with rawXml", () => {
    ...
    assert.throws(() => service.consumeAssertion("corp-idp", assertion), /saml\.invalid_signature:corp-idp/);
});
test("SamlService consumeAssertion requires signature by default", () => {
    ...
    assert.throws(() => service.consumeAssertion("corp-idp", assertion, new Date()), /saml\.signature_required:corp-idp/);
});
...
test("SamlService consumeAssertion rejects recipient mismatch", () => {
    ...
    assert.throws(() => service.consumeAssertion("corp-idp", assertion, new Date()), /saml\.invalid_recipient:corp-idp/);
});
test("SamlService consumeAssertion rejects replayed assertion id", () => {
    ...
    assert.throws(() => service.consumeAssertion("corp-idp", assertion, new Date()), /saml\.assertion_replayed:corp-idp/);
});
test("SamlService buildLogoutRequest creates correct logout request structure", () => {
    ...
    const logoutRequest = service.buildLogoutRequest("corp-idp", {
...
test("SamlService fails closed when provider not found", () => {
    ...
    assert.throws(() => service.consumeAssertion("missing-provider", {
        issuer: "https://test.com",
        audience: "test",
        nameId: "user",
        fingerprint: "AA:BB:CC",
    }), /saml\.provider_not_found:missing-provider/);
    assert.throws(() => service.buildLogoutRequest("missing-provider", {
        sessionId: "sess",
        subjectId: "user",
        sessionIndex: null,
    }), /saml\.provider_not_found:missing-provider/);
...
test("SamlService consumeAssertion handles exact boundary of notOnOrAfter", () => {
    ...
    assert.throws(() => service.consumeAssertion("corp-idp", assertion, now), /saml\.assertion_expired:corp-idp/);
});
//# sourceMappingURL=saml-signature.test.js.map