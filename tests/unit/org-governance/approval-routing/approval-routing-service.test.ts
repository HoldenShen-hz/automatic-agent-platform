...
describe("ApprovalRoutingService", () => {
  ...
  describe("route - with delegations", () => {
    ...
    it("should not apply delegation when current time is before start", () => {
      ...
      const result = service.route(
        { requesterId: "user-1", orgNodeId: "dept-1", riskLevel: "low", amountUsd: 100 },
        "2026-04-20T00:00:00.000Z", // before delegation starts
        "2026-04-20T00:00:00.000Z",
      );
    ...
    it("should not apply delegation when current time is after expiry", () => {
      ...
      const result = service.route(
        { requesterId: "user-1", orgNodeId: "dept-1", riskLevel: "low", amountUsd: 100 },
        "2026-04-20T00:00:00.000Z", // after expiry
        "2026-04-20T00:00:00.000Z",
      );
  ...
  describe("route - with escalation", () => {
    it("should escalate when time threshold exceeded for high risk", () => {
      ...
      const result = service.route(
        ...
        "2026-04-20T00:00:00.000Z", // created at
        "2026-04-20T00:45:00.000Z", // 45 minutes later - exceeds 30 min threshold
      );
    ...
    it("should not escalate when time threshold not exceeded", () => {
      ...
      const result = service.route(
        { requesterId: "user-1", orgNodeId: "dept-1", riskLevel: "high", amountUsd: 1000 },
        "2026-04-20T00:00:00.000Z", // created at
        "2026-04-20T00:15:00.000Z", // 15 minutes later - under 30 min threshold
      );
    ...
    it("should not escalate when risk level not in appliesToRiskLevels", () => {
      ...
      const result = service.route(
        { requesterId: "user-1", orgNodeId: "dept-1", riskLevel: "low", amountUsd: 100 }, // low risk
        "2026-04-20T00:00:00.000Z",
        "2026-04-20T01:00:00.000Z", // well past threshold
      );
  ...
  describe("route - combined delegation and escalation", () => {
    ...
    it("should handle delegation chain then escalation", () => {
      ...
      const result = service.route(
        ...
        "2026-04-20T00:00:00.000Z",
        "2026-04-20T01:30:00.000Z", // 90 minutes later
      );
