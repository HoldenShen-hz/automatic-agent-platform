...
} from "../../../tests/helpers/typed-factories.js";

test("unsafeCast casts unknown to specified type", () => {
  const unknownValue: unknown = "test string";
  const casted = unsafeCast<string>(unknownValue);
  assert.equal(casted, "test string");
});

test("unsafeCast preserves original value", () => {
  const unknownValue: unknown = { key: "value", num: 42 };
  const casted = unsafeCast<{ key: string; num: number }>(unknownValue);
...
test("partial preserves partial nature", () => {
  const result = partial<{ required1: string; required2: number }>({
    required1: "value",
  });
...
test("createMockCacheStore returns object with all CacheStore methods", () => {
  ...
  assert.ok(
    typeof store.invalidateByTag === "function",
    "should have invalidateByTag method"
  );
  assert.ok(
    typeof store.invalidateNamespace === "function",
...
test("unsafeCast handles class instances", () => {
  ...
  const unknownValue: unknown = instance;
  const casted = unsafeCast<TestClass>(unknownValue);
  assert.equal(casted.getValue(), "hello");
});

test("unsafeCast handles typed arrays", () => {
  const typedArray = new Uint8Array([1, 2, 3, 4]);
  const unknownValue: unknown = typedArray;
  const casted = unsafeCast<Uint8Array>(unknownValue);
