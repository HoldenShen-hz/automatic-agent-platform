...
import { RetryableApiClient, buildApiUrl, buildAuthHeaders, createApiClient, encodeCursor, parseCursor, } from "../../../src/sdk/client-sdk/index.js";
...
test("buildAuthHeaders throws for token with only whitespace", () => {
    ...
    assert.throws(() => buildAuthHeaders(config), /bearer token/i);
});
...
test("createApiClient throws for empty baseUrl", () => {
    ...
    assert.throws(() => createApiClient(config), /baseUrl/i);
});
...
test("createApiClient throws for empty apiVersion", () => {
    ...
    assert.throws(() => createApiClient(config), /apiVersion/i);
});
...
test("PluginContext.fork preserves sessionId when not overridden", () => {
    ...
    assert.equal(child.config.sessionId, "session-123");
});
test("PluginContext.fork allows overriding packId", () => {
    ...
    assert.equal(child.config.packId, "pack-child");
});
test("PluginContext.fork allows overriding executionId", () => {
    ...
    assert.equal(child.config.executionId, "exec-child");
});
test("PluginContext.fork allows overriding userId", () => {
    ...
    assert.equal(child.config.userId, "user-child");
});
test("PluginContext.fork allows overriding sandboxTier", () => {
    ...
    assert.equal(child.config.sandboxTier, "workspace_write");
});
...
test("PackTestLocalService.playbackFixture with errorRate throws", async () => {
    ...
    await assert.rejects(() => service.playbackFixture("fixture-1"), /Mock LLM error/);
});
...
//# sourceMappingURL=sdk-additional-coverage.test.js.map