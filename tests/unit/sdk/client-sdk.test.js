...
import { buildApiUrl, buildAuthHeaders, createApiClient, parseCursor, encodeCursor, RetryableApiClient, } from "../../../src/sdk/client-sdk/index.js";
test("buildApiUrl constructs versioned URL with trailing slash normalization", () => {
    const config = { baseUrl: "https://api.example.com/", apiVersion: "/v1/" };
...
test("buildApiUrl appends query parameters", () => {
    const config = { baseUrl: "https://api.example.com", apiVersion: "v1" };
    const request = { path: "/users", query: { page: 1, active: true, name: "Alice" } };
...
test("buildApiUrl omits null and undefined query values", () => {
    const config = { baseUrl: "https://api.example.com", apiVersion: "v1" };
    const request = { path: "/users", query: { a: null, b: undefined, c: 1 } };
    const url = buildApiUrl(config, request);
...
test("buildApiUrl adds tenantId when present", () => {
    const config = { baseUrl: "https://api.example.com", apiVersion: "v1", tenantId: " tenant-123 " };
    const request = { path: "/users" };
    const url = buildApiUrl(config, request);
    assert.ok(url.includes("tenantId=tenant-123"));
});
test("buildApiUrl does not add empty tenantId", () => {
    const config = { baseUrl: "https://api.example.com", apiVersion: "v1", tenantId: "   " };
    const request = { path: "/users" };
    const url = buildApiUrl(config, request);
...
test("buildAuthHeaders throws ValidationError when bearer token is missing", () => {
    const config = { baseUrl: "https://api.example.com", apiVersion: "v1" };
    assert.throws(() => buildAuthHeaders(config), (error) => error instanceof ValidationError && error.code === "client_sdk.missing_bearer_token");
});
test("buildAuthHeaders throws ValidationError when bearer token is whitespace only", () => {
    const config = { baseUrl: "https://api.example.com", apiVersion: "v1", bearerToken: "   " };
    assert.throws(() => buildAuthHeaders(config), (error) => error instanceof ValidationError && error.code === "client_sdk.missing_bearer_token");
});
test("buildAuthHeaders returns authorization header with trimmed token", () => {
    const config = { baseUrl: "https://api.example.com", apiVersion: "v1", bearerToken: "  token-abc  " };
...
test("createApiClient throws ValidationError when baseUrl is missing", () => {
    const config = { baseUrl: "", apiVersion: "v1" };
    assert.throws(() => createApiClient(config), (error) => error instanceof ValidationError && error.code === "client_sdk.missing_base_url");
});
...
test("createApiClient throws ValidationError when apiVersion is missing", () => {
    const config = { baseUrl: "https://api.example.com", apiVersion: "" };
    assert.throws(() => createApiClient(config), (error) => error instanceof ValidationError && error.code === "client_sdk.missing_api_version");
});
...
test("encodeCursor and parseCursor are inverses", () => {
    const original = { cursor: "cursor-abc", limit: 50 };
...
test("RetryableApiClient get method returns ApiResponse", async () => {
    ...
    globalThis.fetch = async () => new Response(JSON.stringify({ id: 1, name: "test" }), {
        status: 200,
...
test("RetryableApiClient put method sends body and returns ApiResponse", async () => {
    ...
    const client = new RetryableApiClient(config);
    const originalFetch = globalThis.fetch;
    globalThis.fetch = async () => new Response(JSON.stringify({ id: 1, name: "Bob" }), {
        status: 200,
        headers: { "content-type": "application/json" },
    });
    try {
...
test("RetryableApiClient getPaginated returns PaginatedResponse", async () => {
    ...
    const client = new RetryableApiClient(config);
    const originalFetch = globalThis.fetch;
    globalThis.fetch = async () => new Response(JSON.stringify([{ id: 1 }, { id: 2 }]), {
        status: 200,
        headers: {
    ...
    try {
        const result = await client.getPaginated("/users", { cursor: "abc", limit: 20 });
...
test("RetryableApiClient getPaginated with no pagination spec omits query params", async () => {
    ...
    try {
        await client.getPaginated("/users");
...
test("RetryableApiClient throws on network error when max retries exhausted", async () => {
    ...
    try {
        await assert.rejects(client.get("/test"), (error) => error instanceof Error && error.message === "Network failure");
    }
    finally {
        globalThis.fetch = originalFetch;
    }
...
test("RetryableApiClient throws after max retries exhausted on persistent 5xx", async () => {
    ...
    const originalFetch = globalThis.fetch;
    globalThis.fetch = async () => new Response(JSON.stringify({ error: "Server error" }), {
        status: 503,
        headers: { "content-type": "application/json" },
    });
    try {
...
test("buildApiUrl normalizes multiple slashes in path", () => {
    const config = { baseUrl: "https://api.example.com/", apiVersion: "/v1/" };
    const request = { path: "/users/posts" };
    const url = buildApiUrl(config, request);
...
//# sourceMappingURL=client-sdk.test.js.map