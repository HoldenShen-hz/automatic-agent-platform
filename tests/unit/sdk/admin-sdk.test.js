...
test("AdminSdk.listDomains returns paginated response", async () => {
    ...
    globalThis.fetch = async () => new Response(JSON.stringify([{ domainId: "d1" }, { domainId: "d2" }]), {
        status: 200,
...
test("AdminSdk.registerDomain sends POST request", async () => {
    ...
    const originalFetch = globalThis.fetch;
    globalThis.fetch = async () => new Response(JSON.stringify({ domainId: "d_new", name: "New Domain" }), {
        status: 201,
        headers: { "content-type": "application/json" },
    });
    try {
        const result = await sdk.registerDomain({
            name: "New Domain",
        });
...
test("AdminSdk.publishPack calls client.publishPack", async () => {
    ...
    const originalFetch = globalThis.fetch;
    globalThis.fetch = async () => new Response(JSON.stringify({ packId: "pack_1", status: "published" }), {
        status: 200,
        headers: { "content-type": "application/json" },
    });
    try {
        const result = await sdk.publishPack("pack_1", {});
...
test("AdminSdk.triggerPanic sends POST to panic endpoint", async () => {
    ...
    const originalFetch = globalThis.fetch;
    globalThis.fetch = async () => new Response(JSON.stringify({ triggered: true }), {
        status: 200,
        headers: { "content-type": "application/json" },
    });
    try {
        const result = await sdk.triggerPanic({ scope: "platform" });
...
test("AdminSdk.resumePanic sends POST to resume endpoint", async () => {
    ...
    const originalFetch = globalThis.fetch;
    globalThis.fetch = async () => new Response(JSON.stringify({ resumed: true }), {
        status: 200,
        headers: { "content-type": "application/json" },
    });
    try {
        const result = await sdk.resumePanic("global", {});
...
test("AdminSdk.manageAgentLifecycle sends POST to agent action endpoint", async () => {
    ...
    const originalFetch = globalThis.fetch;
    globalThis.fetch = async () => new Response(JSON.stringify({ agentId: "agent_1", action: "stopped" }), {
        status: 200,
        headers: { "content-type": "application/json" },
    });
    try {
        const result = await sdk.manageAgentLifecycle("agent_1", "stop");
...
test("AdminSdk.rotateSecrets sends POST to secrets endpoint", async () => {
    ...
    const originalFetch = globalThis.fetch;
    globalThis.fetch = async () => new Response(JSON.stringify({ rotated: true }), {
        status: 200,
        headers: { "content-type": "application/json" },
    });
    try {
        const result = await sdk.rotateSecrets({
            secretName: "api_key",
        });
...
//# sourceMappingURL=admin-sdk.test.js.map