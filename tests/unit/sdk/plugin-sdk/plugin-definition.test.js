...
test("definePlugin throws when pluginId is missing", () => {
    assert.throws(() => definePlugin({ pluginId: "", name: "Test", version: "1.0.0", type: "tool", capabilities: [] }), /Plugin ID is required/);
});
test("definePlugin throws when name is missing", () => {
    assert.throws(() => definePlugin({ pluginId: "test", name: "", version: "1.0.0", type: "tool", capabilities: [] }), /Plugin name is required/);
});
test("definePlugin throws when version is missing", () => {
    assert.throws(() => definePlugin({ pluginId: "test", name: "Test", version: "", type: "tool", capabilities: [] }), /Plugin version is required/);
});
test("definePlugin throws when type is missing", () => {
    assert.throws(() => definePlugin({ pluginId: "test", name: "Test", version: "1.0.0", type: undefined, capabilities: [] }), /Plugin type is required/);
});
test("definePlugin throws when capabilities are empty", () => {
    assert.throws(() => definePlugin({ pluginId: "test", name: "Test", version: "1.0.0", type: "tool", capabilities: [] }), /at least one capability/);
});
...
test("definePlugin throws when capability inputSchema is missing", () => {
    assert.throws(() => definePlugin({
        ...
        version: "1.0.0",
        type: "tool",
        capabilities: [{ name: "cap", description: "test", inputSchema: undefined, outputSchema: {} }],
...
test("definePlugin throws when capability outputSchema is missing", () => {
    assert.throws(() => definePlugin({
        ...
        version: "1.0.0",
        type: "tool",
        capabilities: [{ name: "cap", description: "test", inputSchema: {}, outputSchema: undefined }],
...
test("validatePluginDefinition uses default description when missing", () => {
    const original = {
        ...
        dependencies: [],
        security: { sandboxTier: "process", egressDomains: [] },
    };
    const validated = validatePluginDefinition(original);
    assert.equal(validated.description, "Plugin description");
});
//# sourceMappingURL=plugin-definition.test.js.map