...
test("stable-campaign rejects invalid profile", () => {
    ...
    const isValid = STABLE_PROFILES.includes(profile);
...
test("optionalPositiveNumber rejects zero", () => {
    const env = { AA_TEST: "0" };
    assert.throws(() => optionalPositiveNumber(env, "AA_TEST"), { message: "stable.invalid_env:AA_TEST" });
});
...
//# sourceMappingURL=stable-campaign.test.js.map