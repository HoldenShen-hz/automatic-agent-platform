...
import { loadSecretManagementCliEnv } from "../../../../src/platform/control-plane/config-center/remaining-cli-env-loaders.js";
import { ValidationError } from "../../../../src/platform/contracts/errors.js";
import { SECRET_ACTIONS } from "../../../../src/platform/control-plane/config-center/remaining-cli-env-support.js";
// ---------------------------------------------------------------------------
...
test("loadSecretManagementCliEnv throws for missing dbPath", () => {
    assert.throws(() => loadSecretManagementCliEnv({}), (error) => error instanceof ValidationError && error.code === "missing_env:AA_DB_PATH");
});
test("loadSecretManagementCliEnv throws for invalid action", () => {
    assert.throws(() => loadSecretManagementCliEnv({
        AA_DB_PATH: "/tmp/test.db",
        AA_SECRET_ACTION: "invalid_action",
    }), (error) => error instanceof ValidationError && error.code === "invalid_env:AA_SECRET_ACTION");
});
// ---------------------------------------------------------------------------
...
test("rotate builds args with version info", () => {
    ...
    const args = {
        secretRef: envConfig.secretRef,
        rotationMode: envConfig.rotationMode,
        status: envConfig.rotationStatus,
...
test("unknown action throws ValidationError", () => {
    ...
    assert.throws(() => {
        if (!SECRET_ACTIONS.includes(action)) {
            throw new ValidationError(errorCode, errorCode);
        }
    }, { message: errorCode });
});
//# sourceMappingURL=secret-management.test.js.map