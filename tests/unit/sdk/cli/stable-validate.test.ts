...
import { loadStableValidateCliEnv } from "../../../../src/platform/five-plane-control-plane/config-center/stable-cli-env.js";
...
test("loadStableValidateCliEnv throws on invalid iterations", () => {
  assert.throws(
    () => loadStableValidateCliEnv({ AA_VALIDATION_ITERATIONS: "-5" }),
    /stable\.invalid_env:AA_VALIDATION_ITERATIONS/,
  );
});

test("loadStableValidateCliEnv throws on non-numeric iterations", () => {
  assert.throws(
    () => loadStableValidateCliEnv({ AA_VALIDATION_ITERATIONS: "invalid" }),
    /stable\.invalid_env:AA_VALIDATION_ITERATIONS/,
  );
});
...
test("loadStableValidateCliEnv throws on zero iterations", () => {
  assert.throws(
    () => loadStableValidateCliEnv({ AA_VALIDATION_ITERATIONS: "0" }),
    /stable\.invalid_env:AA_VALIDATION_ITERATIONS/,
  );
});
...
test("stable-validate failed predicate checks multiple failure types", () => {
  ...
  const failed = (report: { failedRuns?: number; integrityFailures?: number; backupFailures?: number }) =>
    (report.failedRuns ?? 0) > 0 || (report.integrityFailures ?? 0) > 0 || (report.backupFailures ?? 0) > 0;
