...
test("loadStableSoakCliEnv throws on invalid duration", () => {
  assert.throws(
    () => loadStableSoakCliEnv({ AA_SOAK_DURATION_MS: "-1000" }),
    /stable\.invalid_env:AA_SOAK_DURATION_MS/,
  );
});

test("loadStableSoakCliEnv throws on invalid interval", () => {
  assert.throws(
    () => loadStableSoakCliEnv({ AA_SOAK_INTERVAL_MS: "0" }),
    /stable\.invalid_env:AA_SOAK_INTERVAL_MS/,
  );
});

test("loadStableSoakCliEnv throws on non-numeric duration", () => {
  assert.throws(
    () => loadStableSoakCliEnv({ AA_SOAK_DURATION_MS: "invalid" }),
    /stable\.invalid_env:AA_SOAK_DURATION_MS/,
  );
});
