...
test("requireEnvValue returns value when non-null", () => {
  const result = (value: string | null, name: string): string => {
    if (value == null) {
...
test("requireEnvValue throws ValidationError when value is null", () => {
  ...
  assert.throws(
    () => requireEnvValue(null, "AA_MISSING"),
    { message: "missing_env:AA_MISSING" },
  );
});
...
test("requireEnvValue throws ValidationError when value is undefined", () => {
  ...
  assert.throws(
    () => requireEnvValue(undefined, "AA_UNDEFINED"),
    { message: "missing_env:AA_UNDEFINED" },
  );
});
...
test("complete_task requires terminalStatus", () => {
  const terminalStatus: "done" | "failed" | "cancelled" | null = null;
