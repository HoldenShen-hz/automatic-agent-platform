...
test("data plane supports create_analytics_fact action", () => {
  ...
  assert.ok(DATA_PLANE_ACTIONS.includes(action as typeof DATA_PLANE_ACTIONS[number]));
});
...
test("data plane throws ValidationError for unknown action", () => {
  ...
  assert.throws(
    () => {
      if (!DATA_PLANE_ACTIONS.includes(action as typeof DATA_PLANE_ACTIONS[number])) {
...
test("data plane supports analytics_etl movement type", () => {
  ...
  assert.ok(DATA_MOVEMENT_TYPES.includes(movementType as typeof DATA_MOVEMENT_TYPES[number]));
});
...
test("data plane supports completed terminal status", () => {
  ...
  assert.ok(DATA_MOVEMENT_TERMINAL_STATUSES.includes(status as typeof DATA_MOVEMENT_TERMINAL_STATUSES[number]));
});
...
test("start_movement_job builds args with required fields", () => {
  const envConfig = {
    ...
    movementType: "analytics_etl" as const,
