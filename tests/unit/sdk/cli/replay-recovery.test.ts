...
test("loadReplayRecoveryCliEnv requires AA_RECOVERY_REPLAY_KIND", () => {
  assert.throws(
    ...
    /missing_env:AA_RECOVERY_REPLAY_KIND/,
  );
});
...
test("resolveLegacyReplayOutput throws for invalid action", () => {
  ...
  assert.throws(
    ...
    /replay_recovery\.database_not_found/, // db check happens before action validation
  );
});
...
function resolveLegacyReplayOutput(env: Record<string, string | undefined>): unknown | null {
