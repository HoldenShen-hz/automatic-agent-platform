...
test("loadMarketplaceCliEnv parses package lifecycle actions with current env names", () => {
...
test("loadMarketplaceCliEnv defaults to summary and rejects unknown actions", () => {
    ...
    assert.throws(() => loadMarketplaceCliEnv({
        ...
        AA_DB_PATH: "/tmp/test.db",
    }), (error) => error instanceof ValidationError && error.code === "invalid_env:AA_MARKETPLACE_ACTION");
});
