...
test("loadStableSequenceCliEnv invalid sleepMs throws", () => {
    assert.throws(() => loadStableSequenceCliEnv({ AA_STABLE_SEQUENCE_SLEEP_MS: "-1000" }), /stable\.invalid_env:AA_STABLE_SEQUENCE_SLEEP_MS/);
});
test("loadStableSequenceCliEnv invalid enforceWallClock throws", () => {
    assert.throws(() => loadStableSequenceCliEnv({ AA_STABLE_SEQUENCE_ENFORCE_WALL_CLOCK: "maybe" }), /stable\.invalid_env:AA_STABLE_SEQUENCE_ENFORCE_WALL_CLOCK/);
});
// ---------------------------------------------------------------------------
...
test("CLI options correctly map profileNames to options", () => {
    ...
    const options = {
        ...
        profileNames,
        profileOptions: Object.fromEntries(profileNames.map((profileName) => [profileName, sharedProfileOptions])),
    };
...
//# sourceMappingURL=stable-sequence.test.js.map