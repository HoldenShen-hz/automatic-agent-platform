...
describe("loadEvolutionCliEnv", () => {
  it("parses current scope types and budget fields", () => {
  ...
  it("rejects invalid actions and scope types", () => {
    assert.throws(
      () =>
        loadEvolutionCliEnv({
      ...
      (error) =>
        error instanceof ValidationError && error.code === "invalid_env:AA_EVOLUTION_ACTION",
    );
    ...
    assert.throws(
      ...
      (error) =>
        error instanceof ValidationError && error.code === "invalid_env:AA_SCOPE_TYPE",
    );
  });
});
