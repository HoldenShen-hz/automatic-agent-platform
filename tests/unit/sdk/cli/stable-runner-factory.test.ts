...
test("StableRunnerOptions requires outputDir", () => {
  const opts: { outputDir: string } = { outputDir: "/test/output" };
...
test("createStableCli accepts custom failed predicate", () => {
  const customFailed = (r: { failed?: boolean }) => r.failed === true;
  const opts = {
    envVar: "AA_STABLE_TEST",
    defaultDir: "data/stable-test",
    runner: () => ({}),
...
test("createStableCli uses default failed predicate checking failedScenarios", () => {
  const defaultFailed = (r: { failedScenarios?: number }) => (r.failedScenarios ?? 0) > 0;
...
test("createStableCli accepts prepare function for custom args", () => {
  const prepare = (outputDir: string) => ({
    outputDir,
    customArg: "value",
    durationMs: 5000,
  });
...
test("runner receives prepared args when prepare is provided", () => {
  const prepare = (_outputDir: string) => ({ durationMs: 5000, custom: true });
...
test("failed predicate returns true when failedScenarios > 0", () => {
  const failed = (r: { failedScenarios?: number }) => (r.failedScenarios ?? 0) > 0;
...
test("failed predicate with custom check works correctly", () => {
  const failed = (r: { failedRuns?: number; integrityFailures?: number; backupFailures?: number }) =>
    (r.failedRuns ?? 0) > 0 || (r.integrityFailures ?? 0) > 0 || (r.backupFailures ?? 0) > 0;
