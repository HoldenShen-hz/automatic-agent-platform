...
test("loadChannelGatewayEnv parses send action with current gateway env names", () => {
...
test("loadChannelGatewayEnv parses delivery actions and failure reason", () => {
...
test("loadChannelGatewayEnv parses nested gateway provider config", () => {
  const config = loadChannelGatewayEnv({
    AA_CHANNEL_GATEWAY_ACTION: "send",
    AA_DB_PATH: "/tmp/test.db",
...
test("loadChannelGatewayEnv requires action-specific identifiers", () => {
  assert.throws(
    () =>
      loadChannelGatewayEnv({
        AA_DB_PATH: "/tmp/test.db",
      }),
    (error) =>
      error instanceof ValidationError && error.code === "missing_env:AA_GATEWAY_MESSAGE",
  );
  ...
  assert.throws(
    ...
    (error) =>
      error instanceof ValidationError && error.code === "missing_env:AA_DELIVERY_MESSAGE_ID",
  );
});
