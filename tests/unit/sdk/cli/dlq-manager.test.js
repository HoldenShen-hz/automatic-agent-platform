...
// ---------------------------------------------------------------------------
const VALID_ACTIONS = ["list", "count", "retry", "purge"];
const VALID_QUEUES = ["gateway", "jobs", "events"];
...
test("parseArguments throws for missing action", () => {
    assert.throws(() => parseArguments({ queue: "gateway" }), /Invalid action/);
});
test("parseArguments throws for invalid action", () => {
    assert.throws(() => parseArguments({ action: "invalid", queue: "gateway" }), /Invalid action/);
});
test("parseArguments throws for missing queue", () => {
    assert.throws(() => parseArguments({ action: "list" }), /Invalid queue/);
});
test("parseArguments throws for invalid queue", () => {
    assert.throws(() => parseArguments({ action: "list", queue: "invalid" }), /Invalid queue/);
});
...
test("jobs queue retry updates status and resets attempts", () => {
    const sql = { prepare: () => ({ run: () => ({ changes: 5 }) }) };
...
test("gateway queue purge deletes all entries", () => {
    const sql = { prepare: () => ({ run: () => ({ changes: 15 }) }) };
...
test("jobs queue purge deletes only dead_letter status entries", () => {
    const sql = { prepare: () => ({ run: () => ({ changes: 8 }) }) };
