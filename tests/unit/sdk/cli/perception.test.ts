...
test("perception supports upsert_source action", () => {
  ...
  assert.ok(PERCEPTION_ACTIONS.includes(action as typeof PERCEPTION_ACTIONS[number]));
});
...
test("perception throws ValidationError for unknown action", () => {
  ...
  assert.throws(
    () => {
      if (!PERCEPTION_ACTIONS.includes(action as typeof PERCEPTION_ACTIONS[number])) {
