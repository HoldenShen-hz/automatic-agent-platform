...
test("PackTestLocalService.test throws on invalid packId", async () => {
    const service = new PackTestLocalService();
    await assert.rejects(() => service.test({
        packId: "",
        version: "1.0.0",
        mode: "unit",
        mockLlm: false,
        recordArtifacts: false,
    }), (err) => err instanceof Error && err.message.includes("Pack ID is required"));
});
test("PackTestLocalService.test throws on invalid version", async () => {
    ...
    await assert.rejects(() => service.test({
        ...
        mockLlm: false,
        recordArtifacts: false,
    }), (err) => err instanceof Error && err.message.includes("Version is required"));
});
test("PackTestLocalService.test throws on invalid mode", async () => {
    ...
    await assert.rejects(() => service.test({
        ...
        mockLlm: false,
        recordArtifacts: false,
    }), (err) => err instanceof Error && err.message.includes("Mode must be one of"));
});
test("PackTestLocalService.test throws on negative timeout", async () => {
    ...
    await assert.rejects(() => service.test({
        ...
        recordArtifacts: false,
        timeoutMs: -1,
    }), (err) => err instanceof Error && err.message.includes("Timeout must be positive"));
});
...
//# sourceMappingURL=pack-test-local-service.test.js.map