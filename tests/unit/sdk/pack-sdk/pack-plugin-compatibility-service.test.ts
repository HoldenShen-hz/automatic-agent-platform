...
test("pack compatibility reports compatible when selected plugins cover manifest capabilities", () => {
  ...
  assert.deepEqual(
    report.capabilityCoverage.map((entry) => ({ capabilityKey: entry.capabilityKey, compatible: entry.compatible })),
    [
...
test("pack compatibility suggests candidate plugins when selected set does not cover a capability", () => {
  ...
  assert.ok(
    report.capabilityCoverage[0]?.reasons.some((reason) => reason.startsWith("suggested_plugins:")),
  );
});
