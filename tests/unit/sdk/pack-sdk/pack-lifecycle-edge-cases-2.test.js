...
import { PackLifecycleOrchestrationService, validateBusinessPackManifest, } from "../../../../src/sdk/pack-sdk/index.js";
...
test("PackLifecycleOrchestrationService rejects duplicate registration", () => {
    ...
    assert.throws(() => service.registerPack({
        manifest: createManifest(),
        owner: "test@example.com",
    }), (error) => error instanceof ValidationError && error.code === "pack_lifecycle.already_registered:test-pack@1.0.0");
});
...
test("PackLifecycleOrchestrationService.publishPack requires certification before publish", () => {
    ...
    }), (error) => error instanceof ValidationError && error.code.startsWith("pack_lifecycle."));
});
test("PackLifecycleOrchestrationService.archivePack only works from deprecated stage", () => {
    ...
    assert.throws(() => service.archivePack("test-pack", "1.0.0"), (error) => error instanceof ValidationError && error.code === "pack_lifecycle.archive_requires_deprecated:test-pack@1.0.0");
});
test("PackLifecycleOrchestrationService.getMutableRecord throws for non-existent pack", () => {
    const service = new PackLifecycleOrchestrationService();
    assert.throws(() => service.getMutableRecord("non-existent", "1.0.0"), (error) => error instanceof ValidationError && error.code === "pack_lifecycle.not_found:non-existent@1.0.0");
});
...
//# sourceMappingURL=pack-lifecycle-edge-cases-2.test.js.map