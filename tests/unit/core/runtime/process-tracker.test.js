...
import { ProcessTracker, getProcessTracker, resetProcessTracker, spawnTracked, } from "../../../../src/core/runtime/process-tracker.js";
// ---------------------------------------------------------------------------
...
test("TrackedProcessState includes expected states", () => {
    const states = ["running", "terminating", "killed", "exited"];
...
test("spawnTracked with args works", () => {
    resetProcessTracker();
    const tracker = new ProcessTracker();
    const child = spawnTracked(tracker, "node", ["-e", "console.log('test')"], undefined, "exec-file");
    try {
        assert.ok(child.pid > 0);
    }
    finally {
...
test("spawnTracked uses provided cwd", () => {
    resetProcessTracker();
    const tracker = new ProcessTracker();
    const child = spawnTracked(tracker, "pwd", [], { cwd: "/tmp" }, "bash-tool");
    try {
        assert.ok(child.pid > 0);
    }
    finally {
...
//# sourceMappingURL=process-tracker.test.js.map