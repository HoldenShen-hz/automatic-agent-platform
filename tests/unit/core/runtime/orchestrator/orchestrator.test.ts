...
test("runMultiStepOrchestration with oapeflir plan request", async () => {
  ...
  try {
    ...
    assert.ok(
      ...
      "workflowId should have oapeflir prefix",
    );
    assert.ok(
      result.plannedWorkflow.executionSteps.length === 2,
      "should have 2 execution steps",
    );
  } finally {
    cleanupDb(dbPath);
    haCleanup();
...
test("runMultiStepOrchestration compaction result property", async () => {
  ...
  try {
    ...
    assert.ok(
      result.compaction === null || typeof result.compaction === "object",
      "compaction should be null or object",
    );
  } finally {
    cleanupDb(dbPath);
    haCleanup();
...
test("executeMultiStepToolCallForTests is exported as function", () => {
  assert.equal(
    typeof executeMultiStepToolCallForTests,
    "function",
    "executeMultiStepToolCallForTests should be a function",
  );
});

test("resetMultiStepToolRegistryForTests is exported as function", () => {
  assert.equal(
    typeof resetMultiStepToolRegistryForTests,
    "function",
    "resetMultiStepToolRegistryForTests should be a function",
  );
});
