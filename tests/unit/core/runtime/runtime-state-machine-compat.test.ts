...
test("RuntimeStateMachine can be instantiated and emit fact events [runtime-state-machine-compat]", () => {
...
test("RuntimeStateMachine validates and applies a harness-run created -> admitted transition [runtime-state-machine-compat]", () => {
  const persisted: string[] = [];
  const machine = new RuntimeStateMachine({
    persistEvent: (event) => {
      persisted.push(event.eventType);
...
test("RuntimeStateMachine rejects noop and invalid harness-run transitions [runtime-state-machine-compat]", () => {
...
test("RuntimeStateMachine applies a node-run created -> ready transition [runtime-state-machine-compat]", () => {
  const machine = new RuntimeStateMachine({ persistEvent: () => undefined });
  const aggregate = {
