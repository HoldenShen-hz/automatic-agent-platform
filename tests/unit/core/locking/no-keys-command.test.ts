...
test("[SYS-PERF-3.2] redis lock adapter uses SCAN not KEYS", () => {
  ...
  assert.ok(
    existsSync(lockAdapterPath),
    `redis-lock-adapter.ts must exist at ${lockAdapterPath}`,
  );
  ...
  assert.ok(
    !match,
    "redis-lock-adapter.ts must NOT use .keys() command. Use .scan() instead to avoid blocking Redis event loop.",
  );
  ...
  assert.ok(
    hasScan,
    "redis-lock-adapter.ts should use .scan() for iterating keys (found in listHeldAsync)",
  );
});
...
test("[SYS-PERF-3.2] redis lock adapter listHeldAsync uses SCAN", () => {
  ...
  // Verify SCAN is used
  assert.ok(
    /\bscan\b/.test(listHeldCode),
    "listHeldAsync should use scan() method",
  );

  // Verify KEYS is NOT used
  assert.ok(
    !/\.keys\(/.test(listHeldCode),
    "listHeldAsync must not use .keys() which blocks Redis",
  );
});
...
test("[SYS-PERF-3.2] other files in execution layer must not use KEYS", () => {
  ...
  for (const filePath of filesToCheck) {
    ...
    assert.ok(
      !keysMatch,
      `${filePath} must NOT use .keys() - it blocks the Redis event loop. Use .scan() instead.`,
    );
  }
});
