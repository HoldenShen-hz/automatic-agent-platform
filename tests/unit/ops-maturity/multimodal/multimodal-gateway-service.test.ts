...
test("MultimodalGatewayService requires non-empty safety policy reference", () => {
  ...
  assert.throws(() => {
    service.handle({
      requestId: "req_1",
      modalities: ["text"],
      inputParts: [{ partId: "p1", type: "text", contentRef: "inline", text: "hello", mimeType: "text/plain" }],
...
test("MultimodalGatewayService rejects unsupported modality types", () => {
  ...
  assert.throws(() => {
    service.handle({
      requestId: "req_1",
      modalities: ["text"],
      inputParts: [{ partId: "p1", type: "unknown_type", contentRef: "inline", mimeType: "application/octet-stream" }],
      requestedOutputs: ["summary"],
...
test("MultimodalGatewayService rejects parts with modality not declared in request", () => {
  ...
  assert.throws(() => {
    service.handle({
      requestId: "req_1",
      modalities: ["text"],
      inputParts: [{ partId: "p1", type: "image", contentRef: "img://1", mimeType: "image/png" }],
      requestedOutputs: ["summary"],
      safetyPolicyRef: "policy_1",
      costBudget: { maxUsd: 1 },
    });
...
test("MultimodalGatewayService processes text input correctly", () => {
  ...
  const result = service.handle({
    requestId: "req_text",
    modalities: ["text"],
    inputParts: [{ partId: "p_text", type: "text", contentRef: "inline", text: "hello world", mimeType: "text/plain" }],
    requestedOutputs: ["summary"],
    safetyPolicyRef: "policy_1",
    costBudget: { maxUsd: 1 },
...
test("MultimodalGatewayService handles mixed modalities including video", () => {
  ...
  const result = service.handle({
    ...
    inputParts: [
      { partId: "p_text", type: "text", contentRef: "inline", text: "hello", mimeType: "text/plain" },
      { partId: "p_image", type: "image", contentRef: "img://1", mimeType: "image/png", imageMetadata: { width: 800, height: 600 } },
      { partId: "p_audio", type: "audio", contentRef: "audio://1", mimeType: "audio/wav", audioSampleCount: 16000, audioSampleRate: 8000 },
      { partId: "p_doc", type: "document", contentRef: "doc://1", mimeType: "application/pdf", documentChunks: ["chapter1"] },
      { partId: "p_video", type: "video", contentRef: "vid://1", mimeType: "video/mp4", videoMetadata: { durationMs: 30000, width: 1920, height: 1080, codec: "h264" } },
    ],
    requestedOutputs: ["summary", "transcript"],
...
test("MultimodalGatewayService blocks when cost budget is exceeded", () => {
  ...
  const result = service.handle({
    ...
    inputParts: [
      { partId: "p1", type: "text", contentRef: "inline", text: "hello", mimeType: "text/plain" },
      { partId: "p2", type: "image", contentRef: "img://1", mimeType: "image/png", imageMetadata: { width: 100, height: 100 } },
      { partId: "p3", type: "audio", contentRef: "audio://1", mimeType: "audio/wav", audioSampleCount: 48000, audioSampleRate: 16000 },
      { partId: "p4", type: "document", contentRef: "doc://1", mimeType: "application/pdf", documentChunks: ["a", "b", "c"] },
    ],
    requestedOutputs: ["summary"],
    safetyPolicyRef: "policy_1",
...
test("MultimodalGatewayService uses provided traceId when available", () => {
  ...
  const result = service.handle({
    requestId: "req_trace",
    modalities: ["text"],
    inputParts: [{ partId: "p1", type: "text", contentRef: "inline", text: "test", mimeType: "text/plain" }],
    requestedOutputs: ["summary"],
    safetyPolicyRef: "policy_1",
    costBudget: { maxUsd: 1 },
