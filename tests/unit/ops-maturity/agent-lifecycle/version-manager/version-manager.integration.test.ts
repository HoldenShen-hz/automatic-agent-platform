...
test("ComponentSnapshotSchema: rejects empty strings for required fields", () => {
  for (const field of ["packVersion", "promptBundleVersion", "modelBindingHash", "trustProfileHash", "triggerSetHash", "autonomyConfigHash"] as const) {
    assert.throws(() =>
      ComponentSnapshotSchema.parse({
