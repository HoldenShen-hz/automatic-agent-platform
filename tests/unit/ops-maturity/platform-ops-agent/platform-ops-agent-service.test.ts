...
import {
  ...
  type PlatformOpsAgentDefinition,
  type OpsSignalInput,
...
function createHealthProbe(overrides: Partial<OpsHealthProbe> = {}): OpsHealthProbe {
  return {
...
function createOpsAgentDefinition(overrides: Partial<PlatformOpsAgentDefinition> = {}): PlatformOpsAgentDefinition {
  return {
    agentId: "ops_agent_1",
...
function createOpsSignalInput(overrides: Partial<OpsSignalInput> = {}): OpsSignalInput {
  return {
...
test("PlatformOpsAgentService throws for missing proposal", () => {
  ...
  assert.throws(
    () => service.recordApproval("nonexistent_id", "admin"),
