...
// Helper type for parsed JSON output
...
test("renderPlainTextExplanation all sections appear when all inputs provided", () => {
  const links: CausalLink[] = [{ source: "start", target: "end", rationale: "done" }];
...
test("renderForAudience default case falls back to plain text", () => {
  const result = renderForAudience("test", "Summary", [], [], [], "unknown" as any);
...
test("DecisionTreeNode types are valid strings", () => {
  ...
  for (const node of tree.allNodes) {
    assert.ok(
      ["decision", "factor", "evidence", "outcome"].includes(node.type),
...
test("renderPlainTextExplanation with empty strings in causal links", () => {
  const links: CausalLink[] = [{ source: "", target: "", rationale: "" }];
  const result = renderPlainTextExplanation("test", "summary", links, [], []);
...
test("buildDecisionTree maxDepth is 2 for a single causal link", () => {
  // Root -> source-A (depth 1) -> target-B (depth 2)
  const links: CausalLink[] = [
    { source: "A", target: "B", rationale: "A leads to B" },
  ];
  const tree = buildDecisionTree("Root", links, [], []);
