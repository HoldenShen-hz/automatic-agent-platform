...
describe("ExplanationCache", () => {
  describe("putExplanationCacheEntry", () => {
    test("adds a new entry to empty cache", () => {
      ...
      const entry: ExplanationCacheEntry = { cacheKey: "task-1:plan:L2", summary: "Task completed" };
    ...
    test("adds entry to non-empty cache preserving existing entries", () => {
      ...
      const entry: ExplanationCacheEntry = { cacheKey: "new-key", summary: "New summary" };
    ...
    test("returns new cache object without mutating original", () => {
      ...
      const entry: ExplanationCacheEntry = { cacheKey: "key", summary: "summary" };
