...
function seedFailedMultiStepWorkflow(taskId, executionId, sessionId) {
    return {
        taskId,
        executionId,
        sessionId,
...
test("E2E: async human takeover retries a failed multi-step workflow, repositions recovery, and injects manual output", async () => {
    const seeded = seedFailedMultiStepWorkflow("task-runtime-recovery", "exec-runtime-recovery", "sess-runtime-recovery");
    ...
    try {
        ...
        harness.db.transaction(() => {
            harness.db.connection
                ...
                .run(seeded.workflowId, "workflow_planner", "failed", "execution.failed", now, now, seeded.executionId);
        ...
        const workflowOutputs = JSON.parse(snapshot.workflow?.outputsJson ?? "{}");
...
//# sourceMappingURL=human-takeover-recovery-flow.test.js.map