...
test("E2E Task Lifecycle: cancelled task cannot transition to any other state", async () => {
    ...
    try {
        ...
        }, /invalid transition/i, "Should not allow transition from cancelled to pending");
        ...
        }, /invalid transition/i, "Should not allow transition from cancelled to in_progress");
        ...
        }, /invalid transition/i, "Should not allow transition from cancelled to done");
...
test("E2E Task Lifecycle: task retry preserves partial workflow progress", async () => {
    ...
    try {
        ...
        harness.db.transaction(() => {
            harness.store.updateWorkflowState(taskId, "running", 2, JSON.stringify({ step0_output: "completed", step1_output: "also completed" }), nowIso(), null);
        });
        ...
        harness.db.transaction(() => {
            harness.store.updateWorkflowState(taskId, "completed", 3, JSON.stringify({ step0_output: "completed", step1_output: "also completed", step2_output: "final" }), nowIso(), null);
        });
        ts.transitionExecutionStatus(makeExecCommand(executionId2, "executing", "succeeded", traceId2));
        // Complete task
        ts.transitionTaskTerminalState({
