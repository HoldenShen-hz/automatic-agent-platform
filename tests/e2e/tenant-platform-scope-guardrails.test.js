...
test("E2E: tenant-platform rejects namespace creation when workspace and tenant belong to different organizations", async () => {
    ...
    try {
        ...
        assert.equal(mismatchError?.name, "TenantBoundaryError");
        assert.equal(mismatchError?.code, "tenant.workspace_tenant_organization_mismatch");
...
test("E2E: tenant-platform blocks cross-workspace data movement even within the same tenant", async () => {
    ...
    try {
        ...
        assert.equal(boundaryError?.code, "data_plane.cross_workspace_movement_denied");
...
//# sourceMappingURL=tenant-platform-scope-guardrails.test.js.map