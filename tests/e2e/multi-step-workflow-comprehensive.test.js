...
test("E2E Multi-Step: five-step workflow completes all steps in sequence", () => {
    ...
    try {
        ...
        for (let stepIndex = 0; stepIndex < 5; stepIndex++) {
            ...
            h.db.transaction(() => {
                h.store.updateWorkflowState(taskId, isLastStep ? "completed" : "running", stepIndex + 1, JSON.stringify(accumulatedOutputs), now, null);
            });
...
test("E2E Multi-Step: workflow pauses at step boundary and resumes", () => {
    ...
    try {
        ...
        h.db.transaction(() => {
            h.store.updateWorkflowState(taskId, "running", 2, JSON.stringify({ step0_result: "completed", step1_result: "resumed" }), now, null);
        });
        workflow = h.store.getWorkflowState(taskId);
...
test("E2E Multi-Step: step retry recovers from transient failure", () => {
    ...
    try {
        ...
        // Retry succeeds
        h.db.transaction(() => {
            h.store.updateWorkflowState(taskId, "running", 2, JSON.stringify({ step0_result: "completed", step1_result: "retry_success" }), nowIso(), null);
        });
        workflow = h.store.getWorkflowState(taskId);
...
test("E2E Multi-Step: conditional branch selection based on step output", () => {
    ...
    try {
        ...
        h.db.transaction(() => {
            h.store.updateWorkflowState(taskId, "running", 1, JSON.stringify({ validation_passed: true, validation_result: "all_checks_ok" }), now, null);
        });
        // Step 1 should branch to "success_path" based on validation result
        h.db.transaction(() => {
            h.store.updateWorkflowState(taskId, "running", 2, JSON.stringify({
...
test("E2E Multi-Step: workflow resumes from correct step after failure", () => {
    ...
    try {
        ...
        h.db.transaction(() => {
            h.store.updateWorkflowState(taskId, "running", 1, // Reset to step 1
            JSON.stringify({ step0_result: "completed", step1_result: "completed" }), nowIso(), 1);
        });
        workflow = h.store.getWorkflowState(taskId);
...
//# sourceMappingURL=multi-step-workflow-comprehensive.test.js.map