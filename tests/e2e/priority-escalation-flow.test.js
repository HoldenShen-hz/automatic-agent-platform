...
function insertTask(harness, taskId, priority, status = "queued", createdAtMinutesAgo = 0) {
    const now = nowIso();
...
test("E2E Priority Escalation: priority escalates through the chain low->normal->high->urgent", async () => {
    ...
    try {
        ...
        const getPriorityIndex = (p) => priorityOrder.indexOf(p);
        assert.ok(getPriorityIndex(highTask.priority) > getPriorityIndex(normalTask.priority), "High priority should be greater than normal");
        assert.ok(getPriorityIndex(urgentTask.priority) > getPriorityIndex(highTask.priority), "Urgent priority should be greater than high");
    }
    finally {
        harness.cleanup();
    }
...
//# sourceMappingURL=priority-escalation-flow.test.js.map