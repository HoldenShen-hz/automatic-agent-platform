...
test("E2E: connector framework enforces prod verification, defers degraded connectors, and fail-closes unsupported events", () => {
    ...
    let prodVerificationError = null;
    try {
    ...
    assert.equal(prodVerificationError?.message, "connector_framework.prod_requires_verified:draft-sync-e2e");
    ...
    assert.deepEqual(connectors.listEnabled().map((manifest) => manifest.connectorId).sort(), ["ops-webhook-e2e"]);
});
...
//# sourceMappingURL=ecosystem-validation-and-integration-flow.test.js.map