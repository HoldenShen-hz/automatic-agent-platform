...
// Test Harness
// ---------------------------------------------------------------------------
function createScalingHarness(prefix) {
...
function createFairQueueItem(itemId, tenantId, ageMs, priority) {
    return {
        itemId,
        tenantId,
        priority,
...
function seedTask(store, db, taskId, status, tenantId = "tenant-001", createdAt = nowIso()) {
    db.transaction(() => {
        store.insertTask({
            id: taskId,
            parentId: null,
...
test("E2E Scaling: fair scheduling does not preempt when quota not exceeded", (t) => {
    const service = new FairSchedulingService();
    const queueItems = [createFairQueueItem("item-1", "tenant-001", 30_000, 50)];
    const claim = createResourceClaim({ requestedUnits: 1 });
    const request = {
...
test("E2E Scaling: mixed task statuses tracked correctly", (t) => {
    ...
    try {
        ...
        const statuses = ["queued", "in_progress", "in_progress", "done", "failed", "cancelled"];
...
//# sourceMappingURL=scaling-flow.test.js.map