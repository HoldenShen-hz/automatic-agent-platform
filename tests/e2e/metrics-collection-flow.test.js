...
function createMetricsHarness(prefix) {
...
function seedTask(store, db, taskId, status, costUsd = 0, createdAt) {
    db.transaction(() => {
        store.insertTask({
            id: taskId,
            parentId: null,
...
function seedWorkflow(store, db, taskId, status, retryCount = 0) {
    db.transaction(() => {
...
function seedExecution(store, db, executionId, taskId, status, attempt = 1) {
    db.transaction(() => {
        const existing = store.getTask(taskId);
        if (!existing) {
            store.insertTask({
...
test("E2E Metrics Collection: metrics include health status", () => {
    ...
    try {
        ...
        assert.ok(["ok", "degraded", "overloaded", "unhealthy"].includes(summary.runtimeMetrics.status), "Health status should be a valid value");
        assert.ok(["none", "queue_only", "fast_only", "pause_non_critical", "read_only_operations_only"].includes(summary.runtimeMetrics.degradationMode), "Degradation mode should be valid");
...
test("E2E Metrics Collection: global metrics registry singleton", () => {
    ...
    assert.ok(runtimeMetricsRegistry instanceof RuntimeMetricsRegistry, "runtimeMetricsRegistry should be an instance of RuntimeMetricsRegistry");
...
//# sourceMappingURL=metrics-collection-flow.test.js.map