...
import { TenantBoundaryError } from "../../src/platform/contracts/errors.js";
...
test("E2E: tenant-platform blocks cross-tenant data movement between isolated namespaces", async () => {
  ...
  try {
    ...
    assert.equal((boundaryError as { code?: string } | null)?.code, "data_plane.cross_tenant_movement_denied");
