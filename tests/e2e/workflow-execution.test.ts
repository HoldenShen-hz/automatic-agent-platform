...
function buildOapeflirPlanRequest(steps: Array<{
  stepId: string;
  dependencies?: string[];
  outputs?: string[];
  timeout?: number;
...
test("E2E: multi-step workflow executes all steps successfully", async () => {
  ...
  try {
    ...
    assert.equal(
      result.plannedWorkflow.executionSteps.length,
      3,
      "Should have 3 execution steps",
    );
...
test("E2E: multi-step workflow with single step", async () => {
  ...
  try {
    ...
    assert.equal(
      result.plannedWorkflow.executionSteps.length,
      1,
      "Should have 1 execution step",
    );
    assert.ok(result.plannedWorkflow.executionSteps[0], "First step should exist");
    assert.equal(
      result.plannedWorkflow.executionSteps[0].stepId,
      "only_step",
...
test("E2E: workflow with step dependencies respects order", async () => {
  ...
  try {
    ...
    const secondStep = result.plannedWorkflow.executionSteps.find(
      (s) => s.stepId === "second_step",
    );
    assert.ok(secondStep, "Second step should exist");
    assert.ok(
      secondStep!.dependsOnStepIds.includes("first_step"),
...
test("E2E: workflow completes with aggregated outputs", async () => {
  ...
  try {
    ...
    assert.ok(
      task!.completedAt != null || task!.status === "done" || task!.status === "failed",
      "Task should be in terminal state or have completedAt",
    );
  } finally {
    harness.cleanup();
  }
...
test("E2E: workflow produces final output", async () => {
  ...
  try {
    ...
    assert.ok(
      result.snapshot.task.outputJson != null || result.snapshot.task.status !== null,
      "Task should have output or status set",
    );
  } finally {
    harness.cleanup();
  }
...
test("E2E: workflow with injected step failure marks workflow as failed", async () => {
  ...
  try {
    ...
    assert.ok(
      task!.status === "failed" || task!.errorCode != null,
      "Task should indicate failure state or error",
    );
  } finally {
    harness.cleanup();
  }
...
test("E2E: workflow with step failure injection on first step", async () => {
  ...
  try {
    ...
    assert.ok(
      task!.status === "failed" || task!.errorCode != null,
      "Task should indicate failure when first step fails",
    );
  } finally {
    harness.cleanup();
  }
...
test("E2E: workflow with many sequential steps", async () => {
  ...
  try {
    ...
    assert.equal(
      result.plannedWorkflow.executionSteps.length,
      10,
      "Should have 10 execution steps",
    );
  } finally {
    harness.cleanup();
  }
