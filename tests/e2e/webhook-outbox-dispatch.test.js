...
test("E2E: webhook ingress receives event and stages to outbox", () => {
    ...
    try {
        ...
        const githubEntries = entries.filter((e) => e.aggregateType === "webhook_endpoint" && e.aggregateId === "github");
...
test("E2E: unregistered endpoint is rejected", () => {
    ...
    try {
        ...
        }, /Webhook endpoint is not registered/, "Should throw for unregistered endpoint");
    }
    finally {
        h.db.close();
        cleanupPath(h.workspace);
...
test("E2E: disabled endpoint rejects events", () => {
    ...
    try {
        ...
        }, /Webhook endpoint is disabled/, "Should throw for disabled endpoint");
    }
    finally {
        h.db.close();
        cleanupPath(h.workspace);
...
test("E2E: multiple endpoints have separate outbox entries", () => {
    ...
    try {
        ...
        const githubEntries = allEntries.filter((e) => e.aggregateType === "webhook_endpoint" && e.aggregateId === "github");
        const gitlabEntries = allEntries.filter((e) => e.aggregateType === "webhook_endpoint" && e.aggregateId === "gitlab");
...
//# sourceMappingURL=webhook-outbox-dispatch.test.js.map