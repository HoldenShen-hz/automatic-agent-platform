...
test("E2E Environment Readiness: registers multiple components with different types", () => {
    ...
    const components = [
        ...
        { type: "gateway", id: "gateway-1" },
        { type: "sandbox", id: "sandbox-1" },
        { type: "worker_fleet", id: "workers-1" },
        { type: "artifact_store", id: "artifacts-1" },
        { type: "notification_channel", id: "notifications-1" },
        { type: "external_service", id: "external-1" },
    ];
...
test("E2E Environment Readiness: evaluate promote to production - all requirements met", () => {
    ...
    const componentTypes = ["provider", "gateway", "sandbox", "worker_fleet", "artifact_store", "notification_channel", "external_service"];
    ...
    const drillTypes = ["backup_restore", "rolling_upgrade", "maintenance_drain", "tenant_gray_rollout", "regional_failover", "worker_reassignment", "queue_repair"];
    ...
    const sloMetrics = [
        { metric: "task_success_rate", comparator: "min", target: 0.95, observed: 0.97 },
        { metric: "task_start_latency", comparator: "max", target: 5000, observed: 3200 },
        { metric: "recovery_success_rate", comparator: "min", target: 0.90, observed: 0.95 },
        { metric: "approval_delivery_availability", comparator: "min", target: 0.99, observed: 0.995 },
        { metric: "tier1_event_delivery_latency", comparator: "max", target: 1000, observed: 800 },
    ];
...
test("E2E Environment Readiness: evaluatePromotion throws for unknown environment", () => {
    const service = new EnvironmentReadinessOrchestrationService();
    assert.throws(() => service.evaluatePromotion({
        environment: "prod",
        targetStatus: "production_ready",
    }), /does not have any readiness records/i, "Should throw for environment without records");
});
...
//# sourceMappingURL=environment-readiness-orchestration-e2e.test.js.map