...
// Helper Functions
// ---------------------------------------------------------------------------
function makeExecCommand(executionId, fromStatus, toStatus, traceId, reasonCode = "e2e_error_handling") {
    return {
...
test("E2E Error: task execution times out and transitions to failed state", async () => {
    ...
    try {
        ...
        ts.transitionExecutionStatus(makeExecCommand(executionId, "executing", "failed", traceId, "execution.timeout"));
...
test("E2E Error: worker failure marks execution as failed and triggers recovery", async () => {
    ...
    try {
        ...
        ts.transitionExecutionStatus(makeExecCommand(executionId, "executing", "failed", traceId, "worker.failure"));
        let exec = harness.store.getExecution(executionId);
...
test("E2E Error: memory exhaustion causes execution failure", async () => {
    ...
    try {
        ...
        ts.transitionExecutionStatus(makeExecCommand(executionId, "executing", "failed", traceId, "resource.memory_exhausted"));
        let exec = harness.store.getExecution(executionId);
        assert.equal(exec?.status, "failed", "Execution should be failed");
...
test("E2E Error: budget exhaustion prevents execution start", async () => {
    ...
    try {
        ...
        ts.transitionExecutionStatus(makeExecCommand(executionId, "created", "failed", traceId, "budget.exhausted"));
...
test("E2E Error: disk space exhaustion triggers workflow failure", async () => {
    ...
    try {
        ...
        ts.transitionExecutionStatus(makeExecCommand(executionId, "executing", "failed", traceId, "resource.disk_exhausted"));
        let exec = harness.store.getExecution(executionId);
        assert.equal(exec?.status, "failed", "Execution should be failed");
...
test("E2E Error: network failure causes provider error and execution retries", async () => {
    ...
    try {
        ...
        harness.db.transaction(() => {
            harness.store.updateWorkflowState(taskId, "running", 0, "{}", nowIso(), null);
        });
        // Retry succeeds
        ts.transitionExecutionStatus(makeExecCommand(executionId2, "executing", "succeeded", traceId2));
        // Complete task
