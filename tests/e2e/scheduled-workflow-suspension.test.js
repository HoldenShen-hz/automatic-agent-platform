...
function seedWorkflowWithTask(store, db, taskId, executionId, workflowId, currentStepIndex = 0) {
    const now = nowIso();
    db.transaction(() => {
        store.insertTask({
            id: taskId,
...
test("E2E: suspension of terminal workflow throws error", () => {
    ...
    try {
        ...
        h.db.transaction(() => {
            h.store.updateWorkflowState(taskId, "completed", 3, JSON.stringify({ final: "done" }), nowIso(), null);
        });
        ...
        }, /workflow_sleep.terminal_workflow/, "Should throw for terminal workflow");
    }
    finally {
        h.db.close();
        cleanupPath(h.workspace);
...
//# sourceMappingURL=scheduled-workflow-suspension.test.js.map