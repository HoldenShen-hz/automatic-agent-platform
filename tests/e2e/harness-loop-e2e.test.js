...
test("E2E: loop triggers replan and continues when evaluator score < 0.5", (t) => {
    ...
    try {
        ...
        assert.ok(run.decision?.action === "replan" || run.decision?.action === "abort", "decision should be replan or abort (budget may exhaust)");
        assert.ok(run.feedbackEnvelope, "feedbackEnvelope should be present on non-accept");
        if (run.decision?.action === "replan") {
            assert.equal(run.feedbackEnvelope?.learnedActions.includes("update_plan_bundle"), true, "replan should add update_plan_bundle learned action");
        }
    }
    finally {
        harness.cleanup();
...
test("E2E: HITL request not found error when resolving without open request", (t) => {
    ...
    try {
        ...
        assert.throws(() => service.resolveHitlReview(run, "approved", "someone"), (err) => err instanceof Error && err.message.includes("harness.hitl.request_not_found"), "should throw when no HITL request is open");
    }
    finally {
        harness.cleanup();
    }
...
test("E2E: loop aborts when max iterations reached without accept", (t) => {
    ...
    try {
        ...
        assert.ok(run.decision, "decision should be present");
        assert.ok(run.decision?.action === "abort" || run.decision?.action === "replan" || run.decision?.action === "retry_same_plan", "decision should be terminal or retry action");
...
test("E2E: loop aborts when max duration exceeded", (t) => {
    ...
    try {
        ...
        // Duration guard should trigger when maxDurationMs is exceeded (1ms vs current time)
        // Abort reason must specifically indicate duration guard was the cause
        assert.equal(runLoopResult.status, "aborted", "duration guard should abort the run");
        assert.ok(runLoopResult.decision?.reasonCodes?.includes("harness.guard.max_duration_exceeded"),
            "decision reasonCodes should contain harness.guard.max_duration_exceeded");
    }
    finally {
        harness.cleanup();
    }
...
// End of E2E tests
// ---------------------------------------------------------------------------
//# sourceMappingURL=harness-loop-e2e.test.js.map