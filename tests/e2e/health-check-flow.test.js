...
function createHealthHarness(prefix, options) {
...
function seedTask(store, db, taskId, status, createdAt = nowIso()) {
    db.transaction(() => {
        store.insertTask({
            id: taskId,
            parentId: null,
...
function seedExecution(store, db, executionId, taskId, status) {
    db.transaction(() => {
...
test("E2E Health Check: returns valid status report", () => {
    ...
    try {
        ...
        assert.ok(["ok", "degraded", "overloaded", "unhealthy"].includes(report.status), `Status should be valid, got: ${report.status}`);
        ...
        assert.ok(["healthy", "degraded", "failed"].includes(report.providerHealth), "Provider health should be valid");
        ...
        assert.ok(["none", "queue_only", "fast_only", "pause_non_critical", "read_only_operations_only"].includes(report.degradationMode), "Degradation mode should be valid");
...
test("E2E Health Check: queued tasks trigger degraded status", () => {
    ...
    try {
        ...
        assert.ok(["queue_only", "fast_only", "pause_non_critical", "read_only_operations_only", "none"].includes(report.degradationMode), "Degradation mode should be set");
    }
    finally {
        h.db.close();
        cleanupPath(h.workspace);
...
test("E2E Health Check: queue governance summary populated", () => {
    ...
    try {
        ...
        assert.ok(typeof report.queueGovernance.dispatchableBacklogSize === "number", "Dispatchable backlog size should be a number");
        assert.ok(typeof report.queueGovernance.claimedBacklogSize === "number", "Claimed backlog size should be a number");
        assert.ok(Array.isArray(report.queueGovernance.queueNames), "Queue names should be an array");
        assert.ok(typeof report.queueGovernance.starvationDetected === "boolean", "Starvation detected should be a boolean");
    }
    finally {
        h.db.close();
        cleanupPath(h.workspace);
...
test("E2E Health Check: memory high watermark affects status", () => {
    ...
    try {
        ...
        assert.ok(["degraded", "overloaded"].includes(report.status), `Status should be degraded or overloaded, got: ${report.status}`);
...
test("E2E Health Check: degraded mode transitions correctly", () => {
    ...
    try {
        ...
        assert.ok(["degraded", "overloaded"].includes(report.status), `Status should be degraded, got: ${report.status}`);
...
//# sourceMappingURL=health-check-flow.test.js.map