...
test("E2E: ServiceRegistry registers and initializes service", () => {
    ...
    registry.register("test-service-1", {
        init: () => ({
            name: "test-service",
            initialized: true,
            dispose() {
...
test("E2E: ServiceRegistry initializes service lazily", () => {
    ...
    registry.register("test-lazy-service", {
        init: () => {
            initCalled = true;
            return { value: 42 };
        },
    ...
    const service = registry.get("test-lazy-service");
...
test("E2E: ServiceRegistry returns same instance on multiple gets", () => {
    const registry = ServiceRegistry.getInstance();
    registry.register("test-singleton-service", {
        init: () => ({ id: "singleton-instance" }),
    });
    const instance1 = registry.get("test-singleton-service");
    const instance2 = registry.get("test-singleton-service");
...
test("E2E: ServiceRegistry teardown is called on reset", async () => {
    ...
    registry.register("test-teardown-service", {
        init: () => ({
            dispose() {
                teardownCalled = true;
            },
    ...
    registry.get("test-teardown-service");
...
test("E2E: ServiceRegistry throws for unregistered service", () => {
    const registry = ServiceRegistry.getInstance();
    assert.throws(() => registry.get("non-existent-service"), /no service registered/i, "Should throw for unregistered service");
});
test("E2E: ServiceRegistry supports service with dependencies", () => {
    const registry = ServiceRegistry.getInstance();
    registry.register("test-dependency-service", {
        init: () => ({ value: 100 }),
    });
    registry.register("test-dependent-service", {
        init: () => {
            const dep = registry.get("test-dependency-service");
            return { depValue: dep.value, computed: dep.value * 2 };
        },
        dependsOn: ["test-dependency-service"],
    });
    const dependent = registry.get("test-dependent-service");
...
test("E2E: ServiceRegistry can register multiple services", () => {
    const registry = ServiceRegistry.getInstance();
    registry.register("multi-service-1", {
        init: () => ({ id: 1 }),
    });
    registry.register("multi-service-2", {
        init: () => ({ id: 2 }),
    });
    registry.register("multi-service-3", {
        init: () => ({ id: 3 }),
    });
    const service1 = registry.get("multi-service-1");
    const service2 = registry.get("multi-service-2");
    const service3 = registry.get("multi-service-3");
...
//# sourceMappingURL=service-registry-lifecycle.test.js.map