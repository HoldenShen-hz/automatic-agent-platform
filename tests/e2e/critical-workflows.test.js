...
test("E2E Critical: multi-step workflow executes all steps in dependency order", async () => {
    ...
    try {
        ...
        // Step 0 -> Step 1
        harness.db.transaction(() => {
            harness.store.updateWorkflowState(taskId, "running", 1, JSON.stringify({ step0_output: "result_from_step_0" }), nowIso(), null);
        });
        let workflow = harness.store.getWorkflowState(taskId);
        assert.equal(workflow?.currentStepIndex, 1, "Should advance to step 1");
        assert.equal(JSON.parse(workflow.outputsJson).step0_output, "result_from_step_0", "Step 0 output should be preserved");
        // Step 1 -> Step 2
        harness.db.transaction(() => {
            harness.store.updateWorkflowState(taskId, "running", 2, JSON.stringify({ step0_output: "result_from_step_0", step1_output: "result_from_step_1" }), nowIso(), null);
        });
...
test("E2E Critical: cancelled task cannot transition to any other state", async () => {
    ...
    try {
        ...
        }, /invalid transition/i, "Should not allow transition from cancelled");
        ...
        }, /invalid transition/i, "Should not allow cancelled to done");
    }
    finally {
        harness.cleanup();
    }
...
//# sourceMappingURL=critical-workflows.test.js.map