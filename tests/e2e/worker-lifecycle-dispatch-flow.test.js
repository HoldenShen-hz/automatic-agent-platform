...
function makeWorkerSnapshot(workerId, status, overrides = {}) {
    return {
        workerId,
        status,
        placement: "local",
...
test("E2E Worker Lifecycle: worker tracks active executions against max concurrency", () => {
    ...
    try {
        ...
        assert.ok(JSON.parse(worker.runningExecutionsJson).length <= worker.maxConcurrency, "Running executions should not exceed maxConcurrency");
    }
    finally {
        h.db.close();
        cleanupPath(h.workspace);
...
test("E2E Worker Lifecycle: multiple workers can coexist with different statuses", () => {
    ...
    try {
        ...
        const statuses = ["idle", "busy", "draining"];
...
//# sourceMappingURL=worker-lifecycle-dispatch-flow.test.js.map