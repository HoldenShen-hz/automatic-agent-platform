...
// ---------------------------------------------------------------------------
function insertFailedExecutionForFailover(h, taskId, parentExecutionId, errorCode, errorMessage, agentId = "agent-1") {
...
test("E2E HA Failover: controller selects active candidate over draining", () => {
    ...
    try {
        ...
        const failedExecId = insertFailedExecutionForFailover(h, taskId, executionId, "LEADER_FAILURE", "Leader node failed", "agent-1");
...
test("E2E HA Failover: leader lease expiration detected", () => {
    ...
    try {
        ...
        const failedExecId = insertFailedExecutionForFailover(h, taskId, executionId, "LEASE_EXPIRED", "Leader lease expired", "agent-1");
        const failedExec = h.store.getExecution(failedExecId);
...
test("E2E HA Failover: heartbeat missing triggers failover decision", () => {
    ...
    try {
        ...
        const failedExecId = insertFailedExecutionForFailover(h, taskId, executionId, "HEARTBEAT_MISSING", "Leader heartbeat stopped", "agent-1");
        const failedExec = h.store.getExecution(failedExecId);
...
test("E2E HA Failover: node unhealthy triggers failover", () => {
    ...
    try {
        ...
        const failedExecId = insertFailedExecutionForFailover(h, taskId, executionId, "NODE_UNHEALTHY", "Leader node became unhealthy", "agent-1");
        const failedExec = h.store.getExecution(failedExecId);
...
test("E2E HA Failover: operator can force failover", () => {
    ...
    try {
        ...
        const failedExecId = insertFailedExecutionForFailover(h, taskId, executionId, "OPERATOR_FORCED", "Operator forced failover", "agent-1");
        const failedExec = h.store.getExecution(failedExecId);
...
test("E2E HA Failover: epoch preemption triggers failover", () => {
    ...
    try {
        ...
        const failedExecId = insertFailedExecutionForFailover(h, taskId, executionId, "EPOCH_PREEMPTED", "Higher epoch leader elected", "agent-1");
        const failedExec = h.store.getExecution(failedExecId);
...
//# sourceMappingURL=failover-flow.test.js.map