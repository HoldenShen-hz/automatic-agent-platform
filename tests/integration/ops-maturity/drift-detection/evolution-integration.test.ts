...
interface MockEvolutionProposal {
  id: string;
...
interface MockReflectionRecord {
  id: string;
  agentId: string;
  reflectionType: "performance" | "strategy" | "error";
  content: string;
  timestamp: string;
}

test("Evolution proposal creation", () => {
  const proposal: MockEvolutionProposal = {
    id: newId("evo"),
    targetVersion: "2.0.0",
    sourceVersion: "1.9.0",
    status: "draft",
...
test("Multiple benchmarks per proposal", () => {
  ...
  const results: MockBenchmarkResult[] = [];
...
test("Reflection record creation", () => {
  const record: MockReflectionRecord = {
    id: newId("refl"),
    agentId: newId("agent"),
...
test("Reflection types", () => {
  const types: MockReflectionRecord["reflectionType"][] = ["performance", "strategy", "error"];
