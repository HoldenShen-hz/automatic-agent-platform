...
function makeTraceFrame(overrides = {}) {
    return {
        workflowId: "wf_test",
        stepId: "step_001",
        status: "done",
...
test("integration: debugger registers and lists breakpoints", () => {
    ...
    service.registerBreakpoint({ actorId: "dev_1", allowedRuntime: "non_prod" }, "dev", {
        breakpointId: "bp_001",
...
test("integration: debugger rejects production breakpoint without permission", () => {
    const service = new WorkflowDebuggerService();
    assert.throws(() => service.registerBreakpoint({ actorId: "dev_1", allowedRuntime: "non_prod" }, "prod", {
        breakpointId: "bp_prod_001",
        workflowId: "wf_critical",
        stepSelector: "rollback",
        condition: "always",
        action: "compare",
    }), (err) => err.message.includes("prod_breakpoint_forbidden"), "Should reject production breakpoint without permission");
});
test("integration: debugger evaluates trace and identifies breakpoint hits", () => {
    const service = new WorkflowDebuggerService();
    service.registerBreakpoint({ actorId: "sre_1", allowedRuntime: "replay_sandbox" }, "prod", {
...
test("integration: empty trace returns empty results", () => {
    const service = new WorkflowDebuggerService();
    service.registerBreakpoint({ actorId: "tester", allowedRuntime: "non_prod" }, "dev", {
...
//# sourceMappingURL=workflow-debugger-integration.test.js.map