...
const CLI_PATH = `${REPO_ROOT}/dist/src/cli/acceptance-readiness.js`;
...
function writeEvidenceState(evidenceRoot: string): void {
...
test("acceptance-readiness CLI summarizes and exports the current remaining mainline work", () => {
  ...
  try {
    ...
    ) as { overallStatus: string; currentFocusItemId: string; stableEvidence: { status: string } };
    ...
    ) as { jsonArtifact: { uri: string }; report: { registryPublish: { status: string } } };
