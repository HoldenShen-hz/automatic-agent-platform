...
test("gateway targets CLI can upsert list and resolve canonical targets", () => {
  ...
  try {
    ...
    const created = runCli<{ targetId: string; displayName: string }>({
      AA_DB_PATH: dbPath,
    ...
    const listed = runCli<Array<{ targetId: string; displayName: string }>>({
      AA_DB_PATH: dbPath,
    ...
    const resolved = runCli<{ entry: { targetId: string; displayName: string }; matchedBy: string }>({
      AA_DB_PATH: dbPath,
