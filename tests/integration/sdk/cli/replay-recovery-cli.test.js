...
function runReplayRecoveryCli(env) {
    try {
        const stdout = execFileSync(process.execPath, [join(repoRoot, "dist", "src", "sdk", "cli", "replay-recovery.js")], {
            cwd: repoRoot,
            env: { ...process.env, ...env },
            encoding: "utf8",
        });
...
test("replay-recovery CLI shows help with --help flag", () => {
    ...
    assert.ok(result.stdout.includes("replay") || result.stderr.includes("replay"), "Help should mention replay");
});
...
//# sourceMappingURL=replay-recovery-cli.test.js.map