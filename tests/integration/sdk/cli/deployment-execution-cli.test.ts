...
const CLI_PATH = `${REPO_ROOT}/dist/src/sdk/cli/deployment-execution.js`;
...
function seedManagedDeploymentSecrets(
...
test("deployment-execution CLI exports masked deployment plan and can simulate execution", () => {
  ...
  try {
    ...
    ) as { executionMode: string; registrySecret: { resolved: boolean } };
    ...
    const executed = JSON.parse(
      ...
      report: {
        ...
        registrySecret: { accessMode: string; leaseId: string | null; leaseStatus: string | null };
        deploymentSecret: { accessMode: string; leaseId: string | null; leaseStatus: string | null };
    ...
    const persistedExecution = store.getDeploymentExecutionReportRecord(
      executed.report.executionId,
    );
...
test("deployment-execution CLI resolves vault-backed credentials without leaking plaintext", () => {
  ...
  try {
    ...
    const summary = JSON.parse(output) as {
      registrySecret: { providerKind: string; source: string; maskedValue: string | null; accessMode: string; leaseId: string | null };
      deploymentSecret: { providerKind: string; source: string; maskedValue: string | null; accessMode: string; leaseId: string | null };
    };
