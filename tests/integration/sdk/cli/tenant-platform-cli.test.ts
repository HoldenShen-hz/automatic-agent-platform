...
function runCli<T>(env: NodeJS.ProcessEnv): T {
  const stdout = execFileSync(process.execPath, [join(repoRoot, "dist", "src", "cli", "tenant-platform.js")], {
    cwd: repoRoot,
    env: {
      ...process.env,
      ...env,
...
test("tenant platform CLI creates a scoped topology and prints summary", () => {
  ...
  try {
    ...
    const topology = runCli<{
      counts: { organizations: number; workspaces: number; tenants: number; dataNamespaces: number };
      organizations: Array<{ organizationId: string; defaultTenantId: string | null }>;
