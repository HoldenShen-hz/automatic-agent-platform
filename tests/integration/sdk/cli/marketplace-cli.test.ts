...
test("marketplace CLI supports package registration, review, publish, revoke, and export", () => {
  ...
  try {
    ...
    const pkg = runCli<{ packageId: string }>({
      AA_DB_PATH: dbPath,
    ...
    const review = runCli<{ reviewId: string }>({
      AA_DB_PATH: dbPath,
    ...
    const approved = runCli<{ status: string }>({
      AA_DB_PATH: dbPath,
    ...
    const publication = runCli<{ publicationId: string; status: string }>({
      AA_DB_PATH: dbPath,
    ...
    const summary = runCli<{ report: { summary: { overallVerdict: string; packagesReady: number } } }>({
      AA_DB_PATH: dbPath,
    ...
    const revoked = runCli<{ status: string; revocationReasonCode: string | null }>({
      AA_DB_PATH: dbPath,
