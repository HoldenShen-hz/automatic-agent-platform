...
const CLI_PATH = `${REPO_ROOT}/dist/src/cli/environment-deployment.js`;
...
test("environment-deployment CLI summarizes and exports a target-ready environment matrix", () => {
  ...
  try {
    ...
    ) as { targetEligible: boolean; targetReleaseBundle: { environment: string } | null };

    assert.equal(summary.targetEligible, true);
    assert.equal(summary.targetReleaseBundle?.environment, "prod");
    assert.equal((summary.targetReleaseBundle as { registryCredentialRef?: string }).registryCredentialRef, "secret://system/registry/ghcr/prod");
    ...
    ) as { jsonArtifact: { uri: string }; report: { highestReadyEnvironment: string } };

    assert.match(exported.jsonArtifact.uri, /environment_deployment/);
    assert.equal(exported.report.highestReadyEnvironment, "prod");
    assert.equal(
      (exported.report as { entries?: Array<{ environment: string; secretInjection: { ready: boolean } }> }).entries?.find(
        (entry) => entry.environment === "prod",
      )?.secretInjection.ready,
      true,
    );
  } finally {
    db.close();
