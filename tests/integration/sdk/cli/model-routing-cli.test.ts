...
test("model routing CLI returns auditable conservative route decisions", () => {
  ...
  try {
    createFile(
      join(configRoot, "providers", "models.json"),
      JSON.stringify({
    ...
    const lowRisk = runCli<{ profileName: string; trace: { routeReason: string } }>({
    ...
    const highRisk = runCli<{ profileName: string; trace: { routeReason: string } }>({
      AA_CONFIG_ROOT: configRoot,
...
test("model routing CLI emits and reuses turn-scoped fallback leases", () => {
  ...
  try {
    ...
    const firstTurn = runCli<{
      profileName: string;
      trace: { routeReason: string; turnScopedFallbackIssued: boolean };
      fallbackLease: { turnId: string; primaryProfileName: string; fallbackProfileName: string } | null;
    }>({
      AA_CONFIG_ROOT: configRoot,
    ...
    const sameTurn = runCli<{
      profileName: string;
      trace: { routeReason: string; turnScopedFallbackActive: boolean };
      fallbackLease: { turnId: string; primaryProfileName: string; fallbackProfileName: string } | null;
    }>({
      AA_CONFIG_ROOT: configRoot,
      AA_MODEL_ROUTE_PREFERRED_PROFILE: "balanced",
