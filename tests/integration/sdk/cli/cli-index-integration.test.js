...
test("CLI_ENTRYPOINTS contains all expected operational commands", () => {
    ...
    for (const cmd of expectedCommands) {
        assert.ok(CLI_ENTRYPOINTS.includes(cmd), `Expected command "${cmd}" should be in CLI_ENTRYPOINTS`);
    }
});
test("CLI_ENTRYPOINTS contains all stability commands", () => {
    ...
    for (const cmd of stabilityCommands) {
        assert.ok(CLI_ENTRYPOINTS.includes(cmd), `Stability command "${cmd}" should be in CLI_ENTRYPOINTS`);
    }
});
test("CLI_ENTRYPOINTS contains all execution and dispatch commands", () => {
    ...
    for (const cmd of executionCommands) {
        assert.ok(CLI_ENTRYPOINTS.includes(cmd), `Execution command "${cmd}" should be in CLI_ENTRYPOINTS`);
    }
});
test("CLI_ENTRYPOINTS contains governance and ops commands", () => {
    ...
    for (const cmd of governanceCommands) {
        assert.ok(CLI_ENTRYPOINTS.includes(cmd), `Governance command "${cmd}" should be in CLI_ENTRYPOINTS`);
    }
});
test("CLI_ENTRYPOINTS contains data and memory commands", () => {
    ...
    for (const cmd of dataCommands) {
        assert.ok(CLI_ENTRYPOINTS.includes(cmd), `Data command "${cmd}" should be in CLI_ENTRYPOINTS`);
    }
});
test("CLI_ENTRYPOINTS contains marketplace and ecosystem commands", () => {
    ...
    for (const cmd of ecosystemCommands) {
        assert.ok(CLI_ENTRYPOINTS.includes(cmd), `Ecosystem command "${cmd}" should be in CLI_ENTRYPOINTS`);
    }
});
...
test("CLI_ENTRYPOINTS includes demo and testing commands", () => {
    ...
    for (const cmd of demoCommands) {
        assert.ok(CLI_ENTRYPOINTS.includes(cmd), `Demo command "${cmd}" should be in CLI_ENTRYPOINTS`);
    }
});
test("CLI_ENTRYPOINTS includes repair and maintenance commands", () => {
    ...
    for (const cmd of maintenanceCommands) {
        assert.ok(CLI_ENTRYPOINTS.includes(cmd), `Maintenance command "${cmd}" should be in CLI_ENTRYPOINTS`);
    }
});
...
//# sourceMappingURL=cli-index-integration.test.js.map