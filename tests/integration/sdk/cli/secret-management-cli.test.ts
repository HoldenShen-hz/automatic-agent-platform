...
test("secret-management CLI registers, resolves, rotates, and summarizes managed secrets", () => {
  ...
  try {
    ...
    ) as { secretRef: string; status: string };
    ...
    ) as { metadata: { resolved: boolean; maskedValue: string; auditId: string } };
    ...
    ) as { nextVersion: string | null };
    assert.equal(rotation.nextVersion, "v2");

    const leaseIssued = JSON.parse(
      execFileSync("node", ["--enable-source-maps", CLI_PATH], {
        cwd: REPO_ROOT,
        env: {
          ...process.env,
    ...
    ) as { metadata: { leaseId: string; leaseStatus: string }; lease: { leaseId: string } };
    ...
    ) as { leases: Array<{ leaseId: string; status: string }> };
    ...
    ) as { status: string; revocationReasonCode: string | null };
    ...
    ) as { registry: { currentVersion: string | null }; usageAudits: unknown[]; rotationEvents: unknown[]; leases: unknown[] };
...
test("secret-management CLI can issue provider-backed short-lived leases without local TTL fallback", () => {
  ...
  try {
    ...
    ) as { metadata: { leaseSource: string; providerLeaseId: string | null; renewable: boolean } };
...
test("secret-management CLI refreshes provider metadata and requests due rotations", () => {
  ...
  try {
    ...
    ) as { metadata: { providerKind: string; auditId: string | null } };
    ...
    ) as { events: Array<{ status: string; reasonCode: string }> };
