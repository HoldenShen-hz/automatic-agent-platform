...
test("integration: domains mainline turns all 24 baselines into active smoke-passing rollout-ready domain capabilities", async () => {
    ...
    try {
        ...
        assert.equal(bootstrapped.baselines.every((baseline) => bootstrapped.domainRegistry.validate(baseline.domainId).passed), true);
        assert.equal(bootstrapped.baselines.every((baseline) => bootstrapped.domainRegistry.buildCapabilityEntry(baseline.domainId).toolNames.length >= 1), true);
        assert.equal(bootstrapped.baselines.every((baseline) => baseline.governancePolicy.rollout.strategy.length > 0), true);
        assert.equal(bootstrapped.baselines.every((baseline) => baseline.metaModelValidation.completeness === 100), true);
        assert.equal(bootstrapped.baselines.every((baseline) => existsSync(baseline.ownershipProfile.configPath)), true);
        assert.equal(bootstrapped.baselines.every((baseline) => baseline.workflowSpecialization.stageNames.length >= 4), true);
        ...
        assert.equal(architectureRecords.every((record) => record.architectureSections.length === 8 && record.workflow.stageNames.length >= 4), true);
        ...
        for (const phase of ["domain_modeling", "pack_development", "security_certification", "gray_rollout"]) {
...
//# sourceMappingURL=domains-mainline-integration.test.js.map