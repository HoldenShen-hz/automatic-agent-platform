...
import type { DomainMetaModel, MetaModelAnswer, MetaModelQuestionId } from "../../../../src/domains/canonical-meta-model/types.js";
import { META_MODEL_QUESTION_IDS } from "../../../../src/domains/canonical-meta-model/types.js";

function createCompleteAnswer(questionId: MetaModelQuestionId): MetaModelAnswer {
  return {
    questionId,
...
test("integration: MetaModelValidator detects incomplete answers", () => {
  ...
  const answers: MetaModelAnswer[] = META_MODEL_QUESTION_IDS.map((qId, index) => {
    if (index === 3) {
      return {
        questionId: qId,
        title: `Answer to ${qId}`,
...
test("integration: MetaModelValidator empty answer string is incomplete", () => {
  ...
  const answers: MetaModelAnswer[] = META_MODEL_QUESTION_IDS.map((qId) => {
    return {
      questionId: qId,
      title: `Answer to ${qId}`,
      answer: "   ",
