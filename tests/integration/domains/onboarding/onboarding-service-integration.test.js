...
function registerMinimalDomain(registry, domainId, status = "testing") {
    registry.register({
        domainId,
...
test("DomainOnboardingService: advance requires evidence", () => {
    ...
    try {
        ...
        assert.throws(() => onboarding.advance("test_evidence", []), (error) => error.code === "domain_onboarding.evidence_required");
    }
    finally {
        ctx.cleanup();
    }
...
test("DomainOnboardingService: start fails for unknown domain", () => {
    ...
    try {
        const registry = new DomainRegistryService();
        const onboarding = new DomainOnboardingService(registry);
        assert.throws(() => onboarding.start("nonexistent_domain"), (error) => error.code === "domain_onboarding.domain_not_found");
    }
    finally {
        ctx.cleanup();
    }
...
//# sourceMappingURL=onboarding-service-integration.test.js.map