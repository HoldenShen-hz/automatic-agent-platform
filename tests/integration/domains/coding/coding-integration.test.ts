...
import {
  CODING_DOMAIN_PRESET,
  requiresCodingReview,
  type CodingTaskType,
} from "../../../../src/domains/coding/index.js";
...
test("integration: requiresCodingReview returns true for implement and release", () => {
  const reviewRequired: CodingTaskType[] = ["implement", "release"];
...
test("integration: requiresCodingReview returns false for other task types", () => {
  const noReviewRequired: CodingTaskType[] = ["analyze", "plan", "test", "review"];
...
test("integration: All coding task types are valid", () => {
  const validTypes: CodingTaskType[] = ["analyze", "plan", "implement", "test", "review", "release"];

  for (const taskType of validTypes) {
    assert.ok(
      CODING_DOMAIN_PRESET.reviewRequiredTaskTypes.includes(taskType) ||
        !requiresCodingReview(taskType),
      `${taskType} is a valid task type`,
    );
  }
});
