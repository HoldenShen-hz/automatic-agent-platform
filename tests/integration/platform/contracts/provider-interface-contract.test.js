 ...
 */
function createTestRequest(model) {
    return {
        model,
        messages: [
...
test("contract: ChatMessage supports all valid role types", () => {
    ...
    for (const role of validRoles) {
        ...
        assert.ok(["system", "user", "assistant"].includes(message.role), `Role ${role} should be valid`);
    }
});
/**
...
test("contract: ChatProviderType accepts all expected values", () => {
    const validTypes = ["anthropic", "openai", "minimax"];
    for (const type of validTypes) {
        assert.ok(validTypes.includes(type), `Provider type ${type} should be valid`);
    }
});
/**
...
test("contract: request with tools has correct structure", () => {
    ...
    assert.ok(["auto", "none"].includes(requestWithTools.toolChoice), "toolChoice should be 'auto' or 'none'");
});
/**
...
//# sourceMappingURL=provider-interface-contract.test.js.map