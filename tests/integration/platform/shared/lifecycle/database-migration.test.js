...
test("lifecycle: Applied migrations are ordered by version", () => {
    ...
    try {
        ...
        for (let i = 1; i < applied.length; i++) {
            assert.ok(applied[i].version > applied[i - 1].version, `Migration versions should be in order: ${applied[i - 1].version} < ${applied[i].version}`);
        }
        db.close();
    }
    finally {
...
test("lifecycle: getSchemaStatus returns currentVersion matching highest applied migration", () => {
    ...
    try {
        ...
        if (applied.length > 0) {
            ...
            assert.strictEqual(status.currentVersion, maxAppliedVersion, "currentVersion should match highest applied migration");
        }
        db.close();
    }
    finally {
    ...
    try {
        ...
        db.transaction(() => {
            db.connection
                ...
                .run("checkpoint_task", null, "checkpoint_task", "general_ops", "Checkpoint test", "pending", "user", "normal", "{}", "{}", null, 0, 0, null, new Date().toISOString(), new Date().toISOString(), null);
        });
        // Manual checkpoint
...
//# sourceMappingURL=database-migration.test.js.map