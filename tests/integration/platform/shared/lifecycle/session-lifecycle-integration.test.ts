  ...
  try {
    ...
    db.transaction(() => {
      for (let i = 0; i < 5; i++) {
        db.connection
          .prepare(
            "INSERT INTO session_events (id, session_id, event_type, payload_json, created_at) VALUES (?, ?, ?, ?, ?)",
          )
          .run(
            newId("sess-event"),
            sessionId,
    ...
    const events = db.connection
      ...
      .all(sessionId) as Array<{ id: string; event_type: string; payload_json: string }>;
