...
test("service-registry-bootstrap: registers all core platform services", async () => {
    ...
    for (const serviceId of expectedServices) {
        assert.ok(registry.isInitialized(serviceId) || registry.topologicalSort().includes(serviceId), `Service ${serviceId} should be registered`);
    }
});
...
test("service-registry-bootstrap: topological sort respects dependency order", async () => {
    ...
    if (auditIndex >= 0 && policyIndex >= 0) {
        assert.ok(auditIndex < policyIndex, "network-egress-audit should come before network-egress-policy in topological sort");
    }
});
...
//# sourceMappingURL=service-registry-bootstrap-integration.test.js.map