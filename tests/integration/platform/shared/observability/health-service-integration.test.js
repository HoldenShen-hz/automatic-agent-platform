...
test("health-service: backpressure detection when queue is saturated", () => {
    ...
    try {
        ...
        assert.ok(["degraded", "overloaded", "ok"].includes(report.status), `Expected degraded/overloaded/ok but got ${report.status}`);
        db.close();
    }
    finally {
        cleanupPath(workspace);
...
test("health-service: health status transitions based on load", () => {
    ...
    try {
        ...
        assert.ok(["degraded", "overloaded"].includes(report2.status), `Expected degraded/overloaded but got ${report2.status}`);
        db.close();
    }
    finally {
        cleanupPath(workspace);
...
//# sourceMappingURL=health-service-integration.test.js.map