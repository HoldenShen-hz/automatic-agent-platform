...
test("inspect service exposes structured dispatch decision traces", () => {
    ...
    try {
        ...
        assert.ok(taskInspect.dispatchDecisions[0]?.evaluations.some((evaluation) => evaluation.workerId === "worker-observe-basic" && evaluation.rejectionReason === "missing_capabilities"));
...
test("inspect service query layer lists task workflow and decision summaries for operator triage", () => {
    ...
    try {
        ...
        db.connection.prepare(`UPDATE tasks SET status = ?, updated_at = ? WHERE id = ?`).run("awaiting_decision", "2026-04-05T12:00:00.000Z", "task-observe-query");
        db.connection.prepare(`UPDATE executions SET status = ?, updated_at = ? WHERE id = ?`).run("created", "2026-04-05T12:00:00.000Z", "exec-observe-query");
        ...
        assert.ok(decisionSummaries.some((summary) => summary.decisionType === "dispatch" && summary.selectedWorkerId === "worker-observe-query"));
        db.close();
    }
    finally {
        cleanupPath(workspace);
...
test("health service surfaces queue governance and stale worker findings", () => {
    ...
    try {
        ...
        db.connection.prepare(`UPDATE executions SET status = ?, updated_at = ? WHERE id = ?`).run("created", oldIso, "exec-health-findings");
...
//# sourceMappingURL=inspect-health.test.js.map