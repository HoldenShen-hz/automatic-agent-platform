...
class FakeEventBus {
  ...
  private throwError = new Error("publish error");
  ...
  public setThrowOnPublish(shouldThrow: boolean): void {
...
test.describe("OutboxPollerService integration tests", () => {
  ...
  let outboxPoller: OutboxPollerService;
  let taskId: string;
  let now: string;
  ...
  test("OutboxPoller skips entries exceeding max retry count", async () => {
    ...
    db.connection.exec(
      `UPDATE outbox SET retry_count = 5, last_attempt_at = '${now}' WHERE id = '${entry.id}'`,
    );
  ...
  test("OutboxPoller applies backoff for entries with retries", async () => {
    ...
    db.connection.exec(
      `UPDATE outbox SET retry_count = 1, last_attempt_at = '${now}' WHERE id = '${entry.id}'`,
    );

    outboxPoller = new OutboxPollerService(
      outboxService,
      { intervalMs: 10000, initialBackoffMs: 10000 }, // 10 second backoff
    );
