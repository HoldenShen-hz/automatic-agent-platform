...
test("AsyncTaskRepository updateTaskStatusCas uses optimistic locking", async () => {
    ...
    try {
        ...
        const affected1 = await harness.repo.updateTaskStatusCas("task-cas-001", "queued", "in_progress", "2026-04-23T11:00:00.000Z", null, null);
        assert.equal(affected1, 1);
        const affected2 = await harness.repo.updateTaskStatusCas("task-cas-001", "queued", "done", "2026-04-23T12:00:00.000Z", null, null);
...
test("AsyncTaskRepository countQueuedTasks counts pending and queued tasks", async () => {
    const harness = createHarness();
    try {
        const statuses = ["queued", "pending", "in_progress", "done", "queued"];
