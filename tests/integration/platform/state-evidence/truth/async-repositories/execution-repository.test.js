...
test.describe("AsyncExecutionRepository", () => {
    ...
    test("listExecutionsByStatuses filters by status list", async () => {
        ...
        const statuses = ["pending", "executing", "completed", "pending"];
    ...
    test("updateExecutionStatus updates status and timestamps", async () => {
        ...
        await harness.executionRepo.insertExecution(exec);
        const affected = await harness.executionRepo.updateExecutionStatus("exec-update-001", "executing", "2026-04-23T10:05:00.000Z", "2026-04-23T10:05:00.000Z", null, null);
    ...
    test("countActiveExecutions counts executing and prechecking", async () => {
        ...
        const statuses = ["executing", "prechecking", "pending", "completed", "executing"];
        for (let i = 0; i < statuses.length; i++) {
            const exec = createExecution({
                id: `exec-active-${i}`,
