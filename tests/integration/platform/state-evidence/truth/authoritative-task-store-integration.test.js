...
test("workflow state persists and updates correctly", () => {
    ...
    try {
        ...
        for (let step = 1; step <= 4; step++) {
            ctx.db.transaction(() => {
                ctx.store.updateWorkflowState(taskId, "running", step, JSON.stringify({ completedSteps: step }), now);
            });
        ...
        // Complete workflow
        ctx.db.transaction(() => {
            ctx.store.updateWorkflowState(taskId, "completed", 4, JSON.stringify({ completedSteps: 4, result: "success" }), now);
        });
        workflow = ctx.store.getWorkflowState(taskId);
...
test("concurrent workflow updates with CAS version check", () => {
    ...
    try {
        ...
        let affected = ctx.db.transaction(() => {
            return ctx.store.updateWorkflowStateCas(taskId, 1, // expected version
            "running", "running", 1, '{"step": 1}', now);
        });
        assert.equal(affected, 1);
        // Second update with wrong version should fail (version 1 is stale)
        affected = ctx.db.transaction(() => {
            return ctx.store.updateWorkflowStateCas(taskId, 1, // stale version
...
//# sourceMappingURL=authoritative-task-store-integration.test.js.map