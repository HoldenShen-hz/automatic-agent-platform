...
test("PostgreSQL async repositories cover execution authoritative view reads", async () => {
    if (!pgSupport.enabled) {
        assert.match(pgSupport.reason ?? "", /(postgres runtime dependency is not installed|PostgreSQL test connection is not configured)/);
        return;
    }
    ...
    try {
        ...
        assert.deepEqual((await repos.event.getRequiredConsumerIds(event.id)).sort(), ["inspect_projection", "task_projection"]);
