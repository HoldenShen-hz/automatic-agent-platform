...
test("integration: artifacts can be persisted and retrieved by task", () => {
  ...
  try {
    ...
    const artifact = ctx.db.connection
      ...
      .get(artifactId) as { artifact_id: string; task_id: string; size_bytes: number } | undefined;
...
test("integration: multiple artifacts can be queried by execution", () => {
  ...
  try {
    ...
    const artifacts = ctx.db.connection
      ...
      .all(executionId) as Array<{ artifact_id: string }>;
...
test("integration: checkpoint envelope wraps and unwraps data correctly", async () => {
  ...
  try {
    ...
    const unpacked = await unpackCheckpointEnvelope<{ stepId: string; status: string; outputs: { result: string }; iteration: number }>(envelope);
...
test("integration: checkpoint envelope detects corruption via checksum", async () => {
  ...
  try {
    ...
    await assert.rejects(
      ...
      (err: Error) => err.message.includes("Checksum verification failed"),
      "Should reject corrupted envelope",
    );
  } finally {
    cleanupPath(workspace);
  }
