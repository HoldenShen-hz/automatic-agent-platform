...
test("consolidation integration: fails when scopes have mixed scope values", () => {
  ...
  try {
    ...
    memory.remember({
      taskId: "task-mixed-scope",
      scope: "project",
      content: { note: "project memory three" },
      classification: "internal",
      createdAt: "2026-04-10T08:10:00.000Z",
    });

    // This should throw due to single-scope requirement
    assert.throws(() => {
      memory.consolidate({
        taskId: "task-mixed-scope",
