...
test("decay integration: generateDecaySummary shows decayed memories after time passes", () => {
  ...
  try {
    ...
    // Evaluate at 20 minutes later - working memory half-life is 5 minutes
    // After 20 minutes (4 half-lives), freshness should be ~0.0625 which is below minFreshness 0.1
    const summary = decayService.generateDecaySummary(memories, "2026-04-10T08:20:00.000Z");

    assert.equal(summary.totalMemories, 2);
    assert.ok(summary.decayedMemories > 0, "Working memories should be decayed after 20 minutes");
...
test("decay integration: evaluateCompressionCandidates with freshness below minimum", () => {
  ...
  try {
    ...
    // Evaluate at 20 minutes later - freshness should be below minFreshness (0.1)
    const result = decayService.evaluateCompressionCandidates(memories, "2026-04-10T08:20:00.000Z");
