...
test("unpackCheckpointEnvelope verifies checksum integrity", async () => {
    ...
    await assert.rejects(() => unpackCheckpointEnvelope(envelope), CheckpointEnvelopeInvalidError);
});
...
test("createCheckpointEnvelope rejects data exceeding size limit", async () => {
    ...
    await assert.rejects(() => createCheckpointEnvelope(largeData, "test.v1"), CheckpointSizeExceededError);
});
...
test("unpackCheckpointEnvelope rejects invalid envelope structure", async () => {
    ...
    await assert.rejects(() => unpackCheckpointEnvelope(invalidEnvelope), CheckpointEnvelopeInvalidError);
});
test("createCheckpointEnvelope with custom maxSizeBytes", async () => {
    const smallData = { content: "test" };
    const customMaxSize = 5;
    await assert.rejects(() => createCheckpointEnvelope(smallData, "test.v1", { maxSizeBytes: customMaxSize }), CheckpointSizeExceededError);
});
...
//# sourceMappingURL=checkpoint-envelope-integration.test.js.map