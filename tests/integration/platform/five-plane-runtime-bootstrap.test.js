...
import { buildFivePlaneRuntimeCatalog, FIVE_PLANE_RUNTIME_CATALOG_SERVICE_ID, X1_FABRIC_BOOTSTRAP_SERVICE_ID, registerFivePlaneRuntimeCatalog, } from "../../../src/platform/five-plane-runtime-bootstrap.js";
...
test("integration: each plane catalog has unique capability IDs", () => {
    const catalog = buildFivePlaneRuntimeCatalog();
    const checkUniqueness = (planeCapabilities, planeName) => {
...
test("integration: total capability count across all planes", () => {
    const catalog = buildFivePlaneRuntimeCatalog();
    const total = catalog.interfacePlane.length +
