...
function distModuleHref(relativePathFromTest: string): string {
...
test("multi-step runtime routes postgres dual-run sync access through shadow sqlite", () => {
  ...
  try {
    createFile(shadowPath, "");
    const source = `
      import { runMultiStepOrchestration } from ${JSON.stringify(distModuleHref("../../../../src/platform/five-plane-execution/execution-engine/multi-step-orchestration.ts"))};
      void (async () => {
...
test("authoritative storage admin uses the async postgres path and surfaces driver installation requirements", () => {
  ...
  try {
    ...
    assert.ok(
      /storage\.postgres_driver_not_installed|postgres\.connection_failed/.test(result.stderr),
      `Expected fail-close error, got: ${result.stderr}`,
    );
  } finally {
    cleanupPath(workspace);
  }
