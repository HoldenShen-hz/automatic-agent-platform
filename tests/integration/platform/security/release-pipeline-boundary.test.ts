...
import { ReleasePipelineService } from "../../../../src/platform/five-plane-control-plane/incident-control/release-pipeline-service.js";
...
class FailAfterBuildRunner {
  ...
  public async run(request: { step: "build_image" | "publish_workflow"; command: string; args: string[]; cwd: string }): Promise<{ step: "build_image" | "publish_workflow"; command: string; args: string[]; executed: boolean; exitCode: number; stdout: string; stderr: string; durationMs: number; }> {
...
function seedManagedReleaseSecrets(store: AuthoritativeTaskStore, environment: "staging"): void {
...
test("release pipeline fail-closes malformed registry credential refs in environment config", async () => {
  ...
  try {
    ...
    writeFileSync(
      join(configDir, "staging.json"),
      source.replace(
        '"registryCredentialRef": "secret://system/registry/ghcr/staging"',
        '"registryCredentialRef": "plaintext-staging-secret"',
      ),
    );
    const service = new ReleasePipelineService({
      repoRootDir: REPO_ROOT,
