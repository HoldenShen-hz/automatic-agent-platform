...
test("runStableBackupRestoreRehearsal passes sqlite_backup_restore_roundtrip scenario and produces artifacts", async () => {
  ...
  try {
    ...
    const playbook = JSON.parse(readFileSync(report.artifacts.playbookPath, "utf8")) as StableDisasterRecoveryPlaybook;
    ...
    const saved = JSON.parse(readFileSync(reportPath, "utf8")) as StableBackupRestoreRehearsalReport;
...
test("runStableBackupRestoreRehearsal backup and restore details contain valid counts", async () => {
  ...
  try {
    ...
    const details = roundtripScenario.details as {
      backup: { valid: boolean };
