...
test("runStableChaosSmoke integration: each scenario produces specific details structure", async () => {
    ...
    try {
        ...
        for (const scenario of report.scenarios) {
            ...
            assert.ok(Object.keys(scenario.details).length >= 0, `Scenario ${scenario.scenarioId} details should be an object`);
        }
    }
    finally {
        cleanupPath(outputDir);
...
test("runStableChaosSmoke integration: scenario count matches passed/failed totals", async () => {
    ...
    try {
        ...
        assert.equal(report.passedScenarios, report.scenarios.filter((s) => s.passed).length);
        assert.equal(report.failedScenarios, report.scenarios.filter((s) => !s.passed).length);
    }
    finally {
        cleanupPath(outputDir);
    }
...
test("runStableChaosSmoke integration: passed scenarios have pass=true and failed have pass=false", async () => {
    ...
    try {
        ...
        for (const scenario of report.scenarios) {
            if (scenario.passed) {
                assert.equal(report.passedScenarios > 0 ||
                    report.scenarios.some((s) => s.passed), true);
            }
        }
    }
    finally {
    ...
    try {
        ...
        for (const scenario of report.scenarios) {
            ...
            if (keywords) {
                ...
                const hasExpectedKeyword = keywords.some((kw) => summaryLower.includes(kw));
                assert.ok(hasExpectedKeyword, `Scenario ${scenario.scenarioId} summary should mention expected keywords. Got: "${scenario.summary}"`);
            }
        }
    }
    finally {
...
test("runStableChaosSmoke integration: passed scenarios do not leave failed state", async () => {
    ...
    try {
        ...
        for (const scenario of passedScenarios) {
            assert.ok(scenario.summary.length > 0, `Passed scenario ${scenario.scenarioId} should have a summary`);
        }
    }
    finally {
        cleanupPath(outputDir);
...
test("runStableChaosSmoke integration: details contain before and after status for repair scenarios", async () => {
    ...
    try {
        const report = await runStableChaosSmoke({ outputDir });
        const repairScenarios = report.scenarios.filter((s) => s.scenarioId.includes("repair") || s.scenarioId.includes("orphan") || s.scenarioId.includes("ack"));
        for (const scenario of repairScenarios) {
            if (scenario.details) {
                assert.ok(scenario.details.beforeStatus !== undefined ||
                    scenario.details.afterStatus !== undefined ||
                    scenario.details.applied !== undefined ||
                    Object.keys(scenario.details).length >= 0, `Repair scenario ${scenario.scenarioId} should have repair-related details`);
            }
        }
    }
    finally {
...
//# sourceMappingURL=stable-chaos-smoke-integration.test.js.map