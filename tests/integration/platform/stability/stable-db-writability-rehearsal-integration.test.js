...
test("runStableDbWritabilityRehearsal health_and_doctor_fail_close_when_db_is_not_writable scenario passes", async () => {
    ...
    try {
        const report = await runStableDbWritabilityRehearsal({ outputDir: workspace });
        const healthScenario = report.scenarios.find((s) => s.scenarioId === "health_and_doctor_fail_close_when_db_is_not_writable");
        ...
        const details = healthScenario.details;
...
test("runStableDbWritabilityRehearsal multi_step_admission_rejects_new_work_in_read_only_mode scenario passes", async () => {
    ...
    try {
        const report = await runStableDbWritabilityRehearsal({ outputDir: workspace });
        const admissionScenario = report.scenarios.find((s) => s.scenarioId === "multi_step_admission_rejects_new_work_in_read_only_mode");
        ...
        const details = admissionScenario.details;
...
test("runStableDbWritabilityRehearsal dispatch_blocks_claims_without_dropping_pending_ticket_in_read_only_mode scenario passes", async () => {
    ...
    try {
        const report = await runStableDbWritabilityRehearsal({ outputDir: workspace });
        const dispatchScenario = report.scenarios.find((s) => s.scenarioId === "dispatch_blocks_claims_without_dropping_pending_ticket_in_read_only_mode");
        ...
        const details = dispatchScenario.details;
...
//# sourceMappingURL=stable-db-writability-rehearsal-integration.test.js.map