...
interface MockDeploymentRecord {
  id: string;
  version: string;
  status: "planned" | "rolling" | "complete" | "rollback";
  startedAt: string;
...
test("Deployment record creation", () => {
  const deployment: MockDeploymentRecord = {
    id: newId("deploy"),
    version: "2.0.0",
    status: "planned",
    startedAt: nowIso(),
...
test("Multiple active deployments per service", () => {
  const deployments: MockDeploymentRecord[] = [
