...
import { SqliteQueueAdapter, QUEUE_JOBS_DDL } from "../../../../src/platform/five-plane-execution/queue/queue-adapter.js";
import { cleanupPath, createTempWorkspace } from "../../../helpers/fs.js";
import { runConcurrentInvariant } from "../../../helpers/concurrent-runner.js";

function createQueueContext(prefix: string) {
...
test("concurrent dequeue: no two workers get same job (invariant)", async () => {
  ...
  try {
    ...
    const result = await runConcurrentInvariant(
      async (_workerId: number) => {
        ...
        if (r) {
          ...
          r.ack();
          return payload.index as number;
        }
        return null;
      },
      { concurrency: 10 },
    ...
    const receivedJobs = result.values.filter((v) => v !== null) as number[];
...
test("concurrent ack: same job cannot be acked twice", async () => {
  ...
  try {
    ...
    const ackResult = await runConcurrentInvariant(
      async (_workerId: number) => {
        try {
          dequeueResult.ack();
          return "acked";
