...
test("stable runtime validator produces a repeatable baseline report", async () => {
  ...
  try {
    ...
    const persistedReport = JSON.parse(readFileSync(report.artifacts.reportPath, "utf8")) as {
      baselineComparison: { status: string; baselineCreated: boolean };
      artifacts: { reportPath: string; baselinePath: string; inventoryPath: string };
      caseSummaries: unknown[];
    };
    const persistedInventory = JSON.parse(readFileSync(report.artifacts.inventoryPath, "utf8")) as {
      totalCases: number;
      missingRequiredClasses: string[];
    };
    const persistedBaseline = JSON.parse(readFileSync(report.artifacts.baselinePath, "utf8")) as {
      totalRuns: number;
      failedRuns: number;
      caseSummaries: unknown[];
    };
