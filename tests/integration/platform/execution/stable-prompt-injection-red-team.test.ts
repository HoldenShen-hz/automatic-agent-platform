...
test("stable prompt injection red-team writes a reusable report artifact", async () => {
  ...
  try {
    ...
    const persisted = JSON.parse(readFileSync(reportPath, "utf8")) as {
      ...
      scenarios: Array<{ scenarioId: string; actualRisk: string }>;
    };
    assert.equal(persisted.failedScenarios, 0);
    assert.ok(
      persisted.scenarios.some(
        (scenario) => scenario.scenarioId === "system_prompt_dump_request" && scenario.actualRisk === "high",
      ),
    );
  } finally {
