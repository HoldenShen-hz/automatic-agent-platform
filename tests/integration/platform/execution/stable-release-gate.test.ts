...
function writeEvidenceReport(root: string, profile: string, summary: Record<string, unknown>): void {
  ...
  const acceptanceStatus = "partial" as const;
  writeFileSync(
...
test("stable release gate marks backup restore evidence partial when the disaster recovery playbook is missing", () => {
  ...
  try {
    ...
    const backupRestoreCriterion = report.optionalCriteria.find(
      (criterion) => criterion.criterionId === "backup_restore_tested",
    );
...
test("stable release gate keeps production promotion conditional when 24h/72h evidence exists but QA-64 acceptance is still partial", () => {
  ...
  try {
    ...
    const acceptanceCriterion = report.requiredCriteria.find(
      (criterion) => criterion.criterionId === "stable_acceptance_line",
    );
...
test("stable release gate approves tenant-gray promotion only when gray rollout evidence is attached", () => {
  ...
  try {
    ...
    const grayCriterion = report.requiredCriteria.find(
      (criterion) => criterion.criterionId === "tenant_gray_rollout_tested",
    );
