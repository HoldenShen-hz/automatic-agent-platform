...
test("lease: acquire creates audit record for granted lease", () => {
  ...
  try {
    ...
    assert.ok(
      audits.some((a) => a.eventType === "lease_granted" && a.workerId === "worker-audit"),
      "Audit should record lease_granted event",
    );
...
test("lease: renew extends lease expiration and updates heartbeat", () => {
  ...
  try {
    ...
    assert.ok(
      Date.parse(renewed.lease!.expiresAt) > Date.parse(originalExpiresAt),
...
test("lease: renew creates audit record for lease_renewed event", () => {
  ...
  try {
    ...
    assert.ok(
      audits.some((a) => a.eventType === "lease_renewed" && a.workerId === "worker-audit-renew"),
      "Audit should record lease_renewed event",
    );
...
test("lease: release creates audit record for lease_released event", () => {
  ...
  try {
    ...
    assert.ok(
      audits.some(
        (a) =>
          a.eventType === "lease_released"
          && a.workerId === "worker-audit-release"
          && a.reasonCode === "test_completion",
      ),
      "Audit should record lease_released event with reason code",
    );
...
test("lease: reclaimExpiredLeases finds and closes all expired leases", () => {
  ...
  try {
    ...
    assert.ok(
      audits.some((a) => a.eventType === "lease_reclaimed"),
      "Audit should record lease_reclaimed event",
    );
