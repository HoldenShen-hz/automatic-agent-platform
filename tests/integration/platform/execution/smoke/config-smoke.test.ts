...
test("smoke: default configuration loads without exceptions", async () => {
  ...
  assert.ok(
    ...
    "Config should have maxConcurrentTasks",
  );
  assert.ok(
    config.defaultTaskTimeoutMs !== undefined,
    "Config should have defaultTaskTimeoutMs",
  );
  assert.ok(
    config.defaultStepTimeoutMs !== undefined,
    "Config should have defaultStepTimeoutMs",
  );
