...
test("tool execution: can insert and retrieve execution records", () => {
  ...
  try {
    ...
    db.connection
      .prepare(
        `INSERT INTO tasks (id, parent_id, root_id, division_id, title, status, source, priority,
    ...
    db.connection
      .prepare(
        `INSERT INTO executions (id, task_id, workflow_id, parent_execution_id, agent_id, role_id,
      ...
      .run(executionId, taskId, "single_agent_minimal", null, "agent-1", "general_executor",
        "task_run", "executing", null, "tool-exec-trace", 1, 60000, 1.0, 0,
        "workspace_write", "[]", "[]", 0, "none", null, null, now, null, now, now);

    // Verify execution was inserted
    const execution = db.connection
      .prepare("SELECT * FROM executions WHERE id = ?")
      .get(executionId) as { id: string; task_id: string; status: string } | undefined;
...
test("tool execution: execution timeout is recorded correctly", () => {
  ...
  try {
    ...
    db.connection
      ...
      .run(executionId, taskId, "single_agent_minimal", null, "agent-1", "general_executor",
        "task_run", "executing", null, "timeout-trace", 1, 30000, 1.0, 0,
        "workspace_write", "[]", "[]", 0, "none", null, null, now, null, now, now);

    // Verify timeout value
    const execution = db.connection
      .prepare("SELECT timeout_ms FROM executions WHERE id = ?")
      .get(executionId) as { timeout_ms: number } | undefined;
...
test("tool execution: allowed tools and paths are enforced", () => {
  ...
  try {
    ...
    db.connection
      ...
      .run(executionId, taskId, "single_agent_minimal", null, "agent-1", "general_executor",
        "task_run", "executing", null, "allowed-trace", 1, 60000, 1.0, 0,
        "workspace_write", allowedTools, allowedPaths, 0, "none", null, null, now, null, now, now);

    // Verify allowed tools and paths
    const execution = db.connection
      .prepare("SELECT allowed_tools_json, allowed_paths_json FROM executions WHERE id = ?")
      .get(executionId) as { allowed_tools_json: string; allowed_paths_json: string } | undefined;
