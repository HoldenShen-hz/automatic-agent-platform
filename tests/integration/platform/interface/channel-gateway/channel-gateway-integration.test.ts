...
test("ChannelGatewayDeliveryService creates and tracks delivery messages", () => {
  ...
  try {
    ...
    const message = h.deliveryService.createDeliveryMessage(
      "webhook",
      target.targetId,
      { text: "Test message", metadata: { traceId: "trace-1" } },
    );
...
test("ChannelGatewayDeliveryService dead-letters exhausted messages", () => {
  ...
  try {
    ...
    const message = h.deliveryService.createDeliveryMessage(
      ...
      target.targetId,
      { text: "Will fail" },
      2, // maxRetries = 2 (allows 1 retry before dead-letter)
    );
    ...
    assert.ok(
      deadLetters[0]?.failureReason === "gateway.delivery_retries_exhausted"
