...
test("parseGatewaySendPayload rejects empty text field", () => {
  assert.throws(
    ...
    (err: unknown) => {
      ...
      return error.code.includes("api.invalid_gateway_payload") && error.message.includes("String must contain at least 1 character");
    }
  );
});
...
test("parseGatewaySendPayload rejects missing text field", () => {
  assert.throws(
    ...
    (err: unknown) => {
      const error = err as { code: string; message: string };
      return error.code.includes("api.invalid_gateway_payload") && error.code.includes("text");
    }
  );
});
...
test("parseGatewaySendPayload rejects payload with extra unknown fields (strict)", () => {
  assert.throws(
    ...
    (err: unknown) => {
      const error = err as { code: string; message: string };
      return error.code.includes("api.invalid_gateway_payload") && error.message.includes("Unrecognized key");
    }
  );
});
...
test("parseApprovalDecisionPayload rejects option_selected without selectedOptionId", () => {
  assert.throws(
    ...
    (err: unknown) => {
      const error = err as { code: string; message: string };
      return error.code.includes("api.invalid_decision_payload") && error.message.includes("selectedOptionId is required");
    }
  );
});
...
test("parseApprovalDecisionPayload rejects text_input without inputText", () => {
  assert.throws(
    ...
    (err: unknown) => {
      const error = err as { code: string; message: string };
      return error.code.includes("api.invalid_decision_payload") && error.message.includes("inputText is required");
    }
  );
});
...
test("parseApprovalDecisionPayload rejects payload with extra unknown fields", () => {
  assert.throws(
    ...
    (err: unknown) => {
      const error = err as { code: string; message: string };
      return error.code.includes("api.invalid_decision_payload") && error.message.includes("Unrecognized key");
    }
  );
});
...
test("parseBillingReconcilePayload rejects payload with extra unknown fields", () => {
  assert.throws(
    ...
    (err: unknown) => {
      const error = err as { code: string; message: string };
      return error.code.includes("api.invalid_billing_reconcile_payload") && error.message.includes("Unrecognized key");
    }
  );
});
...
test("parseCreateTaskPayload rejects payload with extra unknown fields", () => {
  assert.throws(
    ...
    (err: unknown) => {
      const error = err as { code: string; message: string };
      return error.code.includes("api.invalid_create_task_payload") && error.message.includes("Unrecognized key");
    }
  );
});
...
test("parseUpdateTaskPayload rejects payload with extra unknown fields", () => {
  assert.throws(
    ...
    (err: unknown) => {
      const error = err as { code: string; message: string };
      return error.code.includes("api.invalid_update_task_payload") && error.message.includes("Unrecognized key");
    }
  );
});
...
test("parseCreateWebhookEndpointPayload rejects payload with extra unknown fields", () => {
  assert.throws(
    ...
    (err: unknown) => {
      const error = err as { code: string; message: string };
      return error.code.includes("api.invalid_webhook_endpoint_payload") && error.message.includes("Unrecognized key");
    }
  );
});
...
test("parseAuthTokenPayload rejects empty string apiKey", () => {
  assert.throws(
    ...
    (err: unknown) => {
      const error = err as { code: string; message: string };
      return error.code.includes("api.invalid_auth_payload") || error.code === "api.invalid_api_key";
    }
  );
});
