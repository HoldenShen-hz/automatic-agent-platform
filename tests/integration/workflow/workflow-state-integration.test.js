...
function createWorkflowStateHarness(workspacePrefix) {
...
test("workflow state: step progression updates currentStepIndex", () => {
    ...
    try {
        ...
        h.db.transaction(() => {
            h.store.workflow.updateWorkflowState(h.taskId, "running", 1, JSON.stringify({ step0: "completed" }), now, null);
        });
        ...
        h.db.transaction(() => {
            h.store.workflow.updateWorkflowState(h.taskId, "running", 2, JSON.stringify({ step0: "completed", step1: "completed" }), now, null);
        });
...
test("workflow state: retry increments retry count but keeps running", () => {
    ...
    try {
        ...
        h.db.transaction(() => {
            h.store.workflow.updateWorkflowState(h.taskId, "running", 0, JSON.stringify({}), now, null);
        });
...
test("workflow state: outputs JSON accumulates step results", () => {
    ...
    try {
        ...
        h.db.transaction(() => {
            h.store.workflow.updateWorkflowState(h.taskId, "running", 3, JSON.stringify(outputs), now, null);
        });
        const state = h.store.getWorkflowState(h.taskId);
...
//# sourceMappingURL=workflow-state-integration.test.js.map