...
test("core/runtime index re-exports TransitionService", () => {
    assert.ok("TransitionService" in runtimeIndex || runtimeIndex.TransitionService != null, "Should re-export TransitionService");
});
test("core/runtime index re-exports StateTransitionMachine", () => {
    assert.ok("StateTransitionMachine" in runtimeIndex || runtimeIndex.StateTransitionMachine != null, "Should re-export StateTransitionMachine");
});
test("core/runtime index re-exports ExecutionLeaseService", () => {
    assert.ok("ExecutionLeaseService" in runtimeIndex || runtimeIndex.ExecutionLeaseService != null, "Should re-export ExecutionLeaseService");
});
test("core/runtime index re-exports WorkerRegistryService", () => {
    assert.ok("WorkerRegistryService" in runtimeIndex || runtimeIndex.WorkerRegistryService != null, "Should re-export WorkerRegistryService");
});
...
test("core/runtime index re-exports state-transition components", () => {
    assert.ok("TransitionService" in runtimeIndex || "StateTransitionMachine" in runtimeIndex, "Should re-export state transition components");
});
test("core/runtime index re-exports workflow-step-checkpoint", () => {
    assert.ok("WorkflowStepCheckpoint" in runtimeIndex || runtimeIndex.WorkflowStepCheckpoint != null, "Should re-export WorkflowStepCheckpoint");
});
