...
test("operations: PlatformOperatorService can be instantiated with mock dependencies", () => {
    const service = new PlatformOperatorService(createMockDb(), createMockStore());
    assert.ok(service);
});
test("operations: PlatformOperatorService buildReport returns complete report structure", () => {
    const mockStore = createMockStore();
    const service = new PlatformOperatorService(createMockDb(), mockStore);
    const input = {
...
test("operations: PlatformOperatorService counts execution tickets by status", () => {
    const mockStore = createMockStore();
    mockStore.worker.listWorkerSnapshots = () => [];
    mockStore.worker.listExecutionTicketsByStatuses = ((statuses) => {
        if (statuses.includes("pending"))
            return { length: 5 };
        if (statuses.includes("claimed"))
            return { length: 3 };
        if (statuses.includes("consumed"))
            return { length: 10 };
...
test("operations: PlatformOperatorService counts execution leases by status", () => {
    ...
    mockStore.worker.listWorkerSnapshots = () => [];
    mockStore.worker.listExecutionTicketsByStatuses = () => [];
    mockStore.worker.listExecutionLeasesByStatuses = ((statuses) => {
        ...
        if (statuses.includes("released"))
            return { length: 1 };
        if (statuses.includes("reclaimed"))
            return { length: 0 };
        if (statuses.includes("handed_over"))
            return { length: 0 };
        return { length: 0 };
    });
    mockStore.release.listEnvironmentReadinessRecords = () => [];
...
test("operations: PlatformOperatorService collects topology counts", () => {
    ...
    mockStore.worker.listExecutionLeasesByStatuses = () => [];
    mockStore.release.listEnvironmentReadinessRecords = () => [];
    mockStore.organization.listOrganizationRecords = () => [1, 2, 3];
    mockStore.organization.listWorkspaceRecords = () => [10, 11, 12, 13];
    mockStore.organization.listTenantRecords = () => [20, 21];
    mockStore.organization.listDeploymentBindings = () => [100, 101, 102];
    mockStore.organization.listDataNamespaces = () => [200, 201];
    const service = new PlatformOperatorService(createMockDb(), mockStore);
    const report = service.buildReport({
        environment: "production",
        evidenceRootDir: "/tmp/evidence",
...
test("operations: PlatformOperatorService lease count exceeding worker count is a promotion risk", () => {
    ...
    mockStore.worker.listExecutionLeasesByStatuses = ((statuses) => {
        if (statuses.includes("active"))
            return { length: 10 }; // More active leases than workers
        return { length: 0 };
    });
    mockStore.release.listEnvironmentReadinessRecords = () => [];
    mockStore.organization.listOrganizationRecords = () => [];
...
//# sourceMappingURL=operations-integration.test.js.map