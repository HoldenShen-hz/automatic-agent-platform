...
function createSandboxPolicy(workspace) {
    return {
...
test("command-executor blocks semicolon injection (;)", async () => {
    ...
    try {
        ...
        assert.ok(result.error?.code.includes("meta_syntax") || result.error?.code.includes("denied"), "Should have metacharacter denial code");
    }
    finally {
...
test("command-executor blocks inline code execution via -c flag", async () => {
    ...
    try {
        ...
        assert.ok(result.error?.code === "tool.inline_code_denied", "Should have inline code denial code");
    }
    finally {
        cleanupPath(workspace);
    }
...
test("command-executor blocks curl to shell pipe", async () => {
    ...
    try {
        ...
        assert.ok(result.error?.code === "tool.remote_script_pipe_denied", "Should have remote script pipe denial code");
    }
    finally {
        cleanupPath(workspace);
    }
...
//# sourceMappingURL=command-injection.test.js.map