...
test("command-executor blocks undefined in args array", async () => {
    ...
    try {
        ...
        const args = ["first", undefined, "last"];
        const request = {
            callId: newId("call"),
            taskId: newId("task"),
            traceId: newId("trace"),
...
test("command-executor blocks function in args array", async () => {
    ...
    const executor = new CommandExecutor();
    try {
        const args = ["safe", function malicious() { return "hacked"; }, "last"];
        const request = {
            callId: newId("call"),
            taskId: newId("task"),
            traceId: newId("trace"),
...
test("command-executor blocks non-numeric timeout", async () => {
    ...
    try {
        const request = {
            ...
            args: ["test"],
            cwd: workspace,
            timeoutMs: "five seconds", // Non-numeric string instead of number
            sandboxPolicy: createSandboxPolicy(workspace),
            allowedTools: ["echo"],
            allowedPathRoots: [workspace],
        };
...
test("command-executor blocks invalid sandbox mode", async () => {
    ...
    try {
        const request = {
            ...
            sandboxPolicy: {
                policyId: "test",
                mode: "invalid_mode", // Invalid sandbox mode
                allowedRoots: [workspace],
                deniedRoots: [],
                realpathEnforced: true,
                symlinkPolicy: "deny",
...
test("command-executor blocks numeric command name", async () => {
    ...
    try {
        const request = {
            ...
            executionId: null,
            toolName: "bash",
            command: "12345", // Numeric command name
            args: [],
            cwd: workspace,
            timeoutMs: 5000,
            sandboxPolicy: createSandboxPolicy(workspace),
...
//# sourceMappingURL=input-validation.test.js.map