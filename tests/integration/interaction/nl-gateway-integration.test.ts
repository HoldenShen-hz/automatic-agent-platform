...
test("integration: ConversationContextManager manages multi-turn conversation", () => {
  ...
  const updated = manager.addTurn(
    "tenant-1",
    "user-1",
    "帮我创建任务",
    { intentType: "task_create", confidence: 0.9, entities: [], domainHint: null, urgency: "normal" },
  );
...
test("integration: ConversationContextManager respects window size limit", () => {
  ...
  for (let i = 0; i < maxTurns + 5; i++) {
    manager.addTurn(
      ...
      "user-2",
      `Turn ${i + 1}`,
      { intentType: "task_query", confidence: 0.8, entities: [], domainHint: null, urgency: "low" },
    );
  }
...
test("integration: DisambiguationHandler generates clarification questions", () => {
  const handler = new DisambiguationHandler();
  const result = handler.generateClarification(
    "帮我处理",
    0.5,
    { intentType: "task_query", confidence: 0.5, entities: [], domainHint: null, urgency: "normal" },
    [],
  );
